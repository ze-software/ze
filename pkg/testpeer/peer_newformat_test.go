...
// TestPeerLoadExpectNewFormatInvalid verifies error on old format.
//
// VALIDATES: Old format lines produce errors.
// PREVENTS: Silent acceptance of deprecated format.
func TestPeerLoadExpectNewFormatInvalid(t *testing.T) {
	tests := []struct {
		name    string
		content string
		wantErr string
		...
		{
			name:    "old_raw_format",
			content: "1:raw:FFFFFFFF",
			wantErr: `use "expect=bgp:conn=`,
		},
		{
			name:    "old_option_bind",
			content: "option:bind:ipv6",
			wantErr: `use "option=bind:value=`,
		},
		{
			name:    "old_option_asn",
			content: "option:asn:65000",
			wantErr: `use "option=asn:value=`,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, os.WriteFile(ciFile, []byte(tt.content), 0o600))

			_, _, err := LoadExpectFile(ciFile)
