...
per-destination buffer today (`forward_rs.go:351-360` and `reactor_api_forward.go:526-535`
...
`ebgpSlotASN2`, plus release plumbing at `recent_cache.go:461,527`. **Accepted, documented
...
`!facts.rsClient` gate (`forward_rs.go:352`, `reactor_api_forward.go:527`) already does — so
...
(`config/peers.go:646+`) prepends loop-detection refs to `ImportFilters` unless already
present (`filterChainContains`, `:667`). `ValidateFilterNames` (`:188`) and
`canonicalizeFilterRefs` (`:202`) must accept the reserved prefix. `show`/config renders it
...
(`commit.go:444-464`, which prepends for EBGP) is the intended behavior and the
`update text`/`BuildPlugin` path (`update_build_plugin.go:58-69`, which does not) is the one
...
- **Phases 3-4 (prepend-relocation, AC-3/AC-4/AC-5/AC-8) — GATED.** This half retires
  `wireu.rewriteASPathPrepend` (verified on disk: sole definition at `aspath_rewrite.go:63`;
...
> `wireu.rewriteASPathPrepend` (`aspath_rewrite.go:63`), MUST NOT START until ALL FOUR of the
...
- RFC 7947 §2.2.2 — an RS MUST NOT modify AS_PATH for RS-client peers — the prepend auto-entry
  ...
  gate (`forward_rs.go:352`, `reactor_api_forward.go:527`) is preserved.
