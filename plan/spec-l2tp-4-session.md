...
Implement session state machines for both incoming calls (LNS and LAC sides)
and outgoing calls (LNS and LAC sides). Includes ICRQ/ICRP/ICCN for incoming,
OCRQ/OCRP/OCCN for outgoing, CDN for teardown.

Session management within tunnels: create, lookup by Session ID, destroy.
Proxy LCP and proxy authentication AVP handling. WEN (WAN-Error-Notify)
and SLI (Set-Link-Info) message handling. Session limits enforcement.
