...
| AC-17 | the Phase 3 tree | every old name answers `unknown command` and exit 1, except a retired ACTION of a live command (`verify lock`, `verify lint`, `repo tracked-build`, `repo tracked-le`), which that command's action table refuses with `no such action` naming the action and exit 2, and no work runs (amended 2026-09-25, main thread); `cmd/ze` has no `ze_chaos`, `ze_analyze` or `ze_perf` register file; `binarySuffixRoot` is gone; `cmd/ze-gok`, `cmd/ze-terminal-pty` and `cmd/ze-perf-run` are gone; nothing reads `ZE_PERF_BIN`; the tracked-build and lint matrices hold no deleted tag; no builder writes `ze-test`; no `ze.test.bin`, `ze.qemu.test.bin` or `ze.test.no.build` entry is registered |
