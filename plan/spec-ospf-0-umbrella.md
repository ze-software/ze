...
### Delivered as a follow-up extension (originally deferred; now in scope)

Virtual links were deferred by the first base set (a guide §10 decision to add
them once the backbone was solid) and were subsequently delivered on the shared
AF-neutral engine, so they are now in scope. The base umbrella records the delivery here; the detailed feature
tracking lives in the extension umbrella (`plan/spec-ospf-ext-0-umbrella.md`,
row ext-7). Because virtual links attach to the shared ISM/NSM/SPF/interface
model, one implementation serves BOTH address families (RFC 2328 §15 for IPv4,
RFC 5340 §4.2 for IPv6).

| Area | Delivered as | Evidence |
|------|-------------|----------|
| Virtual links: virtual-link adjacencies across a transit area to repair a partitioned or non-contiguous backbone, both address families. A virtual link is a synthetic backbone (Area 0) point-to-point interface whose packets are ROUTED (unicast, TTL / hop-limit > 1) across the transit area to the remote ABR, with cost and next-hop computed from the transit-area intra-area SPF | `ext-7` on the shared engine (`plan/spec-ospf-ext-0-umbrella.md`) | Engine `internal/plugins/ospf/virtual_link.go`, `virtuallink_v6.go`, `spf/transitarea.go`; closure `plan/learned/1043-ospf-ext-7-virtual-links.md`; functional `test/ospf/ospf-virtual-link-config.ci`; interop `test/interop/scenarios/ospf-virtual-link-frr/` and `ospfv3-vlink-frr/` |
...
| R-9 | Scope creep into opaque/TE/SR | spec churn | Hard out-of-scope table above; no opaque consumers in v1 |
...
- Opaque/TE/SR/GR/BFD interop deferred with the corresponding out-of-scope features; virtual-link interop is delivered via ext-7 (test/interop/scenarios/ospf-virtual-link-frr/, ospfv3-vlink-frr/)
...
- Opaque-LSA framework, TE, SR, GR, BFD, demand circuits, multi-instance, L3VPN DN bit, SNMP MIB are out of scope (future)
- Virtual links (RFC 2328 §15 / RFC 5340 §4.2), originally deferred by this base set, were delivered as a follow-up extension (ext-7) on the shared engine for both address families; see `plan/learned/1043-ospf-ext-7-virtual-links.md` and `internal/plugins/ospf/virtual_link.go`
