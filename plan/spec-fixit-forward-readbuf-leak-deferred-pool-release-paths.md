...
**Item 1: outgoing peer-pool MOD buffer, body-build failure path.** The 2026-07-21
record says this. `forwardUpdateCore` gets a mod buffer, then `continue`s when
`buildFwdBody` fails, and drops the item without `Return`. `forwardRSUpdate`
(`forward_rs.go`) has the same shape.

**That record is a hypothesis, and it is this spec's FIRST task to re-confirm it**
(`ai/rules/no-fabrication.md`). The `continue` on `!ok` is still in both functions.
But the code around it was rewritten since. The fan-out dedup landed
(`plan/learned/1321-wire-edit-5-fanout-dedup.md`) and brought `fwdBodyCache`,
per-destination groups, and `adoptFwdHandle`. Read the two functions and answer two
...
calls `fp.releaseItem(&item)`, which is the function that returns the item's pool
resources and which the two drain paths in the same file do call. The doc comment
above it says done() is called "to prevent cache leaks", which is true and is not
the same as returning the buffers. Shutdown-window only: the pool is stopped, so
the process is on its way out.

-> Constraint: item 2 is a shutdown-window leak and its blast radius is a process
that is already stopping. It is not a reason to touch the pool's hot path. Bound the
fix to the stopped branch.
