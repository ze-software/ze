...
not re-bind the existing corpus. Measured on the working tree at that spec's
closure (`python3 scripts/dev/rfc_requirements.py --check`):

    evidence: unit/verify 2571, functional/verify 6, editor/verify 0, interop/nightly 0

So roughly 2571 gated MUST-level requirements are still proven only in-process,
by a test a peer never sees. `ai/rules/integration-completeness.md` states that a
unit test proves the algorithm while only a functional test proves the feature is
reachable; a wire obligation proven only by a Go table test is proven at the
wrong altitude, which is the thesis the parent spec exists to act on.
...
the remainder as tracked backlog. The remainder was first homed on
`plan/spec-rfcgate-0-umbrella.md`, which does not accept it: umbrella decision D4
says "This spec set is MACHINERY ONLY. The fleet drain is named as follow-on
work, not scoped here", and Constraint(D4) adds "no child may open the backlog".
A destination that disclaims the work is not a home
...
precision and poor recall, puts **at least 76%** of the 2720 gated MUSTs in the
wire-visible class, sample-implied 85-90%. That number sizes the problem and
nothing else. The parent spec's A-4 forbids deriving any pass/fail decision from
it, and the shipped implementation contains no trace of it (`grep -rin
'wire-visible\|classifier' scripts/dev/rfc_requirements.py` returns nothing).
**A gate built on the classifier would manufacture obligations for every
mis-classified requirement. Do not build one.**

**What is NOT in scope here.** Requirements that are genuinely internal, with no
wire form, keep unit-only evidence and are correct as they stand. The goal is not
"every requirement gets a `.ci`"; it is "every requirement whose obligation is
about bytes on a wire is proven by something that puts bytes on a wire."

**The work, in the order the parent spec's own ordering constraint implies:**

1. Derive the applicable set from requirement text plus the producing code, per
   RFC, and record it. Do not derive it from the classifier.
2. Bind in batches, `.ci` first (parent AC-18, umbrella D3): a `.ci` runs inside
   `ze-verify` on every push, interop is nightly and advisory.
3. Mutation-verify every binding, recording the RED (parent AC-17). A binding
   with no recorded RED does not land.
4. Let the per-tier ratchet (`check_evidence_ratchet`,
   `scripts/dev/rfc_requirements.py:1355`) hold each batch as the new floor.

Reader beware, from the parent spec's Known Limitations: a carrier's tier is an
ASSERTION in `CARRIERS`, not something re-derived from `stagesForMode`. Before
trusting a batch's tier, check the claim against the suite list a `ze-verify`
stage actually runs.
