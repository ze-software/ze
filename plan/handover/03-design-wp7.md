...
PRODUCES it, not a caller and not a comment (`ai/rules/no-fabrication.md`). Other agents
...
`engine/responder.go` and `wire/payload_ts.go` (`:708-709`). That estimate predates the
...
> (internal/component/ike/engine/responder.go:477-482) but never narrows them to the
> configured policy and never emits TS_UNACCEPTABLE; narrowTS
> (internal/component/ike/engine/child.go:351) is unused in production and
> NotifyTSUnacceptable (internal/component/ike/wire/payload_notify.go:22) is never sent}`
...
|-----------------|-------------|----------|
| `responder.go:477-482` | `responder.go:596-604` | `buildAuthResponse` |
| `child.go:351` | `child.go:392` | `narrowTS` |
| `payload_notify.go:22` | `payload_notify.go:45` | `NotifyTSUnacceptable` declaration |
...
- `narrowTS` is unused in production. `grep -rn 'narrowTS' internal/` returns exactly two
  files: the definition at `internal/component/ike/engine/child.go:392` and
  `child_test.go:373,383`. **It has no non-test caller.**
- `NotifyTSUnacceptable` is never sent. `grep -rn 'TSUnacceptable\|TS_UNACCEPTABLE'
  ...
  `internal/component/ike/wire/payload_notify.go:45`.
- The responder does record the initiator's TS and install XFRM from them:
  `buildAuthResponse` at `responder.go:598-603` sets `sa.NegotiatedTSi/TSr`, and
  `createFirstChildSA` at `child.go:174-183` copies them into `ChildSA.TSLocal/TSRemote`,
  which `installChildSA` puts into the policy at `child.go:312-321` and `:328-337`.
...
|------|----------|-------------|--------------|
| 1 | `buildAuthResponse` | `internal/component/ike/engine/responder.go:613` | `espSPI, saPayload, respTSi, respTSr, err := buildChildSAResponsePayloads(sa)` |
| 2 | `buildChildSAResponsePayloads` | `internal/component/ike/engine/initiator.go:447` | `tsi, tsr := anyChildTSPayloads(sa)`. It never reads `sa.NegotiatedTSi/TSr` |
| 3 | `anyChildTSPayloads` | `internal/component/ike/engine/rekey.go:63-69` | returns one `anyTrafficSelector` for each of TSi and TSr |
| 4 | `anyTrafficSelector` | `internal/component/ike/engine/initiator.go:451-470` | `StartAddress: []byte{0,0,0,0}`, `EndAddress: []byte{255,255,255,255}`, `IPProtocol: 0`, `StartPort: 0`, `EndPort: 65535` |
| 5 | `buildAuthResponse` | `internal/component/ike/engine/responder.go:649-650` | appends `respTSi`, `respTSr` to the inner payload chain |
...
3. **The wire and the dataplane disagree.** The response says ANY. The installed policy
   uses the initiator's proposed TS (`child.go:174-183`). Whenever the peer proposes
   anything narrower than `0.0.0.0/0`, Ze tells the peer one thing and programs another.
   The comment at `responder.go:596-597` asserts the opposite ("so the installed Child SA
   and the echoed TS agree"). **That comment is false in code**, and per
   `ai/rules/no-fabrication.md` a comment is its author's belief, not a decision record.
...
point (`:340`): a peer selects its own traffic selectors and Ze installs XFRM policy for
...
- `internal/component/ike/ipsec/yang/ze-ipsec-conf.yang` has **no** traffic-selector leaf.
  Under `vpn/ipsec/site-to-site/peer` (`:323`) the leaves are `name` `:327`,
...
- `SiteToSitePeer` (`internal/component/ike/ipsec/types.go:389-399`) carries
  `LocalAddress` and `RemoteAddress` (`:394-395`) and no selector field.
...
selector via `ipToFullNet` (`child.go:410-415`) as a /32 fallback when nothing was
negotiated (`child.go:169-170`).
...
`narrowTS` (`internal/component/ike/engine/child.go:392-403`) is:
...
(`internal/component/ike/wire/payload_ts.go:13-21`: `TSType`, `IPProtocol`, `StartPort`,
...
`child_test.go:370-372`, whose text claims the narrowed result reaches a Child SA install.
...
(`child_test.go:366-387`) carries an `RFC requirement:` tag, so removing it needs
...
The spec says WP-7 clears it (`:710`, `:744`), and AC-18 (`:417`) makes any surviving
annotation a blocker. Clearing means **deleting the `{gap: ...}` text from
`rfc/short/rfc7296.md:470`**, which is only honest once all four of these hold:
...
Critical Review Checklist calls the one thing the gate cannot catch (`:743`).

**`2.9-1` and `2.9-2` are different obligations and their tags must say so**, or they fall
into the trap the spec recorded for `1.4.1-4` (`:1816-1841`): two rows proving one
...
|----------|--------------------|-------------|
| The rekey REQUEST carries wildcard TS | `initiateChildRekey` | `internal/component/ike/engine/rekey.go:79` (calls `anyChildTSPayloads`) |
| The rekey RESPONSE carries wildcard TS | `respondChildRekey` | `internal/component/ike/engine/rekey.go:226` (same call) |
| The rekeyed child inherits the OLD selectors for the dataplane | `newRekeyedChild` | `internal/component/ike/engine/rekey.go:254-255` (`TSLocal: old.TSLocal`, `TSRemote: old.TSRemote`) |
...
`child.go:175`), so both halves are in scope, and the initiator half is where these three
MUSTs bind.
...
|----------|--------------------|-------------|---------|
| The mode constant exists | declaration | `internal/component/ike/engine/child.go:52` (`modeTunnel = 2`) | engine-local duplicate of `dataplane.ModeTunnel` |
| Every policy and state install hardcodes tunnel | `installChildSA` | `internal/component/ike/engine/child.go:257`, `:286`, `:317`, `:333` (`Mode: modeTunnel`) | four sites, no other value reachable |
...
| Property | Producing function | `file:line` |
|----------|--------------------|-------------|
| `ModeTransport uint8 = 1` is Ze's vocabulary, 1-based so an unset field is invalid | declaration | `internal/component/ike/dataplane/dataplane.go:40` |
| Kernel translation handles transport and fails closed on anything else | `kernelXFRMMode` | `internal/component/ike/dataplane/dataplane.go:68-77` |
| A transport-mode policy with tunnel endpoints is REJECTED | `tunnelEndpoints` | `internal/component/ike/dataplane/dataplane.go:98-106` |
| The mode reaches the kernel state | `InstallSA` | `internal/component/ike/dataplane/xfrm_linux.go:22-34`, `Mode: netlink.Mode(mode)` |
| The mode reaches the kernel policy template | `xfrmPolicyFromParams` | `internal/component/ike/dataplane/xfrm_linux.go:149`, `:170` |

`kernelXFRMMode`'s doc comment (`dataplane.go:51-67`) records that a wrong mode number is
...
(`child.go:312-321`, `:328-337`). `tunnelEndpoints` (`dataplane.go:98-106`) returns an
...
The final sentence (`:809-810`) is worth carrying into the tags: it is the RFC's own
...
`internal/component/ike/wire/payload_notify.go:57`. I grepped the whole IKE tree:

    grep -rn "NotifyUseTransportMode\|16391" internal/component/ike/
    internal/component/ike/wire/payload_notify.go:57:  NotifyUseTransportMode  uint16 = 16391
...
`internal/component/ike/engine/responder.go:394-401`) handles `NotifyInitialContact` and
...
records (`:1581-1583`), and here it applies for real, unlike in WP-5.
...
`anyTrafficSelector` (`internal/component/ike/engine/initiator.go:451-470`) is the only
...
(`internal/component/ike/engine/initiator.go:512-535`) reads `selectors[0]` only (`:516`)
...
`tsToIPNet` also returns nil for any range that is not CIDR-aligned (`:531-533`), and a nil
`NegotiatedTSi` falls back to `ipToFullNet` at `child.go:169-170`, silently substituting a
...
operator input, and operator input is YANG (`ai/rules/config-surface.md`: the default
...
Under `vpn/ipsec/site-to-site/peer` (`:323`), add:

| Element | Shape | Why |
|---------|-------|-----|
| `list tunnel { key "number"; ... }` | one entry per selector pair | RFC 7296 permits several selectors; a list is the only shape that carries them. `number` keyed as a string per `ai/rules/cli-grammar.md` "Identifiers Are Strings" |
| `container local { leaf prefix; leaf port; }` | `zt:ip-prefix` and a port | TSi when Ze initiates, TSr when it responds. Mirrors `child.go:171-177`'s role mapping |
...
| `leaf transport-required { type boolean; default false; }` | on the peer | `1.3.1-2`'s "if this is unacceptable to the initiator". Distinguishes prefer from require. Positive assertion per `ai/rules/config-naming.md` |

Use `zt:ip-prefix` rather than a raw string, per `ai/rules/yang-structure.md` Value Typing.
...
Mirror in `SiteToSitePeer` (`internal/component/ike/ipsec/types.go:389-399`): a
...
(`internal/component/ike/engine/config.go:134-153`) IS the plugin's `OnConfigVerify` body,
registered at `internal/component/ike/engine/register.go:254`. It already chains
...
and `ze config commit` reach, which is exactly what `ai/rules/exact-or-reject.md` requires.
...
distinct one (`ai/rules/file-modularity.md`). It needs `// Design:` and `// Related:`
headers pointing at `child.go` and `responder.go` (`ai/rules/design-doc-references.md`,
`ai/rules/related-refs.md`).

Delete `narrowTS` and `maskLen` from `child.go:392-408` first (`ai/rules/no-layering.md`).
Keep `ipToFullNet` (`child.go:410-415`); it is still the no-TS fallback.
...
protocol. `tsToIPNet` (`initiator.go:512-535`) stays only if some caller still needs the
...
|----------|-------------|--------|
| `buildAuthResponse` | `responder.go:596-604` | replace the unconditional record with a narrowing call against `sa.PeerCfg`'s selectors. On empty, return a sentinel the caller turns into TS_UNACCEPTABLE. Fix the false comment at `:596-597` |
| `buildChildSAResponsePayloads` | `initiator.go:434-449` | take the narrowed selectors as a parameter; delete the `anyChildTSPayloads(sa)` call at `:447` |
| `buildAuthResponse` | `responder.go:645-651` | unchanged in shape; it appends whatever `respTSi`/`respTSr` now carry |
| `startResponderEAP` | `responder_eap.go:116-127` | same narrowing; the EAP path stashes TS separately and would otherwise keep the old behaviour. **This is the second-producer trap the spec has recorded three times** (`:1778-1783`) |
| `handleAuthRequest` | `responder.go:394-401` | add a `NotifyUseTransportMode` case to the notify switch |
...
| Function | `file:line` | Change |
|----------|-------------|--------|
| `respondChildRekey` | `rekey.go:226` | narrow with `floor` set to the SA being rekeyed (`2.9.2-2`) |
| `initiateChildRekey` | `rekey.go:79` | propose the old SA's selectors, not a wildcard (`2.9.2-1`) |
| `newRekeyedChild` | `rekey.go:254-255` | unchanged; inheriting `old.TSLocal/TSRemote` is correct |

### 6.6 Transport mode -- `internal/component/ike/engine/child.go`

| Site | `file:line` | Change |
|------|-------------|--------|
| mode constant | `child.go:52` | add `modeTransport = 1`. Better: delete the engine-local duplicates and use `dataplane.ModeTransport`/`ModeTunnel` (`dataplane.go:40-41`) directly, removing a two-vocabulary hazard the `kernelXFRMMode` comment already warns about |
| `createFirstChildSA` | `child.go:185-198` | carry the negotiated mode on `ChildSA` |
| `installChildSA` state installs | `child.go:257`, `:286` | `Mode:` becomes the negotiated mode |
| `installChildSA` policy installs | `child.go:317`, `:333` | `Mode:` becomes the negotiated mode, **AND `TunnelSrc`/`TunnelDst` must be omitted in transport mode** (`child.go:312-321`, `:328-337`), or `tunnelEndpoints` (`dataplane.go:98-106`) rejects the install |

The initiator half of `1.3.1-2` lives in `handleAuthResponse`
(`internal/component/ike/engine/fsm.go:539`, TS branch at `:611-617`): when Ze proposed
...
`ai/rules/exact-or-reject.md` applies hard here. It has **two homes in this package**, and
...
|-----------------|---------------|--------------------|
| A CIDR prefix | yes | `XfrmPolicy.Src/Dst` are `*net.IPNet`; `selFromPolicy` writes `PrefixlenS/PrefixlenD` (`xfrm_policy_linux.go:107-114`) |
| A non-CIDR address range (`10.0.0.5-10.0.0.9`) | **NO** | the selector carries a prefix length, not a range. There is no field for it |
| All ports (0..65535) | yes | `Dport == 0` leaves `DportMask` 0, matching any port (`xfrm_policy_linux.go:118-125`) |
| A single port (`N..N`) | yes | `Dport = N` sets `DportMask = ^uint16(0)` (`xfrm_policy_linux.go:121-123`) |
| **Any other port range (`1024..2048`)** | **NO** | the kernel selector is port + MASK (`nl/xfrm_linux.go:192-195`: `Dport`, `DportMask`, `Sport`, `SportMask`), and `selFromPolicy` only ever writes mask 0 or mask 0xffff. An arbitrary inclusive range has no encoding |
| A protocol ID | yes | `SPParams.UpperProto` (`dataplane.go:217-220`) threads onto the selector |

So `ValidateTrafficSelectors` MUST reject, with the value and the valid alternatives in the
message (`ai/rules/error-messages.md`):
...
4. transport mode combined with a `vti bind` (`ze-ipsec-conf.yang:449`), since an XFRM
...
**`SPParams` has no port fields today** (`dataplane.go:209-227`: `Src`, `Dst`, `Dir`,
...
return nil (`initiator.go:531-533`) and `createFirstChildSA` substitutes a /32
(`child.go:169-170, 178-183`); a port range is discarded outright.
...
|------------|--------------------|-------------|
| The kernel mode number differs (0 vs 1) | `kernelXFRMMode` | `dataplane.go:68-77` |
| A transport-mode policy carries NO tunnel endpoints, and endpoints are rejected | `tunnelEndpoints` | `dataplane.go:98-106` |
| The mode is written into the policy TEMPLATE | `xfrmPolicyFromParams` | `xfrm_linux.go:149`, `:170` |
| The mode is written into the STATE | `InstallSA` | `xfrm_linux.go:22-34`, `:85` (`netlink.XfrmStateAdd`) |
...
`ai/rules/design-context.md`: "If the feature has a netlink implementation, add the VPP
...
`register_vpp.go:14`) contains **zero references to `Mode`**:
...
(`dataplane.go:60-65`), reproduced in the other backend.
...
Doing neither ships a backend that lies about the mode it programmed. `ai/rules/no-parking.md`
applies: this defect is now in scope because WP-7 is the entry point that reaches it.

Note `internal/plugins/ospf/ipsec_install.go:413` already passes
...
`ai/rules/qemu-testing.md`: linux-only code must ship with integration tests that run in
...
- `ZE_QEMU_INTEGRATION_PKGS` (`mk/test-integration.mk:523`) is **derived**: it greps for
...
automated (`ai/rules/qemu-testing.md`, "What actually RUNS these suites"). It must be run
...
| `test/ipsec/*.ci` | `functional/verify` | the `ipsec` suite IS in `all_suites` and HAS a `run_suite` line (`mk/test-functional.mk:192`, `:217`), so it earns the verify tier |

The interop scenarios `15-ts-narrowing` and `16-transport-mode` (`:504`, `:440`) are
...
| `2.23.1-1` | a transport-mode Child SA has exactly one address in every TSi and TSr entry | a transport-mode proposal carrying MULTIPLE selectors with the SAME single address is ACCEPTED (`:3715-3718`). Guards against rejecting multiple selectors |
...
**Reach BOTH responder producers.** `buildAuthResponse` (`responder.go:581`) and
`startResponderEAP` (`responder_eap.go:116-127`) both stash TS. A test covering only the
first leaves the EAP path unproven. This is the second-producer shape the spec has recorded
three times (`:1778-1783`), and this package has it again.
...
|---|----------|------|-------------|
| 1 | narrowing call replaced by `anyChildTSPayloads(sa)` (today's behaviour) | `buildAuthResponse`, `responder.go:613` | `2.9-2` positive AND `2.9-1` positive |
...
| 6 | narrowing skipped on the EAP path only | `startResponderEAP`, `responder_eap.go:116-127` | the EAP-fixture half. **If nothing reddens, the suite does not reach the second producer** |
| 7 | `Mode: modeTunnel` restored at one of the four sites | `child.go:257`, `:286`, `:317`, `:333` | the `1.3.1-1` positive, and the QEMU transport case. Run all FOUR separately; a test reaching only the policy pair leaves the state pair unproven |
| 8 | `TunnelSrc`/`TunnelDst` still supplied in transport mode | `child.go:317`, `:333` | the transport install fails via `tunnelEndpoints` (`dataplane.go:98-106`). Confirms the guard is exercised rather than bypassed |
| 9 | the response notify is omitted when transport is accepted | `buildAuthResponse` | `1.3.1-1` positive |
| 10 | `transport-required` ignored | `handleAuthResponse`, `fsm.go:539` | `1.3.1-2` positive |
| 11 | `StartPort: 0` becomes `1` | `anyTrafficSelector`, `initiator.go:465` (and its replacement) | `3.13.1-1` positive |
| 12 | `EndPort: 65535` becomes `65534` | `initiator.go:466` | `3.13.1-2` positive |
...
`check_id_allocation` (`scripts/dev/rfc_requirements.py:477-506`) refuses a new id whose
...
|-----------|-------------------|-----------|
| **Every `test/ipsec-interop/` scenario stays green** | R-2 (`:290`) names WP-7's TS narrowing as one of four packages that change what a peer sees. Scenarios 01-11 all negotiate Child SAs, and today they get a wildcard. Once Ze narrows, a scenario whose `ze.conf` has no selector config must still produce a working SA | `make ze-ipsec-interop-test` at the package boundary, not at the end. A red is this package's own defect and is fixed here (`ai/rules/no-parking.md`) |
...
| `RFC4301-4.4.2-1`'s existing coverage | `child_test.go:416-418` (`TestChildSAInboundPolicyUsesNegotiatedTS`) asserts the inbound policy carries the negotiated TS. Section 6 changes what "negotiated" means | The test sets `sa.NegotiatedTSi/TSr` directly, so it survives. Re-read it before editing; do not weaken it |
| `RFC4301-4.4.2-1`'s OTHER binding | `child_test.go:370-372` tags `TestNarrowTS`, which section 1.5 deletes | The obligation must be RE-BOUND to the new engine's test in the SAME commit, or `check_evidence_ratchet` fires. **Needs Thomas's `rfc-test-change-approved`** |
| `RFC4301-4.1-1/-3/-4` tunnel-mode assertions | `child_test.go:160-178` asserts every installed Child SA carries `Mode == modeTunnel`. Section 6.6 makes the mode variable | These tests must keep asserting tunnel for a TUNNEL-mode SA. If they sweep "every child", they need a mode-aware fixture. **`RFC4301-4.1-1`'s transport half is currently delegated to an OSPF test**; once IKE can do transport, re-point it |
| The dataplane fail-closed guards | `kernelXFRMMode` (`dataplane.go:68-77`) and `tunnelEndpoints` (`:98-106`) exist because a wrong mode is silent | WP-7 must not add a permissive default to either. Mutation 8 confirms the guard fires |
| `ipToFullNet` remains the no-TS fallback | `child.go:169-170` | Keep it when deleting `narrowTS`'s neighbours |
...
| R-WP7-3 | **The EAP responder path is left unnarrowed.** `startResponderEAP` (`responder_eap.go:116-127`) is a second producer | mutation 6 leaves the suite green | Fixtures for BOTH paths. This shape has now cost this spec three times (`:1778-1783`) |
| R-WP7-4 | **Transport mode installs fail with a confusing error** because `TunnelSrc`/`TunnelDst` were left in place | `tunnelEndpoints`' error text appears in logs | Section 6.6 names the exact sites (`child.go:317`, `:333`). Mutation 8 pins it |
...
| R-WP7-8 | **`3.13.1-3` is classified `{gap}` or `{not-applicable}`** because OPAQUE looks exotic | a `{gap}` appears in the diff | AC-18 (`:417`) makes any annotation a blocker. Section 5.2: WP-7's own port work creates the antecedent, so the row is implemented, not classified. If that is contested, it is an owner question, not a self-service call (`ai/rules/rfc-compliance.md`) |
| R-WP7-9 | **`2.9-1` and `2.9-2` prove the same observable** and neither tag establishes which case it is in | a reviewer cannot tell the two tags apart | Section 1.6: `2.9-1` is the empty case, `2.9-2` the non-empty one. The fixtures differ, and both tags must say how. This is the `1.4.1-4` trap (`:1816-1841`) |
...
The phase list names three files (`:708-709`). Sections 6.1 and 6.2 add a YANG surface, a
...
`SiteToSitePeer`, `types.go:389-399`, has no selector field). This is strictly MORE work,
so it needs no permission under `ai/rules/no-asking.md`. It is raised because it changes the
...
"Responder may narrow traffic selectors but never widen; if narrowed result is empty,
respond with TS_UNACCEPTABLE" and is classed `[MUST]` (`rfc/short/rfc7296.md:470`). The two
...
completely free", `scripts/dev/rfc_requirements.py:495-497`) and
...
`child_test.go:366-387` carries `RFC requirement: RFC4301-4.4.2-1 positive`
(`child_test.go:370-372`), whose text claims the narrowed result "a Child SA install then
writes into the inbound policy". That claim is false: `narrowTS`
(`internal/component/ike/engine/child.go:392`) has no non-test caller. Section 1.5 deletes
...
failure the netlink guard was written to stop (`dataplane.go:60-65`). It is reachable today
through `internal/plugins/ospf/ipsec_install.go:413`, independently of WP-7.
`ai/rules/design-context.md` says add the VPP implementation in the same work. Implementing
...
- `3.13.1-3` restores "working with IPSECARCH", which Appendix A drops (`:1226`). Without
  it the row binds every system; the RFC binds systems working with RFC 4301.

And `rfc/short/rfc7296.md:470` loses its `{gap: ...}` annotation, keeping id `RFC7296-2.9-1`
