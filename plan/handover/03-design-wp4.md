...
| `ai/RFC-REQUIREMENTS.md` | regenerated by `./le rfc index-update` | no |
...
`test/interop-ipsec/lab.py`. The make target is `./le integration interop-ipsec`, with
`IPSEC_INTEROP_SCENARIO=<name>` to select one (the retired `mk/test-integration.mk` (current producer: `internal/le/integration/gates.go`)).
...
`check_id_allocation` (the retired `scripts/dev/rfc_requirements.py` (current producer: `internal/le/rfc/rfc.go`), refusal at `:498-506`) refuses
...
package's ledger regeneration to a single `./le rfc index-update`.
...
| **All eleven `test/interop-ipsec/scenarios/`** | `responder-psk`, `responder-eap-mschapv2`, `responder-ike-rekey`, `responder-accepts-reinit` drive strongSwan as initiator against a Ze responder | strongSwan implements COOKIE, so these should pass with `cookie-threshold 1`. **Verify, do not assume**: run `./le integration interop-ipsec` before and after |
...
| R-WP4-7 | **A data race on the new `SA` fields.** `sa.Cookie` and `sa.SAInitRetries` are written on the dispatch goroutine and `runInitiator`'s goroutine reads `sa.RetransmitCount` beside them | `go test -race` on the engine package | The pattern is pre-existing (`handleSAInitResponse` already writes `sa.RetransmitTime` at `internal/component/ike/engine/fsm.go`), so WP-4 does not introduce it. **Run the engine package under `-race` with `-count=20`** and, if it fires, fix it rather than record it (`ai/rules/completion.md`). `go test -race ./internal/component/bgp/reactor/...` is BGP-only and will not cover this |
...
`./le rfc check` fail on a stale ledger (`ai/rules/testing.md`, RFC-Tagged Tests). Add the six
...
| Gate | Command | Why |
|------|---------|-----|
| Unit, engine package | `./le test-unit bgp` does not cover it; use `go test -race -count=20 ./internal/component/ike/...` | R-WP4-7. The `-count=20` is the reactor-style stress, applied to the package that actually changed |
| Functional | `./le functional` | the eight `test/ipsec/*.ci`, plus the new one |
| Interop, before AND after | `./le integration interop-ipsec` | the eleven existing scenarios, four of which are responder-side. Section 8 |
| One scenario while iterating | `./le integration interop-ipsec IPSEC_INTEROP_SCENARIO=invalid-ke-retry` | the retired `mk/test-integration.mk` (current producer: `internal/le/integration/gates.go`) |
| Ledger | `./le rfc index-update` then `./le rfc check` | section 11 |
| Full gate | `./le verify current mode full` | `ai/rules/git-safety.md` |
