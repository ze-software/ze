...
`RFC7296-2.6.1-1`.
Source spec: `plan/spec-rfcgate-1b-rfc7296-pilot.md`, phase list item 9 (`:685-689`).
...
work packages (`plan/spec-rfcgate-1b-rfc7296-pilot.md:1566-1581`). "WP-4" now names "Notify
...
Appendix A (`plan/spec-rfcgate-1b-rfc7296-pilot.md:1040`) quotes from "If the initiator
...
**One correction to the spec's own record.** `plan/spec-rfcgate-1b-rfc7296-pilot.md:1797`
...
(`plan/spec-rfcgate-1b-rfc7296-pilot.md:338`).
...
(`plan/spec-rfcgate-1b-rfc7296-pilot.md:1607-1609`). None of WP-4's rows is on it, and the
...
(`plan/spec-rfcgate-1b-rfc7296-pilot.md:688`), written before the rows were mapped to
...
| `12-invalid-ke-retry` | `test/ipsec-interop/scenarios/` | Ze as initiator configured `dh-group 14` first against a strongSwan proposing only `modp3072`, so strongSwan sends INVALID_KE_PAYLOAD and Ze must retry and establish. **AC-14** (`plan/spec-rfcgate-1b-rfc7296-pilot.md:413`) |
...
(`plan/spec-rfcgate-1b-rfc7296-pilot.md:1363-1387`, `:1399-1414`, `:1416-1438`). The
...
package's ledger regeneration to a single `make ze-rfc-index`.
...
`engine/responder.go` (`plan/spec-rfcgate-1b-rfc7296-pilot.md:688`). This design needs nine,
...
(`plan/spec-rfcgate-1b-rfc7296-pilot.md:1571`). The brief names 6. The four most likely
