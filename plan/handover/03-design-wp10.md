...
| **`ai/RFC-REQUIREMENTS.md` stays fresh** | Nine new tagged tests move line numbers throughout | Run `make ze-rfc-index-update` and commit the ledger in the SAME commit. `ze-rfc-check` fails on a stale ledger in both verify modes (`ai/rules/testing.md`, RFC-Tagged Tests) |
