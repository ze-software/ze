...
Source spec: `plan/spec-rfcgate-1b-rfc7296-pilot.md`, phase list item 14 (`:717-721`).
...
new count" (`plan/spec-rfcgate-1b-rfc7296-pilot.md:343`).
...
**Doctor check.** The spec requires one (`plan/spec-rfcgate-1b-rfc7296-pilot.md:564`): the
...
(`plan/spec-rfcgate-1b-rfc7296-pilot.md:343`).
...
`1.4-5` (`plan/spec-rfcgate-1b-rfc7296-pilot.md:1399-1414`).
...
| R-WP10-5 | **`4-4` is classified partial without an owner ruling, or is "fixed" by pointing at the unset-`remote-id` path.** Accepting via an absent check is acceptance by fail-open, and `ai/rules/rfc-compliance.md:53` voids prior answers that point away from full compliance | none mechanical | Section 12. Raise before landing. Section 6's negative half 2 pins the warning so the fail-open path stays visible |
