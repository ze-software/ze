...
Make birdwatcher routes_filtered real (currently always 0). Also a per-peer cumulative import-reject counter at the reactor reject gate (`reactor_notify.go`) as a diagnostic

### From `lg-birdwatcher-peer-fields.md`, 2026-07-15

Deferred by spec-lg-birdwatcher-peer-fields (Phase 4).

Populate the birdwatcher `routes_received` / `routes_imported` / `routes_exported` / `routes_filtered` fields. `state_changed` and `last_error` are IMPLEMENTED (see the spec Progress section); only the route counts remain
