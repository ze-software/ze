...
**What the operator is promised.** `internal/component/plugin/yang/ze-plugin-conf.yang`
declares `leaf respawn` under `plugin { external <name> { ... } }` with the
description "Respawn on exit". A reader takes that to mean the engine starts the
plugin process again after it exits, which is what the ExaBGP `respawn` option
it is named after does. `docs/architecture/system-architecture.md` publishes the
promise twice in worked configuration blocks, the first annotated
`respawn true;           # Restart if crashes`. The documentation therefore
teaches the knob as a crash-recovery control.

**What Ze does instead.** `ExtractPluginsFromTree`
(`internal/component/config/loader.go`) is the only producer that turns the
`plugin` container into `[]plugin.PluginConfig`. Over the `external` list it
reads `run`, `use`, `encoder` and `timeout`, and it never reads `respawn`. So
`PluginConfig.Respawn` and `PluginConfig.RespawnEnabled`
(`internal/component/plugin/types.go`) stay false for every configured plugin.
`cmd/ze/hub/main.go` copies `pc.Respawn` from one `PluginConfig` to the next,
which moves a value no producer ever set. `ProcessManager.Respawn`
(`internal/component/plugin/process/manager.go`) tests
`!cfg.RespawnEnabled && !cfg.Respawn` and returns `ErrRespawnNotEnabled`, so it
refuses every plugin in a running daemon. Its only non-test caller is
`(*Server).restartPlugin` (`internal/component/plugin/server/reload_tx.go`),
reached from `restartPluginFn` when a config-reload rollback ack reports
`CodeBroken`. No code watches a plugin process exit and calls `Respawn`, so a
crashed plugin is never restarted whatever the leaf holds. Two behaviors are
therefore dead: the leaf, and the restart the reload path attempts, which fails
with `respawn not enabled` on the one occasion it runs. The leaf's own `ze:help`
already says the daemon does not act on it, so the schema and the architecture
...
right, and the choice is wider than the leaf, because the reload restart path is
dead on the same evidence: a refusal leaves `restartPlugin` unable to ever
succeed, so the refusal alone does not make the tree honest. Building it means
parsing the leaf into `PluginConfig`, and deciding whether a process exit
watcher calls `Respawn` or whether the leaf only unblocks the reload restart.
That decision belongs to the design, not to this skeleton. Whichever is picked,
...
- `docs/architecture/system-architecture.md` - two configuration blocks publish `respawn true;`, one annotated as restart on crash. Both are wrong whichever way this spec closes.
