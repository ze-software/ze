...
**What Ze does instead.** `parseLevelInterface` (`internal/plugins/isis/config.go`)
stores the four values in `LevelInterfaceConfig.HelloInterval` and `.HoldMult`.
Neither field has a reader. The only `Level1` and `Level2` field reads in the
plugin are `.AuthKeyChain` (`internal/plugins/isis/auth_keystore.go`), `.Priority`
through `disPriority` (`internal/plugins/isis/dis_wiring.go`) and `.Metric`
through `levelMetric` (`internal/plugins/isis/lsdb_wiring.go`). The circuit runs
one hello timer for both levels: `launchCircuitGoroutine`
(`internal/plugins/isis/circuits.go`) builds a single ticker from the
circuit-wide `ic.HelloInterval`, and every tick calls `SendHello`
(`internal/plugins/isis/circuit/runtime.go`). On a LAN that call sends one IIH
per configured level through `sendLANHellos`, so both levels leave at the same
period. The holding time comes from the same place: `buildCircuit`
(`internal/plugins/isis/circuits.go`) passes `ic.HelloInterval` and `ic.HoldMult`
into `circuit.New`, which computes `HoldTime(cfg.HelloInterval, cfg.HoldMult)`
(`internal/plugins/isis/circuit/circuit.go`). The four leaves change nothing. The
`ze:help` on both containers now states this, in the sentence "`hello-interval`
and `hold-multiplier` are stored and not acted on". The four `description`
strings still say override.
...
obviously right, and the deciding fact is the circuit kind. A per-level hello
period is meaningful only on a broadcast circuit, where `sendLANHellos` already
builds one PDU per level. On a point-to-point circuit `SendHello` sends a single
IIH that serves both levels, so two periods have nothing to apply to and the
design must say what the leaves mean there. Building the LAN half means one
ticker per level instead of one per circuit, plus a per-level holding time
carried into `buildLANHello`. Refusing costs one validator and removes the false
promise, and it costs an operator nothing that works today. The design should
also read ISO/IEC 10589 clause 10.9 before choosing, because the committed
