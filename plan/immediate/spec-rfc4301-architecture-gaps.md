...
`plan/pre-release/spec-rfcgate-6-supported-extraction-signoff.md`, and this spec's last phase lands
...
- the retired deferral shard "rfc4301-architecture-gaps" - the shard named in the metadata table
...
9. **Phase: summary, annotations and the public row** -- the closing phase
   ...
   - Verify: every void annotation carries a fresh answer, every new obligation carries a new requirement id and a tagged test, the row reads `Supported`, and `./le rfc check` names `rfc4301` in no violation. The sign-off artifact lands under the rules of `plan/pre-release/spec-rfcgate-6-supported-extraction-signoff.md`
...
- **It does not write `rfc/extraction/rfc4301.json`.** That artifact belongs to `plan/pre-release/spec-rfcgate-6-supported-extraction-signoff.md`. Phase 9 lands it under that spec's rules and its checks.
