...
| `internal/component/ike/ipsec/yang/ze-ipsec-conf.yang`, `remote-access` | The `x509` leaves say "It has no effect today" | Unchanged by this spec; those leaves belong to `plan/immediate/spec-ipsec-remote-access.md` |
...
**Relationship to `plan/immediate/spec-ipsec-remote-access.md` (OPEN OWNER DECISION, OI-1).** That
...
`plan/immediate/spec-ipsec-remote-access.md`. Without admission, the address assignment specified here
...
- `plan/immediate/spec-ipsec-remote-access.md` - the rows OI-1 repoints, whichever way it is answered
...
- Road-warrior ADMISSION is not in this spec. Until
  `plan/immediate/spec-ipsec-remote-access.md` lands, a client must connect as a configured responder
...
- No lease lifetime independent of the IKE SA. A lease lives exactly as long as its SA.
  ...
  `plan/immediate/spec-ipsec-remote-access.md` AC-26.
...
rows are homed in `plan/immediate/spec-ipsec-remote-access.md` today; OI-1 decides which spec closes
them.
...
- [ ] OI-1 answered by Thomas: this spec or `plan/immediate/spec-ipsec-remote-access.md` owns the Configuration payload work
