...
**What Ze does instead.** `parseLDPConfig` (`internal/plugins/ldp/register.go`)
writes the value into `ldpConfig.KeepaliveTime`. That field has no reader. The
session is built by `startSessionForAdj` in the same file, which calls
`NewSession` (`internal/plugins/ldp/session.go`) with no keepalive argument.
`NewSession` sets `keepaliveTime: DefaultKeepaliveTime`, which is 60 seconds, and
...
message, so Ze proposes 60 seconds to every peer whatever the operator wrote.
...
cannot influence from Ze's side. An operator who sets `keepalive-time 15` to
detect a dead peer inside 45 seconds gets 60 seconds and a 180-second hold,
unless the peer happens to propose less. The `ze:help` beside the leaf states
this, in the sentence "Ze proposes 60 seconds in every Initialization message and
reads this leaf nowhere, so a change here moves no timer." The `description` does
not.
...
exist and are correct; only the configured value is missing from the path.
...
already takes eight parameters, so the design should decide whether the value
