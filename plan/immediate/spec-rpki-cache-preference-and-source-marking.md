# Spec: rpki-cache-preference-and-source-marking
...
**What the operator is promised.** `bgp/rpki/cache-server/preference`
(`internal/component/bgp/plugins/rpki/yang/ze-rpki.yang`) is a `uint8` with
default 100, and its description says "Preference of this cache server". The
list it belongs to says Ze reads the validated ROA and ASPA records from those
servers, and that "Each entry carries the port, the preference and the source
address". RFC 8210 Section 10 says what a preference on a cache list means:
"The client router attempts to establish a session with each potential serving
cache in preference order and then starts to load data from the most preferred
cache to which it can connect and authenticate." The same section says "the
lower the value, the more preferred". A reader takes the leaf for cache
selection and failover order. This spec covers that one leaf and the multi-cache
behavior behind it.

**What Ze does instead.** `(*rPKIPlugin).startSessions`
(`internal/component/bgp/plugins/rpki/rpki.go`) walks the configured cache
servers and starts one RTR session per entry at once, and every session writes
the same two caches the plugin holds. The value reaches `RTRSession.preference`
(`internal/component/bgp/plugins/rpki/rtr_session.go`) and the status snapshot
that `show bgp rpki status` renders, and no code sorts, selects or fails over on
it. The merge also drops the source of each record: `vrpEntry`
(`internal/component/bgp/plugins/rpki/roa_cache.go`) holds a MaxLength and an
ASN alone, so a withdraw from one cache removes a VRP a different cache
announced. RFC 8210 Section 10 states: "A client may hold data from multiple
caches but MUST keep the data marked as to source, as later updates MUST affect
the correct data." `rfc/short/rfc8210.md` already records both as gaps, at
`RFC8210-4-1` and `RFC8210-10-2`, and Section 8.4 adds a SHOULD that turns on
the same order, recorded at `RFC8210-8.4-2`. The leaf's own `ze:help` says the
value "changes no behavior today", which the description above it does not.

**What closing it means.** The implementer builds the behavior, or refuses the
leaf at commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses the `vrf` leaf. Refusing is
not available here, and this is the one leaf in this class where that is true.
The source-marking MUST binds Ze whatever the leaf does, because Ze already
holds records from several caches in one store, so removing the leaf leaves the
RFC gap open and closes nothing (`ai/rules/rfc-compliance.md`). The real choice
is the selection model, and the design phase states it. The first reading is
preference order, with one loaded cache and failover to the next on error, which
is Section 10's own words. The second is the concurrent load Ze performs today,
with each record marked by the cache that sent it. The second keeps Ze's current
freshness and still satisfies Section 10's MUST, so the two are not one answer
with two names. The spec picks against the RFC text, never against the smaller
diff. Both readings need the source marking, so that part is not a choice.
...
- Each RTR cache server sends ROA and ASPA PDUs over its own TCP session.
