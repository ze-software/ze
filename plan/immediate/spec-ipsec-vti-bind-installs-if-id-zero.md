# Spec: ipsec-vti-bind-installs-if-id-zero
...
`internal/component/ike/ipsec/yang/ze-ipsec-conf.yang` declares `container vti`
on a site-to-site peer, described as "Virtual Tunnel Interface binding", holding
`leaf bind`, described as "VTI interface name to bind this peer's traffic to." A
reader takes `peer P { vti { bind vti0 } }` to mean the traffic routed into the
XFRM interface named `vti0` is the traffic this peer encrypts. That is
route-based IPsec, and `plan/spec-ipsec-0-umbrella.md` names it as the target
deployment.

**What Ze does instead.** The name is stored and never resolved.
`parseSiteToSitePeer` (`internal/component/ike/ipsec/config.go`) reads
`vti/bind` into `SiteToSitePeer.VTIBind` (`internal/component/ike/ipsec/types.go`).
The only reader outside the config package is
`internal/component/ike/ipsec/traffic_selector.go`, which refuses a `bind` name
beside `mode transport`. That refusal is the container's whole current effect.
The field the dataplane actually binds on is a different one. `resolveIfID`
(`internal/component/ike/engine/established.go`) returns `SiteToSitePeer.IfID`,
and `runEstablished` in the same file and the responder path in
`internal/component/ike/engine/responder.go` pass that value into every Child SA.
No non-test code assigns `IfID`: the assignments in the tree are in
`internal/component/ike/engine/reconcile_test.go` and other tests. Nothing maps
the interface name the peer gives to the identifier the interface declares. That
identifier exists and is mandatory: `leaf if-id` on the `xfrm` interface list
(`internal/component/iface/yang/ze-iface-conf.yang`) is described as the "XFRM
identifier that binds security associations to this interface", it is
`mandatory true`, and its range starts at 1 because, in its own words, "The value
must be non-zero, because 0 means unset in the kernel." So every Child SA Ze
installs carries `if_id 0`, which the kernel reads as unset, and the interface
the operator named carries no traffic.

**What closing it means.** The choice is to build the resolution or to refuse
`vti bind` at commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses `vrf`. Building it is the
right answer, and the shape of the tree says why. The consumer already exists and
is already wired: `resolveIfID` reads `SiteToSitePeer.IfID`, the engine passes it
to the Child SA, and `show ipsec` already reports it
(`internal/component/ike/cmd/show_dataplane.go`). What is missing is one lookup
from the interface name to that interface's `if-id`, and a writer for a field
that has no non-test writer today. Refusing instead would remove route-based
IPsec, which is the deployment the umbrella spec was written for. The design has
to decide where the lookup runs, because the peer config and the interface config
are separate subtrees delivered to separate consumers, and it has to decide what
happens when the named interface does not exist or is not an `xfrm` interface. A
zero returned in either case is the failure `ai/rules/principles.md` names first,
because it is indistinguishable from a peer with no `vti` block at all.
`plan/spec-ipsec-vpp-policy-interface.md` is a different field on a different
backend: it supplies `SPParams.IfIndex`, the VPP interface index, and this spec
supplies the XFRM `if_id`.
