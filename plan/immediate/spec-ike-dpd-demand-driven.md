...
question raised in the retired deferral shard "fixit-ike-dpd-cleartext". This spec is that
...
**What this spec is not.** It is not `spec-fixit-ike-resource-lifetime-leaks` (closed 2026-08-22)
...
| No test carries the tag in either polarity | a repository-wide search for the id finds it only in `rfc/short/rfc7296.md`, `ai/RFC-REQUIREMENTS.md`, the retired deferral shard "fixit-ike-dpd-cleartext" and `skip-blocked.md`. No `.go` and no `.ci` |
...
| R-3 | Per-peer per-second SAD dumps at scale cost measurable CPU and netlink traffic | Profiling, or `le perf` on a many-peer fixture | Share one dump across peers with a cadence independent of the 1-second tick |
...
`./le test qemu run command "./le test qemu all-tests"`, or a `_linux_test.go` integration test under
`./le test qemu run command "./le test qemu all-tests"` alongside the existing
...
- [ ] the retired deferral shard "fixit-ike-dpd-cleartext" row 3 is resolved
...
### From `fixit-ike-dpd-cleartext.md`, 2026-08-03

Deferred by closure review of spec-fixit-ike-dpd-cleartext, RFC-conformance lens.

Row `RFC7296-2.4-2` [SHOULD] "liveness checks are demand-driven, not periodic" is not met. `handleDPDResponse` (`internal/component/ike/engine/dpd.go`) clears `awaitReply` but never resets `dpd.lastSent`, so `shouldSend` keeps firing at `lastSent + interval` whatever authenticated traffic arrived. RFC 7296 Section 2.4: "Receipt of a fresh cryptographically protected message on an IKE SA or any of its Child SAs ensures liveness". Child SA data-plane traffic is not observed at all
