# Spec: bgp-rib-state-subtree-has-no-producer

| Field | Value |
|-------|-------|
| Status | skeleton |
| Scope | config |
| Depends | spec-improve-2-gnmi-state |
| Phase | - |
| Handoff | - |
| Updated | 2026-09-06 |
...
**What the operator is promised.** The RIB plugin augments `bgp` with a
`config false` container named `rib`
(`internal/component/bgp/plugins/rib/yang/ze-rib.yang`). It holds two
containers. `adj-rib-in` says "Routes received from peers", `adj-rib-out` says
"Routes sent to peers", and each holds a `list peer` keyed by address whose
`route-count` says "Number of routes from this peer" and "Number of routes to
this peer". A reader takes a declared `config false` subtree for operational
state they can fetch by path, over gNMI or any other northbound that walks the
schema. This spec covers those two containers and the nodes under them.

**What Ze does instead.** No producer fills the subtree. `(*Server).Get`
(`internal/component/gnmi/get.go`) resolves every requested path with `walkTree`
over the running CONFIG tree, so a node the schema marks `config false` has no
entry to reach and the RPC answers NotFound. The numbers themselves exist and
are reachable another way: the RIB plugin's `show bgp rib status` handler
(`internal/component/bgp/plugins/rib/rib_commands.go`) builds a per-peer map
with an `in` count and an `out` count. That answer is a command result, and it
reaches no YANG path. The subtree's own `ze:help` already says "No code answers
a read of this subtree", which the descriptions above it contradict. The wider
defect is that gNMI Get ignores the request's DataType and serves the config
tree whatever a client asks for, and `plan/immediate/spec-improve-2-gnmi-state.md`
owns that mechanism.

**What closing it means.** The implementer fills the subtree from a state
producer, or deletes the declaration and leaves `show bgp rib status` as the one
answer. Filling it is obviously right on the data: the counts exist, the RIB
plugin already computes them per peer, and the schema names the exact shape they
would take. It is not reachable alone, because there is no registry a state
producer can register with until the operational-state provider work lands, so
this spec depends on that one and MUST NOT invent a second mechanism beside it
(`ai/rules/no-layering.md`). Deleting the declaration is the honest smaller
answer if that work does not run, and it is a real loss: a northbound client
then has no path for a number the CLI prints. Whichever way it lands, the
`description` and the `ze:help` on each node are brought back into agreement in
the same work (`ai/rules/documentation.md`).
...
### Entry Point
- A gNMI client sends a Get request naming a path under the bgp rib subtree.
- The RIB plugin holds the per-peer route counts in memory.
