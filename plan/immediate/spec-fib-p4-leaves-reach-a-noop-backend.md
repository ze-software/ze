# Spec: fib-p4-leaves-reach-a-noop-backend
...
`internal/plugins/fib/p4/yang/ze-fib-p4-conf.yang` augments the `fib` container
with `container p4`, described as "P4 switch route programming via
gRPC/P4Runtime", and declares three leaves under it. `target` is the
"P4Runtime gRPC target address (host:port)". `device-id` is the "P4Runtime
device ID", default 1. `flush-on-stop` is "Remove all forwarding entries when
the plugin stops", default false. A reader takes the three together to mean Ze
opens a P4Runtime session to `target`, addresses the pipeline named by
`device-id`, and honors `flush-on-stop` when the plugin unloads.

**What Ze does instead.** No leaf under `fib/p4` is read. `runFIBP4Plugin`
(`internal/plugins/fib/p4/register.go`) calls `newBackend("", 0)` with two
literals, and `newBackend` (`internal/plugins/fib/p4/backend.go`) discards both
arguments and returns `&noopBackend{}`. That backend's `addRoute`, `delRoute`
and `replaceRoute` each write one log line and return nil, so no switch is
programmed. The same function starts the worker with `go f.run(ctx, false)`, and
`(*fibP4).run` (`internal/plugins/fib/p4/fibp4.go`) takes that literal as its
`flushOnStop` parameter, so `flushRoutes` is unreachable from the configured
value. The `OnConfigVerify` and `OnConfigApply` callbacks registered in the
same function take their `[]sdk.ConfigSection` and `[]sdk.ConfigDiffSection` as `_` and return
without parsing anything, so the delivered subtree is discarded before any leaf
could be seen. The plugin still subscribes to the system RIB best-change event
and holds the routes it would program, which `show fib p4` prints through
`(*fibP4).showInstalled`.

**What closing it means.** This is one work item for all three leaves, because
one absence explains all three: the P4Runtime gRPC client does not exist.
`backend.go` states the reason in its own header, that a real backend needs
`google.golang.org/grpc` and a P4Runtime protobuf dependency added to `go.mod`.
The implementer chooses between building that backend and refusing the whole
`fib/p4` container at commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses `vrf`. Neither is obviously
right. A refusal is the honest placeholder while the dependency question is
open, and it costs the operator the `show fib p4` visibility the noop backend
gives today, which is real if narrow. Building the backend adds two external
dependencies to a daemon that carries few, so the dependency decision is the
