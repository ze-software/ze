...
|------|------|-----------|--------|
| `TestBuildOperationGraphDerivesProducerBeforeConsumer` | `internal/component/config/transaction/depgraph_test.go` | A create operation producing a resource precedes a create operation consuming it, with no constraint rule registered | |
| `TestBuildOperationGraphDerivesConsumerBeforeProducerOnDestroy` | `internal/component/config/transaction/depgraph_test.go` | A destroy operation consuming a resource precedes the destroy of its producer | |
| `TestBuildOperationGraphNoDerivedEdgeAcrossDifferentResources` | `internal/component/config/transaction/depgraph_test.go` | Produce and consume of different resource identities create no edge | |
| `TestBuildOperationGraphUnknownResourceKindOrders` | `internal/component/config/transaction/depgraph_test.go` | A resource kind no core constant names still matches by identity (A-4) | |
...
|----------|-----------|-------------|----------------|--------|
| `tx-protocol-external-plugin` | `test/reload/tx-protocol-external-plugin.ci` (existing, extended) | A Ze external plugin over the plugin hub transport | The ABI change reaches a plugin in a separate process: it takes part in an ordered transaction, and its coarse node is applied through `config-apply` | |
...
- `internal/component/bgp/plugin/operation.go` - declare consumption of the local address on peer and listener operations, delete all four constraint rules, move the peer and listener labels into this package. Doc: `docs/architecture/config/apply-ordering.md`
...
| 15 | Registered plugin, event type, send type, command, capability, or inventory changed? | Yes | The stream event set is unchanged, but `docs/features/plugins.md` and `docs/plugin-overview.md` must be checked for a copy of the operation type list. Verify at implementation and record the answer |
...
|------|--------------|---------|
| `docs/architecture/api/architecture.md` | `pkg/plugin/rpc/types.go` | Check at implementation for a copy of the operation type list |
| `docs/architecture/api/commands.md` | `pkg/plugin/rpc/types.go`, `pkg/plugin/sdk/sdk_types.go` | Check for a copy of the operation type list |
...
| `docs/architecture/plugin-manager-wiring.md` | `pkg/plugin/rpc/types.go` | Check for a copy of the callback list |
...
| `docs/guide/plugins.md` | `pkg/plugin/rpc/types.go`, `pkg/plugin/sdk/sdk_types.go` | Check for a copy of the operation type list |
| `docs/plugin-development/README.md` | `pkg/plugin/rpc/types.go`, `pkg/plugin/sdk/sdk_types.go` | Check for a copy of the callback list |
...
| `docs/plugin-development/handlers.md` | `pkg/plugin/rpc/types.go` | Check: it documents the `On*` handlers, and the operation handlers change shape |
...
| `docs/plugin-development/testing.md` | `pkg/plugin/rpc/types.go`, `pkg/plugin/sdk/sdk_types.go` | Check whether its example plugin registers operation callbacks |
