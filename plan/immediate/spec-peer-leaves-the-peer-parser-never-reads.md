# Spec: peer-leaves-the-peer-parser-never-reads
...
**What the operator is promised.** Ten leaves of `grouping peer-fields`
(`internal/component/bgp/yang/ze-bgp-conf.yang`) promise per-peer behavior.
`behavior/auto-flush` says "Withdraw every route advertised to this peer when
the session goes down". `connection/link-local` says "Auto-discover IPv6
link-local address for TCP connection". Three sit under `attach/process`:
`processes` says "Legacy ExaBGP process reference list", `processes-match` says
"Legacy ExaBGP process match patterns for event filtering", and
`content/attribute` says "Filter expression to select which BGP attributes are
included in events". Two sit under `rib/adj`: `in` says "Store received routes
in Adj-RIB-In, before policy", and `out` says "Store advertised routes in
Adj-RIB-Out, after policy". Three sit under `rib/out`: `group-updates` says
"Pack routes with identical attributes into one UPDATE", `auto-commit-delay`
says "Milliseconds to wait before Ze flushes pending route changes to the peer",
and `max-batch-size` says "Maximum number of route changes in one flush cycle". Each reads as a
switch, a delay, or a limit the operator sets on one peer. This spec covers
those ten leaves, and it covers them together because one reason explains all
ten.

**What Ze does instead.** The peer parser never navigates to them.
`parsePeerSettings` (`internal/component/bgp/reactor/config.go`) takes
`connection`, `session`, `behavior`, `timer`, `capture` and `attach` off the
peer map, and under `connection` it reaches `local`, `remote`, `md5`, `ttl` and
`bfd`. It never takes `rib`, and it never reads `link-local` off the connection
map. From `behavior` it reads `group-updates`, `rs-fast-path` and `manual-eor`,
and nothing else. `parseProcessBindingsFromTree` in the same file reads
`encoding` and `format` off the `content` container and the `receive` and `send`
leaf-lists, so an event carries every attribute of the route whatever
`content/attribute` holds. The peer `rib` container has no plugin reader either:
the `bgp-adj-rib-in` plugin (`internal/component/bgp/plugins/adj_rib_in/register.go`)
stores what it is fed whenever it is loaded, and its module holds API nodes
alone. Outside `internal/exabgp/migration/` no Go file in the tree holds the
literal `auto-flush`, and the only two that hold the `adj` map key are the
ExaBGP serializer and an unrelated VPP FIB table. `ze exabgp migrate`
(`internal/exabgp/migration/migrate.go`) writes three of the ten into the
configuration it produces, so a migrated peer carries leaves the reactor never
reads. Five of the ten already say so in their own `ze:help`, which reads "The
BGP reactor reads no value from the leaf" beside a `description` that promises
the behavior, so those nodes contradict themselves.

**What closing it means.** The implementer builds the behavior, or refuses the
leaf at commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses the `vrf` leaf. The answer
is per leaf, and three of them are already clear. `processes` and
`processes-match` are input forms the migration rewrites into `attach process`
entries before it deletes the block that carried them, so nothing downstream
needs them and the refusal is obviously right. `connection/link-local` names the
same word as `session/link-local` one level away, which `applyLinkLocal`
(`internal/component/bgp/reactor/config_nexthop_form.go`) does read, so a schema
that keeps both owes a reader a way to tell them apart, and dropping the inert
one is the smaller change. `rib/adj/in` and `adj/out` name storage the plugin
already performs unconditionally, so the honest answers are to make the plugin
honor the leaf or to state that plugin presence is the switch. The other four
name behavior nothing implements, and the design phase decides each. A phase
