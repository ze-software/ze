# Spec: interface-monitor-container-names-no-behavior
...
`internal/component/iface/yang/ze-iface-conf.yang` declares `container monitor`
as a child of `container interface`, beside `container loopback`, with the
description "Interface monitoring settings". It holds one leaf, `loopback`, a
boolean defaulting to false, described as "Monitor loopback interface". A reader
takes `interface { monitor { loopback true } }` to turn some form of monitoring
on for the loopback interface.

**What Ze does instead.** Nothing reads either node. `parseIfaceConfig`
(`internal/component/iface/config.go`) unmarshals the interface subtree into
`ifaceMap` and reads `backend`, `dhcp-auto`, each interface-kind map, and
`loopback` from it. It never reads `ifaceMap["monitor"]`. No file under
`internal/component/iface/` or `internal/plugins/iface/` names `"monitor"` as a
config key. The hits for that string elsewhere in the tree are the CLI verb table
(`internal/component/command/verbs.go`) and the netlink link monitor
(`internal/plugins/iface/netlink/`), and neither reads this path. Both nodes
parse, validate, commit, and appear in `show configuration`, and neither changes
one line of daemon behavior. The `ze:help` text on both already states this.

**What closing it means.** This gap is unlike the others in its class, and the
difference decides the work. Elsewhere the leaf name says what the behavior
should be, so an implementer can build it: `pd length` names a DHCPv6 prefix
request, `ttl` names an outer header field, `vti bind` names an XFRM interface.
Here the intent is undecidable from the tree. "Monitor loopback interface" does
not say what is watched, what is reported, where the report goes, or what changes
when the value is false. No other node, comment, or document in the repository
supplies the missing definition. So the implementer cannot pick between building
and refusing, because there is no behavior to build. The choice is a definition
from the owner, which then becomes the spec's Task, or removal of both nodes.
Removal is not the deletion alone. The comment on `unimplementedVRFValidator`
(`internal/component/config/validators.go`) records why: nothing in the config
walk emits `ErrTypeUnknown`, so a leaf deleted from the schema is skipped and the
same config still loads silently. Removal therefore means deleting the nodes and
refusing the old path at commit, the way that validator refuses `vrf`, or
accepting that a config carrying `monitor` keeps loading with no message.
