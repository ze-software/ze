# Spec: dhcpv6-client-never-requests-a-delegated-prefix
...
**What the operator is promised.** `internal/component/iface/yang/ze-iface-conf.yang`
declares `container pd` under `interface-unit/unit/ipv6/dhcpv6` with the
description "Prefix delegation", and one leaf inside it, `length`, described as
"Requested prefix length" over the range 48 to 64. A reader takes
`dhcpv6 { pd { length 56 } }` to mean the DHCPv6 client on that unit asks the
server for a delegated /56. RFC 8415 Section 18.2.1 states the mechanism the
name points at: "The client uses IA_NA options (see Section 21.4) to request the
assignment of non-temporary addresses, IA_TA options (see Section 21.5) to
request the assignment of temporary addresses, and IA_PD options (see
Section 21.21) to request prefix delegation." Section 21.22 states the hint: "A
client MAY send a non-zero value in the "prefix-length" field and the
unspecified value (::) in the "IPv6-prefix" field to indicate a preference for
the size of the prefix to be delegated."

**What Ze does instead.** The request half does not exist. `parseDHCPv6Config`
(`internal/component/iface/config.go`) reads `pd["length"]` into
`dhcpv6UnitConfig.PDLength`, and `internal/component/iface/register.go` carries
that value through `pdLength` into `dHCPConfig.PDLength`
(`internal/plugins/iface/dhcp/dhcp_linux.go`). No code reads `PDLength` after
that assignment. `runV6` and `renewV6`
(`internal/plugins/iface/dhcp/dhcp_v6_linux.go`) both call
`client.RapidSolicit(ctx)` with no modifiers. `RapidSolicit`
(`vendor/github.com/insomniacslk/dhcp/dhcpv6/nclient6/client.go`) appends only
`dhcpv6.WithRapidCommit`, and `NewSolicit`
(`vendor/github.com/insomniacslk/dhcp/dhcpv6/dhcpv6message.go`) prepends only
`WithIAID`, whose modifier updates an `OptIANA`. So every Solicit Ze sends
carries IA_NA and no IA_PD. The vendored library supplies `WithIAPD`
(`vendor/github.com/insomniacslk/dhcp/dhcpv6/modifiers.go`) and no file under
`internal/` names it. The receive half is complete and tested: `handleV6Reply`
(`internal/plugins/iface/dhcp/dhcp_v6_linux.go`) iterates `msg.Options.IAPD()`,
installs each prefix, and publishes it as a lease, with
`internal/plugins/iface/dhcp/dhcp_v6_pd_linux_test.go` beside it. Ze installs a
prefix a server volunteers, and it asks no server for one.

**What closing it means.** The implementer either builds the request or refuses
the leaf at commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses the `vrf` leaf. Building it
is the right answer here, and the spec states why rather than weighs the two.
The receive half already installs and publishes a delegated prefix, so a refusal
would remove the only prefix delegation Ze has. The modifier the request needs
is in the vendored library, the two call sites are `runV6` and `renewV6`, and an
operator who writes `pd` wants a prefix. The design settles the IAID the IA_PD
carries, because `NewSolicit` derives the IA_NA IAID from the last four bytes of
the hardware address, and RFC 8415 gives each IA its own identifier. It also
settles what an unset `length` sends, because the leaf has no default and
Section 21.22 makes the hint optional. `rfc/full/` carries no text for RFC 8415
and `rfc/short/` carries no summary for it, so the implementation enrolls the
RFC before it quotes one (`ai/rules/rfc-compliance.md`).
`plan/spec-dhcpv6-server.md` is the server side of DHCPv6 and is a different
subject.
