# Spec: software-version-mode-implements-two-of-four
...
**What the operator is promised.** `session/capability/software-version/mode`
(`internal/component/bgp/plugins/softver/yang/ze-softver.yang`) is an
enumeration of four values, and each carries its own description. `enable` says
"Advertise capability (default)". `disable` says "Do not advertise capability".
`require` says "Advertise and require peer support". `refuse` says "Do not
advertise, reject if peer has it". The capability container that holds it
(`internal/component/bgp/yang/ze-bgp-conf.yang`) states the same contract for
every capability under it: "require advertises it and refuses the session when
the peer does not offer it. refuse does not advertise it and refuses the session
when the peer offers it." A reader takes all four as session outcomes. This spec
covers that one leaf.

**What Ze does instead.** `extractSoftverCapabilities`
(`internal/component/bgp/plugins/softver/softver.go`) reduces the four values to
one boolean: the capability is enabled when the mode is neither `disable` nor
`refuse`. So `require` behaves as `enable` and holds the peer to nothing, and a
peer that offers no Software Version capability stays up. `refuse` behaves as
`disable`, and a peer that offers one stays up as well. The enforcement the two
values name exists in the reactor and the plugin cannot reach it: `applyCapMode`
(`internal/component/bgp/reactor/config_capabilities.go`) appends a capability
code to `RequiredCapabilities` or `RefusedCapabilities` for the capabilities the
reactor parses itself. The declaration a plugin sends carries no such field.
`CapabilityDecl` (`pkg/plugin/rpc/types.go`) holds a Code, an Encoding, a
Payload and a Peers list, so a plugin-declared capability cannot state an
enforcement mode across the SDK boundary at all. The leaf's own `ze:help` states
the collapse, and the four enum descriptions beside it still promise the
session outcomes, so the node contradicts itself.

**What closing it means.** The implementer builds the behavior, or narrows the
enumeration to the two values the plugin implements, which is this leaf's form
of the refusal `unimplementedVRFValidator`
(`internal/component/config/validators.go`) applies to the `vrf` leaf. Building
is obviously right. The enforcement already runs for every capability the
reactor declares itself, the mode is already parsed, and the only missing piece
is a field on the plugin declaration plus the reactor honoring it for a
plugin-owned code. Narrowing the enum takes an operator's control over one
capability away because of where its code lives, which is an implementation
detail no configuration should expose. The design phase decides how the mode
crosses the boundary. It covers both new outcomes with a test that brings a
session down. A test that reads the OPEN message alone cannot tell `require`
from `enable`, and it cannot tell `refuse` from `disable`.
...
- The softver plugin receives the bgp subtree as JSON and declares capabilities from it.
