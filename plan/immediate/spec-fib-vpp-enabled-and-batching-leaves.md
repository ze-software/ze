# Spec: fib-vpp-enabled-and-batching-leaves
...
`internal/plugins/fib/vpp/yang/ze-fib-vpp-conf.yang` declares three leaves under
`fib { vpp { ... } }` that no code honors. `enabled` is described as "Enable VPP
FIB programming. Routes from system RIB are programmed directly into VPP's FIB",
with default false, so a reader takes it to be the switch that turns the feature
on. `batch-size` is "Maximum number of routes per batch dispatch", default 256.
`batch-interval-ms` is "Maximum time in milliseconds before dispatching a partial
batch", default 10. A reader takes the pair to mean Ze accumulates route changes
and dispatches them in bounded groups, and takes the two defaults to be the
values in force when the leaves are absent.

**What Ze does instead.** `parseFibVPPConfig`
(`internal/plugins/fib/vpp/fibvpp.go`) is the only producer that turns the
delivered `fib/vpp` subtree into configuration. It holds a `known` map naming
`enabled` and `table-id`, and a `deferred` map naming `batch-size` and
`batch-interval-ms`. A key in `deferred` returns the error
`fib-vpp config: %q is not yet implemented (deferred)`, so a configuration that
writes either leaf fails to parse. A key in neither map returns
`fib-vpp config: unknown key %q`. The function then builds a `fibVPPConfig`
whose only field is `tableID`, and it stores `table-id` alone. So `enabled`
passes the known check and is dropped: the struct has no field for it and no
reader can consult it. The plugin programs the VPP FIB for as long as it is
loaded, whatever the leaf holds, and the default false disables nothing.

The two batching leaves produce the worse failure. The schema declares defaults
of 256 and 10, which tells an operator those are the values in force and invites
writing a different one. Writing either value fails the config parse against a
number the schema itself suggests. That is a schema and a parser disagreeing
about the same leaf, and the operator meets it as a rejected commit with no way
to reach the documented default.

**What closing it means.** These three leaves are one work item because one
decision settles all of them: whether `fib/vpp` gets the behavior its schema
declares, or whether the schema is cut back to what the plugin does. The
implementer chooses between building the behavior and refusing the leaves at
commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses `vrf`. A refusal is closer
to right for the two batching leaves than a commit-time error usually is,
because the parser already refuses them, and the work would be moving that
refusal to commit time and deleting the misleading `default` statements so no
schema reader is told 256 applies. `enabled` is the opposite case: a leaf that
names the plugin's own on switch and silently does nothing, where refusing it
tells the operator that a plugin loaded from configuration cannot be disabled
from configuration. `internal/plugins/fib/vpp/fibvpp.go` records the design
reason for the deferral in the `fibVPPConfig` doc comment, that the system RIB
already delivers per-family batches and the VPP call `IPRouteAddDel` programs
one route per call, so batching adds machinery for no gain today. That comment
argues for deleting the two leaves rather than implementing them, and the design
owes an answer either way.

`plan/spec-finish-vpp-stub.md` exists at Status `ready` and covers the VPP stub
and its test coverage. It does not cover these three leaves, so this spec does
not duplicate it.
