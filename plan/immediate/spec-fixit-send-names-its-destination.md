...
send bgp * unicast 10.0.0.0/24 next-hop 10.0.0.1
```
...
### The token after the selector names WHAT is sent

Thomas, on 2026-09-05: the word `announce` in
`send bgp * announce unicast 10.0.0.0/24 next-hop 10.0.0.1` "is redundant".

`send` is the verb. `announce` was a second verb in the same command, and to
send a route IS to announce it. So the token after the selector names the thing
that goes on the wire, and it never repeats the act of sending. The closed set
is `unicast`, `blackhole`, `flowspec`, `raw`, `update`, `cached` and `withdraw`.

`withdraw` stays, and the asymmetry with `announce` is correct rather than an
oversight. A withdraw is the opposite act, not the same act named twice, and a
withdrawal is a thing an operator sends: RFC 4271 carries it in the UPDATE's
withdrawn-routes field. It is also the word BGP and ExaBGP both use, so an
operator meets one spelling on each side of the bridge.

This costs nothing when it lands before phase 5, and it is the phase that moves
the six announce and withdraw forms. The three `announce` rows in the move table
below carry the shorter spelling.
...
`plan/immediate/spec-fixit-selector-narrows-through-a-pipe.md` is committed at HEAD and
...
- The selector becomes mandatory on every one of them, and each registers `RequiresSelector: true`. `announce unicast <prefix>` with no selector is refused; `send bgp * unicast <prefix>` is how an operator reaches every peer.
...
| `ze-bgp:announce-unicast` | `announce unicast ...` and `peer <sel> announce unicast ...` | `send bgp <sel> unicast ...` | fan-out |
| `ze-bgp:announce-blackhole` | the same two paths | `send bgp <sel> blackhole ...` | fan-out |
| `ze-bgp:announce-flowspec` | the same two paths | `send bgp <sel> flowspec ...` | fan-out |
...
| A-4 | Every argument the three under-declared handlers read can be stated as a typed leaf, except the update route expression | the announce and withdraw forms already declare theirs; `handleRaw` reads three tokens; `handleCacheForwardRPC` reads two | the declaration is partial and the journal row stays open for a member of this population | the generated usage line for each moved command, asserted in a `.ci` | confirmed: `TestSendArgumentsAreDeclared` pins the four generated lines and `test/ui/cli-completion-send-forms.ci` pins the same lines and the completions. The raw type word is a keyword-introduced leaf rather than a bare word, because the renderer states an optional leaf as `[<name> <value>]` and a pattern-less data leaf would swallow a bare type word before the enum saw it |
...
| Who else touches this path? | `plan/immediate/spec-fixit-selector-narrows-through-a-pipe.md` (its display half stands, and `ze-peer-cmd.yang` is the shared file), and a concurrent session editing `docs/architecture/cli/command-verbs.md`, which this spec edits for the root-verb decision alone |
...
| AC-5 | `send bgp * unicast <prefix>` and `send bgp edge1 withdraw all` | Each behaves exactly as the bare and peer-scoped paths behaved before: the same handler, the same wire method, the same bytes. The groupings are instantiated once, and `*` is the only way to reach every peer |
...
| `TestSendArgumentsAreDeclared` | `internal/component/plugin/server/send_test.go` | the generated usage line for each moved command names every argument its handler reads | PASS. It sits beside the other send tests rather than in `raw_test.go`: the line for `peer update` and `request cache forward` needs the merged tree, which only a package blank-importing `internal/component/plugin/all` can build |
...
| 3 | CLI command added/changed? | Yes | `docs/guide/cli.md` (12 occurrences). `docs/guide/command-reference.md` and `docs/guide/command-catalogue.md` are HAND-AUTHORED and are read for old spellings like any other page. This row said they were regenerated by `./le wiki-catalog update`; that command writes the live registry to a destination you name, and running it against either page destroys the authored content (`plan/journal/test-against-broken-path.md`, 2026-09-05) |
