# Spec: ipsec-key-exchange-ikev1-runs-ikev2
...
`internal/component/ike/ipsec/yang/ze-ipsec-conf.yang` declares `leaf
key-exchange` on `ike-group`, an enumeration of `ikev1` and `ikev2`, defaulting
to `ikev2`, with the description "IKE protocol version." A reader takes
`ike-group G { key-exchange ikev1 }` to mean the peers referencing that group
negotiate with IKEv1.

**What Ze does instead.** This is not an unread leaf. The leaf is live, and one
of its two values lies. `parseIKEGroup`
(`internal/component/ike/ipsec/config.go`) reads `key-exchange`, refuses a value
outside the enumeration through `parseKeyExchange`, and stores the result in
`IKEGroup.KeyExchange` (`internal/component/ike/ipsec/types.go`). No code outside
the `ipsec` package reads that field, and inside the package the only other
reference is its round-trip test. The engine writes the version as a constant.
`buildSAInitRequest` (`internal/component/ike/engine/initiator.go`) sets
`MajorVersion: 2` in the header it builds, and so do the responder paths in
`internal/component/ike/engine/responder.go`, the authentication path in
`internal/component/ike/engine/auth.go`, and the error notify path in
`internal/component/ike/engine/notify_error.go`. So `key-exchange ikev1` passes
validation, commits, appears in `show configuration`, and the daemon puts major
version 2 in every header it sends. The operator is told a protocol version and
gets another one.

**What closing it means.** The leaf-level refusal that the other gaps in this
class reach for is wrong here, and measurement settles it: 54 files under `test/`
set `key-exchange` on 75 lines, and every one of them names `ikev2`. A validator
bound to the leaf would refuse the value Ze does implement and break all 54. The
choice is a value-level refusal of `ikev1`, or an IKEv1 implementation. The
refusal is cheap and already has its mechanism: deleting `ikev1` from the
enumeration makes `parseKeyExchange` return not-valid for that name, and
`parseIKEGroup` turns that into a commit error naming the group and the value, so
no new validator is needed. Implementing IKEv1 is a second protocol engine rather
than a flag, because IKEv1 has its own exchanges, its own header, and its own
state machine, none of which the current engine holds. That size is why the
decision belongs to the owner rather than to the implementer. Whichever way it
goes, the gap the operator meets today is that one accepted enumeration value
runs a different protocol, and a spec that only records the choice has not closed
it (`ai/rules/completion.md`).
