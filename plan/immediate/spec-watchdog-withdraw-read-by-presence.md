# Spec: watchdog-withdraw-read-by-presence
...
**What the operator is promised.** `update/watchdog/withdraw`
(`internal/component/bgp/yang/ze-bgp-conf.yang`, in `grouping update-block`) is
a boolean, and its description says "Start in withdrawn state (default true)".
The container beside it says the block is "Watchdog-controlled route - held
until 'bgp watchdog announce <name>'". A reader takes `withdraw true` for a
route held back until `request bgp watchdog announce <name>` runs, and
`withdraw false` for a route announced as soon as the session reaches
Established. This spec covers that one leaf.

**What Ze does instead.** The plugin reads the key, not the value.
`collectWatchdogRoutes` (`internal/component/bgp/plugins/watchdog/config.go`)
takes the watchdog map off the update block and tests membership alone, then
hands that boolean to `parseNLRIEntries` in the same file as the
`initiallyWithdrawn` argument. So any value at all withdraws the routes:
`withdraw false` starts every route of the block withdrawn, exactly as
`withdraw true` does. The only way to reach the announced state is to delete the
leaf, which is the opposite of what the description says the default is. The
leaf carries no `default` statement in the module either, so three statements
disagree: the description says the default is true, the schema supplies no
default, and the code announces when the leaf is absent. This is a value that IS
read and acted on wrongly, not a leaf nobody reads, so an operator who writes
the false they want gets the opposite behavior with no error and no log line
(`ai/rules/principles.md`).

**What closing it means.** The refusal route
`unimplementedVRFValidator` (`internal/component/config/validators.go`) uses is
not the answer here, because the feature runs: the pools, the announce command
and the withdraw command all exist, and only the read is wrong. Closing it means
reading the boolean and settling the default across the three statements, and
the spec has to say which default it picks. Both choices change what an existing
configuration does. A config carrying `withdraw false` starts announcing, and a
config carrying no `withdraw` leaf starts withdrawn if the description's default
wins. The design phase states the chosen default, corrects whichever of the
description and the `ze:help` no longer matches, and covers both values with a
test that would fail against the presence read.
...
- The watchdog plugin receives its config section as a map, and scans the update blocks.
