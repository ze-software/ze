# Spec: BCP 194 child 4 -- prefix filtering defaults
...
This spec is child 4 of the BCP 194 spec set. The umbrella is
`plan/immediate/spec-bcp194-0-umbrella.md`.

Goal: ship the prefix filtering defaults RFC 7454 S6.1 and S8 recommend.

One root cause sits under every gap. `runIngressPolicyChain`
(`internal/component/bgp/reactor/filter_ordered.go`) accepts on an empty filter
chain, by design, and Ze ships no prefix list of any kind. An operator who
configures nothing gets the posture the BCP argues against.

| RFC 7454 section | Verified gap |
|------------------|--------------|
| S6.1.1 | No IANA special-purpose or bogon list is shipped |
| S6.1.2.1 | No IANA-allocated space table, and no refresh for one |
| S6.1.3 | No per-peer maximum prefix length. Only `prefixEntry.le` exists |
| S6.1.4 | The locally originated prefix set is not derived for import reuse |
| S6.1.5 | No IXP LAN concept exists in the tree |
| S6.1.6 | Import does not drop the default route by default |
| S6.1.2.2.2 | RPKI computes Valid, Invalid and NotFound, and rejects Invalid by default. No local-preference write exists under `internal/component/bgp/plugins/rpki/`, so "accept NotFound at a low preference" is unimplemented |

Maximum prefixes per peer (S8) is complete and conformant, defaults included,
and is in scope as a regression guard only.
