# Spec: rfcgate-1b-rfc7296-pilot-deferred-ike-source-address
...
WP-8), row dated 2026-07-31 in the retired deferral shard "rfcgate-1b-rfc7296-pilot". That spec is
closed and its shard is deleted, so this file is the tracker now. The open owner question is
recorded there as OR-WP8-1.

**The problem.** RFC 7296 Section 2.11 requires a reply to leave from the address AND the
port the request arrived on. WP-8 implemented and tagged the PORT half of
`RFC7296-2.11-3` only, and its tag says so. The ADDRESS half is unmet on a multi-homed
host.

**The symptom.** Both IKE listeners bind the wildcard address by default, so `WriteToUDP`
lets the route table choose the source address of every reply. `pkt.LocalAddr` is captured
on the receive path and no sender reads it. On a host with one address the reply is right by
accident; on a multi-homed host a peer can see a reply from an address it never wrote to.

**The goal.** Reply from the observed local address. Two routes are open and neither is
measured:

1. `IP_PKTINFO` (and its IPv6 sibling) plus a `sendmsg` path that names the source address
   per datagram.
2. One socket per local address, bound explicitly.

**The constraint that makes this an owner question.** Both routes change listener lifecycle,
which is why WP-8 stopped at the port half rather than picking one. Phase 1 is evidence:
measure what each costs before choosing.
