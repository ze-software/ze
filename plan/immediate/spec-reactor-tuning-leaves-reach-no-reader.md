# Spec: reactor-tuning-leaves-reach-no-reader
...
**What the operator is promised.** Two leaves of the `environment/reactor`
container tune the BGP engine. `speed` says "Reactor loop cycle time
multiplier, from 0.1 to 10.0", and its help says a value under 1.0 lowers the
latency and raises the CPU cost. `cache-ttl` says "Seconds Ze keeps a built
UPDATE wire message in the encoding cache", and its help says 0 disables the
cache so Ze builds every UPDATE fresh. Both sit in the `augment
"/hub:environment"` block of `internal/component/bgp/yang/ze-bgp-conf.yang`. A
reader takes the first for a latency-against-CPU dial, and the second for an age
bound on the encoding cache. This spec covers those two leaves and no others.

**What Ze does instead.** Each value reaches an environment key and stops.
`ExtractEnvironment` (`internal/component/config/environment_extract.go`) copies
the `reactor` container into the `reactor` section. `ApplyEnvConfig`
(`internal/component/config/apply_env.go`) sets `ze.bgp.reactor.speed` and
`ze.bgp.reactor.cache-ttl` from `envPlumbingTable`, and
`internal/component/config/environment.go` registers both keys. No non-test file
in the tree reads either key, and no file under
`internal/component/bgp/reactor/` holds the identifier `speed`. The engine keeps
no age on the update cache: `New` (`internal/component/bgp/reactor/reactor.go`)
sizes the cache from `RecentUpdateMax` alone, which arrives from the sibling
leaf `cache-max`. The nine other leaves of the same container do reach behavior,
so the container is not dead and the plumbing is not broken. `New` reads
`ze.fwd.chan.size`, `ze.fwd.batch.limit`, `ze.fwd.pool.maxbytes`,
`ze.fwd.pool.headroom` and `ze.fwd.teardown.grace`. Session setup in
`internal/component/bgp/reactor/session_connection.go` reads `ze.buf.read.size`
and `ze.buf.write.size`. `newUpdateGroups`
(`internal/component/bgp/reactor/update_group.go`) reads
`ze.bgp.reactor.update-groups`, and
`internal/component/bgp/config/loader_create.go` reads
`ze.bgp.reactor.cache-max`.

**What closing it means.** The implementer builds the behavior, or refuses the
leaf at commit the way `unimplementedVRFValidator`
(`internal/component/config/validators.go`) refuses the `vrf` leaf. The two
leaves do not answer that question the same way. `cache-ttl` names a cache that
exists and is already bounded by count, so an age bound is an addition to a live
mechanism, and building it is the obvious answer. `speed` names a loop
multiplier the reactor does not have, so the design phase must say what it would
scale before anyone can build it, and the refusal is the honest placeholder
until then. The environment key is a second surface for each leaf. A refusal at
commit leaves `ze.bgp.reactor.speed` settable from the operating system
environment and equally dead, so the registration in
`internal/component/config/environment.go` and the row in `envPlumbingTable`
belong to whichever answer the implementer picks.
...
- `ExtractEnvironment` turns the `environment` container into per-section string maps.
