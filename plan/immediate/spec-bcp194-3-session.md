# Spec: BCP 194 child 3 -- speaker and session hardening
...
This spec is child 3 of the BCP 194 spec set. The umbrella is
`plan/immediate/spec-bcp194-0-umbrella.md`.

Goal: harden the BGP speaker and its sessions per RFC 7454 S4 and S5. Two gaps
are verified.

Gap 1, GTSM on the listen socket. `RealListenerFactory` in
`internal/core/network/network.go` carries `MD5Peers` and an outgoing
`ListenTTL`. It sets no `IP_MINTTL` on the listen socket. The receive-side GTSM
check therefore runs only on the connection the kernel already accepted, in
`tuneTCPConnectionForSettings`. GTSM stops protecting the control plane from
off-path SYN packets, which is the reason S5.2 recommends it.

Gap 2, the control plane ACL. `translatePolicy` in
`internal/plugins/copp/translate.go` emits accept terms for hand-typed
`trusted-source` entries, and one rate-limit term. It emits no final drop term,
so the ACL that S4 describes cannot be expressed. The `over-limit-policy` leaf
also defaults to `accept`. An operator who turns CoPP on gets a count of the
over-limit packets and no drop.

Out of scope, deliberately: TCP-AO (RFC 5925, S5.1) is absent from Ze. It is a
large separate feature and needs its own spec.
