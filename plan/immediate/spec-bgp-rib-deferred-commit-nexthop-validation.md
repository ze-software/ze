...
| Who else touches this path? | `internal/component/bgp/reactor/reactor_api_batch.go` (`SendRoutes`, the only non-test constructor) and `internal/component/bgp/plugins/cmd/commit/` (the CLI verbs above it). the retired deferral shard "fixit-attribute-length-from-family" and the retired deferral shard "fixit-mpreach-split-undercounts-rd" cover sibling size-versus-write defects in `internal/core/bgp/attribute`; neither touches `commit.go`. |
...
- Whether the set of size-versus-write mismatch sites is CLOSED stays open. It is the
  question the retired deferral shard "fixit-commit-rail-nexthop-unvalidated" records, and the
