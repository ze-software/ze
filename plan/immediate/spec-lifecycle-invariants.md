...
- `internal/le/doc/wiring/wiring.go` `check_wiring` - the existing checker I-1 extends with an emit-versus-subscribe predicate, reached through the `wiring` target of `./le repo check`.
...
| R-3 | The I-1 guard fires on legitimately unsubscribed topics (a topic emitted for external plugin processes only) | `./le repo check` goes red on unrelated work | reuse the existing `WIRING_ALLOWLIST` mechanism for reviewed exceptions rather than inventing a second one |
...
| Who else touches this path? | `plan/pre-release/spec-finish-l2tp.md` (its L42 renegotiation-test gap is closed by this spec; its L41 row is already stale and should be corrected), `plan/spec-radius-acct-timewheel.md` (same `interimLoop`, so land this first), `docs/architecture/traffic/cos-dynamic.md` |
...
| AC-12 | A build declares an event topic that is emitted and never subscribed | `./le repo check` reports it and fails, unless the topic is in the reviewed allowlist |
...
- `plan/pre-release/spec-finish-l2tp.md` - correct the stale L41 row and close L42
...
- the retired deferral shard "lifecycle-invariants" - the deferral shard named in the metadata table
...
| Every subscriber topic has a subscriber | `./le repo check` |
...
| Extend the existing `check_wiring` checker for the I-1 guard | a new AST-walking checker; a runtime subscriber count on the bus | `check_wiring` is already a symbol-versus-reference checker with an allowlist for reviewed exceptions and is already wired into `./le repo check`. The `Emit` return value cannot serve, because its contract excludes in-process subscribers |
...
- [ ] **Commit B:** `git rm plan/immediate/spec-lifecycle-invariants.md` only
