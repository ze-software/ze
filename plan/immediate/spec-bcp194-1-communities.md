...
the set that `plan/immediate/spec-bcp194-0-umbrella.md` coordinates.
...
- [ ] `internal/le/rfc/rfc.go` - `check_new_summaries` continues only for
...
| R-2 | Narrowing `source_keyword_count` weakens a compliance gate for every RFC | `./le rfc check` passes for a summary that should fail | Exclude only the key-words sentence, and add a fixture proving a genuine MUST still fires the gate |
...
| `test_new_summary_boilerplate_only_passes` | `internal/le/` | A 2119-boilerplate-only source with zero gated requirements passes, and a genuine MUST still fails | |
...
- `internal/le/rfc/rfc.go` - `source_keyword_count`
...
6. **Phase: Ledger and gate** -- AC-15, AC-16
   ...
   - Files: `internal/le/rfc/rfc.go`, `rfc/not-enrolled.txt`
   - Verify: `./le rfc check` passes, and a genuine MUST still fires the gate
...
| The RFC gate is green | `./le rfc check` |
