# Spec: config-verify-rejects-unknown-keys
...
Own the accepted strict unknown-key rejection remainder from child 7 of
`plan/spec-improve-0-umbrella.md`, recorded there on 2026-08-29 and inherited
from the retired improve-7 deferral row. Config verification must reject a
written key absent from the applicable schema rather than silently accepting it.
This is existing accepted scope, not authorisation for a new configuration feature.

Source reading on 2026-09-19 found that `validateContainerEntry` in
`internal/component/config/yang/validator.go` validates a data key only when it
exists in `entry.Dir`; an unknown key reaches the function's successful return.
That is a helper-level observation. No runtime reproduction was run, and the
full operator-facing verify path, including parsing and all callers of this
helper, still needs tracing before design or a claim about every config input.

The design must preserve nodes contributed by other loaded modules.
`ValidateTreeAllModules` walks each contributing module separately, so rejecting
all keys absent from one module would reject valid configuration. It must also
preserve the documented `ze:allow-unknown-fields` containers
(`docs/architecture/config/yang-config-design.md`) and legitimate list-key and
metadata handling. Derive the accepted set from the applicable schema rather
than adding a second hand-maintained list.

At design, read the configuration architecture and producing verify functions,
trace a concrete unknown key through an operator entry point, and define the
refusal evidence alongside valid cross-module and allow-unknown-fields cases.
The immediate bucket records the accepted config-fidelity obligation; the
source observation alone is not proof of a current end-to-end defect on every
entry point.
