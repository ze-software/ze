...
- **Four RADIUS subscriber accounting attributes (from spec-radius-subscriber-attributes, homed here 2026-09-03, re-homed the same day)** - Calling-Station-Id (31), Event-Timestamp (55), Acct-Delay-Time (41) and Acct-Terminate-Cause (49). **Settled and moved out: `plan/spec-radius-acct-session-attributes.md` owns this now.** All four are RFC 2119 MAY for a NAS, so the RFCs decide nothing and `ai/rules/rfc-compliance.md` reserved the choice for the owner. Thomas ruled on 2026-09-03: copy Juniper, emit all four, no config knob. The one place ze does not follow Juniper is Acct-Terminate-Cause on Interim, which RFC 2866 Section 5.10 confines to Stop. Nothing is owed here any more; the costs this row used to carry are in that spec's Task section. Row: `plan/deferrals/radius-subscriber-attributes.md`.
