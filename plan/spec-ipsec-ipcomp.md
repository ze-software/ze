...
Four RFC compliance rows were deferred out of `plan/spec-rfcgate-1b-rfc7296-pilot.md` into
...
| Who else touches this path? | `plan/spec-rfcgate-1b-rfc7296-pilot.md` (the four rows, and the generic error-notification sender its WP-3 and WP-4 build), `plan/spec-fixit-vpp-ipsec-inoperable.md` (the VPP backend cannot program a security association at all today), `plan/spec-ipsec-remote-access.md` (the same engine and the same Child SA path) |
...
the design phase, and it serves many other rows in `plan/spec-rfcgate-1b-rfc7296-pilot.md`.
