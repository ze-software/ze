...
| Option | Consequence |
|--------|-------------|
| Refuse the connection | Conformant with Section 5.4-3 read literally. Breaks against every peer that publishes no revocation information, which today includes strongSwan in `test/ipsec-interop/scenarios/06-eap-tls13` AND Ze's own server |
