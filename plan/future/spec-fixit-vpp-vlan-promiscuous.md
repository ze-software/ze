# Spec: fixit-vpp-vlan-promiscuous
...
(`vppBackendImpl.CreateVLAN`, `internal/plugins/iface/vpp/ifacevpp.go`, sending
`interfaces.CreateVlanSubif`) but never puts the parent interface into
promiscuous mode. No promiscuous binapi message is sent anywhere in the
`internal/plugins/iface/vpp` tree.

Suspected symptom: VPP drops VLAN-tagged frames on an interface whose parent is
not promiscuous, so VLAN sub-interfaces of a VPP-managed interface have no
connectivity.

**The VPP-side premise is NOT verified in this repository, and must be settled
before design starts.** `third_party/vpp-linux-cp` vendors only the linux-cp
plugin, so no vnet or ethernet source is present to confirm that VPP requires
promiscuous mode for tagged frames. Settle it from upstream VPP source, or from
a QEMU/VPP functional test that shows tagged frames dropped and then delivered
once promiscuous mode is enabled. If the premise is false, close this spec and
record why.

Goal: settle the premise, then enable promiscuous mode on the parent whenever at
least one VLAN sub-interface exists, and disable it again when the last one is
removed.

Provenance: VyOS T9018 (`vpp: Auto-enable promiscuous mode for interfaces with
VLANs`), which describes the same symptom as a connectivity bug.
