...
| observe | `spec-anomaly-3-observe.md` | A | ready | 1048 | incident **lifecycle** store (open→finalize/EndTime the detect ring lacks) + a NEW `show anomaly observe` query surface. NOT a mirror: `ddos/observe` has no query surface/web card and its incident struct is dest-tuple, not source-prefix. In-memory ring (no durable store today). |
...
| entity-matrix | `spec-anomaly-5-entity-matrix.md` | B | ready | 1048 | generalize the entity axis to **dest** and **port**. Mostly a FACTS-layer (`trafficfeature`) change — dest carries only `inBytes` today, port is a per-source histogram; the detector re-key is the smaller half. Prefix cohort transfers to dest; port has no natural cohort. Excludes ASN (see below). |
...
| as-entities-cohorts | `spec-anomaly-7-as-entities-cohorts.md` | B | ready (dep 6) | 5, 6 | per-ASN entities + AS-origin cohort rarity, reading `fe.SrcAS` (zero new imports). Degrades to prefix cohorts when flowexport/AS is absent. |
...
| A-2 | `ddos/observe` is a faithful template for `anomaly/observe` | `ddos/observe/store.go` | observe child grows unplanned scope | read store.go (agent 1) | **partially** — skeleton (registration, ring, subscribe, open/finalize) transfers; but the template has NO query surface/web card (`list()` is test-only, `store.go:92`), so those are greenfield; incident struct is dest-tuple (`store.go:16-20`) vs anomaly's source-prefix (`event.go:71`) |
