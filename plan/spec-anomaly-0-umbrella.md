...
- [ ] `docs/architecture/core-design.md` - component/plugin registration model every child plugs into
  → Constraint: children register via `init()` in `register.go`; core discovers via registries, never imports plugins directly. Each child self-contains (`ai/rules/plugin-self-containment.md`) — remove it and all its CLI/schema/doctor surface vanishes.
...
- [ ] `internal/plugins/ddos/observe/{store.go,register.go,store_test.go}` - the incident-store SKELETON Phase-A child `observe` reuses (registration, ring, subscribe, open/finalize). NOT its surface: `list()` is test-wired only (`store.go:92`), there is no show handler or web card.
...
- [ ] `internal/plugins/flowexport/enrich/enricher.go:44` - `Lookup(addr) (ASEntry, bool)` — the AS-origin source. It stays INSIDE flowexport; child 6 stamps AS onto the facts surface (a direct import from `detect` fails `ze-tier-check`, `dep_audit.py:486-506`).
- [ ] `internal/component/bgp/reactor/reactor_api_batch.go:28` - `AnnounceNLRIBatch(sel, batch)` — the family-agnostic announce API. Reachable only via `ctx.Reactor()` in a BGP command handler, NOT from a system plugin's `ze.EventBus`; child 8 needs a plugin→reactor seam first.
...
- The facts pipeline is uniformly **1-second** (`trafficstat/service.go:36`,
  `trafficfeature/service.go:25`, `anomaly/detect/register.go:107`). Any sub-second signal
...
- The FlowSpec **wire codec** works (SAFI 133/134 + traffic-action communities are wire-tested,
  ...
  **no new BGP-family work**. But the plugin→reactor **seam** is missing: `shape` holds only
  `ze.EventBus` (Emit/Subscribe), FlowSpec origination is stubbed (`ddos/flowspec/responder.go:28`,
  "cp-survival-4 not yet wired"). Child 8 is blocked on that seam; classic RTBH is the shortcut.
...
- [ ] `internal/component/trafficstat/service.go` - aggregator ticks `tickInterval = time.Second` (service.go:36,269), subscribes `observation.Feed`, exposes `Snapshot()`.
  → Constraint: 1s is the fundamental cadence of the whole facts→judgment chain.
- [ ] `internal/component/trafficfeature/service.go` - neutral feature service, 1s ticker (service.go:25,187), `Snapshot()` surface consumed by the detector.
- [ ] `internal/plugins/anomaly/detect/register.go` - detector runs its own `time.NewTicker(time.Second)` (register.go:107) then `d.onTick(svc.Snapshot())`, scores, emits `anomaly-detect` events + a bounded in-memory recent-incident ring surfaced by `show anomaly detect`.
  → Constraint: the ring is in-memory only — no persistence, no query-by-time. Phase-A `observe` adds that.
- [ ] `internal/plugins/anomaly/shape/responder.go` - `registerTables = firewall.RegisterTables`, `applyAll = firewall.ApplyAll` (responder.go:41-42); `match.go:22 buildActions` builds `firewall.Action`s. Responder action is **local firewall only** — no upstream/flowspec path (grep confirms none).
- [ ] `internal/plugins/flowexport/enrich/enricher.go` - `Lookup(addr netip.Addr) (ASEntry, bool)` (enricher.go:44) returns origin-AS enrichment; lives in the flowexport plugin (cross-plugin access needed for AS cohorts).
- [ ] `internal/component/bgp/reactor/reactor_api_batch.go` - `AnnounceNLRIBatch` (reactor_api_batch.go:28) announces NLRI (incl. FlowSpec SAFI 133) at runtime today.
...
4. **Response (1049, shipped):** the `shape` responder subscribes to events, installs shadow-first local firewall terms.
   - **widen (Phase C, blocked):** an upstream FlowSpec/RTBH action gated by the same state machine — requires a plugin→reactor announce seam that does not exist yet (`shape` holds only `ze.EventBus`).
...
| A-2 | `ddos/observe` is a faithful template for `anomaly/observe` | `ddos/observe/store.go` | observe child grows unplanned scope | read store.go + show.go (child-3 research) | **corrected (was partially)**: skeleton transfers AND the template NOW has a live query surface (`show.go:14-25` registers `ze-show:ddos-status`/`ze-show:ddos-incidents`; `handleShowDdosIncidents` calls `s.list()` at `show.go:60`), so the earlier "no query surface" note is stale (see Mistake Log). Remaining divergences: dest-tuple incident (`store.go:13-24`) vs source-prefix (`event.go:71`), `ddosevent` vs `anomalyevent`, single-node show, still-dead `sweepStale` ticker (`store_test.go:105`) |
| A-3 | The flowexport enricher is reachable from the anomaly domain without a layering violation | `enrich/enricher.go:44` in a sibling plugin | as-cohorts needs a new shared seam | `module-tiers.md` + `dep_audit.py` (agent 4) | **broken** — a direct `detect → flowexport/enrich` import fails `make ze-tier-check` (`dep_audit.py:486-506`: flowexport is an engine, the import flips `engine_depended`). Sanctioned path: stamp AS onto the core `observation`/`trafficfeature` surface at the flowexport producer → child 6 |
| A-4 | Upstream FlowSpec response needs only an origination seam + `shape` action, not new BGP family work | `AnnounceNLRIBatch` (reactor_api_batch.go:28) | child 7 grows scope | agent 5 (announce path + reachability) | **partially** — "no new BGP family" CONFIRMED (`test/encode/flow.ci` proves the codec + traffic-action communities); but the plugin→reactor seam does NOT exist (`ze.EventBus` is Emit/Subscribe only, `eventbus.go:33-73`) and FlowSpec origination is stubbed (`ddos/flowspec/responder.go:28`). RTBH is the shortcut once the seam lands |
...
| A-7 | Children 5 (dest/port) and 7 (per-ASN entities) can emit incidents on the existing event contract + responder unchanged | umbrella framed both as facts/detector work | dest incident filters the victim; port/ASN cannot be a `netip.Prefix` | read `event.go:69-93`, `responder.go:77-107`, `match.go:36-44` (child-5/7 research) | **broken**: `Entity` is `netip.Prefix` (`event.go:71`) so a port/ASN needs an additive `EntityKind` tag; the responder acts on `e.Entity` as a source with no guard (`responder.go:81,90,102`) so a dest incident throttles the victim and an invalid prefix poisons `r.armed`. Fix owned by child 5 (additive discriminator + source-only guard), reused by child 7; non-source incidents stay report-only |

### Risks
| ID | Risk | Early signal | Mitigation / fallback |
|----|------|--------------|----------------------|
| R-1 | Entity-matrix (child 5) explodes tracked-entity memory (per-source + per-dest + per-port) | `ze_anomaly_tracked_entities` climbs; eviction churn | the 10000 `maxTrackedKey` cap exists per map (`detector.go:30`, `feature.go:23`); child 5 decides per-dimension vs shared cap and sizes the ceiling |
...
| AC-2 | Phase-A child `interop-harness` complete | `spec-anomaly-4-interop-harness.md` closed: a test-only `fakeflow` plugin + one `.ci` drives facts→judgment→response through the daemon and asserts an armed responder |
...
| AC-6 | Phase-C child `upstream-response` | `spec-anomaly-8-upstream-response.md` remains blocked and documents its prerequisite (the plugin→reactor announce seam / cp-survival-4); if the seam lands, a classic-RTBH action fires under the responder state machine with auto-revert |
...
7. **child 8 `upstream-response`** — only after the plugin→reactor announce seam (cp-survival-4) exists; start with classic RTBH.
...
- **Sub-second beaconing is out of reach until a sub-second collector exists** (verified: the
  facts pipeline is 1s end to end at `trafficstat/service.go:36`, `trafficfeature/service.go:25`,
  `anomaly/detect/register.go:107`). Child 9 documents the prerequisite; not implemented against 1s.
- **Upstream response is blocked on a plugin→reactor announce seam, not on BGP capability.** The
  FlowSpec codec is wire-tested, but `shape` holds only `ze.EventBus` (`eventbus.go:33-73`) and
  FlowSpec origination is a stub (`ddos/flowspec/responder.go:28`). Child 8 needs the seam
...
|------------------|---------------|----------------|--------|
| Upstream FlowSpec response is "ready(dep)" because `AnnounceNLRIBatch` exists | The reactor API is real, but `shape` can't reach it (`ze.EventBus` is Emit/Subscribe only, `eventbus.go:33-73`) and FlowSpec origination is a stub (`ddos/flowspec/responder.go:28`) | deep read of the announce path + plugin reachability (agent 5) | child 8 reclassified back to **blocked** on the cp-survival-4 seam; RTBH noted as the shortcut |
| The flowexport enricher can be imported by `anomaly/detect` for AS cohorts | A direct import fails `make ze-tier-check` — flowexport is an engine and the import flips `engine_depended` (`dep_audit.py:486-506`) | read `module-tiers.md` + the gate code (agent 4) | AS cohorts split into child 6 (stamp AS onto the facts surface, tier-safe) + child 7 (consume it) |
| Entity-matrix is mostly a detector re-key | Only sources carry a feature vector; dest/port/ASN need new FACTS in `trafficfeature` (`feature.go:129-208`), and ASN needs IP→AS that doesn't exist in the feed (`observation.go:54-60`) | read the facts layer (agent 3) | child 5 rescoped to FACTS-layer work; ASN moved behind the AS-enrichment prerequisite |
...
| `ddos/observe` has no show handler and `list()` is test-only (`store.go:92`) — earlier finding, now stale | It NOW registers a live show surface (`show.go:14-25`) and calls `s.list()` in production (`show.go:60`); the template evolved since the umbrella was written | child-3 research (2026-07-02) reading `show.go` | A-2 corrected to "corrected"; child 3 justification shifts to source-prefix key + still-dead `sweepStale` ticker; the store skeleton + a show surface both transfer |
| Children 5 and 7 are pure facts/detector work on the existing event contract + responder | Non-source entities (port, ASN) cannot be a `netip.Prefix` `Entity` (`event.go:71`), and the `shape` responder acts on `e.Entity` as a source with no guard (`responder.go:81,90,102`) | child-5 and child-7 research (2026-07-02) reading `event.go`, `responder.go`, `match.go` | added A-7; child 5 owns an additive `EntityKind` discriminator + a responder source-only guard; child 7 reuses it, so child 7 truly depends on child 5's contract; dest/port/ASN incidents stay report-only |
...
- [ ] Child 8 (upstream-response) closed OR its plugin→reactor announce-seam dependency documented as the blocker
