...
- Confidence attaches to an incident via a single NEWEST-FIRST scan scoped to the interface, matching the first active incident whose target equals the characterized victim OR is still empty (`observe/store.go`): a victim resolved only from the flow ring (IPv6-only, invisible to trafficusage; or trafficusage lag at confirm) still records its confidence, and a stale resolved-target incident from a prior attack can no longer steal the score from the current one. Empty-target fallback is kept so `finalize` (which also carries an empty target) matches.
...
- **The BPS trigger was blind on multi-interface boxes.** `onRates`/`onRate` bound `maxBps` to whichever interface had the max PACKET rate, so an amplification flood (high-BPS, moderate-PPS) on a non-top-PPS interface was never compared to the BPS baseline -- exactly the case the trigger exists for, since low-PPS makes the attacked link unlikely to be the top-PPS one. Fixed by tracking peak PPS and peak BPS (and their interfaces) independently, and attributing `attackIface` to the BPS interface when only the BPS path fired. Single-interface QEMU tests passed throughout; the gap was multi-interface only (found by the closure review gate, guarded by `TestApplyTick_BpsTriggerFiresOnNonTopPpsInterface`).
- **Resolved-target incidents linger Active.** `AttackCleared` carries an empty target and `finalize` matches by `DstPrefix`, so a `finalize` never clears an incident that was opened with a resolved victim -- it stays Active until `sweepStale`. That let the confidence exact-match attach to a stale prior incident instead of the current one; the newest-first interface-scoped scan (above) works around it. The deeper fix (finalize by interface, or Cleared carrying the resolved target) is untaken.
