...
Agents were unable to CLOSE finished work. `ai/rules/planning.md` said
...
- Tested DEPENDENCY, never causation, for an out-of-scope finding. `ai/rules/completion.md` already asks whether the goal still holds if you leave it. A defect this change did not introduce is in scope the moment the work depends on that path.
...
- `ai/rules/planning.md` owns the disposition tests. Four skills reference them and none restates them.
...
- **The first prescribed home was deleted by the closure it authorised.** Findings were sent to the spec's deferral shard, which `ai/rules/planning.md` `git rm`s at closure.
...
- `ai/rules/planning.md` -- "Bounding the loop", "State the review effort before you spend it"
