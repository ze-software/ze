...
| Peer-down retention must be established before the RIB handles the down event; delivery order alone is not proof of GR readiness | `internal/component/bgp/server/events.go`: `sortByReverseDependencyTier`, `onPeerStateChange`; `internal/component/plugin/process/delivery.go`: `sendBatch` | Historical rationale (350): bgp-gr must mark routes stale before bgp-rib acts; OPEN preceded state-up in the recorded FSM flow. Current dispatch attempts each delivery sequentially in reverse dependency-tier order and waits for its result. DirectBridge calls the handler synchronously; the SDK pipe path acknowledges after its callback returns. A topology error falls back to name order, and a delivery error does not stop fan-out. The 2026-09-08 sender-facts handoff records a retention failure that this ordering mechanism alone cannot settle; current GR readiness needs evidence that retention took effect before RIB release. |
...
| An interop scenario's NAME does not say which role ze plays in it | `test/interop-ipsec/scenarios/` | `eap-tls` has ze as the EAP PEER, not the authenticator: its `ze.conf` carries `connection-type initiate` and strongSwan's `swanctl.conf` carries `remote { auth = eap-tls }`. A comment on `indicateSuccess` (`internal/core/eap/eap_tls.go`), which runs only on the authenticator side, cited that scenario as its proof for two versions. Read the pair of config files, never the directory name. (spec-fixit-eap-tls-escape-hatch-kills-the-daemon) |
