...
Implementation-phase audit (mandatory `ai/rules/evidence.md` read-the-producer
...
- **Pivoted from a daemon-boot audit to two in-process Go regression guards**
  ...
  - **Built-ins are 100% YANG-derived.** `LoadBuiltinsWithAliases`
    (`internal/component/plugin/server/command.go`, sibling `LoadBuiltins:53-66`)
    sets each built-in's dispatch name from `wireToPaths[reg.WireMethod]` and *skips*
    any handler with no YANG path. Production call `server.go` from
  ...
  - **Plugin commands are rejected at registration.** The only two writers to the
    plugin command registry (`command_registry.go` Register, `:319`
    RegisterDeprecated) both run `validateCommandName` first, which
    calls `grammar.CheckName` + `command.IsVerb`; on violation
...
- **Feeder 3 = two deterministic unit tests** in
  ...
  - `TestRuntimeBuiltinSurfaceGrammar` iterates `AllBuiltinRPCs()`, skips
    ...
    the live RPC could not do: `Completion` (`command_registry.go`) strips the
...
- Feeder 3 runs as a normal unit test (no daemon, no config, no `.ci`, ~0.03s), so it
  ...
  `startup_autoload.go`, startup is `ConfiguredPaths`-gated) cannot exist.
...
- `ai/rules/cli.md` Feeder table + `scripts/checks/cli_grammar.go` header
...
**Docs:** `ai/rules/cli.md` (Feeder 3 row + rationale), `scripts/checks/cli_grammar.go` (header comment).
