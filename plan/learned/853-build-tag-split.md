...
- Chose three positive tags (`ze_linux`, `ze_appliance`, `ze_setup`) over a single `ze_device`/`ze_setup` pair, because appliance and provisioning are independent concerns that operators may want separately.
...
- Chose `ze_linux` for the self-update backend gate over a separate `ze_selfupdate` tag, because self-update is a property of the on-device binary, not an independent feature.
...
- `make build` produces four binaries: `ze` (ze_linux), `ze-appliance` (ze_appliance), `ze-setup` (ze_setup), `ze-stripped` (no tags). Dev builds get all features via `ze_linux,ze_appliance,ze_setup`.
...
- Files named `*_linux.go` get an implicit GOOS=linux build constraint from Go. The ze_linux tag file is named `setup_features_distro.go` to avoid this.
...
- Build tag tests that assert negative presence ("appliance NOT registered") fail when all tags are combined. Used compound constraints (`ze_linux && !ze_appliance && !ze_setup`) so each test only runs in its intended single-tag scenario.
...
- Created: `cmd/ze/setup_features_distro.go` (ze_linux), `cmd/ze/setup_features_setup.go` (ze_setup), `cmd/ze/setup_features_appliance.go` (ze_appliance)
...
- Modified: `internal/component/config/system/backend_ze.go`, `selfupdate.go`, `selfupdate_test.go`, `backend_test.go` (changed `!ze_stripped` to `ze_linux`)
- Modified: `internal/component/config/system/backend_ze_minimal.go` (changed `ze_stripped` to `!ze_linux`, updated messages)
...
- Modified: `Makefile` (added bin/ze-appliance, bin/ze-setup targets; bin/ze uses ze_linux; bin/ze-stripped uses no tags)
