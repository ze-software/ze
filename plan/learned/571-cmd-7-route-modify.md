...
- Deferred export modify over implementing it, because the egress `PolicyFilterChain` call in `reactor_api_forward.go` discards the delta (underscore placeholder). Wiring it requires the same `textDeltaToModOps` + `buildModifiedPayload` integration that the import path has at `reactor_notify.go`.
...
- The export path silently discards modify deltas. A future spec needs to wire `textDeltaToModOps` + `buildModifiedPayload` into `reactor_api_forward.go` (same pattern as `reactor_notify.go`).
