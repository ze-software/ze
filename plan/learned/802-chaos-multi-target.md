...
- `cmd/ze-chaos/fork.go` (modified: forkDaemon for temp-file launch, nil-safe Shutdown)
- `cmd/ze-chaos/main.go` (modified: --application, --binary flags, target-aware config/fork)
- `cmd/ze-chaos/orchestrator_run.go` (modified: single-port dialing with per-peer source addr)
