...
- `internal/component/bgp/yang/ze-bgp-conf.yang` (remove tcp/cache augments, add announce-delay)
- `internal/component/hub/yang/ze-hub-conf.yang` (remove api/debug/daemon legacy leaves, add pprof + exabgp/api/ack)
...
- `internal/component/config/cli/cmd_validate.go` (validateEnvironment simplified)
- `internal/test/cli/cmd_bgp.go` (SIGTERM via cmd.Cancel, rename env var)
- `internal/test/cli/cmd_peer.go` (rename ze.bgp.tcp.port -> ze.test.bgp.port)
...
- `docs/guide/debugging-tools.md` (remove tcp.attempts trick)
