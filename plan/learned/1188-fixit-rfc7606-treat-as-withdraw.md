...
- **AC-8 (two-family), per D-3/D-8: one withdraw body PER MP family, not one merged body.**
  `SynthesizeWithdrawFamilies` (`rfc7606_withdraw.go`) emits a primary body (legacy IPv4
...
- **AC-8 forwarding: the extra bodies need a REAL cache entry, NOT an empty BufHandle.**
  ...
  path `reactor_notify.go`, or the rs plugin's `ForwardCached`), not by the RIB. An empty
  BufHandle fails the cache gate (`reactor_notify.go` needs `buf.Buf != nil`), so the
  second family was never forwarded to RS clients AND `ForwardUpdatesDirect` logged a false,
  attacker-triggerable `"BUG: msgID missing from cache"`. Fix (`session_read.go`): dispatch
  the extra body with `BufHandle{ID: noPoolBufID, Buf: extra}`. `noPoolBufID` (`bufmux.go`,
  `^uint32(0)`) is the existing sentinel for a non-nil Buf NOT backed by a pool slot, so
  `ReturnReadBuffer` (`session.go`) no-ops on return (no double-free, no pool slot
...
- **AC-9 (non-negotiated), per D-5: negotiation-aware synthesis + early drop.**
  `mpFamilyDispatchable` (`session_validation.go`) mirrors `validateUpdateFamilies`'
  ...
  family. If synthesis is empty, `processMessage` (`session_read.go`) restarts the
...
- **Synthesis moved OUT of `enforceRFC7606` into `processMessage`.** `enforceRFC7606`
  (`session_validation.go`) now only classifies/logs; synthesis is negotiation-aware and
