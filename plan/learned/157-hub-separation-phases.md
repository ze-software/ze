...
- `internal/component/hub/` is a thin entry point composing existing `internal/component/plugin/` infrastructure (SubsystemHandler, SchemaRegistry, Hub) — avoids duplicating forking/pipe code.
...
- BGP code move (`internal/bgp/*` → `internal/component/bgp/*`) required updating 239 import references across 126 files. Budget 2-4 hours for this kind of wholesale package relocation.
- `internal/rib/` (BGP engine's peer-to-peer routing) and `internal/component/plugin/rib/` (adj-RIB tracking plugin) are DIFFERENT packages with similar names. The former moves with BGP; the latter is a separate process.

## Files

- `internal/component/hub/hub.go`, `config.go`, `orchestrator.go` — hub process entry point
