...
- **Running `make ze-verify-fast` after growing a struct past 128 bytes** is the only way to catch `rangeValCopy`. The linter does not flag growing types retroactively; only iterators over the newly large type get complained about.
