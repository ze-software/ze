...
- The `inactive:` prefix is preserved around canonicalization: `inactive:prefix-list:CUSTOMERS` becomes `inactive:bgp-filter-prefix:CUSTOMERS` so the chain's `inactive:` skip logic (in `filter_chain.go`) still works.
