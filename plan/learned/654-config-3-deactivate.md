...
- **AC-12 positional-list rejection**: lists with all-leaf children (`nlri`, `nexthop`, `add-path`) skip schema-injected inactive in `yang_schema.go`. The CLI verb detects this case via `listNode.Has(InactiveLeafName)` and rejects with a message pointing the user at the parent container. Without the explicit reject, `SetValue(path, "inactive", "true")` would silently store an unknown leaf.
...
- `PruneInactive` runs at every documented apply site (`loader.go`, `editor.go`, `bgp/config/peers.go`, `cmd_validate.go`); the new leaf branch is invoked from inside `pruneNode` via `tree.pruneInactiveLeaves()` so callers do not change.
