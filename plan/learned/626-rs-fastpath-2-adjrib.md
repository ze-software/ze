...
- **Constraint for future plugin authors:** when declaring an `OptionalDependencies` entry, the owner MUST provide a run-time fallback for absence. bgp-rs's one-shot WARN + skip pattern is documented in `ai/rules/plugins.md` as the reference template.
...
- `ai/rules/plugins.md` -- `OptionalDependencies` table row + "Optional Dependencies" section (graceful-fallback pattern).
