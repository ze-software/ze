...
the phase boundaries `ai/rules/planning.md` defines, and a session that
had just written code was the same context asked to review it, which
`ai/rules/planning.md` forbids. Thomas set the orchestration shape on
2026-07-28: use agents for spec work, main thread supervises only, drive each
phase through its `ze-*` skill.

## Decisions

- New rule `ai/rules/planning.md` (blocking) over a section inside `ai/rules/planning.md`: the two share a trigger but answer different questions (WHO executes vs WHICH model), and folding them would have changed model-selection's identity.
...
- Subagent reports are treated as claims, not evidence: the rule requires the main thread to confirm a cited `file:line` before acting on a finding or relaying it, per `ai/rules/evidence.md`.
...
- `ai/rules/planning.md` gained a back-pointer in its Subagents section; the two rules are read together at every phase boundary.
- Nothing enforces this mechanically. Like `planning.md`, the session is the enforcement point, so it lives in `CONDENSED.md` where every session loads it.
...
- `CLAUDE.md` and `AGENTS.md` are generated from `ai/INSTRUCTIONS.md` and are gitignored. The "Before You..." row had to be added to the canonical source, then `make ze-ai-instructions` run; editing `CLAUDE.md` directly is blocked by `ai/rules/repo-maintenance.md`.
...
- `ai/rules/planning.md` (new)
- `ai/rules/planning.md` (back-pointer in Subagents)
- `ai/INSTRUCTIONS.md`, `ai/INDEX.md` (discovery rows)
- `ai/rules/INDEX.md`, CONDENSED.md (regenerated; that digest was deleted 2026-08-03)
