...
**Owner instruction, 2026-08-04:** extend the RFC gate beyond MUST so it also
covers SHOULD, including the lowercase `should` a pre-RFC-2119 document uses.
...
**The measurements, taken 2026-08-04 rather than estimated.**

| Figure | Value | Source |
|--------|-------|--------|
| Requirements extracted | 4618 across 175 summaries | `ai/RFC-REQUIREMENTS.md` header |
| MUST-level declared | 3034 | same |
| MUST-level enrolled and gated | 2949 | same |
| `[SHOULD]` rows already extracted, gated by nothing | **1053** | `grep -ho "\[SHOULD\]" rfc/short/*.md` |
| `[MAY]` rows, excluded here by the constraint below | 699 | same |
| `[SHALL]` / `[REQUIRED]`, already gated | 76 / 5 | same |

So the work is NOT extraction for the most part: a thousand SHOULD rows are
already authored and sitting inert. The gate does not look at them.

**The one line that decides it.** `GATED_LEVELS` in
`scripts/dev/rfc_requirements.py` is `{MUST, MUST NOT, SHALL, SHALL NOT,
REQUIRED}`. Every enrolment, coverage and ratchet check keys off that set.
Widening it is one edit and a very large consequence, which is the whole reason
this is a spec rather than a patch.

**The lowercase half is the harder half, and it is what prompted this.** RFC 4271
Section 9.1.2 says "the BGP route **should** be excluded from the Phase 2
decision function" in lowercase, and that obligation was extracted by nobody
until 2026-08-04, despite Ze implementing it in
`internal/component/bgp/reactor/filter/loop.go`. A keyword-only sweep would
never have found it. The machinery already knows this problem exists for
MUST-level: the report writer in `rfc_requirements.py` prints an uppercase and a
lowercase column for MUST/MUST NOT/SHALL/SHALL NOT, and says the lowercase form
is "the only form a pre-RFC-2119 document has". Nothing equivalent exists for
should.

**What a SHOULD gate must do differently from the MUST gate, and this is the
design question.** A MUST is binary: covered or a `{gap}`. A SHOULD is a
judgement Ze is entitled to decline, so the gate has to distinguish three states
rather than two, and the third one is the point:

1. implemented and proven, exactly like a MUST;
2. **deliberately declined, with a recorded reason** -- the state that does not
   exist today and that carries the value;
3. nobody looked.

A gate that collapses 2 into 3 will be routed around within a week. A gate that
lets 3 masquerade as 2 is worse than no gate, because it launders silence into a
decision. `ai/rules/rfc-compliance.md` already voids prior answers that point
away from full compliance, so the declined state needs an owner and a date, not
an annotation anyone can write.

-> Constraint: do NOT widen `GATED_LEVELS` as the first step. The first step is
deciding what a declined SHOULD looks like on disk and who may write one. The
one-line edit is the last step, not the first.

-> Constraint: `[MAY]` is excluded. 699 rows of optional behavior would drown
the signal, and an unimplemented MAY is not a deviation.
