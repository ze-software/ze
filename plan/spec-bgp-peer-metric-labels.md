...
`internal/component/bgp/yang/ze-bgp-conf.yang`, reaching `PeerSettings.Name`),
the peer's advertised hostname (`extractHostnameCapabilities`,
`internal/component/bgp/plugins/hostname/hostname.go`), and the ibgp/ebgp
distinction. No metric carries any of the three.

Ze's native metrics are also all-or-nothing. `telemetry/prometheus/enabled` gates
the whole registry, and per-collector selection exists only for the Netdata OS
collectors, whose own YANG says it does not affect Ze-native metrics.

Goal: add optional peer labels for description, advertised hostname, peer type
and peer group, each behind an explicit opt-in leaf, because every added label
...
`advertised-prefixes`, `accept-filtered-prefixes`), each opt-in, for the same
cardinality reason.
