...
| Who else touches this path? | `spec-fixit-ike-resource-lifetime-leaks`, closed 2026-08-22, which resolved the `sendDPD` request-window row on 2026-08-07, and `spec-fixit-ike-test-discrimination`, closed 2026-08-15, which added `test/ipsec/ipsec-dpd-holds-tunnel.ci` to assert a healthy tunnel outlives more than one DPD interval. `plan/spec-ipsec-dataplane-inspection.md` owns the SAD read surface. Another agent was editing `internal/component/ike/engine/dpd.go` on 2026-08-07 |
