# Spec: bgp-pcap-decode-real-capture-fixture -- prove the reader on bytes a network produced
...
`spec-bgp-pcap-decode` built the pcap reader and the TCP reassembler in
`internal/core/pcap` and closed on 2026-09-08. Its Files to Create named one
artifact it did not produce: "a pcap fixture captured from a real BGP session,
for the reader tests". Every fixture the suite holds is synthesized, by
`buildTCPCapture` (`internal/core/pcap/reassemble_test.go`), by the writer in
`internal/core/pcap/frame.go`, or by hand.

That is R-3 of the closed spec, in its own words: "The reader works on ze's own
files and fails on a tcpdump capture from a real network." Closure narrowed the
risk without removing it. A capture written by scapy 2.7.0 -- a foreign pcap
writer, Ethernet link type, an ephemeral client port, one message split over two
segments, a duplicate retransmission, and two messages in one segment -- decodes
correctly through `ze bgp decode pcap`, observed 2026-09-08. What no test has
seen is bytes a kernel and a network produced: MSS-sized segmentation, TCP
options that push the data offset past five words, timestamps that are not
round, and the reordering a path actually delivers.

This host cannot produce one. `tcpdump -i lo` answers "You don't have permission
to perform this capture on that device (socket: Operation not permitted)" as the
unprivileged user every session runs as, so the capture has to come from the
interop lab, from a privileged run, or from a public capture archive.

What this spec owes: one capture of a real BGP session committed as a fixture, a
`.ci` that decodes it and asserts the message count and kinds, and a Go test over
whichever reader branch that capture reaches first. TCP options are the likely
one: `dissectTCP` (`internal/core/pcap/parse.go`) handles a data offset above
five words and nothing has ever handed it one.
