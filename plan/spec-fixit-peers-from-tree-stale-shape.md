...
(`internal/component/bgp/plugin/register.go` and `:227`). Only the reactor's own
...
(`reactor_api.go`) selects it whenever `reloadFn == nil || configPath == ""`,
which a stdin config (`ze -`) produces, because
`internal/component/bgp/config/loader.go` guards BOTH setters on
