...
(`internal/component/bgp/reactor/reactor_api.go:703`) before reaching any other leaf.

**Producer.** `parsePeersFromTree` (`internal/component/bgp/reactor/reactor_api.go:701-782`)
expects `remote`, `local`, `receive-hold-time` and `router-id` DIRECTLY under the
peer. `grouping peer-fields` (`internal/component/bgp/yang/ze-bgp-conf.yang:290-805`)
nests them as `connection > remote`, `session > router-id`,
`timer > receive-hold-time` -- which is what the real pipeline reads
(`internal/component/bgp/config/config.go:50-51` and `:144`).

**Reachability.** The live entry point is the config transaction
(`internal/component/bgp/plugin/register.go:208` and `:227`). Only the reactor's own
hand-built test trees use the flat form, so nothing user-facing exercises this path
today -- but it is not dead code either: `loadPeersFullOrTree`
(`reactor_api.go:454`) selects it whenever `reloadFn == nil || configPath == ""`,
which a stdin config (`ze -`) produces, because
`internal/component/bgp/config/loader.go:93-96` guards BOTH setters on
...
**Goal.** One peer shape. Either `parsePeersFromTree` reads the nested YANG shape and
its tests are migrated with it, or the function is deleted and
