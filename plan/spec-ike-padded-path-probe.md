...
|-------|--------|----------|
| No bypassed layers (data flows through the intended path) | No | to fill at implementation: the MTU module reaches the engine only through `ikeprobe`; the engine builds through the SK builder and sends through the transport; no caller writes the socket directly |
| No unintended coupling (components stay isolated) | No | to fill: `mtu/cmd` imports `internal/core/ikeprobe` and `internal/core/probe`, never `ike/engine`; `ikeprobe` imports `internal/core/probe` for `DFMode` only, verified by `./le tier check` |
| No duplicated functionality (extends existing, does not recreate) | No | to fill: retransmission is `serviceRequestRetransmit`, the window is `msgid.go`, the DF option mapping and the error-queue parse are `internal/core/probe`'s, exported rather than copied; the search is `searchPathMTU` |
| Zero-copy preserved where applicable (refs, not copies) | No | to fill: the Notify writes through `PayloadNotify.WriteTo` into the SK builder's buffer; the DF-clear copy re-sends `sa.requestMsg` without a second build |
| Registration over hardcoding, outbound: new commands, views, families, and handlers register, and the core discovers them. No per-feature field, switch case, or factory is added to a core/shared package (`ai/rules/plugins.md`) | No | to fill: the engine registers into `ikeprobe` at `init`; `mtu/cmd` discovers the prober through the leaf; a build without the engine answers `ErrNotRegistered` and no switch in `mtu/cmd` names the engine |
| Registration over hardcoding, inbound: no existing switch, seed map, validator, parser, runner, help string, or completion table has to learn this feature's name. Evidence names every list that was searched for the names this feature introduces, and the registry each one now derives from (`ai/rules/principles.md`) | No | to fill: lists to search for the new Notify type and the `prober` value: `notifyTypeNames` (`payload_notify.go`, the private-use type gets a name there because it is Ze's own), `logIgnoredNotifies` (`inbound.go`), the `show mtu` YANG help and the pipe renderers (`prober` is a payload key, so no completion table learns it), the interop scenario map (`checkers.go`, keyed by directory name) |
...
|------|------|-----------|--------|
| `TestPaddedInformationalReachesRequestedSize` | `internal/component/ike/engine/probe_test.go` | the emitted datagram is exactly the size asked for, on both sockets, the marker counted on 4500 and not on 500 | |
| `TestProbeCarriesOneStatusNotifyOnly` | `internal/component/ike/engine/probe_test.go` | the decrypted request holds one payload, a Notify of the private-use type with SPI size 0, and nothing else | |
| `TestProbeNeverTouchesDPDState` | `internal/component/ike/engine/probe_test.go` | a probe response with the probe's id changes no `dpdState` field and credits no liveness; a DPD response never answers a probe | |
| `TestProbeUnansweredFailsTheSALikeDPD` | `internal/component/ike/engine/probe_test.go` | a probe holder unanswered after every retransmit and `requestWindowTimeout` takes the teardown path DPD takes: `StateDead`, `cleanupChild`, `errSADeletedByPeer` from `maintainSA`; the reply channel carries `sa-failed`; no window is released without a response and `sa.NextMsgID` is not rewound | |
| `TestProbeAnsweredOnDFClearCopyDoesNotFailTheSA` | `internal/component/ike/engine/probe_test.go` | a probe whose DF copy draws nothing and whose DF-clear copy is answered frees the window, answers `too-big`, leaves the SA established and `dpdState` untouched, and the next DPD is answered | |
| `TestProbeRetransmitIsBitwiseIdenticalWithDFClear` | `internal/component/ike/engine/probe_test.go` | the second copy is `sa.requestMsg` byte for byte and leaves through the DF send with the clear mode | |
| `TestProbeSizeCeiling` | `internal/component/ike/engine/probe_test.go` | a size above min(interface MTU, 3000) is refused by name before anything is built | |
| `TestZeResponderIgnoresThePrivateStatusNotify` | `internal/component/ike/engine/probe_test.go` | `handleInformationalOwned` answers a request carrying only the private-use Notify with an empty response and logs it as ignored | |
| `TestProbeAdvancesMsgIDLikeDPD` | `internal/component/ike/engine/probe_test.go` | `sa.NextMsgID` advances by one per probe, in the order `sendDPD` uses (reserve, send, arm, advance) | |
| `TestProbeRefusedWhilstRekeyPending` | `internal/component/ike/engine/probe_test.go` | a pending rekey, a rekey hold, a held window, and an SA below `StateEstablished` each answer a distinct named refusal | |
| `TestProbeOutcomeZeroIsUnspecified` | `internal/component/ike/engine/probe_test.go`, `internal/core/ikeprobe/registry_test.go` | the zero `Outcome` prints as unspecified and is never returned by a completed probe | |
| `TestErrQueueTooBigTriggersDFClearAtOnce` | `internal/component/ike/engine/probe_test.go` | a too-big event for the probe's peer sends the DF-clear copy before the retransmit timer; one for another peer does not | |
| `TestIKEProbeConfirmsThenDescends` | `internal/component/mtu/cmd/run_test.go` | the IKE prober confirms the ICMP figure (size passes, size+1 fails) and descends the ladder only when confirmation fails; the exchange budget of 16 ends the attempt | |
| `TestIKEProbeUnregisteredIsNotSilence` | `internal/component/mtu/cmd/run_test.go` | `ErrNotRegistered` selects ICMP with a note; it is never counted as a silent probe | |
| `TestIKEProbeSAFailedEndsTheRun` | `internal/component/mtu/cmd/run_test.go` | an `sa-failed` outcome ends the IKE attempt for that tunnel at the first size (`ikeProbeStopAtFirstSilence`), no further size is asked, the row keeps the ICMP figure with `prober: icmp` and an `unconfirmed` note naming `sa-failed` and the size; the IKE prober is never asked above the ICMP figure | |
| `TestMeasurementRowNamesTheProber` | `internal/component/mtu/cmd/mtu_test.go` | every measurement row carries `prober`, the IKE row carries no optimism caveat, the non-NAT IKE row carries the different-path caveat | |
| `TestIKEProbeRegisterTwicePanics` | `internal/core/ikeprobe/registry_test.go` | the leaf refuses a nil and a second registration, as `ipsecinventory` does | |
| `TestDFSendRestoresSocketMode` | `internal/component/ike/transport/udp_test.go` | after a DF send the socket's `IP_MTU_DISCOVER` reads its prior value (Linux build tag) | |
...
|------|----------|-------------------|--------|
| `show-mtu-ike-probe` | `test/plugin/show-mtu-ike-probe.ci` | Ze-to-Ze over the compiled fixture `plugin/clamped-path` (`internal/test/fixture`), ICMP errors filtered so only IKE can measure; the figure equals the clamp and the row says `prober: ike`; `option=needs-linux:caps=net-admin,net-raw` | |
...
| Functional test for new RPC/API | Yes | `test/plugin/show-mtu-ike-probe.ci` |
| Pipe completeness | Yes | the payload gains `prober`; `show-mtu-json.ci` already renders `| json`, and the new `.ci` asserts the key through the same pipes (`ai/rules/cli.md`) |
...
| 17 | Existing docs show config/CLI/API examples for this area? | Yes | `docs/guide/ipsec.md` and `docs/architecture/diagnostics/path-mtu.md`: the `show mtu` examples gain the `prober` key |
