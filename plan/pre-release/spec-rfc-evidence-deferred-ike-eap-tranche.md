# Spec: rfc-evidence-deferred-ike-eap-tranche
...
Bind the IKE and EAP unit-only cluster at `functional/verify` through the
...
one had been unit-green for months, and every one was found only by running
against strongSwan. That is the highest measured defect density in the repo.
The cause is the shape the selection rule is keyed on: Ze's producer and Ze's
verifier are the same code path, so an assertion about one is satisfied by
construction from the other.

**Why it was not taken in the source spec.** A concurrent session was reviewing
`internal/component/ike/**`, and editing under a live review costs more than it
buys. Check whether that review has landed before starting.
...
- Run the oracle test as a cheap SCAN across each RFC before picking it. Ranking
  by protocol family is measurably weaker than ranking by oracle.
...
- [ ] `rfc/short/rfc7296.md` - the bulk of the cluster
  → Constraint: fill at design time, per requirement picked.

**Key insights:** (minimal context to resume after compaction)
- The `ipsec` suite is named in `all_suites`, so a `.ci` there earns `functional/verify`.
...
### Entry Point
- Fill at design time: the datagram or command each landed test uses to reach the daemon.
...
| Who else touches this path? | Any session working `internal/component/ike/**` |
...
|------|----------|-------------------|--------|
| Fill at design time | `test/ipsec/*.ci` | Fill at design time | |
...
1. **Phase: Wiring (MANDATORY FIRST)** -- prove a `.ci` tag in `test/ipsec/` resolves to `functional/verify`
   - Tests: `carrier_for("test/ipsec/...")`
   - Files: a draft `.ci` in `test/draft/`
   - Verify: the carrier resolves, and the scanner accepts the tag
2. **Phase: Scan** -- apply the selection rule's oracle test across the cluster's RFCs before picking
...
|-------|-----------------|
| Input validation | the tests send unauthenticated datagrams to a loopback listener; no new exposure |
...
Backfill the IKE and EAP unit-only cluster (254 gated MUSTs: rfc7296 211, rfc3748 12, rfc5216 10, rfc2759 9, rfc4301 8, rfc4303 3, rfc7427 1) at `functional/verify` via the `ipsec` suite
