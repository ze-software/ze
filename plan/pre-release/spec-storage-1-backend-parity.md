...
- [ ] `ai/rules/repo-maintenance.md` - discovery, `./le arch fs-persistence`, generated site and wiki
...
- [ ] `internal/component/cli/sshclient/client.go` `ResolveDBPath` (10 callers: init, connect x4, client x3, cli/client x2; all `DefaultConfigDir`) and the `zefs.Open` at its use
...
| R-5 | The `./le arch fs-persistence` gate goes red on a dead allowlist entry after init's rename moves into the storage package | the gate's `ErrDeadAllowlistEntry` | the entries for `init/main.go`, `ze_core_autoinit.go`, `local/cmd_install.go` are removed in the same change |
...
| AC-21 | `./le arch fs-persistence check` on the finished tree | green; a fixture that adds `zefs.Open(<dir>/database.zefs)` in a component is refused by the extended detector with the rule's name; the A-4 side stores are its allowlist |
...
→ Decision (AC-4): ancestors are not checked; the root's 0700 and owner check bound the store (owner, 2026-09-17). `zefs.OpenDirectory` keeps its no-follow walk and drops the writable-ancestor and ancestor-owner refusals.
...
| `TestStatestoreThroughStorage` | `internal/component/config/storage/statestore_test.go` | AC-18 | |
...
- `internal/component/cli/sshclient/client.go` - `ResolveDBPath` becomes `ResolveStoreDir` (config path, `ze.config.dir`, default); the open goes through `storage.Open`
...
| No live-store open outside the storage package | `./le arch fs-persistence check` green, and `grep -rn "zefs\.Open\|zefs\.Create" cmd internal --include=*.go \| grep -v _test \| grep -v config/storage` lists only the A-4 side stores and seed builders |
