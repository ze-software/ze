...
|---|---|---|
| `rfc4301` | 30 sites unclassified: 12 met-and-undeclared, 18 unmet | `plan/immediate/spec-rfc4301-architecture-gaps.md`, and the owner held the 12 |
...
- [ ] `rfc/short/rfc7296.md`, `rfc/extraction/rfc7296.json` - the largest sign-off, and
  ...
  → Constraint: twelve sites relocated by owner ruling D-1 (2026-07-31) to
    `plan/immediate/spec-ipsec-remote-access.md` and `plan/spec-ipsec-ipcomp.md`. A relocation
...
| `rfc4301` | its Status cell was lowered from `Supported` to `Partial` on 2026-08-30, by owner decision, after the Tier 1 walk found architecture obligations Ze does not meet across ICMP processing, BYPASS and DISCARD fragments, DF and PMTU, PAD matching, SPD ordering and the outer-header DSCP | `plan/immediate/spec-rfc4301-architecture-gaps.md`, whose closing phase implements the obligations, raises the row and lands the sign-off |
...
| Who else touches this path? | `plan/pre-release/spec-followup-rfc-enrollment.md` owns `rfc/enrolled.txt` and the coverage rollup and is the destination of both live rows in the retired deferral shard "rfcgate-0-umbrella". Any concurrent `/ze-rfc` run edits `rfc/short/`. `plan/journal/concurrent-rfc-gate-stale.md` records the shared-checkout failure mode |
...
- the retired deferral shard "rfcgate-6-supported-extraction-signoff" - the shard named in the
  metadata table
...
- **It does not walk the `Partial` and `Experimental` rows.** 64 rows read `Partial` and
  ...
  remain covered by row D4 of the retired deferral shard "rfcgate-0-umbrella".
- **It does not arm `rfc/drain-budget.txt`.** This spec leaves the rate alone; `spec-fixit-rfc-drain-quota-never-armed` owns arming it. Arming it is a one-line
  ...
  that decision. Row D5 of the retired deferral shard "rfcgate-0-umbrella" stays live.
...
row D5 of the retired deferral shard "rfcgate-0-umbrella" wants measured. Tier 4 is where that
