...
- [ ] `internal/component/bgp/reactor/peer.go` - `pendingSync` is the fact `request quiesce` waits on, and is the fence shape that DID hold
...
|----|-----------|--------------------------------|----------|--------------|--------|
| A-1 | One mechanism explains most of the population | 15+ distinct cases, almost no repeats, one load variable | the repair is per-case after all, and the spec becomes a survey | the fix removing the loss at 20, not at 6 | unvalidated |
