...
- The precedent is COMPLETE, not partial. `./le test-health update` regenerates `test/health/latest.json`, `./le test-health check` gates it for staleness, and the check is a prerequisite of `./le repository generated-check`. Its the native action tables under `internal/le/` comment states the property that makes this work: "Output is a pure function of committed state -- no wall-clock value".
...
- [ ] `internal/le/` native action tables - `./le test-health update` / `./le test-health check`, the complete precedent
...
- `fmt_exact` staying exact for the RFC counts, which `internal/le/rfc/rfc.go` compares against the repository.
...
- `./le test-health update` / `./le test-health check` - the target-plus-gate pair to copy
...
|----|------|--------------|----------------------|
| R-1 | The committed JSON goes stale and the site publishes yesterday's repository | A number visibly behind the tree | A staleness gate on the `./le test-health check` model, wired as a prerequisite of `./le repository generated-check` |
| R-2 | The staleness gate itself walks the working tree, reintroducing the defect it exists to catch | The gate disagrees with itself between two sessions in one checkout | It runs against a COMMIT, the way `./le verify worktree` does |
...
| the staleness gate in `./le repository generated-check` | → | the facts staleness check | `test_staleness_gate_names_the_target_that_fixes_it` |
...
| AC-8 | An RFC count is published | It is exact, unrounded, and agrees with `internal/le/rfc/rfc.go` |
...
| 16 | Any changed source file referenced by existing doc source anchors? | Yes | `docs/architecture/core-design.md` -- declared by `internal/le/register.go` and by every `internal/le/*/register.go` as "le's composition, one import per tool". This spec adds one more tool to that composition and changes nothing about how composition works, so the doc is unaffected. Re-derive with `./le spec citation anchors spec plan/pre-release/spec-site-facts-from-committed-data.md` after the code exists |
...
2. **Regeneration target.** One target a person runs, beside `./le test-health update`, writing every published fact about `main` exactly. It warns on a dirty tree (AC-3), which is the half that stops this becoming instance eighteen.
...
| `count_rfc_requirements` | `ai/RFC-REQUIREMENTS.md`, `rfc/enrolled.txt` | No | Already generated AND staleness-gated. A copy in `repo-facts.json` would be a SECOND record of one fact, which is the drift `inventory_counts` exists to prevent, and AC-8 requires these exact and compared by `internal/le/rfc/rfc.go` |
...
**The the native action tables under `internal/le/` shim routes to the PYTHON `le`, and the Python gate runs the GO
...
this tool lives in `cmd/ze`. Nothing in the the native action tables under `internal/le/` names `cmd/ze` today, so a
...
When the the native action tables under `internal/le/` routes to the Go `le` directly, the two Python rows are
