...
- [ ] `ai/rules/cli.md`, `ai/patterns/cli-command.md` - grammar, offline command structure, map dispatch, owner-registered RPCs
  ...
  → Constraint: values follow their keyword: `ze data backup <file> spare 0`, `ze data restore <file> config|full`, `ze init --from <source> [--sha256 <hex>]`
...
- [ ] `plan/journal/store-serializes-in-process-only.md`
  → Constraint: a blob opened offline is single-process by convention only; `ze config edit --backup` takes an advisory `flock` on the file for the session (unix) and refuses a second editor; on a non-unix build the flag is refused naming the platform
...
- [ ] `pkg/zefs/pwrite_other.go` (`//go:build !unix`), `pkg/zefs/store_test.go` `TestBlobStoreNoFlock` (asserts no `.lock` sidecar)

**Behavior to preserve:** (unless the user explicitly said to change it)
- The blob format byte-for-byte; a blob written with any spare value opens with every existing reader
- Default spare of 10% for every writer this spec does not name, so no existing blob changes shape; a variadic option changes no existing call site
...
### Transformation Path
1. Backup: `Storage.List` over `meta` and `file` recursively → for each key `ReadFile` → `BlobStore.WriteFile` into a new blob opened with the policy (default 0 for this verb) → `Close`. The walk holds the store's write lock so a commit's version and pointer never straddle the snapshot. Live: the same walk inside the daemon; the RPC answers the path, the key count and the size.
2. Restore `config`: `zefs.Check(source)` → read the source's per-name active pointer and version (or its `file/active/<name>` mirror when the source has no pointer) → live: `WriteCandidateVersion` then `PromoteCandidate` and reload, exactly the SIGHUP path; offline: `WriteVersion`, per-name pointers, `file/active/<name>` mirror, under the guard. History gains one version; nothing else changes.
3. Restore `full`: `zefs.Check(source)` → `daemonRunning` refusal → `ImportBlob(source, dir)` into `database.import-tmp/` → equality check → rename the live tree to `database.replaced-<stamp>` → rename the new tree into place. The source is not moved aside (it is the operator's backup, not a seed).
4. `ze init --from`: scheme table (`http`, `https` through the download helper with optional SHA-256; anything else a local path) → `zefs.Check` → refuse when the folder already holds a store → `ImportBlob` → the fetched copy is removed, a local source is left in place. Seeds, backups and a `ze data write`-built blob all import the same way.
5. Offline edit: `storage.OpenBlob(file, Spare(n))` returns the blob-backed `Storage`; `flock` held for the session; the editor, sessions, drafts and history behave as on the tree (conformance table); no ephemeral daemon, no reload notifier; on `Release` the blob flushes with its policy.
...
### Integration Points
- `storage.ImportBlob` (storage-1) - `restore full` and `ze init --from` call it; `backup` is its inverse
...
| A-2 | One percentage governs key and data slots; the container stays exact-fit; no absolute floor | owner: "an option to set the spare size"; review answer 3 accepted | a floor or a container reserve is a measurement-driven later change | `TestSparePolicyBoundaries`, `TestContainerExactFit` | unvalidated |
...
| A-5 | `request data restore ... config` can reuse the SIGHUP candidate path unchanged, with the config bytes coming from the source blob instead of the on-disk file | `stageSIGHUPCandidate` reads bytes then calls `WriteCandidateVersion`; `PromoteCandidate` moves pointers | the RPC would write the source config to the on-disk mirror first and send SIGHUP, which is the same path with one extra write | `TestRestoreConfigLive` | unvalidated |
...
| R-4 | `restore config` of a source whose config name differs from the device's | `TestRestoreConfigName` | the device's name (`meta/instance/name`, `resolve.DefaultConfig`) wins; the source's config is committed under it; the source name is logged |
...
| R-7 | Two offline editors on one backup file | second `flock` fails | the second editor is refused naming the file |
...
| R-10 | The RPC writes a file path the operator chose on the daemon's host | `TestBackupRPCPath` | the path must be absolute and inside a directory the daemon's user owns; the RPC refuses `..`, a symlink and an existing file unless `force` |
| R-11 | `TestBlobStoreNoFlock` asserts no `.lock` sidecar | the test | the lock is on the blob's own fd, no sidecar; a `_unix.go`/`_other.go` pair, the non-unix arm refusing `--backup` |
...
| AC-2 | `ze data backup <file> spare 25` | every key and data slot's capacity is used plus 25%; `spare 101`, a negative or a non-integer is refused naming the range 0 to 100 |
| AC-3 | `ze data backup` while a daemon runs | refused, naming the daemon's SSH address and `request data backup` |
| AC-4 | `request data backup path <abs> [spare N]` on a running daemon | the daemon writes the file under its lock; a commit landing concurrently is in the backup both version and pointer, or neither; the answer carries path, key count and size; a relative path, `..`, a symlink or an existing file without `force` is refused |
...
| AC-7 | `ze data restore <file> config` with no daemon | the source's active config for the device's name becomes a new version, the per-name active pointer moves to it, the previous active becomes rollback, `file/active/<name>` is the new bytes; `meta/auth/*`, `meta/ca/*`, identity, runtime state and every other version are byte-unchanged |
| AC-8 | `request data restore path <file> config` on a running daemon | the same result as AC-7 through the candidate and reload path; the daemon serves the new config; a hub with a client named by the key pushes `config-changed`; a source with no active config for the name is refused naming it |
| AC-9 | `ze init --from <path>` (storage-1's local form) in an empty folder, after the scheme table is added | unchanged: the tree holds the source's keys; `ze start` then serves the seeded config and credentials; the source file is left in place |
| AC-10 | `ze init --from https://host/x.zefs --sha256 <hex>` | the file is fetched with the shared helper, the digest checked, `zefs.Check` run, then imported; the fetched copy is removed; a digest mismatch or a failed check imports nothing and names the reason; without `sha256` the digest step is skipped and the check still runs |
...
| AC-12 | `ze config edit --backup <file>` | the editor opens on the blob with session mode, draft and history; no daemon is probed or started; a commit writes a version, pointers and `file/active/*` into the blob; a second `--backup` editor on the same file is refused |
...
| AC-15 | `zefs.Create(p, Spare(0))`, write a key, close, open, write the same length | exact-fit, and the equal-length write is in place; growing it by one byte is a full rewrite that leaves the file exact-fit again |
| AC-16 | `zefs.Open(p)` or `zefs.Create(p)` with no option | 10% data spare and the 20-byte `file/active/` key spare, today's behavior; every existing blob test passes unchanged |
...
| `TestRestoreFullReplaces`, `TestRestoreFullRefusesCorrupt`, `TestRestoreConfigTouchesOnlyConfig`, `TestRestoreConfigFromMirror`, `TestRestoreConfigName` | `storage/restore_test.go` | AC-5 to AC-8, R-3, R-4 | |
...
- `pkg/zefs/lock_unix.go`, `lock_other.go` (new pair, or the existing `pwrite_*` pair extended) - the advisory file lock for offline edit
...
| CLI grammar (keyword before value) | Yes | `backup <file> spare <n>`; `restore <file> config|full`; `request data backup path <p> spare <n>`; `ze init` stays in the offline `--flag` register (`--from`, `--sha256`) beside `--force` and `--seed` (`ai/rules/cli.md`) |
...
| Data flow | the CLI never calls `zefs.Open` on a live store; the offline blob is reached through `storage.OpenBlob`; the live verbs are daemon RPCs, not local handles |
