...
### Transformation Path
1. Backup: take the store's write guard, then `guard.ListKeys("")` over the whole key space → for each raw key `guard.ReadKey` → `BlobStore.WriteFile` into a new blob opened with the policy (default 0 for this verb) → `Close` → `Release`. The walk runs THROUGH THE GUARD, never through `Storage`: `store.acquire` holds `s.mu` until `Release`, and every `Storage` method takes the same mutex, so a `Storage.ListKeys` or `ReadKey` inside the guarded walk HANGS the process rather than failing. `WriteGuard` has no raw-key method today, so this spec adds the two the walk uses, `ReadKey` and `ListKeys`, on both encodings, each reaching the encoding directly as `guard.Has` and `guard.List` do. `guard.List` is NOT the model to copy: it ends in `immediateChildren`, so it is one level deep, and its blob arm descends `node.walk`, which splits the prefix on `/` and returns nothing for a trailing slash (`"file/"` splits to `["file", ""]` and the empty segment matches no child). The guarded `ListKeys` follows `store.ListKeys` instead, which is the producer of the literal-prefix behavior: enumerate every key, filter by `strings.HasPrefix`, sort. `pkg/zefs` has no `ListKeys` of its own, only `BlobStore.List`, whose answer is in map order. Storage-3 adds `WriteKey` and `RemoveKey` when its object sweep needs them. The walk is RAW and RECURSIVE: `Storage.List(prefix)` returns immediate children only and directories are never entries, so `List("file")` over `file/<stamp>/<name>` returns the empty slice (`immediateChildren`, `internal/component/config/storage/store.go`); and the name-based API resolves an unrecognized prefix into `file/active/` (`resolveKey`, `resolveDirKey`), so a key outside `meta/` and `file/` would be silently renamed. A key namespace added later (storage-3's `object/`) travels with no edit to this walk. The walk holds the store's write lock so a commit's version and pointer never straddle the snapshot. Live: the same walk inside the daemon; the RPC answers the path, the key count and the size.
...
3. Restore `full`: resolve the source to an absolute path and REFUSE it when it is `<configdir>/database.zefs`, before the lock and before any write → `zefs.Check(source)` → `daemonRunning` refusal → the import path in `internal/component/config/storage/import.go` with the source PRESERVED. `ImportBlob` and `ReplaceImportBlob` both retire their source unconditionally (`importOwned` → `finishImport` → `retireSource` renames it to `<source>.replaced-<stamp>`), so neither serves a restore: an operator's backup must still be there afterwards. `importOwned` takes a keep-source mode, and the mode is PERSISTED in the intent as a validated field with a closed vocabulary, because an interrupted restore is replayed from that file alone. An absent or unknown mode is refused, never defaulted to the retiring one. An empty `Archive` is not an option: `readImportIntent` requires `Archive` to start with `<source>.replaced-` and to sit in the source's directory, and `missingTree` turns an unreadable intent into `ErrImportPending`, so an empty one would make every later `Open`, `OpenReadOnly` and `Create` refuse until a person deleted the file. What the mode reaches: the `importIntent` struct, both `Archive` validations in `readImportIntent` (gated on the mode), `importOwned`'s `Archive` construction and its `sourcePath = intent.Archive` fallback (in keep mode an absent source is a hard error, because there is no archive to fall back to), the `retired` argument `importOwned` INFERS today from `sourcePath == intent.Archive` and must instead read from the mode, `resumeImport`'s `retired` parameter, `finishImport`, which skips `retireSource` entirely and only unlinks the intent and syncs, and `retireSource` itself, whose rename of the `<source>.lock` sidecar must not happen either. `resumeImport`'s `equalImport` branch needs no mode: in keep mode the source is always present, so the full key-and-value comparison always runs, which is the stronger check. Everything else of the protocol is reused unchanged: the `database.lock` owner lock, the fully copied stage, `equalImport` over the key set and every value, the durable `database.import-intent` written before anything moves, `RenameNoReplace` publication, and `ErrImportPending` recovery. A caller-side two-rename swap is banned: it leaves a crash window `detect` and `missingTree` cannot classify.
   Every recovery string that names `ze init --from` is DERIVED from the intent's mode, and says `ze data restore <source> full` for a restore: `missingTree`'s two arms, `verifyImportIdentity`'s device and inode arms, and `resumeImport`'s difference refusal. A crashed restore whose recovery text sends the operator to `ze init --from` loses the backup, which breaks AC-5's "the source is unchanged" through the crash path.
   The refusal of `<configdir>/database.zefs` as a source exists because keeping it there bricks the store: `detect` stats that name FIRST, before it looks at the tree, and returns "blob artifact %s is not a live store", so a preserved blob at the live-store name makes every later open refuse beside a perfect tree, and the advice it prints then dies at `importOwned`'s "database already exists". Retiring the source is the only thing that clears that name today.
...
| R-12 | A restore crashes between the intent and the publication, and the operator follows the recovery text | `TestRestoreFullResumeKeepsSource` | the intent carries the mode, so `resumeImport` replays as a restore and the printed recovery command is `ze data restore <source> full`; the backup is still there afterwards |
...
| AC-23 | a `restore full` interrupted after the intent is written | the next `Open` reports the pending import naming `ze data restore <source> full`, not `ze init --from`; running that command finishes the restore, the source is still in place, and the previous tree is `database.replaced-<stamp>`; an intent whose mode is absent or unknown is refused naming the field, and no tree is published |
...
| `TestRestoreFullResumeKeepsSource`, `TestRestoreFullRefusesLiveBlobName`, `TestImportIntentModeRoundTrip`, `TestImportIntentUnknownModeRefused` | `internal/component/config/storage/import_test.go` | AC-22, AC-23, R-12, R-13 | |
...
- `internal/component/config/storage/import.go`, `open.go` - `importOwned` takes a keep-source mode for `restore full`, the intent PERSISTS and validates it, `readImportIntent` gates its two `Archive` checks on it, `resumeImport` reads the mode rather than inferring `retired` from the archive path, `finishImport` skips `retireSource` and its lock move, and every recovery string in `missingTree`, `verifyImportIdentity` and `resumeImport` names the command the mode implies. No new lock file pair is written: the offline editor holds the `<artifact>.lock` sidecar `lockStoreFile` already takes (`open.go`)
...
| 12 | Internal architecture changed? | Yes | `docs/architecture/zefs-format.md` (spare policy), `docs/architecture/storage-backends.md` (artifact roles, the two restore modes, `ze init --from`, and three things the page is SILENT on today that this spec's readers need: `List` never returns a directory, a `Storage` call inside a held guard self-deadlocks rather than failing, and the intent's keep-source mode), `docs/architecture/provisioning/image-server.md` (the seed is exact-fit and importable by `ze init --from`) |
...
| `docs/architecture/storage-backends.md` | Yes | artifact roles, restore modes, `ze init --from`, the guarded raw-key read pair and the self-deadlock rule, the intent's keep-source mode (Phase 3) |
...
3. **Phase: Backup and restore** -- the walks, the two modes, the daemon RPCs
   ...
   - Files: `backup.go`, `restore.go`, `data_rpc.go`, `main_reload.go`, `internal/core/statestore/storage.go` and the guard implementations, the YANG, `docs/architecture/storage-backends.md`, `docs/architecture/api/commands.md`, `docs/architecture/hub-architecture.md`, `docs/guide/operations.md`
