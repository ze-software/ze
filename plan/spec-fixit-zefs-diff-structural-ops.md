...
**Producer.** `Editor.listChangeFiles` (`internal/component/cli/editor.go:919-937`)
...
(`internal/component/config/storage/blob.go:186-204`), which does not surface the
...
ALWAYS in session mode (`internal/component/web/editor.go:102-103` calls `SetSession`
