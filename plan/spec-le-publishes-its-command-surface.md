...
| AC-9 | Every registered le area | Either registers an actions provider, or is named in the migration list, and no other area may register none. Measured 2026-09-12: 89 areas, 63 wired, 26 on the list; after review round 1 migrated `verify status`, 64 wired and 25 on the list. The list ratchets both ways, so an unlisted area with no table fails and a listed area that starts publishing fails until its row is deleted. The guarantee is stronger than publication: every area that publishes a table also DISPATCHES through it. `TestEveryPublishedVerbIsDispatchedThroughItsOwnTable` builds a reference `leaction.Area` from each area's published rows alone, sends the real handler and the reference the same line their shared grammar cannot accept, and requires the same exit code and the same refusal text. Matching `Area.Answer` or `Area.Sweep` passes, so a sweep area is accommodated; an area answering the right code in its own hand-written words fails, because those words are a second parser. There is no exemption list: 81 invocations across `doc wiring`, `functional` and `test-chaos` were red and all three are fixed |
...
| `TestEveryValueSlotRefusesAnOptionWithoutStartingTheRun` | `internal/le/stressrepro/stressrepro_test.go` | AC-5. Review round 5: an area that parses its own line owes the dash refusal, and this one now writes it. It reads the PAYLOAD as well as the code, because a setup failure inside the orchestrator also answers 2 | |
| `TestARequiredKeywordIsPublishedRatherThanNewlyEnforced` | `internal/le/leaction/leaction_test.go` | AC-8. Requiredness is PUBLISHED, not newly enforced, and this is the test that reddens if it starts being enforced | |
| `TestCheckDeclaresTheFileKeywordItEnforces` | `internal/le/ste/ste_test.go` | Review round 5: `check` published zero parameters while it read `file <path>` behind the dispatcher. It asserts an OPTIONAL keyword's publication, so it does NOT hold AC-8; the row above does. Named here as AC-8's until the independent review for closure read both bodies | |
...
- Publishing a table and HONORING it are two different properties, and AC-9 only
  asked for the first. `TestEveryRegisteredAreaProvidesActionsOrIsOnTheMigrationList`
  asks whether a table exists. An area can declare one, register it for the
  manifest, and then read the line with a parser of its own, and five did. The
  manifest then states a grammar the area does not answer to, which is worse
  than publishing nothing: a reader who trusts it is misled rather than merely
  unserved.
  → Decision: a second ratchet,
  `TestEveryPublishedVerbIsDispatchedThroughItsOwnTable`, builds a reference
  `leaction.Area` from each area's PUBLISHED rows and compares the real
  refusal, text and code, against what that reference answers for the same
...
- The hand probe found four areas and stopped; the ratchet found a fifth,
  ...
  least likely to be broken, because it is the one the documentation names. The
  defect was in the help it printed: `all` was appended to the listing and to
  the hint outside the table, so every refusal the area wrote named the three
  tools and left `all` out. Nothing a reader would type reveals that.
  → Decision: the three fixes are the same fix. `doc wiring` asks the table
  which lines already name a verb (`leaction.Area.Holds`) instead of supplying
  `check` to every line, `functional` makes `list`, `gating` and `select` rows
...
## Known Limitations
- The BURN is closed. `stressrepro.parseOptions` refuses a dash-leading value itself, so `./le stress-repro run suite -xh` and `./le stress-repro run suite --help burners 4` each answer 2 and start nothing (measured 2026-09-12). Review round 5 read the producer and found the earlier claim here false: the dash refusal needed no table. The REASON round 5 gave for that, and which this bullet repeated, is itself false, and round 6 caught it: `stress-repro` DOES forward to a child. `shellWords` (`internal/le/stressrepro/run.go`) shlex-splits `suite` and `test` into the child's argv, and `docs/functional-tests.md` ships `suite "bgp plugin --draft"`. The refusal is still correct, for a different reason than the one recorded: it tests the first byte of the WHOLE value, and a quoted value carrying an option inside it does not begin with a dash. A later change that widened the refusal on the strength of the false reason would break that documented recipe. What `plan/spec-le-every-area-dispatches-through-one-table.md` still owes is the PUBLISHED grammar. `stress-repro` declares its own `ActionList` type rather than a `leaction` table, so the manifest carries its description and no keywords, `le stress-repro run --help` renders the area node page rather than the action's grammar, and the dispatcher cannot read its value slots. The bare word `help` stays data in a NON-trailing slot there, because reading it needs the table nobody published: `run suite help burners 4` sets the suite to `help`. A trailing one is answered by the dispatcher and runs nothing.
...
| 6 | ONLY the round-5 fixes and the two areas they touch: the dash refusal in `internal/le/stressrepro` `parseOptions`, `internal/le/ste` now dispatching through the table it declares, and the reworded `List.TrailingWordIsValue` doc, AC-5 and Known Limitations. Plus the eight always-in-scope classes anywhere | `3202ec9aa..70a546d23` | 0 BLOCKER, 1 ISSUE. The dash refusal in `parseOptions` is correct and the reason written above it is not: it said the parser forwards nothing to a child, while `realProcessRunner.Invoke` shell-splits `suite` and `test` into the child's argv and `docs/functional-tests.md:280` ships `suite "bgp plugin --draft"`. Round 6 corrected that sentence in the spec and left the copy in the code, so a later widening of the refusal would read a false justification and break the documented recipe. `ste` now dispatches every word of its line through the table it publishes, proven by `TestCheckIsDispatchedThroughItsOwnTable` driving five refusals through the real `Answer` |

| 7 | ONLY commit `70a546d23`: the ratchet `TestEveryPublishedVerbIsDispatchedThroughItsOwnTable`, the five areas it forced through their tables (`ste`, `worktree`, `doc wiring`, `functional`, `test-chaos`), `leaction.Holds` and `AnswerOrSweep`, the payload-shape change on a one-verb line, and the three weakening ledger rows. Plus the eight always-in-scope classes anywhere | `70a546d23` | 0 BLOCKER, 2 ISSUE. `List.TrailingWordIsValue`'s doc still named `worktree` and `doc wiring` as the two areas whose own parsers owe a dash refusal, and still told the reader that only a hand probe can find the next one; this commit fixed both areas and replaced the probe with the ratchet, and corrected AC-5 in the same breath while leaving the comment. Making the toolchain lazy left `newSession`'s `buildFn` and `warmFn` reading `current.tc` at call time, so a zero `Toolchain` reaches `Prepare` and `warmCITestPackages` as an empty root, and only the statement order inside `suiteRunner` kept it populated. The ratchet itself is sound: it captures the real stderr, its reference bodies panic, and every probe is derived from the published rows. All ten ACs verified MET by reading each named test's body |
| 8 | ONLY commit `b762aff95`: the `toolchain()` call in `newSession`'s `buildFn` and `warmFn`, the rewritten `parseOptions` and `List.TrailingWordIsValue` comments, the count removed from `helpTrailing`, and the AC-8 and round 6/7 record rows. Plus the eight always-in-scope classes anywhere | `b762aff95` | 0 BLOCKER, 2 ISSUE, 3 NOTE. The product fix is correct at both entries and no path now hands a zero `Toolchain` to a consumer, but the commit's claim that no regression test was reachable is WRONG: `Prepare` returns `canonicalBinDir(tc.Root)` before it builds anything under `ze.test.canonical`, so a test driving `newSession(...).binaries()` and asserting the set sits under the checkout root reddens pre-fix in milliseconds. The stale count also survived its own removal: `leroot.RegisterActions`'s doc still said "the seven areas that hand-roll their own dispatch" where `areasWithoutAnActionTable` holds 24, which is the identical defect the commit describes fixing, 120 lines away in the same package. Both rewritten comments state exactly what their producers do |

**Round 8 runs on the owner's authorisation (2026-09-13).** Asked whether a
round was owed over the round-7 fix, given that round 7 found a product defect,
he answered yes. It found the fix correct and two things the fix had missed: a
regression test the commit called unreachable, and the sibling of the very count
it removed.
...
distinct real defect and that the round-5 fix touches `stressrepro` and `ste`,
which no round had reviewed, he answered: run round 6. `./le spec session review
record` needs that on `--owner-authorised`, and the `--rounds-reason` is the
product defect round 5 found: an area publishing a grammar it does not dispatch
through, which answered success while checking nothing.
