...
3. `docs/guide/radius.md`, `docs/features.md:87` (the RADIUS admin AAA row)
...
**This spec RECORDS A KNOWN GAP. It does not report a defect.** `docs/features.md:87`
...
(`internal/component/radius/authenticator.go:96-185`) is PAP-only. Its doc comment at
...
`AttrUserName`, `AttrUserPassword` (`:108`), `AttrServiceType`, `AttrNASIdentifier` —
...
| 5 | Update `docs/features.md:87`, whose row currently names all of this as follow-up work |
...
| O-2 | EAP is multi-round-trip; `aaa.Authenticator` is a single `Authenticate(request) (result, error)` call (`authenticator.go:96`). Does the interface admit an EAP conversation, or does EAP need a different seam? **This is the likely blocker and should be answered before anything else.** | Open |
...
- [ ] `docs/features.md:87` - the RADIUS admin AAA row
...
- [ ] `ai/rules/config-surface.md` - YANG vs env var for any new method selector
...
- [ ] `rfc/short/rfc2865.md` - Access-Request/Accept/Reject; User-Password (5.2), CHAP-Password (5.3), CHAP-Challenge (5.40)
  → Constraint: today only Section 5.2 User-Password is implemented (`authenticator.go:108`).
...
- [ ] `internal/component/radius/authenticator.go` - `radiusAuthenticator.Authenticate` (`:96-185`), doc at `:85-86` states PAP. Builds an Access-Request with `AttrUserName`, `AttrUserPassword` (`:108`), `AttrServiceType` (`serviceTypeLogin`), `AttrNASIdentifier` (`:106-111`), plus `AttrNASIPAddress` (`:113`) when an IPv4 source exists. Sends via `a.client.SendToServers` under an `authBudget` timeout (`:72-83`, `:122-125`). Switches on `resp.Code` at `:132`; `mapProfiles` at `:187`. No CHAP-Password, no EAP-Message, no Accounting-Request
...
2. `radiusAuthenticator.Authenticate` (`authenticator.go:96`) generates a random Request Authenticator (`:97`)
3. It builds an Access-Request carrying User-Name and **User-Password** (`:106-111`) — the PAP-only step this spec would extend
4. `Client.SendToServers` (`:125`) XOR-hides the password per-server per RFC 2865 Section 5.2 and performs ordered failover with per-server timeout and retries
5. On Access-Accept (`:133`), `mapProfiles` (`:187`) maps the configured reply attribute (default Filter-Id, or Class) to profile names; an empty result is a rejection
...
- `mapProfiles` (`:187`) - every method must route through it, so the no-profile rejection cannot be bypassed.
...
- [ ] Registration over hardcoding — a new auth method registers rather than adding a per-method switch case to the aaa chain or a shared core struct (`ai/rules/plugin-self-containment.md`)
...
|----|-----------|--------------------------------|----------|--------------|--------|
| A-1 | PAP-only is intended, not an oversight | `docs/features.md:87` states "CHAP/EAP admin auth and admin-session accounting are follow-up work"; the row is marked `Partial` | This is a defect spec, not a follow-up spec, and priority rises | Read `docs/features.md:87` | unvalidated |
| A-2 | `aaa.AuthRequest` carries a cleartext password, so CHAP can compute its response locally | `authenticator.go:108` puts `request.Password` straight into the packet | CHAP does not fit this seam at all (O-1) | Read the `aaa.AuthRequest` definition | unvalidated |
| A-3 | The single-shot `Authenticate` signature cannot carry an EAP conversation | `authenticator.go:96` returns a final result in one call | EAP is additive and this spec is smaller than feared (O-2) | Read the `aaa.Authenticator` interface | unvalidated |
| A-4 | No operator needs this today | Nothing in the deferrals row or `docs/features.md:87` names a requester | Priority rises and the spec leaves skeleton | Ask Thomas (O-4) | unvalidated |

### Risks
| ID | Risk | Early signal | Mitigation / fallback |
|----|------|--------------|----------------------|
| R-1 | A new method bypasses the no-profile rejection (`:133-149`) and re-opens the admin escalation that rule closed | An Accept with no Filter-Id yields a working admin login | Route every method through `mapProfiles`; test the empty-profile case per method |
...
| AC-3 | Any new method; Access-Accept resolving to no profile | Rejected, exactly as PAP is today (`:133-149`) — the no-profile rejection is not bypassed |
...
## Files to Modify
- `internal/component/radius/authenticator.go` - `Authenticate` (`:96`) gains method selection; the Access-Request build at `:106-111` gains CHAP/EAP attributes
...
|-------------------|---------|------|
| YANG schema (new RPCs/config) | [ ] | `internal/component/radius/yang/ze-radius-conf.yang` — read `ai/rules/config-surface.md` and `ai/rules/config-naming.md` |
...
|---|----------|----------|---------------|
| 1 | New user-facing feature? | [ ] | `docs/features.md:87` |
...
| 16 | Any changed source file is referenced by existing doc source anchors? | [ ] | `docs/features.md:87` already anchors `internal/component/radius/authenticator.go -- radiusAuthenticator.Authenticate, mapProfiles` |
...
| Naming | YANG uses kebab-case (`ai/rules/config-naming.md`) |
| YANG validation | The method leaf is an `enumeration`, not a bare `type string` |
| Registration over hardcoding | Methods do not become a switch case in an aaa core struct (`ai/rules/plugin-self-containment.md`) |
...
| `docs/features.md:87` no longer calls this follow-up work | `grep -n "follow-up work" docs/features.md` |
...
| Credential handling | CHAP-Challenge must come from a CSPRNG, like the Request Authenticator does today (`authenticator.go:97`) |
...
## Known Limitations
- PAP-only, by design and documented at `docs/features.md:87`. PAP over RADIUS hides the password with the shared secret per RFC 2865 Section 5.2 rather than leaving it cleartext on the wire, so PAP-only is a real limitation but not an unguarded one.
