...
benchmark output, so the leftover key IS caught: by `make ze-alloc-check`, which runs
the reactor benchmarks for real and drives `TestAllocGateEnforce` with
`ZE_ALLOC_GATE_BENCH` pointing at the output.

**`make ze-unit-pkg-test PKG=./internal/perf` does NOT catch it.** `TestAllocGateEnforce`
...
in place. Run both targets. Only `make ze-alloc-check` discriminates.
...
- The one reference the compiler cannot see is a string key in `internal/perf/allocgate.go`, and only `make ze-alloc-check` catches it.
...
| R-2 | The `AllocCeilings` string key survives the delete and nothing local notices. | Nothing in `make ze-unit-pkg-test PKG=./internal/perf` fails, while `make ze-alloc-check` reports the benchmark absent. | Run `make ze-alloc-check` explicitly. It is a `ze-precommit-verify` stage and is NOT in `ze-precommit-verify-changed`, so a scoped dev loop misses it. |
...
| R-5 | `rfc/requirements/rfc7911.md` goes stale because a tagged test in `forward_body_test.go` moved, reddening `make ze-rfc-check`. | `ze-rfc-check` reports a stale index or a stale per-RFC file. | Run `make ze-rfc-index-update` and commit BOTH `ai/RFC-REQUIREMENTS.md` and every changed file under `rfc/requirements/` in the same commit (`ai/rules/testing.md`). |
...
| A benchmark registered in `AllocCeilings` no longer exists | → | the fail-closed missing-benchmark branch of `CheckAllocCeilings` | `TestAllocGateEnforce` driven by `make ze-alloc-check`. THE DISCRIMINATOR: leave the string key `"BenchmarkEBGPWireCacheHitParallel"` in the map and this goes RED with "absent from benchmark output". Nothing else in the tree does |

**Proving the third row discriminates (`ai/rules/interop-and-goal-validation.md`).**
Before claiming the delete complete, run `make ze-alloc-check` ONCE with the map entry
deliberately still present and confirm it fails naming that benchmark. Then remove the
entry and confirm it passes. Paste both outcomes. Without that pair the row is an
assertion of absence and proves nothing.
...
| AC-3 | `make ze-unit-pkg-test PKG=./internal/component/bgp/reactor` and `make ze-unit-pkg-test PKG=./internal/perf` | Both pass. The reactor package compiles with the four `recent_cache.go` slot reads gone |
| AC-4 | `make ze-alloc-check` with the `AllocCeilings` entry left in place | RED, naming `BenchmarkEBGPWireCacheHitParallel` as absent from benchmark output. This is the discriminating evidence for AC-2 |
| AC-5 | `make ze-alloc-check` with the entry removed | Green. The six surviving registered benchmarks are all present and within their ceilings |
...
| AC-7 | `make ze-unit-reactor-test-race` | Passes. The eviction path and the `ReceivedUpdate` struct lose shared state, so the race surface shrinks and must not have moved |
...
| AC-10 | `make ze-rfc-check` | Passes. `rfc/requirements/rfc7911.md` pins a tagged test that moves, so `make ze-rfc-index-update` has been re-run and both its outputs committed |
...
| `TestAllocGateEnforce` (existing, unchanged) | `internal/perf/allocgate_test.go` | AC-4, AC-5. The discriminator. Runs only under `make ze-alloc-check` | |
...
- `ai/RFC-REQUIREMENTS.md` and `rfc/requirements/rfc7911.md` - regenerate with `make ze-rfc-index-update` after `forward_body_test.go` changes, and commit both with the change
...
1. **Phase: Wiring (MANDATORY FIRST) -- prove the discriminator before deleting anything.**
   - Tests: `TestAllocGateEnforce`, driven by `make ze-alloc-check`
   - Files: none edited in this phase
   - Verify: re-run A-1's tree-wide grep for `.EBGPWire(` over `*.go` and confirm zero non-test call sites. Do NOT proceed on a stale verdict; if a live caller exists, stop and report it. Then run `make ze-alloc-check` and record that it is green today. That is the baseline the third Wiring Test row compares against
2. **Phase: delete the production symbols in one compiling step.**
   - Tests: `make ze-unit-pkg-test PKG=./internal/component/bgp/reactor` must compile and pass
...
3. **Phase: retire the test callers whose subject is gone.**
   - Tests: `make ze-unit-pkg-test PKG=./internal/component/bgp/reactor`
...
5. **Phase: close the untyped reference and PROVE the discriminator.**
   ...
   - Verify: run `make ze-alloc-check` with the `AllocCeilings` entry STILL PRESENT and confirm it reds naming the absent benchmark (AC-4). Paste that output. Then remove the entry, the `sampleBenchOutput` line and the `want` entry, re-run, and confirm green (AC-5). Paste that too. Without both outputs the absence proves nothing
...
| Untyped reference | `AllocCeilings` lost its string key, and `make ze-alloc-check` was shown RED with it and green without it |
...
| Rule: `ai/rules/testing.md` | `make ze-rfc-index-update` was re-run and BOTH outputs committed, because a tagged test's line pin moved |
...
| The discriminator was exercised in both directions | Two pasted `make ze-alloc-check` outputs, one red with the key, one green without |
| Reactor concurrency is unchanged | Pasted `make ze-unit-reactor-test-race` output |
| The RFC index matches the moved pin | `make ze-rfc-check` green, with `ai/RFC-REQUIREMENTS.md` and `rfc/requirements/rfc7911.md` in the same commit |
...
| `make ze-alloc-check` reds after the delete | The string key survived in `AllocCeilings`. That is the gate working |
| `make ze-rfc-check` reports a stale index | R-5. Run `make ze-rfc-index-update` and commit both outputs |
...
them, so `make ze-rfc-index-update` must be re-run and BOTH `ai/RFC-REQUIREMENTS.md` and
...
- [ ] The discriminator was exercised in BOTH directions: `make ze-alloc-check` pasted red with the `AllocCeilings` entry, and green without it
...
- [ ] Tests FAIL (paste output: `make ze-alloc-check` red with the `AllocCeilings` entry still present)
- [ ] Tests PASS (paste output: `make ze-alloc-check` green after the entry is removed)
