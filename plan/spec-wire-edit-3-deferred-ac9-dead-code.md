...
benchmark output, so the leftover key IS caught: by `make ze-alloc-gate`, which runs
the reactor benchmarks for real and drives `TestAllocGateEnforce` with
`ZE_ALLOC_GATE_BENCH` pointing at the output.

**`make ze-test-pkg PKG=./internal/perf` does NOT catch it.** `TestAllocGateEnforce`
...
in place. Run both targets. Only `make ze-alloc-gate` discriminates.
...
- The one reference the compiler cannot see is a string key in `internal/perf/allocgate.go`, and only `make ze-alloc-gate` catches it.
...
| A-4 | `make ze-alloc-gate` is the only check that catches a leftover `AllocCeilings` string key. | `TestAllocGateEnforce` skips unless `ZE_ALLOC_GATE_BENCH` is set, and `mk/alloc-gate.mk` is the only thing that sets it. Every other test in `internal/perf` reads the `sampleBenchOutput` fixture. | If some other gate catches it too, no harm: the spec still requires `make ze-alloc-gate`. | Run `make ze-alloc-gate` and confirm it reds with the key left in | unvalidated |
...
| AC-3 | `make ze-test-pkg PKG=./internal/component/bgp/reactor` and `make ze-test-pkg PKG=./internal/perf` | Both pass. The reactor package compiles with the four `recent_cache.go` slot reads gone |
| AC-4 | `make ze-alloc-gate` with the `AllocCeilings` entry left in place | RED, naming `BenchmarkEBGPWireCacheHitParallel` as absent from benchmark output. This is the discriminating evidence for AC-2 |
| AC-5 | `make ze-alloc-gate` with the entry removed | Green. The six surviving registered benchmarks are all present and within their ceilings |
...
| AC-7 | `make ze-race-reactor` | Passes. The eviction path and the `ReceivedUpdate` struct lose shared state, so the race surface shrinks and must not have moved |
...
| AC-10 | `make ze-rfc-check` | Passes. `rfc/requirements/rfc7911.md` pins a tagged test that moves, so `make ze-rfc-index` has been re-run and both its outputs committed |
...
| `TestAllocGateEnforce` (existing, unchanged) | `internal/perf/allocgate_test.go` | AC-4, AC-5. The discriminator. Runs only under `make ze-alloc-gate` | |
...
- `ai/RFC-REQUIREMENTS.md` and `rfc/requirements/rfc7911.md` - regenerate with `make ze-rfc-index` after `forward_body_test.go` changes, and commit both with the change
...
1. **Phase: Wiring (MANDATORY FIRST) -- prove the discriminator before deleting anything.**
   - Tests: `TestAllocGateEnforce`, driven by `make ze-alloc-gate`
   - Files: none edited in this phase
   - Verify: re-run A-1's tree-wide grep for `.EBGPWire(` over `*.go` and confirm zero non-test call sites. Do NOT proceed on a stale verdict; if a live caller exists, stop and report it. Then run `make ze-alloc-gate` and record that it is green today. That is the baseline the third Wiring Test row compares against
2. **Phase: delete the production symbols in one compiling step.**
   - Tests: `make ze-test-pkg PKG=./internal/component/bgp/reactor` must compile and pass
...
5. **Phase: close the untyped reference and PROVE the discriminator.**
   ...
   - Verify: run `make ze-alloc-gate` with the `AllocCeilings` entry STILL PRESENT and confirm it reds naming the absent benchmark (AC-4). Paste that output. Then remove the entry, the `sampleBenchOutput` line and the `want` entry, re-run, and confirm green (AC-5). Paste that too. Without both outputs the absence proves nothing
...
| Untyped reference | `AllocCeilings` lost its string key, and `make ze-alloc-gate` was shown RED with it and green without it |
...
| Rule: `ai/rules/testing.md` | `make ze-rfc-index` was re-run and BOTH outputs committed, because a tagged test's line pin moved |
...
| The discriminator was exercised in both directions | Two pasted `make ze-alloc-gate` outputs, one red with the key, one green without |
| Reactor concurrency is unchanged | Pasted `make ze-race-reactor` output |
...
- This spec removes the cache. It adds no gate that would have caught an unreachable exported method at commit time; `make ze-verify-wiring-docs` already owns that surface.
...
them, so `make ze-rfc-index` must be re-run and BOTH `ai/RFC-REQUIREMENTS.md` and
...
- [ ] The discriminator was exercised in BOTH directions: `make ze-alloc-gate` pasted red with the `AllocCeilings` entry, and green without it
...
- [ ] Tests FAIL (paste output: `make ze-alloc-gate` red with the `AllocCeilings` entry still present)
- [ ] Tests PASS (paste output: `make ze-alloc-gate` green after the entry is removed)
