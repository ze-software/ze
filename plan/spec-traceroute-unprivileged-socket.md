# Spec: traceroute-unprivileged-socket
...
A daemon without `CAP_NET_RAW` opens Linux's unprivileged ICMP datagram socket
(`probe.OpenICMP`, `internal/core/probe/socket.go`, kind `SocketDatagram`) when
its group is inside `net.ipv4.ping_group_range`. `show ping`, `monitor ping`
and `resolve ping` run on it, including the `do-not-fragment` modes and the
reported next-hop MTU. `show traceroute`, `monitor traceroute` and
`show probe-round` refuse it by name (`openRawProbeConn`,
`errTracerouteNeedsRawSocket`, `internal/component/traceroute/cmd/traceroute.go`)
and `doctor-icmp-probe-unprivileged` says so.

The refusal exists because the datagram kind delivers only echo replies to the
ordinary read (`net/ipv4/ping.c ping_rcv`): a Time Exceeded from an intermediate
router never reaches the trace loops, so every hop would time out and the trace
would report a path it never saw. The kernel does deliver that Time Exceeded to
the socket's error queue when `IP_RECVERR` is set (`ping_err` calls
`ip_icmp_error`), quoting the echo header, with the router's address as the
offender.

The goal is traceroute on the datagram kind: the three trace loops
(`doTracerouteCtx`, `StreamProbeRound` in `stream.go`, `doProbeRound` in
`probe_round.go`) read hop answers off the error queue when the socket is the
datagram kind, with `IP_RECVERR` installed for `DFOff` too (today
`dfControl` in `socket_linux.go` installs it only for a mode that sets DF), and
the refusal in `openRawProbeConn` goes away. Constraints already known:

- The three loops share one socket construction and one matching rule
  (identifier and sequence off the quoted echo, `probe.QueuedError.Echo`), so
  the error-queue hop reader is written once and called from all three.
- A hop answered off the queue carries `Offender` as its address and the
  `Errno` the kernel derived (EHOSTUNREACH for Time Exceeded); the ordinary
  read path stays for the raw kind, whose Time Exceeded arrives as a datagram.
- `doctor-icmp-probe-unprivileged` (`internal/core/diagnostic/codes.go`) and
  `docs/architecture/diagnostics/active-probes.md` "Bounds and privileges",
  `docs/guide/command-reference.md` and `docs/guide/status.md` state the
  refusal; each is corrected in the same work.
- The proof is a `.ci` beside `test/plugin/ping-do-not-fragment-unprivileged.ci`
  running `show traceroute` from a daemon started under `capsh --drop=cap_net_raw`
  in the same three-namespace path, asserting the router hop and the far hop.

Sourced from `spec-probe-do-not-fragment`, Work Not Done (closed 2026-09-15),
which recorded that traceroute refuses the datagram kind. Whether this spec
runs is the owner's decision: the question was put to him at that closure and
...
### Architecture Docs
- [ ] `docs/architecture/diagnostics/active-probes.md` - the socket kinds, the error queue and the traceroute refusal this spec removes
...
- [ ] `rfc/short/rfc792.md` - Time Exceeded, the message the datagram kind delivers only through the error queue
