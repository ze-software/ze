...
- **Session conversions landed 2026-07-14:** rfc7606-withdraw (EOR-fence, commit 9e2bb1821); remove-private-as-export + remove-private-as-replace-peer (observer waits on receiver `updates-sent`, which increments over the RS fast path per `reactor_notify.go:268`, commit 16cd29d00). Baseline 226 -> 223, each verified 3x+ standalone and concurrently, no flake. These exhausted the remaining CLEAN host-verifiable blind sleeps. Everything left is QEMU-gated (needs-linux bulk), P0-blocked (redistribute), infra-gated (above), a deliberate-window timer (concurrent-config-commit `sleep(3)` IS the concurrency race window), raw-protocol pacing (bfd-* 0.05, exabgp-bridge-sdk, l2tp teardown/show), or an already-deterministic poll loop in a non-api CLI/HTTP driver (cli-commit-*, rbac-web-config-deny) whose sleep paces a real predicate poll and is cosmetic to convert.
