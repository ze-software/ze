...
|------|---------|---------|-----------|--------|--------|
| 2026-08-25 | 0e5cdc05 | fix(cli): keep the selected peer row one color to its end (+26 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | open |
| 2026-08-25 | 0e5cdc05 | fix(cli): keep the selected peer row one color to its end (+21 more) | full ./le verify current mode full over this commit's Go | no full ./le verify current mode full recorded (tmp/ze-verify-full.json is missing) | open |
