...
|------|---------|---------|-----------|--------|--------|
| 2026-08-31 | 3b09fdcb | plan: keep the removed weakened.md rows where they cannot be cleaned (+12 more) | full native verification (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | cleared |
| 2026-09-01 | 3b09fdcb | fix(bgp): name the processes the end-of-rib waits for (+4 more) | full native verification over this commit's Go | no full native verification covers this commit's Go | cleared |
...
| 2026-09-01 | 3b09fdcb | test(bmp): pin the post-policy-only scope decision | discovery-index freshness | the change adds and removes no package: one test in internal/component/bgp/plugins/bmp, which ai/PACKAGE-MAP.md already lists | open |
