...
|------|---------|---------|-----------|--------|--------|
| 2026-08-21 | 99068294 | fix(verify-worktree): prune with --expire now so no entry survives (+1 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | cleared |
