...
|------|---------|---------|-----------|--------|--------|
| 2026-08-24 | d90c683c | fix(config): carry a list's entry order across the plugin boundary (+34 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | cleared |
| 2026-08-24 | d90c683c | fix(config): carry a list's entry order across the plugin boundary (+18 more) | full ./le verify current mode full over this commit's Go | no full ./le verify current mode full recorded (tmp/ze-verify-full.json is missing) | cleared |
