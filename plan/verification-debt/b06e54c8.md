...
|------|---------|---------|-----------|--------|--------|
| 2026-08-24 | b06e54c8 | fix(docs): anchor the web AAA authenticator where it is declared (+6 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | cleared |
| 2026-08-24 | b06e54c8 | fix(iface): refuse SLAAC leaves the vpp backend cannot honor (+4 more) | full ./le verify current mode full over this commit's Go | no full ./le verify current mode full recorded (tmp/ze-verify-full.json is missing) | cleared |
