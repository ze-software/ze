...
|------|---------|---------|-----------|--------|--------|
| 2026-08-25 | 617a32aa | build(le): replace the retired make ze-dev-setup (current: ./le setup) with the le python entry point (+28 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | open |
| 2026-08-25 | 617a32aa | build(le): replace the retired make ze-dev-setup (current: ./le setup) with the le python entry point (+9 more) | full ./le verify current mode full over this commit's Go | no full ./le verify current mode full recorded (tmp/ze-verify-full.json is missing) | open |
