...
|------|---------|---------|-----------|--------|--------|
| 2026-08-20 | 71801542 | journal: a full disk red the gate twice, on a device df did not name | ./le verify current mode full (not FRESH-green) | the gate is being re-run over this tree now; the previous two runs were red only on the full cache device, which is cleared | cleared |
