...
| 2026-09-09 | 6e4caf8d | pppoe: every PADO and PADS carries the mandatory Service-Name tag (+8 more) | full native verification over this commit's Go | no full native verification covers this commit's Go | open |
...
| 2026-09-11 | 6e4caf8d | a failing socket is held by seccomp, and the scenario is gated (+2 more) | full native verification (not FRESH-green) | verify-status is not FRESH-green: STALE: last verify failed (exit=1, at 2026-09-11T04:49:35Z) | open |
...
| 2026-09-11 | 6e4caf8d | retire the closed failing-socket injection-point spec | native structural checks (red) | the same two HEAD reds as the commit above, in internal/le/interoplab/pppoe and internal/le/interoplab/bgp; this commit removes one spec file and touches neither | open |
| 2026-09-11 | 6e4caf8d | every execve drains the crash-capture pipe before it leaves | native structural checks (red) | Both reds are in tracked files this commit does not touch and predate it. tier/check fails on internal/le/interoplab/pppoe/check_ipv6cp.go, check_padr_replay.go and check_service_name.go, which import internal/component/l2tp/pppoe without the ze_l2tp tag. iface-resolution fails on a dead allowlist entry for internal/le/interoplab/bgp/isis_inject_linux.go. All four files are unmodified here and were already red at HEAD before this change. | open |
