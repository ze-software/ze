...
|------|---------|---------|-----------|--------|--------|
| 2026-08-25 | 34f056af | fix(demos): make each transcript state what its demo shows (+10 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | cleared |
| 2026-08-25 | 34f056af | fix(cli): make the selected peer row readable (+5 more) | full ./le verify current mode full over this commit's Go | no full ./le verify current mode full recorded (tmp/ze-verify-full.json is missing) | cleared |
