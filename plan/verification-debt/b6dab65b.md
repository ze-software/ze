...
|------|---------|---------|-----------|--------|--------|
| 2026-08-24 | b6dab65b | feat(website): rework the blog, FAQ, and pipe reference pages (+3 more) | ./le verify current mode full (not FRESH-green) | verify-status is not FRESH-green: STALE: no status file (never verified) | cleared |
| 2026-08-24 | b6dab65b | fix(tests): make the python runner count what each file ran | full ./le verify current mode full over this commit's Go | no full ./le verify current mode full recorded (tmp/ze-verify-full.json is missing) | cleared |
