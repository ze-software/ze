# Spec: BCP 194 child 5 -- route flap dampening position
...
This spec is child 5 of the BCP 194 spec set. The umbrella is
`plan/immediate/spec-bcp194-0-umbrella.md`.

Goal: settle Ze's position on RFC 7454 S7 route flap dampening, then implement
the answer.

Ze implements no route flap dampening. The `damp` occurrences in
`internal/component/bgp/fsm/state.go` and `internal/component/bgp/fsm/fsm.go`
are `EventAutomaticStartWithDampPeerOscillations`. That event is the RFC 4271
S8.1.2 session oscillation mechanism. It acts on a different input, and it is
not route dampening.

S7 does not recommend dampening unconditionally. It recommends against the
RFC 2439 default parameters, and it names RIPE-378 and RIPE-580 as the sources
of usable ones. Absence is therefore a candidate for the conformant answer, and
this spec is the place where that question is decided.

The owner decides before any implementation starts. If Ze implements dampening,
it takes the RIPE-580 parameters as its shipped defaults.
