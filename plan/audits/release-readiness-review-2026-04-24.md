...
| P1-6 | L2TP PPP auth | `internal/component/l2tp/drain.go:89-92`, `internal/component/l2tp/plugins/auth_local/auth.go:42-45` | No auth handler or an empty local user table can accept PPP sessions. | Subscriber sessions may be accepted without valid credentials. | Fail closed unless `allow-unauthenticated` is explicitly configured. |
| P1-7 | RADIUS accounting | `internal/component/l2tp/plugins/auth_radius/acct.go:233-240` | Plugin shutdown cancels accounting sessions without sending Accounting-Stop. | Billing/NAS state can leak online users. | Send bounded Stop records for active sessions on shutdown. |
...
| P2-7 | MCP mandatory params | `internal/component/mcp/tools.go:21-318`, `internal/component/l2tp/yang/ze-l2tp-api.yang:42-170` | Generated MCP schemas lose mandatory YANG inputs. | AI clients see required fields as optional. | Preserve mandatory params or split action tools. |
...
| P2-18 | L2TP pool reload | `internal/component/l2tp/plugins/pool/register.go:88-103,182-193` | Pool reload can forget live allocations. | New sessions can receive duplicate subscriber IPs. | Migrate/reserve live allocations or reject reload with live sessions. |
...
| P2-21 | RADIUS reload | `internal/component/l2tp/plugins/auth_radius/acct.go:53-63,118-120`, `register.go:137-140` | Active interim loops can keep a closed old client after reload. | Interim accounting can stop until teardown. | Indirect sends through current client or restart loops on reload. |
| P2-22 | RADIUS CoA secrets | `internal/component/l2tp/plugins/auth_radius/register.go:155-157`, `coa.go:24-83` | CoA/DM validates only with first server secret. | Backup servers with distinct secrets cannot send valid CoA/DM. | Map source address to shared secret. |
