...
- Read project rules, architecture docs, feature inventory, current security policy, the retired Makefile (current producers: `internal/le/` native action tables), existing audit, and active specs.
...
Current inventory from `./le inventory | json`:
...
| Hot path race | `./le test-unit` reports data races between `session_coalesce.go` writes and plugin reads through Adj-RIB-In during `internal/chaos/inprocess.TestInProcessSpeed`. |
...
| ID | Area | Location | Finding | Impact | First Fix |
|----|------|----------|---------|--------|-----------|
| P2-27 | Default release gate | the retired `Makefile:132-154` (current producers: `internal/le/` native action tables), `cmd/ze-test/*`, `test/static`, `test/traffic`, `test/l2tp-wire` | Default functional gate omits shipped suites and some tests are orphaned. | Release evidence excludes user-facing features. | Define and implement a release evidence matrix. |
| P2-28 | Fuzz inventory | `README.md`, the retired `Makefile:190-235` (current producers: `internal/le/` native action tables), `internal/**/fuzz_test.go` | 57 fuzz funcs exist, but the retired Makefile (current producers: `internal/le/` native action tables) enumerates 43 and one name appears wrong. | `./le fuzz run` is not evidence for documented fuzz scope. | Generate fuzz target list or make drift fail. |
