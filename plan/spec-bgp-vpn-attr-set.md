...
Implement the VPN ATTR_SET path attribute (RFC 6368, type 128) for the
L3VPN PE-CE use case. The attribute carries a CE's attributes inside a PE-PE
VPN update. This is the existing obligation inherited from umbrella item
"GR advanced (L86)" in the 2026-07-06 deferral triage and separated from GR by
`plan/spec-gr-advanced.md` on 2026-07-08. It is transferred here during the
2026-09-19 planning reconciliation; no new feature scope is authorised.

The receiving and sending behaviour, malformed-input handling, integration
with the VPN path and functional/interop proof belong to this spec's design.
Read RFC 6368 and the current VPN attribute producers before choosing the
implementation and acceptance criteria. The July source-absence claim in the
originating spec is historical evidence, not a current absence or conformance
verdict. Preserve existing VPN routing behaviour outside the approved ATTR_SET
change and keep GR Hard Reset and selection deferral in the originating spec.

This skeleton captures the inherited implementation and proof obligation.
It makes no claim that research, implementation or verification has run.
