...
|-------------|---|--------------|------|
| aging tick concurrent with SNP generation | → | `lsdb.Entry` accessor lock discipline | `go test -race -run TestISISDISElection ./internal/plugins/isis/` (currently RED) |
| every exported `Entry` accessor | → | the same discipline | a `-race` test reading each accessor while the aging tick runs |
