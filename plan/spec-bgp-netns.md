...
|------|--------------------|
| `vpp.lcp.netns` defaults to `"dataplane"` | `internal/component/vpp/yang/ze-vpp-conf.yang:199-201` (`leaf netns`, `type string`, `default "dataplane"`) |
| `"dataplane"` is NOT root-reachable | `lcpNetnsIsRootReachable` (`internal/plugins/iface/vpp/doctor.go:136-143`) returns true ONLY for `""`, `"host"`, `"root"` |
| So VPP puts the TAP in `dataplane` | `lcpPairNetns` (`internal/plugins/iface/vpp/lcp.go:109-114`) maps root-reachable names to `""` and passes any other name through unchanged |
| BGP cannot bind there | `RealListenerFactory.Listen` (`internal/core/network/network.go:167-194`) binds via `net.ListenConfig` in whatever namespace the calling thread is in; nothing in the reactor references a network namespace |
...
~~The comment directly above `lcpPairNetns` (`lcp.go:105-108`) states the intent that the YANG
...
**SUPERSEDED, 2026-07-16. The claim was FALSE and its method was the error.** `lcp.go:105-108`
...
|----------|------------------------------|
| The recorded decision: make BGP netns-aware "so LCP TAPs in a non-root netns (`vpp.lcp.netns`) are reachable by BGP **without forcing the operator to a root-reachable netns**" | `plan/deferrals.md:36`, row dated 2026-07-10, source `spec-followup-vpp-iface` A-4. This is the origin row of THIS spec |
| `"dataplane"` is IPng Networks' production convention, copied deliberately | `54bffb83b` ("startup.conf generator **following IPng production template**"); `docs/research/vpp-deployment-reference.md:179-180`: all LCP TAPs are created in the `dataplane` netns, which "isolates the forwarding plane from the management plane" |
| The YANG leaf describes the model, it does not concede a gap | `ze-vpp-conf.yang:199-205`: "Network namespace for LCP TAP interfaces. **Routing daemons run in this namespace.**" |
...
`plan/learned/1098-followup-vpp-iface.md` and in the source comments `doctor.go:15` /
`lcp.go:13` ("see A-4"), it is the ORIGIN spec's A-4 = the BGP-netns deferral = THIS spec. In
...
- [ ] `internal/component/bgp/reactor/reactor.go` - lines 1019-1024 (global listener) and
  ...
  → Constraint: verified 2026-07-16. BOTH call `NewListener(...)` then immediately
    ...
    `NewListener`. So `Listener.SetListenerFactory` (`listener.go:66-68`) is an INTERNAL seam
    only; no production caller outside the reactor can inject a factory into a `Listener`. The
    only external seam is `Reactor.SetListenerFactory` (`reactor.go:553`), which sets
...
- [ ] `internal/core/routewatch/routewatch_linux.go` - lines 12-38: a second in-tree consumer
  ...
  → Constraint: precedent that `internal/core/` already imports this library directly, so the
    core import-direction rule (`ai/rules/module-tiers.md`) is not at issue.
...
- [ ] `internal/plugins/iface/vpp/lcp.go` - lines 105-114: `lcpPairNetns` maps a
  ...
  → Constraint (A-13, 2026-07-16): **"" is NOT VPP's host namespace.** VPP resolves an empty
    ...
    (`startupconf.go:106`, `b.kv("default netns", s.LCP.Netns)`). The comment at `lcp.go:105-108`
...
- [ ] `mk/test-integration.mk` - line 319: `ZE_QEMU_INTEGRATION_PKGS` is computed by
  ...
  → Decision: NEW FINDING, verified 2026-07-16. The QEMU integration package list is
    ...
    `ai/rules/qemu-testing.md` step 4 ("Register the package in the Makefile", showing an
...
- [ ] `ai/rules/module-tiers.md` - core/component/plugin placement by dependency direction
...
- [ ] `ai/rules/config-surface.md` (YANG vs env var) and `ai/rules/config-naming.md`
...
- The netns-bind QEMU rail EXISTS and is auto-discovering (`mk/test-integration.mk:319`). Only
...
- [ ] `internal/component/bgp/reactor/reactor.go` - line 553: `Reactor.SetListenerFactory(f)`
      ...
      `internal/chaos/inprocess/runner.go:169`. Every one is defeated by the MD5/GTSM branch
...
- MD5 (`MD5Peers`) and GTSM (`ListenTTL`) application through `lc.Control`
  (`network.go:169-192`): netns composes WITH them, never at their cost (AC-9).
- `lcpPairNetns` mapping host/root/empty to "" (`lcp.go:109-114`), and passing any other name
  ...
  sets from the same leaf (`startupconf.go:106`). Design against A-13, not this comment.
...
→ Decision: the fork "teach BGP netns" vs "force LCP into the host netns" is resolved in
  ...
  forwarding/management isolation the IPng model exists to provide (`plan/deferrals.md:36`
  asks for reachability "without forcing the operator to a root-reachable netns";
  `docs/research/vpp-deployment-reference.md:179-180`). Teaching BGP is the only option that
...
vpp component config (`ze-vpp-conf.yang:199-201`) and reaches VPP through
`lcp_itf_pair_add_del` (`lcp.go:87-102`). The BGP listen address enters via the bgp config and
reaches `NewListener` (`listener.go:50`). The BGP netns value enters via a new config surface
(shape pending Q3).
...
| Config to vpp component | YANG `vpp/lcp` container, `netns` leaf (`ze-vpp-conf.yang:199-201`) | [ ] read 2026-07-16 |
...
- `checkVPPLCPNetns` / `lcpNetnsIsRootReachable` (`internal/plugins/iface/vpp/doctor.go:100-143`)
...
- [ ] Core import direction: `internal/core/network` gains only an external import
      ...
      (`ai/rules/module-tiers.md`, `make ze-tier-check`)
...
| AC-2 | No netns is configured anywhere (every non-LCP deployment) | Listener behavior is byte-for-byte unchanged; no thread pinning, no new failure mode. `RealListenerFactory{}` with an empty `Netns` takes exactly today's code path (`network.go:167-194`) |
...
| AC-8 | Netns listener on linux | Proven by a QEMU test, not only host unit tests (`ai/rules/qemu-testing.md`). Two rails: an `integration && linux` test (auto-discovered per `mk/test-integration.mk:319`) and a `needs-linux` `.ci` |
...
(`mk/test-integration.mk:319`). Use `t.Skip`, never `t.Fatal`, when the capability is absent
(`ai/rules/qemu-testing.md`). Tests that need no kernel capability (`...EmptyNetnsUnchanged`,
...
| `vpp.lcp.netns` | string, YANG default "dataplane" (`ze-vpp-conf.yang:199-201`) | any existing namespace name; "", "host", "root" mean the host netns (`doctor.go:136-143`) | N/A | N/A. NOT this spec's leaf to change: -> Decision (user, 2026-07-16) the default STAYS `"dataplane"` and no spec changes it. This spec makes BGP reach it |
...
- `internal/component/bgp/yang/` - a netns leaf; read `ai/rules/config-surface.md` and
  `ai/rules/config-naming.md` first. Shape pending Thomas (Q3)
...
|-------------------|---------|------|
| YANG schema (new config) | [ ] Yes, pending Thomas (Q3). Read `ai/rules/config-surface.md` (YANG vs env var) and `ai/rules/config-naming.md` | `internal/component/bgp/yang/` |
...
| QEMU rail | [ ] Yes, MANDATORY (`ai/rules/qemu-testing.md`). The netns-bind rail EXISTS and auto-discovers (`mk/test-integration.mk:319`); the `.ci` rail is `option=needs-linux`. The VPP+LCP end-to-end rail is NOT identified (Q12) | `internal/core/network/netns_integration_linux_test.go`, `test/ui/bgp-listener-netns.ci` |
| CLI commands/flags | [ ] No | - |
| Env var registration | [ ] Only if Q3 resolves to a process-level env setting rather than a YANG leaf | per `ai/rules/config-surface.md` |
...
| 10 | Test infrastructure changed? | [ ] Check. A new `needs-linux` `.ci` and a new integration package are additive, but `ai/rules/qemu-testing.md` step 4 is stale for `ze-qemu-integration-test` (the list auto-discovers, `mk/test-integration.mk:319`) | `docs/functional-tests.md`, `ai/rules/qemu-testing.md` |
| 12 | Internal architecture changed? | [ ] Yes. `network.go:1-12`'s package doc and `listener.go:1`'s `// Design:` anchor both describe listener creation; a namespace concept changes that contract | `docs/architecture/core-design.md` |
...
| 16 | Any changed source file referenced by doc source anchors? | [ ] Grep `docs/` for the changed files. Known: `network.go:7` anchors `docs/architecture/chaos-web-dashboard.md`; `lcp.go:1` anchors `docs/research/vpp-deployment-reference.md`; `doctor.go:1` anchors `ai/rules/doctor-checks.md`. Run `scripts/dev/check_doc_links.py --design-only` | per grep |
...
- `internal/core/network/netns_integration_linux_test.go` - `//go:build integration && linux`.
  Auto-discovered by `ze-qemu-integration-test` (`mk/test-integration.mk:319`), no Makefile edit
...
| Registration over hardcoding | No new per-feature switch case or factory in a core/shared package; the doctor check stays registered from the owning plugin (`ai/rules/doctor-checks.md`, `ai/rules/plugin-self-containment.md`) |
...
- **VPP's linux-cp netns model is TWO-LEVEL, and ze drives both levels from ONE leaf. A-13 is
  ...
  `vpp.lcp.netns` leaf (`startupconf.go:106`). Tracing the reachable cases:
  `validateNetns` (`config.go:131-134`, enforced at `config.go:310-313`) REJECTS an empty
  `netns` when LCP is enabled, so `lcpPairNetns` (`lcp.go:109-114`) returns `""` only for
  ...
  and `lcpItfPair` errors (`lcp.go:99-101`). Note `lcp_set_default_ns` returns 0 even when the
  ...
  host or root" (`doctor.go:124-125`), which is precisely the config that breaks LCP pair
  creation, and `lcpNetnsIsRootReachable`'s doc comment (`doctor.go:133-135`) asserts the same
...
- The deepest lesson here is not about netns. This spec twice argued from a code comment
  (`lcp.go:105-108`) about what the project INTENDED, while the recorded decision sat in
  `plan/deferrals.md:36` and is quoted in this spec's own Origin section. A comment is evidence
...
| `ai/rules/module-tiers.md` axis A (config-driven engine?) | Does it call `sdk.NewWithConn(`? | **PASSES.** No. It is a pure library with no lifecycle: `internal/core/` is correct per the authoring rule ("pure library, no `sdk.NewWithConn`, no plugin lifecycle, no component domain owner") |
...
→ Constraint: one genuine asymmetry to respect. `MD5Peers`/`ListenTTL` are applied via
  `lc.Control` (`network.go:169-192`), a callback on the already-created fd. Netns CANNOT use
...
- **Outbound BGP connections stay in ze's namespace.** Only the LISTENER gains netns awareness.
  `RealDialer` (`network.go:56-117`) would need its own `Netns` field for a peer whose outbound
...
- **The VPP end-to-end rail is not identified.** The netns BIND is provable today
  (`mk/test-integration.mk:319`); proving BGP peering over a real VPP LCP TAP needs a VPP image
...
| # | Question | Answer |
|---|----------|--------|
| 1 | Mechanism for binding inside a named netns from Go? | **ANSWERED (design), pending prototype.** Reuse the in-tree idiom `enterTestNetns` (`internal/test/runner/netns_linux.go`): `runtime.LockOSThread` + `netns.Get` + enter + same-thread `restore()`. `github.com/vishvananda/netns v0.0.5` is already a direct dep (`go.mod:23`) and `internal/core/routewatch/routewatch_linux.go:12` already imports it from core. No new dependency, no new idiom. Copy the shape; do NOT import the test-runner package from production code |
| 2 | Does the socket STAY in the namespace after bind, or is the namespace bind-time only? | **HYPOTHESIS, must be prototyped (A-3, Phase 1).** Believed bind-time only: namespace membership attaches at socket creation, so pinning wraps create+bind and `Accept` then works from any thread. This is a claim about KERNEL semantics, NOT read from ze source, so per `ai/rules/no-fabrication.md` it is labelled unverified and gates Phase 2 via `TestNetnsListenerSocketOutlivesThreadUnpin`. If false, the design changes to a dedicated thread or fd-passing helper (R-1) |
| 3 | Should the netns be a BGP config leaf, or a process-level setting? | **PARTLY ANSWERED. Inheriting from `vpp.lcp.netns` is REJECTED** (couples BGP to VPP, inverts the dependency). A generic BGP leaf is the recommendation. **NEEDS THOMAS:** the exact shape (`bgp { listen { netns } }` vs a process-level setting) and whether it is YANG or env per `ai/rules/config-surface.md`. Note a process-level setting would answer Q5 for free. This is the remaining gate on `ready` |
| 4 | Do outbound BGP connections (the dialer) need the same treatment? | **NEEDS THOMAS / follow-up.** `RealDialer` (`network.go:56-117`) has the same structure and would need a matching `Netns` field for a peer whose OUTBOUND session must egress the TAP. For a listener-only LCP peering story it is not required. Recorded as a Known Limitation rather than silently scoped out |
| 5 | Do web / gnmi / looking-glass listeners need this too? | **NEEDS THOMAS / follow-up.** They do NOT go through `network.ListenerFactory` (`web/server.go:198`, `lg/server.go:346`, `dnsserver/secure.go:315` each build listeners directly), so BGP is genuinely special TODAY. If an operator wants the whole box in the dataplane namespace, a process-level netns (Q3) is a better answer than per-service leaves |
