...
- [ ] `docs/functional-tests.md` - the verify runner's stages, artifacts, and the log paths a waiter would follow
  ...
  → Decision: artifact paths are fixed strings in `internal/le/verify/run.go`; a second concurrent run overwrites them, so job isolation must give each run its own log directory before any two runs are allowed to overlap
...
- The coordination primitive already exists (`internal/le/verifylock/answer.go`) and is wired to 3 targets out of ~15 heavy ones. This is mostly a wiring and admission-policy problem, not a new-mechanism problem.
...
- [ ] `internal/le/verifylock/answer.go` - `flock -o` on `tmp/.ze-verify.lock`, re-enters itself as `__inner__` to write `tmp/.ze-verify.lock.owner` (LABEL, PID, PGID, STARTED, CMD), records elapsed time to `tmp/.ze-verify-duration.txt`, and prints the previous run's duration on acquisition. Interface is `LABEL CMD [ARGS...]`, so it already accepts an arbitrary command.
...
- [ ] `internal/le/verify/run.go` - stage names come from `stagesForMode`; artifact paths (`tmp/ze-verify.log`, `tmp/verify/<nn>-<stage>.log`, `tmp/ze-verify-failures.json`) are package constants, so two concurrent runs collide on them.
...
2. The `make` target's recipe calls `internal/le/lejob/lejob.go <label> <command>`.
...
### Integration Points
- `internal/le/verifylock/answer.go` - `ze-run.sh` is its generalization; the old name stays as a thin alias so no caller breaks.
...
| R-6 | Two concurrent jobs overwrite the fixed artifact paths in `internal/le/verify/run.go`, so a failure index describes the wrong run | a failure summary naming stages the session never ran | per-job artifact directory, with the documented paths published as symlinks or copies once the job completes |
...
phase-1 shape: the public target calls `internal/le/lejob/lejob.go`, and
...
| Builds, and the source-reading Python checks in `internal/le/doccheck/actions.go` | single-threaded, seconds long, and not implicated in the 2026-08-17 freeze. `go build` is bounded by the shared cache |
...
(`stagesForMode` in `internal/le/verify/run.go`), beside `./le verify-lint run` from
phase 1. One code path serves all of them: the nested-job branch of
`internal/le/lejob/lejob.go` execs straight through when `ZE_RUN_JOB` names an
existing entry, and `execStage` (`internal/le/verify/run.go`) extends
...
| `TestConcurrentRunsDoNotShareArtifactPaths` | `internal/le/verify/verify_test.go` | AC-9 | |
...
- `internal/le/verifylock/answer.go` - becomes a thin alias over the generalized wrapper
- `internal/le/verify/run.go` - per-job artifact directory, with documented paths published on completion
...
## Files to Create
- `internal/le/lejob/lejob.go` - the admission wrapper
...
1. **Phase: Wiring (MANDATORY FIRST)** -- the wrapper exists and one target uses it
   ...
   - Files: `internal/le/lejob/lejob.go`, `internal/le/`, `internal/le/` native action tables (`./le verify-lint run` only)
...
3. **Phase: Liveness replaces age** -- stop killing slow runs
   ...
   - Files: `internal/le/lejob/lejob.go`
...
4. **Phase: Attach and share** -- the dedup win
   ...
   - Files: `internal/le/lejob/lejob.go`, `internal/le/verify/run.go`
...
5. **Phase: Artifact isolation** -- concurrency becomes safe
   ...
   - Files: `internal/le/verify/run.go`
