...
- `make ze-verify` - where the new boundary guard runs.
...
| AC-5 | Run `make ze-verify` | A mechanical guard fails the build if any exported `pkg/plugin/**` signature references an `internal/` type, preventing regression |
...
|-------------------|---------|------|
| Boundary guard in verify | [ ] | `scripts/checks/pkg-no-internal-in-api` or `pkg/plugin/boundary_test.go`, wired into `make ze-verify` |
...
4. **Phase: Guard in verify** — wire the boundary guard into `make ze-verify`.
...
| Guard in verify | `make ze-verify` fails on a deliberately-reintroduced leak |
