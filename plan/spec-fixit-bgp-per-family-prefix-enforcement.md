...
|------|-----------|--------------|-----------|------------|
| `teardown` | `ze-bgp-conf.yang:617` | `true` | `PrefixTeardown bool` | `peersettings.go:372` |
| `idle-timeout` | `ze-bgp-conf.yang:622` | `0` | `PrefixIdleTimeout uint16` | `peersettings.go:376` |
| `updated` | `ze-bgp-conf.yang:627` | none | `PrefixUpdated string` | `peersettings.go:381` |
...
(`peersettings.go:364` and `peersettings.go:368`).

All three scalar assignments happen in `parsePrefixLimitFromFamily`
(`internal/component/bgp/reactor/config.go:648`). The assignments are at
`config.go:682`, `config.go:686`, and `config.go:690`. The function is called
once per family from `config.go:620`.

### The mechanism, corrected

The defect is **deterministic**, not random. The enclosing loop sorts the family
keys, then iterates (`config.go:551-557`). The sort exists to keep
Multiprotocol capability ordering stable in the OPEN message. A comment at
`config.go:548-550` states that purpose.
...
(`internal/component/config/schema_defaults.go:49-57`). The `prefix` container
is a non-presence container, so it is created and filled
(`schema_defaults.go:62-79`). A family that omits `teardown` therefore arrives
...
(`config_test.go:1836`), `TestPrefixLimitConfigTeardownFalse`
(`config_test.go:1850`), and `TestPrefixLimitConfigIdleTimeout`
(`config_test.go:1864`). Each declares only `ipv4/unicast`.

One test does exercise two families. `TestPrefixPerFamilyIsolation`
(`session_prefix_test.go:184`) gives `ipv6/unicast` its own maximum and warning,
...
overwrite. Correct the misleading comment at `config.go:680`, which claims
...
- The family loop sorts keys (`config.go:551-557`). Order is stable, so the bug is deterministic.
- YANG defaults are materialized per list entry (`schema_defaults.go:49-57`). An omitting family actively overwrites.
...
- [ ] `internal/component/bgp/yang/ze-bgp-conf.yang:604` - the per-family `prefix` container holding all five prefix leaves
- [ ] `internal/component/bgp/reactor/peersettings.go:361` - `PrefixMaximum` and `PrefixWarning` are maps, the other three are scalars
- [ ] `internal/component/bgp/reactor/peersettings.go:498` - `NewPeerSettings` sets `PrefixTeardown: true` as the direct-caller fallback
- [ ] `internal/component/bgp/reactor/config.go:542` - `parseFamiliesFromTree` sorts family keys, then loops
- [ ] `internal/component/bgp/reactor/config.go:648` - `parsePrefixLimitFromFamily` writes maps for two leaves and scalars for three
- [ ] `internal/component/bgp/reactor/session_prefix.go:370` - `applyPrefixCheck` reads the scalar to decide teardown
- [ ] `internal/component/bgp/reactor/session_prefix.go:340` - `familyString` converts a packed `uint32` family key to the `afi/safi` string
- [ ] `internal/component/bgp/reactor/session_prefix.go:448` - `buildPrefixNotification` already receives the offending family key
- [ ] `internal/component/bgp/reactor/session_read.go:257` - wraps the sentinel error without any family information
- [ ] `internal/component/bgp/reactor/session.go:176` - `ErrPrefixLimitExceeded` is a bare sentinel
- [ ] `internal/component/bgp/reactor/peer_run.go:88` - reads `PrefixIdleTimeout` for the reconnect backoff
- [ ] `internal/component/bgp/reactor/reactor_dynamic.go:141` - copies `PrefixTeardown` from a template for dynamic peers
- [ ] `internal/component/config/schema_defaults.go:41` - `applyChildDefault` fills list entries and non-presence containers
- [ ] `internal/component/bgp/reactor/config_test.go:1836` - the three existing single-family parser tests

**Behavior to preserve:** (unless the user explicitly said to change it)
- The family key sort in `parseFamiliesFromTree` stays. It keeps OPEN capability ordering deterministic.
- A missing or zero `prefix maximum` stays a hard config error (`config.go:651` and `config.go:656`).
- `warning` still defaults to 90 percent of `maximum` (`config.go:677`).
- `warning` greater than or equal to `maximum` stays a config error (`config.go:673`).
- The NOTIFICATION stays `Cease` with subcode `NotifyCeaseMaxPrefixes`, carrying AFI, SAFI, and count in 7 bytes (`session_prefix.go:451-462`).
- The JSON key `prefix-updated` keeps its current shape, one string per peer (`internal/component/bgp/plugins/cmd/peer/peer.go:330`).
- `plugin.PeerInfo.PrefixUpdated` stays a single string (`internal/component/plugin/types_bgp.go:86`).
- Warn-only mode still drops excess NLRI instead of installing it (`session_prefix.go:415`).
- The exponential backoff shape for repeated prefix teardowns stays, capped at one hour (`peer_run.go:96-103`).
...
- The comment at `config.go:680` stops claiming per-family behavior that the code does not deliver.

## Data Flow (MANDATORY - see `ai/rules/data-flow-tracing.md`)

### Entry Point
- Operator config text, parsed to a `map[string]any` tree, with YANG defaults materialized by `ApplyDefaults` (`schema_defaults.go:16`).
- Format at entry: nested maps. Every leaf value is a string, including booleans and integers.

### Transformation Path
1. `ApplyDefaults` fills `teardown` and `idle-timeout` into every family entry that lacks them (`schema_defaults.go:49-57`).
2. `parseFamiliesFromTree` collects family keys, sorts them, and loops (`config.go:551-557`).
3. `parsePrefixLimitFromFamily` writes two maps and three scalars (`config.go:667` through `config.go:691`).
4. `PeerSettings` reaches the session and the peer.
5. `applyPrefixCheck` reads the scalar teardown flag on overflow (`session_prefix.go:409`).
6. On teardown, `session_read.go:257` returns the sentinel error with no family attached.
7. `peer_run.go:88` reads the scalar idle-timeout to size the reconnect delay.

### Boundaries Crossed
| Boundary | How | Verified |
|----------|-----|----------|
| Config tree → PeerSettings | string map values coerced in `parsePrefixLimitFromFamily` | Yes, read `config.go:648-693` |
| PeerSettings → Session | struct field read in `applyPrefixCheck` | Yes, read `session_prefix.go:399-416` |
| Session → Peer (teardown cause) | `ErrPrefixLimitExceeded` sentinel, no family payload | Yes, read `session.go:176` and `session_read.go:257` |
| PeerSettings → dynamic peer | field copy from template | Yes, read `reactor_dynamic.go:141` |
| Reactor → plugin JSON | `plugin.PeerInfo.PrefixUpdated`, key `prefix-updated` | Yes, read `types_bgp.go:86` and `peer.go:329-331` |
| Reactor → report bus | staleness raise and clear from `PrefixUpdated` | Yes, read `reactor_peers.go:152-158` |

### Integration Points
- `familyString` (`session_prefix.go:340`) already bridges the packed `uint32` key to the `afi/safi` string used by the maps. The teardown branch is a cold path, so the conversion cost is permitted there.
- `prefixConfigLookup` (`session_prefix.go:320-336`) is the existing per-family lookup pattern. New per-family reads MUST follow it, and MUST NOT invent a second one.
- `buildPrefixNotification` (`session_prefix.go:448`) already receives the offending family key. The family is available at the teardown decision point.
...
| Registration over hardcoding, per `ai/rules/plugin-self-containment.md`. New commands, views, families, and handlers register themselves. The core discovers them. No per-feature field, switch case, or factory is added to a core or shared package | Yes | No new command, view, or family. Families already come from the registry via `family.LookupFamily` (`config.go:602`). |
...
|----|-----------|--------------------------------|----------|--------------|--------|
| A-1 | Only `session_prefix.go` reads `PrefixTeardown`, and only `peer_run.go` reads `PrefixIdleTimeout` | Original brief | Fix misses a caller and leaves a stale scalar read | Grep, already run | **broken**. `PrefixTeardown` is also copied at `reactor_dynamic.go:141` and logged at `session_prefix.go:406`. `PrefixIdleTimeout` is read only at `peer_run.go:88` and `peer_run.go:95`, plus set in `session_prefix_test.go:325` |
| A-2 | `PrefixUpdated` has a narrow surface, so making it per-family is cheap | Field sits in the same three-line block | Scope grows into JSON and report-bus surfaces | Grep for every reader | **broken**. Five readers: `reactor_api.go:137`, `reactor_api.go:860`, `reactor_peers.go:152`, `session_prefix.go:577`, `peer.go:330`. It also crosses to `plugin.PeerInfo` (`types_bgp.go:86`) |
...
| A-4 | No existing unit test asserts cross-family prefix enforcement | Grep of `config_test.go` and `session_prefix_test.go` | Duplicate or conflicting coverage | Grep, already run | **confirmed**. All three parser tests declare only `ipv4/unicast` (`config_test.go:1839`, `:1853`, `:1867`) |
| A-5 | YANG defaults reach every family entry, so a family that omits the leaf still carries `teardown true` | `schema_defaults.go:49-57` list recursion, non-presence container fill at `:62-79` | The overwrite only happens on explicit disagreement, which narrows the bug but does not remove it | Read `ApplyDefaults`, then a parser test with one family omitting the leaf | **confirmed** by reading. Still needs the test to pin it |
...
| A-7 | `idle-timeout 0` means no reconnect, as the YANG description states | `ze-bgp-conf.yang:625` says "0 = no reconnect" | The documented contract and the code disagree, and one of them is a separate defect | Read `peer_run.go:86-103` in full, then test the zero case | unvalidated. A read of `peer_run.go:88` suggests zero selects the **normal** reconnect path, not "no reconnect". Settle this before you write the AC-4 test |
...
| A-8 | The family that triggers teardown is knowable where the reconnect delay is chosen | `buildPrefixNotification` receives the family key (`session_prefix.go:448`) | Per-family `idle-timeout` is not implementable without new plumbing | Trace the error path from `session_prefix.go:411` to `peer_run.go:88` | **broken as written**. The family is known at the decision point but is dropped. `ErrPrefixLimitExceeded` (`session.go:176`) carries no family, and `session_read.go:257` adds none |

### Risks
| ID | Risk | Early signal | Mitigation / fallback |
|----|------|--------------|----------------------|
| R-1 | A `map[string]bool` for teardown fails **open**. A miss yields `false`, which disables protection for any family whose key is absent or misspelled | A family overflows and no NOTIFICATION is sent | Never read the map directly. Add an accessor whose absent case returns enabled. Drive the accessor from the session entry point, per `ai/rules/fail-closed-guards.md` |
| R-2 | Dynamic peers built from a template share the template's map instead of owning a copy. One peer's parse or mutation then leaks into siblings | Two dynamic peers change enforcement together | Copy the maps in `reactor_dynamic.go:141`, do not alias them. Add a test with two dynamic peers |
...
| R-6 | The fix is correct but the test still cannot see it, because it asserts aggregate behavior | The test passes against unfixed code | Mutation-verify. Revert the parser fix and confirm the new test goes red, per `ai/rules/functional-test-gate.md` |
| R-7 | The `rfc-tagged-test` hook BLOCKS the edit this fix needs. `TestPrefixExceedDrop` writes `ps.PrefixTeardown = false` at `session_prefix_test.go:149`, and it carries an `RFC requirement: RFC4271-6.7-4 negative` tag at `:144`. Changing the field to a map forces a change inside a tagged function | A Write or Edit on `session_prefix_test.go` is rejected at exit 2, naming the tagged scope | Do NOT reword the tag and do NOT reach for `// test-relax:`, which does not satisfy this hook. Prefer changing only the shared helper `newTestPeerSettingsWithPrefix` (`session_prefix_test.go:661`), which sits outside every tagged function, so the tagged bodies stay untouched. If a tagged body genuinely must change, that needs Thomas's explicit approval recorded as `// rfc-test-change-approved:`, per `ai/rules/rfc-compliance.md`. Only the user can authorize it |
...
|-------------|---|--------------|------|
| Peer config with two families disagreeing on `teardown` | → | `parsePrefixLimitFromFamily` (`config.go:648`) | `TestPrefixTeardownPerFamilyDisagreement` |
| Overflow on the warn-only family, session established | → | `applyPrefixCheck` (`session_prefix.go:370`) | `test/plugin/prefix-per-family-teardown.ci` |
| Overflow on the teardown family, `Cease` sent | → | `buildPrefixNotification` (`session_prefix.go:448`) | `test/plugin/prefix-per-family-teardown.ci` |
| Prefix teardown reconnect delay | → | `peer_run.go:88` backoff selection | `TestPrefixIdleTimeoutPerFamily` |
| Dynamic peer built from a template | → | `reactor_dynamic.go:141` settings copy | `TestDynamicPeerOwnsPrefixMaps` |
...
| 3 | Reads `show bgp peer` and expects the prefix staleness date to stay a single field | reactor → `plugin.PeerInfo` → `peer.go:330` → JSON key `prefix-updated` | `TestPrefixUpdatedAggregatesOldest` |
...
| `prefix warning` | 0 to maximum minus 1 | maximum minus 1 | N/A | maximum, rejected at `config.go:673` |
| `prefix idle-timeout` | 0 to 65535 | 65535 | N/A | N/A, uint16 ceiling |
| prefix teardown backoff exponent | 1 to 60 | 60 | N/A | 61, clamped at `peer_run.go:92` |
...
|-------------------|----------|---------------|
| YANG schema (new RPCs/config) | No | The grammar is already correct. `ze-bgp-conf.yang:617` and `:622` are already per-family. Only the Go storage is wrong |
...
| Prometheus counters/metrics | Yes | Confirm the existing per-family labels stay correct. `setPrefixCountMetric` and `incrPrefixTeardownMetric` (`session_prefix.go:556`) are already family-labeled or peer-scoped. Record which is which |
...
2. **Phase: Resolve A-7 first** -- settle the zero-value contract before you write timeout code
   ...
   - Files: read `peer_run.go:86-103` in full
   - Verify: the YANG description at `ze-bgp-conf.yang:625` and the code agree, or the disagreement is recorded and raised. Do not encode a guess
3. **Phase: teardown per family** -- the highest-risk leaf first
   ...
   - Files: `peersettings.go`, `config.go`, `session_prefix.go`, and the shared test helper at `session_prefix_test.go:661`
...
| Naming | Field names keep the `Prefix` prefix and the YANG leaf spelling, per `ai/rules/config-naming.md` |
| Data flow | The offending family reaches `peer_run.go`, and `errors.Is` on the sentinel still matches |
| Rule: `ai/rules/fail-closed-guards.md` | The absent-key case denies. A zero value is never a valid-looking answer |
| Rule: `ai/rules/exact-or-reject.md` | No family's setting is silently approximated by another family's |
| Rule: `ai/rules/no-fabrication.md` | Every citation in this spec was read, and A-1, A-2, A-8, A-9 are recorded as broken |
...
| Resource exhaustion | Warn-only mode must still drop excess NLRI rather than install it (`session_prefix.go:415`) |
| Input validation | Family keys come from the config tree. An unknown key is already rejected at `config.go:604`. Confirm the new map writes cannot introduce an unvalidated key |
...
## Design Insights
- The sort at `config.go:551-557` is why this bug is deterministic rather than flaky. A correct fix for one problem, stable capability ordering, converted a random-looking defect into a silent and reproducible one. Deterministic wrongness is harder to notice than intermittent wrongness.
...
- A `map[string]bool` is a poor container for a safety flag whose safe value is `true`. The Go zero value points the wrong way. This is the `fail-closed-guards.md` zero-value trap in its exact documented form.
...
|----------|------------------------|-----------|
| Make all three fields per-family maps | Reject a config where two families disagree, at verify time | The YANG grammar plainly offers per-family control at `ze-bgp-conf.yang:617` and `:622`. The two sibling leaves in the same container are already maps at `peersettings.go:364` and `:368`. Rejecting would remove a capability the schema advertises, which is a config-surface narrowing. `ai/rules/exact-or-reject.md` says to reject only when the implementation **cannot** deliver exactly what the config asks. Here delivering is straightforward, so exact wins. If the implementer disagrees, the choice goes to Thomas rather than being decided in the diff |
...
| Carry the offending family beside the sentinel error, not instead of it | Add a new error type. Store the family on the Peer struct at teardown | `errors.Is(err, ErrPrefixLimitExceeded)` at `peer_run.go:88` must keep matching. A replacement type would break every existing matcher. A field on Peer would add cross-goroutine state to a path that already documents single-goroutine access at `session_prefix.go:425-432` |
| Store `updated` per family, aggregate to the oldest at the boundary | Keep `updated` a peer scalar. Change the JSON to a per-family object | Storage matches the YANG. Aggregation preserves the external `prefix-updated` shape at `peer.go:330` and `types_bgp.go:86`. Oldest is the conservative choice, because it keeps the staleness alarm firing when any family is stale |
| Keep the family key sort | Iterate the map directly, now that order no longer decides the winner | The sort still serves OPEN capability ordering, which `config.go:548-550` documents. Removing it would trade one determinism bug for another |

## Known Limitations
- RFC 4486 NOTIFICATION still names one family, the one that overflowed first. Simultaneous overflow in two families is reported as one family. That matches the wire format at `session_prefix.go:451-462` and is not changed here.
...
- A-7 can reveal a second, separate defect in the `idle-timeout 0` contract. If the YANG description and `peer_run.go:88` genuinely disagree, this spec fixes it only when the fix is small. Otherwise the implementer raises it with Thomas and homes it. It is never silently left.
...
enforcing code is `applyPrefixCheck` (`session_prefix.go:399-416`) and
`buildPrefixNotification` (`session_prefix.go:448`). Keep the existing subcode
