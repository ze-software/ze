...
the retired deferral directory.
...
| Shard | Row | Subject |
|-------|-----|---------|
| the retired deferral shard "test-coverage-gaps" | 2026-07-10 AC-3 | five IPsec `.ci` exist. Their `vpn-ipsec/sa-up` event assertions were dropped, blocked on two engine gaps |
| the retired deferral shard "test-coverage-gaps" | 2026-07-10 AC-3 (`ipsec-dpd-timeout.ci`, deleted) | needs a runner primitive to stop a background daemon mid-test |
| the retired deferral shard "test-coverage-gaps" | 2026-07-10 AC-3 (`ipsec-monitor.ci`, deleted) | `monitor vpn ipsec` streaming, blocked on the bgp-locked startup subscription |
| the retired deferral shard "fixit-plugin-event-subscription" | 2026-07-19 | functional `.ci` for the SDK-fork end-to-end path |
| the retired deferral shard "fixit-perf-alloc-ci-gate" | 2026-07-19 | full the retired `ze-alloc-check` (current: `./le verify deps alloc`) run, needs a bench log |
| the retired deferral shard "fixit-fuzz-target-discovery" | 2026-07-19 | bounded `ze-fuzz-test` mutation run over the newly-enabled IS-IS / OSPF targets |
| the retired deferral shard "fixit-agent-tooling-misleads" | 2026-07-19 | functional proof beyond the corrected gates (the row's own What cell says "none") |
| the retired deferral shard "fixit-runner-kill-background" | 2026-07-19 | `stop-background.ci` live run |
| the retired deferral shard "fixit-ddos-test-infra" | 2026-07-16 (AC-10) | proving packets are FORWARDED needs a receiver on the far veth end |
| the retired deferral shard "fixit-pppoe-orphaned-tests" | 2026-07-19 | the `docs/features.md` PPPoE row stays Partial |
| the retired deferral shard "fixit-doc-gate-and-refs" | 2026-07-19 | the optional local `check-doc-drift.sh` hook |
| the retired deferral shard "fixit-ipsec-clear-reestablish" | 2026-07-19 | strongSwan interop scenarios clear-reestablish and responder-accepts-reinit need Docker plus charon |
...
### From `fixit-ddos-test-infra.md`, 2026-07-16

Deferred by spec-fixit-ddos-test-infra (AC-10 transit proof).

Proving packets are FORWARDED (not merely admitted) needs a receiver on the far veth end counting arrivals; the current shape can prove the drop but not the forward

### From `fixit-plugin-event-subscription.md`, 2026-07-19

Deferred by spec-fixit-plugin-event-subscription functional-proof.

functional .ci for the SDK-fork end-to-end path not authored ~~(unrunnable here)~~

### From `fixit-pppoe-orphaned-tests.md`, 2026-07-19

Deferred by spec-fixit-pppoe-orphaned-tests functional-proof.

docs/features.md PPPoE row left Partial (reflects feature completeness, not tests)

### From `test-coverage-gaps.md`, 2026-07-10

Deferred by spec-test-coverage-gaps AC-3 (ipsec-monitor.ci, DELETED).

`monitor vpn ipsec` streaming `.ci`: prove the monitor surface emits sa-up/child-up as they occur. Un-runnable: lifecycle events reach subscribers only via the startup event subscription, namespace-locked to bgp (`dispatch.go registerSubscriptions`), and sa-up/child-up fire at establishment before the engine-steps stream can attach. The deleted `.ci` was a single non-establishing daemon.
