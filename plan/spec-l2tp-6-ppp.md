# Spec: l2tp-6 -- PPP Protocol Engine

| Field | Value |
|-------|-------|
| Status | skeleton |
| Depends | spec-l2tp-5-kernel |
| Phase | - |
| Updated | 2026-04-13 |
...
3. `docs/research/l2tpv2-implementation-guide.md section 22, Appendix C`

## Task

Implement the PPP protocol engine for L2TP sessions:

LCP (RFC 1661): FSM with 10 states and ~30 transitions, option negotiation
(MRU, authentication method, magic number, echo request/reply). Runs on
/dev/ppp channel fd.

PPP Authentication: PAP (RFC 1334), CHAP-MD5 (RFC 1994), MS-CHAPv2
(RFC 2759). Integrates with l2tp-auth plugin via events.

IPCP (RFC 1332): IPv4 address negotiation, DNS server negotiation
(RFC 1877). Integrates with l2tp-pool plugin via events. Runs on /dev/ppp
unit fd.

IPv6CP (RFC 5072): Interface identifier negotiation. Note: does NOT assign
addresses or prefixes. Prefix delegation is DHCPv6-PD (out of scope).

PPP worker pool for blocking /dev/ppp I/O. Interface configuration via
netlink (address, peer address, routes, MTU) after IPCP completes.

Reference: docs/research/l2tpv2-implementation-guide.md section 22,
Appendix C (PPP gap).
