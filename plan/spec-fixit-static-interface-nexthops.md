...
| VPP static backend + NON-vpp iface backend (R-7) | -> | C-4 gate in `toVPPRoute` (shape per D-1) | `TestToVPPRouteRefusesResolveWhenIfaceBackendNotVPP` |
...
| AC-2 | Static route with an interface-only next-hop, linux backend, no iface backend loaded | Diagnosable failure naming the missing `interface { backend ... }` stanza, not the bare `iface: no backend loaded` (`dispatch.go:12`). Shape per D-2 |
| AC-3 | Static section mixing a good address-next-hop route and an unresolvable interface next-hop | Blast radius is a deliberate, documented choice; today the whole section fails AND aborts daemon startup (`inject.go:87-92` -> `register.go:138-140`, observed in the 2026-07-16 `005` run). Resolved by D-3 |
...
| AC-10 | **NEW.** VPP static backend active while the iface backend is NOT vpp (R-7) | Resolution is refused with a clear error; a kernel ifindex is NEVER emitted as a VPP `sw_if_index`. Shape per D-1 |
...
| Doctor check for runtime dependencies | [ ] **Recommended Yes, pending D-2.** An interface next-hop depends on a loaded iface backend, which is a runtime dependency per `ai/rules/doctor-checks.md`. The check belongs to the STATIC plugin (Proximity Principle), not to iface | `internal/plugins/static/` + `internal/core/diagnostic/codes.go` |
...
and 4 are unaffected by all three.
...
3. **Phase 3: Linux diagnosis (C-2, + doctor check per D-2)**.
   - Tests: `TestResolveNexthopIndexNoBackendErrorIsDiagnosable` (AC-2);
     `test/static/006-interface-nexthop-no-backend.ci`.
   - Files: `internal/plugins/static/backend_linux.go`, plus a doctor check in the static
     plugin's own package if D-2 picks (b) (`ai/rules/doctor-checks.md`,
     `ai/rules/plugin-self-containment.md`: the check belongs to static, and its code to
...
| 9 | Is there an equivalent gap in other `iface.Resolve` consumers (e.g. `ldp`)? | **OPEN, deliberately out of scope.** Not investigated: this spec is bounded to static. Flagged for Thomas as a possible follow-up rather than silently widened. | `ai/rules/no-partial-completion.md`: scope changes need approval |
