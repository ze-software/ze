...
| AC-62 | SUPERSEDED by AC-63 (owner review, 2026-09-01) | The requirements table dropped the site-wide weight by opting out through a class of its own, which left two conventions on one page: eight of the eleven tables put a LABEL in the first column and the site rule bolds identities. The whole family now shares one look, and the weight goes for all of it |
...
| AC-63 | Every table this family renders | ONE convention on one page. The requirement id sits in a narrow left cell and its sentence beside it, in a cell spanning the rest of the row. No table opts out by class: the family's own class carries the look, and the weight the site-wide `td:first-child` rule adds is dropped for the whole family rather than for one table. SUPERSEDES AC-62's opt-out |
| AC-64 | Every stem page | It proves its own arithmetic: the coverage table carries a total row stating whether the partitioning buckets account for the gated population, and each bucket says whether the count the gate produced and the membership this page names agree. An overlay bucket is marked as one and is not summed |
| AC-65 | The index's accounting row | It compares two counts produced INDEPENDENTLY: the sum of the binding buckets against the gate's own gated total less the out-of-scope buckets. Holding the sum against a field that IS that sum is a tautology whose mismatch branch cannot execute |
| AC-66 | The ledger prose | Rejoining the split reproduces the cell for every shape an author can write, including a cell that ends in a semicolon. Prose with an unclosed code span is published as plain text, every character of it, rather than having a backtick eaten and its tail marked up as code |
| AC-67 | Retiring a page | The removal is keyed on the marker THIS family writes, never on a directory name absent from the live set. The output of a real build is the published checkout, so a name test deletes any directory under the prefix another producer or an author put there |
| AC-68 | Every un-enrolled kind | The index and the stem page state what the kind MEANS, in a sentence read from `internal/le/rfc` rather than listed in the site package. Six kinds exist as of 2026-09-01 and `out-of-scope` is the newest; a page that spelled its kinds would print a bare word for a seventh |
| AC-69 | A page for a summary the gate does not hold | It never says the gate HOLDS its requirements. `evaluate` skips every requirement of an un-enrolled RFC, so the population card names what the summary DECLARES and says the gate holds none of it |
| AC-70 | Every published provenance string | No `_sources` entry of `data/site-facts.json` names a GENERATED file as an input. `rfc/enrolled.txt`, `rfc/not-enrolled.txt` and `docs/features/rfc-status.md` are rendered from each summary's `## Meta` table, so a producer that names one is claiming an input it does not read. A false provenance claim in machine-readable data is worse than a stale comment, because a consumer acts on it |
...
| `TestTheSubjectCarriesNoWeightOfItsOwn` | `internal/le/site/rfcdetail_test.go` | AC-63: the family class, the weight override, no weight tag in the subject row, and no bold in the mirror | green |
...
| `TestEveryTableOfThisFamilySharesOneLook` | `internal/le/site/rfcdetail_test.go` | AC-63: every table of the stem page and the index goes through one producer and carries the family class, with no bare `<table>` left | green, and RED with two index tables written inline: 2 tables outside the convention |
| `TestEveryStemPageAccountsForItsGatedRequirements` | `internal/le/site/rfcdetail_test.go` | AC-64: over 185 stem pages, the partitioning buckets sum to the gated count, the two walks agree per bucket, and both renderings carry the total row | green, and RED with one bucket dropped from the partition |
| `TestTheIndexAccountingRowCanSayTheBucketingIsIncomplete` | `internal/le/site/rfcdetail_test.go` | AC-65: a split whose gated total disagrees with its buckets reaches the mismatch sentence | green |
| `TestProseOfAnyShapeIsPublishedWholeAndLosesNoCharacter` | `internal/le/site/rfcdetail_test.go` | AC-66: eight shapes, including a trailing semicolon and an odd backtick, rejoin byte for byte and render whole | green, and RED against both retired behaviors: 3 shapes lose text, 2 lose a backtick |
| `TestRetiringAPageDeletesOnlyThisFamilysOwnPages` | `internal/le/site/rfcdetail_test.go` | AC-67: a retired page of this family goes, an authored directory and a directory with no page stay | green, and RED with the marker check removed: both survivors deleted |
| `TestEveryDispositionKindOnTheIndexSaysWhatItMeans` | `internal/le/site/rfccompliance_test.go` | AC-68: every kind in use on this checkout carries its published meaning, on the index, in the mirror and on the stem page | green |
| `TestAnUnenrolledPageNeverClaimsTheGateHoldsIt` | `internal/le/site/rfcdetail_test.go` | AC-69: over this checkout's enrolled and declined summaries, the population card states the right fact on each | green |
| `TestEveryDispositionKindCarriesItsMeaning` | `internal/le/rfc/carriers_test.go` | AC-68: the meaning table and the parser's closed set agree, in both directions and in size | green |
| `TestNoPublishedSourceNamesAGeneratedLedgerFile` | `internal/le/site/facts_test.go` | AC-70: over this checkout's own 15 published provenance strings, none names a generated ledger file | green, and RED with the retired `rfc` string restored |
...
- `internal/le/site/rfccompliance.go` - `renderRFCCompliance` reads the new snapshot,
  writes the two link tables into the index body and mirror, loops the detail writer,
  answers every route, and THEN retires the pages this family wrote and this run did not
...
- `internal/le/rfc/carriers.go` - `kindEditor` and `kindUnknown` become named constants
  ...
  `CarrierRank` and `CarrierLabelRank` declare the reading order beside the vocabulary it
  orders. `CarrierKinds` and `CarrierTiers` were exported with them and are gone again:
  tests alone called them, which `./le doc wiring` refuses, and each test now reads the
  order variables directly
- `internal/le/rfc/artifact.go` - `dispositionKinds` states what each un-enrolled kind
  MEANS, beside `exclusionKinds` and for the same reason; `DispositionKinds` and
  `DispositionKindMeaning` publish it. `ExclusionGroups` is deleted for the same reason
  as the two carrier wrappers
