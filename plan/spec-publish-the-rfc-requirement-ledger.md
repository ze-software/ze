...
| AC-31 | The requirement table of a stem page | It carries no Text column. The requirement's own sentence sits behind a `<details>` disclosure inside the id cell, reachable by keyboard and by touch, and is still present in the markup. The Markdown mirror keeps its Text column whole, and AC-2's shard equality is unchanged |
...
| 7 | Requirements | One row per requirement, with the row's anchor on the id, the section's own title beside its number, both polarities in ONE Tests column each cited by NAME and linked to its tagged line, and the requirement's sentence on a second row spanning the table | AC-2, AC-12, AC-21, AC-25, AC-31, AC-53, AC-55, AC-56 |
