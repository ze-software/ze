...
| AC-2 | `filter dscp <name>` | Verifier accepts; QosRecordEnableDisable(SOURCE_IP) on ingress + QosEgressMapUpdate + QosMarkEnableDisable on egress, in that order; removal disables all three |
...
| AC-4 | `qdisc prio` with classes carrying dscp matches | Verifier accepts prio under vpp; `egressMapFromPrioClasses` (new) builds the DSCP→class egress map rows; map applied; classless-prio config without matches rejected with actionable error |
...
| DSCP via 3-step record+map+mark (SOURCE_IP) | QosStoreEnableDisable static classification | Matches the proven VLAN pipeline and the L209 deferral intent; Store recorded as A-2 fallback |
| Mark = SET_METADATA steering semantic, explicitly NOT SKB parity | Pretend parity / keep rejected | VPP has no SKB; metadata is the native primitive (L205's own wording); honesty in docs beats a silent semantic lie; A-3 spike gates it |
| prio binding reuses per-class `match { type dscp }` entries | New YANG leaf for dscp→class map | Config surface exists; no schema growth without a proven need (A-5) |
...
- Classify/QoS pipeline applies to the ip-ACL arc; L2-only interfaces are not handled (as today).
- Real-VPP packet-level probes limited by the Docker evidence environment; state-level vppctl assertions are the contract.
