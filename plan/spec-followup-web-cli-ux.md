...
| AC-2 (session profiles, R-1) | PARTIAL: session/context DONE | `WebSession.Profiles` + `withProfiles`/`GetProfilesFromRequest` (`internal/component/web/auth.go`); login stores `AuthResult.Profiles`; middleware carries them into context. `TestSessionStoresProfiles`, `TestProfilesInRequestContext` PASS; fail-open R-1 verified. REMAINING: switch web from `LocalAuthenticator` to the live AAA-chain authenticator (hub timing: chain built post-config-load) so RADIUS/TACACS admins authenticate on web |
| AC-3,4 (upload/download), AC-5 (web-route registry), AC-6,7 (i18n/mobile), AC-8,9 (completion), AC-10 (renderWriter) | NOT STARTED (this session) | umbrella too large for one pass (R-7); recommend split into per-phase specs |
