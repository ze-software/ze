...
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-005: `functionalGateSuites` (`scripts/docvalid/doc_drift.go`) now follows `include`/`-include`/`sinclude` directives (`doc_drift.go`, `:221`); the "could not derive ze-functional-test suites" error no longer appears in the 2026-07-10 `make ze-doc-test` run.
...
- RA-DOC-002 (retained, narrowed): the drift checker now passes ("No documentation drift detected"); `docs/functional-tests.md` includes `ze-vet-evidence` in the documented ze-verify order; fuzz docs match the Makefile (54 targets at `functional-tests.md`, "10s each" at `:1503`; `mk/test-fuzz.mk` uses 10s per target). `make ze-doc-test` still exits non-zero, but only on the discovery-index freshness stage: `ai/DOCS-TO-CODE.md` and `ai/LEARNED-FULL-INDEX.md` are stale against the wave (fix: `make ze-discovery-index`). The finding's remaining substance is that index regeneration plus RA-DOC-009 below.
- RA-DOC-008 (retained, still reproducible): `make ze-consistency` re-run 2026-07-10 fails with 84 errors and 1021 warnings (was 42/712; the backlog grew). The cited stale refs persist (`internal/component/bgp/plugins/rib/storage/pathset.go` -> `familyrib_bart.go`, `:3` -> `familyrib_map.go`, both missing), as do the `cmd/cache`/`cmd/commit` missing-schema errors.

Fresh instances (added as rows in Initial Findings above):

- RA-DOC-009: the documented `make ze-verify` order (`docs/functional-tests.md`) omits five gates the live producer runs: `stagesForMode` (`scripts/status/verify_run.go`, default branch `:135-148`) executes `ze-tier-check`, `ze-iface-resolution-check`, `ze-plugin-boundary-check`, `ze-port-defaults-check`, `ze-platform-vet` between ze-lint and ze-verify-wiring-docs. The dead `_ze-verify-impl` target (`Makefile:287`; zero callers per the comment at `Makefile:280-286`) additionally lists `ze-cli-grammar-check`, which is not in the live stage list.
