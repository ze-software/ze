...
- `make ze-consistency` failed with 42 errors and 712 warnings; doc-relevant output includes stale source refs in `internal/component/bgp/plugins/rib/storage/pathset.go:2-3` and missing plugin command package documentation/schema markers. (2026-07-10 re-run: 84 errors, 1021 warnings, same pathset refs; see Post-wave corrections.)
- (Command-run bullets for the removed findings RA-DOC-002-as-filed, RA-DOC-006, and RA-DOC-007 deleted 2026-07-10; the 2026-07-10 `make ze-doc-test` state is recorded in Post-wave corrections.)
...
| Release gate documentation | `docs/functional-tests.md`, `scripts/status/verify_run.go` | Documented ze-verify order omits five live gates | RA-DOC-009 |
...
| ze-verify order documentation | -> | `scripts/status/verify_run.go` stagesForMode | Future drift check requested by RA-DOC-009 |
...
- `docs/functional-tests.md` - ze-verify order sentence (RA-DOC-009).
...
| Release gate and inventory docs | RA-DOC-009, RA-DOC-010 | ze-verify order sentence from stagesForMode; interop scenario counts from the tree |
...
| ze-verify order docs drift | RA-DOC-009 docs/tooling future fix |
...
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-001: `README.md:86` now says "Go 1.26+", matching `go.mod:3`; `docs/guide/quickstart.md:13` says Go 1.26+ and its expected output "configuration valid: example.conf" (`quickstart.md:144`) matches the producer `internal/component/config/cli/cmd_validate.go:603` (the validate command moved there from `cmd/ze/config/cmd_validate.go`).
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-003 (as filed): no doc says 32 or 33 scenarios any more; `docs/DESIGN.md:796` says "101 interop scenarios" (the tree has 101 scenario directories); `docs/architecture/testing/interop.md:142` lists `33-bfd-frr` in its core table and its coverage text names BFD failover (`interop.md:251`). Residual count drift re-filed as RA-DOC-010 below.
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-004: all nine plugins the finding named (`bgp-filter-aspath-length`, `bgp-filter-remove-private-as`, `dhcpserver`, `flowspec-firewall`, `ike`, `imageserver`, `kernel`, `routing-table`, `tftpserver`) now appear in `docs/DESIGN.md`, and the doc-drift stage of `make ze-doc-test` reports "No documentation drift detected" (run 2026-07-10).
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-005: `functionalGateSuites` (`scripts/docvalid/doc_drift.go:153`) now follows `include`/`-include`/`sinclude` directives (`doc_drift.go:205`, `:221`); the "could not derive ze-functional-test suites" error no longer appears in the 2026-07-10 `make ze-doc-test` run.
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-006: check mode is read-only (`scripts/dev/code_to_docs.py:165-168` writes `ai/CODE-TO-DOCS.md` only in non-check mode) and the description separator accepts `--`, single hyphen, and the long dash (`code_to_docs.py:24`); the 2026-07-10 run reports "checked 1366 code paths, 429 packages ... all references valid".
- Removed 2026-07-10 per user instruction: resolved by followup wave -- RA-DOC-007: all 8 cited broken links now resolve (`docs/guide/benchmarking.md`, `docs/guide/config-archive.md`, `docs/guide/looking-glass.md`, `docs/guide/healthcheck.md`, `docs/guide/rpki.md`, `docs/guide/web-interface.md` exist for the `docs/features/*` links; `docs/guide/mcp/overview.md:127` -> `ai/rules/exact-or-reject.md` resolves; `docs/architecture/config/environment.md` links at `:115`/`:126` resolve). A local Markdown link checker is still absent from `make ze-doc-test`, so the Future item for it is retained.

Retained findings, status re-verified 2026-07-10:

- RA-DOC-002 (retained, narrowed): the drift checker now passes ("No documentation drift detected"); `docs/functional-tests.md:19-20` includes `ze-vet-evidence` in the documented ze-verify order; fuzz docs match the Makefile (54 targets at `functional-tests.md:1498`, "10s each" at `:1503`; `mk/test-fuzz.mk` uses 10s per target). `make ze-doc-test` still exits non-zero, but only on the discovery-index freshness stage: `ai/DOCS-TO-CODE.md` and `ai/LEARNED-FULL-INDEX.md` are stale against the wave (fix: `make ze-discovery-index`). The finding's remaining substance is that index regeneration plus RA-DOC-009 below.
- RA-DOC-008 (retained, still reproducible): `make ze-consistency` re-run 2026-07-10 fails with 84 errors and 1021 warnings (was 42/712; the backlog grew). The cited stale refs persist (`internal/component/bgp/plugins/rib/storage/pathset.go:2` -> `familyrib_bart.go`, `:3` -> `familyrib_map.go`, both missing), as do the `cmd/cache`/`cmd/commit` missing-schema errors.

Fresh instances (added as rows in Initial Findings above):

- RA-DOC-009: the documented `make ze-verify` order (`docs/functional-tests.md:19-21`) omits five gates the live producer runs: `stagesForMode` (`scripts/status/verify_run.go:112`, default branch `:135-148`) executes `ze-tier-check`, `ze-iface-resolution-check`, `ze-plugin-boundary-check`, `ze-port-defaults-check`, `ze-platform-vet` between ze-lint and ze-verify-wiring-docs (`:137-141`). The dead `_ze-verify-impl` target (`Makefile:287`; zero callers per the comment at `Makefile:280-286`) additionally lists `ze-cli-grammar-check`, which is not in the live stage list.
- RA-DOC-010: interop scenario counts re-drifted: `docs/features/interoperability-testing.md:13` says 96, `docs/architecture/testing/interop.md:101` says 97 scenario directories, `docs/DESIGN.md:796` says 101, and the tree has 101 scenario directories (2026-07-10). Successor to the removed RA-DOC-003.
