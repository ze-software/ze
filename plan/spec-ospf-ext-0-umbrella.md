...
| Shared AF-neutral engine machinery (FSM, flooding, DR election, SPF, LSDB sequencing, lifecycle) | both bases, unified per `docs/architecture/ospf/ospf-af-unify.md` | STABLE; both-AF extensions (TI-LFA, BFD, LDP-IGP sync, virtual links, NBMA/P2MP, debug) attach here once and serve both families |
...
- [ ] `docs/architecture/ospf/ospf-af-unify.md` -- the delivered unified-engine decision
...
| A second OSPF engine rested (forbidden) | Fork the engine per protocol version | The delivered design (`docs/architecture/ospf/ospf-af-unify.md`) is one engine: the FSM, flooding, DR, SPF, and LSDB sequencing are AF-neutral and shared. Forking reintroduces drift |
