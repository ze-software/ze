...
`vppBackendImpl.CreateDummy` (`internal/plugins/iface/vpp/ifacevpp.go`) sends an
empty `interfaces.CreateLoopback{}` and lets VPP assign the MAC address. An
operator can set one afterwards through the `interface-l2` grouping, which
reaches `SetMACAddress`, but there is no default.

VPP derives a loopback's default MAC from the interface instance number alone,
so two Ze nodes that create the same loopback instance receive the same MAC
address. Where such a loopback is a bridge BVI on a shared L2 segment, the peer's
MAC arriving on another port is rejected as a MAC move violation, and ARP between
the two loopbacks is silently dropped while ordinary bridged traffic is
unaffected.

Ze does not create BVIs today: `BridgeAddPort` sends
`l2.SwInterfaceSetL2Bridge` and sets no port-type field. The collision is
therefore not currently reachable. It becomes reachable as soon as BVI support
lands, and the fix is cheap now.

Ze already holds the ingredient. `identity.Resolve`
(`internal/core/identity/identity.go`) yields a stable machine id from zefs
`meta/instance/machine-id`, then `/etc/machine-id`, then hostname, then
`crypto/rand`. Nothing in the tree derives a MAC address from it.

Goal: derive a deterministic, host-unique, locally-administered MAC from the
machine identity and pass it at loopback creation whenever the operator has not
configured one.

The VPP premise (default loopback MAC derives from the instance number only) is
upstream behavior that this repository's vendored sources cannot confirm.
Validate it before relying on it.

Provenance: VyOS T8367 (`vpp: Fix identical default MAC on bridged loopback
interfaces`).
