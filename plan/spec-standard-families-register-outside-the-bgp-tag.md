# Spec: standard families register outside the bgp tag
...
<!-- Scope: plugin. The family registry and the NLRI plugin registrations are
...
`internal/component/bgp/plugins/nlri/`, behind the `ze_bgp` build tag
(`internal/component/plugin/all/all_ze_bgp.go`).

The seven rows:

| Package | Symbol |
|---------|--------|
| `internal/chaos/peer/simulator.go` | `RunSimulator` |
| `internal/chaos/peer/simulator_reader.go` | `afiSafiFamily` |
| `internal/chaos/scenario/generator.go` | `allFamilies` |
| `internal/component/kernelcap/predicate.go` | `labeledFamilies` |
| `internal/test/fixture/plugin_fixture_06.go` | `fixture06Registration` |
| `internal/test/runner/json.go` | `isSupportedFamily` |
| `internal/test/runner/runner_validate.go` | `nlriFamilyKeys` |

Each needs a non-base family name (flow, mpls-vpn, mpls-label, evpn, flow-vpn).
Deriving there renders `afi-1/safi-128`, because the registration is not
linked into the binary.

The one that matters is `kernelcap.labeledFamilies`. It decides whether the
Linux kernel is asked for an AF_MPLS table (`MPLSInUse`, registered by
`internal/plugins/fib/kernel/kernelcap_linux.go`). A registry miss there would
answer "no MPLS needed": a fail-open guard (`ai/rules/principles.md`, first
directive).

The decision this spec makes: where the STANDARD families (the IANA-assigned
AFI/SAFI pairs Ze speaks) are declared, so every package can name them without
linking a plugin, and without the family registry becoming a second copy of the
NLRI plugins' registrations. The chaos, fixture and runner rows are tooling and
follow whatever the kernelcap answer is.
...
### Source Files
- [ ] `internal/core/family/registry.go` - the family registry; the `MustRegister` calls near its end are the only unconditional registrations (four families)
  → Constraint: a family not registered renders as `afi-N/safi-N`, never as its name
- [ ] `internal/component/bgp/plugins/nlri/` - every other family registers from an NLRI plugin package here
  → Constraint: the registry MUST NOT become a second copy of these registrations (`ai/rules/principles.md`, declare once)
- [ ] `internal/component/plugin/all/all_ze_bgp.go` - the NLRI plugins link only behind the `ze_bgp` build tag
  → Constraint: a package without that tag, or one that does not link the plugin, sees none of those families
- [ ] `internal/component/kernelcap/predicate.go` - `labeledFamilies` decides whether the kernel is asked for an AF_MPLS table
  → Decision: a registry miss MUST refuse, never answer "no MPLS needed" (`ai/rules/principles.md`, first directive)
- [ ] `internal/plugins/fib/kernel/kernelcap_linux.go` - registers `MPLSInUse`, the consumer of `labeledFamilies`
- [ ] `internal/chaos/peer/simulator.go` - `RunSimulator`, one of the seven rows (tooling)
- [ ] `internal/chaos/peer/simulator_reader.go` - `afiSafiFamily`, one of the seven rows (tooling)
- [ ] `internal/chaos/scenario/generator.go` - `allFamilies`, one of the seven rows (tooling)
...
- Four families register unconditionally; the rest register behind `ze_bgp`, so a package that does not link an NLRI plugin cannot name flow, mpls-vpn, mpls-label, evpn or flow-vpn through the registry.
- `kernelcap.labeledFamilies` is a guard: a miss must refuse, not answer "no MPLS".

## Current Behavior (MANDATORY)

**Source files read:** (must read BEFORE you write this spec)
- [ ] `internal/core/family/registry.go` - holds the family registry; only four families are registered by its own `MustRegister` calls
- [ ] `internal/component/bgp/plugins/nlri/` - each NLRI plugin package registers its family from its own `init()`
- [ ] `internal/component/plugin/all/all_ze_bgp.go` - links the NLRI plugins, behind the `ze_bgp` build tag only
- [ ] `internal/component/kernelcap/predicate.go` - `labeledFamilies` restates the labeled family names; a derived lookup that misses would answer "no MPLS needed"
- [ ] `internal/plugins/fib/kernel/kernelcap_linux.go` - registers `MPLSInUse`, which reads `labeledFamilies` to decide whether to ask the kernel for an AF_MPLS table
- [ ] `internal/chaos/peer/simulator.go` - `RunSimulator` restates family names
- [ ] `internal/chaos/peer/simulator_reader.go` - `afiSafiFamily` restates family names
- [ ] `internal/chaos/scenario/generator.go` - `allFamilies` restates family names
- [ ] `internal/test/fixture/plugin_fixture_06.go` - `fixture06Registration` restates family names
- [ ] `internal/test/runner/json.go` - `isSupportedFamily` restates family names
- [ ] `internal/test/runner/runner_validate.go` - `nlriFamilyKeys` restates family names

**Behavior to preserve:** (unless the user explicitly said to change it)
- The NLRI plugins stay the owners of their families' codecs and register from their own packages; removing a plugin still removes its wire behavior (`ai/rules/plugins.md`).
- `MPLSInUse` still asks the kernel for an AF_MPLS table exactly when a labeled family is configured.
