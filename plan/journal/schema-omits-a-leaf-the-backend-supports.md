...
|------|------|---------|---------|-----|
| 2026-09-14 | the-fix-pass-for-restated-registries | the `backend` leaf (`internal/component/hub/yang/ze-hub-conf.yang:65`) against `createHandler` (`internal/core/slogutil/slogutil.go:320`) | `ze.log.backend=kmsg` selects the kernel ring buffer handler, and `backendNames` accepts it (`slogutil.go:348`), but the YANG enum at `environment/log/backend` declares only `stderr`, `stdout` and `syslog`, so an operator can reach the backend by env var and not by config. Three surfaces give two answers: the package doc comment (`slogutil.go:10`) names kmsg, the env registration's own Description (`slogutil.go:54`) does not, and the YANG does not. Found while judging the enumeration row for that const block | not fixed: it is a missing enum value rather than a wrong answer, and which way it goes is a decision (declare `kmsg` in the enum, or drop the handler), not a repair |
...
| 2026-09-14 | the-fix-pass-for-restated-registries | the `protocol` leaf of the `diag` capture command against its dispatcher (`internal/plugins/diag/cmd/`) | The leaf offers `l2tp` and `bgp`; the Go dispatcher also handles `bfd`. Two values is below the enumeration gate's floor of three, so no row reports the disagreement, and an operator cannot ask for a BFD capture the code can produce | not fixed: add `bfd` to the leaf, or delete the dispatcher arm; either way the two must agree, and a three-value enum brings the pair under the gate |
