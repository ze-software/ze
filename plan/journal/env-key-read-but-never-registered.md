...
|------|------|---------|---------|-----|
| 2026-09-22 | - | SDK identity bootstrap and `newObserver` (`internal/test/fixture/fixture.go`) | External observers aborted on `ze.plugin.name` because only the plugin CLI registered it. | Move the declaration beside the SDK transport keys. The isolated composition smoke fails before the move and passes after it: `plan/verification-evidence/2026-09-22-baseline/rpki-runtime-preparation/identity-bootstrap-fix/verification.json`. |
| 2026-09-07 | mgmt-version-header-suppress | `internal/component/config/apply_env.go`, `envPlumbingTable`, the table that carries an `environment` YANG leaf into an env key | A plumbing row that spells its key as a literal registers nothing, so `env.Set` ends the daemon the first time an operator writes that leaf. The config package cannot register the key either, because the reader lives in another package and only that package's link decides whether the registration is present | the key is an exported constant declared beside its reader, with `env.MustRegister` in the same file (`internal/core/version/version.go`, `EnvKeyHideVersion`), and the plumbing row names the constant. The import that carries the spelling then carries the registration into every binary that links the row |
