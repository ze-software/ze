...
|------|------|---------|---------|-----|
| 2026-09-22 | - | SDK identity bootstrap and `newObserver` (`internal/test/fixture/fixture.go`) | External observers aborted on `ze.plugin.name` because only the plugin CLI registered it. | Move the declaration beside the SDK transport keys. The isolated composition smoke fails before the move and passes after it. |
