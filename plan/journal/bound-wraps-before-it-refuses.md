...
|------|------|---------|---------|-----|
| 2026-09-07 | l2tp-shaper-upload-rate-is-not-enforced | `ParseRateBps` (`internal/component/traffic/config.go`), which reads every rate a RADIUS server writes into a Filter-Id or a MikroTik Rate-Limit | The suffix multiply was unchecked: `n * rs.mult` over a uint64 parsed straight off the attribute. Measured before the fix, `ParseRateBps("18446744074gbit")` returned `290448384, nil`, a plausible 290 Mbit/s that every downstream bound accepts, so the subscriber was shaped and policed at a rate nobody configured and nothing said so. The residue is any multiple of 512, so the wrap defeats the bound in the lets-traffic-through direction as easily as the other one. Found at this spec's closure security review, whose own checklist claimed "traffic.ParseRateBps bounds the value". It did not | fixed here: the stated number is compared against `math.MaxUint64/rs.mult` before the multiply, and `TestParseRateBpsRefusesOverflowingRate` pins both polarities, the wrapping input and the largest that does not wrap. RED observed with the guard disabled |
