...
| 2026-08-12 | - | config.transaction shutdown | `Server.Stop` closed plugin connections under an in-flight reload, which the bridge read as crashed plugins. Rollback restarted plugins shutdown was about to kill and emitted misleading WARN lines; the daemon exited without losing operator state. | Fixed 2026-08-12: `Server.Stop` cancels the transaction with `transaction.ErrShutdown` and waits up to `txShutdownGrace` (3s) before closing connections. `Execute` skips abort and rollback for that cause alone; ordinary cancellation still rolls back. The evidence and cancellation distinction are retained under "Shutdown under a running transaction" below. |
| 2026-09-17 | storage-1-backend-parity | BGP candidate reload and reactor shutdown | Reload read the old active config instead of the candidate; shutdown could wait without ending the reactor loop. | Corrected candidate selection and reactor cancellation. The affected reload cases and actual CLI transaction pass. |
