...
| 2026-09-09 | plugin-query-mode | `docs/guide/command-reference.md`, the `### show plugin declarations` section beside `### show plugin list` | Third row of this class, on the same page as the row above, and the first one where the sibling hunk is provably unsafe rather than merely foreign. The section is written: the six row keys are read off `declarationRow` (`internal/component/plugin/declarations.go`), the five states off the state constants beside it, and the budget sentence off `declarationBudget`. The same file carries a `show bgp update-delay` row from another session whose `<!-- source: -->` anchor names `internal/component/bgp/plugins/cmd/peer/update_delay.go`, an UNTRACKED file, and two "nineteen -> twenty" edits that count a command absent from HEAD. Carrying the file would publish a page whose anchor resolves to nothing and whose count is wrong, which `ai/rules/git-safety.md` names as the one case a foreign hunk is held back for | Not fixed. The other nine documentation obligations of this spec are committed, `docs/features/introspection.md` carries the whole command with anchors, and `ai/INDEX.md` gained the keyword row, so the operator can reach the command; the missing piece is the reference page's own section. It lands when the update-delay session commits its code, or when the owner authorizes one commit to carry the whole file. Until then `git diff -- docs/guide/command-reference.md` is the only place the text exists |
