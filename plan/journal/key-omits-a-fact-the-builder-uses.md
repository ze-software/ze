...
|------|------|---------|---------|-----|
| 2026-09-10 | spec-ipsec-dataplane-inspection | IPsec counter and drift joins | SPI-only keys could use another SA's counters or presence. | Join SPI, destination, protocol, and XFRM interface identity through `SAIdentity`. |
| 2026-09-10 | spec-ipsec-dataplane-inspection | Netlink SAD and SPD address decoding | Inferring the family from address bytes collapsed IPv6 `2001:db8::` into IPv4 `32.1.13.184`. Distinct installed SAs then shared one reported identity. | Decode outer addresses and selectors with their message families. The real-kernel collision regression fails with inference restored and passes with the fix. |
