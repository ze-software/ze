...
| 2026-09-07 | fixit-ci-runner-cannot-test-stdin | `cmdDecode` in `internal/plugins/isis/cli/decode.go` and `internal/plugins/ospf/cli/decode.go` | Both read `io.ReadAll(io.LimitReader(os.Stdin, ...))` unconditionally after flag parsing. Neither takes a path argument and neither accepts `-`, so `ze isis decode` typed at a terminal blocks in that read with no output. `ai/rules/cli.md` puts the stdin form behind an explicit `-`, and `internal/core/cliio` is the route every other `ze` decode verb takes | not fixed. Found while auditing which `ze` commands read standard input for the `.ci` stdin routing; it changes a shipped command's argument grammar, which is not that spec's surface |
