...
|----|------|---------|----------|------|-----------------|
| P0-1 | Release gate evidence | Still open, with the remaining local blocker narrowed. `.woodpecker/verify.yml` runs `make ze-verify`, Makefile help/drift checks agree with the 11-suite functional gate, and `make ze-verify` passed locally on 2026-05-05 after the release-check script fix, linter pin, Linux lint fixes, and review updates. `make ze-release-check` now exists as a Docker-backed clean-source substitute. 2026-05-05 clean-clone attempts exposed and fixed verifier reproducibility issues (Docker login shell needed an explicit Go `PATH`; verifier/CI/dev setup needed a pinned `golangci-lint v2.10.1` instead of floating `@latest`) plus five Linux-only lint issues. The two known flake rows that were waiting for a full local gate are now resolved. Clean release-candidate evidence still has to be rerun after these fixes are included in a clean candidate. | `.woodpecker/verify.yml`, `Makefile:146-178`, `Makefile:356-362`, `Makefile`, `scripts/evidence/effective-verify.sh`, `scripts/docvalid/doc_drift.go`, `plan/known-failures.md` | A release can still ship without a clean, reproducible signal from the intended runner or clean Docker substitute. Current source state includes verifier reproducibility and Linux lint fixes that must be included before rerunning clean evidence. | Include the `effective-verify.sh` PATH fix, golangci-lint pin, and Linux lint fixes in the release candidate; rerun `make ze-release-check` or clean Linux `make ze-verify`; and keep known failures triaged as blocker, platform-only, post-release, or resolved. |
