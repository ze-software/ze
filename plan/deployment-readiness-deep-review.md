...
| BGP diagnostics | Route-refresh decode CLI hard-codes route-refresh code 2 and ignores code 70. | `internal/component/bgp/plugins/route_refresh/route_refresh.go:156-171` | Decode actual input and cover enhanced route-refresh. |
...
| API startup | REST/gRPC bind or TLS errors are warnings, not startup failures, even when configured. | `cmd/ze/hub/api.go:90-127` | Fail closed when an explicitly configured management listener cannot start. |
...
| Telemetry exposure | Prometheus defaults to `0.0.0.0:9273` without auth. | `internal/core/metrics/server.go:228-230` | Default to loopback or require explicit bind for all-interface exposure, and document firewalling. |
...
| TACACS+ | Response header validation is incomplete; fail-open local fallback is a deployment policy risk. | `internal/component/tacacs/client.go:344-373`, `internal/component/aaa/types.go:95-123`, `internal/component/tacacs/authorizer.go:73-97` | Validate response type/version/seq/flags strictly; add configurable strict mode that denies on TACACS infrastructure failure. |
...
|------|---------|----------|--------|
| Documentation | Functional-test docs say the release gate runs 8 suites and excludes L2TP/firewall/web, while Makefile runs 11 suites including those. | `docs/functional-tests.md:17-30`, `Makefile:131-160` | Update docs and add doc drift checks for gate composition. |
| Feature inventory | `docs/features.md` mixes supported, stub-backed, rejected, and not-proven features in one table. Examples: REST/gRPC streaming not wired, L2TP redistribution not proven, VPP stub harness. | `docs/features.md:66`, `docs/features.md:82-87`, `docs/features.md:106` | Classify each feature as supported, experimental, partial, rejected, or stub-backed. |
| Doc drift tooling | Drift checks cover only selected docs and miss README, feature inventory, and functional-test claims. | `scripts/docvalid/doc_drift.go:84-85`, `Makefile:508-539` | Extend generated checks to user-facing release docs. |
| README counts | README test counts are manually stale, though conservative. | `README.md:3`, `README.md:57-60` | Generate counts or phrase as approximate with date. |
| API streaming | REST/gRPC streaming endpoints exist but production hub passes nil stream backend. | `cmd/ze/hub/api.go:176-185`, `internal/component/api/engine.go:123-126`, `docs/features.md:66` | Hide from production docs/OpenAPI or wire a real stream backend. |
| Config sessions | `ConfigSessionManager.get` returns editor pointers without per-session serialization. | `internal/component/api/config_session.go:192-207` | Add session-level mutex or serialize REST/gRPC config mutations. |
...
- Add TACACS header validation and strict fallback mode.
