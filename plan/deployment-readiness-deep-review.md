...
Status: not deployment-ready. The original P0 surface has shrunk substantially, and several P1 security/protocol gaps are code-remediated. Release still depends on clean release-candidate gate evidence, target-runner privileged dataplane evidence, VPP traffic/restart evidence, and full L2TP PPP/NCP/kernel peer proof.
...
| Traffic control | Code-remediated for reviewed in-process safety paths; local privileged tc evidence added, VPP traffic evidence still open. VPP apply has undo-on-error, same-run removal reconciliation, stale-index tolerant deletes, and a startup scan that removes undesired `ze/` policers before re-applying desired state. Netlink now snapshots the original root qdisc before replacement, persists the snapshot under the config state directory, restores via `RestoreOriginal`/`Close`, rejects generic qdiscs and root class/filter state before changing tc state, and no longer synthesizes `fq_codel` on traffic-control removal. A privileged netns integration test now snapshots a real `fq` qdisc, applies HTB through the backend, reloads the persisted snapshot into a fresh backend, and restores the original qdisc. Real VPP FIB add/withdraw evidence exists; VPP traffic-control idempotency/restart and reactor boot/reload kernel-state evidence remain open. | `internal/plugins/traffic/netlink/backend_linux.go`, `internal/plugins/traffic/netlink/snapshot_linux.go`, `internal/plugins/traffic/netlink/backend_linux_test.go`, `internal/plugins/traffic/netlink/integration_linux_test.go`, `internal/plugins/traffic/netlink/translate_linux.go`, `internal/component/traffic/register.go`, `internal/plugins/traffic/vpp/backend_linux.go`, `internal/plugins/traffic/vpp/apply_test.go`, `plan/deferrals.md:200-213` | Run privileged tc integration on the target runner, add reactor-level boot/reload kernel-state proof when CI can expose a managed interface, and produce real VPP traffic-control idempotency/restart evidence. |
| L2TP/PPP | Closed in code for the reviewed safety policy. PPP required auth rejects `AuthMethodNone`; L2TP defaults have finite caps and CHAP-MD5 auth; no-auth requires explicit `allow-no-auth true`; hidden mandatory AVPs fail closed. Docker-backed `xl2tpd` evidence now proves a real LAC can establish Ze's control tunnel and incoming-call session. Full PPP/NCP/kernel peer proof remains open. | `internal/component/ppp/start_session.go`, `internal/component/ppp/session_run.go`, `internal/component/l2tp/config.go`, `internal/component/l2tp/avp.go`, `internal/component/l2tp/schema/ze-l2tp-conf.yang`, `scripts/evidence/l2tp-external-peer.py`, `Makefile` | Keep PPP required-auth, L2TP auth-policy, caps, reload, CLI, hidden mandatory AVP tests, and the external control/session evidence in the gate. Add full PPP/NCP/kernel peer proof when the target runner has suitable L2TP PPP support. |
...
| Feature inventory | Current repo has `docs/features.md` status labels for every row: supported, experimental, partial, rejected, stub-backed, or future. It includes explicit partial/stub caveats for REST/gRPC streaming, L2TP control/session evidence versus full PPP/NCP/kernel peer proof, VPP, dataplane evidence, config validation, reload atomicity, TACACS strict mode, DNS resolver policy, RADIUS Access-Accept rejection, and VPP traffic orphan cleanup. | `docs/features.md:5-113` | Keep status labels current as feature claims change. |
...
| Open deferrals | Open release-relevant rows remain for full L2TP PPP/NCP/kernel peer tests, VPP traffic real-daemon CI, traffic privileged evidence, BMP Loc-RIB, and raw plugin IPC. L2TP route redistribution producer, synthetic BGP advertise/withdraw proof, real VPP FIB add/withdraw evidence, and external L2TP control/session evidence are now closed. Duplicate parser keys, L2TP auth/caps/hidden mandatory AVPs, DNS/NTP safety policy, RADIUS Access-Accept policy, VPP traffic orphan scan, and in-process static plugin validation parity are code-remediated or re-triaged. | `plan/deferrals.md:162-181`, `plan/deferrals.md:200-213`, `plan/deferrals.md:231` | Triage remaining open rows into release blocker, scoped deployment exclusion, post-deployment backlog, or explicitly unsupported. |
...
- Unsupported production API streaming, VPP real-daemon status, L2TP access gaps, and validation limitations stay explicit until closed.
...
make ze-deployment-vpp-test # Docker-backed real VPP FIB add/withdraw evidence
...
UNAVAILABLE: this environment lacks `xl2tpd`, `pppd`, and PPPoL2TP kernel support for full PPP/NCP proof. Docker-backed `xl2tpd` control/session evidence is available through `make ze-deployment-l2tp-test`.
make ze-deployment-preflight
FAIL (strict, expected in this environment): Docker can run the clean `ze-verify` substitute, real VPP evidence, and L2TP control/session evidence. `target-runner`, `xl2tpd`, `pppd`, and PPPoL2TP kernel support remain missing, so deployment evidence is not 100% complete.
make ze-verify
