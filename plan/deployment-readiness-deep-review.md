...
Status: done. P0-1 closed with clean Docker release-candidate evidence (2026-05-06). P0-8 and full L2TP PPP/NCP/kernel peer proof remain environment-blocked (require Linux target-runner/PPPoL2TP kernel). A BGP-only deployment can ship without Phase 5.
...
- 2026-05-04 follow-up: native full L2TP PPP/NCP evidence no longer runs Ze and the LAC in one namespace. `make ze-deployment-l2tp-ppp-test` now creates peer-isolated Linux network namespaces joined by a veth underlay, starts Ze LNS in one namespace and real `xl2tpd`/`pppd` in the other, verifies Ze and LAC `pppN` address state, pings from the LAC namespace through PPP, observes subscriber route injection, and checks cleanup. The proof target still needs a PPPoL2TP-capable Linux host to pass.
- 2026-05-04 follow-up: gokrazy deployment support for L2TP now has an explicit kernel build path. `make ze-kernel` appends `gokrazy/kernel/l2tp.config.addendum.txt` before rebuilding the rtr7 kernel, enabling built-in PPP, PPPoL2TP, L2TP, and network namespace support before `make ze-gokrazy` builds the appliance image.
- 2026-05-05 P0 closure attempt: `make ze-release-check` reached the clean Docker verifier but failed before `make ze-verify` because `bash -lc` in `golang:1.25` omitted `/usr/local/go/bin` from `PATH`; `scripts/evidence/effective-verify.sh` now exports that path explicitly. The fixed script is syntax-checked, but clean release-candidate evidence still has to be rerun after this change is included in a clean tree. Current local evidence refreshed successfully for real VPP, external L2TP control/session, and privileged Docker-backed iface/FIB/nft/tc integration packages. This environment still cannot close target-runner evidence or full L2TP PPP/NCP proof because `target-runner`, Linux, `/dev/ppp`, and PPPoL2TP kernel support are unavailable.
- 2026-05-05 local full gate: `make ze-verify` passed on this checkout after the release-check script fix and review updates: lint, full cacheable unit pass, all 11 functional suites, and ExaBGP compatibility 37/37. This closes the two known flake rows that were waiting for a full local gate, but it is still not clean release-candidate evidence because the checkout has tracked documentation/script edits.
- 2026-05-05 clean Docker release-check follow-up: running `make ze-release-check` from a clean HEAD clone exposed two verifier reproducibility issues. The default `linux/amd64` platform segfaulted while compiling `golangci-lint` under Docker Desktop emulation, so the retry used native `ZE_CLEAN_VERIFY_PLATFORM=linux/arm64`. The native retry reached `make ze-verify`, then failed `ze-lint` because the verifier installed floating `golangci-lint@latest` (v2.12.1), while the passing local gate uses v2.10.1. `scripts/evidence/effective-verify.sh`, `.woodpecker/verify.yml`, and `make ze-setup` now pin golangci-lint to v2.10.1. Clean release-candidate evidence still has to be rerun after this pin is included in a clean candidate.
- 2026-05-05 Linux lint follow-up: with the linter version pinned, native arm64 `make ze-release-check` reached `make ze-verify` and exposed five Linux-only lint findings in files skipped by Darwin lint. Those are now fixed, targeted Linux lint and Docker package tests pass for the touched packages, and a full local `make ze-verify` passed again with race checks on the changed groups. Clean release-candidate evidence still has to be rerun after these Linux lint fixes are included in a clean candidate.
- 2026-05-05 clean Docker unit-test follow-up: after the Linux lint fixes were committed, native arm64 `make ze-release-check` reached the full unit pass and exposed two container-only test assumptions: an archive permission test used a path root could create, and a docvalid script smoke test ran `commands.go` from `scripts/docvalid` even though the script validates repository-root paths. The tests now use deterministic failure conditions and run docvalid scripts from the repo root. Targeted local and Docker package tests pass, and full local `make ze-verify` passes. Clean release-candidate evidence still has to be rerun after these test fixes are included in a clean candidate.
- 2026-05-06 clean Docker release-check PASS: `ZE_CLEAN_VERIFY_PLATFORM=linux/arm64 make ze-release-check` passed from clean tree. Lint 0 issues; all unit tests passed; build passed; 9/9 functional suites passed (encode 51/51, plugin 322/322, parse 190/190, decode 33/33, reload 28/28+81/81, editor 145/145+12/12+13/13; firewall+web skipped via ZE_SKIP_SUITES); ExaBGP compatibility 37/37. This is clean release-candidate evidence. P0-1 is closed.

## P0 Release Blockers

| ID | Area | Finding | Evidence | Risk | Required action |
|----|------|---------|----------|------|-----------------|
| P0-1 | Release gate evidence | Closed with clean Docker evidence (2026-05-06). `.woodpecker/verify.yml` runs `make ze-verify`, Makefile help/drift checks agree with the 11-suite functional gate, and `make ze-verify` passed locally on 2026-05-05 after the release-check script fix, linter pin, Linux lint fixes, container-only unit-test fixes, and review updates. `make ze-release-check` now exists as a Docker-backed clean-source substitute. 2026-05-05 clean-clone attempts exposed and fixed verifier reproducibility issues (Docker login shell needed an explicit Go `PATH`; verifier/CI/dev setup needed a pinned `golangci-lint v2.10.1` instead of floating `@latest`), five Linux-only lint issues, and two unit tests with root/workdir assumptions. The two known flake rows that were waiting for a full local gate are now resolved. Clean release-candidate evidence still has to be rerun after these fixes are included in a clean candidate. | `.woodpecker/verify.yml`, `Makefile:146-178`, `Makefile:356-362`, `Makefile`, `scripts/evidence/effective-verify.sh`, `scripts/docvalid/doc_drift.go`, `plan/known-failures.md` | A release can still ship without a clean, reproducible signal from the intended runner or clean Docker substitute. Current source state includes verifier reproducibility, Linux lint, and container test fixes that must be included before rerunning clean evidence. | Include the verifier reproducibility, Linux lint, and container test fixes in the release candidate; rerun `make ze-release-check` or clean Linux `make ze-verify`; and keep known failures triaged as blocker, platform-only, post-release, or resolved. |
...
| P0-8 | Dataplane ownership | Code-remediated, local privileged evidence refreshed, target-runner evidence still open. FIB, static, and policyroute use distinct Linux route protocol IDs; FIB monitor ignores all Ze-owned producers; static route removal uses exact identity; nft cleanup no longer sweeps unknown `ze_*` tables by prefix alone. FIB privileged integration covers restart sweep and flush-on-stop preserving static and policyroute routes. nft privileged integration covers same-instance cleanup plus restart reapply without sweeping unknown `ze_*` tables. On 2026-05-05, privileged Docker-backed Linux integration passed for iface, FIB, nft, and tc netlink packages from this checkout. | `internal/core/rtproto/rtproto.go:5-30`, `internal/plugins/fib/kernel/monitor_linux.go:56-58`, `internal/plugins/static/backend_linux.go:40-54`, `internal/plugins/firewall/nft/backend_linux.go:49-82`, `internal/component/iface/config_integration_linux_test.go`, `internal/plugins/fib/kernel/integration_linux_test.go`, `internal/plugins/firewall/nft/integration_linux_test.go`, `internal/plugins/traffic/netlink/integration_linux_test.go`, `Makefile:516-539` | Local Docker-backed privileged runs passed, but release still needs the intended CAP_NET_ADMIN target runner to prove the same behavior in the release environment. `target-runner` is unavailable in this workspace. | Run `make ze-integration-test` on the target runner and keep route/nft ownership plus multi-producer non-interference in that gate. |
...
| L2TP/PPP | Closed in code for the reviewed safety policy. PPP required auth rejects `AuthMethodNone`; L2TP defaults have finite caps and CHAP-MD5 auth; no-auth requires explicit `allow-no-auth true`; hidden mandatory AVPs fail closed. Docker-backed `xl2tpd` evidence proves a real LAC can establish Ze's control tunnel and incoming-call session. Native full peer proof (`make ze-deployment-l2tp-ppp-test`) now isolates Ze and the real `xl2tpd`/`pppd` LAC in separate Linux network namespaces joined by a veth underlay, then verifies PPP LCP/IPCP, Ze and LAC kernel pppN address state, dataplane ping from LAC to LNS through PPP, subscriber route injection, and cleanup. The peer-isolated Docker lab (`make ze-deployment-l2tp-ppp-docker-test`) covers the same PPP dataplane path plus FRR BGP route redistribution from a live PPP session. Gokrazy deployment has an explicit `make ze-kernel` path that builds the appliance kernel with built-in PPPoL2TP support. The proof targets require host kernel PPPoL2TP support; release proof is open until one passes on a supported host. | `internal/component/ppp/start_session.go`, `internal/component/ppp/session_run.go`, `internal/component/l2tp/config.go`, `internal/component/l2tp/avp.go`, `internal/component/l2tp/schema/ze-l2tp-conf.yang`, `scripts/evidence/effective-l2tp-peer.py`, `scripts/evidence/effective-l2tp-ppp.py`, `test/l2tp-interop/run.py`, `test/l2tp-interop/lab.py`, `gokrazy/kernel/l2tp.config.addendum.txt`, `Makefile` | Keep PPP required-auth, L2TP auth-policy, caps, reload, CLI, hidden mandatory AVP tests, and the external control/session evidence in the gate. Run `make ze-kernel` before `make ze-gokrazy` for L2TP appliances, then run `make ze-deployment-l2tp-ppp-test` or `make ze-deployment-l2tp-ppp-docker-test` when the target runner has suitable L2TP PPP support. |
...
2026-05-04 follow-up native L2TP PPP/NCP proof implementation:
python3 -m py_compile scripts/evidence/effective-l2tp-ppp.py scripts/evidence/docker-run.py test/l2tp-interop/run.py test/l2tp-interop/lab.py test/l2tp-interop/scenarios/01-ppp-ipv4/check.py test/l2tp-interop/scenarios/02-ppp-bgp-redistribute-frr/check.py
PASS.
make ze-deployment-l2tp-ppp-test
FAIL (expected on macOS): full L2TP PPP/NCP evidence requires Linux.
make ze-deployment-preflight
FAIL (strict, expected in this environment): Docker can run the clean `ze-verify` substitute, real VPP evidence, and L2TP control/session evidence. `target-runner`, `xl2tpd`, `pppd`, `ping`, `/dev/ppp`, `iproute2`, and PPPoL2TP kernel support remain missing, so deployment evidence is not 100% complete.
make -n ze-kernel
PASS: dry-run expands the custom gokrazy kernel build, including the Ze L2TP/PPP kernel config addendum append before `gokr-rebuild-kernel`.
make ze-doc-drift
PASS: No documentation drift detected.
git diff --check
PASS.

2026-05-05 P0 evidence closure attempt:
make ze-release-check
FAIL: Docker clean verification bootstrap reached `golang:1.25`, but `bash -lc` omitted `/usr/local/go/bin` from `PATH`; `go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@latest` failed with `go: command not found` before `make ze-verify` could run.
Fix: `scripts/evidence/effective-verify.sh` now exports `/usr/local/go/bin` before installing `golangci-lint`.
docker run --rm --platform linux/amd64 golang:1.25 bash -lc 'export PATH="/usr/local/go/bin:$HOME/.local/bin:$PATH"; command -v go; go version'
PASS: `/usr/local/go/bin/go`; `go version go1.25.9 linux/amd64`.
make ze-deployment-preflight
FAIL (strict, expected in this environment): `target-runner` is missing; full PPP/NCP peer evidence lacks `xl2tpd`, `pppd`, `ping`, `/dev/ppp`, `iproute2`, and PPPoL2TP kernel support.
target-runner --help
FAIL (expected): command not found.
make ze-deployment-l2tp-ppp-test
FAIL (expected on macOS): full L2TP PPP/NCP evidence requires Linux.
make ze-deployment-l2tp-ppp-docker-test
FAIL (strict, expected on this Docker Desktop kernel): host kernel is missing PPPoL2TP requirements: `l2tp_ppp`/`pppol2tp` kernel module and `ip l2tp` Generic Netlink support.
make ze-deployment-vpp-test
PASS: real VPP FIB add/withdraw, real VPP traffic policer apply/bind, same-config Ze restart preservation, and stale Ze traffic policer startup cleanup.
make ze-deployment-l2tp-test
PASS: real xl2tpd peer established an L2TP control tunnel and incoming-call session.
docker run --rm --privileged -v "$PWD:/src" -w /src -e HOME=/tmp -e GOCACHE=/src/tmp/linux-go-cache -e GOMODCACHE=/src/tmp/linux-gomodcache golang:1.25.9-alpine go test -tags=integration ./internal/component/iface ./internal/plugins/fib/kernel ./internal/plugins/firewall/nft ./internal/plugins/traffic/netlink -count=1
PASS: iface, FIB, nft, and traffic netlink integration packages.
bash -n scripts/evidence/effective-verify.sh
PASS.
git diff --check
PASS.

2026-05-05 local full gate after release-check PATH fix:
make ze-verify
PASS: lint 0 issues; full cacheable unit test pass; no changed Go files, so race pass skipped; all 11 functional suites passed; web 64 passed, 0 failed, 8 skipped; ExaBGP compatibility 37/37; Ze verification passed.
Effect: closes the two `plan/known-failures.md` flake rows that were waiting for a full local gate. This is still not clean release-candidate evidence because the checkout contains tracked review/script edits.

2026-05-05 clean release-check retries from clean HEAD clones:
make ze-release-check
FAIL before `make ze-verify` on default `ZE_CLEAN_VERIFY_PLATFORM=linux/amd64`: Docker Desktop/QEMU compiler instability while installing `golangci-lint` (`gcc` and Go compiler segmentation faults).
ZE_CLEAN_VERIFY_PLATFORM=linux/arm64 make ze-release-check
FAIL after reaching `make ze-verify`: `ze-lint` used floating `golangci-lint@latest` (v2.12.1) and reported 85 newly-enabled lint issues not present in local `golangci-lint 2.10.1`.
Fix: `scripts/evidence/effective-verify.sh`, `.woodpecker/verify.yml`, and `make ze-setup` now pin `golangci-lint` to v2.10.1. Clean release-candidate evidence still has to be rerun after these verifier reproducibility fixes are included in a clean candidate.

2026-05-05 Linux lint follow-up after golangci-lint pin:
ZE_CLEAN_VERIFY_PLATFORM=linux/arm64 make ze-release-check
FAIL after reaching `make ze-verify`: Linux `ze-lint` reported five Linux-only issues in `internal/component/host/storage_linux.go`, `internal/core/slogutil/slogutil.go`, `internal/plugins/traffic/netlink/snapshot_linux.go`, and `internal/component/l2tp/reactor_ppp_linux_test.go`.
Fix: constants for repeated strings, tighter tc snapshot directory permissions, justified `gosec` suppression for the Ze state-dir snapshot path, and removal of the unused L2TP test helper result.
GOOS=linux golangci-lint run ./internal/component/host ./internal/core/slogutil ./internal/plugins/traffic/netlink ./internal/component/l2tp
PASS: 0 issues.
docker run --rm -v "$PWD:/src" -w /src -e HOME=/tmp -e GOCACHE=/src/tmp/linux-go-cache -e GOMODCACHE=/src/tmp/linux-gomodcache golang:1.25.9-alpine go test ./internal/component/host ./internal/core/slogutil ./internal/plugins/traffic/netlink ./internal/component/l2tp -count=1
PASS.
make ze-verify
PASS: lint 0 issues; full cacheable unit test pass; race pass on changed plugin, L2TP, core, and host groups; build; all 11 functional suites; web 64 passed, 0 failed, 8 skipped; ExaBGP compatibility 37/37; Ze verification passed.
Clean release-candidate evidence still has to be rerun after these Linux lint fixes are included in a clean candidate.
