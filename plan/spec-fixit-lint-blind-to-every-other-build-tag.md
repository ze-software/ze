...
| AC-3 | A deliberate lint violation is added to a `//go:build !ze_<feature>` stub | `make ze-lint` reports it | **MET.** The `compile-out` row derives its `without` from `.golangci.yml`'s own tag list, so the build is `ze_core` and nothing else, and one row selects every stub because the gates are independent. Measured with `behaviour` planted in a comment in `internal/plugins/static/backend_vpp_off_linux.go` (`linux && !ze_vpp`) and restored from a pristine copy afterwards (`git status` clean): `python3 scripts/dev/lint_flavors.py --scope "./internal/plugins/static"` reports the `misspell` from the `compile-out` flavor and exits 1. The owner ruled on 2026-08-24 that `unused` stays enabled and each feature-only helper takes its consumer's build constraint; 46 of the 47 findings are gone that way, not by an exclusion, or with code no build could reach. The AC's own condition -- a planted violation IS reported -- is MET, and so is the burn-down behind it: the 47th was `forceExitOnSignal` and `bgpDecodeLinked` joined it when the decode tests stopped reading the constant, and both were fixed on 2026-08-24 rulings at `89c35d8ed` and `d580c0870`. AC-7 carries the measurement |
...
- AC-3, the `compile-out` row: `scripts/dev/lint_flavors.py` (`feature_gate_tags`, the row, the deletion of `COMPILE_OUT_REASON`, `COMPILE_OUT_STUBS` and `compile_out_stub`, and `population()` skipping a tracked path the working tree no longer has -- `git ls-files` answers from the index, so deleting a `.go` file made the coverage assertion red with every pass at 0 issues), `scripts/dev/lint_flavors_test.py` (`test_the_compile_out_row_keeps_ze_core_alone` and `test_a_file_deleted_in_the_working_tree_leaves_the_population` replace `TestCompileOutStub` and `TestStubCeiling`)
...
`make ze-lint` analysed one build. It now analyses eighteen. `scripts/dev/lint_flavors.py`
...
`scripts/checks/verify_scope_selector.go` lost the rule that kept an edit under
...
- Eleven `//go:build ze_chaos` tests in `cmd/ze/ze_chaos_main_test.go` were executed by nothing.
  `mk/test-chaos.mk` now runs them (`CHAOS_CLI_TAGS`), and `chaosRun` dispatches through
...
- `ai/rules/points/commands/lint-gate/what-ze-lint-changed-covers-and-what-it-costs.md` and
...
- `make ze-doc-verify` exits 0, re-run by this closure on 2026-08-25. The one anchor that
...
|-------------|--------|----------|-------|
| Cause 1: the package set names four roots | Done | `Makefile`, `ZE_LINT_PKGS := ./...` | 55 files, 0 findings once loaded |
| Cause 2: every gate is ON, so no compile-out stub is loaded | Done | `scripts/dev/lint_flavors.py`, `tagless_config`, the `compile-out` row | mechanism found (A-2 broken); burn-down 65 of 67 |
| Cause 3: personality and capability tags neither pass carries | Done | `scripts/dev/lint_flavors.py`, `FLAVORS` | installer, installer-nofault, distro, appliance, setup, setup-standalone, capability, tinygo |
| Cause 4: GOARCH, which no pass varies | Done | `FLAVORS`, `linux-arm64` and `linux-other-arch` | promoted from "a floor" to covered |
| Zero findings remain in the newly reached files | Done | `scripts/dev/lint_flavors.py`, all 17 rows | `python3 scripts/dev/lint_flavors.py` exit 0 on 2026-08-25 at `d580c0870`, every row `0 issues.` |

### Acceptance Criteria
| AC ID | Status | Demonstrated By | Notes |
|-------|--------|-----------------|-------|
| AC-1 | Done | an unplanted `misspell` in `scripts/status/verify_run_test.go` failed the first run after `ZE_LINT_PKGS := ./...` | |
...
| AC-5 | Done | `make generate` then `git diff --exit-code .golangci.yml` clean | |
| AC-6 | Done | every `FLAVORS` tag checked against `feature-gates.txt`: no entry | |
| AC-7 | Done | `python3 scripts/dev/lint_flavors.py` exit 0 at `d580c0870`, all 17 rows `0 issues.` | the last two were fixed on 2026-08-24 owner rulings (`89c35d8ed`, `d580c0870`), not suppressed |
| AC-8 | Done | `python3 scripts/dev/lint_flavors.py --coverage` exit 0, "every tracked Go file is linted, except the 2 stated above" | 245 to 2 |

### Tests from TDD Plan
| Test | Status | Location | Notes |
|------|--------|----------|-------|
| `TestLintRunsEveryBuildFlavor` | Done | `scripts/status/verify_run_test.go` | also pins `ZE_LINT_PKGS` at `./...` |
| `TestChangedLintRunsEveryBuildFlavor` | Done | `scripts/status/verify_run_test.go` | refuses a `;` anywhere in the `&&` chain |
| `TestLintFlavorDriverCarriesTheLinterCeilings` | Done | `scripts/status/verify_run_test.go` | pins `-j` and `GOMEMLIMIT` against `ZE_LINT_RUN` |
| `TestSelectorWidensForTheInstallerInitrd` | Done | `scripts/checks/verify_scope_selector_test.go` | |
...
| `lint_flavors_test.py` (8 cases) | Done | `scripts/dev/lint_flavors_test.py` | run by `go test ./scripts/dev/` through `python_tests_test.go` |
| `TestIsLocalhostPprof` | Done | `cmd/ze/pprof_test.go` | moved so it carries `pprof.go`'s `!tinygo` |

### Files from Plan
| File | Status | Notes |
|------|--------|-------|
| `scripts/dev/lint_flavors.py` | Done | created, 600L |
| `scripts/dev/lint_flavors_test.py` | Done | created |
| `cmd/ze/pprof_test.go` | Done | created |
| `Makefile`, `mk/test-chaos.mk`, `scripts/checks/verify_scope_selector{,_test}.go`, `scripts/status/verify_run_test.go` | Done | committed `d16046963`, `98f4297eb` |
...
| The 245 blind files fall to a number this spec names | measurement, re-derived from the tree | `python3 scripts/dev/lint_flavors.py --coverage`, exit 0: "every tracked Go file is linted, except the 2 stated above". The two are `examples/plugin/go/main.go` and `tools.go`, each with the change that would end its exclusion |
...
| Both lint paths are covered, not just the full one | guard test, proven to discriminate | `TestLintRunsEveryBuildFlavor`, `TestChangedLintRunsEveryBuildFlavor`, `TestLintFlavorDriverCarriesTheLinterCeilings`. Each goes RED against a Makefile with the driver line removed, with the four roots restored, or with the `&&` replaced by `;` |
...
| **Zero findings remain in the newly reached files** | measurement, re-run by this closure | ACHIEVED. `python3 scripts/dev/lint_flavors.py` exit 0 on 2026-08-25 at `d580c0870`: all 17 flavor rows print `0 issues.`, and the run ends with the coverage assertion. The last two findings were fixed on 2026-08-24 owner rulings, not suppressed and not excluded |
...
| Rounds | 4 |
| Reviewer lenses used | rounds 3 and 4, one agent per round, every lens in each: gate-population (does the pass load what the record says), record-accuracy (each closure claim read against the producing file), style (`docs/contributing/ze-style.md` over every changed Go file), guard discrimination, test integrity, commit-hygiene (which of the diff is landed and which is not). Round 4 read the three commits that landed after round 3 -- `587f86ad4`, `89c35d8ed`, `d580c0870` -- and re-measured every claim round 3 had left open |

### Findings fixed
| # | Severity | Finding | Location | Fixed by |
|---|----------|---------|----------|----------|
| 1 | ISSUE (round 2) | a blocking rule still stated the selector behaviour the fix removed | `ai/rules/points/precommit-verify/running-the-gate/what-each-changed-path-selects.md` | the row now names only the module root, and a second row sends `cmd/ze-installer` to `./...`; rendered by `make ze-rules-render-update` (committed `98f4297eb`) |
| 2 | NIT (round 2) | the file header said `uncompiledTreeReaders` maps two directories | `scripts/checks/verify_scope_selector.go` | the header says one, and says where `gokrazy/modcache` is answered instead (committed `98f4297eb`) |
...
| 10 | NOTE (round 4), escalated | Another session's uncommitted `test/weakened.md` DELETES the `bgp_decode_nolink_test` row `d580c0870` committed, while appending seven of its own. A lost update: that session wrote the file back from a copy taken before the row landed. If it commits as it stands, the record of why a test file was deleted is destroyed | `test/weakened.md`, working tree | IT COMMITTED. `af92e8a04` landed the file with the row gone, so `git show HEAD:test/weakened.md` no longer holds it and the record of why `bgp_decode_nolink_test.go` was deleted is out of the ledger. Restoring it is a one-row append and belongs to whoever owns that commit; this closure was right not to clobber an in-flight file, and the cost of that correctness is that the warning outlived the window in which it could be acted on |
...
Two reds were checked and charged elsewhere. `make ze-repository-check` exits 1 on
...
session's uncommitted work. `scripts/dev/audit-test-relaxation.py` exits 1 over
...
|------|--------|----------|
| `scripts/dev/lint_flavors.py` | yes | `ls -la` 2026-08-24, 22352 bytes |
| `scripts/dev/lint_flavors_test.py` | yes | `ls -la` 2026-08-24, 7190 bytes |
...
|-------|-------|----------------|
| AC-3 | a violation in a compile-out stub is reported | the row loads `cmd/ze/hub` and reports on it. On 2026-08-24 `python3 scripts/dev/lint_flavors.py --scope "./cmd/ze/hub"` exited 1 with the `compile-out` row naming two real `unused` findings. Re-run by this closure on 2026-08-25 at `d580c0870`: exit 0, `0 issues.` from both the `capability` and the `compile-out` row, because those two findings were fixed |
| AC-7 | zero findings remain | **TRUE at `d580c0870`.** `python3 scripts/dev/lint_flavors.py` exit 0, all 17 rows printing `0 issues.`, re-measured by this closure rather than carried from an earlier run |
| AC-8 | the blind count is 2 | `python3 scripts/dev/lint_flavors.py --coverage` exit 0, re-run 2026-08-25: "every tracked Go file is linted, except the 2 stated above", the two being `examples/plugin/go/main.go` and `tools.go` |
...
|-------------|----------|----------|
| `make ze-lint` -> the driver -> the coverage assertion | `TestLintRunsEveryBuildFlavor` (`scripts/status/verify_run_test.go`) | `make ze-unit-pkg-test PKG=./scripts/status RUN='TestLintRunsEveryBuildFlavor\|TestChangedLintRunsEveryBuildFlavor\|TestLintFlavorDriverCarriesTheLinterCeilings'` exit 0, 6.1s. Read: each test expands the make variables and reads the recipe lines, so it fails on a recipe that drops the driver |
| `make ze-lint-changed` on an installer edit -> `./...` | `TestSelectorWidensForTheInstallerInitrd` (`scripts/checks/verify_scope_selector_test.go`) | `make ze-unit-pkg-test PKG=./scripts/checks RUN='TestSelectorWidensForTheInstallerInitrd\|TestSelectorMapsGoTreesTheUnitBuildNeverCompiles'` exit 0, 6.5s |
| `_ze-lint-impl` -> `BASE_PASSES` | `test_base_passes_match_the_lint_recipe` (`scripts/dev/lint_flavors_test.py`) | `python3 scripts/dev/lint_flavors_test.py` exit 0, 8 tests. It reads the Makefile, so a recipe change that leaves `BASE_PASSES` behind fails it |
| `make ze-lint` -> the whole population | the coverage assertion itself | `--coverage` exit 0, residue 2 |
...
| A-5 | confirmed, because each flavor is SCOPED | a full-tree pass is 286s; a flavor pass is 8-39s over 1 to 59 packages. The Makefile comment records the added minute |
...
|---------------------------------|-----------------|----------|
| #10 test infrastructure changed -> `docs/contributing/testing.md` | the new section's pass table is read against `FLAVORS` and `BASE_PASSES` in `scripts/dev/lint_flavors.py`; the `--coverage` and `--list` commands in it were both run | yes |
| #10 -> `docs/architecture/testing/verify-freshness-scope.md` | the paragraph is read against `uncompiledTreeReaders` in `scripts/checks/verify_scope_selector.go`, which now holds one directory | yes |
| #10 -> `ai/rules/points/commands/lint-gate/...` and `.../precommit-verify/running-the-gate/...` | `make ze-doc-verify` reports "rules-points: 29 rules are fresh" and "all 29 rules round-trip byte-identical" | yes |
| #16 source anchors | `make ze-doc-verify` exits 0, re-run 2026-08-25. Its "Source anchors" stage reports "checked 2192 code paths, 517 packages, all references valid". The one anchor that failed on 2026-08-24 was `docs/guide/web-interface.md` naming `liveAAABundleAuthenticator.Authenticate`, never this spec's, and another session landed the fix at `dcbb0efa6` | yes |
...
| `ai/DOCS-TO-CODE.md` | regenerated: `python3 scripts/dev/docs_to_code.py --check` exit 0, "266 design docs, up to date". `git check-ignore` confirms it is gitignored and untracked, so it belongs in no commit | yes |
...
|-------|-------|----------|
| driver, Makefile, selector, guard tests, chaos make, rpki/chaos/tinygo compile fixes, hub build constraints, rules points | committed `d16046963`, `2e0ebcf18`, `98f4297eb`, `448740364`, `f1153a3d5` | `git show --stat` on each |
