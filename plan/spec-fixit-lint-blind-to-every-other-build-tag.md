...
| Python, not Go, for the driver | a `//go:build ignore` program beside `scripts/checks/tracked_build.go` | `docs/contributing/ze-go-style.md` puts Ze's tooling in Python, and `scripts/dev/python_tests_test.go` already runs a `<tool>_test.py` inside `make ze-unit-test` with no make target of its own |
