...
Record and resolve one RFC 4271 Section 9.1.2.2 finding. Ze's public MED removal syntax runs before route selection. Two lower-level egress paths can still remove MED after route selection and before IBGP readvertisement. This reopens the route-loop sequence that Section 9.1.2.2 describes.
...
This spec does not authorize an implementation. It records the finding, the proposed boundary, and the proof required before a change can land.
...
- Raw egress replacement and accumulator suppression make that claim false.
...
- [ ] `internal/component/bgp/reactor/forward_med.go` - returns without a MED operation for internal destinations. It does not inspect whether an egress raw replacement or accumulated suppression removed MED.
- [ ] `internal/component/bgp/reactor/forward_med_test.go` - proves configured ingress removal and normal internal propagation, but does not exercise post-selection raw or accumulator removal.
...
- [ ] `test/plugin/med-removal-export-refused.ci` - proves the public export directive is refused. It does not cover lower-level raw replacement or accumulator suppression.
...
- Prevent a raw egress replacement from removing MED between route selection and IBGP readvertisement.
- Prevent a final accumulated MED suppression from removing MED between route selection and IBGP readvertisement.
...
5. Egress policy can return a whole-payload raw replacement or append attribute operations.
...
| In-process filter to reactor | `ModAccumulator` attribute operations | Yes, producer read |
...
- `runEgressPolicyChain` and in-process filters supply accumulated operations.
- `applyFactsMED` is the shared post-policy MED enforcement point on both forward rails.
...
|----|------------|-------|----------|--------------|--------|
| A-1 | A raw egress replacement that omits MED is reachable after route selection | `runEgressPolicyChainASN4` and `forwardUpdateCore` producers | The raw-path half of the proposed guard is unnecessary | A focused wiring test through `forwardUpdateCore` | unvalidated |
| A-2 | An in-process egress filter can append a final MED suppression | Generic attribute-operation API accepts attribute code 4 suppression | The accumulator half of the proposed guard is unnecessary | A focused wiring test through the in-process egress chain | unvalidated |
| A-3 | A destination-specific MED Set is permitted and must survive | Existing Section 5.1.4 provenance design and `modsSetMED` behavior | Restoring the selected value could override valid policy | Positive test with a later Set | unvalidated |
| A-4 | The source payload passed to `applyFactsMED` is exactly the post-ingress payload selected by the RIB | Both forward call sites and reactor notification path | A presence comparison would use the wrong baseline | Call-site wiring test and source trace | validated by source |
...
| R-5 | A unit-only proof misses a bypass on one forward rail | One rail has no wiring test or interop observation | Drive both `forwardUpdateCore` and `reactorForwardRS`, then add discriminating wire-visible proof |
...
|-------------|---|--------------|------|
| Raw egress policy response for an IBGP destination | → | `forwardUpdateCore` then `applyFactsMED` | `TestRFC4271IBGPReadvertisementRejectsRawMEDRemoval` |
| In-process egress MED suppression for an IBGP destination | → | `reactorForwardRS` then `applyFactsMED` | `TestRFC4271IBGPReadvertisementRejectsAccumulatedMEDRemoval` |
...
| AC-2 | A selected source payload contains MED and the final accumulated MED operation suppresses it for an IBGP destination | The final wire UPDATE contains the selected MED |
| AC-3 | The post-ingress selected source payload has no MED | IBGP egress does not add MED |
| AC-4 | Egress policy records a valid MED Set after selection | The final wire UPDATE contains the policy value; the guard does not replace it |
| AC-5 | The destination is an ordinary EBGP peer | Existing received-MED suppression is unchanged |
| AC-6 | The destination is an RFC 7947 route-server client | Existing MED transparency is unchanged |
| AC-7 | No post-selection removal occurs | No MED operation is added and the zero-copy path remains available |
| AC-8 | RFC4271-9.1.2.2-2 is changed from not-applicable to proven | One positive and one negative tagged test fail when the guard is removed and pass with it |
| AC-9 | The existing raw-base dedup fixture runs with the guard | It still proves unequal bases do not share materialization without using MED as the base marker |
| AC-10 | The change is reviewed | The diff contains no MED syntax, parser, formatter, help, or public policy documentation change |

## End-to-End User Stories

| # | User does | Path through system | Test proving it works |
|---|-----------|--------------------|-----------------------|
| 1 | Receives an EBGP route with MED and readvertises it over IBGP after egress policy | Wire ingress to RIB selection to egress policy to peer wire | `TestRFC4271IBGPReadvertisementRejectsRawMEDRemoval` plus a discriminating GoBGP scenario |
| 2 | Removes MED with the existing import syntax | Import policy to pre-selection payload to RIB to IBGP wire | Existing `med-removal-before-decision.ci` and scenario 61 remain unchanged |
...
|------|------|-----------|--------|
| `TestRFC4271IBGPReadvertisementRejectsRawMEDRemoval` | `internal/component/bgp/reactor/forward_med_test.go` | AC-1 and raw-policy wiring | not written |
| `TestRFC4271IBGPReadvertisementRejectsAccumulatedMEDRemoval` | `internal/component/bgp/reactor/forward_med_test.go` | AC-2 and in-process wiring | not written |
| `TestRFC4271IBGPReadvertisementDoesNotSynthesizeMED` | `internal/component/bgp/reactor/forward_med_test.go` | AC-3 | not written |
| `TestIBGPEgressMEDSetWins` | `internal/component/bgp/reactor/forward_med_test.go` | AC-4 | not written |
| `TestDifferentBaseSameEditSetNeverShares` | `internal/component/bgp/reactor/forward_dedup_test.go` | AC-9 after fixture-only marker change | existing, fixture change required only if implementation proceeds |
...
|------|----------|-------------------|--------|
| `med-ibgp-post-selection-removal` | `test/plugin/med-ibgp-post-selection-removal.ci` | A reachable egress policy attempts removal after selection; the internal peer still receives MED | design required |
...
|----------|-----------|-------------|----------------|--------|
| `med-ibgp-post-selection-removal-gobgp` | `test/interop/scenarios/` | GoBGP | A foreign IBGP peer receives MED after a reachable post-selection removal attempt; reverting the guard removes MED and fails the check | design required |
...
- `rfc/short/rfc4271.md` - remove the disposition only after proof passes.
- `ai/RFC-REQUIREMENTS.md` - regenerated requirement index.
- `rfc/requirements/rfc4271.md` - regenerated RFC 4271 evidence shard.

## Files to Create

- `test/plugin/med-ibgp-post-selection-removal.ci` - functional producer-to-wire proof, subject to research of a reachable test policy producer.
- `test/interop/scenarios/NN-med-ibgp-post-selection-removal-gobgp/` - discriminating foreign-peer proof, subject to the same producer research.
...
| Functional test for new RPC/API | Yes | `test/plugin/med-ibgp-post-selection-removal.ci`; no new RPC or API |
...
| 8 | Plugin SDK/protocol changed? | No | Existing raw and accumulator contracts remain |
| 9 | RFC behavior implemented, changed, or newly proven? | Yes | `rfc/short/rfc4271.md`, `rfc/requirements/rfc4271.md`, `ai/RFC-REQUIREMENTS.md` |
...
| 16 | Changed source referenced by existing anchors? | Yes | `docs/architecture/bgp/egress-attribute-rules.md` anchors `forward_med.go` |
| 17 | Existing docs show config examples? | Preserve | Do not edit the other session's MED syntax examples |

## Implementation Steps

1. **Phase: Wiring**
   - Prove the raw and accumulator removal paths through the two production forward rails.
   - Stop if either claimed producer is unreachable.
2. **Phase: Discriminating tests**
   - Add positive and negative RFC-tagged tests.
   - Remove the proposed enforcement and confirm the positive tests fail for the intended wire difference.
...
4. **Phase: Functional and interop proof**
   - Exercise a reachable post-selection removal producer.
   - Observe the final MED state from a foreign IBGP peer.
5. **Phase: RFC disposition and documentation**
   - Remove the not-applicable annotation only after all proof passes.
   - Regenerate the requirement ledgers.

### Critical Review Checklist

| Check | What to verify for this spec |
|-------|------------------------------|
| Completeness | Both raw and accumulator producers are proved reachable or removed from scope with evidence |
| Feature completeness | Both forward rails enforce the same boundary |
| Correctness | Only MED absence is restored; a valid later Set is preserved |
| Naming | No public MED syntax or identifier changes |
| Data flow | The source is the selected post-ingress payload and the base is the post-policy destination payload |
| RFC compliance | Tagged tests prove RFC4271-9.1.2.2-2 without weakening RFC4271-9.1.2.2-3 |

### Deliverables Checklist

| Deliverable | Verification method |
|-------------|---------------------|
| Reachability proof for both egress producers | Focused wiring tests through each forward rail |
| RFC-tagged positive and negative tests | `make ze-rfc-check` plus mutation run |
...
| Functional wire behavior | Focused plugin suite target for the new `.ci` |
| Foreign-peer behavior | Focused GoBGP interop scenario |
...
| MED syntax file appears in the diff | Stop; remove it from this spec's work |

## Design Insights

- A public directive can be correctly confined while lower-level generic policy mechanisms bypass that confinement.
...
- The exact functional and interop producer fixture requires a separate research pass before this spec can move from design to ready.
...
- [ ] AC-1 through AC-10 demonstrated
- [ ] Every user story has a working path and a passing test
- [ ] Wiring Test table complete
- [ ] Feature code integrated through the existing forward rails
- [ ] Integration and Documentation checklists revalidated
- [ ] Architectural Verification revalidated
- [ ] Every assumption confirmed or broken

### TDD

- [ ] Tests written
- [ ] Tests fail without enforcement
- [ ] Tests pass with enforcement
- [ ] MED boundary values covered
- [ ] Functional test passes
- [ ] Interop test passes and discriminates
