...
3. `internal/component/plugin/ipc/tls.go` - current TLS auth implementation
4. `pkg/plugin/sdk/sdk.go` - SDK TLS client

## Task

Harden the TLS plugin transport with per-plugin unique tokens and certificate pinning. Current implementation uses a shared secret for all plugins (any plugin can impersonate any other) and `InsecureSkipVerify` (MITM on non-localhost).

Deliverables: (1) per-plugin random tokens with name binding, (2) cert fingerprint pinning via env var, (3) token passing via temp file instead of env var
...
### Entry Point
- Config: `plugin { hub { secret "..."; } }` parsed into HubConfig
- Runtime: plugin connects via TLS, sends `#0 auth {"token":"...","name":"..."}`

### Transformation Path
1. Engine generates per-plugin random token during process creation
2. Token written to temp file, path passed via `ZE_PLUGIN_HUB_TOKEN_FILE` env var
3. Plugin reads token from file, deletes file, connects via TLS
4. Engine validates token against expected per-plugin value + name binding
5. Cert fingerprint passed via `ZE_PLUGIN_CERT_FP`, plugin verifies server cert
...
|-------------|---|--------------|------|
| Config with per-plugin token | -> | Authenticate validates token+name binding | `test/plugin/tls-per-plugin-token.ci` |
| Plugin connects to non-localhost with cert pinning | -> | SDK verifies cert fingerprint | `TestSDKCertPinning` |
