# Spec: ospfv3-ext-1 -- OSPFv3 Multiple Address Families (RFC 5838)
...
5. `plan/spec-ospfv3-0-umbrella.md` "Out of Scope" + the reserved, validated Instance-ID plumbing ext-1 consumes; the FIB-install-via-Loc-RIB path; the scope-aware LSA model
6. `plan/spec-ospfv3-ext-0-umbrella.md` rows ext-1 (this spec): "Map address families to OSPFv3 Instance-ID ranges (§2.4) using the base's reserved Instance-ID plumbing; per-AF topologies and route install; AF-aware LSDB / SPF without re-opening the RFC 5340 codec"
...
8. `internal/plugins/ospf/register.go` -- `eng` (v4) + `eng6` (v6 IPv6-unicast) spawn; `newEngineWithCodec(transport, codec)`; `cfg.V6` drives the v6 instance
...
Add OSPFv3 multiple-address-family support (RFC 5838) to the unified OSPF engine
at `internal/plugins/ospf/`. The OSPFv3 base (delivered via
`plan/spec-ospfv3-0-umbrella.md`) runs a single IPv6-unicast OSPFv3 instance
over the ospfv3 codec/transport, and deliberately reserved a validated 8-bit
Instance-ID field (`dispatcher.instanceID`, `cfg.V6.InstanceID`,
`address-family/ipv6/instance-id` in YANG) precisely so multi-AF could attach
here without re-opening the RFC 5340 codec.

RFC 5838 carries multiple address families over OSPFv3 by mapping each AF to a
reserved **Instance ID range** and tagging each instance's packets with the
**AF-bit** in the Options field:

| Address family | Instance ID range | Ze Loc-RIB family |
|----------------|-------------------|-------------------|
| IPv6 unicast (base default) | 0-31 | `family.IPv6Unicast` |
...
the wire only by the Instance ID. The base already runs one engine per family
(`eng` for v4, `eng6` for v6); this spec generalises the v6 family from a single
fixed IPv6-unicast instance to **one engine instance per configured OSPFv3
address family**, each constructed with the v6 codec/transport, a per-AF Instance
ID validated to fall inside that AF's RFC 5838 range, the AF-bit set in its
originated Options, an AF-specific LSDB (already inherent: each engine owns its
own `LSDB`), and route install into the **correct Loc-RIB family** (the existing
installer hardcodes `family.IPv4Unicast` regardless of address family -- a
latent gap this spec fixes by parameterising the install family).

This includes **IPv4-over-OSPFv3**: an IPv4-unicast OSPFv3 instance (Instance ID
64-95) carries IPv4 prefixes inside the OSPFv3 address-free LSA model. RFC 5838
§2.7 reuses the RFC 5340 prefix encoding for IPv4 (a 0-32-bit prefix in a single
32-bit word) and carries the IPv4 next-hop as the link-local-derived adjacency
address; the resulting routes install into `family.IPv4Unicast`. This is the AF
that most exercises the abstraction (a non-IPv6 prefix family over the IPv6
transport) and is the headline RFC 5838 capability beyond the base IPv6-unicast
instance.
...
| AF-bit handling | Add the AF-bit (`OptAF`, RFC 5838 §2.4) to `v3/types/options.go`; set it in originated Hello/DD Options for ANY multi-AF instance (RFC 5838 §2.4: an AF-bit-capable router sets it); per RFC 5838 §2.5 the AF-bit MUST be set for non-IPv6-unicast AFs and the base IPv6-unicast instance sets it only when it is itself multi-AF-aware; mismatch handling per §2.6 (an OSPFv3 instance ignores the AF-bit for the default IPv6-unicast AF for backward compatibility) |
| Per-AF LSDB | Each AF instance already owns a private `LSDB` (one engine per family); ext-1 spawns one engine per configured AF so the separation is structural, not a new key field (RFC 5838 §2: "separate link-state database per address family") |
| Per-AF route install | Parameterise the SPF `Installer` family: an IPv6-unicast instance installs into `family.IPv6Unicast`, IPv4-unicast into `family.IPv4Unicast`, etc.; fixes the current `NewInstaller` hardcode to `family.IPv4Unicast` for the v6 engine |
| IPv4-over-OSPFv3 | An IPv4-unicast OSPFv3 instance decodes/originates IPv4 prefixes through the RFC 5340 prefix model (RFC 5838 §2.7: a 0-32-bit prefix in one 32-bit word) and resolves the IPv4 next-hop from the adjacency; routes install into `family.IPv4Unicast` |
| Config | Generalise `address-family` to accept `ipv6-unicast`/`ipv4-unicast`/`ipv6-multicast`/`ipv4-multicast` (or an AF + instance-id pair) with a validated per-AF Instance-ID range; preserve the existing `address-family ipv6` shape as the IPv6-unicast AF |
...
| Item | Where |
|------|-------|
| Any change to the RFC 5340 common header / packet types | base (unchanged; only the existing Instance-ID field is consumed) |
...
| New LSA types or a new scope model | base (RFC 5838 reuses the RFC 5340 LSA set verbatim) |
| Opaque / TE / SR extension LSAs | ext-6 and later (RFC 5340 carries extensions as native LSAs; no opaque carrier in v3) |
| RFC 7166 / RFC 4552 auth changes for multi-AF | base auth path is per-instance already; ext-2 owns IPsec |
| OSPFv2 multi-instance (RFC 6549) | not applicable -- this is OSPFv3-only |
...
- [ ] `plan/spec-ospfv3-ext-0-umbrella.md` ext-1 row + "Multi-AF uses reserved plumbing" -- the umbrella contract this child fulfils
  -> Constraint: ext-1 attaches to the reserved, validated Instance-ID field WITHOUT redefining the RFC 5340 header; it maps AF to Instance-ID ranges (§2.4) and adds per-AF topologies + route install; it does not re-open the codec
  -> Decision: ext-1 is base-only (no dependency on ext-2..ext-8); it consumes exactly the Instance-ID plumbing and the IPv6 Loc-RIB FIB-install path the base reserved
- [ ] `plan/spec-ospfv3-0-umbrella.md` "Out of Scope" (RFC 5838 multi-AF row) + the reserved Instance-ID plumbing
  -> Constraint: the base reserved an explicit, validated Instance-ID field so multi-AF could attach later; the base "Out of Scope" table is the authoritative source of this follow-up
...
- The single real latent defect the base left is `NewInstaller` hardcoding `family.IPv4Unicast`: the v6 IPv6-unicast engine today installs IPv6 routes under the IPv4Unicast family label. ext-1 must parameterise the install family per AF (and this also corrects the base's IPv6-unicast install family).
...
- [ ] `internal/plugins/ospf/dispatcher.go` -- `dispatcher.instanceID uint8`; `dispatch()` decodes the header, verifies the checksum, then drops a packet whose `h.InstanceID != instanceID` (RFC 5340 §4.2.2 demux); 0 for the v4 family
...
- [ ] `internal/plugins/ospf/register.go` -- spawns `eng` (v4) via `newEngine` and a SINGLE `eng6` (v6) via `newEngineWithCodec(ospfv3transport.New(...), v6Codec{})`; `cfg.V6 != nil` drives `eng6.setConfig(*cfg.V6)` / `eng6.reconcile` / `eng6.openInterfaces`; `consumer.SetV6Injector(eng6)` wires IPv6 redistribution
  -> Constraint: the v6 spawn is hardwired to one IPv6-unicast instance; multi-AF generalises this to a slice of v6-style engines keyed by AF, each from `cfg.AddressFamilies[i]`; redistribution injection must target the matching AF's engine
...
- All existing OSPFv3 functional/interop tests (`test/interop/scenarios/ospf-v6-*`).

**Behavior to change:** (all RFC-5838-required or correcting a base latent gap)
- `NewInstaller` family is no longer hardcoded to `family.IPv4Unicast`; the v6 family installs into its AF's Loc-RIB family.
...
- The v6 family spawns one engine per configured AF (today: exactly one IPv6-unicast engine).
...
- **Receive:** an OSPFv3 datagram arrives on the ospfv3 transport -> the transport's `PeekInstanceID` selects the engine whose `dispatcher.instanceID` matches -> `dispatch()` runs the §4.2.2 demux (drop on mismatch) -> per-AF handler.
...
### Transformation Path
1. **Config -> per-AF engines (new):** `parseOSPFConfig` yields a v4 config plus a set of OSPFv3 AF sub-configs (each with a validated Instance ID in its AF range); `register.go` spawns `newEngineWithCodec(ospfv3transport..., v6Codec{})` per AF, sets `dispatch.instanceID`, derives the AF from the Instance-ID range, and selects the install family.
...
3. **AF-bit on send (new):** the v6 encoder sets `OptAF` in Hello/DD Options for a multi-AF instance (RFC 5838 §2.4/§2.5); the base IPv6-unicast instance follows the §2.6 back-compat rule.
...
- [ ] No bypassed layers (per-AF datagrams flow transport -> instance demux -> per-AF LSDB/SPF -> per-AF Loc-RIB, the same spine as the single-AF v6 family)
...
| R-7 | Show commands cannot distinguish AF instances, breaking the §2 debugging requirement | `show ipv6 ospf neighbor` mixes two AFs' neighbours | every AF instance is identified by AF + Instance ID in the show output; `ospfv3-multiaf-show.ci` asserts both AFs are listed distinctly |
...
|-------------|---|--------------|------|
| `ospf { address-family ipv4-unicast { instance-id 64; ... } }` in config | -> | `parseOSPFConfig` yields a v4-unicast AF sub-config; `register.go` spawns a v6-codec engine at Instance ID 64 with install family `family.IPv4Unicast` | `TestMultiAFEngineSpawn` (unit) + `test/ospfv3/ospfv3-multiaf-config.ci` |
| An OSPFv3 datagram with Instance ID 64 arrives | -> | the transport routes it to the v4-unicast instance; `dispatch()` accepts (matches), the IPv6-unicast instance (Instance ID 0) drops it | `TestMultiAFInstanceDemux` (unit) + `ospfv3-multiaf.ci` |
...
| An SPF run completes on the v4-unicast instance | -> | the family-parameterised `Installer` inserts the IPv4 route into `family.IPv4Unicast` | `TestInstallerFamilyPerAF` (unit) + `ospfv3-multiaf-v4-route.ci` |
| `show ipv6 ospf` with two AFs configured | -> | the show handler lists each AF instance with its AF + Instance ID | `ospfv3-multiaf-show.ci` |
...
| AC-7 | An IPv6-unicast OSPFv3 instance computes a route | the route installs into `family.IPv6Unicast` (correcting the base's IPv4Unicast hardcode) |
...
| AC-11 | A single IPv6-unicast instance (Instance ID 0) only | the on-wire Hello/DD/LSA bytes and adjacency formation against FRR `ospf6d` are unchanged from the base, except the route now installs into `family.IPv6Unicast` |
...
|---|-----------|--------------------|-----------------------|
| 1 | Configures an IPv6-unicast AND an IPv4-unicast OSPFv3 AF on one interface | config -> two AF sub-configs -> two engine instances (Instance ID 0 and 64) -> two adjacencies form, each on its own LSDB/RIB | `test/ospfv3/ospfv3-multiaf.ci` + `ospfv3-multiaf-frr` (or Ze<->Ze) interop |
| 2 | Advertises an IPv4 prefix over the IPv4-unicast OSPFv3 instance and reaches it | redistribute/connected -> IPv4 prefix in an OSPFv3 LSA on Instance ID 64 -> peer SPF -> route in `family.IPv4Unicast` -> FIB | `ospfv3-multiaf-v4-route.ci` + `ospfv3-multiaf-v4-frr` interop |
| 3 | Runs `show ipv6 ospf` and sees both AF instances distinctly identified | show handler -> per-AF instance listing with AF + Instance ID | `ospfv3-multiaf-show.ci` |
...
| 5 | Removes the IPv4-unicast AF from the config | reconcile -> the v4-over-v3 engine stops, its interfaces close, its routes withdraw; the IPv6-unicast instance is unaffected | `TestAFReconcileAddRemove` + `ospfv3-multiaf-reconcile.ci` |
...
|------|----------|-------------------|--------|
| `ospfv3-multiaf-config` | `test/ospfv3/ospfv3-multiaf-config.ci` | two AFs configured; both engines start; `show` lists both | |
| `ospfv3-multiaf` | `test/ospfv3/ospfv3-multiaf.ci` | two AFs reach Full adjacency, each in its own LSDB | |
| `ospfv3-multiaf-v4-route` | `test/ospfv3/ospfv3-multiaf-v4-route.ci` | an IPv4 prefix over OSPFv3 installs into the IPv4 RIB | |
| `ospfv3-multiaf-show` | `test/ospfv3/ospfv3-multiaf-show.ci` | show output identifies each AF + Instance ID | |
| `ospfv3-multiaf-reconcile` | `test/ospfv3/ospfv3-multiaf-reconcile.ci` | adding/removing an AF starts/stops its engine; the other AF is unaffected | |

### Interop Tests (MANDATORY for protocol features)
| Scenario | Directory | Peer Daemon | What It Proves | Status |
|----------|-----------|-------------|----------------|--------|
| `ospfv3-multiaf-frr` | `test/interop/scenarios/ospfv3-multiaf-frr/` | FRR `ospf6d` (IPv6-unicast + IPv4-unicast AFs, if supported) else a second Ze instance | Ze sets the AF-bit, forms per-AF adjacencies, exchanges per-AF LSDBs, and installs per-AF routes; the AF-bit demux interoperates | |
| `ospfv3-multiaf-v4-frr` | `test/interop/scenarios/ospfv3-multiaf-v4-frr/` | FRR `ospf6d` IPv4-unicast-over-OSPFv3, else Ze<->Ze | an IPv4 route advertised over OSPFv3 (Instance ID 64) is learned and installed into the IPv4 FIB on both ends (§2.7) | |
...
> consistent with the rest of the OSPFv3 interop set. Per A-9, if FRR `ospf6d`
> lacks IPv4-over-OSPFv3, the interop falls back to Ze<->Ze plus a captured FRR
> multi-AF Hello in the unit corpus; the fallback is recorded as a Deviation, not
> a silent skip.
...
| Functional test for new RPC/API | [ ] yes | `test/ospfv3/ospfv3-multiaf-*.ci` |
...
| Doctor check for runtime dependencies | [ ] no | no new socket/port/binary/cert; each AF instance reuses the existing ospfv3 raw IPv6 socket (the transport doctor check already covers it) |
...
> `ze_ospf_routes_installed` already exists (owned by `spf/install.go`); ext-1
> adds the `af` label so per-AF counts are distinguishable. The two new series use
> the `ze_ospf_af_*` prefix. The OSPFv3 umbrella metrics table gains these rows
> when this spec lands.

### Documentation Update Checklist (BLOCKING)
| # | Question | Applies? | File to update |
|---|----------|----------|---------------|
| 1 | New user-facing feature? | [ ] yes | `docs/features.md` -- OSPFv3 multiple address families (RFC 5838) |
...
| 6 | Has a user guide page? | [ ] yes | `docs/guide/ospf.md` (or the OSPFv3 page) -- a multi-AF section |
| 7 | Wire format changed? | [ ] yes | the OSPFv3 wire doc -- the AF-bit in Options + the Instance-ID-range AF demux |
...
| 11 | Affects daemon comparison? | [ ] yes | `docs/comparison.md` -- OSPFv3 multi-AF parity with FRR |
| 12 | Internal architecture changed? | [ ] yes | the OSPF subsystem doc -- per-AF engine instances + install family |
...
| 17 | Existing docs show examples for this area? | [ ] check | verify any OSPFv3 config/CLI examples against the generalised `address-family` shape |
...
- `test/ospfv3/ospfv3-multiaf-config.ci`, `ospfv3-multiaf.ci`, `ospfv3-multiaf-v4-route.ci`, `ospfv3-multiaf-show.ci`, `ospfv3-multiaf-reconcile.ci`
- `test/interop/scenarios/ospfv3-multiaf-frr/` -- `ze.conf`, `frr.conf` (or a second `ze.conf`), `check.py`
- `test/interop/scenarios/ospfv3-multiaf-v4-frr/` -- `ze.conf`, `frr.conf` (or a second `ze.conf`), `check.py`
...
1. **Phase: Wiring (MANDATORY FIRST)** -- the AF type + per-AF engine spawn + failing wiring tests
   - Tests: `TestAFFromInstanceID`, `TestMultiAFEngineSpawn`, `test/ospfv3/ospfv3-multiaf-config.ci`
...
5. **Phase: IPv4-over-OSPFv3** -- AF-aware prefix + next-hop
   - Tests: `TestIPv4OverV3PrefixRoundTrip`, `TestV6PrefixToNetipAFWidth`, `TestIPv4OverV3NextHop`, `ospfv3-multiaf-v4-route.ci`
...
6. **Phase: Show + reconcile + metrics** -- user surface + lifecycle
   - Tests: `TestAFReconcileAddRemove`, `TestRedistTargetsAFEngine`, `ospfv3-multiaf-show.ci`, `ospfv3-multiaf-reconcile.ci`
...
9. **Interop** -> `ospfv3-multiaf-frr` + `ospfv3-multiaf-v4-frr` QEMU scenarios; `ospf-v6-frr` regression
...
| Doctor checks | none added (reuses the existing ospfv3 transport doctor check) -- confirm |
...
| Interop scenarios present | `ls test/interop/scenarios/ospfv3-multiaf-frr test/interop/scenarios/ospfv3-multiaf-v4-frr` |
| Functional tests present | `ls test/ospfv3/ospfv3-multiaf-*.ci` |
...
## Core Insight
Multi-AF is a *fan-out* problem, not a codec problem: the OSPFv3 base already
...
ID, set the AF-bit, and install into the AF's Loc-RIB family." The only base
...
| Parameterise the SPF installer family | a per-AF RIB wrapper or a family switch at each insert | one constructor parameter threads the family through the single insert/remove chokepoint; minimal blast radius and it corrects the base hardcode |
...
- No new LSA types; RFC 5838 reuses the RFC 5340 LSA set, so any future v3 extension LSA (ext-6 SR) is orthogonal to multi-AF.
...
|--------------------------|---------------|-------------------|
| Instance-ID-range AF demux | unit + functional | `TestAFFromInstanceID`, `TestMultiAFInstanceDemux`, `ospfv3-multiaf.ci` |
| AF-bit handling (set + gate) | unit + interop | `TestAFBitInHelloAndDD`, `TestAFBitGatesFullNonDefault`, `ospfv3-multiaf-frr` |
| Per-AF LSDB + route install | unit + functional | `TestPerAFLSDBIsolation`, `TestInstallerFamilyPerAF`, `ospfv3-multiaf-v4-route.ci` |
| IPv4-over-OSPFv3 | unit + interop | `TestIPv4OverV3PrefixRoundTrip`, `ospfv3-multiaf-v4-frr` |
...
- [ ] Interop tests for protocol features (`ospfv3-multiaf-frr`, `ospfv3-multiaf-v4-frr`)
...
- [ ] Write learned summary to `plan/learned/NNN-ospfv3-ext-1-multi-af.md`
- [ ] **Commit A:** code + tests + docs + spec + learned summary + counter bump
- [ ] **Commit B:** `git rm plan/spec-ospfv3-ext-1-multi-af.md`
