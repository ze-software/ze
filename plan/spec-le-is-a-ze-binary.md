...
| AC-3 | `le` MAY link the product composition root for introspection. It MUST register no product command as its own. `internal/le/register.go` imports only `internal/le/` packages and the shared engine registry | `TestLeRegistersOneRootAndNoToolRoots` |
...
The first closure pass found one CONTRACT violation and recorded it as B-1:
`TestNoPythonLeRemains` was RED on two tracked first-party Python files,
`plan/rfcgate-6-partial-walks/classify-rfc3748.py` and `classify-rfc5176.py`,
committed by `49c145d3b` three days after the swap. It was fixed on 2026-09-05
in `456bf8fc1`, by the third route neither earlier reading had: build the
capability the scripts carried, then delete them. `./le rfc extraction-classify
decisions <path>` (`internal/le/rfc/actions.go`) applies one authored decision
per site and per section from one decisions file, and the owner authorised the
deletion once that verb existed. `firstPartySource`
(`internal/le/contract_test.go`) was not narrowed, so AC-12 is carried by the
same test it was always carried by.

The second closure pass found one record defect and one closure debt. AC-3's
third clause said `internal/le/register.go` "imports only `internal/le/`
packages", which the file contradicts and AC-3b requires it to contradict: it
imports `internal/component/command/registry`, the shared engine. The clause is
corrected above. The debt is in Documentation Updates below.
...
- **Closure debt, found and paid in the closure commit.** 29 tracked Go files
  opened with `// Design: plan/spec-le-is-a-ze-binary.md`, and two more cited
  that path in prose. Commit B removes the spec, so all 31 would have become
  broken references in `designReferenceFindings`
  (`internal/le/doc/wiring/designrefs.go`), read by `./le doc check links`. Each
  now names the durable page its siblings already name:
  `docs/architecture/testing/qemu-integration.md` for the seven
  `internal/le/qemu/` files, `docs/architecture/testing/interop.md` for the two
  `internal/le/interoplab/l2tp/` files, and `docs/architecture/core-design.md`
  for the other 22.
- `docs/architecture/core-design.md`, "Command engine and composition roots",
  gained the paragraph the two prose citations were citing the spec for: the
  never-linked rule is DIRECTIONAL, `le` MAY link
  `internal/component/plugin/all` because a registry-derived answer cannot be
  computed without it, and the canonical `le <tool>` local-data path is what
  keeps a product root out of `le`'s surface rather than a second ownership
  table.
- `ai/skills/ze-close.md` step 6d now says `git grep` over the whole tracked
  tree rather than `grep plan/*.md`, names `designrefs.go` as the resolver, and
  says a `// Design:` header takes the repoint and never the bare stem. Five
  rows of `plan/journal/closure-deletes-a-cited-document.md` had argued for that
  edit. The edit lands in `ai/skills/`, which is the canonical source;
  `.claude/skills/ze-close/SKILL.md` is generated by `./le ai skills-sync` and
  is gitignored, so a hand edit there is destroyed at the next sync with nothing
  saying so.

### Deviations from Plan

- The migration-time parity packages are gone with their Python halves, so the
  comparisons A-3 was confirmed by cannot be re-run. They were performed and
  recorded while both halves existed, and that record is the confirmation. What
  pins the ported behavior from here on is the 24 `test/ui/le-*.ci` answer
  tests, which is a different job and is not a second confirmation of A-3.
...
|------|---------------|----------------------|----------------|--------|
| assumption | AC-12 was treated as satisfied once the swap landed, and nothing watched the tree afterwards | `TestNoPythonLeRemains` walks every tracked file, so any later commit can break it. `49c145d3b` (2026-08-31) added two first-party `.py` files under `plan/rfcgate-6-partial-walks/`, three days after the swap, and the test was red for five days | The first closure pass ran the test | Fixed in `456bf8fc1` (2026-09-05). The transferable half is the SHAPE, and it is the row in `plan/journal/gate-red-where-nothing-blocks-on-it.md`: a tree-wide contract test is a standing obligation on every future commit, so the session that breaks it is never the session that owns it. Here the throwaway script was the tell that a `le` verb was missing, and building the verb was what let the deletion happen |
| assumption | The closure scan for citers was taken to be the one `/ze-close` step 6d names, a grep over `plan/*.md` | That grep reads `speccitation.Scan`'s population. The gate that goes red is `./le doc check links`, which resolves the `// Design:` header of every tracked Go file. The narrow grep found ONE citer; the tree held 31 | The second closure pass ran `git grep` over the whole tree before preparing commit B | Fixed before commit B: all 31 repointed, and the skill's step 6d corrected so the next closure greps the population that goes red |
...
| All first-party development tooling is compiled Go | Done | `internal/le/` | `TestNoPythonLeRemains` PASS 0.75s on 2026-09-05. `git ls-files` filtered to `.py`, `.sh`, `.bash`, `.mk` and `Makefile`, excluding `vendor/` and `test/exabgp-compat/`, returns nothing |
...
| AC-9 | Done | `TestEveryMakeProducerHasAReachableNativeAction` PASS 0.12s; `TestNoPythonLeRemains` PASS 0.75s | The census half is met, `internal/le/parity/` is gone, and the residue half went green with `456bf8fc1` |
| AC-10 | Done | `TestTheCheckoutsOwnHeadIsClean` PASS 99.43s | |
| AC-11 | Done, with foreign reds | 24 `test/ui/le-*.ci` answer tests exist; 18 PASS and 6 FAIL on 2026-09-05 | Each of the six fails on tree state another session holds, never on `le` dispatch: `le-placement-answers` and `le-checks-answers` on `internal/component/pki/` and `internal/component/hub/yang/`, `le-functional-answers` on an integration action a peer added, `le-qemu-run-answers` on its own fixture go.mod, `le-rules-answers` on a ratchet that did not run, `le-spec-status-answers` on another spec's bucket. `le-binary-dispatches` PASS |
| AC-12 | Done | `TestNoPythonLeRemains` PASS 0.75s | Fixed in `456bf8fc1`. `firstPartySource` still excludes only `vendor/`, `third_party/` and `test/exabgp-compat/`, so the test was not narrowed to reach green |
...
| `TestNoPythonLeRemains` | Done | `internal/le/contract_test.go` | PASS 0.75s on 2026-09-05. It was RED at the first closure pass, correctly, because the tree violated AC-12 |
...
| All first-party development tooling is compiled Go | functional | `TestNoPythonLeRemains` PASS 0.75s on 2026-09-05, walking every tracked file. `git ls-files` filtered to `.py`, `.pyc`, `.sh`, `.bash`, `.mk` and `Makefile`, minus `vendor/` and `test/exabgp-compat/`, returns nothing. The two files that made it red were deleted in `456bf8fc1` once `./le rfc extraction-classify` carried their capability, and `firstPartySource` was not narrowed to reach that green |
...
|-------------------|-----|---------------------------|
| Nothing. The one item that was outstanding at the first closure pass was a contract VIOLATION rather than unfinished scope, and it was fixed in `456bf8fc1` before this pass | -- | -- |

## Review Gate

Two closure passes, both on 2026-09-05, each run by a context that authored none
of the code under review. Every lens was run inline and no reader was spawned.
Round 1 opened B-1 and stopped. Round 2 re-measured every evidence row against
source rather than relaying round 1's, judged the fix that closed B-1, and found
the citer debt that commit B would otherwise have created.

| Field | Value |
|-------|-------|
| Artifact | `tmp/review/le-is-a-ze-binary-zeclose-lebin2.md`, over the 39 files commit A carries |
| `./le spec session review check` | `review_gate: OK (31 code files, clean, hashes match ...)`, exit 0 |
| Rounds | 2 |
| Reviewer lenses used | round 1: contract and residue; wiring and functional-test coverage. round 2: evidence re-measurement against source; citer and Design-header population; record accuracy |
...
|---|----------|---------|-------------|
| B-1 | BLOCKER | AC-12 is not met and `TestNoPythonLeRemains` (`internal/le/contract_test.go`) is RED. Two tracked first-party Python files remain: `plan/rfcgate-6-partial-walks/classify-rfc3748.py` and `classify-rfc5176.py`. They were added by `49c145d3b` on 2026-08-31, three days AFTER the swap in `eae282592`, by a session preserving four RFC extraction walks that could not sign | FIXED in `456bf8fc1`, 2026-09-05, and by neither of the two routes round 1 could see. The capability the scripts carried is now `./le rfc extraction-classify decisions <path>` (`internal/le/rfc/actions.go`), and the owner authorised deleting them once it existed. Verified at round 2 rather than taken: the two paths are gone, `firstPartySource` still excludes only `vendor/`, `third_party/` and `test/exabgp-compat/`, and the test PASSES |
| B-2 | BLOCKER | Commit B removes the spec, and 29 tracked Go files opened `// Design: plan/spec-le-is-a-ze-binary.md` while two more cited the path in prose. `designReferenceFindings` (`internal/le/doc/wiring/designrefs.go`) resolves that header for every tracked Go file, so `./le doc check links` would have gained 31 broken references the moment the spec left the tree. `/ze-close` step 6d's `grep plan/*.md` found ONE of the 31 | FIXED in commit A, before commit B. Each header names the durable page its siblings already name, the two prose citations name `docs/architecture/core-design.md`, and the directional rule they were citing the spec for was written onto that page in the same commit. `./le doc check links` reports the same 30 foreign findings before and after, and names no file this spec touches |
| N-1 | NOTE | Six of the 24 `test/ui/le-*.ci` answer tests fail at round 2, one more than at round 1: `le-checks-answers` (cli-grammar reports two `--flag` mentions in `internal/component/hub/yang/ze-hub-conf.yang`), `le-placement-answers` (`./le repository check` red on six exported symbols in `internal/component/pki/`), `le-functional-answers` (`the integration area listed 14 actions, want 13`), `le-qemu-run-answers` (its own fixture writes a go.mod with no `go` directive, which `goDirective` now refuses), `le-rules-answers` (`a ratchet did not run over this checkout`) and `le-spec-status-answers` (another spec's bucket placement) | not fixed. Each fails on tree state another session holds and none on `le` dispatch. `le-binary-dispatches` and the other 18 pass. The qemu fixture is a journal row in `plan/journal/fixture-encodes-an-impossible-state.md` |
...
| N-3 | NOTE | AC-3's third clause said `internal/le/register.go` "imports only `internal/le/` packages". It also imports `internal/component/command/registry`, which AC-3b requires it to import | FIXED as a record defect: the clause now says "and the shared engine registry" |

### What was run, not narrated

Round 2 re-ran every measurement rather than reading round 1's. Where a number
differs, round 2's is the one recorded above.

| Check | Result |
|-------|--------|
| `go test` over `./internal/le/`, `./internal/le/leroot/`, `./internal/le/leaction/`, `./internal/le/tracked/` and `./cmd/ze/`, tags `ze_core ze_le` plus the `feature-gates.txt` set, `CGO_ENABLED=0`, `-count=1` | 12 named tests PASS, `TestNoPythonLeRemains` among them |
| `go list -deps` over `./cmd/ze`, with and without `ze_le` | 1128 packages and 0 `internal/le`; 1372 packages and 109 `internal/le`. Identical to round 1's figures |
| `git ls-files` filtered to `.py`, `.sh`, `.bash`, `.mk` and `Makefile`, minus `vendor/` and `test/exabgp-compat/` | no hits |
| `./le functional ui` | `le-binary-dispatches` PASS. 18 of the 24 `le-*` answer tests pass; the six failures are N-1 |
| `./le repository check` | 7 findings, all in `internal/component/pki/` and `internal/le/qemu/alltests_report.go`, both another session's uncommitted work. None in a file this spec owns |
| `./le commit audit` | 9 WEAKENED findings, all in `internal/core/eap/` and `internal/le/verify/`, both another session's uncommitted work. None in a file this spec owns |
| `./le doc check links` | 30 broken references before this closure's edits and 30 after. None names this spec, and none names a file it touches |
...
| AC-9 | Native action completeness comes from current producer and action tables; no Python parity census remains | `TestEveryMakeProducerHasAReachableNativeAction` PASS 0.12s, `ls internal/le/parity` answers `No such file or directory`, and `TestNoPythonLeRemains` PASS 0.75s |
| AC-10 | The committed tree builds and loads every registered tool | `TestTheCheckoutsOwnHeadIsClean` PASS 99.43s |
| AC-11 | Each migrated producer's observable contract is defended by a native test | 24 `test/ui/le-*.ci` answer tests exist; `./le functional ui` passed 18 and failed 6. The six are named in the Review Gate, each failing on tree state another session holds rather than on `le` dispatch |
| AC-12 | After the swap, no first-party Python, shell or Make source remains | `TestNoPythonLeRemains` PASS 0.75s, walking every tracked file. `git ls-files` filtered to `.py`, `.pyc`, `.sh`, `.bash`, `.mk` and `Makefile`, minus `vendor/` and `test/exabgp-compat/`, returns nothing. Round 2 read `firstPartySource` (`internal/le/contract_test.go`) to confirm the population was not narrowed: it still excludes only `vendor/`, `third_party/` and `test/exabgp-compat/` |
...
| A-3 | confirmed, historically, and the confirmation is the row's own | The evidence for A-3 is the four side-by-side comparisons recorded in the assumption row itself, each performed on 2026-08-26 while both halves existed: `lint` (same 1250 lines, same exit), `vendor` (same exit, stdout, stderr and a byte-identical written TREE over 7 fixture trees), `inventory` (byte-identical page and `--json` over the checkout and two fixture trees, under the full feature tag set), `codegen` (the resulting TREE compared after each write over 11 fixture trees). Round 2 states plainly what cannot be redone and does not dress it up: the comparison is unrepeatable, because the Python half is gone by design and AC-12 requires it to be. The 24 `test/ui/le-*.ci` answer tests are NOT a second confirmation of A-3 -- they compare nothing to a Python original -- they are what pins the ported behavior from here on, which is a different job |
| A-4 | confirmed, historically, and the surviving artifact agrees | The evidence is the measurement in the assumption row, taken on 2026-08-26 on `post_weekly_test.py`: 11 cases, ten transferring as intent through a field the caller fills, one not transferring at all, becoming 53 Go tests. It is not re-runnable, because the Python is gone. What can still be checked is the artifact it predicted, and it is there: `internal/le/weekly/` holds 54 top-level `Test` functions across `poster_test.go`, `weekly_test.go`, `send_test.go` and `answer_test.go`, one more than the row recorded. `TestNoDevelopmentToolTestShellsOutToGoRun` PASS is weaker support and is recorded as such: it proves no test invokes a tool by `go run`, not that content transferred as intent |
...
| `docs/architecture/core-design.md` shared registry and composition roots | The section "Command engine and composition roots" states the shared registry, names `internal/le/register.go` as the composition root, states that a normal `ze` build imports no `internal/le` package, and names `cmd/ze/ze_le_register.go` as the `ze_le` companion. Round 2 added the paragraph that states the rule's DIRECTION and its enforcement, which lived only in this spec until now: `TestNormalZeLinksNoInternalLe` measures the `ze` half, `le` MAY link `internal/component/plugin/all` because a registry-derived answer cannot be computed without it, and the canonical `le <tool>` local-data path keeps a product root out of `le`'s surface. Verified against the producers before it was written: `Owns` and `CommandPath` (`internal/le/leroot/leroot.go`) and the package comment of `internal/le/leroot/dispatch.go`, which states the same mechanism -- filtering the shared registry by the `le ` prefix, with no second ownership table | Yes |
| `// Design:` headers naming this spec | 29 tracked Go files opened `// Design: plan/spec-le-is-a-ze-binary.md`, and two more cited the path in prose. All 31 repointed in commit A, before commit B removes the spec. `git grep "plan/spec-le-is-a-ze-binary"` over the tracked tree now returns only this spec's own text | Yes |
