...
3. `ai/rules/feature-gate-registration.md`, `ai/rules/functional-test-gate.md`, `ai/rules/interop-and-goal-validation.md`
...
| `test/parse/no-bgp-fib-only.ci`, `no-bgp-empty.ci` | `ze config validate` accepts `fib{}` / empty config | runs against the **full** functional binary (`TestBuildTags` folds every default-on gate incl. `ze_bgp` — `internal/test/runner/runner.go:50,227`); it is `config validate` only, never a running daemon |
...
with `ze_bgp` off), fixing it is in scope (`ai/rules/no-parking.md`); it is not
...
- [ ] `ai/rules/feature-gate-registration.md` - the invariant this spec verifies at runtime
...
- [ ] `ai/rules/qemu-testing.md` - `option=needs-linux`, the stripped QEMU binary
...
**Key insights:**
- `ze-stripped` (the shipped minimal binary) is built `-tags 'ze_core ze_ssh'` (`mk/test-functional.mk:131`) — no `ze_bgp` — and is already on PATH in the functional runner, with a working precedent (`test/ui/ze-stripped-surface.ci`). This is the ready-made vehicle for a bgp-absent functional `.ci`.
- The QEMU stripped binary is built `-tags 'ze_core $(ZE_TAGS)'` (`mk/test-integration.mk:233`) — `ze_core` only, **no ze_ssh** — so a QEMU/needs-linux test against it must assert via config-file + kernel readback, not SSH CLI.
- `cmd/ze/hub` unit tests run a second time under bare `ze_core` (`mk/test-unit.mk:36,51`), so a `//go:build !ze_bgp` Go test there is a deterministic, no-dataplane vehicle for the presence guard.

## Current Behavior (MANDATORY)

**Source files read:** (read BEFORE writing this spec)
- [ ] `internal/plugins/fib/kernel/fibkernel.go` — fib-kernel subscribes to **sysrib** best-change events (`sysribevents`, `:147`), NOT bgp directly, and programs the kernel FIB using the core-leaf `routeaction` verb vocabulary (`:25`). Config root `fib/kernel` (`register.go:62`).
...
- [ ] `internal/plugins/mrt/component.go` / `register.go` / `dump.go` — MRT's data sources are the reactor message observer (`OnBGPMessage`, `component.go:100`) and the RIB-dump seam (`registry.GetRIBDumpCallback()`, `register.go:129`), both nil with bgp off. `dumpRIB()` nil-guards the callback (`dump.go:102-105`, logs "RIB dump skipped, no RIB callback"). Config root `mrt` (`register.go:97`); command `request mrt dump-rib` (`register.go:201`) returns cleanly ("mrt not configured" when idle, "rib dump triggered" otherwise) — no panic.
...
- [ ] `internal/plugins/flowexport/enrichbgp.go` / `register.go` — the BGP enrich builder subscribes to `ribevents.BestChange` on the EventBus (`enrichbgp.go:107`). Nothing publishes those events with bgp off, so the enrich radix tree stays empty; the rebuild worker ticks and no-ops. Config root `flow-export` (`register.go:63`). Raw flow export (netflow9/ipfix/sflow) is independent of BGP.
...
3. fib-kernel consumes the batch (`fibkernel.go:147`), translates via `routeaction`, programs the kernel FIB (netlink, linux only).
...
| A-2 | `ze-stripped` (`ze_core ze_ssh`) is on PATH in the functional runner | `test/ui/ze-stripped-surface.ci` invokes it; `mk/test-functional.mk:131` builds it | no native vehicle; must fall back to QEMU-only | run a trivial `ze-stripped config validate` `.ci` | unvalidated |
| A-3 | FIB installs sysrib-fed static routes with bgp off | `fibkernel.go:147` consumes `sysribevents`, bgp-independent | AC-4 unachievable; deeper coupling exists | QEMU `.ci`: static route appears in kernel FIB | unvalidated |
| A-4 | Daemon boots healthy on `ze-stripped` with fib+mrt+flow-export configured | nil-guards read (`dump.go:102`, `enrichbgp.go:106`) | AC-3 red = real boot defect to fix | AC-3 `.ci` | unvalidated |
| A-5 | The QEMU stripped binary (`ze_core`, no ssh) can be driven config-file-only | `mk/test-integration.mk:233`; needs-linux fib tests already assert via kernel readback | AC-4 must use the native `ze-stripped` + a linux runner instead | inspect an existing kernel-FIB `.ci` assertion mechanism during implementation | unvalidated |
...
| R-2 | `request mrt dump-rib` or another reachable command panics on the nil seam with bgp off | AC-5 `.ci` crashes the daemon | `dump.go:102` already nil-guards; if another reachable path does not, fix it at the owner (`ai/rules/no-parking.md`), do not skip the assertion. |
...
| AC-5 | On the AC-3 bgp-off daemon: trigger `request mrt dump-rib` (with `mrt{routes ...}` configured) and let flow-export run | No panic; MRT RIB dump is gracefully skipped (`dump.go:102` path), command returns cleanly; flow-export runs with an empty BGP-enrichment tree and logs no enrichment-subscription error. Documents+asserts graceful BGP-source absence. |
...
- `ai/rules/feature-gate-registration.md` - add a line to the "add a feature gate" procedure: an always-on plugin surviving a large gate needs a *functional* present-half proof, not just registration/symbol-drop (this spec is the worked example).
...
| 10 | Test infrastructure changed? | Yes | `docs/functional-tests.md` and/or `ai/rules/feature-gate-registration.md` — note the bgp-off functional pattern (drive `ze-stripped`) so future gate work reuses it |
...
6. **Mutation-verify each behavior-guarding test** (`ai/rules/functional-test-gate.md`): break the always-on registration / FIB install, confirm the matching test flips red, revert.
...
| Graceful degradation asserted, not assumed | AC-5 observes the nil-guard path (`dump.go:102`) and empty enrichment, not merely "no crash" |
...
| Failure | Route To |
|---------|----------|
| Daemon panics on boot with bgp off | Real defect — fix at the owning plugin (`ai/rules/no-parking.md`), add a Deviations row |
...
## Design Insights
<!-- LIVE -->
- The `wiring-at-commit` warning is advisory and fires on any `internal/plugins/**/*.go` commit lacking a `.ci`. For a pure import-requalification it is a false positive by the letter of `functional-test-gate.md` — but here it correctly surfaced a real gap: the *new capability* the requalification unlocks (running with `ze_bgp` off) is unproven functionally. The lesson: read the warning as "what new reachability did this change enable?", not "does this exact hunk need a test?".
...
| MRT/flow-export proof bounded to presence + boot + graceful degradation | A "MRT dumps with bgp off" data test | MRT/flow-export enrichment are *inert* without a BGP source (`component.go:100,129`, `enrichbgp.go:107`); a data-production test would be vacuous or impossible |
