...
| Q-7 | **The surface.** `ze config fmt` and its `--check` mode exist. Does the gate run them, over which population, and does it live in `make ze-doc-verify`, in `make ze-wiring-docs-check`, or in a new target? | `ai/rules/repo-maintenance.md` owns whether a docs gate is worth building, and its Current Discovery Surfaces table lists the changed-file-aware gates that already exist. `ai/rules/cli.md` owns the grammar, and `config fmt` already satisfies action before identifier |
