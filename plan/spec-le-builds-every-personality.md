...
To build the functional test runner a developer wrote a `./le job run` of a
hand-typed `go build`, naming the harness build tag plus `ze_bgp` and a separate
harness artifact under `bin/`, and got no help from anything. Since 2026-09-25
the runner is `le` itself (spec-le-subject-first-command-tree), so that tag and
that artifact are gone, and the example stands for any personality a developer
