...
is started. Both rows are recorded in the retired deferral shard "cli-order-pipe".
...
### From `cli-order-pipe.md`, 2026-08-19

Deferred by spec-cli-order-pipe.

Row ordering: an operator that sorts the ROWS by a field's value, as distinct from `display` and `fill`, which order and select the COLUMNS

### From `cli-order-pipe.md`, 2026-08-19

Deferred by spec-cli-order-pipe.

Exclusion syntax, so an operator can say "every column except these two" rather than naming the other seventeen
