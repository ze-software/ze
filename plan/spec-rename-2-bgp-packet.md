...
Check `make ze-spec-status` before starting.
...
- [ ] `scripts/dev/protocol_skeleton_report.py` - LEGACY_EXCEPTIONS holds ("bgp", "message") (:57)
- [ ] `scripts/dev/hook-parity-check.py` - fixture rows name `internal/component/bgp/message/pack.go` (:151) and `.../p.go` (:232)
...
| report selftest (fail-first: flip fixture expectations before the table edit) | `scripts/dev/protocol_skeleton_report.py` | ("bgp","message") no longer an exception | |
...
1. **Phase: gate check + re-measure** — confirm rib-arch closed (`make ze-spec-status`); rerun the importer/anchor/literal greps; update scope numbers here if drifted.
...
| hook-parity fixtures updated | run `scripts/dev/hook-parity-check.py` |
