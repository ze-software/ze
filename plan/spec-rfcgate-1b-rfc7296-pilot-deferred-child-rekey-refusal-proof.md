# Spec: rfcgate-1b-rfc7296-pilot-deferred-child-rekey-refusal-proof
...
real handler and reads the real datagram off a real UDP socket. Neither end-to-end carrier
exists.
...
the SA never establishes at all. And `respondChildRekey` matches against
`ESPGroup.Proposals[0]`, which `selectResponderESP` has already narrowed to the negotiated
