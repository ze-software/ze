...
`doctor.go,157`), so the shared-file coordination note still holds.
...
argued from the code comment at `lcp.go`. It is not a mistake:

| Correction | Evidence (verified 2026-07-16) |
|-----------|-------------------------------|
| The default is DELIBERATE and STAYS | -> Decision (user, 2026-07-16). `plan/deferrals.md` (2026-07-10) records the real intent: make BGP netns-aware "so LCP TAPs in a non-root netns are reachable by BGP **without forcing the operator to a root-reachable netns**" |
| `"dataplane"` is IPng Networks' production convention, copied on purpose | `54bffb83b` ("startup.conf generator **following IPng production template**"); `docs/research/vpp-deployment-reference.md` ("isolates the forwarding plane from the management plane") |
...
whole time. See `ai/rules/evidence.md` "Evidence corollary".
...
which implies a niche opt-in. The code says the opposite. `ze-vpp-conf.yang` defaults
`netns` to `"dataplane"`; `lcpNetnsIsRootReachable` (`doctor.go`) accepts ONLY `""`,
`"host"`, `"root"`, so `"dataplane"` is not root-reachable; `lcpPairNetns` (`lcp.go`)
passes it through; BGP has zero netns awareness (`network.go`). **The DEFAULT config
puts LCP TAPs where BGP cannot bind.**

~~That contradicts the intent stated in the comment directly above the code (`lcp.go`:
...
| Correction | Evidence (verified 2026-07-16) |
|-----------|-------------------------------|
| The recorded design intent is the OPPOSITE of the comment-derived reading: BGP should follow the TAP, the TAP should not be dragged to BGP | `plan/deferrals.md` (2026-07-10, this work's origin row): teach the BGP listener to bind in a named netns "so LCP TAPs in a non-root netns are reachable by BGP **without forcing the operator to a root-reachable netns**" |
| `"dataplane"` is IPng's production convention, adopted deliberately | `54bffb83b` ("following IPng production template"); `docs/research/vpp-deployment-reference.md`: LCP TAPs live in `dataplane`, which "isolates the forwarding plane from the management plane" |
...
exists TODAY (`checkVPPLCPNetns`, `doctor.go`, confirmed absent by `ls`, Q11), and a
...
- [ ] `internal/component/bgp/reactor/reactor.go` - lines 1001-1004 (global listener) and
      ...
      set by `NewListener` (`listener.go`)
  → Constraint: `Listener.SetListenerFactory` is an INTERNAL seam only. No production caller
    ...
    `Reactor.SetListenerFactory` (`reactor.go`), which sets `r.listenerFactory`.
...
- [ ] `internal/component/doctor/register.go` - lines 13-21: `registry.RegisterRoot("doctor",
      ...
      ...)`; `Run` (`doctor/doctor.go`) parses args and calls `runChecks(configPath)`
      (`doctor.go`), which builds only a storage handle and a platform value
...
- [ ] `internal/component/vpp/vpp.go` - lines 67-80: `GetActiveConnector` returns
      ...
      Manager calls from its `Run` path (`vpp.go` constructs `NewConnector(settings.APISocket)`)
  → Constraint: THE PRODUCER READ THAT KILLS THE SPEC'S OWN PROPOSAL. `connectorRef` is
    ...
    and is corrected below. `GetActiveLCPSettings` (`vpp.go`) is nil/false in doctor for
...
- [ ] `go.fd.io/govpp@v0.13.0/api/api.go` - line 109: the `api.Channel` interface includes
      `CheckCompatiblity(msgs ...Message) error`; implemented at `core/channel.go`,
  ...
  → Decision: this IS the GoVPP plugin-presence probe, and it resolves A-5 in GoVPP's
    ...
    naming it. `Connector.NewChannel()` (`internal/component/vpp/conn.go`) returns
...
- [ ] `ai/rules/repo-maintenance.md` - cited by `doctor.go` as the design rule for
...
- [ ] The followup-vpp-iface record both rows came from (retired with the learned corpus)
...
- [ ] `ai/rules/plugins.md` - constrains where the check and any netns code live
...
- ~~The BGP listener already has a `ListenerFactory` injection seam; netns support likely
  ...
  overwrites it (`reactor.go`, `:1402`) and `newListenerFactory` (`reactor.go`)
...
- ~~The only runtime VPP probe precedent uses `vppctl` exec from the central doctor package,
  ...
  REFINED. The tension dissolves: `api.Channel.CheckCompatiblity` (govpp `api/api.go`)
...
- The real constraint on the probe is a PROCESS boundary, not a mechanism choice: `ze doctor`
  is an offline local command (`doctor/register.go`), so `vppcomp.GetActiveConnector()`
  (`vpp.go`) is nil there. The probe must open its own short-lived connector from the
  config's `vpp.api-socket` (`ze-vpp-conf.yang`). A-6 is BROKEN as written.
...
- [ ] `internal/component/bgp/reactor/reactor.go` - line 553: `Reactor.SetListenerFactory(f)`
      ...
      `internal/chaos/inprocess/runner.go`. Every one of them is defeated by the
...
- [ ] `go.fd.io/govpp@v0.13.0/api/api.go` - `api.Channel` declares
      `CheckCompatiblity(msgs ...Message) error`. `core/channel.go` implements it:
...
- [ ] `go.fd.io/govpp@v0.13.0/binapi/lcp/lcp.ba.go` - `LcpDefaultNsGet struct{}`, an empty
...
- [ ] The followup-vpp-iface record, Gotchas: "LCP enabled + no
...
against source and CONFIRMED real. Exact today: `registerDoctorChecks` (`doctor.go`,
Order 740 `vpp-wireguard-plugin` / 741 `vpp-lcp-netns`, so the new check takes Order 742);
`checkVPPWireguardPlugin` (`doctor.go`) / `wireguardPluginEnabled` (`doctor.go`);
`codes.go` rows (`doctor-vpp-wireguard` line 301, `doctor-vpp-lcp-netns` line 307);
`api.Channel.CheckCompatiblity` (govpp `api/api.go`, impl `core/channel.go`);
`LcpDefaultNsGet` (`binapi/lcp/lcp.ba.go`); `NewConnector`/`Connect`/`NewChannel`/`Close`
(`conn.go/44/93/154`); `GetActiveConnector` (`vpp.go`); doctor offline/local
(`register.go`); `api-socket` (`ze-vpp-conf.yang`); `lcp` container (`:171`, `enabled`
`:177`, `netns` `:199`); `linux_cp_plugin.so` (`startupconf.go`); `LcpItfPairAddDel`
(`lcp.go`). LINE DRIFT to respect when implementing (behavior unchanged, numbers grew as
comments were added): `checkVPPLCPNetns` now spans `doctor.go` (SeverityWarning at line
138, not 128); `lcpNetnsIsRootReachable` now `doctor.go` (spec cites 136-143);
`lcpPairNetns` now `lcp.go` (spec cites 105-115). Also `test/ui/doctor-vpp-socket.ci` now
...
- The `doctor-vpp-lcp-netns` warning while BGP cannot bind in a non-root netns
  (`doctor.go`); it is the only thing making the constraint visible today.
- `lcpPairNetns` mapping host/root/empty to "" so VPP places the TAP in its host namespace
  (`lcp.go`), and passing any other name through deliberately.
- The `doctor-vpp-wireguard` check's current config-toggle behavior (`doctor.go`).
...
| 2 | `newListenerFactory` carries the netns into BOTH branches, and the MD5/GTSM branch stops discarding configured state | `internal/component/bgp/reactor/reactor.go` | Today the MD5/GTSM branch drops `r.listenerFactory` entirely (A-2 BROKEN). Without this, netns silently does nothing for MD5/GTSM peers |
...
| 4 | New `checkVPPLCPPlugin` doctor check + `doctor-vpp-lcp-plugin` code | `internal/plugins/iface/vpp/doctor.go`, `internal/core/diagnostic/codes.go` | Problem B |
| 5 | `checkVPPLCPNetns` NARROWS from "netns is not root-reachable" to "vpp.lcp.netns and the BGP listener netns disagree" | `internal/plugins/iface/vpp/doctor.go` | AC-3. Once BGP can bind in a namespace, the current warning is stale; the real hazard becomes a MISMATCH, not non-root-ness |
| 6 | Non-scope, flagged: the `doctor-vpp-wireguard` description over-claims a runtime check it does not do | `internal/core/diagnostic/codes.go` | See Design Insights; needs Thomas's ruling on whether to fix here or separately |

→ Decision: the fork "teach BGP netns" vs "force LCP into the host netns" is resolved in
  favour of TEACHING BGP, because `lcpPairNetns` (`lcp.go`) already deliberately
  ...
  regression, and the YANG default is "dataplane" (`ze-vpp-conf.yang`). Confirm with
...
- Problem A: config `vpp { lcp { netns "dataplane" } }` plus a `bgp` stanza with a listen
  ...
  (`internal/component/vpp/yang/ze-vpp-conf.yang`, default "dataplane") and reaches
  VPP through `lcp_itf_pair_add_del` (`lcp.go`). The BGP listen address enters via the
  bgp config and reaches `NewListener` (`listener.go`).
...
2. startup.conf generation: `linux_cp_plugin.so` and `linux_nl_plugin.so` are enabled when
   LCP is on (`startupconf.go`).
3. Doctor, post-config phase: `checkVPPLCPNetns` warns if the netns is not root-reachable
   (`doctor.go`). No check exists for plugin presence.
4. Apply: `lcpItfPair` sends `LcpItfPairAddDel` with `lcpPairNetns(settings.Netns)`
   (`lcp.go`, `:87-102`). A missing `linux_cp_plugin.so` fails HERE, failing the apply.
5. VPP creates the TAP in the named namespace.
6. BGP: `NewListener(addr)` (`listener.go`), then the reactor replaces the factory with
   `r.newListenerFactory(port)` (`reactor.go` global, `:1402` per-port), which returns
   either a fresh `RealListenerFactory{MD5Peers, ListenTTL}` or the injected
   `r.listenerFactory` (`reactor.go`). `StartWithContext` calls
   `listenerFactory.Listen` (`listener.go`) -> `RealListenerFactory.Listen`
   (`network.go`) -> `lc.Listen` (`network.go`), on the reactor goroutine's thread:
...
|----------|-----|----------|
| Config to vpp component | YANG `vpp/lcp` container, `netns` leaf (`ze-vpp-conf.yang`) | [ ] |
| ze to VPP (control) | GoVPP binapi `LcpItfPairAddDel` (`lcp.go`) | [ ] |
| ze to VPP (startup) | generated startup.conf plugin stanzas (`startupconf.go`) | [ ] |
...
| Doctor to running VPP | today only `vppctl` exec (`checks_linux.go`); no GoVPP probe from the plugin | [ ] read 2026-07-16 |
| Doctor process to daemon process (NEWLY FOUND) | none. Doctor is offline/local (`doctor/register.go`); `GetActiveConnector()` (`vpp.go`) is nil there | [ ] read 2026-07-16 |

### Integration Points

- `network.ListenerFactory` (`internal/core/network/network.go`) - the interface.
...
- ~~`Listener.SetListenerFactory` (`internal/component/bgp/reactor/listener.go`) - the
  existing injection point; no reactor surgery needed to swap the factory.~~ SUPERSEDED: the
  reactor overwrites it at `reactor.go` / `:1402`. It is not an external seam.
...
- `diagnostic.RegisterDoctorCheck` (`internal/plugins/iface/vpp/doctor.go`) - where a
  third check registers.
- `internal/core/diagnostic/codes.go` (lines 288-299) - where a new code's registry row goes.
- ~~`vppcomp.GetActiveConnector` (used by `internal/plugins/static/backend_vpp_linux.go`) -
  an existing way to reach a live VPP connector, a candidate for the GoVPP probe.~~
  **WRONG, corrected 2026-07-16.** `GetActiveConnector` (`vpp.go`) returns process-local
  state set by the daemon's Manager Run. `ze doctor` is a different process
  (`doctor/register.go`), so it returns nil there, always. The static plugin can use it
  only because it runs INSIDE the daemon.
- `vppcomp.NewConnector(apiSocket)` (`internal/component/vpp/conn.go`) + `Connect`
  (`:44`) + `NewChannel` + `Close` - the correct probe path: the check reads
  `vpp/api-socket` from the config tree and drives its own short-lived connection.
- `api.Channel.CheckCompatiblity` (govpp `api/api.go`) with `&lcp.LcpDefaultNsGet{}`
  (`binapi/lcp/lcp.ba.go`) - the plugin-presence test itself.
...
| A-2 | A netns-aware listener can be a `ListenerFactory` implementation with no reactor surgery | `SetListenerFactory` (`listener.go`) and the factory call site (`listener.go`) | The change reaches into reactor lifecycle and grows a lot | Prototype a factory that binds in a named netns | **BROKEN, 2026-07-16.** Read the producer `newListenerFactory` (`reactor.go`): when `md5PeersForListener(port)` is non-empty or `listenTTLForListener(port)` is non-zero it returns a FRESH `RealListenerFactory{...}` and discards `r.listenerFactory`. Also `Listener.SetListenerFactory` is overwritten by the reactor at `reactor.go` / `:1402`, so it is not an external seam at all. A netns factory injected as a separate implementation is silently dropped on MD5/GTSM ports. Reactor surgery IS required (small: thread netns through `newListenerFactory`). Mistake Log row added |
| A-3 | Binding in a named netns needs OS-thread locking for the socket's lifetime, not just at bind | `setns` semantics are per-thread; `runtime.LockOSThread` is used in the tree's netns tests | The design needs a dedicated thread or a helper process; blast radius grows | Prototype plus a QEMU test | **REFINED, still unvalidated.** Basis corrected: the cited path `internal/plugins/static/resolve_integration_linux_test.go` was not verified; the real in-tree idiom is `internal/test/runner/netns_linux.go` (`enterTestNetns`: `runtime.LockOSThread` + `netns.Get` + `netns.NewNamed` + same-thread `restore()`). HYPOTHESIS (kernel semantics, NOT read from ze source, must not be treated as verified): a socket's namespace is fixed at creation, so pinning is needed only around socket create+bind, not for the socket's lifetime; `Accept` then works from any thread. Supporting but non-probative in-tree evidence: `netns_linux.go`'s header records fork+exec'd children inheriting the locked thread's netns (validated by `TestNetnsLaunchChildInheritsNamespace`). Settle by prototype + QEMU before coding |
| A-4 | A missing `linux_cp_plugin.so` is detectable from a live VPP before apply | `startupconf.go` asks for the plugin; the failure lands at `lcp.go` | The check cannot exist pre-apply and the answer is a better error at the binapi layer | Probe a VPP built without the plugin | **CONFIRMED, 2026-07-16.** Two independent lines. (1) Mechanism: `api.Channel.CheckCompatiblity` (govpp `api/api.go`, impl `core/channel.go`) resolves each message ID and reports `adapter.UnknownMsgError` as `CompatibilityError.IncompatibleMessages`; `LcpDefaultNsGet` (`binapi/lcp/lcp.ba.go`) is supplied only by linux_cp. (2) Real-VPP evidence already recorded: the followup-vpp-iface record states `ligato/vpp-base` lacks `linux_cp_plugin.so` and the binapi call returns **"unknown message"** -- the exact condition `CheckCompatiblity` detects. Premise UNCHANGED, so that record needed no correction (see note below) |
| A-5 | A GoVPP probe is the right mechanism, as the deferrals row states | deferrals 2026-07-10 row; but the only precedent (`checks_linux.go`) uses `vppctl` exec | The check uses vppctl instead, which conflicts with plugin self-containment | DESIGN decision with the user | **CONFIRMED, 2026-07-16.** GoVPP wins on merit, not just on the deferrals row's say-so: `CheckCompatiblity` is a pure message-ID lookup (it never sends the message), needs no `vppctl` binary, parses no human text, and lets the check live in the owning plugin per `ai/rules/plugins.md`. The vppctl precedent exists only because the central doctor package had no VPP client; ifacevpp already has one |
| A-6 | Doctor runs with a live VPP available at `DoctorPhasePostConfig` | `checkVPPVersion` execs vppctl at doctor time (`checks_linux.go`) | A runtime probe is impossible in that phase; the check needs a different phase or trigger | Trace the doctor phase's runtime context | **BROKEN, 2026-07-16.** Doctor is an OFFLINE, LOCAL command (`doctor/register.go`: `Mode: "offline"` + `MustRegisterLocalMeta("doctor", Run, ...)`); `Run` -> `runChecks` (`doctor/doctor.go,68`) builds only storage + platform and never starts a VPP Manager. The producer `GetActiveConnector` (`vpp.go`) returns `connectorRef`, set only by `setActiveConnector` on the daemon's Run path, so it is nil in the doctor process. Consequence: the probe MUST open its own connector from `vpp/api-socket` (`ze-vpp-conf.yang`). Not fatal to the check, but it invalidates the spec's own proposed integration point. Mistake Log row added |
| A-7 | Operators actually want LCP TAPs in a non-root netns, so teaching BGP netns is worth it rather than forcing host netns | The `netns` leaf exists and defaults to "dataplane" (`ze-vpp-conf.yang`) | The cheaper answer is to default LCP to the host netns and keep the doctor warning | User / operator input | ~~**UNVALIDATED -- NEEDS THOMAS.**~~ ~~**ANSWERED (user, 2026-07-16): "Both: fix the default now, keep netns as real work."**~~ **CORRECTED, 2026-07-16 (same day). The real answer: YES, support a non-root netns, because it IS the default and the documented model.** The QUESTION was REFRAMED first, and that is the load-bearing part: A-7 as written implies a niche opt-in, but the unreachable case is the **DEFAULT** (`ze-vpp-conf.yang` defaults to "dataplane"; `lcpNetnsIsRootReachable` at `doctor.go` accepts only "", "host", "root"). ~~which contradicts the design intent stated at `lcp.go`~~ **FALSE, withdrawn:** `lcp.go` is a code comment recording its author's belief, not a decision. The recorded decision is `plan/deferrals.md` (2026-07-10): reachability "**without forcing the operator to a root-reachable netns**". `"dataplane"` is IPng's production convention, copied on purpose (`54bffb83b`; `docs/research/vpp-deployment-reference.md`). -> Decision (user, 2026-07-16): **the default STAYS**; there is no default-fix spec and `plan/spec-fixit-vpp-lcp-netns-default.md` must not be created. The whole answer is owned by `plan/spec-bgp-netns.md`, whose priority is RAISED. **No longer gates anything in THIS spec** (it never gated Problem B), and no longer blocks the netns half either |
...
| R-3 | "Parallel to doctor-vpp-wireguard" misleads: the wireguard check is config-only and the LCP one cannot be | A design that greps the config tree for a non-existent LCP toggle | Confirmed already by reading `doctor.go`; the LCP check needs a runtime probe |
| R-4 | A GoVPP probe from the doctor path opens a VPP connection at diagnosis time, with its own failure modes (socket absent, VPP down) | Doctor errors on a box where VPP simply is not running yet | Degrade to a warning when VPP is unreachable, as `checkVPPVersion` does (`checks_linux.go`) |
| R-5 | LCP is enabled by default (`ze-vpp-conf.yang`), so a new error-severity check could fail apply for existing working deployments | Deployments that were fine start failing doctor | Choose severity deliberately; the netns check chose Warning (`doctor.go`) |
| R-6 | Linux-only work lands without QEMU proof | The netns leg is proven only by unit tests | `ai/rules/platform-linux.md` is mandatory; identify the rail during research |
...
| AC-6 | `lcp.enabled` is true and VPP is unreachable at doctor time (socket absent, VPP down) | Degrades to a warning about the probe, never a false "plugin missing" claim. Mirrors `checkVPPVersion` (`checks_linux.go`) |
...
|-------|-------|------------|---------------|---------------|
| `vpp.lcp.netns` | string, YANG default "dataplane" (`ze-vpp-conf.yang`) | any existing namespace name; "", "host", "root" mean the host netns (`doctor.go`) | N/A (no numeric range) | N/A |
| Probe timeout | candidate 3s, matching `checkVPPVersion` (`checks_linux.go`) | 3s | a timeout so short the probe flaps | a timeout that stalls `ze doctor` |
...
| LCP host TAP name | existing constraint, unchanged by this spec: <= 15 bytes (`lcp.go` `lcpMaxHostName`, Linux IFNAMSIZ) | 15 bytes (`lcp.go` rejects longer) | N/A | 16 bytes -> error, never silent truncation |
...
- The followup-vpp-iface record - ~~update if A-4's premise changes~~ **NO CHANGE
  NEEDED.** A-4's premise is CONFIRMED, not changed: that record's Gotcha carried
  the "unknown message" failure and the fact that `doctor-vpp-lcp-netns` does NOT catch
  plugin absence. Research agrees with it on every point; it correctly predicted this work.
  The record was retired with the learned corpus, so there is nothing left to correct.
- **NAMING COLLISION, flagged:** "A-4" means two different things. In the
  followup-vpp-iface record and in `doctor.go` / `lcp.go` ("see A-4"), it is the ORIGIN
  spec's A-4 = the BGP-netns deferral (Problem A). In THIS spec's Assumptions table,
  A-4 = "a missing plugin is detectable" (Problem B). They are unrelated. The source
  comments' "(A-4)" references point at a retired spec (`spec-followup-vpp-iface.md`), and
  the record that replaced it was retired too, so they need a live design target
...
|-------------------|---------|------|
| YANG schema | [ ] Problem A only, pending Thomas (Q3). Read `ai/rules/config.md` (YANG vs env var) and `ai/rules/config.md` | `internal/component/bgp/yang/` |
...
| QEMU rail | [ ] Problem A only. Mandatory (`ai/rules/platform-linux.md`) and NOT identified: no VPP QEMU rail found, and `1098` records that real-VPP LCP proof needs a VPP image carrying the linux-cp plugins | to identify before Phase 3 (Q12) |
...
| 12 | Internal architecture changed? | [ ] Problem A: Yes. `network.go`'s package doc and `listener.go`'s `// Design:` anchor both describe listener creation; a namespace concept changes that contract | `docs/architecture/core-design.md` |
...
| 16 | Any changed source file referenced by doc source anchors? | [ ] Grep `docs/` for the changed files. Known: `lcp.go` anchors `docs/research/vpp-deployment-reference.md`; `network.go` anchors `docs/architecture/chaos-web-dashboard.md`; `doctor.go` anchors `ai/rules/repo-maintenance.md`. Run `scripts/dev/check_doc_links.py --design-only` | per grep |
| 17 | Design doc declared by a changed file's `// Design:` header? | [ ] **No change owed.** `internal/core/diagnostic/codes.go` declares `docs/features/ai-first.md`. That document specifies the `ze explain <code>` contract and states that every diagnostic carries a stable code, naming codes only by example ("e.g., `config-parse`, `config-yang-type`, `config-listener-conflict`"). It holds no enumeration of doctor codes. This spec ADDS one row to the registry through exactly that documented mechanism and changes neither the contract nor a list, so the design it declares is unaffected. Recorded because `scripts/dev/spec_doc_anchors.py` requires every declared design doc to be named | `docs/features/ai-first.md` (unaffected) |
...
| Registration over hardcoding | The new doctor check registers via `diagnostic.RegisterDoctorCheck` from the owning plugin; no switch case or field added to a core/shared package (`ai/rules/repo-maintenance.md`, `ai/rules/plugins.md`) |
| Doctor checks | New runtime dependency has a check, a code in `codes.go`, a unit test, and a functional test per `ai/rules/repo-maintenance.md` |
...
|------------------|---------------|----------------|--------|
| (A-2) A netns listener can be a new `ListenerFactory` implementation injected through the existing seam, with no reactor surgery | `Listener.SetListenerFactory` (`listener.go`) is overwritten by the reactor immediately after `NewListener` (`reactor.go`, `:1402`), so it is not an external seam. And the producer `newListenerFactory` (`reactor.go`) DISCARDS the injected `r.listenerFactory` whenever MD5 or GTSM applies to the port, returning a fresh `RealListenerFactory{...}` | Read the producer `newListenerFactory` instead of stopping at the consumer `listener.go`. The skeleton had read only the call site and the setter, and inferred the seam was usable | Design change: netns becomes a FIELD on `RealListenerFactory`, not a rival factory. Reactor DOES change. Prevented a silent wrong-namespace bind for every MD5/GTSM peer (R-8) |
| (A-6) A live VPP connector is available to doctor checks, via `vppcomp.GetActiveConnector` | Doctor is an offline LOCAL command (`doctor/register.go`) running in the `ze doctor` process; `GetActiveConnector` (`vpp.go`) returns `connectorRef`, set only by the daemon's Manager Run, so it is nil there | Read the producer `setActiveConnector`/`GetActiveConnector` and doctor's registration, rather than reasoning from `internal/plugins/static/backend_vpp_linux.go` which uses it successfully (that plugin runs INSIDE the daemon) | The spec's own proposed Integration Point was wrong. The probe must open its own connector from `vpp/api-socket`. Also explains WHY the vppctl precedent exists, dissolving the A-5 "tension" |
...
| (A-7) The `vpp.lcp.netns` default `"dataplane"` is a MISTAKE that "contradicts its own design intent", per the comment at `lcp.go` | The default is DELIBERATE. `plan/deferrals.md` (2026-07-10) records the actual decision: reachability "**without forcing the operator to a root-reachable netns**". `"dataplane"` is IPng's production convention, copied on purpose (`54bffb83b` "following IPng production template"; `docs/research/vpp-deployment-reference.md` "isolates the forwarding plane from the management plane"). The YANG leaf states the model plainly: "Routing daemons run in this namespace" | Thomas rejected the premise, 2026-07-16. The session had ALREADY read `plan/deferrals.md` earlier the same day (it is this spec's Origin section) and still took a code comment as the intent record | Invented a phantom spec (`plan/spec-fixit-vpp-lcp-netns-default.md`, never created) referenced from both specs, and inverted `plan/spec-bgp-netns.md`'s priority to "build it last". **A comment states what its author believed, not what was decided.** Rule updated: `ai/rules/evidence.md` "Evidence corollary" |
...
- The deferrals row frames Problem B as "parallel to `doctor-vpp-wireguard`". A code read
  shows the parallel is intent-only: `checkVPPWireguardPlugin` (`doctor.go`) is a
  CONFIG check reading the `vpp.plugins.wireguard` toggle, while LCP has no such toggle
  (`startupconf.go` enables the plugin straight from `lcp.enabled`). The new check
...
- FINDING (pre-existing, unrelated to this spec's scope): the `doctor-vpp-wireguard` code
  description in `internal/core/diagnostic/codes.go` claims the check covers "not enabled
...
- `vpp.lcp.netns` defaults to "dataplane" (`ze-vpp-conf.yang`) and
  `lcpNetnsIsRootReachable` (`doctor.go`) accepts only "", "host", "root". So the
...
- ~~The BGP listener already takes an injectable `ListenerFactory` (`listener.go`, `:66-68`,
...
| `ai/rules/architecture.md` tier test A (config-driven engine?) | Does it call `sdk.NewWithConn(`? | **PASSES.** No. It is a pure library with no lifecycle: `internal/core/` is correct per the authoring rule ("pure library, no `sdk.NewWithConn`, no plugin lifecycle, no component domain owner") |
...
- **Outbound BGP connections stay in ze's namespace.** Only the LISTENER gains netns
  awareness. `RealDialer` (`network.go`) would need its own `Netns` field for a peer
...
- **The doctor probe reports what the RUNNING VPP loaded, not what is on disk.** ~~`CheckCompatiblity` proves
  the linux_cp API is unreachable on the running VPP; it cannot distinguish "the `.so` is
  absent" from "the `.so` is present but failed to load" or a CRC drift (R-10).~~ **CORRECTED
  2026-08-24: that sentence describes the DESIGNED mechanism, which was not the one delivered.**
  Q8 records the departure; this bullet had not followed it. The delivered probe is
  `vppctlShowPlugins` (`internal/plugins/iface/vpp/doctor.go`), which reads the plugin rows of
  `vppctl show plugins`. What survives from the original wording: the probe still cannot
  distinguish "the `.so` is absent" from "the `.so` is present but failed to load", because
  either way VPP lists no such plugin. What does NOT survive: CRC drift is a failure mode of
  `CheckCompatiblity`, not of a name lookup over a plugin listing, so R-10 does not reach the
  delivered check (Q10 records the same finding). The diagnostic is worded to the delivered
  mechanism, saying the running VPP "does not load" the plugin rather than asserting the file
  is missing (R-7 warns against repeating the wireguard code's over-claim).
...
|------------------|---------------|-------------------|
| LCP misconfiguration is diagnosed by `ze doctor` before apply, so a VPP without `linux_cp_plugin.so` is named pre-apply instead of failing the whole apply at the binapi layer | Functional (`.ci`) through the user entry point, plus unit | `ze-test ui --pattern doctor-vpp-lcp` -> `pass 2/2 100.0%`, re-run at closure 2026-08-24 over `doctor-vpp-lcp-netns` and `doctor-vpp-lcp-plugin`. The Error wording itself is proven by `TestCheckVPPLCPPluginMissingIsAnError`, which asserts Severity Error, `linux_cp_plugin.so` and `linux_cp API` in the message |
...
| The DELIVERED `checkVPPLCPNetns` is reachable from the operator's entry point (AC-12) | Functional (`.ci`) | `test/ui/doctor-vpp-lcp-netns.ci`, in the run above. Its three assertions gate on doctor output only; the `ze explain` command that would have satisfied the code assertion on its own was removed from the file, and that removal is recorded in the file's own trailer |
...
| Field | Value |
|-------|-------|
| Artifact | `tmp/review/fixit-vpp-lcp-reachability-9ad8358c-695f-41be-8019-5d92ba08f8e6.md` |
| `review_gate.py check` | `review_gate: OK (3 code files, clean, hashes match ...)`, run 2026-08-24 over `doctor.go` and both `.ci` files |
| Rounds | 2 |
| Reviewer lenses used | Round 1 (implementing session): test-validity, evidence/guard, spec-bookkeeping. Round 2 (this closure, independent): every `/ze-review` lens run by one agent, the `docs/contributing/ze-style.md` pass included |
| Superseded artifact | `tmp/review/fixit-vpp-lcp-reachability-6c2adacb-9282-4d78-9180-bab7cb6a2f15.md` recorded `verdict=findings` and is not on disk in this checkout |

Round 1 found 0 BLOCKER and 3 ISSUE, and all three were fixed by the session
that authored them, so they carried no independent pass. Round 2 ran that pass.
Its scope was written before it ran (`ai/rules/planning.md`, "Bounding the
loop"): the `vppctlPluginsHeader` guard in `checkVPPLCPPlugin` and its sibling
branches, the rewritten `TestVPPLCPPluginCheckIsRegistered`, the new
...
### Run 2 (2026-08-24, independent closure pass)

0 BLOCKER, 0 ISSUE, 3 NOTE. The gate is satisfied. The three NOTEs are in the
artifact and none blocks: `vppctlShowPlugins` drops an `ExitError`'s stderr the
way its model `checkVPPVersion` (`internal/component/doctor/checks_linux.go`)
does; `checkVPPLCPPlugin` builds its probe context from `context.Background()`
because `diagnostic.DoctorCheckContext`
(`internal/core/diagnostic/doctor_registry.go`) carries none, and
`vppProbeTimeout` bounds it at 3s; `ai/DOCS-TO-CODE.md` reads stale under `make
ze-doc-verify` while both of this spec's rows are present and correct, and the
file is gitignored, so it must not ride on commit A.

Two gate reds were read and charged elsewhere, each named in the artifact:
`make ze-repository-check` reports 8 unwired exported symbols, every one in
another session's uncommitted file, and `make ze-doc-verify` fails on one
anchor in `docs/guide/web-interface.md`, which names
`liveAAABundleAuthenticator.Authenticate` in `cmd/ze/hub/aaa_authenticator_web.go`
where that symbol is no longer declared. Neither reaches a file this spec owns.
...
- `checkVPPLCPPlugin` (`internal/plugins/iface/vpp/doctor.go`), registered by
  `registerDoctorChecks` in the same file at `DoctorPhasePostConfig` order 742,
  beside `vpp-wireguard-plugin` at 740 and `vpp-lcp-netns` at 741. It probes the
  RUNNING VPP through `vppctlShowPlugins` and reports before apply.
- Its supporting values in the same file: `lcpPluginSO`, `vppctlPluginsHeader`,
  `vppProbeTimeout`, the `lcpPluginProbe` var with `defaultLCPPluginProbe`, and
  `lcpEnabled`, which reads an absent `enabled` leaf as the YANG default ON.
- The `doctor-vpp-lcp-plugin` row in `internal/core/diagnostic/codes.go`, so
  `ze explain doctor-vpp-lcp-plugin` resolves.
- `test/ui/doctor-vpp-lcp-plugin.ci` and `test/ui/doctor-vpp-lcp-netns.ci`, the
  functional halves `ai/rules/repo-maintenance.md` requires beside the units.
- The unit suite in `internal/plugins/iface/vpp/doctor_lcp_plugin_test.go`.
- Problem A was never implemented here. It moved to `spec-bgp-netns` on
  2026-07-16 with AC-1, AC-2, AC-3, AC-8, AC-9, AC-10 and Phases 2-6.

### Bugs Found/Fixed

- A zero-exit non-answer reached a confident Error. `vppctlShowPlugins` returns
  `(string(out), nil)` for any zero exit, and the check read a missing substring
  as proof the plugin is absent. Fixed by requiring `vppctlPluginsHeader`, and
  covered by `TestCheckVPPLCPPluginUntrustedOutputWarns`.
- `TestVPPLCPPluginCheckIsRegistered` asserted only the diagnostic-CODE registry
  while its name and its failure messages claimed check registration. Fixed: it
  fetches the entry from `diagnostic.DoctorChecksForPhase` and CALLS it.
- The `doctor-vpp-lcp-netns` remediation recommended `host` or `root`, which VPP
  resolves as namespace names, so following it broke LCP pair creation. Fixed
  before this closure and recorded in `plan/deferrals/bgp-netns.md`.

### Documentation Updates

- `docs/guide/vpp.md`: the severity paragraph now states both branches and the
  `Plugin path is:` condition, under the anchor
  `<!-- source: internal/plugins/iface/vpp/doctor.go -- checkVPPLCPPlugin, lcpEnabled, lcpPluginSO, vppctlPluginsHeader -->`.
- `internal/core/diagnostic/codes.go`: the registered description carries the
  same four Warning cases, and it is what `ze explain` prints.
- `make ze-doc-verify` is red on one anchor,
  `docs/guide/web-interface.md`, and on `ai/DOCS-TO-CODE.md` staleness. Neither
  reaches a file this spec owns; see Run 2 above.

### Deviations from Plan

- The probe is `vppctl show plugins`, not GoVPP `CheckCompatiblity`. A-5
  reasoned for GoVPP and A-6 then broke: doctor is an offline command with no
  VPP connector, so the check would have had to open its own. Q8 records the
  departure, and Known Limitations was corrected on 2026-08-24 to describe the
  delivered mechanism rather than the designed one.
- Q10 resolved to Error, not the Warning the readiness loop defaulted to, for
  the one case where the probe ANSWERED. A probe that cannot answer stays a
  Warning, which is what AC-6 asked for.

## Deferrals Resolved

| Row (from the deferral shard) | Final Status | Destination or evidence |
|-------------------------------|--------------|-------------------------|
| `plan/deferrals/fixit-vpp-lcp-netns-remediation.md`, 2026-07-16, R-5: `doctor-vpp-lcp-netns` stayed silent for `host` and `root` | done | Landed 2026-08-07. `checkVPPLCPNetns` splits the question: `lcpNetnsMarkerDiagnostic` warns on a marker, `lcpNetnsConfigDiagnostic` and `lcpNetnsHostDiagnostic` cover an ordinary name, and the empty leaf is the only silent value. Tests `TestDoctorLCPNetnsRootMarkerWarns`, `TestDoctorLCPNetnsEmptyLeafSilent`, `TestDoctorLCPNetnsAbsentFromHost`, `TestDoctorLCPNetnsPresentOnHost`, `TestDoctorLCPNetnsProbeError` (`internal/plugins/iface/vpp/register_test.go`) |
| `plan/deferrals/fixit-vpp-lcp-netns-remediation.md`, 2026-08-07: `validateNetns` admitted whitespace | done | Fixed 2026-08-14 by `5503a81c5`. `validateNetns` (`internal/component/vpp/config.go`) refuses any rune where `unicode.IsSpace` holds or `unicode.IsPrint` does not, so `netns "my ns"` is rejected at parse. Tests beside `TestValidate` in `internal/component/vpp/config_test.go` drive a space, a tab, a newline and a NUL. The row was still `open` at this closure and is set `done` here |
| `plan/deferrals/bgp-netns.md`, 2026-07-16: the `doctor-vpp-lcp-netns` remediation recommended a config that breaks LCP | done | No doctor surface names a netns VALUE as the remedy now. `lcpNetnsMarkerDiagnostic` and `lcpNetnsConfigDiagnostic` both print "Leave vpp.lcp.netns empty", and `lcpNetnsIsRootMarker` carries the corrected premise. `TestDoctorLCPNetnsRemediation` and `TestDoctorLCPNetnsCodeDescription` assert the message and the registered description. The row was still `deferred` at this closure and is set `done` here |

This spec has no shard of its own: `plan/deferrals/fixit-vpp-lcp-reachability.md`
does not exist. The two FOREIGN shards above now hold terminal rows only, so both
are residue and both are removed. `deferral_shard_removal_problems`
(`scripts/dev/commit_helper.py`) reads a shard at HEAD, so the removal MUST be
prepared after the commit that sets the rows `done` has landed, never in the same
helper run.
...
|------|--------|----------|
| `internal/plugins/iface/vpp/doctor.go` | yes | `ls -l` 2026-08-24: 21070 bytes |
| `internal/plugins/iface/vpp/doctor_lcp_plugin_test.go` | yes | `ls -l` 2026-08-24: 14760 bytes |
| `test/ui/doctor-vpp-lcp-plugin.ci` | yes | `ls -l` 2026-08-24: 1433 bytes |
| `test/ui/doctor-vpp-lcp-netns.ci` | yes | `ls -l` 2026-08-24: 3417 bytes |
| `internal/core/diagnostic/codes.go` | yes | `grep -c doctor-vpp-lcp-plugin` returns 2, the Code row and its `ze explain` example |

### AC Verified (grep/test)

| AC ID | Claim | Fresh Evidence |
|-------|-------|----------------|
| AC-4 | A VPP that answers without the plugin yields one Error naming the linux_cp API | `make ze-unit-pkg-test PKG=./internal/plugins/iface/vpp/... RUN='TestCheckVPPLCPPlugin\|TestLCPEnabledTreatsAbsentLeafAsOn\|TestVPPLCPPluginCheckIsRegistered\|TestDoctorLCPNetns'` exits 0 with `ok github.com/ze-software/ze/internal/plugins/iface/vpp 1.164s` under race. The sibling `yang` package in the same run prints `[no tests to run]`, which is the marker Go emits when a `-run` filter matches nothing, so its absence on the vpp package is what proves these tests RAN |
| AC-5 | The plugin present yields no diagnostic | same run, `TestCheckVPPLCPPluginPresentSilent` |
| AC-6 | A probe that cannot answer degrades to a Warning | same run, `TestCheckVPPLCPPluginProbeUnavailable` and `TestCheckVPPLCPPluginUntrustedOutputWarns`. The functional half is `test/ui/doctor-vpp-lcp-plugin.ci` |
| AC-7 | The code is registered and `ze explain` resolves it | same run, `TestVPPLCPPluginCheckIsRegistered`, which reads `diagnostic.DoctorChecksForPhase` and calls the entry. `grep '"vpp-lcp-plugin"' internal/plugins/iface/vpp/doctor.go` finds the registration beside `Codes: []string{"doctor-vpp-lcp-plugin"}` and `Check: checkVPPLCPPlugin` |
| AC-11 | LCP off, an absent tree and a non-Linux platform each skip and open no probe | same run, `TestCheckVPPLCPPluginSkipsWhenNotApplicable` asserting `fake.calls == 0`, and `TestLCPEnabledTreatsAbsentLeafAsOn` |
| AC-12 | The delivered `checkVPPLCPNetns` is reachable from the operator's entry point | same run, the `TestDoctorLCPNetns*` set. The functional half is `test/ui/doctor-vpp-lcp-netns.ci` |

### Wiring Verified (end-to-end)

Both files were re-run at closure on 2026-08-24: `ze-test ui --pattern
doctor-vpp-lcp` reports `pass 2/2 100.0% 1.1s`, and both were read line by line
rather than trusted by name.

| Entry Point | .ci File | Verified |
|-------------|----------|----------|
| `vpp { lcp { enabled true } }` plus `ze doctor --json` on a host whose VPP cannot be probed | `test/ui/doctor-vpp-lcp-plugin.ci` | Read at closure. It writes the config through `tmpfs=`, runs ONE command, and asserts `doctor-vpp-lcp-plugin` and `could not be probed`. `option=needs-linux` is present and its reason is stated |
| `vpp { lcp { enabled true } }` plus a `bgp` stanza, `ze doctor --json`, netns omitted so the YANG default applies | `test/ui/doctor-vpp-lcp-netns.ci` | Read at closure. ONE command, three assertions on doctor output alone. The file records why a second `ze explain` command was dropped: assertions match ACCUMULATED stdout, so explain's own output satisfied the code assertion with the check stubbed out |
| The netns listener wiring | none here | MOVED to `spec-bgp-netns` |

### Assumptions Resolved

| ID | Final Status | Evidence |
|----|--------------|----------|
| A-1 | confirmed | BGP still has no netns awareness. Not this spec's work; it travels with `spec-bgp-netns` |
| A-2 | broken | `newListenerFactory` (`internal/component/bgp/reactor/reactor.go`) still returns a fresh `network.RealListenerFactory{MD5Peers, ListenTTL}` and discards `r.listenerFactory`, and `RealListenerFactory` has no `Netns` field. Re-read at the producer 2026-08-24. Mistake Log row exists |
| A-3 | moved | Kernel-semantics prototype belongs to `spec-bgp-netns` with the QEMU rail. Nothing in this spec depends on it |
| A-4 | confirmed | A missing plugin IS detectable pre-apply. The delivered detector is `vppctlShowPlugins` rather than the GoVPP route the assumption argued, and the conclusion holds |
| A-5 | broken in the delivered form | GoVPP was the design answer and A-6 removed its premise. The check execs `vppctl` like its sibling `checkVPPVersion`, and it still lives in the owning plugin, which is what `ai/rules/plugins.md` asked of it |
| A-6 | broken | Doctor is offline and starts no VPP Manager, so no live connector exists at check time. The delivered probe needs none |
| A-7 | answered by Thomas, 2026-07-16 | The `vpp.lcp.netns` default STAYS `"dataplane"`. It never gated Problem B |
| A-8 | broken, resolved | Thomas ruled SPLIT on 2026-07-16. `plan/spec-bgp-netns.md` exists and holds Problem A |

### Documentation Verified

| Documentation claim or category | Source evidence | Verified |
|---------------------------------|-----------------|----------|
| `docs/guide/vpp.md` states both severities and the header condition | Read against `checkVPPLCPPlugin`: the Error branch fires only after `vppctlPluginsHeader` is present and `lcpPluginSO` is absent, which is what the paragraph says | yes |
| The registered code description matches the check | Read against the same function. Four Warning cases in the description, four Warning paths in the code counting the two error branches and the two non-answers | yes |
| Row 15, registered diagnostic code changed | `doctor-vpp-lcp-plugin` added to `internal/core/diagnostic/codes.go`; `docs/guide/vpp.md` updated | yes |
| Row 17, design doc declared by a changed file | `internal/core/diagnostic/codes.go` declares `docs/features/ai-first.md`, which specifies the `ze explain` contract and enumerates no doctor codes. This spec adds a row through that documented mechanism, so the design is unaffected | yes, no change owed |
| Row 12, internal architecture changed | No. That row is Problem A's, and Problem A moved out | not applicable |
| Doctor check for a new runtime dependency | The dependency is `vppctl` plus a reachable VPP, and the check IS the doctor check. `ai/rules/repo-maintenance.md` also asks for a functional test beside the unit tests, and both `.ci` files exist | yes |
| RFC status | No protocol behavior changes. The check touches no wire format, no capability and no FSM path | not applicable |

## Core Insight

A probe has three answers, not two, and the third one is the dangerous one. The
check began with "the command failed" and "the command said no", and its own
comment argued correctly that a failure proves nothing about the plugin set. The
missing case was a command that SUCCEEDS and says nothing: `vppctl` exits zero
for empty or truncated output, so the absence of a name in that output is the
absence of a name in nothing. Any code that reads a substring out of a command's
stdout owes a test that the stdout is an answer at all, and the cheapest such
test is the header the command always prints first.
