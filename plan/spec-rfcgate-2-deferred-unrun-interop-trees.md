...
|------|-----------|--------|------------------|
| `test/ipsec-interop/` | 10 | `make ze-ipsec-interop-test` | none |
| `test/l2tp-interop/` | 3 | the L2TP interop runner | none |
| `test/pppoe-interop/` | 1 | `make ze-deployment-pppoe-accel-docker-test` | none |
...
Note for whoever picks this up: all three lab launchers shared a defect with the
BGP one -- they started ze with the removed bare `ze <config>` form, so every
scenario died at container health. That was fixed in the evidence spec for all
four trees at once, but none of the three has been RUN since, so treat their
green as unverified until you see it.
