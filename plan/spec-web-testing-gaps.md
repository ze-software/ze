...
| G-3 | `make ze-tracked-build-check` compiles what git holds with `go build`, which compiles no `_test.go`. A commit that breaks only tests leaves HEAD red and the check green | It bit twice in one day. Once when this session deleted a directory three test files still embedded, and once when a closure found four uncommitted files leaving HEAD red on a test-only failure | main thread, both incidents |
