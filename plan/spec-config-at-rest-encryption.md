# Spec: config-at-rest-encryption
...
Ze's configuration and state store is plaintext on disk, and it holds every
operator secret in a reversible form.

`BlobStore.encode` (`pkg/zefs/store.go`) emits a plain netcapstring container,
which `BlobStore.flushFull` writes through `atomicWrite`. There is no encryption
path, opt-in or otherwise, and no TPM, sealed-key, LUKS or dm-crypt integration
anywhere in `internal/`, `pkg/`, `cmd/` or `gokrazy/`. The installer mounts a
plain ext4 filesystem (`mountFS`, `internal/install/disk/system.go`).

Protocol secrets in that file are obfuscated, not encrypted.
`internal/component/config/secret/secret.go` says so in its own doc comment: the
JunOS-style `$9$` scheme "can always be decoded back to the original plaintext".
It covers BGP MD5 passwords, RADIUS and TACACS keys, IPsec pre-shared keys,
plugin hub secrets, and OSPF and IS-IS authentication keys. Login passwords are
the exception and are properly bcrypt-hashed
(`internal/component/config/password_hash.go`).

Consequence: whoever reads the zefs blob holds every peering password, RADIUS
secret and IPsec pre-shared key on the device. That is a reasonable position for
a device whose disk is trusted. It is an unreasonable one for a stolen or
decommissioned appliance.

Goal: decide whether Ze encrypts its store at rest, what anchors the key (TPM
sealing, an operator passphrase supplied at boot, or nothing), and what the
recovery story is when the anchor is unavailable. Answer the design questions
before writing code. An encrypted store that cannot be recovered is a worse
outcome than a plaintext one.

Provenance: VyOS ships config encryption on a TPM-backed volume. Their July 2026
commit (T8344) was a symlink-preservation bugfix inside a feature Ze does not
have at all.
