...
| Depends | - |
| Phase | - |
| Deferral shard | `plan/deferrals/ipsec-auth-piggyback.md` |
| Updated | 2026-08-01 |
...
`RFC7296-2.21.2-2` is committed at `rfc/short/rfc7296.md:564`. It is gated in both
polarities by `TestErrInitiatorSurvivesPiggybackedErrorNotify`
(`internal/component/ike/engine/rfc7296_errornotify_test.go:409`), recorded at
`ai/RFC-REQUIREMENTS.md:3697` with its positive tag at
`internal/component/ike/engine/rfc7296_errornotify_test.go:396` and its negative tag at
...
(`internal/component/ike/engine/responder.go:566-569`). Read it before you start.

### The obligation, quoted verbatim

`rfc/full/rfc7296.txt:3298-3315`, with the page break at `:3303-3310`. The block below
is quoted text, and not a code sample. `ai/rules/simplified-technical-english.md` keeps
...
- [ ] `ai/rules/fail-closed-guards.md` - the new state is a resource an attacker reaches
...
- [ ] `internal/component/ike/engine/responder.go:562-574` -
  ...
  this spec's work.
- [ ] `internal/component/ike/engine/responder.go:867` - `respondAuthError` sends one
...
- [ ] `internal/component/ike/engine/notify_error.go:193` - `notifyForRefusal` maps the
  refusal to INVALID_SYNTAX, TS_UNACCEPTABLE or NO_PROPOSAL_CHOSEN.
- [ ] `internal/component/ike/engine/responder.go:818` -
  ...
  `internal/component/ike/engine/fsm.go:771`, which is the initiator path.
- [ ] `internal/component/ike/engine/established.go:17` - `(*PeerSession).runEstablished`
  ...
  child SA".
- [ ] `internal/component/ike/engine/established.go:48-55` - the initiator branch of the
  same function calls `createFirstChildSA` with no test of what the response accepted.
- [ ] `internal/component/ike/engine/child.go:154` - `createFirstChildSA`. At `:194-202`
  it generates a random outbound SPI when `sa.ChildOutboundSPI` is zero.
- [ ] `internal/component/ike/engine/fsm.go:707-713` - `handleAuthResponse` sets
...
- [ ] `internal/component/ike/engine/fsm.go:623` - `handleAuthResponse`. It aborts on
...
- [ ] `internal/component/ike/engine/fsm.go:757-763` - the accepted-offer check runs
  under `if childOffer != nil`. A response with no SA payload skips it.
- [ ] `internal/component/ike/wire/payload_notify.go:123` - `NotifyTypeRecognized` reads
  ...
  reaches the abort at `fsm.go:654`.
- [ ] `internal/component/ike/engine/inbound.go:441-448` -
  ...
  a new Child SA.
- [ ] `internal/component/ike/engine/inbound.go:261` - the same function. Its Child rekey
  responder path at `:346-390` reads the existing Child SA at `:347` and replaces it.
- [ ] `internal/component/ike/engine/fsm.go:201` - `(*PeerSession).runResponder` polls
...
| # | Gap | Producer |
|---|-----|----------|
| G-1 | No responder establish path with no Child SA | `finishResponderEstablish` (`internal/component/ike/engine/responder.go:818`) |
| G-2 | The owner loop refuses a Child-SA-free SA | `runEstablished` (`internal/component/ike/engine/established.go:56-64`) |
| G-3 | No exchange attaches a Child SA later | `handleCreateChildSAOwned` (`internal/component/ike/engine/inbound.go:441-448`) |
...
`runEstablished`, which calls `createFirstChildSA` at `established.go:50` with no test of
whether the responder accepted a Child SA.

`sa.ChildOutboundSPI` is zero in that case (`fsm.go:707-713`). `createFirstChildSA`
therefore invents an outbound SPI (`child.go:194-202`). The result is a Child SA whose
...
- Every scenario under `test/ipsec-interop/scenarios/` stays green.
...
- A failed AUTH verification still deletes the IKE SA
  (`internal/component/ike/engine/responder.go:548-553`).
...
1. AUTH verification. `verifyRemoteAuth` passes
   (`internal/component/ike/engine/responder.go:548`).
2. Child SA construction. `buildAuthResponse`
   (`internal/component/ike/engine/responder.go:562`) returns an error.
3. Classification. `notifyForRefusal` (`internal/component/ike/engine/notify_error.go:193`)
...
### Integration Points
- `finishResponderEstablish` (`internal/component/ike/engine/responder.go:818`) already
  separates the parallel and the direct case. The Child-SA-free case is a third case, and
  not a new function.
- `respondChildRekey` (`internal/component/ike/engine/rekey.go:243`) builds a replacement
  Child SA from a request. The attachment path in G-3 needs the same construction with no
  old Child SA.
- `createFirstChildSA` (`internal/component/ike/engine/child.go:154`) is the single
...
|----|-----------|--------------------------------|----------|--------------|--------|
| A-1 | `finishResponderEstablish` is the only responder path to `StateEstablished` | A search for `= StateEstablished` outside test files returns `internal/component/ike/engine/responder.go:847` and `fsm.go:771` only, 2026-08-01 | A second path establishes an SA this design never reaches | Rerun the search at the start of the design phase | unvalidated |
| A-2 | The owner loop needs no Child SA for its liveness and lifetime timers | `runEstablished` (`internal/component/ike/engine/established.go:17`), read for the nil branch only | The timers read Child SA fields and the loop needs a wider change | Read every field access in the loop body | unvalidated |
| A-3 | The initiator installs a Child SA with an invented outbound SPI when the response accepted none | `established.go:48-55`, `fsm.go:707-713` and `child.go:194-202`, read 2026-08-01 | The initiator defect does not exist and phase 4 is unnecessary | Drive a response with a piggybacked notification and read the installed SPI | unvalidated |
...
| A-6 | `test/ipsec-interop/` stays outside the automated tiers | `plan/spec-rfcgate-2-deferred-unrun-interop-trees.md` | An interop tag becomes legal evidence | Rerun `make ze-rfc-check` at landing time | unvalidated |
| A-7 | No committed RFC row needs to change | `RFC7296-2.21.2-2` is bound in both polarities at `ai/RFC-REQUIREMENTS.md:3697` | The spec touches a gated row and the proof ratchet applies | Rerun `make ze-rfc-check` after the change | unvalidated |

### Risks
| ID | Risk | Early signal | Mitigation / fallback |
|----|------|--------------|----------------------|
| R-1 | An unauthenticated peer accumulates Child-SA-free IKE SAs. Each one costs keys, a session and a table entry, and none of them carries traffic. This is the primary security risk of the whole spec | The SA table grows against a peer that never completes a Child SA | Only an AUTHENTICATED peer reaches the new state, because the change sits after `verifyRemoteAuth` (`internal/component/ike/engine/responder.go:548`). Add a bounded lifetime for a Child-SA-free SA, and delete it when the timer expires. `ai/rules/fail-closed-guards.md` governs the bound |
...
|-------------|---|--------------|------|
| An IKE_AUTH request whose AUTH verifies and whose Child SA request Ze refuses | → | The refusal branch of `handleAuthRequest` (`internal/component/ike/engine/responder.go:562-574`) | `TestPiggybackResponderKeepsIKESAAlive` |
| A responder SA with no Child SA reaches the owner loop | → | `runEstablished` (`internal/component/ike/engine/established.go:56-64`) | `TestOwnerLoopRunsWithoutChildSA` |
| A CREATE_CHILD_SA request arrives on a Child-SA-free SA | → | `handleCreateChildSAOwned` (`internal/component/ike/engine/inbound.go:441-448`) | `TestCreateChildSAAttachesFirstChild` |
| An IKE_AUTH response carries an error notification in place of SAr2 | → | The initiator branch of `runEstablished` (`internal/component/ike/engine/established.go:48-55`) | `TestInitiatorInstallsNoChildSAWhenNoneAccepted` |
| An operator reads a Child-SA-free SA | → | `show vpn ipsec sa` (`internal/component/ike/cmd/show_ipsec.go:72`) | `test/ipsec/ipsec-show-sa-no-child.ci` |
...
| 5 | Runs Ze as initiator against a strict peer that refuses the Child SA | wire → `handleAuthResponse` → established SA with no Child SA | `test/ipsec-interop/scenarios/22-auth-piggyback` |
...
(`internal/component/ike/engine/rfc7296_errornotify_test.go:409`) already proves AC-12
...
|----------|-----------|-------------|----------------|--------|
| `22-auth-piggyback` | `test/ipsec-interop/scenarios/` | strongSwan | A strongSwan peer whose Child SA Ze refuses keeps its IKE SA | |
| `23-auth-piggyback-attach` | `test/ipsec-interop/scenarios/` | strongSwan | The same peer attaches a Child SA with a later CREATE_CHILD_SA exchange | |
...
- `internal/component/ike/engine/responder.go` - the refusal branch of
  `handleAuthRequest` (`:562-574`), `finishResponderEstablish` (`:818`), and
  `respondAuthError` (`:867`) for the cached response
- `internal/component/ike/engine/established.go` - `runEstablished` (`:17`), the nil Child
  SA branch (`:56-64`), and the initiator branch (`:48-55`)
- `internal/component/ike/engine/inbound.go` - `handleCreateChildSAOwned` (`:441-448`)
...
- `internal/component/ike/engine/child.go` - the attachment entry to `createFirstChildSA`
  (`:154`)
- `internal/component/ike/cmd/show_ipsec.go` - the Child-SA-free display (`:72`)
...
- `test/ipsec-interop/scenarios/22-auth-piggyback/`
- `test/ipsec-interop/scenarios/23-auth-piggyback-attach/`
...
The name follows `ai/rules/config-naming.md`: kebab-case, no abbreviations, and the unit
...
once" or "no bound". `ai/rules/config-surface.md` decides that question, and the default
...
| Correctness: only an authenticated peer reaches the new state | The change sits after `verifyRemoteAuth` (`internal/component/ike/engine/responder.go:548`), and no earlier branch reaches it |
...
| Rule: `ai/rules/fail-closed-guards.md` | The bound denies by default. An unset or unreadable bound never means "no limit" |
...
| Rule: `ai/rules/no-fabrication.md` | Every claim about the engine cites the producing function, and not its caller |
