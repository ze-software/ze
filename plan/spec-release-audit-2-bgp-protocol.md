...
4. `ai/rules/rfc-reading.md` and `ai/rules/rfc-compliance.md` - protocol claim rules
...
9. `internal/component/bgp/capability/capability.go` - capability and optional-parameter parsing current behavior
...
- [ ] `ai/rules/rfc-reading.md` - RFC summaries before protocol claims
...
- [ ] `ai/rules/functional-test-gate.md` - user-visible wire behavior needs functional coverage
...
- [ ] `internal/component/bgp/capability/capability.go` - parses capability TLVs, extracts capabilities from OPEN optional parameters, and silently stops on malformed optional parameter boundaries
- [ ] `internal/component/bgp/capability/capability_test.go` - tests normal, empty, truncated, unknown, and valid ADD-PATH capabilities, but not overlong known capabilities or invalid ADD-PATH modes
- [ ] `internal/component/bgp/capability/negotiated.go` - tracks peer capability codes and performs directional ADD-PATH negotiation for modes 1, 2, and 3
...
- [ ] `internal/component/bgp/attribute/wire.go` - lazy attribute index rejects duplicate attributes when consumers parse attributes later
...
- [ ] `test/interop/scenarios/*/check.py` - actual interop scenario tree currently includes scenarios through `37-remove-private-as-as4path-frr`
- [ ] `test/interop/scenarios/11-addpath-frr/check.py` - checks ADD-PATH capability negotiation and multiple path receipt with FRR
- [ ] `test/interop/scenarios/12-route-refresh-frr/check.py` - checks normal route refresh via FRR soft-in clear and route stability
...
| Malformed optional parameter extraction | `internal/component/bgp/capability/*_test.go` | Short header, length overrun, malformed capability TLV | Suggested for RA-BGP-002 |
| Known capability exact length | `internal/component/bgp/capability/capability_test.go` | MP, ASN4, Route Refresh, Enhanced RR, Extended Message length policy | Suggested for RA-BGP-003 |
| Invalid ADD-PATH mode | `internal/component/bgp/capability/capability_test.go` | Mode 0 and 4 ignored or rejected per chosen policy | Suggested for RA-BGP-004 |
...
|----------|-----------|-------------|----------------|--------|
| `11-addpath-frr` | `test/interop/scenarios/11-addpath-frr` | FRR | ADD-PATH happy-path negotiation and multiple paths | Existing |
| `12-route-refresh-frr` | `test/interop/scenarios/12-route-refresh-frr` | FRR | Normal route refresh happy path and session stability | Existing |
| `37-remove-private-as-as4path-frr` | `test/interop/scenarios/37-remove-private-as-as4path-frr` | FRR plus BIRD | AS4_PATH handling with remove-private-as policy | Existing |
...
|------------------|---------------|----------------|--------|
| RFC 7606 duplicate non-MP attribute tests were absent | `rfc7606_test.go` already includes duplicate ORIGIN and MED helper tests | Grep and read of `rfc7606_test.go:898-930` | Finding changed from missing unit tests to missing downstream first-wins enforcement evidence |
