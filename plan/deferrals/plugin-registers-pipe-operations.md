...
| 2026-08-22 | spec-plugin-registers-pipe-operations | A daemon-backed source for the published command catalog, so `make ze-command-list` and `ze help command --json` report a plugin's pipe aliases | Both read the compiled tree in their own process and start no plugin, so neither can see a declaration that only ever reaches a running daemon. The wiki catalog therefore lists a plugin's commands without its aliases. The running daemon answers through completion and `command help`, which is where an operator looks, so the gap costs the published page and costs an operator nothing | a spec of its own, not yet written | deferred |
