...
|------|--------|------|--------|-------------|--------|
| 2026-07-09 | spec-followup-web-cli-ux AC-8/9 | Nushell shell-generator glue for the new flag inventory + `ze config show` config-section completion | AC-8 scopes completion to bash/zsh/fish (all wired + tested); nushell's single-completer model needs separate, un-runnable-here wiring | `plan/spec-finish-web-cli-ux.md` (work item added; re-homed 2026-07-16 -- the original destination `spec-followup-web-cli-ux.md` was retired in `49d1185ed`, and its closure record did not mention nushell, so this was never done) | deferred |
