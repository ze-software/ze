...
|------|--------|------|--------|-------------|--------|
| 2026-08-02 | spec-ipsec-esp-dual-form-receive | Write `test/interop-ipsec/scenarios/esp-encap-no-nat/`, the strongSwan proof that an encapsulated ESP form is accepted with no NAT present. The spec's Interop table names it and the directory does not exist | Gated by tier, not by difficulty. `test/interop-ipsec/` has no automated caller, so the retired `scripts/dev/rfc_requirements.py` (current producer: `internal/le/rfc/rfc.go`) derives it as unrun and the gate refuses an RFC tag placed there. Writing the scenario is useful before that lands, but it earns no compliance evidence until the tree is wired, which is `plan/spec-rfcgate-2-deferred-unrun-interop-trees.md` AC-1 to AC-3 territory for the ipsec tree | `plan/spec-ipsec-esp-dual-form-receive.md` | deferred |  <!-- doc-links: ignore (interop scenario this document plans; it does not exist in the tree) -->
...
- `test/ipsec/ipsec-esp-form-change.ci` and `test/ipsec/ipsec-esp-encap-no-nat.ci` --
  ...
  path needs. the retired `scripts/evidence/qemu-all-tests.sh` (current producer: `internal/le/qemu/alltests.go`) also has no `fsuite ipsec` line, so
  ...
  `TestEncapEstablishedSAServesAPeerFormChange` plus interop scenario esp-form-change.
...
esp-form-change proves it against strongSwan, and the page now cites that evidence.
