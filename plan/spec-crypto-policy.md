...
Ze has no operator-facing crypto policy of any kind, and its TLS floor is
inconsistent between components.

Every `tls.Config` in the tree is a hardcoded literal. `selfcert.NewTLSConfig`
(`internal/core/selfcert/selfcert.go`) sets `MinVersion: VersionTLS12` and no
cipher suites and no curve preferences. The plugin hub
(`internal/component/plugin/ipc/tls.go`) hardcodes TLS 1.3. So web,
looking-glass, gNMI, MCP and gRPC sit at 1.2 while the hub sits at 1.3, and an
operator cannot raise the floor. The TLS YANG containers across those components
expose only `cert` and `key`.

The SSH server (`internal/component/ssh/ssh.go`) sets no ciphers, key exchanges,
MACs or host-key algorithms, and takes library defaults. The PKI component
models storage only. Per-protocol algorithm selection does exist where an RFC
requires negotiation (IPsec `encryption` and `hash`, OSPF and IS-IS
`algorithm`), but that is per-protocol and is not a system policy.

There is no FIPS mode and no approved-algorithm restriction anywhere.

Goal: decide and build a system crypto policy surface, and decide separately
whether Ze offers a FIPS-140-3 mode.

**Design is required before any code, and the design has a Ze-specific
constraint.** Ze is Go 1.26, so a FIPS mode routes through the toolchain's own
FIPS 140-3 module (`GOFIPS140`) plus a build variant and a runtime assertion. It
does not route through an OpenSSL configuration file. Decide first whether a
policy surface and a FIPS mode are one feature or two.

Provenance: VyOS T8529 added `system option fips` for OpenSSL. Their mechanism
does not transfer, and they ship it with a runtime warning that support is
incomplete. The gap it points at is real for Ze regardless of their
approach.
