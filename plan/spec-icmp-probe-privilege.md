...
Ze's ping and traceroute open raw ICMP sockets (`NetworkICMPv4 = "ip4:icmp"` in
`internal/core/probe/icmp.go`, used by `doPingCtx` and by the traceroute and
stream paths). Two things are missing around that.

**No doctor check.** VRRP, OSPF, OSPFv3 and IS-IS each register a raw-socket
doctor check, for example `checkVRRPRawSocket`
(`internal/plugins/vrrp/transport/doctor.go`, diagnostic code
`doctor-vrrp-raw-socket`), so an operator can diagnose a missing CAP_NET_RAW for
those. For ping and traceroute the failure surfaces only as a wrapped socket
error at the moment of use.

**No unprivileged fallback.** Linux offers unprivileged ICMP through `SOCK_DGRAM`
on `udp4` and `udp6` when the caller's group falls inside `ping_group_range`. Ze
reads neither and never opens that socket type.
`internal/core/privilege/check_linux.go` `CheckPrivileges` does parse `CapEff`
for CAP_NET_RAW, but it prints warnings at startup and gates nothing.

This is masked today because the gokrazy appliance runs everything as root:
`gokrazy/ze/config.json` sets no user, group or capability, and `dropPrivileges`
is a no-op when `ze.user` is empty. It stops being masked the moment Ze drops
privileges or runs outside the appliance.

Goal: register a doctor check for the ICMP probe dependency with its own
diagnostic code, and fall back to unprivileged `SOCK_DGRAM` ICMP when the raw
socket is unavailable and the kernel permits it.

Provenance: VyOS T9002 granted CAP_NET_RAW to blackbox-exporter for the same
underlying reason, because its exporter user could not create ICMP sockets under
a restricted `ping_group_range`.
