...
tagged pair proves it. `ai/rules/exact-or-reject.md` makes the refusal the correct answer.
...
| Where | What it does |
|-------|--------------|
| `vendor/github.com/vishvananda/netlink/nl/xfrm_linux.go:189-203` | `XfrmSelector` declares `Dport`, `DportMask`, `Sport` and `SportMask`, all big endian |
| `vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:82-96` | the public `XfrmPolicy` declares `DstPort` and `SrcPort`, and NO mask field of any kind |
| `vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:117-124` | `selFromPolicy` writes the ports, then sets `DportMask` to `^uint16(0)` only when `Dport` is non-zero, and `SportMask` to `^uint16(0)` only when `Sport` is non-zero |
| `vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:318-333` | `parseXfrmPolicy` reads `Dport`, `Sport`, `Proto` and `Ifindex` back into the public struct, and reads NEITHER mask |
...
Step one. `RFC7296-3.13.1-3` lands in `plan/spec-rfcgate-1b-rfc7296-pilot.md` as
...
### Architecture Docs
- [ ] `ai/rules/exact-or-reject.md` - why the refusal is correct today
...
- [ ] `ai/rules/qemu-testing.md` - Linux-only code needs a real kernel
  ...
  → Constraint: a package carrying `//go:build integration && linux` is auto-enrolled in
    `ZE_QEMU_INTEGRATION_PKGS` (`mk/test-integration.mk:523`). No new make target is needed
    for a test placed in `internal/component/ike/dataplane/`.
- [ ] `ai/rules/no-fabrication.md` - a foreign system's semantics come from its source
...
- [ ] `vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:103-126` - `selFromPolicy`,
  ...
  `vendor/github.com/vishvananda/netlink/xfrm_state_linux.go:694` (the STATE selector,
  `x->sel`).
- [ ] `vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:152-199` -
  ...
  it.
- [ ] `vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:282-300` -
...
- [ ] `vendor/github.com/vishvananda/netlink/nl/xfrm_linux.go:189-203` - the kernel
  selector. `XfrmUserpolicyInfo` (`nl/xfrm_policy_linux.go:56-67`) embeds it.
- [ ] `internal/component/ike/dataplane/xfrm_linux.go:188-218` - `xfrmSelectorPort`. It
  accepts only mask 0 and mask 0xffff, and it REFUSES port 0 with mask 0xffff. Its comment
  names `selFromPolicy` as the reason.
- [ ] `internal/component/ike/dataplane/xfrm_linux.go:149-186` - `xfrmPolicyFromParams`, the
  single builder shared by install and delete. `InstallPolicy` (`:105`),
  `RemovePolicyParams` (`:128`) and `RemovePolicy` (`:116`) are its callers.
- [ ] `internal/component/ike/dataplane/dataplane.go:208-231` - `PortMatch`, a value plus a
...
- [ ] `internal/component/ike/dataplane/dataplane.go:243-252` - `SPParams.SrcPort` and
  `.DstPort`, the seam between the IKE engine and both dataplane backends.
- [ ] `internal/component/ike/engine/child.go:503-539` - `selectorPort`. It maps
...
- [ ] `internal/component/ike/dataplane/xfrm_transport_integration_linux_test.go` - the
  ...
  `TestXFRMOpaquePortIsRefused` (`:204`) pins the current refusal.
...
- [ ] `go.mod:22` and `vendor/modules.txt:320` - `github.com/vishvananda/netlink v1.3.1`, a
...
- The delete selector keeps matching the installed selector. The kernel identifies a policy
  by its whole selector (`internal/component/ike/dataplane/xfrm_linux.go:139-141`).
- Every caller of `netlink.XfrmPolicyAdd`, `XfrmPolicyDel` and `XfrmPolicyList` outside the
  ...
  `internal/plugins/iface/netlink/xfrm_linux.go:69`,
  `internal/plugins/ospf/doctor_ipsec_linux.go:14`, and the OSPF integration test at
  `internal/plugins/ospf/ipsec_integration_linux_test.go:105`.
- The OSPFv3 state selector of RFC 4552 keeps its current bytes
  (`internal/component/ike/dataplane/xfrm_linux.go:44-50`). It sets no port.
...
- Config: an operator sets `port opaque` on a traffic selector under an IPsec peer.
  `ipsec.ParsePortSelector` (`internal/component/ike/ipsec/traffic_selector.go:224-225`)
  already accepts the word.
- Wire: a peer proposes a traffic selector whose start port is 65535 and whose end port is
  0. `ipsec.PortSelector` parsing (`traffic_selector.go:87`) already recognizes the form.

### Transformation Path
1. Config parse or wire parse yields `ipsec.PortSelector{Form: PortOpaque}`.
2. Commit validation. `checkPortProgrammable`
   (`internal/component/ike/ipsec/traffic_selector.go:305`) refuses the form TODAY. After
   this spec it accepts it.
3. Negotiation. `programmableSelector` (`internal/component/ike/engine/ts_narrow.go:282`)
   drops the form TODAY. After this spec it keeps it.
4. Selector mapping. `selectorPort` (`internal/component/ike/engine/child.go:521`) returns
   `dataplane.ExactPortMatch(0)`. Unchanged.
5. Backend translation. `xfrmSelectorPort`
   (`internal/component/ike/dataplane/xfrm_linux.go:203`) refuses port 0 with a full mask
...
### Integration Points
- `dataplane.PortMatch` (`internal/component/ike/dataplane/dataplane.go:219`) needs no
...
- `nl.NewNetlinkRequest`, `NetlinkRequest.AddData`, `NetlinkRequest.Execute`,
  ...
  (`vendor/github.com/vishvananda/netlink/nl/nl_linux.go:678`, `:505`, `:519`,
  `nl/xfrm_policy_linux.go:56`, `nl/xfrm_linux.go:189`). A Ze-side wrapper is therefore
...
| 2. Ze-side wrapper | Build `XFRM_MSG_NEWPOLICY` and `XFRM_MSG_DELPOLICY` in Ze, through the exported `nl` package | Reproduce `xfrmPolicyAddOrUpdate` (`:152-199`) and `xfrmPolicyGetOrDelete` (`:282-300`): message header, policy info, template array, mark, if_id | Ze owns a second netlink encoder that must stay byte-compatible with the library one, forever. The install and the delete path must agree exactly, or a delete silently matches nothing |
...
(`internal/appliance/updater_hardening_markers_test.go:62`), which fails when the re-apply
...
| State path | `xfrm_state_linux.go:694` inherits the change through `selFromPolicy` | The state selector shares the derivation, so it shares the fix |
...
| Fact | Value | Evidence |
|------|-------|----------|
| Vendored version | `v1.3.1`, a direct dependency, marked `## explicit` | `go.mod:22`, `vendor/modules.txt:320-321` |
...
field rather than asserted. Sources: `nl/xfrm_linux.go:189-203` for the kernel struct,
`xfrm_policy_linux.go:82-96` for the public type, `:103-126` for the write derivation, and
`:318-333` for the read path.

| Kernel field | Public representative | How it is set | Lossless? |
|--------------|-----------------------|---------------|-----------|
| `Daddr` | `Dst.IP` | direct (`:107`) | yes |
| `Saddr` | `Src.IP` | direct (`:112`) | yes |
| `Dport` | `DstPort` | direct (`:117`) | yes |
| `DportMask` | **none** | DERIVED from `Dport` (`:119-121`) | **no** |
| `Sport` | `SrcPort` | direct (`:118`) | yes |
| `SportMask` | **none** | DERIVED from `Sport` (`:122-124`) | **no** |
| `Family` | none | DERIVED from `Dst.IP`, and it defaults to IPv4 when `Dst` is nil (`:104-106`) | only when `Dst` is set |
| `PrefixlenD` | `Dst.Mask` | DERIVED through `Mask.Size()` (`:108-109`) | only when the mask is well formed |
| `PrefixlenS` | `Src.Mask` | DERIVED through `Mask.Size()` (`:113-114`) | only when the mask is well formed |
| `Proto` | `Proto` | direct (`:116`) | yes |
| `Ifindex` | `Ifindex` | direct (`:125`) | yes |
...
(`internal/component/ike/engine/ts_narrow.go:286-289`) rejects a selector whose mask reports
zero ones and zero bits. `Family` defaults to IPv4 when `Dst` is nil, and Ze always sets
both `Src` and `Dst` (`internal/component/ike/dataplane/xfrm_linux.go:167-169`).
...
| A-3 | `vendor/` is regenerated only by an explicit `go mod vendor`, so a patched file survives a normal build | `scripts/dev/reapply-updater-fixes.py:4-10` states exactly this hazard and this cadence | Route 1 is unusable and route 2 becomes mandatory | Read the makefile targets that touch `vendor/`, and confirm the marker test catches a revert | unvalidated |
| A-4 | No caller outside the IKE dataplane sets a port on an `XfrmPolicy` | Measured 2026-08-01. The only other callers are `internal/plugins/iface/netlink/xfrm_linux.go:69`, `internal/plugins/ospf/doctor_ipsec_linux.go:14` and the OSPF integration test, and all three list rather than install | An additive patch changes bytes for a caller this spec did not consider | Re-grep at implementation time | unvalidated |
| A-5 | The VPP dataplane needs no change, because it programs nothing at all today | `plan/spec-fixit-vpp-ipsec-inoperable.md`, and `internal/component/ike/dataplane/vpp.go` | The VPP backend must express or refuse the opaque form in the same work (`ai/rules/design-context.md`) | Read `vpp.go` and the VPP IPsec binary API | unvalidated |
| A-6 | `TestXFRMOpaquePortIsRefused` (`xfrm_transport_integration_linux_test.go:204`) is the only test pinning the refusal | Measured 2026-08-01 by grep for `Opaque` under `internal/` | A second test pins the old behavior and reddens unexpectedly | Re-grep at implementation time | unvalidated |
...
| R-6 | Only the destination mask is fixed. An opaque SOURCE port still installs as any-port | An asymmetric selector protects more traffic in one direction | Both directions in one change. `xfrmSelectorPort` is already called for both sides (`xfrm_linux.go:159-166`) |
| R-7 | The state selector path (`xfrm_state_linux.go:694`) is forgotten. RFC 4552 OSPFv3 selectors then keep the old derivation | Nothing today, because Ze sets no port on a state selector | Note it in the patch and assert the shared derivation covers it. It is a latent hazard rather than a live defect |
...
| Who else touches this path? | `plan/spec-rfcgate-1b-rfc7296-pilot.md` (owns the `RFC7296-3.13.1-3` row and its tags), `plan/spec-ipsec-ipcomp.md` and `plan/spec-ipsec-auth-piggyback.md` (same engine), `plan/spec-fixit-vpp-ipsec-inoperable.md` (the other dataplane backend), and `internal/plugins/ospf/` (an RFC 4552 state selector) |

## Wiring Test (MANDATORY -- NOT deferrable)

| Entry Point | → | Feature Code | Test |
|-------------|---|--------------|------|
| A config file sets `port opaque` on a traffic selector and is committed | → | `checkPortProgrammable` (`internal/component/ike/ipsec/traffic_selector.go:305`) accepts the form | `test/ipsec/ipsec-ts-port-opaque.ci` |
| A peer proposes a traffic selector with start port 65535 and end port 0 | → | `programmableSelector` (`internal/component/ike/engine/ts_narrow.go:282`) keeps it | `TestOpaquePortSelectorIsNarrowedRatherThanDropped` |
| An established Child SA with an opaque selector installs its policy | → | `xfrmSelectorPort` and `xfrmPolicyFromParams` (`internal/component/ike/dataplane/xfrm_linux.go:203`, `:149`) | `TestXFRMOpaquePortReachesTheKernelExactly` (QEMU) |
| The same Child SA is torn down | → | `RemovePolicyParams` (`internal/component/ike/dataplane/xfrm_linux.go:128`) | `TestXFRMOpaquePortPolicyIsRemoved` (QEMU) |
...
(`internal/component/ike/engine/ts_narrow_test.go:277`) carries the `RFC7296-3.13.1-3`
...
### QEMU Integration Tests (MANDATORY - `ai/rules/qemu-testing.md`)
...
`internal/component/ike/dataplane/xfrm_transport_integration_linux_test.go:5-8`. The kernel
...
auto-enrolled in `ZE_QEMU_INTEGRATION_PKGS` (`mk/test-integration.mk:523`) and run under
`make ze-qemu-integration-test`. **No new make target is needed.** The VM installs
`iproute2` (`mk/test-integration.mk:536`), so `ip xfrm policy show` is available as an
...
## Files to Modify
- `internal/component/ike/dataplane/xfrm_linux.go` - `xfrmSelectorPort` (`:203`) stops
  refusing port 0, and `xfrmPolicyFromParams` (`:149`) carries the mask
...
- `internal/component/ike/dataplane/xfrm_transport_integration_linux_test.go` -
  `TestXFRMOpaquePortIsRefused` (`:204`) is replaced by the new proof. It pins behavior this
  spec deliberately changes, so it is a legitimate removal under `ai/rules/no-test-deletion.md`
- `internal/component/ike/dataplane/vpp.go` - express or refuse the opaque form
- `internal/component/ike/ipsec/traffic_selector.go` - `checkPortProgrammable` (`:305`)
- `internal/component/ike/engine/ts_narrow.go` - `programmableSelector` (`:282`)
...
- `docs/features/rfc-status.md:229` - the Remaining cell for RFC 7296 names the OPAQUE
...
|-------------------|----------|---------------|
| YANG schema (new RPCs/config) | No | `port opaque` already parses (`internal/component/ike/ipsec/traffic_selector.go:224`). No leaf is added |
...
| 9 | RFC behavior implemented, changed, or newly proven? | Yes | `docs/features/rfc-status.md:229`. Section 3.13.1 stops being a disclosed platform limit |
...
| Correctness: the state selector | `xfrm_state_linux.go:694` shares the derivation, and the change reaches it |
...
| Naming | The mask fields follow the kernel's own names. No Ze invention where a kernel name exists (`ai/rules/naming.md`) |
| Data flow | The mask travels from `PortMatch` to `nl.XfrmSelector` without a second representation |
| Rule: `ai/rules/exact-or-reject.md` | Every port form Ze accepts at commit is programmable exactly. A mask that is neither 0 nor 0xffff is still refused |
| Rule: `ai/rules/qemu-testing.md` | The kernel proof runs, and it is not replaced by a unit test with a fake backend |
| Rule: `ai/rules/no-fabrication.md` | A-1 is confirmed against the kernel source and not against a comment |
...
- Ze already models the kernel correctly. `dataplane.PortMatch`
  (`internal/component/ike/dataplane/dataplane.go:219`) carries a value AND a mask, and its
...
- The read path is the half a reader loses. `parseXfrmPolicy`
  (`vendor/github.com/vishvananda/netlink/xfrm_policy_linux.go:318-333`) restores four
...
| Relax the two refusals LAST | Relax them first so the config surface works while the backend catches up | Accepting a config Ze cannot program is exactly the failure `ai/rules/exact-or-reject.md` names |
