...
|------|--------|-----------------|---------|
| `labs/l2tp-interop/` | Ze as LNS against real `xl2tpd`/`pppd` LAC; FRR proves BGP redistribution of a subscriber /32 from a live PPP session | `docs/architecture/testing/l2tp-interop.md`, `test/l2tp-interop/lab.py` | `make ze-deployment-docker-l2tp-ppp-test` (Docker); `make ze-qemu-l2tp-ppp-test` (QEMU, needs `tmp/kernel/vmlinuz`, `mk/test-integration.mk`) |
