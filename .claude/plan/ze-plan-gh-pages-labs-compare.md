...
|------|--------|-----------------|---------|
| `labs/l2tp-interop/` | Ze as LNS against real `xl2tpd`/`pppd` LAC; FRR proves BGP redistribution of a subscriber /32 from a live PPP session | `docs/architecture/testing/l2tp-interop.md`, `test/l2tp-interop/lab.py` | `make ze-deployment-l2tp-ppp-docker-test` (Docker); `make ze-qemu-l2tp-ppp-test` (QEMU, needs `tmp/kernel/vmlinuz`, `mk/test-integration.mk:320-323`) |
| `labs/pppoe-interop/` | Ze's PPPoE client against real `accel-ppp`, "the dominant open-source BRAS/AC"; explains why L2TP-vs-accel-ppp isn't buildable (both LNS-only) | `docs/architecture/testing/pppoe-interop.md` | `make ze-deployment-pppoe-accel-docker-test`; `make ze-qemu-pppoe-accel-test` (`mk/test-integration.mk:334-340`) |
| `labs/ipsec-interop/` | Ze as IKE initiator against real strongSwan/charon, FRR redistribute scenarios | `test/ipsec-interop/lab.py` docstring, `Makefile:467` (no long-form design doc exists for this one -- page stays modest, doesn't invent rationale beyond what's written) | `make ze-ipsec-interop-test` (`IPSEC_INTEROP_SCENARIO=name` optional) |
...
| `labs/appliance-install/` | The installer boots and completes for real in QEMU across HTTP/PXE, ISO, and Ventoy-on-FAT paths, plus failure-path/fault/pin/rescue scenarios | Docstrings of `scripts/evidence/effective-install-qemu.py`, `-iso-qemu.py`, `-ventoy-qemu.py`, `-scenarios-qemu.py`; `docs/guide/ze-install.md` for the operator-supplied-kernel requirement | `make ze-install-qemu-test`, `ze-install-iso-qemu-test`, `ze-install-ventoy-qemu-test`, `ze-install-scenarios-qemu-test` (`mk/test-integration.mk:282-317`) |
| `labs/vpp-dataplane/` | Ze programs FIB, traffic, and firewall into a real VPP daemon via GoVPP; backs the production performance numbers already quoted in `docs/guide/vpp.md` (IPng Networks AS8298) | `scripts/evidence/effective-vpp.py` docstring, `docs/guide/vpp.md` | `make ze-deployment-vpp-test` (`mk/test-integration.mk:96-98`) |
...
1. Fix the mislabeled link-list entry (`:2345-2351`): instead of pointing
   ...
   correct link elsewhere in the Core cards grid (`:1266-1271`), so nothing
   is lost.
...
- All 39 Features cards tagged, verified per-card against
  `docs/features.md:42` (self-update backend differs but is supported on
