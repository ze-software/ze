...
|------|--------|-----------------|---------|
| `labs/l2tp-interop/` | Ze as LNS against real `xl2tpd`/`pppd`; FRR proves BGP redistribution of a subscriber /32 from a live PPP session | `docs/architecture/testing/l2tp-interop.md`, `internal/le/interoplab/l2tp`, `internal/le/test/deployment` | `./le test deployment docker-l2tp-ppp-test`; `./le test deployment gokrazy-l2tp-ppp-test` |
...
| `labs/appliance-install/` | The installer boots and completes in QEMU across HTTP/PXE, ISO, Ventoy-on-FAT, and failure scenarios | `internal/le/test/qemu/install.go`, `install_iso.go`, `install_ventoy.go`, `install_scenarios.go`; `docs/guide/ze-install.md` | `./le test qemu install-test`, `install-iso-test`, `install-ventoy-test`, `install-scenarios-test` |
| `labs/vpp-dataplane/` | Ze programs FIB, traffic, and firewall into a real VPP daemon via GoVPP | `internal/le/test/deployment/vppevidence.go`, `docs/guide/vpp.md` | `./le test deployment vpp-test` |
