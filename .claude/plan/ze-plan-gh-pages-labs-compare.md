...
|------|--------|-----------------|---------|
| `labs/l2tp-interop/` | Ze as LNS against real `xl2tpd`/`pppd` LAC; FRR proves BGP redistribution of a subscriber /32 from a live PPP session | `docs/architecture/testing/l2tp-interop.md`, `test/l2tp-interop/lab.py` | `make ze-deployment-l2tp-ppp-docker-test` (Docker); `make ze-qemu-l2tp-ppp-test` (QEMU, needs `tmp/kernel/vmlinuz`, `mk/test-integration.mk`) |
| `labs/pppoe-interop/` | Ze's PPPoE client against real `accel-ppp`, "the dominant open-source BRAS/AC"; explains why L2TP-vs-accel-ppp isn't buildable (both LNS-only) | `docs/architecture/testing/pppoe-interop.md` | `make ze-deployment-pppoe-accel-docker-test`; `make ze-qemu-pppoe-accel-test` (`mk/test-integration.mk`) |
