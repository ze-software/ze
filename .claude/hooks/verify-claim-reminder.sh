...
# start does not. Full rule: ai/rules/evidence.md.

cat >/dev/null  # consume the prompt payload on stdin

echo "Reminder: verify a claim about code by reading the function that PRODUCES it, not the caller. Unread means unverified. Cite file + symbol, not a line number. Report the conclusion, not the search. (evidence.md, writing.md)"
