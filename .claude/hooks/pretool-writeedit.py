...
# `none -- <why>`.
# `make ze-rules-gate-map-report` joins these against the points on disk: it reports
...
def std_content(ti):
    ...
    if c is None and isinstance(ti.get('edits'), list):
        c = '\n'.join(
            edit.get('new_string') or '' for edit in ti['edits'] if isinstance(edit, dict)
        )
    return c or ''
...
_SID_MODULE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'lib', 'session_id.py')
...
_scratch_spec = importlib.util.spec_from_file_location('ze_scratch_path', _SCRATCH_MODULE_PATH)
...
def _load_rfc_scope():
    ...
    """
    try:
        spec = importlib.util.spec_from_file_location('ze_rfc_tagged_scope', _SCOPE_MODULE_PATH)
...
def c_and_functions(ctx):
    if not _go_we(ctx) or re.search(r'_test\.go$', ctx['fp']):
        return None
    hits = grep_lines(ctx['content'], r'^func[ \t]+(\([^)]+\)[ \t]+)?[A-Z][a-zA-Z]*And[A-Z]')
    if hits:
        names = []
        for n, l in hits[:4]:
            m = re.search(r'func[ \t]+(?:\([^)]+\)[ \t]+)?([A-Z][a-zA-Z]*And[A-Z][a-zA-Z]*)', l)
...
def c_os_exit(ctx):
    ...
    if (
        ...
        return None
    hits = filter_out(grep_lines(ctx['content'], r'os\.Exit\('), r'//.*os\.Exit', ignorecase=True)
    if hits:
...
def c_encoding_alloc(ctx):
    ...
    for n, l in a_hits[:2]:
        reasons.append(f'  L{n}: append() -> use caller-owned buffer with WriteTo(buf, off)')
    m_hits = filter_out(
    ...
    for n, l in bf_hits[:2]:
        reasons.append(f'  L{n}: build*() returning []byte -> use WriteTo(buf, off) pattern')
    len_hits = filter_out(
    ...
    if len_hits:
        ...
        if wa:
            for n, l in len_hits[:2]:
                reasons.append(f'  L{n}: .Len() with WriteAttrTo -> use WriteAttrToWithLen')
    if reasons:
        detail = '\n'.join(reasons[:6])
        fix = (
...
def c_silent_ignore(ctx):
    ...
    for p in [
        ...
        if m and not any(
            re.search(r'(forbidden|wrong|bad|dont|do not)', l, re.IGNORECASE) for _, l in m
        ):
            errs = True
    # empty default: case
    lines = content.split('\n')
...
def c_lint_exclusions(ctx):
    ...
    if ctx['tool'] not in ('Write', 'Edit'):
        return None
    if not re.search(r'\.golangci', fp) and 'golangci.yml' not in fp and 'golangci.yaml' not in fp:
        return None
    content = ctx['content']
    errs = False
    if grep_any(content, r'exclude-rules:|exclude:|issues-exclude:|skip-files:|skip-dirs:'):
...
def c_version_config(ctx):
    ...
    if ctx['tool'] not in ('Write', 'Edit'):
        return None
    if '/config/' not in fp and not fp.endswith('.conf') and not re.search(r'config.*\.go$', fp):
        return None
    for p in [
        r'version[ \t]*[=:][ \t]*[0-9]',
        r'Version[ \t]*[=:][ \t]*[0-9]',
...
def c_fake_bufhandle(ctx):
    ...
    bad = [
        (n, l)
        for (n, l) in bad
        if not re.match(r'^[ \t]*[0-9]*:[ \t]*//', f'{n}:{l}') and 'noPoolBufID' not in l
    ]
...
def c_generated_files(ctx):
    ...
    given = ctx['fp'] if os.path.isabs(ctx['fp']) else os.path.join(PROJECT_DIR, ctx['fp'])
...
_RULES_ARTIFACTS = {
    'INDEX.md': ('scripts/dev/rules_index.py', 'make ze-rules-index-update'),
    'TRIGGERS.md': ('scripts/dev/rules_condensed.py', 'make ze-rules-condensed-update'),
    'CORE.md': ('scripts/dev/rules_condensed.py', 'make ze-rules-condensed-update'),
}
...
def c_rendered_rules(ctx):
    ...
    `make ze-rules-render-update` and silently lost. The three all-caps artifacts beside
    ...
    if base in _RULES_ARTIFACTS:
        ...
        fix = (
            ...
            f'\n    make ze-rules-render-update && {target}'
    ...
    if stem.isupper():
        fix = (
            ...
            '\n  Find the generator that owns it before editing; run `make ze-generated-files-update`'
    ...
    fix = (
        ...
        '\n  Then run: make ze-rules-render-update'
...
def c_point_overwrite(ctx):
    ...
    runs afterwards: `make ze-rules-render-update` reports the manifest duplicate, and
    ...
    if tool == 'Write':
        ...
        replaces = any(not (e or {}).get('old_string') for e in (ctx['ti'].get('edits') or []))
    else:
        replaces = False
    if not replaces:
        return None
    ...
    try:
        given = fp if os.path.isabs(fp) else os.path.join(PROJECT_DIR, fp)
        resolved = os.path.realpath(given)
        points_dir = os.path.realpath(os.path.join(PROJECT_DIR, 'ai', 'rules', 'points'))
    ...
    if len(tail) == 2 and tail[1] == 'manifest.md':
        ...
        what = 'the sections, the reading order, the title, the trigger and the severity'
    elif len(tail) == 3:
...
def _strip_point_fences(text):
    ...
    for line in text.splitlines(keepends=True):
        ...
        if fence_char:
            ...
            if indent <= 3 and len(marker) >= fence_length and marker == fence_char * len(marker):
                fence_char = ''
                fence_length = 0
            continue
...
def c_throwaway_tests(ctx):
    ...
    if re.search(r'^/tmp/', fp) or re.search(r'^/var/tmp/', fp):
        if re.search(r'\.(go|py|sh)$', fp):
            errs = True
    if re.search(r'test_.*\.(go|py|sh)$', fp) or re.search(r'_test_.*\.(go|py|sh)$', fp):
        if (
...
def c_enforce_naming(ctx):
    ...
    kind = ''
    if fp.endswith('.md'):
        if re.match(r'^(README|INDEX|CLAUDE|LICENSE|CONTRIBUTING|CHANGELOG)\.md$', base):
            return None
...
def c_design_without_lsp(ctx):
    ...
    if kinds:
        ...
        stale = [
            k for k in sorted(kinds) if evidence.get(k) is not None and now - evidence[k] > fresh
        ]
        if missing or stale:
...
def c_check_existing_patterns(ctx):
    ...
    if ctx['tool'] != 'Write':
        return None
    if not re.search(r'^.*/internal/.*\.go$', fp) or isfile(fp) or re.search(r'_test\.go$', fp):
        return None
    ...
    funcs = [m for m in re.findall(r'^func[ \t]+([A-Z][a-zA-Z0-9]*)\(', content, re.MULTILINE)][:5]
    go_files = [
...
def _test_weakening_errs(old, new, fp):
    ...
    if fp.endswith(('.ci', '.et')):
        ...
        if old_rej > new_rej:
            soft.append(f'removing negative expectations ({old_rej} -> {new_rej} reject=)')
    ...
    if new_taut > old_taut:
        errs.append(f'introducing an assertion that cannot fail ({old_taut} -> {new_taut})')
    return errs, soft
...
_GO_IMPORT_LINE = re.compile(r'^(?:import\s*\(|\)|import\s+)?(?:[A-Za-z_.][\w.]*\s+)?"[^"]+"$')
...
def _behavior_bytes(text, fp):
    ...
    stripped = _CI_LINE_COMMENT.sub('', text) if hash_comments else _GO_LINE_COMMENT.sub('', text)
...
def _edited_file_pair(fp, tool, ti, old, new):
    ...
    for e in edits:
        ...
        if not hunk or hunk not in after:
            return old, new
        after = after.replace(hunk, e.get('new_string', ''), -1 if e.get('replace_all') else 1)
    return text, after
...
def _weakened_hatch(fp, tool, ti, old, new):
    ...
    if mod is None:
        ...
        return (
            [],
            ['scripts/dev/check_weakened_tests.py did not import, so the hatch could not be read'],
            'test/weakened.md',
        )
    ...
    missing = mod.rows_missing(rows, package, _weakened_names(mod, fp, file_old, file_new))
...
def c_test_weakening(ctx):
    ...
    is_test = bool(re.search(r'_test\.go$', fp) or (fp.endswith(('.ci', '.et')) and '/test/' in fp))
    ...
    tags = _rfc_tagged_change_err(old, new, fp, tag_scope=_enclosing_tagged_scope(fp, hunks))
    if tags:
...
def c_ci_sleep_justification(ctx):
    ...
    # time.sleep( in a .ci functional test must carry `# sleep(<kind>): <reason>`
    # on the line above it or trailing it. A blind sleep hides real races and
    # hides why it was left.
    #
    # It BLOCKS rather than warns, and the marker is what made that safe. The old
    # check accepted any `#`, so an Edit fragment that did not carry the comment
    # already sitting above the sleep read as unjustified, and a block would have
    # been a false refusal. A marker is written WITH the sleep, in the same
    # fragment, so its absence is the author's omission rather than the diff's.
    fp = ctx['fp']
    ...
    if tool == 'Write':
        ...
        new = '\n'.join((e.get('new_string') or '') for e in (ctx['ti'].get('edits') or []))
    elif tool == 'Edit':
    ...
    for i, line in enumerate(lines):
        ...
        if _SLEEP_MARKER.search(after):
            continue  # marker trailing the call on the same line
        prev = lines[i - 1].strip() if i > 0 else ''
        if _SLEEP_MARKER.search(prev):
            continue  # marker on the line directly above
        bad.append((i + 1, line.strip()))
    if not bad:
        return None
    detail = '\n'.join(f'  +{n}: {l}' for n, l in bad[:4])
    fix = (
        '\n  A sleep is what remains when you cannot NAME the thing you are waiting\n'
        '  for, so try a synchronisation primitive first. test/scripts/ze_api.py\n'
        '  offers wait_until, wait_for_output, wait_for_event, dispatch_until,\n'
        '  wait_for_daemon_ready, wait_for_shutdown, wait_peer_counter and more.\n'
        '  wait_until takes a PREDICATE, so it converts any readback the test can\n'
        '  already perform, and its own internal sleep is exempt from the ratchet.\n'
        '\n  If none of them fits, declare which kind of sleep this is, on the line\n'
        '  above it or trailing it:\n'
        '      # sleep(<kind>): <reason>\n'
        f'  kind is one of: {", ".join(_SLEEP_KINDS)}\n'
        '  A timer, timeout-under-test or no-signal reason MUST name the mechanism\n'
        '  and where its period is set, so a later reader can check it.\n'
        '  Rule: the testing rule, "CI Sleep Justification".'
    )
    return (
        2,
        f'{RED}{BOLD}BLOCKED: time.sleep( with no sleep(<kind>) marker in {fp}{RESET}\n{detail}{fix}',
    )
...
# `RFC requirement:` tags on every `make ze-rfc-index-update`. One such file exists per
...
def main():
    ...
    for check in CHECKS:
        try:
            ...
            sys.stderr.write(f'[pretool-writeedit] {check.__name__} errored (failing open):\n')
            traceback.print_exc()
            continue
        if r is None:
            continue
