...
# --- pre-commit-spec-audit.sh ---------------------------------------------- #


def _sed_range(lines, start_re, end_re):
    """Reproduce `sed -n '/start/,/end/p'` for a single range (boundaries inclusive)."""
    out = []
    started = False
    for line in lines:
        if not started:
...
def _count_rows(section, excludes):
    """Count `| ... |` table rows excluding header/separator (grep -E '^\\|' | grep -vE ...)."""
    n = 0
    for line in section:
...
def check_spec_audit(cmd, ctx):
    """pre-commit-spec-audit.sh: verify the selected spec's obligations are met."""
    if "git commit" not in cmd:
        return None
    sel = os.path.join(ctx["dir"], "tmp/session/selected-spec")
    if not os.path.isfile(sel):
        return None
    with open(sel, encoding="utf-8", errors="replace") as fh:
        names = [
            l
    ...
    if not names:
        return None
    spec_name = re.sub(r"\s", "", names[-1])
    if not spec_name:
        return None
    spec_file = os.path.join(ctx["dir"], "plan", spec_name)
    if not os.path.isfile(spec_file):
        return None
    with open(spec_file, encoding="utf-8", errors="replace") as fh:
        text = fh.read()
    lines = text.splitlines()
    errors = []

    def missing_ci(section, label):
        for ci in sorted(
            set(re.findall(r"test/[a-zA-Z0-9_./-]+\.ci", "\n".join(section)))
        ):
            if not os.path.isfile(os.path.join(ctx["dir"], ci)):
                errors.append(f"{label}: '{ci}' does not exist on disk")

    # 1 + 2: Wiring Test / Functional Tests .ci files exist.
    missing_ci(_sed_range(lines, r"^## Wiring Test", r"^## [^W]"), "Wiring Test")
    missing_ci(
        _sed_range(lines, r"^### Functional Tests", r"^###|^## "), "Functional Tests"
    )

    # 3: Files to Create all exist.
    create = _sed_range(lines, r"^## Files to Create", r"^## ")
    for m in sorted(
        set(re.findall(r"`([a-zA-Z0-9_./-]+\.(?:go|ci|yang|md))`", "\n".join(create)))
    ):
        if not os.path.isfile(os.path.join(ctx["dir"], m)):
            errors.append(f"Files to Create: '{m}' does not exist on disk")

    # 4: TDD Plan test files exist.
    tdd = _sed_range(lines, r"^### Unit Tests", r"^### Boundary|^### Functional|^## ")
    for m in sorted(set(re.findall(r"`([a-zA-Z0-9_./-]+_test\.go)`", "\n".join(tdd)))):
        if not os.path.isfile(os.path.join(ctx["dir"], m)):
            errors.append(f"TDD Plan: test file '{m}' does not exist on disk")

    # 5: Implementation Audit tables have data rows.
    audit = _sed_range(
        lines, r"^## Implementation Audit", r"^## Pre-Commit|^## Checklist"
    )
    if (
        _count_rows(
            _sed_range(audit, r"^### Requirements from Task", r"^### "),
            [r"^\|.*Requirement", r"^\|.*---"],
        )
        < 1
    ):
        errors.append("Audit: 'Requirements from Task' table is empty")
    ac_table = _sed_range(audit, r"^### Acceptance Criteria", r"^### ")
    ac_rows = _count_rows(ac_table, [r"^\|.*AC ID", r"^\|.*---"])
    if ac_rows < 1:
        errors.append("Audit: 'Acceptance Criteria' table is empty")

    # 6: Every AC row has non-empty "Demonstrated By".
    if ac_rows > 0:
        empty = 0
        for row in [l for l in ac_table if l.startswith("|")]:
            if re.search(r"^\|.*AC ID", row) or re.search(r"^\|.*---", row):
                continue
            fields = row.split("|")
            evidence = fields[3].strip() if len(fields) > 3 else ""
            if evidence == "":
                ac_id = fields[1].strip() if len(fields) > 1 else ""
                errors.append(
                    f"Audit: {ac_id} has empty 'Demonstrated By' — every AC needs evidence"
                )
                empty += 1
                if empty >= 3:
                    errors.append("Audit: ... and more ACs with empty evidence")
                    break
    if (
        _count_rows(
            _sed_range(audit, r"^### Tests from TDD Plan", r"^### "),
            [r"^\|.*Test ", r"^\|.*---"],
        )
        < 1
    ):
        errors.append("Audit: 'Tests from TDD Plan' table is empty")
    if (
        _count_rows(
            _sed_range(audit, r"^### Files from Plan", r"^### "),
            [r"^\|.*File ", r"^\|.*---"],
        )
        < 1
    ):
        errors.append("Audit: 'Files from Plan' table is empty")
    summary = _sed_range(audit, r"^### Audit Summary", r"^## |^### ")
    if any(re.search(r"Total items:\s*$", l) for l in summary):
        errors.append("Audit: Audit Summary has no totals")

    # 7: Pre-Commit Verification section exists and is filled.
    verify = _sed_range(lines, r"^## Pre-Commit Verification", r"^## Checklist")
    if not verify:
        errors.append(
            "Missing '## Pre-Commit Verification' section — add it from TEMPLATE.md"
        )
    else:
        if (
            _count_rows(
                _sed_range(verify, r"^### Files Exist", r"^### "),
                [r"^\|.*File ", r"^\|.*Exists", r"^\|.*---"],
            )
            < 1
        ):
            errors.append(
                "Pre-Commit Verification: 'Files Exist' table is empty — verify files with ls"
            )
        if (
            _count_rows(
                _sed_range(verify, r"^### AC Verified", r"^### "),
                [r"^\|.*AC ID", r"^\|.*Claim", r"^\|.*---"],
            )
            < 1
        ):
            errors.append(
                "Pre-Commit Verification: 'AC Verified' table is empty — re-verify each AC independently"
            )
        if (
            _count_rows(
                _sed_range(verify, r"^### Wiring Verified", r"^### |^## "),
                [r"^\|.*Entry Point", r"^\|.*Verified", r"^\|.*---"],
            )
            < 1
        ):
            errors.append(
                "Pre-Commit Verification: 'Wiring Verified' table is empty — verify each wiring path"
            )

    # 8: Spec references at least one .ci functional test.
    if not re.findall(r"test/[a-zA-Z0-9_./-]+\.ci", text):
        errors.append(
            "Spec has ZERO .ci functional test references — features without end-to-end tests are dead code (see retrospective: deferred functional tests never land)"
        )

    # 9: Learned summary exists and does not claim incompleteness.
    base = re.sub(
        r"^spec-",
        "",
        os.path.basename(spec_file)[:-3]
        if spec_file.endswith(".md")
        else os.path.basename(spec_file),
    )
    learned_dir = os.path.join(ctx["dir"], "plan/learned")
    learned = (
        sorted(f for f in os.listdir(learned_dir) if base in f and f.endswith(".md"))
        if os.path.isdir(learned_dir)
        else []
    )
    if not learned:
        errors.append(f"No learned summary for '{base}' in plan/learned/")
    else:
        with open(
            os.path.join(learned_dir, learned[0]), encoding="utf-8", errors="replace"
        ) as fh:
            ltext = fh.read()
        if re.search(
            r"not yet implemented|not wired|infrastructure only|library only|not yet wired|wiring not.*implemented",
            ltext,
            re.IGNORECASE,
        ):
            errors.append(
                f"Learned summary '{learned[0]}' says work is incomplete — fix or finish the work"
            )

    if not errors:
        return None
    msg = [
        f"{RED}{BOLD}❌ Spec obligations not met — commit blocked ({len(errors)} issues):{RESET}"
    ]
    msg += [f"  {RED}✗{RESET} {e}" for e in errors]
    msg += [
        "",
        f"{YELLOW}Fix these, or clear tmp/session/selected-spec for unrelated commits.{RESET}",
        "See: rules/implementation-audit.md, rules/integration-completeness.md",
    ]
    return (2, "\n".join(msg))
