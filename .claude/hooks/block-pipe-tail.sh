...
# The git-safety.md rule says: capture to tmp/ze-test-SESSION.log
# Use: make ze-verify > tmp/ze-test-SESSION.log 2>&1
...
if [[ "$COMMAND" =~ make\ ze-.*\| ]]; then
    ...
    echo "  -- Use: make ze-verify > tmp/ze-test-SESSION.log 2>&1" >&2
    echo "  -- Then: grep -E 'FAIL|TEST FAILURE' tmp/ze-test-SESSION.log" >&2
    exit 2
fi
