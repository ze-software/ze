...
# Check if session-state.md exists and has docs listed
SESSION_STATE=".claude/session-state.md"
if [[ ! -f "$SESSION_STATE" ]]; then
    ERRORS+=("No session-state.md - cannot verify docs were read")
    ERRORS+=("→ Create from .claude/session-state.md.template")
    ERRORS+=("→ List docs read before writing spec")
fi
...
if [[ ${#WARNINGS[@]} -gt 0 ]]; then
    echo -e "${YELLOW}⚠️  Architecture docs may not be read:${RESET}" >&2
    for warn in "${WARNINGS[@]}"; do
        echo -e "  ${YELLOW}${warn}${RESET}" >&2
    done
    echo -e "  ${YELLOW}→ Add doc names to session-state.md after reading${RESET}" >&2
fi
