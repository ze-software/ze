...
if ! grep -q '| Status |' "$FILE_PATH"; then
    ...
    case "$SPEC_STATUS" in
        skeleton|design|ready|in-progress|verification|blocked|deferred|done) ;;
        *) ERRORS+=("Invalid Status '$SPEC_STATUS'. Must be: skeleton, design, ready, in-progress, verification, blocked, deferred, done") ;;
    esac
