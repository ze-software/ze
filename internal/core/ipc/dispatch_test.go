...
func TestRPCDispatchCodedError(t *testing.T) {
	...
	err := d.Register("ze-bgp:peer-teardown", func(_ string, _ json.RawMessage) (any, error) {
		return nil, rpc.NewCodedError("command-not-available", `command "show bgp rib" not available (plugin may not be running)`)
	})
	require.NoError(t, err)
	...
	require.NoError(t, json.Unmarshal(errResp.Params, &detail))
	assert.Equal(t, `command "show bgp rib" not available (plugin may not be running)`, detail.Message)
}
