...
// packages uses clock.Clock instead of direct time package calls.
//
// VALIDATES: All time.Now/After/Sleep/AfterFunc/NewTimer calls replaced with clock.Clock.
		...
		for _, entry := range entries {
			...
			for _, pattern := range forbidden {
				if strings.Contains(content, pattern) {
					t.Errorf("%s contains direct %s call — use clock.Clock instead", name, pattern)
				}
			}
		}
	}
...
// uses network.Dialer/network.ListenerFactory instead of direct net package calls.
//
// VALIDATES: All net.Listen/net.Dial calls replaced with network interfaces.
...
func TestNoDirectNetCalls(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, pattern := range forbidden {
			if strings.Contains(content, pattern) {
				t.Errorf("%s contains direct %s call — use network.Dialer/network.ListenerFactory instead", name, pattern)
			}
		}
	}
}
