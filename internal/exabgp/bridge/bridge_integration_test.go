...
func setupTestBinaries(t *testing.T) {
	...
	testSetupOnce.Do(func() {
		...
		testZeTestPath = filepath.Join(testTmpDir, "le")
		buildCmd = exec.CommandContext(ctx, "go", "build", "-tags", "ze_le", "-o", testZeTestPath, "./cmd/ze")
		buildCmd.Dir = projectRoot
		buildCmd.Env = append(os.Environ(), "CGO_ENABLED=0")
		buildOutput, err = buildCmd.CombinedOutput()
		if err != nil {
			testSetupErr = fmt.Errorf("build le: %w\n%s", err, buildOutput)
			return
		}
	})
...
func newBridgeTestHarness(t *testing.T, ctx context.Context, testMode string, flags ...string) *bridgeTestHarness {
	...
	args = append(args, testZeTestPath, "test", "interop-bgp", "exabgp-api", "echo")
...
func TestBridgeIntegration_RealPlugin(t *testing.T) {
	...
	bridgeCmd := exec.CommandContext(ctx, testZePath, "exabgp", "plugin",
		"--family", "ipv4/unicast", testZeTestPath, "test", "interop-bgp", "exabgp-api", "echo")
	...
	zebgpEvent := map[string]any{
		...
		"peer":    map[string]any{"remote": map[string]any{"address": "10.0.0.1", "as": 65001}},
		"origin":  "igp",
		"ipv4/unicast": []any{
...
func TestBridgeIntegration_StartupProtocol(t *testing.T) {
	...
	bridgeCmd := exec.CommandContext(ctx, testZePath, "exabgp", "plugin",
		...
		"--family", "ipv6/unicast",
		"--route-refresh",
		testZeTestPath, "test", "interop-bgp", "exabgp-api", "echo")
...
func TestBridgeIntegration_IPv6(t *testing.T) {
	...
	h.SendJSON(map[string]any{
		...
		"peer":    map[string]any{"remote": map[string]any{"address": "2001:db8::1", "as": 65001}},
		"origin":  "igp",
		"ipv6/unicast": []any{
