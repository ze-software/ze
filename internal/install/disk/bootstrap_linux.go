...
func bootstrap() {
	...
	os.MkdirAll("/proc", 0o755)                            //nolint:errcheck // PID-1 init; no filesystem to fail against yet
	...
	os.MkdirAll("/sys", 0o755)                            //nolint:errcheck // PID-1 init
	...
	os.MkdirAll("/dev", 0o755)                            //nolint:errcheck // PID-1 init
	if err := unix.Mount("none", "/dev", "devtmpfs", 0, ""); err != nil {
		os.Stdout.WriteString("[ze-install] WARNING: mount /dev failed\n") //nolint:errcheck // pre-slog
	}

	os.MkdirAll("/tmp", 0o1777)     //nolint:errcheck // PID-1 init; rootfs is writable
