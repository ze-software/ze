...
// Related: virtualclock.go — VirtualClock used as inner clock for ChaosClock
...
// ChaosClock wraps a clock.Clock with seed-driven fault injection.
...
func (d *ChaosDialer) DialContext(ctx context.Context, nw, address string) (net.Conn, error) {
	...
	conn, err := d.inner.DialContext(ctx, nw, address)
	if err != nil {
		return nil, err
	}
...
func (f *ChaosListenerFactory) Listen(ctx context.Context, nw, address string) (net.Listener, error) {
	if f.rng.shouldFault() {
		...
		return nil, fmt.Errorf("chaos: bind failure on %s %s", nw, address)
	}

	ln, err := f.inner.Listen(ctx, nw, address)
	if err != nil {
		return nil, err
	}
