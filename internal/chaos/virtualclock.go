...
// Related: chaos.go — chaos wrappers using VirtualClock
...
func (vc *VirtualClock) AfterFunc(d time.Duration, f func()) clock.Timer {
	vc.mu.Lock()
	defer vc.mu.Unlock()
...
func (vc *VirtualClock) NewTimer(d time.Duration) clock.Timer {
	vc.mu.Lock()
	defer vc.mu.Unlock()
...
func (vc *VirtualClock) NewTicker(d time.Duration) clock.Ticker {
	vc.mu.Lock()
	defer vc.mu.Unlock()
...
// virtualTimer implements clock.Timer backed by a timerEntry in the VirtualClock heap.
...
// virtualTicker implements clock.Ticker backed by VirtualClock's timer heap.
