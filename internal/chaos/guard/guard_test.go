...
func TestGuardAllowsChaosOnFreshPeer(t *testing.T) {
	...
	actions := []engine.ActionType{
		engine.ActionTCPDisconnect,
...
func TestGuardBlocksDuplicateHoldTimerExpiry(t *testing.T) {
	...
	ok, _ := g.AllowChaos(0, engine.ActionHoldTimerExpiry)
	assert.True(t, ok)
...
func TestGuardPeerIsolation(t *testing.T) {
	...
	// Peer 2 unaffected.
	ok, _ := g.AllowChaos(2, engine.ActionHoldTimerExpiry)
	assert.True(t, ok)
	...
	ok, _ = g.AllowChaos(1, engine.ActionHoldTimerExpiry)
	assert.True(t, ok)
