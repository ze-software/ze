...
func shortFileNameBoth(name string, seen map[string]bool) (primary, ext string) {
	...
	basis = strings.ReplaceAll(basis, " ", "")
	...
	if len(primary) < 8 {
		primary += strings.Repeat(" ", 8-len(primary))
	}
...
func (fw *Writer) Flush() error {
	...
	reservedSectors := fullClusters(int(sectorSize)) * int(sectorsPerCluster)
	...
	if err := fw.dataTmp.Close(); err != nil {
		return err
	}
