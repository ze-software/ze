...
//
// The generated headers name `./le ai rules condensed-update`, the sole producer
// of both artifacts.
...
var triggersHeader = []string{
	...
	"",
	"<!-- GENERATED by ./le ai rules condensed-update -- do not edit -->",
	"<!-- Regenerate: ./le ai rules condensed-update -->",
	"",
...
var coreHeader = []string{
	...
	"(`./le ai rules router-report` prints that set and the corpus it read).",
	"",
	...
	for _, artifact := range r.Artifacts {
		if artifact.Stale {
			tb.Str("WARNING: ").Str(artifact.File).
				Str(" is stale -- run: ./le ai rules condensed-update\n")
		}
	}
	return tb.String()
}
