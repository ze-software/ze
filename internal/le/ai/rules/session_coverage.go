...
//
// session_coverage.go ports internal/le/ai/rules/coverage.go. It measures one
...
func loadAlwaysOnRules(rulesDir string, errOut io.Writer) map[string]bool {
	...
	if len(members) == 0 {
		...
		_, _ = fmt.Fprintln(errOut, tb.Str("rule-coverage: ").Str(path).
			...
			Str("/<name>.md` line; excluding no always-on rule. The generator (internal/le/ai/rules.RenderArtifacts) has most likely changed shape").String())
	}
	return members
}
