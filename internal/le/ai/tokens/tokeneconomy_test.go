// Unit coverage for the ai tokens report.
//
...
func TestParseOptionsHoldsANumberToTheScriptsBounds(t *testing.T) {
	...
	for _, args := range accepted {
		if _, code := ParseOptions(args); code != 0 {
			t.Errorf("le ai tokens %v was refused with %d, and it is inside the bound", args, code)
		}
	}
	refused := [][]string{
	...
	for _, args := range refused {
		if _, code := ParseOptions(args); code == 0 {
			t.Errorf("le ai tokens %v was accepted, and it is outside the bound", args)
		}
	}
}
