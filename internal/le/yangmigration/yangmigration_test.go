...
func TestCommandsToPluginsPreviewApplyAndIdempotence(t *testing.T) {
	...
	source := filepath.Join(root, "internal/component/aaa/yang/ze-aaa-cmd.yang")
	...
	mustContain(t, filepath.Join(root, "internal/plugins/aaa-cmd/yang/ze-aaa-cmd.yang"), "module ze-aaa-cmd")
	mustContain(t, filepath.Join(root, "internal/component/cmd/show/yang/owner_test.go"), "internal/plugins/aaa-cmd/yang")
...
func TestCommandsToPluginsCoalescesIdenticalDestination(t *testing.T) {
	...
	mustNotExist(t, source)
	mustExist(t, filepath.Join(root, "internal/plugins/aaa-cmd/yang/ze-aaa-cmd.yang"))
}
...
func TestSchemaToYangPreviewApplyMergeAndIdempotence(t *testing.T) {
	...
	mustExist(t, filepath.Join(root, "internal/component/demo/schema/ze-demo.yang"))

	report, err := schemaToYang(root, true)
	if err != nil || report.Refused() {
		t.Fatalf("apply failed: report=%+v err=%v", report, err)
	}
	mustNotExist(t, filepath.Join(root, "internal/component/demo/schema"))
	mustContain(t, filepath.Join(root, "internal/component/demo/yang/model.go"), "package yang")
	mustContain(t, filepath.Join(root, "internal/consumer/use.go"), `demoyang "github.com/ze-software/ze/internal/component/demo/yang"`)
	mustContain(t, filepath.Join(root, "internal/consumer/use.go"), "demoyang.Value")
	mustContain(t, filepath.Join(root, "docs/tree.md"), "internal/component/demo/yang/ze-demo.yang")
	mustNotExist(t, filepath.Join(root, "internal/component/merged/schema"))
	mustExist(t, filepath.Join(root, "internal/component/merged/yang/ze-merged.yang"))
	mustNotExist(t, filepath.Join(root, "internal/component/merged/yang/embed.go"))
...
func TestPathRefactorRemovePreviewApplyAndIdempotence(t *testing.T) {
	...
	mustContain(t, filepath.Join(root, "test/path.ci"), "connection {")

	report, err := refactorPaths(root, op, true)
	if err != nil || report.Refused() {
		t.Fatalf("apply = %+v, err=%v", report, err)
	}
	mustNotContain(t, filepath.Join(root, "test/path.ci"), "connection {")
	mustContain(t, filepath.Join(root, "test/path.ci"), "remote {")
	mustContain(t, filepath.Join(root, "test/path.ci"), "--context bgp/peer/p1/remote")
	mustContain(t, filepath.Join(root, "test/path.et"), "set bgp peer p1 remote ip")
	mustContain(t, filepath.Join(root, "internal/path.go"), `peer.GetContainer("remote")`)
	mustContain(t, filepath.Join(root, "internal/path.go"), `terminal := peer.GetContainer("connection")`)
	mustContain(t, filepath.Join(root, "internal/model.yang"), `"remote/ip"`)
...
func TestPathRefactorRenameWorkflow(t *testing.T) {
	...
	mustContain(t, filepath.Join(root, "internal/path.go"), `GetContainer("protocol")`)
	mustContain(t, filepath.Join(root, "test/path.ci"), "protocol {")
}
...
func TestPathRefactorMoveWorkflow(t *testing.T) {
	...
	mustContain(t, filepath.Join(root, "internal/path.go"), `"bgp/peer/p1/capability/graceful-restart"`)
}
