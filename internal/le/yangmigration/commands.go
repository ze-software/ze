...
func commandsToPlugins(root string, apply bool) (Report, error) {
	...
	for _, owner := range intrinsicCommandOwners {
		paths, err := commandFiles(filepath.Join(root, "internal", "component", filepath.FromSlash(owner), "yang"), false)
		if err != nil {
			return report, err
		}
	...
	for _, owner := range commandOwners {
		sourceDir := filepath.Join(root, "internal", "component", filepath.FromSlash(owner.component), "yang")
		destinationDir := filepath.Join(root, "internal", "plugins", filepath.FromSlash(owner.plugin), "yang")
...
func commandFiles(directory string, tests bool) ([]string, error) {
	...
	for _, entry := range entries {
		...
		name := entry.Name()
		if tests {
			if filepath.Ext(name) == ".go" && strings.Contains(name, "cmd_schema") {
				paths = append(paths, filepath.Join(directory, name))
			}
			continue
		}
		if filepath.Ext(name) == ".yang" && strings.Contains(strings.TrimSuffix(name, ".yang"), "-cmd") {
			paths = append(paths, filepath.Join(directory, name))
		}
	}
	sort.Strings(paths)
