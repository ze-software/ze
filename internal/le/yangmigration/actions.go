...
var actions = leaction.New(area,
	leaction.Action{
		...
		Writes:     true,
		Parameters: []leaction.Parameter{{Keyword: "apply"}},
	...
	leaction.Action{
		...
		Parameters: []leaction.Parameter{
			...
			{Keyword: "under", Value: "path"},
			{Keyword: "source", Value: "path"},
			{Keyword: "destination", Value: "path"},
...
func runCommandsToPluginsHere(args leaction.Arguments) (any, int) {
	...
	report, err := commandsToPlugins(root, args.Has("apply"))
...
func runSchemaToYangHere(args leaction.Arguments) (any, int) {
	...
	report, err := schemaToYang(root, args.Has("apply"))
	return actionResult(report, err)
}
...
func runPathRefactorHere(args leaction.Arguments) (any, int) {
	...
	report, err := refactorPaths(root, operation, args.Has("apply"))
	if err != nil {
		leaction.ReportError(err)
		return report, 1
	}
