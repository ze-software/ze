...
func (s *claudeServerSetup) prepare() {
	...
	if s.Paths.Home == "" {
		s.Paths.Home = filepath.Join("/home", s.User)
	}
	if s.Paths.GoRoot == "" {
...
func (s *claudeServerSetup) setup() (*claudeServerReport, int, error) {
	...
	for _, prerequisite := range []string{"apt-get", "sudo"} {
	...
	repo := filepath.Join(s.Paths.Home, "Code/github.com/ze-software/ze/main")
...
func (s *claudeServerSetup) installSystem(report *claudeServerReport) error {
	...
	if err := s.run(report, "apt-get update", Cmd{Argv: []string{"apt-get", "update", "-qq"}}); err != nil {
		return err
	}
	argv := []string{"apt-get", "install", "-y", "-qq", "build-essential", "git", "curl", "wget", "jq", "unzip", "mosh"}
...
func (s *claudeServerSetup) configureFirewall(report *claudeServerReport) error {
	...
	if !s.Shell.Present("ufw") {
		report.add("Firewall", "skipped", "ufw not found, skipping firewall setup")
		return nil
	}
	for _, argv := range [][]string{{"ufw", "allow", "OpenSSH"}, {"ufw", "allow", "60000:61000/udp", "comment", "mosh"}} {
		if err := s.run(report, "ufw", Cmd{Argv: argv}); err != nil {
			return err
		}
	}
	status := s.Shell.Run(Cmd{Argv: []string{"ufw", "status"}})
	if !status.OK() {
	...
	if strings.Contains(strings.ToLower(firstLine(status.Out)), "status: active") {
		...
		if err := s.run(report, "ufw enable", Cmd{Argv: []string{"ufw", "enable"}, Stdin: []byte("y\n")}); err != nil {
			return err
		}
		report.add("Firewall", "installed", "ufw enabled")
	}
	return s.run(report, "ufw status", Cmd{Argv: []string{"ufw", "status"}})
}

func (s *claudeServerSetup) installGo(report *claudeServerReport) error {
	report.add("", "section", "--- Installing Go "+claudeServerGoVersion+" ---")
	goBinary := filepath.Join(s.Paths.GoRoot, "bin/go")
	version := s.Shell.Run(Cmd{Argv: []string{goBinary, "version"}})
	...
	installed := s.Shell.Run(Cmd{Argv: []string{goBinary, "version"}})
	if !installed.OK() {
...
func (s *claudeServerSetup) installGoTools(report *claudeServerReport) error {
	...
	if err := s.run(report, "create GOPATH", Cmd{Argv: []string{"sudo", "-u", s.User, "mkdir", "-p", filepath.Join(gopath, "bin")}}); err != nil {
		return err
	}
	...
	lint := s.Shell.Run(Cmd{Argv: []string{"golangci-lint", "--version"}, Env: environment})
	if lint.OK() {
		report.add("golangci-lint", "present", "already installed: "+firstLine(lint.Out))
	} else {
		target := "github.com/golangci/golangci-lint/v2/cmd/golangci-lint@" + GolangCIVersion
		if err := s.run(report, "install golangci-lint", Cmd{Argv: []string{"go", "install", target}, Env: environment}); err != nil {
			return err
		}
		if err := s.run(report, "own GOPATH", Cmd{Argv: []string{"chown", "-R", s.User + ":" + s.User, gopath}}); err != nil {
			return err
		}
		report.add("golangci-lint", "installed", GolangCIVersion)
	}
	goBinary := filepath.Join(s.Paths.GoRoot, "bin/go")
	staticcheck := []string{"sudo", "-u", s.User, "env", "CGO_ENABLED=0", goBinary, "install", staticcheckTarget}
	if err := s.run(report, "install staticcheck", Cmd{Argv: staticcheck}); err != nil {
		return err
	}
	result := s.Shell.Run(Cmd{Argv: []string{"staticcheck", "-version"}, Env: environment})
	if !result.OK() {
		return fmt.Errorf("verify staticcheck: %s", result.complaint())
	}
	report.add("staticcheck", "installed", strings.TrimSpace(result.Out))
	goimports := s.Shell.Run(Cmd{Argv: []string{"goimports", "--help"}, Env: environment})
	if goimports.OK() {
		report.add("goimports", "present", "already installed")
		return nil
	}
	argv := []string{"sudo", "-u", s.User, "env", "CGO_ENABLED=0", goBinary, "install", goimportsTarget}
	if err := s.run(report, "install goimports", Cmd{Argv: argv}); err != nil {
		return err
	}
	report.add("goimports", "installed", "installed")
	return nil
}
...
func (s *claudeServerSetup) installNode(report *claudeServerReport) error {
	...
	version := s.Shell.Run(Cmd{Argv: []string{"node", "--version"}})
	...
	if err := s.run(report, "apt-get update for NodeSource", Cmd{Argv: []string{"apt-get", "update", "-qq"}}); err != nil {
		return err
	}
	if err := s.run(report, "install nodejs", Cmd{Argv: []string{"apt-get", "install", "-y", "-qq", "nodejs"}}); err != nil {
		return err
	}
	installed := s.Shell.Run(Cmd{Argv: []string{"node", "--version"}})
	if !installed.OK() {
...
func (s *claudeServerSetup) installClaude(report *claudeServerReport) error {
	...
	version := s.Shell.Run(Cmd{Argv: []string{"claude", "--version"}})
	if version.OK() {
		report.add("Claude CLI", "present", "already installed, skipping")
		return nil
	}
	if err := s.run(report, "install Claude CLI", Cmd{Argv: []string{"npm", "install", "-g", "@anthropic-ai/claude-code"}}); err != nil {
		return err
	}
	installed := s.Shell.Run(Cmd{Argv: []string{"claude", "--version"}})
	detail := "ok"
...
func (s *claudeServerSetup) cloneRepository(report *claudeServerReport) error {
	...
	if err := s.run(report, "create repository directory", Cmd{Argv: []string{"sudo", "-u", s.User, "mkdir", "-p", parent}}); err != nil {
		return err
	}
	clone := s.Shell.Run(Cmd{Argv: []string{"sudo", "-u", s.User, "git", "clone", claudeServerRepo, repo}})
	if clone.OK() {
...
func (s *claudeServerSetup) verify(report *claudeServerReport) {
	...
	checks := []struct {
		...
		{"Go", []string{filepath.Join(s.Paths.GoRoot, "bin/go"), "version"}},
		{"Node", []string{"node", "--version"}},
		{"npm", []string{"npm", "--version"}},
		{"Claude", []string{"claude", "--version"}},
		{"Lint", []string{"golangci-lint", "--version"}},
		{"Types", []string{"staticcheck", "-version"}},
		{"Imports", []string{"goimports", "--help"}},
		{"Git", []string{"git", "--version"}},
		{"Mosh", []string{"mosh-server", "--version"}},
		{"Firewall", []string{"ufw", "status"}},
	}
...
func serverGoVersion(output string) string {
	for _, field := range strings.Fields(output) {
		if len(field) > len("go") && strings.HasPrefix(field, "go") {
...
func extractGoEntry(stage string, header *tar.Header, reader io.Reader) error {
	...
	case tar.TypeReg, tar.TypeRegA:
		...
		file, err := os.OpenFile(target, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, os.FileMode(header.Mode).Perm())
		if err != nil {
			return err
		}
