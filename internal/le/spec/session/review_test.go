...
func TestReviewRoundSixNeedsAProductDefectAndOwnerAuthorization(t *testing.T) {
	// VALIDATES: round 6 requires both bounds to be lifted independently.
	// PREVENTS: a session authorizing its own unbounded review loop or using the
	...
	_, err = recordReview(root, base)
	if err == nil {
		t.Fatal("missing owner authorization was accepted")
	}
...
func TestModelCommandUsesPayloadTranscriptInsteadOfANewerNeighbor(t *testing.T) {
	// VALIDATES: a hook payload's explicit transcript is the exact session read,
	// even when ambient discovery would select a newer neighboring session.
	...
	neighbor := filepath.Join(dir, "neighbor.jsonl")
	if err := os.WriteFile(same, []byte("{\"message\":{\"model\":\"claude-opus-5-same\"}}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(neighbor, []byte("{\"message\":{\"model\":\"claude-sonnet-newer\"}}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	now := time.Now()
...
func TestReviewCommandUsesTheOwnerAuthorisedKeyword(t *testing.T) {
	...
	args := []string{
		...
		"owner-authorised", "Thomas approved one more pass", //nolint:misspell // owner-authorised is the keyword under test, not prose; see the note on the case clause in actions.go
	}
	...
	if record.OwnerAuthorised == "" {
		t.Fatal("owner-authorised value was not mapped") //nolint:misspell // owner-authorised is the keyword under test, not prose; see the note on the case clause in actions.go
	}
