...
func (c ReviewCheck) Text() string {
	...
	case reasonMissing:
		tb.Str("review-gate: BLOCKED — no independent-review artifact at ").Str(c.Path).Str("\n").
			Str("  Run an INDEPENDENT critical review (subagents / fresh session, never your own inline reasoning) and record it with le spec review record. See ai/rules/planning.md.\n")
	case keywordVerdict:
