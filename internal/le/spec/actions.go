...
func refuseLifecycle(got string) int {
	fmt.Fprintf(os.Stderr, "usage: le spec {claim spec <spec>|current|release|wip|state current|state latest spec <stem>|model current [transcript <absolute-path>]|review <hash|record|check>} (got %q)\n", got) //nolint:errcheck // CLI output
	return 2
}
