...
const area = "doc yang-contract"

// action is one callable `le doc yang-contract` operation.
type action struct {
	verb   string
	why    string
	writes bool
...
// ActionList is what `le doc yang-contract` answers when no action is named. It is the
...
// Answer is the `le doc yang-contract` command. The action is a KEYWORD in first
