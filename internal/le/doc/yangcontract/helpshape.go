...
// HelpShapeReport is the whole answer of one `le doc yang-contract help-shape` run: the
...
func (r *HelpShapeReport) judgeSummary(surface, label, description string) bool {
	...
	if count := len(docste.Sentences(summary)); count > 1 {
		var detail textbuf.Buffer
...
func (r *HelpShapeReport) judgeCaps(surface, label, description string) {
	...
	if count := docste.WordCount(summary); count > docste.MaxDescriptiveWords {
		var detail textbuf.Buffer
		r.refuse(surface, label, ruleWordCap, detail.Str("the summary is ").Int(int64(count)).
			Str(" words (STE Rule 6.3 allows ").Int(int64(docste.MaxDescriptiveWords)).Byte(')').String(), summary)
	}
