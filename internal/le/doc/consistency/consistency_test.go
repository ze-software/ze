...
//
// The gate's previous test (internal/le/doc/consistency/consistency_test.go) forked the tool
...
// TestCheckReportsPathsRelativeToTheTree is what makes `le doc consistency` answer
...
// TestAnswerRefusesArguments is the command's grammar: `le doc consistency` takes
...
func TestAnswerVerdictFollowsTheReport(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if code != tc.want {
				t.Errorf("`le doc consistency` exited %d, want %d: %+v", code, tc.want, payload)
			}
