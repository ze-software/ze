...
func TestTheWholeIndexIsRenderedByteForByte(t *testing.T) {
	...
	const want = "# Documentation to Code Index\n" +
		"\n" +
		"<!-- GENERATED by ./le doc index write -- do not edit -->\n" +
		"<!-- Regenerate: ./le doc index write -->\n" +
		"\n" +
...
func TestOnlyTheWriteActionWrites(t *testing.T) {
	list := Actions()

	// Two: one check and one write, each covering the design-doc index and
	// the source-anchor reverse index (D-5).
...
func TestAStaleIndexAnswersTheCodeACallerReadsApartFromOne(t *testing.T) {
	...
	if _, code := Answer([]string{"write"}); code != 0 {
		t.Errorf("write answered %d over a stale index", code)
	}
}
