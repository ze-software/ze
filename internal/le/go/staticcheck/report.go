...
//
// report.go holds what the two actions of `le go staticcheck`
...
func (v Verdict) stage() string {
	var tb textbuf.Buffer
	return tb.Str("go staticcheck/check/part/").Int(int64(v.Part)).
...
func (v Verdict) rerun() string {
	var tb textbuf.Buffer
	return tb.Str("./le go staticcheck check part ").Int(int64(v.Part)).
		Str(" of ").Int(int64(v.Parts)).String()
}
