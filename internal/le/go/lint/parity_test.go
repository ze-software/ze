...
func TestFlavorMatrixPinsEveryCurrentBuildInOrder(t *testing.T) {
	want := []Flavor{
		...
		{Name: "capability", GOOS: "linux", Tags: []string{"debug", "race", "live", "stress", "maprib", "fleetperf", "zetest", "gokrazy", "ze_le", "integration", "ze_docvalid_fixture"}, Why: "every additive capability tag that is not a mutually exclusive personality"},
...
func TestPlanPinsEveryArgvEnvironmentScopeAndOrder(t *testing.T) {
	...
	wantCommands := [][]string{
		...
		{"golangci-lint", "run", "-j", "8", "--allow-serial-runners", "--build-tags", "debug,race,live,stress,maprib,fleetperf,zetest,gokrazy,ze_le,integration,ze_docvalid_fixture", "./pkg/p10"},
