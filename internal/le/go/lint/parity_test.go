...
func TestVerifyLintRunActionIsNativeAndGateless(t *testing.T) {
	listing := Actions()
	if listing.Area != "go lint" || len(listing.Actions) != 1 {
...
func TestFlavorMatrixPinsEveryCurrentBuildInOrder(t *testing.T) {
	want := []Flavor{
		...
		{Name: "capability", GOOS: "linux", Tags: []string{"debug", "race", "live", "stress", "maprib", "fleetperf", "zetest", "gokrazy", "ze_perf", "ze_analyze", "ze_chaos", "ze_le", "integration", "ze_docvalid_fixture"}, Why: "every additive capability tag that is not a mutually exclusive personality"},
...
func TestPlanPinsEveryArgvEnvironmentScopeAndOrder(t *testing.T) {
	...
	wantCommands := [][]string{
		...
		{"golangci-lint", "run", "-j", "8", "--allow-serial-runners", "--build-tags", "debug,race,live,stress,maprib,fleetperf,zetest,gokrazy,ze_perf,ze_analyze,ze_chaos,ze_le,integration,ze_docvalid_fixture", "./pkg/p10"},
...
func TestExecuteCopiesChildOutputToTheSlotLog(t *testing.T) {
	root, _ := lintFixture(t)
	const finding = "internal/le/go/lint/verifylint.go:12:3: something (errcheck)\n"
	ops := runnerOps{
		lookPath: func(name string) (string, error) { return name, nil },
		capture:  func(context.Context, []string, string, []string) commandResult { return commandResult{} },
	...
	want := []string{"internal/le/go/lint/verifylint.go"}
