...
const findingLines = "" +
	"internal/le/go/lint/verifylint.go:41:2: field is unused (unused)\n" +
	"level=info golangci-lint has version 2.1.0\n" +
	"internal/le/go/lint/verifylint.go:88:6: comment should end in a period (godot)\n" +
...
func TestALintRedNamesTheFilesItFailedAbout(t *testing.T) {
	...
	want := []string{"internal/core/family/family.go", "internal/le/go/lint/verifylint.go"}
...
func TestAFindingSplitAcrossTwoWritesIsStillSeen(t *testing.T) {
	...
	cut := len("internal/le/go/lint/verify")
