...
func Rename(root string, options RenameOptions) (RenameReport, error) {
	...
	err = applyRename(root, plan, options, &report)
	return report, err
}
...
func renameReport(plan renamePlan, options RenameOptions) RenameReport {
	...
	return RenameReport{
		...
		Goimports: goimportsNotRun, Left: []CountedPath{}, Resealed: []string{}, ResealRefused: []string{},
		ResidualHost: []CountedPath{}, Code: 0,
	}
}

func applyRename(root string, plan renamePlan, options RenameOptions, report *RenameReport) error {
	for _, edit := range plan.edits {
	...
	for _, move := range plan.moves {
		...
		if _, err := os.Stat(source); os.IsNotExist(err) {
			...
			report.Code = 2
			return fmt.Errorf("inspect move source %s: %w", move.From, err)
		}
		if err := os.MkdirAll(filepath.Dir(destination), 0o755); err != nil { //nolint:gosec // a directory inside the checkout, which the tree keeps world-readable so a second agent account can read it
			report.Code = 2
			return fmt.Errorf("create parent for %s: %w", move.To, err)
		}
	...
	if len(report.Left) != 0 {
		report.Code = 1
		return fmt.Errorf("%d tracked file(s) still contain the old module path", len(report.Left))
	}

	if !options.NoReseal {
		note := fmt.Sprintf("Mechanical re-stamp by `le go module rename`: the module path moved from %s to %s. Each named file was independently proved to differ from HEAD by this rename only; verdicts failing that proof remain stale.", plan.old, plan.new)
...
func gitTrackedFiles(root string) ([]string, error) {
	command := exec.CommandContext(context.Background(), "git", "ls-files", "-z")
	command.Dir = root
...
func renameGoTargets(edits []textEdit, moves []PathMove) []string {
	...
	for _, edit := range edits {
		if filepath.Ext(edit.Relative) != goSuffix {
			continue
		}
		...
		if name == "all.go" || strings.HasPrefix(name, "all_") && strings.HasSuffix(name, goSuffix) {
			continue
		}
		set[relative] = true
	}
...
func renameOnlySinceHead(root, relative, old, new string) bool {
	command := exec.CommandContext(context.Background(), "git", "show", "HEAD:"+relative) //nolint:gosec // relative is a tracked path this run listed from git
	command.Dir = root
