...
// Detail: report.go -- what `le go version-pin check` answers
//
// Package goversionpin pins every build carrier in this repository to the Go minor
...
//     internal/le/verify/evidence/evidence.go).
//
...
func selfImportPath() string {
	...
	if !ok {
		panic("BUG: goversionpin.selfImportPath: the runtime does not know this frame")
	}
	...
	if dot < 0 {
		panic("BUG: goversionpin.selfImportPath: a function name with no package separator")
	}
...
// container build file or Go source, outside excluded trees.
