...
func TestTheWalkEntersNeitherVendorNorTestdata(t *testing.T) {
	for _, row := range []struct {
		...
		{"internal/le/verify/evidence/evidence.go", true},
		{"vendor/example.com/other/Dockerfile", false},
		{"internal/le/go/versionpin/testdata/drift/Dockerfile", false},
		{"internal/le/go/versionpin/testdata/drift/image.go", false},
...
func TestThisPackageIsSubtractedFromThePopulation(t *testing.T) {
	...
	if !strings.HasSuffix(self, "/versionpin") {
	...
	want := []string{"docker/Dockerfile", "internal/le/verify/evidence/evidence.go"}
