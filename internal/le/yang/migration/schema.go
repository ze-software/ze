...
func schemaToYang(root string, apply bool) (Report, error) {
	...
	for _, directory := range directories {
		...
		newRel := relative(root, filepath.Join(filepath.Dir(directory), yangDir))
...
func discoverSchemaDirectories(root string) ([]string, error) {
	...
	err := filepath.WalkDir(internal, func(path string, entry fs.DirEntry, err error) error {
		...
		if entry.Name() != schemaDir || relative(root, path) == skippedSchemaDirectory {
			return nil
		}
		...
		for _, child := range children {
			if child.Type().IsRegular() && filepath.Ext(child.Name()) == yangSuffix {
...
func planSchemaDirectory(root, sourceDirectory string, report *Report) {
	destinationDirectory := filepath.Join(filepath.Dir(sourceDirectory), yangDir)
	...
	for _, entry := range entries {
		...
		if filepath.Ext(entry.Name()) == yangSuffix {
...
func discoverGoFiles(root string) ([]string, error) {
	var paths []string
	for _, directory := range []string{"internal", "cmd", "pkg", testTree} {
		found, err := walkFiles(filepath.Join(root, directory), func(path string) bool { return filepath.Ext(path) == goSuffix })
		if err != nil {
			return nil, err
		}
...
func transformSchemaGo(path string, content []byte, oldToNew map[string]string, movingDirectory map[string]bool) ([]byte, error) {
	...
	for _, importSpec := range file.Imports {
		...
		if importSpec.Name != nil {
			...
			case strings.Contains(oldAlias, schemaDir):
				newAlias = strings.ReplaceAll(oldAlias, schemaDir, yangDir)
	...
	if movingDirectory[filepath.Clean(filepath.Dir(path))] {
		...
		case schemaDir:
			replacements = append(replacements, nodeReplacement(fset, file.Name, yangDir))
		case "schema_test":
