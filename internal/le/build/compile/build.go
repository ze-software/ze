...
//
// Package buildcompile builds the host-side appliance driver and both
...
//
// It registers no command. `le build host-driver` and `le build installer`
// each own a command surface and call this package, because the two builds
// share one runner and one structured report.

package buildcompile
...
// Host builds ze-host at the checkout root. It is the whole of `le build
// host-driver`.
func Host() (any, int) {
...
// Installer cross-builds the installer initrd PID 1 for arch into
// bin/ze-installer-<arch>. It is the whole of `le build installer <arch>`.
func Installer(arch string) (any, int) {
