...
func renderMarkdown(metrics []Metric, history []object) string {
	...
	add("# Testing Health")
	add("")
	add("GENERATED by `make ze-test-health-update` -- do not edit. Source: " +
	...
	add("")
	//nolint:misspell // the page's own bytes, which both halves must print
	// identically while the script and the command run side by side.
...
func renderTrends(add func(string), history []object) {
	...
	if len(history) < minSamples {
		add(tb.Str("Insufficient data: ").Int(int64(len(history))).Str(" recorded sample(s), ").
			...
			"is noise with a direction. Append a sample with `make ze-test-health-record`.").String())
		add("")
		return
	}
