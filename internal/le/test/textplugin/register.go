// Design: docs/architecture/testing/ci-format.md -- the harness command `le test text-plugin`
//
...
const name = "test text-plugin"

func init() {
	leroot.Register(name, leroot.GroupSuite, harnesstool.Answer(cli.CmdTextPlugin), harnesstool.Meta("Run minimal text-mode plugin (for .ci tests)"))
