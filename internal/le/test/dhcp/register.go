// Design: docs/architecture/testing/ci-format.md -- the harness command `le test dhcp`
//
...
var suite = cli.CIRunnerConfig{
	...
	Detail:          "Run DHCP server functional tests (.ci files in test/dhcp/).\nCovers the RFC 2131 / 2132 address-pool surface the daemon accepts and refuses: the subnet a range must fall inside, the static mapping bound to the same subnet, and the options a lease carries.",
	DefaultParallel: 0,
}
