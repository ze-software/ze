...
// run_gating (internal/le/test/functional/actions.go) removes an unresolved name from its run list.
...
func TestCatalogDerivesGatingFromTheRunList(t *testing.T) {
	...
	for _, row := range rows {
		...
		if row.Rerun != "./le test functional "+row.Action {
...
func TestTheKillPublishesAFailureGroupWithARerun(t *testing.T) {
	...
	for _, want := range []string{
		...
		`"rerun":"./le test functional plugin"`, `"parallel":"stage"`,
	} {
...
// starts nothing, while `le test functional gating` and `le test functional list` each
...
func TestARunListVerbRefusesToRunBesideASuite(t *testing.T) {
	...
	for _, line := range [][]string{
		...
		if code != 2 {
			t.Errorf("`le test functional %s` answered %d, want 2: it ran a population instead of refusing the line",
				strings.Join(line, " "), code)
		}
		if payload != nil {
			t.Errorf("`le test functional %s` answered %v, want no payload at all",
	...
	if code == 2 {
		t.Error("`le test functional encode plugin` was refused; a multi-suite line is what a sweep is for")
	}
	if _, isSweep := swept.(leaction.Sweep); !isSweep {
		t.Errorf("`le test functional encode plugin` answered %T, want the sweep envelope", swept)
	}
...
// VALIDATES: `le test functional` answers the area's verbs with `gating` among them
// and builds nothing, `le test functional list` still answers the suite catalog,
// and `le test functional gating` reaches runGating.
