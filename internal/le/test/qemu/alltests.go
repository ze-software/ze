...
//
// The ARGUMENTS are stated here rather than derived from testfunctional.Suites.
...
var vmSuites = []vmSuite{
	...
	{Name: "l2tp", Args: []string{"l2tp", allTests}, Concurrency: scaledConcurrency, Namespace: guestRoot},
	{
		...
		Why: "firewall-nat-exclude installs a nat prerouting chain, and in the guest root namespace" +
			...
			" launcher has only ever been exercised one test at a time (./le test qemu netns-test)",
	},
	{
...
var excludedIntegrationPackages = map[string]string{
	"internal/plugins/as112": "runs as its own verb, `./le test integration as112`" +
...
type allTestsRun struct {
	...
	BinDir string
	// The two daemon binaries, as the host named them. A relative path is
	// resolved against the workspace, which is how the native host action
	// passes them. The harness is this run's own le (guestLeLink).
	ZeBin       string
	StrippedBin string
...
func (a *allTestsRun) verify() error {
	...
	for _, bin := range []string{a.ZeBin, a.StrippedBin} {
...
// this run lists.
//
// The population is testfunctional.Suites, which is the repository's own declaration
...
func suiteCoverage() (population.Coverage, error) {
	declared := make(map[string]bool, len(testfunctional.Suites))
...
func (a *allTestsRun) integrationArgs() ([]string, error) {
	...
	if len(missing) > 0 {
		...
		return nil, errors.New(tb.Str("qemu: the integration package list names path(s) that do not exist: ").
			Join(missing, " ").Str(" -- this is a bug in internal/le/test/qemu, not a test failure").String())
	}
