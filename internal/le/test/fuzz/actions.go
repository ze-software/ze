...
// (ai/rules/cli.md). internal/le/repo/compiles took the same route for REV.
...
// Answer is the `le test fuzz` command.
...
// runList is the `le test fuzz list` action.
...
// runSweep implements `le test fuzz run`.
// Its payload is the complete verdict, while per-target progress has already gone to stderr.
// Thus, `le test fuzz run | json` returns one document for the sweep.
