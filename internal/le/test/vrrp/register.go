// Design: docs/architecture/testing/ci-format.md -- the harness command `le test vrrp`
//
...
var suite = cli.CIRunnerConfig{
	...
	TestSubdir:      "vrrp",
	Description:     "VRRP",
	Detail:          "Run VRRP functional tests (.ci files in test/vrrp/).\nCovers the vrrp YANG augment under interface units, the plugin's cross-leaf verifier (vrid, priority, per-version interval encodings, accept-mode, IPv6 first-address link-local, duplicate vrid/address, VPP backend rejection), and the show/doctor surfaces.",
	DefaultParallel: 0,
}
