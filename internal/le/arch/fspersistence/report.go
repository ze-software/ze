...
//
// report.go holds what `le arch fs-persistence check` ANSWERS, apart from what
// produced it.
//
...
func (f Findings) Text() string {
	...
	tb.Str("Document genuine artifact exceptions in internal/le/arch/fspersistence/fspersistence.go.\n")
	return tb.String()
}
