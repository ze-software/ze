...
// Answer is the `le arch tier` command.
func Answer(args []string) (any, int) { return actions.Answer(args) }

// runCheck is the `le arch tier check` action.
//
...
// runReport is the `le arch tier report` action, over the three default registries.
...
// runWriteBaseline is the `le arch tier write-baseline` action.
