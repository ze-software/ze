...
//
// report.go contains `le arch tier` ANSWERS, separate from their producers.
//
...
// AuditReport answers `le arch tier report` with the module and each area's rows.
//
...
// baselineReport is what `le arch tier write-baseline` answers.
