...
func TestCheckJudgesOnlyTheChangeSet(t *testing.T) {
	report, err := inChangeSet(changeSetRows(), repochanged.ScopeReport{Packages: []string{"./internal/component/bgp/..."}}, noWorkingTree)
	if err != nil {
...
func TestWidenedChangeSetFallsBackToTheWorkingTree(t *testing.T) {
	...
	report, err := inChangeSet(changeSetRows(), repochanged.ScopeReport{Packages: []string{"./..."}, Widened: true, Reason: "no green commit"}, diff)
	if err != nil {
		t.Fatalf("scope the findings: %v", err)
	}
...
func TestBothRoutesUnavailableFailsClosed(t *testing.T) {
	_, err := inChangeSet(changeSetRows(), repochanged.ScopeReport{Packages: []string{"./..."}, Widened: true}, noWorkingTree)
...
func TestEmptyChangeSetFailsClosed(t *testing.T) {
	if _, err := inChangeSet(changeSetRows(), repochanged.ScopeReport{}, noWorkingTree); !errors.Is(err, ErrNoChangeSet) {
		t.Fatalf("an empty change set answered %v, want ErrNoChangeSet", err)
	}
}

// TestEnumerationActionIsRegistered is the wiring row: `le arch enumeration check`
...
func publishScope(t *testing.T, root string, packages []string) {
	...
	previous := env.Get(repochanged.ScopeFileKey)
	if err := env.Set(repochanged.ScopeFileKey, path); err != nil {
		t.Fatalf("name the scope answer: %v", err)
	}
	t.Cleanup(func() { _ = env.Set(repochanged.ScopeFileKey, previous) })
}
...
// and PUBLISHED, and repochanged.WriteScopePackages carries the packages without the
...
func TestAPublishedNarrowScopeJudgesOnlyThatPackage(t *testing.T) {
	...
	publishScope(t, root, []string{"./internal/le/arch/enumeration"})
...
func TestCheckDoesNotBlockOnAGatedRow(t *testing.T) {
	...
	report, err := inChangeSet(found, repochanged.ScopeReport{Packages: []string{"./gated-closed"}}, noWorkingTree)
	if err != nil {
		t.Fatalf("scope the findings: %v", err)
	}
