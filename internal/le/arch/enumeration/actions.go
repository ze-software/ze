...
// Answer is the `le arch enumeration` command.
func Answer(args []string) (any, int) { return actions.Answer(args) }

// runCheck is the `le arch enumeration check` action. It blocks on what the change
...
func runCheck() (any, int) {
	...
	scope, scopeCode := repochanged.Packages(tree)
	if scopeCode != 0 {
		leaction.ReportError(fmt.Errorf("%w: the change-set selector exited %d", ErrNoChangeSet, scopeCode))
		return nil, 2
	}
	report, err := inChangeSet(found, scope, func() ([]string, error) { return repochanged.WorkingTreePaths(tree) })
	if err != nil {
		leaction.ReportError(err)
		return nil, 2
	}
...
// runReport is the `le arch enumeration report` action. It answers every row in the
...
// answers the packages it was asked about (internal/le/repo/changed, Scope.Resolve).
...
func inChangeSet(found Findings, scope repochanged.ScopeReport, workingTree func() ([]string, error)) (CheckReport, error) {
...
// publishes the PACKAGES to a file (repochanged.WriteScopePackages), which carries
// no Widened column, so repochanged.Scope.fromFile hands a widened answer back with
...
// one fact: `./...` is written by widen (internal/le/repo/changed/scope.go) and by
// failOpen (internal/le/repo/changed/selector.go), and by nothing else. So the
...
func widening(scope repochanged.ScopeReport) (bool, string) {
	if scope.Widened {
