...
//
// report.go holds what `le arch iface-resolution` ANSWERS, apart from what produced
// it.
//
...
func (f Findings) Text() string {
	...
	tb.Str("must resolve directly, add it to the allowlist in internal/le/arch/ifaceresolution/ifaceresolution.go\n")
