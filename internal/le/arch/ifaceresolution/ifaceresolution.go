...
//
// Package archifaceresolution enforces the interface-resolution invariant from the
...
// carry. The retired Make target used the equivalent
// ze-iface-resolution-check spelling.
const name = "arch iface-resolution"
...
var allowlist = map[string]string{
	...
	"internal/le/test/deployment/l2tpdiag_linux_ops.go":           "the diagnostic reads the pppN device that PPPIOCNEWUNIT just asked the kernel to allocate; the name is kernel-assigned and no logical selector exists.",
	"internal/le/interoplab/rawframe_linux.go":                    "SendFrameInNamespace enters the lab peer's network namespace and resolves the veth name the fixture created there, to bind the AF_PACKET socket it injects the frame on; the name is lab topology and no Ze logical selector exists in the lab process. The entry named internal/le/interoplab/bgp/isis_inject_linux.go until 2026-09-11, when that file stopped resolving and started handing the name here instead.",
	...
	"internal/test/fixture/plugin_fixture_08_tunnel_ttl_linux.go": "compiled tunnel fixture reading back the outer-header TTL the kernel stores on the tunnel netdev the test just created; the value is a device attribute no `show interface` reports, the name is fixture state, and the fixture process has no Ze interface backend or logical selector.",
	...
	"internal/core/probe/socket_linux.go":                         "datagramBindAddr turns the zone of a probe's source address (fe80::1%eth0 from the `source` leaf of ping and traceroute) into the sockaddr's ZoneId for the unprivileged datagram ICMP bind. The raw socket path hands the same string to net.ListenConfig, which resolves the zone inside the standard library by the same kernel name lookup, so the datagram bind resolves the zone exactly as the raw bind does; the leaf has always taken a kernel zone, never a Ze logical name, and internal/core/probe is a core leaf that cannot reach internal/component/iface.",
	"internal/test/fixture/vrrp_track_linux.go":                   "compiled VRRP priority-decrement fixture querying the veth pair and tracked link it created inside its test namespace, to take the tracked link down and to bind the AF_PACKET capture that reads the advertised priority byte; the fixture process has no Ze interface backend or logical selector.",
	"internal/test/fixture/plugin_fixture_clamped_path_linux.go":  "compiled netns fixture programming the literal veth pairs and loopback it created inside the three namespaces it just made, through a netlink handle bound to each namespace; the names are fixture topology, and the fixture process has no Ze interface backend or logical selector.",
}
...
// Answer is the `le arch iface-resolution` command. The tree is the checkout, so the
