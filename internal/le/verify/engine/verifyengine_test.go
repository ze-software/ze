...
func TestFullStagesMatchesNativeActionPopulation(t *testing.T) {
	want := []string{
		"go lint/run", "arch tier/check", "rfc/check", "arch iface-resolution",
		"plugin boundary/check", "config coercion/check", "arch fs-persistence/check",
		"cli stdio/check", "config ports/check", "go version-pin/check", "config claims",
		"test sensitivity/check", "test weakened/check",
		"go staticcheck/check/part/1/of/6",
		"go staticcheck/check/part/2/of/6",
		"go staticcheck/check/part/3/of/6",
		"go staticcheck/check/part/4/of/6",
		"go staticcheck/check/part/5/of/6",
		"go staticcheck/check/part/6/of/6",
		"repo tracked-build/check", "go vet-platforms/darwin/freebsd", "doc wiring",
		...
		"ai rules/condensed-check", "ai rules/lint", "repo arch-map/check",
...
func TestRunCertificateRecordsSkippedSuitesAsPartialEvidence(t *testing.T) {
	...
	report := RunMode(context.Background(), root, "abc", Mode, runner, Slot{})
...
func TestEmptyAndUnregisteredAnswersFailClosed(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			report := RunMode(context.Background(), t.TempDir(), "abc", Mode, test.run, Slot{})
...
func TestInterruptionStopsBeforeTheNextStageAndRecordsStatus(t *testing.T) {
	...
	report := RunMode(ctx, root, "abc", Mode, runner, Slot{})
