...
func TestEvidenceVetPlanPreservesItsExactPopulation(t *testing.T) {
	root := t.TempDir()
	for _, directory := range []string{"internal/le/verify/evidence", "internal/le/test/deployment", "internal/le/test/qemu"} {
	...
	want := []string{"go", "vet", "./internal/le/verify/evidence/...", "./internal/le/test/deployment/...", "./internal/le/test/qemu/..."}
