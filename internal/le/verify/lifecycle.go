...
// Package verify materializes a commit in a fresh detached worktree and
...
type Report struct {
	...
	Failure     *verifyengine.Failure `json:"failure,omitempty"`
...
func run(ctx context.Context, root string, options Options, actions verifyengine.ActionRunner, deps dependencies) (report Report) {
	...
	if err != nil {
		report.Failure = &verifyengine.Failure{Kind: "commit-resolution", Message: err.Error()}
	...
	if prune := pruneWorktrees(ctx, root, deps.git); prune != nil {
		report.Failure = &verifyengine.Failure{Kind: "stale-registration-cleanup", Message: prune.Error()}
		report.Diagnostics = append(report.Diagnostics,
	...
	if branch.Code == 0 {
		report.Failure = &verifyengine.Failure{Kind: "branch-refusal", Message: text.Reset().
			Str("worktree is on branch ").Quoted(strings.TrimSpace(branch.Output)).
			Str("; detached worktree required").String()}
		return report
	}
	if branch.Code != 1 {
		report.Failure = &verifyengine.Failure{Kind: "branch-check", Message: commandFailure("git symbolic-ref", branch, nil)}
		return report
	}
