...
func TestMovePreviewIsBoundarySafeDeterministicAndReadOnly(t *testing.T) {
	...
	before := mustRead(t, filepath.Join(root, "internal/plugins/client/client.go"))
	...
	if got := mustRead(t, filepath.Join(root, "internal/plugins/client/client.go")); got != before {
		t.Fatalf("preview changed importer:\n%s", got)
	}
	if _, err := os.Stat(filepath.Join(root, "internal/component/edge")); err != nil {
...
func TestMoveApplyRelocatesTreeRewritesImportsAndPluginDiscovery(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "internal/component/edge")); !os.IsNotExist(err) {
		t.Fatalf("old source still exists: %v", err)
	}
	moved := mustRead(t, filepath.Join(root, "internal/plugins/edge/edge.go"))
	if !strings.Contains(moved, testOldModule+"/internal/plugins/edge/sub") {
		t.Fatalf("moved import was not rewritten: %s", moved)
	}
	client := mustRead(t, filepath.Join(root, "internal/plugins/client/client.go"))
...
func TestMoveApplyRefusesRPCHazardAndCollisionBeforeMutation(t *testing.T) {
	t.Run("rpc hazard", func(t *testing.T) {
		...
		if _, statErr := os.Stat(filepath.Join(root, "internal/component/edge/rpc.go")); statErr != nil {
	...
	t.Run("merge collision", func(t *testing.T) {
		...
		if got := mustRead(t, filepath.Join(root, "internal/plugins/edge/same.go")); !strings.Contains(got, "destination") {
