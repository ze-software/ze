...
var sourceAreas = []string{componentTree, pluginsTree, coreTree}

var residualExtensions = map[string]bool{
	goSuffix: true, ".mk": true, ".sh": true, ".ci": true, ".yang": true,
...
func Move(root string, options MoveOptions) (MoveReport, error) {
	...
	err = applyMove(root, plan, &report)
	return report, err
}
...
func moveReport(plan movePlan, apply bool) MoveReport {
	...
	return MoveReport{
		...
		Residual: append([]CountedPath{}, plan.residual...), Goimports: goimportsNotRun,
...
func applyMove(root string, plan movePlan, report *MoveReport) error {
	...
	if report.GeneratorCode != 0 {
		...
		return fmt.Errorf("plugin generator exited with code %d", report.GeneratorCode)
	}
	if len(report.Registrations.Dropped) != 0 {
		report.Code = 4
		return fmt.Errorf("generated registration set dropped %d package(s)", len(report.Registrations.Dropped))
	}
	return nil
}

func readModulePath(root string) (string, error) {
	file, err := os.Open(filepath.Join(root, "go.mod")) //nolint:gosec // the module file at the checkout root
	if err != nil {
...
func destinationAlias(value string) string {
	...
	case "component", componentTree:
		return componentTree
	case "plugins", pluginsTree:
		return pluginsTree
	case "core", coreTree:
		return coreTree
	default:
...
func moveDestination(source, value string) (string, error) {
	...
	for short, full := range map[string]string{"component": componentTree, "plugins": pluginsTree, "core": coreTree} {
...
func planImportEdits(root, oldPrefix, newPrefix string) ([]textEdit, error) {
	...
	err := walkFiles(root, func(relative, absolute string) error {
		if filepath.Ext(relative) != goSuffix {
			return nil
		}
...
func plannedPluginDirs(entries, nested []string, source, destination string) []string {
	...
	effective := make([]string, 0, len(entries)+len(nested))
...
func findRPCPackages(root, source string) ([]string, error) {
	...
	err := walkFiles(base, func(_ string, absolute string) error {
		if filepath.Ext(absolute) != goSuffix || strings.HasSuffix(absolute, "_test.go") {
			return nil
		}
...
func mergeTrees(root, source, destination string) error {
	...
	for _, relative := range files {
		...
		if err := os.MkdirAll(filepath.Dir(to), 0o755); err != nil { //nolint:gosec // a directory inside the checkout, which the tree keeps world-readable so a second agent account can read it
			return err
		}
...
func runPluginGenerator(root string) int {
	command := exec.CommandContext(context.Background(), "go", "run", pluginGenerator)
	command.Dir = root
...
func runMoveGoimports(root string, plan movePlan) string {
	...
	args := make([]string, 0, 3+len(targets))
	args = append(args, "-w", "-local", plan.module)
	args = append(args, targets...)
	command := exec.CommandContext(context.Background(), binary, args...) //nolint:gosec // binary is goimports from PATH, and every argument is a checkout-relative target
	command.Dir = root
	command.Stdout = os.Stdout
	command.Stderr = os.Stderr
	if err := command.Run(); err != nil {
