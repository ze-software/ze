...
func Audit(request AuditRequest) AuditReport {
	...
	head, problem := resolveCommit(request.Root, "HEAD")
	if problem != "" {
	...
	findings, examined, problem := auditDiff(request.Root, "HEAD", "", nil)
	if problem != "" {
		report.Problem = problem
		return report
	}
...
// scopedRowMatches in weakened.go) must still read as explained when a later
