...
//
// Package webassets derives, for each page ze serves, the set of vendored web
// assets that page must load, so a head block renders the imports that page
// needs instead of the union every page needs.
//
...
// those components names. Source derivation OVER-approximates, because a branch
// no request takes still contributes its asset. Deriving from rendered output
// instead would UNDER-approximate, and the two errors do not cost the same. An
// asset shipped and unused costs bytes. An asset needed and absent gives a page
// that renders correctly and does nothing in the browser.
...
type surface struct {
	...
	Dir string
	// Design is the architecture document the generated file references.
	Design string
	// Pkg is the Go package the generated file declares.
	Pkg string
	// Prefix is where this package serves its assets. The generated Go holds
	// the whole URL, because a head block renders one; the pages answer holds
...
//
// It is a var rather than a const table so a test can narrow it to the one
// surface its fixture stands up. Nothing else writes it.
var surfaces = []surface{
	{Dir: "internal/component/web", Pkg: "web", Prefix: "/assets/",
		Design: "docs/architecture/web-interface.md -- per-page web asset sets"},
	{Dir: "internal/component/lg", Pkg: "lg", Prefix: "/lg/assets/",
		Design: "docs/architecture/web-interface.md -- looking glass web asset sets"},
...
type page struct {
	// Key names the page in the pages answer: the source that renders its head,
...
func parseTempl(root, dir, name string) ([]*component, error) {
	...
	body, err := os.ReadFile(filepath.Join(root, dir, name)) //nolint:gosec // a build tool reads the checkout it was pointed at
	if err != nil {
		var tb textbuf.Buffer
		return nil, errors.New(tb.Str("read ").Str(source).Str(": ").Err(err).String())
	}
	...
	if current != nil {
		var tb textbuf.Buffer
		return nil, errors.New(tb.Str(source).Str(": templ ").Str(current.Name).
			Str(" has no closing brace at column 0").String())
	}
...
func templComponents(root string, s surface) (map[string]*component, []string, error) {
	...
	if err != nil {
		var tb textbuf.Buffer
		return nil, nil, errors.New(tb.Str("read ").Str(s.Dir).Str(": ").Err(err).String())
	}
	...
	for _, entry := range entries {
		...
		parsed, parseErr := parseTempl(root, s.Dir, entry.Name())
		if parseErr != nil {
		...
		for _, c := range parsed {
			if _, clash := components[c.Name]; clash {
				var tb textbuf.Buffer
				return nil, nil, errors.New(tb.Str(s.Dir).Str(" declares templ ").Str(c.Name).Str(" twice").String())
			}
	...
	if len(components) == 0 {
		var tb textbuf.Buffer
		return nil, nil, errors.New(tb.Str(s.Dir).Str(" holds no templ component").String())
	}
...
func surfaceUnion(root string, s surface) ([]string, error) {
	...
	if err != nil {
		var tb textbuf.Buffer
		return nil, errors.New(tb.Str("read ").Str(s.Dir).Str(": ").Err(err).String())
	}
	...
	for _, entry := range entries {
		...
		body, readErr := os.ReadFile(filepath.Join(root, s.Dir, name)) //nolint:gosec // a build tool reads the checkout it was pointed at
		if readErr != nil {
			var tb textbuf.Buffer
			return nil, errors.New(tb.Str("read ").Str(filepath.ToSlash(filepath.Join(s.Dir, name))).
				Str(": ").Err(readErr).String())
		}
...
// pageKeyFor names one body page in the pages answer: the shell that renders
// its head, then the body.
...
func templPages(root string, s surface) ([]page, error) {
	...
	for _, name := range order {
		...
		if !shell.Asks {
			var tb textbuf.Buffer
			return nil, errors.New(tb.Str(shell.File).Str(": templ ").Str(shell.Name).
				Str(" renders <head> and does not render pageAssets, so it still hand-writes its script tags").String())
		}
		...
		if shell.Names != "" {
			if want := constantFor(shell.Name); shell.Names != want {
				var tb textbuf.Buffer
				return nil, errors.New(tb.Str(shell.File).Str(": templ ").Str(shell.Name).Str(" names ").
					Str(shell.Names).Str(" in its head block, and its own page is ").Str(want).String())
			}
...
func checkMarked(components map[string]*component, order []string, pages []page) error {
	...
	for _, name := range order {
		if components[name].Marked && !served[name] {
			var tb textbuf.Buffer
			return errors.New(tb.Str(components[name].File).Str(": templ ").Str(name).Str(" carries ").
				Str(pageMarker).Str(" and no shell of the package asks for a page at render time, ").
				Str("so nothing renders its assets").String())
		}
	}
...
func goPages(root string, s surface) ([]page, error) {
	...
	for _, entry := range entries {
		...
		if readErr != nil {
			var tb textbuf.Buffer
			return nil, errors.New(tb.Str("read ").Str(source).Str(": ").Err(readErr).String())
		}
...
func render(s surface, pages []page, union []string) ([]byte, error) {
	...
	b.Str("// Design: ").Str(s.Design).Str("\n//\n")
	b.Str("// Code generated by ./le web-assets write; DO NOT EDIT.\n\n")
	b.Str("// The vendored assets each page of this package loads, derived from the markup\n")
	b.Str("// every page reaches. To change a set, change the markup. Then: ./le web-assets write\n\n")
	...
	if err != nil {
		var tb textbuf.Buffer
		return nil, errors.New(tb.Str("format the generated file for ").Str(s.Dir).Str(": ").Err(err).String())
	}
...
// generated file owes.
func derive(root string) (sets PageSets, generated map[string][]byte, err error) {
	sets = PageSets{}
	generated = map[string][]byte{}

	for _, s := range surfaces {
		pages, pageErr := surfacePages(root, s)
		if pageErr != nil {
		...
		if len(pages) == 0 {
			var tb textbuf.Buffer
			return nil, nil, errors.New(tb.Str(s.Dir).Str(": no page renders a head block").String())
		}

		union, unionErr := surfaceUnion(root, s)
		if unionErr != nil {
			return nil, nil, unionErr
		}

		out, renderErr := render(s, pages, union)
		if renderErr != nil {
	...
	if len(sets) == 0 {
		return nil, nil, errors.New("no page carries an asset set, and ze serves pages")
	}
...
// Pages answers the derived per-page asset sets. It is READ-ONLY: a test asks
// for the sets while it runs, and a query that rewrote the tree would leave the
// working tree changed by the act of reading it.
func Pages(root string) (PageSets, error) {
	sets, _, err := derive(root)

	return sets, err
}

// Check reports the FIRST generated file that disagrees with the markup its
// package renders. It writes nothing.
//
// It stops at the first because the script does, and the answer is `make
// generate` whichever file it names. Derived carries every file the run
// compared, so a reader can see the check covered all three surfaces rather
// than stopping early for a different reason.
func Check(root string) (CheckReport, error) {
	_, generated, err := derive(root)
	if err != nil {
		return CheckReport{}, err
	}

	report := CheckReport{Derived: sortedKeys(generated)}

	for _, path := range report.Derived {
		got, readErr := os.ReadFile(filepath.Join(root, filepath.FromSlash(path))) //nolint:gosec // a build tool reads the checkout it was pointed at
		if readErr != nil {
			var tb textbuf.Buffer
			return report, errors.New(tb.Str("read ").Str(path).Str(": ").Err(readErr).Str("; run: ./le web-assets write").String())
		}
		if !bytes.Equal(got, generated[path]) {
			report.Stale = path
...
// Write generates every surface's file and answers the ones whose bytes
// changed.
func Write(root string) (WriteReport, error) {
	...
	report := WriteReport{Derived: sortedKeys(generated)}

	for _, path := range report.Derived {
		full := filepath.Join(root, filepath.FromSlash(path))

		got, readErr := os.ReadFile(full) //nolint:gosec // a build tool reads the checkout it was pointed at
		if readErr == nil && bytes.Equal(got, generated[path]) {
			continue
		}
		if err := os.WriteFile(full, generated[path], 0o644); err != nil { //nolint:gosec // generated source
			var tb textbuf.Buffer
			return WriteReport{}, errors.New(tb.Str("write ").Str(path).Str(": ").Err(err).String())
		}
