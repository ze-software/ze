...
// Commands the scenarios run inside a peer, the ip(8) words they use to move
// an address, and the container interface that address lives on. Each is
// named once for every place that runs or resolves it.
const (
	cmdSwanctl         = "swanctl"
	cmdPing            = "ping"
	ipArgDev           = "dev"
