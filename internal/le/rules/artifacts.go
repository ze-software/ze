...
//
// artifacts.go renders the outputs of `rules_condensed.py`. These outputs are
// TRIGGERS.md, CORE.md, and the payload report that measures both files.
//
...
//
// The generated headers still name the Python script, because until the swap
// the two halves write the same bytes into the same files. Repointing that line
// is step 14 of spec-le-is-a-ze-binary, not this port.
	...
	for _, artifact := range r.Artifacts {
		if artifact.Stale {
			tb.Str("WARNING: ").Str(artifact.File).
				Str(" is stale -- run: make ze-rules-condensed-update\n")
		}
	}
	return tb.String()
}
...
func Digest(tree string, check bool) (DigestReport, error) {
	...
	core, err := CoreMembers(rules, Corpus{Read: true, Tasks: tasks}, &report.EmptyCorpus)
	if err != nil {
		return report, err
	}
