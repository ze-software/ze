...
func recordReview(root string, request reviewRecord) (reviewArtifact, error) {
	...
	if request.Rounds > reviewOwnerRoundCap {
		if owner == "" {
			return reviewArtifact{}, fmt.Errorf("review_gate: %d review rounds needs Thomas's authorisation; a session must not authorise itself", request.Rounds)
		}
	}
...
func CheckReview(root, spec, sessionID string, files []string) (ReviewCheck, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return ReviewCheck{Spec: specStem(spec), Path: relativeReviewPath(root, path), Blocked: true, Reason: "missing"}, nil
		}
