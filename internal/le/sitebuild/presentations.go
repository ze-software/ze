// Design: website/presentations -- presentation artifacts are reproducible Go outputs
package sitebuild
...
func bundlePresentation(input string) (string, error) {
	...
	bundled, err := bundlePresentationHTML(string(content), filepath.Dir(input))
	if err != nil {
		return "", err
	}
...
func bundlePresentationHTML(content, base string) (string, error) {
	...
	content = replaceRegexpGroups(iframePattern, content, func(match string, groups []string) string {
		...
		data, err := os.ReadFile(filepath.Join(base, filepath.Clean(filepath.FromSlash(path))))
		if err != nil {
			return match
		}
	...
	if slides, err := os.ReadFile(slidesPath); err == nil {
...
func renderActivity(options ActivityOptions) error {
	...
	defer cancel()
	command := exec.CommandContext(ctx, "git", "-c", "core.quotePath=false", "-C", options.Repository, "log", options.Ref, "--since="+start, "--date=short", "--format=@@%ad", "--numstat")
	...
	return os.WriteFile(options.Output, out.Bytes(), 0o644)
}
