...
// cleaning it, so unchanged generated pages keep their exact bytes.
func TestBuildPreservesExistingArtifactSeed(t *testing.T) {
	...
	want := []byte("<html>generated bytes</html>\n")
	page := filepath.Join(output, "reference", "cli", "index.html")
	...
	if err := os.WriteFile(filepath.Join(output, "data", "cli-commands.json"), []byte("[]\n"), 0o644); err != nil {
		t.Fatal(err)
	}
