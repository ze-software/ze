...
var (
	...
	goSizePattern       = regexp.MustCompile(`- Only \*\*[0-9][^*]*\*\*[^\n]* of Go code`)
...
func updateTalk(options talkUpdateOptions) (talkUpdateReport, error) {
	...
	if !options.BundleOnly {
		...
		content, readErr := os.ReadFile(slides)
		switch {
		case readErr == nil:
			...
			if writeErr := os.WriteFile(slides, content, 0o644); writeErr != nil {
...
func collectTalkStats(repository string) (talkStats, error) {
	...
	defer cancel()
	command := exec.CommandContext(ctx, "git", "-C", root, "log", "--format=%H", "--grep=Co-Authored-By")
	...
	err = filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			if relative == ".git" || entry.Name() == "vendor" {
