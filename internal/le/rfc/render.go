...
// scripts/dev/check_doc_links.py reads a backticked path out of ANY tracked
...
func RenderShards(in RenderInput) map[string]string {
	...
	for _, rfc := range ShardStems(in.Requirements) {
		...
		out = append(out,
			...
			"",
			banner.
				Str("GENERATED by `make ze-rfc-index-update` -- do not edit. Requirement ").
				Str("text is authored in `").Str(summaryRelOf(rfc)).
				Str("`; the test links are derived from `RFC requirement:` tags in the ").
				Str("tests themselves. Run `python3 scripts/dev/rfc_requirements.py ").
				Str("--locate <requirement-id>` for the exact lines. The index over every ").
				Str("RFC is `ai/RFC-REQUIREMENTS.md`.").String(),
			"",
...
// the evidence legend, the coverage rollup scripts/dev/testing_health.py parses,
...
func RenderIndex(in RenderInput) (string, error) {
	...
	out := []string{
		...
		"",
		"GENERATED by `make ze-rfc-index-update` -- do not edit. Requirement text is " +
	...
	out = append(out, counts.Int(int64(len(in.Requirements))).Str(" requirements across ").
		...
		Str(" of those are enrolled and gated by `make ze-rfc-check`.").String(), "")
...
func renderEvidenceLegend(carriers []Carrier) []string {
	...
	out = append(out, tail.
		Str("A tag in a carrier nothing executes is REFUSED by `make ze-rfc-check`, not ").
