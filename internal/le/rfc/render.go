...
// the gated RFCs were analyzed for every MUST they state.
//
...
func extractionBoundSentence(in RenderInput) string {
	...
	if walked == enrolled {
		return tb.Str("All ").Int(int64(enrolled)).
			Str(" RFCs were analyzed for every MUST they state, so the counts above ").
...
func RenderIndex(in RenderInput) (string, error) {
	...
	// been analyzed for the MUSTs they state, and the walks that
