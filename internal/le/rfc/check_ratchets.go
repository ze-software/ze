	...
	for _, stem := range sortedMissing(stems, baselineStems) {
		...
		if gated[stem] > 0 {
			...
			errs = append(errs, tb.Str("rfc/short/").Str(stem).Str(".md is new and declares ").Int(int64(gated[stem])).
				Str(" gated MUST-level requirement(s), but does not declare `| Enrolment | enrolled |` -- so none of them is checked. Enroll it in its own Meta table (see .claude/skills/ze-rfc/SKILL.md), classifying each requirement as tested, {single-polarity}, {gap}, {lower-layer}, {feature-declined} or {not-applicable}, or as a {rollup} over rows already classified").String())
			continue
		}
