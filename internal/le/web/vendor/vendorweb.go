...
//
// Package webvendor is the two halves of one contract. third_party/web/ is the
...
func consumerDirs(root string) ([]string, error) {
	...
	err := filepath.WalkDir(base, func(found string, d fs.DirEntry, walkErr error) error {
		...
		// internal/le is the development tooling. It serves no page and embeds
		// no vendored asset, and its `web assets` command lives at
		// internal/le/web/assets because a space in a command name is a
		// directory level. That directory is Go source, not a consumer.
		if found == filepath.Join(root, "internal", "le") {
			return fs.SkipDir
		}
