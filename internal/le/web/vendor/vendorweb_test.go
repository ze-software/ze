...
//
// This is the-sync-restores-an-edited-copy (internal/le/web/vendor/vendorweb_test.go) as
...
// PREVENTS: a generator that rewrites every file it visits. `./le web vendor
...
// internal/le/web/vendor/vendorweb_test.go pins the script still failing open, so that
...
// PREVENTS: `./le web vendor sync` exiting 0 after failing to write a file,
...
// PREVENTS: a typo reading as a failed check. `le web vendor chekc` exiting 1
...
// PREVENTS: `le web vendor check | match DRIFT` being turned away over an
