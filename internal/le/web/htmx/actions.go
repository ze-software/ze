// Design: docs/architecture/core-design.md -- the web htmx area, as one command
// Detail: tree.go -- the package scan and explanation verdict each action calls.
//
// The table carries the two permanent verbs. leaction keeps their write
// markers, listing, help, and dispatch on one declaration.
...
var actions = leaction.New(
	area,
	leaction.Action{
		Verb: "check",
		Why: "the native Go scanner reports no htmx 4 issue that " +
			"test/htmx-upgrade-explained.txt does not account for. It builds a DOM, " +
			"so it reads inheritance carriers a text search cannot",
		Answer: func() (any, int) {
			return run(false)
		},
	},
...
// Answer is the `le web htmx` command.
