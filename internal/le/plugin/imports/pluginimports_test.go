...
func fixture(t *testing.T) string {
	...
	write(t, root, "internal/component/config/yang/register.go",
		"package yang\n\nimport (\n\t_ \""+module+"/internal/component/config/yang\"\n)\n")
	// A schema-shaped package under testdata is parser input, not a production
	// registration package.
	write(t, root, "internal/le/migration/testdata/case/internal/component/demo/yang/register.go",
		"package yang\n\nimport (\n\t_ \""+module+"/internal/component/config/yang\"\n)\n")
	...
	// Development tools describe the registration syntax but are not product
	// namespaces. The scanner must not import its own implementation.
	write(t, root, "internal/le/plugin/imports/self.go",
		"package pluginimports\n\nfunc describe() { _ = \".RegisterNamespace(\" }\n")
...
//
// This is the proof internal/le/plugin/imports/pluginimports_test.go
...
func TestGeneratedAllGoIsByteExact(t *testing.T) {
	got := string(allSource(imports{
		...
		RootHandlers: []string{"m/c/cli"},
	}))

	want := "// Code generated by ./le plugin imports write; DO NOT EDIT.\n\n" +
...
//
// This is the proof internal/le/plugin/imports/pluginimports.go carried as its own
...
func TestBothReportsRenderNativeWording(t *testing.T) {
	...
	current.Counts = Counts{Plugins: 2, Schemas: 3, RPCs: 4, Namespaces: 5, RootHandlers: 6, GatedGroups: 7}
	want := "internal/component/plugin/all/all.go is current (2 plugins, 3 schemas, 4 rpcs, 5 namespaces, 6 cli commands, 7 gated groups)\n"
	...
	if got := stale.Text(); got != "./le plugin imports check: internal/component/plugin/all/all.go is stale; run ./le plugin imports write\n" {
		t.Errorf("a stale check renders %q", got)
	}

	var written WriteReport
	written.Plugins = 1
	wantWritten := "Generated internal/component/plugin/all/all.go with 1 plugins, 0 schemas, 0 rpcs, 0 namespaces, 0 cli commands, 0 gated groups\n"
...
// VALIDATES: a package whose init() registers a CLI command, and which no blank
// import reaches, is reported by the check with exit 1.
// PREVENTS: the failure the fifth discovery kind exists to close. A correctly
// written internal/component/<x>/cli compiles, links nowhere and registers
// nothing, so `ze <x>` answers "unknown command" while the build, the lint and
// every gate stay green.
func TestUnimportedRootHandlerIsReported(t *testing.T) {
	root := fixture(t)
	if _, code := runWrite(root); code != 0 {
		t.Fatal("the first write failed")
	}

	// A command owner outside every plugin search root, which is where one
	// normally lives: the component owns the command, and pluginDirs states
	// where a PLUGIN may live rather than where a command owner may.
	write(t, root, "internal/component/firewall/cli/register.go",
		"package cli\n\nfunc init() {\n\tregistry.MustRegisterRootHandler(\"firewall\", dispatch, registry.Meta{})\n}\n")

	report, code := runCheck(root)
	if code != 1 {
		t.Fatalf("check answered %d over a tree whose command owner is unimported, want 1", code)
	}
	if report.Stale != filepath.ToSlash(filepath.Join(allDir, "all.go")) || report.Reason != reasonStale {
		t.Errorf("check named %q %q, want all.go stale", report.Stale, report.Reason)
	}
	...
	if strings.Join(found.RootHandlers, ",") != module+"/internal/component/firewall/cli" {
		t.Errorf("the cli commands are %v, want the firewall command owner", found.RootHandlers)
	}
}

// VALIDATES: a command owner that reaches the composition root, and one marked
// codegen:skip, are both left out of the generated imports.
// PREVENTS: two ways of breaking the product. all.go naming a package that
// imports all.go does not compile, and the universal file naming a package that
// a build-tagged personality root wires links that personality's commands into
// every binary.
func TestACyclingOrSkippedCommandOwnerIsNotGenerated(t *testing.T) {
	root := fixture(t)

	// One hop: the client imports the composition root, and the command owner
	// imports the client.
	write(t, root, "internal/component/cli/client/main.go",
		"package client\n\nimport (\n\t_ \""+module+"/"+allDir+"\"\n)\n")
	write(t, root, "internal/plugins/completion/complete.go",
		"package completion\n\nimport (\n\t_ \""+module+"/internal/component/cli/client\"\n)\n\n"+
	...
	if len(found.RootHandlers) != 0 {
		t.Errorf("the cli commands are %v, want neither the cycling owner nor the skipped one", found.RootHandlers)
	}
}
