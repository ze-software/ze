...
// PREVENTS: the regression where internal/le/repo/changed answers nothing and exits
...
func TestAPathNoPrefixClaimsIsNotGrouped(t *testing.T) {
	if group, mapped := groupOf("internal/le/repo/changed/changed.go"); mapped {
...
func TestAnUnmappedFileBecomesItsOwnPackageDirectory(t *testing.T) {
	selection := groupFiles([]string{"internal/le/repo/changed/changed.go", "internal/le/repo/changed/scope.go"})
	if len(selection.Groups) != 0 {
		t.Errorf("%d groups, want 0: %v", len(selection.Groups), selection.Groups)
	}
	if len(selection.unmapped) != 1 || selection.unmapped[0] != "./internal/le/repo/changed" {
		t.Fatalf("unmapped is %v, want exactly [./internal/le/repo/changed]", selection.unmapped)
	}
}
...
func TestAPackageResolutionFailureAnswersAnErrorRatherThanDroppingThePackage(t *testing.T) {
	...
	rec := &recorder{fail: map[string]error{"go list -e -f {{if not .Error}}{{.Dir}}{{end}}" +
		" ./internal/le/repo/changed": boom}}

	if _, err := (Selector{Root: t.TempDir(), Run: rec.run}).Packages([]string{"./internal/le/repo/changed"}); err == nil {
...
func TestAResolvedDirectoryLeavesNothingUnresolved(t *testing.T) {
	...
	if len(selection.Rest) != 1 || selection.Rest[0] != "./internal/le/repo/changed" {
		t.Fatalf("rest is %v, want exactly [./internal/le/repo/changed]", selection.Rest)
	}
...
func TestTheGroupListingAndThePackageListingRenderOneSelection(t *testing.T) {
	selection := Selection{
		...
		Rest:   []string{"./internal/le/repo/changed"},
	}
