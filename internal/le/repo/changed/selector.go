...
//
// Action: le repo changed scope [print <packages|tags|both>] [depth <n>]
//
...
// every one of them.
func loadFeatureGates(root string) ([]repofeaturetags.Gate, error) {
	gates, err := repofeaturetags.Gates(root)
	if err != nil {
...
func manifestTags(gates []repofeaturetags.Gate) []string {
...
func tagForPackage(dir string, gates []repofeaturetags.Gate) string {
	tag := ""
	longest := 0
	for _, gate := range gates {
...
func reachedTags(root string, paths, seeds []string, gates []repofeaturetags.Gate, everyTag []string) []string {
	seen := map[string]bool{}
	for _, dir := range seeds {
