...
//
// report.go holds what `le repo` ANSWERS, apart from what produced it.
//
...
func (r Report) Text() string {
	...
	tb.SetColor(true)
	if len(r.Findings) == 0 {
		return tb.Colored(colorGreen).Str("./le repo: all checks passed").
			Colored(colorReset).Byte('\n').String()
	}

	if r.Issues > 0 {
		tb.Colored(colorRed).Str("./le repo: ").Int(int64(r.Issues)).
	...
	if r.Warnings > 0 {
		tb.Colored(colorYellow).Str("./le repo: ").Int(int64(r.Warnings)).
