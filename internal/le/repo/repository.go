...
//
// Package repo is the post-verify validation gate: six checks, each
...
// without .ci coverage.
//
// The two gates select between those scopes. `le repo check` gets the
// changed set from git and judges a developer's tree. `le repo
