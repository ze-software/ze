...
//
// Package repopackagemap generates one ai/PACKAGE-MAP.md line for each Go
...
// coverage worklist.
//
// Nothing here is hand-maintained: `./le repo package-map update` regenerates
...
// roots are the trees the map covers. The internal root includes le's plugin
// tree. Roots publishes the complete set so an omitted tree cannot disappear
// from the map that an agent reads.
var roots = [...]string{"internal", "pkg", "cmd"}
...
func Render(packages []Package) string {
	var tb textbuf.Buffer
	tb.Str("# Package Map\n\n")
	tb.Str("<!-- GENERATED by ./le repo package-map update -- do not edit -->\n")
	tb.Str("<!-- Regenerate: ./le repo package-map update -->\n\n")
	tb.Str("One line per Go package under `internal/`, `pkg/`, and `cmd/`.\n")
