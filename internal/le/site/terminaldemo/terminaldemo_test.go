...
// VALIDATES: `le site terminal-demo render` with no demo id is refused, and never falls through to the whole gallery.
...
func TestRenderRefusesAnUnbuiltRendererImage(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "le site terminal-demo image-build") {
