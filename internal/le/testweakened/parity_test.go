...
func TestVerificationArchiveChangesDoNotReadTheBaseline(t *testing.T) {
	root := t.TempDir()
	archived := filepath.Join(root, "plan", "verification-evidence", "a_test.go")
	result := Check(Request{
		Root: root, Session: fixtureSession, Anchor: "not-a-revision",
		Paths: []string{archived}, Removed: []string{"plan/verification-evidence/old_test.go"},
		RenamePairs: []RenamePair{{OldPath: "plan/verification-evidence/old_test.go", NewPath: archived, Score: 100}},
	})
	if result.ExitCode() != 0 || result.PathsJudged != 0 || len(result.Findings) != 0 {
		t.Fatalf("archive-only changes reached the baseline: %#v", result)
	}
}

func TestMovingACanonicalTestIntoTheArchiveIsRemoval(t *testing.T) {
	root := newParityRepository(t)
	const current = "pkg/a_test.go"
	const archived = "plan/verification-evidence/a_test.go"
	destination := filepath.Join(root, filepath.FromSlash(archived))
	if err := os.MkdirAll(filepath.Dir(destination), 0o750); err != nil {
		t.Fatal(err)
	}
	if err := os.Rename(filepath.Join(root, filepath.FromSlash(current)), destination); err != nil {
		t.Fatal(err)
	}
	result := CheckCommit(Request{
		Root: root, Session: fixtureSession, Paths: []string{archived}, Removed: []string{current},
		RenamePairs: []RenamePair{{OldPath: current, NewPath: archived, Score: 100}},
	}, false)
	if result.ExitCode() != 1 || len(result.Findings) != 1 ||
		result.Findings[0].Path != current || result.Findings[0].Name != "TestA" {
		t.Fatalf("archiving unchanged bytes concealed canonical removal: %#v", result)
	}
}

func TestMovingAnArchivedTestCannotSupplyACanonicalBaseline(t *testing.T) {
	root := newParityRepository(t)
	const archived = "plan/verification-evidence/b_test.go"
	const current = "pkg/b_test.go"
	writeParityFile(t, root, archived, "package a\nfunc TestB(t *testing.T) { require.Equal(t, 1, got) }\n")
	if !runSelfTestGit(root, "add", archived) || !runSelfTestGit(root,
		"-c", "user.email=t@t", "-c", "user.name=t", "-c", "commit.gpgsign=false",
		"commit", "-q", "-m", "historical fixture") {
		t.Fatal("commit the historical fixture")
	}
	if err := os.Rename(filepath.Join(root, filepath.FromSlash(archived)),
		filepath.Join(root, filepath.FromSlash(current))); err != nil {
		t.Fatal(err)
	}
	writeParityFile(t, root, current, "package a\nfunc TestB(t *testing.T) { t.Skip(\"later\") }\n")
	result := CheckCommit(Request{
		Root: root, Session: fixtureSession, Paths: []string{current}, Removed: []string{archived},
		RenamePairs: []RenamePair{{OldPath: archived, NewPath: current, Score: 100}},
	}, false)
	if result.ExitCode() != 0 || len(result.Findings) != 0 {
		t.Fatalf("historical bytes supplied a current baseline: %#v", result)
	}
}
