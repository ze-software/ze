...
// Related: testweakened.go -- comparison and ledger pairing over the exact commit population.
...
func prospectiveDiff(root string, paths, removed []string, diffArgs ...string) ([]byte, string) {
	...
	_, code, runErr := run("rev-parse", "--verify", "-q", headRevision)
	if runErr != nil {
		return nil, fmt.Sprintf("git rev-parse HEAD: %v", runErr)
	}
	if code == 0 {
		output, readCode, readErr := run("read-tree", headRevision)
		if readErr != nil {
...
func parseRenamePairs(raw []byte) ([]RenamePair, string) {
	...
	for index := 0; index < len(fields); index += 3 {
		if index+2 >= len(fields) {
			return nil, gitRenameUnreadable
		}
