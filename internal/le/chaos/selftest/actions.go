...
//
// Package chaosselftest runs the chaos simulator's Go tests, its reduced-tag CLI
// tests, and its linter.
// This package owns their exact arguments and environment.
package chaosselftest
...
func (g Action) Argv(tc gotoolchain.Toolchain) []string {
	...
	default:
		panic("BUG: chaosselftest.Action has an unknown command kind")
	}
}
...
// publishes and the table does not hold. `le chaos selftest zzprobe` named the
...
// Answer is the `le chaos selftest` command. A bare command lists the actions and
// runs nothing; `chaos selftest all` is the run it used to start. Named actions run
