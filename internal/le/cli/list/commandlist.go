...
//
// Package clilist is the `ze-command-list` gate: every command ze
...
// words this function held until 2026-09-14 left `./le cli list` reporting
...
// Answer is the `le cli list` command. It takes no arguments: the registry
...
func Answer(args []string) (any, int) {
	if len(args) > 0 {
		...
		fmt.Fprintln(os.Stderr, "usage: le cli list [| json | yaml | table]")               //nolint:errcheck // CLI output
		return nil, 1
	}
