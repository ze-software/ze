...
// ---------------------------------------------------------------------------
// Receive, IPv4: the ICMP errors a GTSM peer's address sends ze
// ---------------------------------------------------------------------------

// TestGTSMDropsADangerousQuotedICMPError publishes the filter ze publishes for
// a GTSM peer and injects the message RFC 5082 classifies as Dangerous: an
// ICMPv4 error from the peer's address, quoting a TCP header of the peer's BGP
// session, arriving below the session's TTL floor.
//
...
// reaches the kernel's ICMP receive path, which is the verification RFC 5082
// Section 6.1 mandates for related messages.
func TestGTSMDropsADangerousQuotedICMPError(t *testing.T) {
	bed := newTestbed(t)
	publishGTSMFilter(t)

	before := icmpInType(t, icmpv4DestinationUnreachable)
	bed.injectQuotedICMPv4Error(1, bgpPort)
	awaitChainEvaluated(t)

	if after := icmpInType(t, icmpv4DestinationUnreachable); after != before {
		t.Fatalf("the kernel received %d ICMP type 3 messages, want the %d it had: the Dangerous message was delivered", after, before)
	}
...
// above: the same message, from the same address, quoting the same session,
// differing only in the outer TTL. A rule that dropped this one would be
// refusing every related message rather than the Dangerous ones.
//
// RFC requirement: RFC5082-3-2 negative -- the same ICMPv4 error arriving with
// an outer TTL of 255 is matched by no drop rule and reaches the kernel's ICMP
// receive path, so a related message carrying the TTL RFC 5082 Section 3
// mandates is delivered.
func TestGTSMDeliversAQuotedICMPErrorAtTTL255(t *testing.T) {
	bed := newTestbed(t)
	publishGTSMFilter(t)

	before := icmpInType(t, icmpv4DestinationUnreachable)
	bed.injectQuotedICMPv4Error(255, bgpPort)
...
// classified as Trusted or Unknown". An ICMP error from the peer's address
// about any other flow belongs to no GTSM session, so it is Unknown, and a low
// TTL must not be enough to drop it.
//
...
func TestGTSMDeliversAnICMPErrorNoSessionClaims(t *testing.T) {
	...
	before := icmpInType(t, icmpv4DestinationUnreachable)
	bed.injectQuotedICMPv4Error(1, otherPort)
	after := awaitICMPInType(t, icmpv4DestinationUnreachable, before+1)
...
// injectQuotedICMPv4Error sends ze an ICMPv4 destination-unreachable from the
// peer's address, carrying the outer TTL the caller chooses and quoting a TCP
// header ze would have sent to quotedPort on the peer.
func (b *testbed) injectQuotedICMPv4Error(outerTTL uint8, quotedPort uint16) {
	b.t.Helper()

	quoted := quotedTCPv4Datagram(zeV4, peerV4, localPort, quotedPort)
	message := icmpv4Message(icmpv4DestinationUnreachable, icmpv4PortUnreachable, quoted)
	packet := ipv4Packet(peerV4, zeV4, outerTTL, unix.IPPROTO_ICMP, message)
	b.inject(ethernetFrame(b.zeMAC, peerMAC, ethertypeIPv4, packet))
}
