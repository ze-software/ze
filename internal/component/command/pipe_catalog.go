...
// PipeClass separates operators that are answer-shape independent, operators
// that act on row data, and operators that act on a sequence of answers.
type PipeClass uint8

const (
	// ClassGlobal acts on the answer whatever it holds: the formats, paging,
	// and saving. Every output-producing command owes these on each surface
	// where the operator's LocalOnly contract permits it.
...
// The names of the pipe operators this package spells in more than one place:
// the catalog entry, the format selector and the chain metadata.
const (
	pipeNameJSON  = "json"
	pipeNameText  = "text"
	pipeNameCount = "count"
...
type PipeOperator struct {
	...
	// LocalOnly marks an operator that MUST be expanded by the process the
	// operator started. A daemon-expanded remote chain refuses it at the
	// effect boundary because otherwise it would act with daemon privileges.
	LocalOnly bool
...
// Renders reports whether this operator decides the FORM the answer is printed
// in, rather than which rows or fields it holds: `json`, `ndjson`, `table`,
// `text`, `yaml` and `raw`.
//
// It reads isFormatOp, the predicate the parser refuses a second format by, so
// the catalog and the parser cannot disagree about which operators render. The
// flag-register gate derives the flag spellings it bans from this
// (internal/component/command/grammar/flags.go, F3).
func (op PipeOperator) Renders() bool { return isFormatOp(op.Kind) }
...
var pipeCatalog = []PipeOperator{
	{Name: pipeNameJSON, Kind: pipeJSON, Class: ClassGlobal, Arg: ArgOptional, Repeat: RepeatRefuse, shapes: anyShape,
	...
	{Name: pipeNameText, Kind: pipeText, Class: ClassGlobal, Arg: ArgNone, Repeat: RepeatRefuse, shapes: anyShape,
	...
	{Name: "save", Kind: pipeSave, Class: ClassGlobal, Arg: ArgPath, Repeat: RepeatCompose, shapes: anyShape,
		Description: "Write the answer to a file", LocalOnly: true},
	...
	{Name: pipeNameCount, Kind: pipeCount, Class: ClassData, Arg: ArgNone, Repeat: RepeatRefuse, shapes: rowShapes,
...
func RenderOperatorReference() string {
	...
	b.Reset()
	b.Str("<!-- GENERATED by `./le doc yang-contract pipe-operators-update` from\n")
	b.Str("     internal/component/command/pipe_catalog.go. Do not edit by hand:\n")
	b.Str("     `./le doc check verify` fails when this file and the catalog disagree. -->\n\n")
	b.Str("| Operator | Class | Surface | Argument | Repeated | What it does |\n")
	b.Str("|----------|-------|---------|----------|----------|--------------|\n")
	for _, op := range pipeCatalog {
		arg := op.ArgHint()
		if arg == "" {
		...
		surface := "all"
		if op.LocalOnly {
			surface = "local process only"
		}
		b.Str("| `").Str(op.Name).Str("` | ")
		b.Str(class).Str(" | ")
		b.Str(surface).Str(" | ")
		b.Str(arg).Str(" | ")
		b.Str(repeat).Str(" | ")
		b.Str(op.Description).Str(" |\n")
	}
	b.Str("\nAn operator that acts on rows is refused BY NAME over an answer that has\n")
	b.Str("none, rather than answering something plausible. `resolve` and `origin` act\n")
	b.Str("on a field holding an IP address, and are refused unless the command\n")
	b.Str("declares one.\n")
	return b.String()
}
