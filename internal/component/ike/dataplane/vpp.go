...
type vppBackend struct {
	...
	//
	// MEASURED on VPP v26.06 (internal/le/test/deployment/vppevidence.go, run_ipsec_evidence):
...
func (b *vppBackend) InstallSA(p SAParams) error {
	...
	// both. The AC-7 deployment run (internal/le/test/deployment/vppevidence.go,
...
func vppSAFlags(p SAParams) ipsec_types.IpsecSadFlags {
	...
	// (internal/le/test/deployment/vppevidence.go, run_ipsec_evidence) reads the installed SA
...
// (internal/le/test/deployment/vppevidence.go, run_ipsec_evidence). An AES-GCM-256 SA whose
