// VALIDATES: spec-ike-padded-path-probe. The wiring rows: the engine registers into
// the ikeprobe leaf at init, and a request sent through the leaf reaches the owner
// loop (maintainSA) and is answered from there. The exchange: one INFORMATIONAL
// request carrying one status Notify pads the datagram to the requested size, the
// DF-clear repeat is the same bytes, the answer is correlated by the probe's own id,
// and an unanswered probe fails the SA through the unchanged request window.
// PREVENTS: a leaf nobody registers into, so show mtu reads "no engine in this
// build" on a box that has one; a request answered off the owner goroutine; a probe
// response credited to Dead Peer Detection; a window released without a response
package engine
