...
func TestGroupCommands(t *testing.T) {
	commands := []CommandInfo{
		...
		{Name: "show bgp rib", ShortHelp: "Show routes"},
		{Name: "show bgp rib best status", ShortHelp: "Best-path status"},
		{Name: "show bgp peer list", ShortHelp: "List peers"},
		{Name: "show config dump", ShortHelp: "Dump config"},
		{Name: "show config diff", ShortHelp: "Diff configs"},
		{Name: "show config validate", ShortHelp: "Validate config"},
		{Name: "show schema list", ShortHelp: "List schemas"},
		{Name: "show schema methods", ShortHelp: "List methods"},
		{Name: "show version", ShortHelp: "Show version"},
		{Name: "metrics values", ShortHelp: "Metric values"},
		{Name: "metrics list", ShortHelp: "List metrics"},
		{Name: "log levels", ShortHelp: "Log levels"},
		{Name: "log set", ShortHelp: "Set log level"},
		{Name: "cache list", ShortHelp: "List cache"},
	}
...
func TestGenerateToolsSkipsHandcrafted(t *testing.T) {
	groups := []toolGroup{
		{prefix: "show bgp", actions: []action{{name: "rib status", shortHelp: "RIB summary", full: "show bgp rib status"}}},
		{prefix: "metrics", actions: []action{{name: "values", shortHelp: "Metric values", full: "metrics values"}}},
	}
...
func TestBuildToolDefActionEnum(t *testing.T) {
	g := toolGroup{
		prefix: "show bgp",
		actions: []action{
			{name: "rib", shortHelp: "Show routes", full: "show bgp rib"},
			{name: "rib status", shortHelp: "RIB summary", full: "show bgp rib status"},
		},
	}
...
func TestDispatchGenerated(t *testing.T) {
	...
	s := &server{
		dispatch: func(_ context.Context, _ plugin.CallerIdentity, cmd string) (*plugin.Response, error) {
			dispatched = cmd
			return plugin.NewResponse(plugin.StatusDone, plugin.Map{"result": "ok"}), nil
		},
	}
	...
	if len(content) == 0 || content[0]["text"] != `{"result":"ok"}` {
...
func TestExecuteWithoutCommandFailsClosed(t *testing.T) {
	...
	runner := &server{
		dispatch: func(_ context.Context, _ plugin.CallerIdentity, _ string) (*plugin.Response, error) {
			dispatched++
			return plugin.NewResponse(plugin.StatusDone, plugin.Map{"result": "unreachable"}), nil
		},
	}
...
func TestCallToolGeneratedViaHTTP(t *testing.T) {
	...
	hs, cleanup := newTestStreamable(t, StreamableConfig{
		Dispatch: func(_ context.Context, _ plugin.CallerIdentity, cmd string) (*plugin.Response, error) {
			dispatched = cmd
			return plugin.NewResponse(plugin.StatusDone, plugin.Map{"result": "result-ok"}), nil
		},
		Commands: func() []CommandInfo {
			return []CommandInfo{
				{Name: "show bgp rib status", ShortHelp: "RIB summary"},
	...
	// The fake's structured payload reaches the caller as the JSON object it
	// is: RawJSON refuses a text answer, so no surface quotes one any more.
	if text, _ := first["text"].(string); text != `{"result":"result-ok"}` {
...
func TestGroupCommandsActionContent(t *testing.T) {
	...
	for i, w := range want {
		a := g.actions[i]
		if a.name != w.name || a.full != w.full || a.shortHelp != w.help {
			t.Errorf("action[%d] = {%q, %q, %q}, want {%q, %q, %q}",
				i, a.name, a.full, a.shortHelp, w.name, w.full, w.help)
		}
	}
}
...
func TestGroupCommandsSingleToken(t *testing.T) {
	...
	groups := groupCommands([]CommandInfo{{Name: "summary", ShortHelp: "BGP summary"}})
	if len(groups) != 1 {
...
func TestBuildToolDefNoAction(t *testing.T) {
	...
	g := toolGroup{
		prefix:  "version",
		actions: []action{{name: "", shortHelp: "Show version", full: "version"}},
	}
	tool := buildToolDef(g)
	if tool == nil {
		t.Fatal("buildToolDef returned nil")
...
func TestBuildToolDefSingleAction(t *testing.T) {
	...
	g := toolGroup{
		prefix:  "cache",
		actions: []action{{name: "list", shortHelp: "List cache entries", full: "cache list"}},
	}
	tool := buildToolDef(g)
	desc, ok := tool["description"].(string)
	if !ok {
...
func TestBuildToolDefSingleActionEmptyHelp(t *testing.T) {
	...
	g := toolGroup{
		prefix:  "cache",
		actions: []action{{name: "list", shortHelp: "", full: "cache list"}},
	}
	tool := buildToolDef(g)
	desc, ok := tool["description"].(string)
	if !ok {
	...
	s := &Streamable{cfg: StreamableConfig{
		Commands: func() []CommandInfo {
			return []CommandInfo{
				...
				{Name: "commands list", ShortHelp: "List commands"},
				{Name: "commands help", ShortHelp: "Help on commands"},
				{Name: "show bgp rib status", ShortHelp: "RIB summary"},
				{Name: "show bgp peer list", ShortHelp: "List peers"},
			}
		},
	...
	s := &Streamable{cfg: StreamableConfig{
		Commands: func() []CommandInfo {
			return []CommandInfo{
				{
					Name:      "show bgp rib",
					ShortHelp: "Show routes",
					Params: []ParamInfo{
						{Name: "family", Type: "string", ShortHelp: "Address family", Required: false},
						{Name: "count", Type: "uint32", ShortHelp: "Max results", Required: false},
					},
				},
				{
					Name:      "show bgp rib status",
					ShortHelp: "RIB summary",
				},
				{
				...
				},
				{
					...
					ShortHelp:     "Peer detail",
					TakesSelector: true,
				},
				{
					Name:      "show config dump",
					ShortHelp: "Dump config",
				},
			}
		},
	}}
...
func TestToolDescriptor_TaskSupportField(t *testing.T) {
	commands := []CommandInfo{
		{Name: "show bgp rib dump", ShortHelp: "Dump RIB", TaskSupport: TaskSupportRequired},
		{Name: "show bgp rib status", ShortHelp: "RIB summary", TaskSupport: TaskSupportOptional},
		{Name: "show config dump", ShortHelp: "Dump config", TaskSupport: TaskSupportOptional},
	}
	groups := groupCommands(commands)
	...
	commands2 := []CommandInfo{
		{Name: "ping host", ShortHelp: "Ping", TaskSupport: TaskSupportForbidden},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			srv, err := NewStreamable(StreamableConfig{
				Commands: func() []CommandInfo {
					return []CommandInfo{{Name: "probe cmd", ShortHelp: "Probe", TaskSupport: tc.level}}
				},
...
// TestMCPToolDefUsesSummaryAndHelp proves story 7. The action enum offers the
// one-line summary of each action. The tool's own description is where the long
// explanation goes. It drives buildToolDef over a group with two named actions,
// then over a group with one action. Those are the two shapes that build a
// description from different halves.
//
// PREVENTS: the long explanation reaching the action enum, which is a one-line
// list a model reads to pick an action, and the explanation reaching no MCP
// client at all because only the summary is published.
func TestMCPToolDefUsesSummaryAndHelp(t *testing.T) {
	const ribSummary = "Show every route in the RIB."
	const ribHelp = "One row for each prefix.\nThe best path is marked with a star."

	g := toolGroup{
		prefix: "show bgp",
		actions: []action{
			{name: "rib", shortHelp: ribSummary, description: ribHelp, full: "show bgp rib"},
			{name: "rib status", shortHelp: "Count the routes in the RIB.", full: "show bgp rib status"},
		},
	}
	...
	want := "rib: " + ribSummary + ". rib status: Count the routes in the RIB."
	if schema.Properties.Action.Description != want {
		t.Errorf("action description = %q, want %q", schema.Properties.Action.Description, want)
	}
	if strings.Contains(schema.Properties.Action.Description, "best path is marked") {
		t.Error("the long explanation reached the action enum, which is a one-line list")
	}

	// One named action: the tool IS that command, so its description carries
	// both halves, the summary first.
	single := toolGroup{
		prefix:  "show bgp",
		actions: []action{{name: "rib", shortHelp: ribSummary, description: ribHelp, full: "show bgp rib"}},
	}
	desc, ok := buildToolDef(single)["description"].(string)
	if !ok {
		t.Fatal("description not string")
	}
	if desc != ribSummary+"\n\n"+ribHelp {
		t.Errorf("tool description = %q, want the summary then the explanation", desc)
	}

	// The command IS the prefix: the same rule, through the unnamed-action arm.
	bare := toolGroup{
		prefix:  "show version",
		actions: []action{{name: "", shortHelp: "Print the version.", description: "The build stamp is the git SHA.", full: "show version"}},
	}
	bareDesc, ok := buildToolDef(bare)["description"].(string)
	if !ok {
		t.Fatal("description not string")
	}
