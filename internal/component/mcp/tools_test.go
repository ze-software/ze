...
func TestGroupCommands(t *testing.T) {
	commands := []CommandInfo{
		...
		{Name: "show bgp rib best status", Description: "Best-path status"},
		{Name: "show bgp peer list", Description: "List peers"},
		{Name: "show config dump", Description: "Dump config"},
		{Name: "show config diff", Description: "Diff configs"},
		{Name: "show config validate", Description: "Validate config"},
		{Name: "show schema list", Description: "List schemas"},
		{Name: "show schema methods", Description: "List methods"},
		{Name: "show version", Description: "Show version"},
		{Name: "metrics values", Description: "Metric values"},
		{Name: "metrics list", Description: "List metrics"},
		{Name: "log levels", Description: "Log levels"},
		{Name: "log set", Description: "Set log level"},
		{Name: "cache list", Description: "List cache"},
	}
...
func TestGenerateToolsSkipsHandcrafted(t *testing.T) {
	groups := []toolGroup{
		{prefix: "show bgp", actions: []action{{name: "rib status", description: "RIB summary", full: "show bgp rib status"}}},
		{prefix: "metrics", actions: []action{{name: "values", description: "Metric values", full: "metrics values"}}},
	}
...
func TestBuildToolDefActionEnum(t *testing.T) {
	g := toolGroup{
		prefix: "show bgp",
		actions: []action{
			{name: "rib", description: "Show routes", full: "show bgp rib"},
			{name: "rib status", description: "RIB summary", full: "show bgp rib status"},
		},
	}
...
func TestGroupCommandsActionContent(t *testing.T) {
	...
	for i, w := range want {
		a := g.actions[i]
		if a.name != w.name || a.full != w.full || a.description != w.help {
			t.Errorf("action[%d] = {%q, %q, %q}, want {%q, %q, %q}",
				i, a.name, a.full, a.description, w.name, w.full, w.help)
		}
	}
}
...
func TestGroupCommandsSingleToken(t *testing.T) {
	...
	groups := groupCommands([]CommandInfo{{Name: "summary", Description: "BGP summary"}})
	if len(groups) != 1 {
...
func TestBuildToolDefNoAction(t *testing.T) {
	...
	g := toolGroup{
		prefix:  "version",
		actions: []action{{name: "", description: "Show version", full: "version"}},
	}
	tool := buildToolDef(g)
	if tool == nil {
		t.Fatal("buildToolDef returned nil")
...
func TestBuildToolDefSingleAction(t *testing.T) {
	...
	g := toolGroup{
		prefix:  "cache",
		actions: []action{{name: "list", description: "List cache entries", full: "cache list"}},
	}
	tool := buildToolDef(g)
	desc, ok := tool["description"].(string)
	if !ok {
...
func TestBuildToolDefSingleActionEmptyHelp(t *testing.T) {
	...
	g := toolGroup{
		prefix:  "cache",
		actions: []action{{name: "list", description: "", full: "cache list"}},
	}
	tool := buildToolDef(g)
	desc, ok := tool["description"].(string)
	if !ok {
	...
	s := &Streamable{cfg: StreamableConfig{
		Commands: func() []CommandInfo {
			return []CommandInfo{
				...
				{Name: "commands list", Description: "List commands"},
				{Name: "commands help", Description: "Help on commands"},
				{Name: "show bgp rib status", Description: "RIB summary"},
				{Name: "show bgp peer list", Description: "List peers"},
			}
		},
...
func TestToolDescriptor_TaskSupportField(t *testing.T) {
	commands := []CommandInfo{
		{Name: "show bgp rib dump", Description: "Dump RIB", TaskSupport: TaskSupportRequired},
		{Name: "show bgp rib status", Description: "RIB summary", TaskSupport: TaskSupportOptional},
		{Name: "show config dump", Description: "Dump config", TaskSupport: TaskSupportOptional},
	}
	groups := groupCommands(commands)
	...
	commands2 := []CommandInfo{
		{Name: "ping host", Description: "Ping", TaskSupport: TaskSupportForbidden},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			srv, err := NewStreamable(StreamableConfig{
				Commands: func() []CommandInfo {
					return []CommandInfo{{Name: "probe cmd", Description: "Probe", TaskSupport: tc.level}}
				},
...
func TestMCPToolDefUsesSummaryAndHelp(t *testing.T) {
	...
	g := toolGroup{
		prefix: "show bgp",
		actions: []action{
			{name: "rib", description: ribSummary, longHelp: ribHelp, full: "show bgp rib"},
			{name: "rib status", description: "Count the routes in the RIB.", full: "show bgp rib status"},
		},
	}
	...
	single := toolGroup{
		prefix:  "show bgp",
		actions: []action{{name: "rib", description: ribSummary, longHelp: ribHelp, full: "show bgp rib"}},
	}
	...
	bare := toolGroup{
		prefix:  "show version",
		actions: []action{{name: "", description: "Print the version.", longHelp: "The build stamp is the git SHA.", full: "show version"}},
	}
