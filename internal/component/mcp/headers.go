...
//
// The transport mirrors selected JSON-RPC body fields into HTTP headers.
// Intermediaries (load balancers, gateways, observability tooling) can
// therefore route and inspect a request without a read of its body.
//
// The server owns the other half of that contract. It rejects any request
// whose headers disagree with its body. An intermediary and the server can
// therefore never act on different values.
//
...
const (
	...
	headerName            = "Mcp-Name"
	// headerParamPrefix is the prefix of the custom headers a server can
...
//
// Each names which header disagreed with which body field, and none echoes
...
// spelled here, so it cannot drift from what the server accepts. lead is the
// caller's stable error phrase. The rest of the message is common to the
// header-validation rejection and to the unknown-method rejection. Those are
// the two errors an `initialize` POST can receive.
...
// present. A MISSING `_meta` field is a different failure (-32602), and the
// caller reports that failure next.
//
...
func validateStandardHeaders(r *http.Request, req *request, params map[string]any) error {
	...
	// its error twin -- is refused here, first. It is refused BY NAME, and not
	// as a side effect of the Mcp-Method rule below.
	//
	// This server writes no JSON-RPC request to a client, and it therefore
	// never asks one a question. MCP 2026-07-28 replaced server-initiated
	...
	// (mrtr.go). A response frame that arrives here answers a question this
	// server did not ask. No pending question therefore exists for that frame.
	//
	...
	// a response has no method for any header value to equal.
	//
...
//
// The request type decides which of the two source fields applies. The pair's
// order in that cell does not. `CallToolRequest` and `GetPromptRequest` both
// carry `name`, and only `ReadResourceRequest` carries `uri`.
//
// A read of prompts/get out of `params.uri` mirrors a field that is never
// present. Every prompts/get therefore failed header validation with -32020.
// And the 404 the specification requires for an unimplemented method was
// unreachable.
//
// prompts/get is covered even though this server does not dispatch it. Header
// validation is a transport guard that runs before dispatch. A prompts/get
// POST is therefore header-checked on its way to the unknown-method 404.
...
//
// A value not in sentinel form is returned unchanged with ok=true. The
// sentinel is opt-in, and a conformant client only uses it when the plain
// value cannot be carried in an HTTP field. The markers are matched exactly as
// the specification prints them. An uppercase `=?BASE64?` is therefore a plain
// value and not a malformed sentinel.
//
...
// key). No Mcp-Param-* header therefore has a body field to compare against,
// and the header/body half of the contract has nothing to check here. Per the
...
// applies to every recognized Mcp-Param-* header, and Ze enforces it.
...
// Section 5.5 permits in an HTTP field value. Those octets are visible ASCII
// (0x21-0x7E), space (0x20) and horizontal tab (0x09).
