...
//
// The revision removed the initialize handshake. A request's protocol version,
// client identity and client capabilities therefore no longer arrive once at
...
// `{ form?: JSONObject; url?: JSONObject; }`. The server must therefore gate
// on the MODE it is about to use, not on the key. elicitationFormSupported
// (mrtr.go) owns that reading.
...
// extension, and the ONLY way a client can declare task support: this
// identifier, under `extensions`. The bare `tasks` member of 2025-11-25 is not
// accepted. Read parseClientCapabilities below for why a server that honors it
// would fail open.
//
...
// have, and what to send instead. A client can therefore correct the request
// from the error line alone. All three are answered with JSON-RPC -32602 and
// HTTP 400.
var (
...
//
// It is a value type. The zero value means the client did not identify itself,
// which the specification permits (clients only SHOULD send it). The value is
// self-reported and unverified, so Ze carries it for display and logging only.
// It never reaches an authorization or ownership decision.
...
//
// It is a value type with a fail-closed zero value. Every field is a bool that
// stays false until the client declares the capability. A handler that holds a
...
// something it can demand of a caller.
type clientCapabilities struct {
	...
	// A client that declares only mime types Ze cannot produce therefore reads
	// false.
	//
	...
	// the `elicitation` key. A client that declares `{"url":{}}` supports
...
// no Mcp-Name source field. Every caller reads that as "field absent" and
// rejects it on its own terms. MCP uses camelCase wire keys, so the decode is
...
// "io.modelcontextprotocol/clientCapabilities" Required=Yes, and it marks
...
// header/body contract, and the two failures stay distinguishable. A client can
// therefore tell "you sent the wrong header" from "you omitted a required
// field".
...
func parseClientCapabilities(caps map[string]any) clientCapabilities {
	...
	// no longer accepted. That is a fail-closed decision, not tidying. Under
	// the server-directed model (D-1) this bit decides whether the server can
	// return an unsolicited `resultType: "task"` handle.
	//
	// A 2025-11-25-era client that declared `{"tasks":{}}` opted into a model
	// where IT asked for each task. A server that honors that declaration now
	// pushes an unsolicited task handle at a client that never agreed to
	// receive one. That is exactly the failure the per-request extension check
	// exists to prevent.
	//
