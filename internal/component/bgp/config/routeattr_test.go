...
// TestEncodePrefixSIDSRv6Integration verifies SRv6 Prefix-SID parsing flow.
//
...
func TestEncodePrefixSIDSRv6Integration(t *testing.T) {
	...
	assert.NotEmpty(t, attrs.PrefixSID, "PrefixSID bytes should not be empty for SRv6 format")
	t.Logf("PrefixSID bytes: %x", attrs.PrefixSID)
}

// TestEncodePrefixSIDSRv6_ReservedFieldsZero verifies that the SRv6 Prefix-SID
// encoder emits every RFC 9252 reserved/flags octet as zero. The wire layout of
// EncodePrefixSIDSRv6("l3-service <sid>") is:
//
...
func TestEncodePrefixSIDSRv6_ReservedFieldsZero(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, ps, 28, "unexpected SRv6 Prefix-SID wire length")

	assert.Equal(t, byte(5), ps[0], "outer TLV type must be 5 (L3 Service)")
	assert.EqualValues(t, 0, ps[3], "RFC9252-3.1-1: Service TLV Reserved must be 0")
	assert.Equal(t, byte(1), ps[4], "sub-TLV type must be 1 (SID Information)")
	assert.EqualValues(t, 0, ps[7], "RFC9252-3.2-1: RESERVED1 must be 0")
	assert.EqualValues(t, 0, ps[24], "RFC9252-3.2-2: Service SID Flags must be 0")
	...
	assert.Equal(t, []byte{0x00, 0x3e}, ps[25:27], "Behavior must round-trip")
	assert.EqualValues(t, 0, ps[27], "RFC9252-3.2-3: RESERVED2 must be 0")
}
