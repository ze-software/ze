...
func TestPeersFromConfigTree_GroupRoutes(t *testing.T) {
	...
	peer := config.NewTree()
	peer.Set("local-address", "127.0.0.1")
	...
	group.AddListEntry("peer", "10.0.0.1", peer)
...
func TestPeersFromConfigTree_GroupRoutesIsolation(t *testing.T) {
	...
	peerA.Set("local-address", "127.0.0.1")
	groupA.AddListEntry("peer", "10.0.0.1", peerA)
	bgp.AddListEntry("group", "group-a", groupA)

	// Group B: has a route to 20.0.0.0/24
	groupB := config.NewTree()
	groupB.Set("peer-as", "65002")
	...
	peerB.Set("local-address", "127.0.0.1")
	groupB.AddListEntry("peer", "10.0.0.2", peerB)
