...
func TestAnnounceOversizeDropsWithNamedLog(t *testing.T) {
	...
	t.Run("batch-rail", func(t *testing.T) {
		...
		update, _ := adapter.buildBatchAnnounceUpdate(make([]byte, 256), make([]byte, message.MaxMsgLen),
			...
			announceFacts{nextHop: netip.MustParseAddr("10.0.0.1"), isIBGP: true, asn4: true, prepend: localASOnly(65000)})
...
func TestAnnounceBuilderModeIsEditSetOverEmptyBase(t *testing.T) {
	...
	build := func(batch bgptypes.NLRIBatch) []byte {
		update, _ := adapter.buildBatchAnnounceUpdate(make([]byte, message.MaxMsgLen), make([]byte, message.MaxMsgLen),
			batch, announceFacts{nextHop: netip.MustParseAddr("10.0.0.1"), isIBGP: true, asn4: true, prepend: localASOnly(65000)})
...
func TestAnnounceStripsLocalPrefTowardExternalPeer(t *testing.T) {
	...
	batchRail := func(isIBGP bool) []byte {
		update, _ := adapter.buildBatchAnnounceUpdate(make([]byte, message.MaxMsgLen), make([]byte, message.MaxMsgLen),
			...
			announceFacts{nextHop: netip.MustParseAddr("10.0.0.1"), isIBGP: isIBGP, asn4: true, prepend: localASOnly(65000)})
		require.NotNil(t, update)
		return update.PathAttributes
	}
