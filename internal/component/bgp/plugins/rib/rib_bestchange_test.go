...
func TestPurgeBestPrevForPeer(t *testing.T) {
	...
	fam := family.Family{AFI: 1, SAFI: 1}
	leavingPeer := netip.MustParseAddr("192.0.2.1")
	survivingPeer := netip.MustParseAddr("192.0.2.2")
...
func TestPurgeBestPrevForPeerLocRIB(t *testing.T) {
	...
	r.handleStructuredState(&rpc.StructuredEvent{PeerAddress: peerAddr.String(), State: rpc.SessionStateDown})
...
func TestPurgeBestPrevForPeerAddPath(t *testing.T) {
	...
	fam := family.Family{AFI: 1, SAFI: 1}
	leavingPeer := netip.MustParseAddr("192.0.2.20")
	...
	r.bgpPeers[survivingPeer].SetAddPath(fam, true)
...
func TestBestChangeEntryPathIDPropagation(t *testing.T) {
	...
	r.bgpPeers[leavingPeer].Remove(fam, nlri7)
...
func TestPurgeBestPrevForPeerHandleState(t *testing.T) {
	...
	event := &Event{
		...
		Peer:    mustMarshal(t, map[string]any{"remote": map[string]any{"address": peerAddr.String()}, "state": "down"}),
	}
	r.handleState(event)
...
func TestRIBBestChangeWithdraw(t *testing.T) {
	...
	// Withdraw the route.
	r.bgpPeers[peerAddr].Remove(fam, prefix)
...
func TestRIBBestChangeBatchPeerDown(t *testing.T) {
	...
	for _, p := range prefixes {
		r.bgpPeers[peerAddr].Remove(fam, p)
	}
...
func TestRIBBroadcastReplayCharacterization(t *testing.T) {
	...
	for _, oct := range []byte{0, 1} {
		...
		r.bgpPeers[peerAddr].Insert(fam, makeAttrBytes([4]byte{192, 168, 1, 1}), prefix)
		r.checkBestPathChange(fam, prefix, false, nil)
	}
...
func TestPurgeBestPrevForPeerReclaimsInternerSlot(t *testing.T) {
	...
	idxA, ok := r.bestPathInterner.peerIdxOf(peerA.String())
	...
	// extending peers[].
	peerB := netip.MustParseAddr("192.0.2.51")
	sizeBefore := len(r.bestPathInterner.peers)
	idxB, ok := r.bestPathInterner.internPeer(peerB.String())
	require.True(t, ok)
