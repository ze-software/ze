...
// TestRIBMarkStaleCommand verifies that "request bgp rib mark-stale" marks all routes for a specific peer as stale.
//
...
func TestRIBMarkStaleCommand(t *testing.T) {
	...
	status, data, err := r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "120"})
...
func TestRIBMarkStaleCommandNonExistentPeer(t *testing.T) {
	...
	status, data, err := r.handleCommand("request bgp rib mark-stale", "*", []string{"10.10.10.10", "120"})
	require.NoError(t, err)
	assert.Equal(t, statusDone, status)
...
func TestRIBMarkStaleCommandMissingArgs(t *testing.T) {
	...
	// No args at all.
	_, _, err := r.handleCommand("request bgp rib mark-stale", "*", nil)
	assert.Error(t, err, "should error with no args")

	// Only peer, no restart time.
	_, _, err = r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1"})
...
func TestRIBMarkStaleCommandExplicitLevel(t *testing.T) {
	...
	status, data, err := r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "120", "2"})
	require.NoError(t, err)
	assert.Equal(t, statusDone, status)
...
func TestRIBMarkStaleCommandRejectsLevelZero(t *testing.T) {
	...
	status, _, err := r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "120", "0"})
...
func TestRIBMarkStaleCommandInvalidRestartTime(t *testing.T) {
	...
	status, _, err := r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "abc"})
...
func TestRIBMarkStaleCommandInvalidLevel(t *testing.T) {
	...
	// Non-numeric level.
	status, _, err := r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "120", "abc"})
	...
	status, _, err = r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "120", "256"})
	assert.Error(t, err, "level > 255 should be rejected")
	assert.Equal(t, statusError, status)
}

// TestRIBPurgeStaleCommand verifies that "request bgp rib purge-stale" deletes only stale routes.
//
...
func TestRIBPurgeStaleCommand(t *testing.T) {
	...
	attrBytes := concatBytes(testWireOriginIGP, testWireASPath65001, testWireNextHop)
	r.bgpPeers[netip.MustParseAddr("192.0.2.1")].Insert(ipv4Family, attrBytes, []byte{24, 192, 168, 0}) // 192.168.0.0/24
...
func TestRIBPurgeStalePreservesFresh(t *testing.T) {
	...
	r.bgpPeers[netip.MustParseAddr("192.0.2.1")].Insert(ipv4Family, newAttrBytes, []byte{24, 10, 0, 0}) // 10.0.0.0/24
...
func TestGRFlowMarkAndPurge(t *testing.T) {
	...
	r.bgpPeers[netip.MustParseAddr("192.0.2.1")].Insert(ipv4Family, freshAttr, []byte{24, 10, 0, 0}) // re-announce 10.0.0.0/24

	// Also sends a brand new route
	r.bgpPeers[netip.MustParseAddr("192.0.2.1")].Insert(ipv4Family, freshAttr, []byte{24, 10, 1, 0}) // new 10.1.0.0/24
	...
	_, data, err = r.handleCommand("request bgp rib purge-stale", "*", []string{"192.0.2.1", "ipv6/unicast"})
	require.NoError(t, err)
...
func TestGRConsecutiveRestart(t *testing.T) {
	...
	freshAttr := concatBytes(testWireOriginIGP, testWireASPath65001, testWireNextHop, testWireLocalPref100)
	r.bgpPeers[netip.MustParseAddr("192.0.2.1")].Insert(ipv4Family, freshAttr, []byte{24, 10, 0, 0}) // refreshes 10.0.0.0/24
	...
	_, data, err := r.handleCommand("request bgp rib purge-stale", "*", []string{"192.0.2.1"})
	require.NoError(t, err)
	...
	_, _, err = r.handleCommand("request bgp rib mark-stale", "*", []string{"192.0.2.1", "90"})
	require.NoError(t, err)
...
func TestRIBShowInStaleFlag(t *testing.T) {
	...
	attrBytes := concatBytes(testWireOriginIGP, testWireASPath65001, testWireNextHop)
	r.bgpPeers[netip.MustParseAddr("192.0.2.1")].Insert(ipv4Family, attrBytes, []byte{24, 192, 168, 0}) // fresh 192.168.0.0/24
...
func TestRIBPurgeStaleStopsTimer(t *testing.T) {
	...
	_, _, err = r.handleCommand("request bgp rib purge-stale", "*", []string{"192.0.2.1"})
	require.NoError(t, err)
...
func TestAttachCommunity(t *testing.T) {
	...
	status, data, err := r.handleCommand("request bgp rib attach-community", "*", []string{"192.0.2.1", "ipv4/unicast", "ffff0006"})
	require.NoError(t, err)
	assert.Equal(t, statusDone, status)
...
func TestDeleteWithCommunity(t *testing.T) {
	...
	status, data, err := r.handleCommand("request bgp rib delete-with-community", "*", []string{"192.0.2.1", "ipv4/unicast", "ffff0007"})
	require.NoError(t, err)
	assert.Equal(t, statusDone, status)
...
func TestAttachCommunity_NoCommunities(t *testing.T) {
	...
	_, data, err := r.handleCommand("request bgp rib attach-community", "*", []string{"192.0.2.1", "ipv4/unicast", "ffff0006"})
	require.NoError(t, err)
...
func TestAttachCommunity_MissingArgs(t *testing.T) {
	...
	status, _, err := r.handleCommand("request bgp rib attach-community", "*", nil)
	assert.Equal(t, statusError, status)
	assert.Error(t, err)
}
...
func TestDeleteWithCommunity_MissingArgs(t *testing.T) {
	...
	status, _, err := r.handleCommand("request bgp rib delete-with-community", "*", nil)
	assert.Equal(t, statusError, status)
	assert.Error(t, err)
}
