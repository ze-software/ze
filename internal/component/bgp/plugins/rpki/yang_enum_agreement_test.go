...
//
// Related: rpki_config.go -- aspaPolicyNames, the table held here
//
...
func testModules() map[string]string {
	return map[string]string{
		...
		"ze-rpki.yang":     rpkiyang.ZeRPKIYANG,
	}
}
...
// TestASPAPolicyNamesMatchTheYANGModel proves aspaPolicyNames spells the
// `rpki/action/invalid` enumeration and nothing else.
//
// PREVENTS: an action added to the model that aspaActionFromString refuses, so
// the peer silently keeps the default action while the config says otherwise.
func TestASPAPolicyNamesMatchTheYANGModel(t *testing.T) {
	declared := yangEnumValues(t, "ze-bgp-conf", "bgp", "group", "peer", "rpki", "action", "invalid")

	named := slices.Clone(aspaPolicyNames[:])
	slices.Sort(named)
	require.Equal(t, declared, named)

	// Every declared value parses back to the action that names it, so both
	// readers of the table are exercised.
	for _, value := range declared {
		action, ok := aspaActionFromString(value)
		require.True(t, ok, "the model declares action %q and aspaActionFromString refuses it", value)
		require.Equal(t, value, actionString(action))
	}
}
