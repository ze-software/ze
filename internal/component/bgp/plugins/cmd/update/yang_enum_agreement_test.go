...
//
// Related: update_text.go -- updateEncodings, the dispatch table held here
//
...
import (
	...
	updateyang "github.com/ze-software/ze/internal/component/bgp/plugins/cmd/update/yang"
...
func testModules() map[string]string {
	return map[string]string{
		"ze-update-cmd.yang": updateyang.ZeUpdateCmdYANG,
	}
}
...
// TestUpdateEncodingsMatchTheYANGModel proves updateEncodings carries a handler
// for every value of the `encoding` enumeration.
//
// PREVENTS: an encoding the CLI grammar offers with no handler behind it, which
// answers "unknown encoding" for a word completion just suggested.
func TestUpdateEncodingsMatchTheYANGModel(t *testing.T) {
	declared := yangEnumValues(t, "ze-update-cmd", "send", "bgp", "update", "encoding")

	dispatched := make([]string, 0, len(updateEncodings))
	for encoding, handler := range updateEncodings {
		require.NotNil(t, handler, "encoding %q dispatches to no handler", encoding)
		dispatched = append(dispatched, encoding)
	}
	slices.Sort(dispatched)

	require.Equal(t, declared, dispatched)
}
