...
//
// Related: role.go -- roleNames and roleValues, the tables held here
//
...
// TestRoleNamesMatchTheYANGModel proves the RFC 9234 role tables spell the
// `role/import` enumeration and nothing else.
//
// PREVENTS: a role added to the model that roleNameToValue refuses, so the peer
// advertises no Role capability and every RFC 9234 Section 5 gate reads the
// session as roleless.
func TestRoleNamesMatchTheYANGModel(t *testing.T) {
	declared := yangEnumValues(t, "ze-bgp-conf", "bgp", "group", "peer", "role", "import")

	named := make([]string, 0, len(roleNames))
	...
	// Every declared value maps to a wire value and back, so both tables are
	// read rather than only the one that spells the names.
	for _, value := range declared {
		wire, ok := roleNameToValue(value)
		require.True(t, ok, "the model declares role %q and roleNameToValue refuses it", value)
		name, ok := roleValueToName(wire)
		require.True(t, ok)
