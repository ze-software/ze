...
const (
	...
	//
	// The family names are not declared here. family.MustRegister in types.go
	// joins each one from its AFI and SAFI parts, and labeledFamilies reads
	// them back, so no second spelling exists to drift from.
...
// labeledFamilies returns the registry's own name for each family this plugin
// decodes. The plugin registration and the CLI help both read it.
func labeledFamilies() []string {
	return []string{IPv4LabeledUnicast.String(), IPv6LabeledUnicast.String()}
}
...
func runLabeledPlugin(conn net.Conn) int {
	...
	err := p.Run(ctx, sdk.Registration{
		Families: []sdk.FamilyDecl{
			familyDecl(IPv4LabeledUnicast),
			familyDecl(IPv6LabeledUnicast),
		},
	})
	if err != nil {
