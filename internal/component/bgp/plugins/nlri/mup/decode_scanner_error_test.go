...
// TestRunDecodeReportsReadError checks that a read failure is not reported as a
// clean end of input.
//
// VALIDATES: RunDecode returns non-zero and writes the read error when the
// input reader fails part way through a line.
// PREVENTS: a truncated decode stream exiting 0 as if every request decoded.
func TestRunDecodeReportsReadError(t *testing.T) {
	var output bytes.Buffer
	code := RunDecode(&truncatedReader{}, &output)
	if code == 0 {
