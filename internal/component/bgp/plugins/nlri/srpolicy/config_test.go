...
func TestParseConfigRoute_MPLS(t *testing.T) {
	...
	pr, err := parseConfigRoute(content, "192.0.2.1", false)
	if err != nil {
...
func TestParseConfigRoute_IPv6(t *testing.T) {
	...
	pr, err := parseConfigRoute(content, "2001:db8::2", true)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !pr.IsIPv6 {
...
func TestParseConfigRoute_MissingEndpoint(t *testing.T) {
	...
	_, err := parseConfigRoute(content, "192.0.2.1", false)
	if err == nil {
...
func TestParseConfigRoute_EmptyContent(t *testing.T) {
	t.Parallel()
	_, err := parseConfigRoute(nil, "192.0.2.1", false)
	if err == nil {
