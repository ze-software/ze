...
func (r *truncatedReader) Read(p []byte) (int, error) {
	...
	r.sent = true
	return copy(p, "decode nlri "), nil
}

// TestRunFlowSpecDecodeReportsReadError checks that a read failure is not
// reported as a clean end of input.
//
// VALIDATES: RunFlowSpecDecode returns non-zero and writes the read error when
// the input reader fails part way through a line.
// PREVENTS: a truncated decode stream exiting 0 as if every request decoded.
func TestRunFlowSpecDecodeReportsReadError(t *testing.T) {
	var output bytes.Buffer
	code := RunFlowSpecDecode(&truncatedReader{}, &output)
	if code == 0 {
