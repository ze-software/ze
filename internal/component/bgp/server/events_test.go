...
func TestStateChangeRespectsEncoding(t *testing.T) {
	...
	textState := string(format.AppendStateChange(nil, &peer, rpc.SessionStateUp, "", nil, plugin.EncodingText))
	jsonState := string(format.AppendStateChange(nil, &peer, rpc.SessionStateUp, "", nil, plugin.EncodingJSON))
