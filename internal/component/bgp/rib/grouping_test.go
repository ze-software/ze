...
// which returns AttributeGroup and ASPathGroup and is covered by the two-level tests
// below. Single-level grouping no longer exists, so no live behavior loses coverage.
