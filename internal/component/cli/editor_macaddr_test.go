...
// `unit 0 ipv4 address` (address lives under the ipv4 container), and that container
// is now present on every OS, so the write-through set must succeed and the change
// file must re-parse cleanly on the host OS — including darwin, where ipv4 used to be
// pruned and `set ... unit 0 address ...` produced an unparseable change file.
//
// VALIDATES: editor write-through of interface mac-address and unit ipv4 address.
...
func TestWriteThroughInterfaceAddressRoundTrip(t *testing.T) {
	...
	// mac-address (value contains ':') and the unit ipv4 address (value contains '/').
