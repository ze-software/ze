...
func TestDashboardSortAddressByValue(t *testing.T) {
	...
	got := addressesOf(sortDashboardPeers(peers, sortColumnAddress, true))
	want := []string{"9.0.0.1", "10.0.0.9", "10.0.0.10", "2001:db8::1"}
	assertOrder(t, "address ascending", got, want)

	got = addressesOf(sortDashboardPeers(peers, sortColumnAddress, false))
...
func TestDashboardSortUptimeByDuration(t *testing.T) {
	...
	got := addressesOf(sortDashboardPeers(peers, sortColumnUptime, true))
...
func TestDashboardSortStateByProgress(t *testing.T) {
	...
	got := addressesOf(sortDashboardPeers(peers, sortColumnState, true))
...
func TestDashboardSortTiesAreTotal(t *testing.T) {
	...
	for _, ascending := range []bool{true, false} {
		want := addressesOf(sortDashboardPeers(first, sortColumnState, ascending))
		got := addressesOf(sortDashboardPeers(second, sortColumnState, ascending))
	...
	up := addressesOf(sortDashboardPeers(first, sortColumnState, true))
	down := addressesOf(sortDashboardPeers(first, sortColumnState, false))
