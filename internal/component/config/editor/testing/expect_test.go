...
func TestExpectCompletionCount(t *testing.T) {
	...
	comps := []editor.Completion{{Text: "a"}, {Text: "b"}, {Text: "c"}}
	err := CheckExpectation(exp, &MockState{completions: comps})
	assert.NoError(t, err)

	comps = []editor.Completion{{Text: "a"}, {Text: "b"}}
	err = CheckExpectation(exp, &MockState{completions: comps})
	assert.Error(t, err)
}
...
func TestExpectCompletionEmpty(t *testing.T) {
	...
	err = CheckExpectation(exp, &MockState{completions: []editor.Completion{}})
	assert.NoError(t, err)

	err = CheckExpectation(exp, &MockState{completions: []editor.Completion{{Text: "x"}}})
	assert.Error(t, err)
}
...
func (m *MockState) Completions() []editor.Completion                 { return m.completions }
func (m *MockState) GhostText() string                                { return m.ghostText }
func (m *MockState) ValidationErrors() []editor.ConfigValidationError { return m.validationErrors }
