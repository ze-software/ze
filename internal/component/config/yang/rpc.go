...
//
// Description and LongHelp are the two help texts a command node also declares
...
// empty LongHelp means nobody has written the explanation yet.
type RPCMeta struct {
...
func ExtractRPCs(loader *Loader, moduleName string) []RPCMeta {
	...
	for _, rpc := range mod.RPC {
		meta := RPCMeta{
			...
			LongHelp:    GetHelpExtension(rpc.Exts()),
		}
