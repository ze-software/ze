...
//
// VALIDATES: Core YANG modules (extensions, types) load without errors.
// PREVENTS: Syntax errors in YANG files breaking startup.
// NOTE: ze-bgp-conf, ze-hub-conf, ze-plugin-conf are in separate schema packages,
// loaded via init() registration (LoadRegistered), not LoadEmbedded.
