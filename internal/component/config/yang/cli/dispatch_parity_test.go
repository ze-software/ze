...
// VALIDATES: handover §5a -- yangCommands stays in sync with the dispatch
...
// without updating yangCommands, leaving Meta.Subs and the unknown-command
// suggestion list stale.
func TestDispatchParity(t *testing.T) {
	// help and its flag aliases are handled by the switch but are not
	// listed in yangCommands (help is universal, per register.go).
	...
	gotCommands := append([]string(nil), yangCommands...)
	sort.Strings(gotCommands)

	for _, c := range wantCommands {
		if !slices.Contains(gotCommands, c) {
			t.Errorf("switch case %q has no matching entry in yangCommands", c)
		}
	}
	for _, c := range gotCommands {
		if !slices.Contains(switchCases, c) {
			t.Errorf("yangCommands entry %q has no matching switch case in Run", c)
		}
	}
}
