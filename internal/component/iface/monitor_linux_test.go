...
func TestHandleLinkUpdate_StateChange(t *testing.T) {
	...
	if bus.events[0].Topic != TopicCreated {
		t.Fatalf("first event topic = %q, want %q", bus.events[0].Topic, TopicCreated)
	}

	// Second event (same index, OperUp): state change to up.
	m.handleLinkUpdate(lu)
	if len(bus.events) != 2 {
		t.Fatalf("expected 2 events, got %d", len(bus.events))
	}
	if bus.events[1].Topic != TopicUp {
		t.Errorf("second event topic = %q, want %q", bus.events[1].Topic, TopicUp)
	}

	// Third event (same index, OperDown): state change to down.
	...
	if bus.events[2].Topic != TopicDown {
		t.Errorf("third event topic = %q, want %q", bus.events[2].Topic, TopicDown)
	}
}
...
func TestHandleLinkUpdate_Delete(t *testing.T) {
	...
	if bus.events[1].Topic != TopicDeleted {
		t.Errorf("delete topic = %q, want %q", bus.events[1].Topic, TopicDeleted)
	}

	// Re-create with same index after delete: should be TopicCreated again.
	lu.Header = unix.NlMsghdr{Type: unix.RTM_NEWLINK}
	m.handleLinkUpdate(lu)
	if len(bus.events) != 3 {
		t.Fatalf("expected 3 events, got %d", len(bus.events))
	}
	if bus.events[2].Topic != TopicCreated {
		t.Errorf("re-create topic = %q, want %q", bus.events[2].Topic, TopicCreated)
	}
}
...
func TestAddrUpdateToTopic(t *testing.T) {
	...
	tests := []struct {
		...
		{"addr added", true, TopicAddrAdded},
		{"addr removed", false, TopicAddrRemoved},
	}
...
func TestPayloadVlanUnit(t *testing.T) {
...
func TestNewMonitorNilBus(t *testing.T) {
	// VALIDATES: NewMonitor rejects nil bus.
	// PREVENTS: Deferred panic deep in goroutine.
	_, err := NewMonitor(nil)
	if err == nil {
		t.Error("expected error for nil bus, got nil")
	}
}
...
func TestHandleLinkUpdate_NilAttrs(t *testing.T) {
	...
	m.handleLinkUpdate(lu) // must not panic
	if len(bus.events) != 0 {
		t.Errorf("expected 0 events for nil attrs, got %d", len(bus.events))
	}
}

// nilAttrsLink is a Link implementation that returns nil from Attrs().
...
// Ensure nl package is used (imported by netlink, needed for Header field).
var _ = nl.FAMILY_ALL
