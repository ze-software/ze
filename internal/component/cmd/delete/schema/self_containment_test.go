...
// TestDeleteSchemaHasNoMigratedOwnerCommands enforces ai/rules/plugin-self-containment.md
...
func TestDeleteSchemaHasNoMigratedOwnerCommands(t *testing.T) {
	banned := map[string]string{
		`"ze-delete:bgp-peer"`: "BGP peer removal -> internal/component/bgp/plugins/cmd/peer/schema",
	}
	for token, owner := range banned {
		if strings.Contains(ZeCliDeleteCmdYANG, token) {
			t.Errorf("central delete schema declares owner command %q; move it to %s (see ai/rules/plugin-self-containment.md)", token, owner)
		}
	}
}
