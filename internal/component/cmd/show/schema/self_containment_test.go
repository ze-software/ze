...
func TestShowSchemaHasNoMigratedOwnerCommands(t *testing.T) {
	banned := map[string]string{
		...
		`"ze-show:system-ntp"`:            "NTP status -> internal/plugins/ntp/schema",
		`"ze-show:system-ntp-peers"`:      "NTP peers -> internal/plugins/ntp/schema",
		`"ze-show:system-conntrack"`:      "conntrack -> internal/component/firewall/schema",
		`"ze-show:system-update"`:         "system update -> internal/component/update/schema",
		`"ze-show:system-update-history"`: "update history -> internal/component/update/schema",
		...
		`"ze-show:tcp-check"`:             "TCP check -> internal/component/diag/schema",
		`"ze-show:policy-chain"`:          "policy chain -> internal/component/bgp/plugins/cmd/policy/schema",
		`"ze-show:policy-test"`:           "policy test -> internal/component/bgp/plugins/cmd/policy/schema",
		`"ze-show:config-`:                "config inspection -> internal/component/config/cli/schema",
		`"ze-show:data-`:                  "data store -> internal/component/config/storage/schema",
		`"ze-show:env-`:                   "env vars -> internal/core/env/schema",
		`"ze-show:schema-`:                "schema introspection -> internal/component/config/schema/cli/schema",
		`"ze-show:yang-`:                  "YANG tools -> internal/component/config/yang/schema",
	}
	for token, owner := range banned {
		if strings.Contains(ZeCliShowCmdYANG, token) {
