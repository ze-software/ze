...
// MatchICMPErrorQuotedDestination matches an ICMPv4 error message whose
// QUOTED IPv4 header carries Addr as its destination. It never reads the
// packet's own source or destination.
//
// An ICMP error is generated by whichever router met the problem. The kernel
// delivers it to the session by the header it quotes: tcp_v4_err looks the
// socket up by the quoted 4-tuple. So the quoted destination says which
// session the error is about, and the outer source says nothing. An error ze
// receives is about a packet ze sent, whose destination is the peer.
//
// The lowering reads the quoted destination at a fixed offset, after the same
// 0x45 guard MatchICMPErrorQuotedTCPPort describes. A quoted header carrying
// IP options therefore matches nothing. The lowering does NOT check the quoted
// protocol or the ICMP type. A term MUST carry a MatchICMPErrorQuotedTCPPort
// and a MatchICMPType beside this match, and those two perform the checks.
//
// Addr MUST be a specified IPv4 address, and validateMatch refuses anything
// else: the read is 4 octets, and the unspecified address is no peer.
//
// IPv4 only, for the same reason as MatchIPv4TTLBelow.
//
// Daemon-only. No config leaf produces it, so an operator cannot write it.
type MatchICMPErrorQuotedDestination struct {
	Addr netip.Addr
}
