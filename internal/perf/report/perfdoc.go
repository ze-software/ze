...
> not as absolute performance claims. If performance matters to you, run le perf
...
This document is auto-generated by `+"`le perf report --doc`"+`. To regenerate
with fresh results:

`+"```bash\n"+`# Build all DUT Docker images, then run benchmarks
./le perf run
...
func writeDUTSetup(results []perf.Result, w io.Writer) error {
	...
	if _, err := fmt.Fprintf(w, "All DUTs run in Docker containers on the same host. Each DUT is\n"+ //nolint:errcheck // output
		...
		"and AS 65000 as the local AS. The benchmark tool (le perf send) establishes both\n"+
