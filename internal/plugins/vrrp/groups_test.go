...
func TestBoundaryPriority(t *testing.T) {
	...
	// RFC requirement: RFC5798-5.2.4-2 positive -- a VRRP router backing up a virtual router is configured with a priority in 1-254, the range RFC 5798 Section 5.2.4 mandates (validateGroup groups.go:503)
	...
	// RFC requirement: RFC5798-5.2.4-2 negative -- priority 0 (the relinquishing value) and 255 (the address owner's) are rejected, so a Backup can never run outside the 1-254 range (groups.go:503).
	cases := []struct {
		priority float64
		wantErr  bool
	}{
...
// RFC requirement: RFC5798-5.2.9-1 positive -- an IPv6 group whose FIRST virtual address is the link-local one validates, so its advertisements lead with that address as RFC 5798 Section 5.2.9 requires (validateGroup groups.go:524)
...
// RFC requirement: RFC5798-5.2.9-1 negative -- an IPv6 group whose first virtual address is global is rejected, so ze cannot transmit an advertisement whose first address is not the link-local one (groups.go:524).
...
func TestOwnerAutoDetection(t *testing.T) {
	...
	// RFC requirement: RFC5798-5.2.4-1 positive -- the VRRP router that owns the IPvX address associated with the virtual router runs with priority 255, whatever priority was configured, as RFC 5798 Section 5.2.4 requires (EffectivePriority groups.go:141)
	...
	// RFC requirement: RFC5798-5.2.4-1 negative -- a non-owner is NOT raised to 255; it keeps its configured Backup priority, so 255 marks the address owner alone (groups.go:141)
