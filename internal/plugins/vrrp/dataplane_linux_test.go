...
func TestDataplaneApplyIPv4SetsRecipe(t *testing.T) {
	...
	// RFC requirement: RFC5798-6.4.3-1 positive -- the virtual-MAC macvlan is made the sole ARP responder for the virtual address, so a Master answers ARP requests for it (vmacSysctls dataplane_linux.go:64, parentSysctls dataplane_linux.go:73)
...
func TestDataplaneRestoreOnLastGroup(t *testing.T) {
	...
	// RFC requirement: RFC5798-6.4.3-1 negative -- the sole-responder recipe is not permanent: the parent's ARP knobs are restored on the last group's teardown, so virtual-MAC ARP ownership is scoped to a live virtual router (revertDataplaneSysctls dataplane_linux.go:199)
