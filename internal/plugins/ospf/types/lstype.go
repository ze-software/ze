...
// ASExternal reports whether the LS type is an AS-External LSA: the OSPFv2 Type 5
// (RFC 2328) or any OSPFv3 AS-scope LS Type such as 0x4005 (RFC 5340 sec A.4.2.1). These
// flood AS-wide and live in the LSDB's AS-wide store. The OSPFv2 8-bit types never set the
// scope bits, so for OSPFv2 this is exactly Type 5 (Opaque-AS Type 11 has no scope bits and
// stays false), preserving OSPFv2 store routing.
