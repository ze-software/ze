...
// TestLowerICMPErrorQuotedDestinationReadsTheQuotedAddressUnderAnNfprotoGuard
// proves the quoted-destination match reads the right bytes in the right
// order. The nfproto guard comes first. The quoted version and IHL byte at
// transport offset 8 is then compared against 0x45. A 4-octet read at
// transport offset 24 (8 octets of ICMP header, then 16 into the quoted IPv4
// header) is then compared against the address's 4 bytes. An ICMP error is generated by whichever
// router met the problem, so no outer-source read appears anywhere in the
// expression list.
//
// The expected expression list is asserted whole so that a reordering, a
// dropped guard, or an added outer-source read each fail the test.
func TestLowerICMPErrorQuotedDestinationReadsTheQuotedAddressUnderAnNfprotoGuard(t *testing.T) {
	exprs, err := lowerMatch(inetCtx(nil), firewall.MatchICMPErrorQuotedDestination{
	...
	want := []expr.Any{
		...
		&expr.Cmp{Op: expr.CmpOpEq, Register: 1, Data: []byte{192, 0, 2, 2}},
	}
	if len(rest) != len(want) {
		t.Fatalf("expected %d expressions after the guard, got %d: %#v", len(want), len(rest), rest)
	}
	for i := range want {
		if !reflect.DeepEqual(rest[i], want[i]) {
			t.Errorf("expr[%d] = %#v, want %#v", i, rest[i], want[i])
		}
	}
	for _, e := range exprs {
		p, ok := e.(*expr.Payload)
		...
		if p.Base == expr.PayloadBaseNetworkHeader {
			t.Errorf("the match reads the outer network header at offset %d; it must read only the quoted header", p.Offset)
		}
	}
}

// TestLowerICMPErrorQuotedDestinationRefusesAnAddressItCannotRead is the
// fail-closed half. A zero address and an IPv6 address have no 4-octet form
// at quoted offset 24. The lowering refuses them, and emits no compare against
// bytes that name nothing.
func TestLowerICMPErrorQuotedDestinationRefusesAnAddressItCannotRead(t *testing.T) {
	for _, addr := range []netip.Addr{{}, netip.MustParseAddr("2001:db8::1"), netip.MustParseAddr("0.0.0.0")} {
