...
//	offset 24: quoted IPv4 destination address (8 + 16)
//	offset 28: quoted TCP source port (8 + 20 + 0)
//	offset 30: quoted TCP destination port (8 + 20 + 2)
//
// The port offsets assume a 20-byte quoted header, so the compare before them
// is the quoted version and IHL byte against 0x45 (quotedIPv4HeaderGuard). A
// quoted header carrying IP options fails that compare and the rule stops,
// rather than reading an option byte as a port. Ze sets no IP option on a BGP
// socket, so its own quoted headers are always 20 bytes.
//
...
func lowerICMPErrorQuotedTCPPortMatch(family nftables.TableFamily, port uint16, side firewall.QuotedPortSide) ([]expr.Any, error) {
	...
	return append(quotedIPv4HeaderGuard(family),
...
// lowerICMPErrorQuotedDestinationMatch matches an ICMPv4 error whose quoted
// IPv4 header carries addr as its destination. The layout is the one
// lowerICMPErrorQuotedTCPPortMatch draws. The quoted destination is the 4
// octets at transport offset 24: 8 octets of ICMP header, then 16 into the
// quoted IPv4 header (RFC 791 Section 3.1). That offset holds only for a
// 20-byte quoted header, so the read follows quotedIPv4HeaderGuard.
//
// The quoted protocol at offset 17 is NOT checked here. The
// MatchICMPErrorQuotedTCPPort a term carries beside this match performs that
// check, and a second copy would be a second answer to one question. Nothing
// here reads the packet's own source or destination, on purpose. An ICMP
// error is generated by whichever router met the problem, and the session it
// is about is named by the header it quotes.
//
// addr MUST be a specified IPv4 address. validateMatch
// (internal/component/firewall/validate.go) already refuses any other value.
// The check is repeated here so a caller that bypassed validation fails
// closed, and never compares 4 octets that name nothing.
func lowerICMPErrorQuotedDestinationMatch(family nftables.TableFamily, addr netip.Addr) ([]expr.Any, error) {
	if !addr.Is4() || addr.IsUnspecified() {
		return nil, errIcmpQuotedDestinationNamesNoIPv4Address
	}
	quoted := addr.As4()

	return append(quotedIPv4HeaderGuard(family),
		// The quoted IPv4 header's destination is the peer.
		&expr.Payload{DestRegister: 1, Base: expr.PayloadBaseTransportHeader, Offset: 24, Len: 4},
		&expr.Cmp{Op: expr.CmpOpEq, Register: 1, Data: quoted[:]},
	), nil
}

// quotedIPv4HeaderGuard is the prefix every read of a header QUOTED inside an
// ICMPv4 error shares. It has three parts. The nfproto guard comes first
// (nfprotoGuard carries why). The transport protocol is then ICMP, so the
// transport header base is the ICMP header. The quoted version and IHL byte
// at transport offset 8 is then 0x45, so the fixed offsets after it name the
// fields they claim. A quoted header carrying IP options fails the last
// compare and the rule stops.
//
// One helper rather than two copies, so the two quoted-header lowerings cannot
// drift apart on which guard precedes the read.
func quotedIPv4HeaderGuard(family nftables.TableFamily) []expr.Any {
	return append(nfprotoGuard(family, unix.NFPROTO_IPV4),
		&expr.Meta{Key: expr.MetaKeyL4PROTO, Register: 1},
