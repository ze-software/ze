// Design: docs/architecture/isis/isis-6-lsdb.md -- the Link-State Database store.
...
type ReceiveResult struct {
	...
	// nothing.
	Stored bool
	// OwnConflict reports that the received LSP bears one of THIS system's own LSP
	// IDs and is not the copy this system originated -- a purge of our own LSP, a
	// higher sequence we never issued, or an equal sequence with a different
	// checksum. The LSP is NOT stored (ISO/IEC 10589 clause 7.3.16.4 c-1: "shall
	// not overwrite with the received LSP"). The engine answers it by raising the
	// origination sequence floor above ConflictSequence and re-originating
	// (clause 7.3.16.1 / 7.3.16.4 c-2..c-4); nothing else resolves it, because
	// this system is the only authority for its own LSP IDs.
	OwnConflict bool
	// ConflictSequence is the sequence the other system claimed for our LSP ID.
	// Meaningful only when OwnConflict is set.
	ConflictSequence types.SequenceNumber
}
...
//
// own marks an LSP whose System ID is ours -- this node's router LSP or a
// pseudo-node it originated as DIS. Such an LSP is NEVER stored unless it is the
// exact copy this node originated: clause 7.3.16.4 c-1 forbids overwriting an own
// LSP with a received one, and clause 7.3.16.4 a-2 forbids retaining a purge for
// a source this database holds nowhere. The result then carries OwnConflict and
// the claimed sequence, and the engine answers by re-originating above it. See
// ownConflictResult for the clause text and for the one state excluded from it.
//
...
func (d *LSDB) Receive(level Level, lsp *packet.LSP, raw []byte, own bool) ReceiveResult {
	...
	if own {
		if res, conflict := ownConflictResult(existing, lsp); conflict {
			return res
		}
	}
	if ok {
...
// ownConflictResult decides whether a received LSP bearing one of THIS system's
// own LSP IDs is a conflict this system must answer by re-originating, and
// returns the ReceiveResult to give the caller when it is. existing is the
// currently stored entry for the LSP ID, or nil.
//
// ISO/IEC 10589 clause 7.3.16.4, quoted from the identical text of RFC 1142
// section 7.3.16.4 (the IETF republication of the same protocol specification;
// the ISO text itself is not redistributable, see iso/README.md). On receipt of
// an LSP from source S with zero Remaining Lifetime:
//
...
//
// Clause 7.3.16.2 adds the equal-sequence case in the same terms: "If the LSP is
// in the current set of LSPs generated by the local system then the IS shall
// change the LSP's sequence number to be the next number greater than that of the
// received LSP and regenerate the LSP."
//
// The decision keys on the SEQUENCE, which is what both clauses key on:
//
//   - No entry (case a). Never retained, whatever its Remaining Lifetime: a
//     foreign copy of our own LSP is not evidence about our own state. Reported
//     as a conflict so the sequence floor still rises above the claim -- the
//     network holds that value even though this database does not.
//   - Above our sequence. The claim of clause 7.3.16.1, and case c) when it
//     arrives as a purge. A conflict.
//   - At our sequence, and not the copy we originated (a purge against our
//     un-expired LSP, or a differing checksum). Case c) and clause 7.3.16.2. A
//     conflict. An IDENTICAL arrival is the ordinary echo of our own flood and is
//     acknowledged, not answered.
//   - Below our sequence. NOT a conflict: there is nothing to move above, and
//     clause 7.3.16.4 b-3-i already answers it by setting SRM on the arrival
//     circuit so the sender receives the copy it is behind on. Routing it here
//     would replace that answer with a pointless sequence bump.
//
// The other deliberate exclusion is an own entry ALREADY purged here. Clause
// 7.3.16.4 c) is conditioned on "an un-expired LSP from S ... in memory", and
// this system purges its own LSPs on purpose: a fragment the state shrank past, a
// pseudo-node yielded on losing the DIS role, or an ID suspended after a sequence
// wraparound. Re-originating on the echo of a purge we ourselves issued would
// resurrect a withdrawal, and in the wraparound case would fight the suspension.
func ownConflictResult(existing *Entry, lsp *packet.LSP) (ReceiveResult, bool) {
	conflict := ReceiveResult{
		Freshness:        Older, // we keep (or hold) our own view; nothing was stored.
		Stored:           false,
		OwnConflict:      true,
		ConflictSequence: lsp.SequenceNumber,
	}
	switch {
	case existing == nil:
		return conflict, true
	case existing.IsPurged():
		return ReceiveResult{}, false
	case lsp.SequenceNumber > existing.sequence:
		return conflict, true
	case lsp.SequenceNumber < existing.sequence:
		return ReceiveResult{}, false
	case lsp.RemainingLifetime.IsPurge() || lsp.Checksum != existing.checksum:
		return conflict, true
	default:
		return ReceiveResult{}, false
	}
