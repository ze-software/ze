...
func init() {
	Register("plugin/prefix-filter-chain-order", p12ObserveDriver("test-chain", p12PrefixChainOrder))
	Register("plugin/prefix-filter-entry-order", p12ObserveDriver("test-prefix-order", p12PrefixEntryOrder))
	Register("plugin/prefix-filter-modify-partial", p12ObserveDriver("test-modify-partial", p12WaitRoutes(2)))
	Register("plugin/prefix-filter-plain", p12ObserveDriver("test-plain", p12WaitRoutes(1)))
	Register("plugin/prefix-filter-reject", p12ObserveDriver("test-prefix-reject", p12PrefixReject))
	Register("plugin/prefix-filter-shortform", p12ObserveDriver("test-shortform", p12WaitRoutes(1)))
	Register("plugin/prefix-warn-only-drops-nlri", p12ObserveDriver("prefix-drop-test", p12WarnOnlyDropsNLRI))
	Register("plugin/prefixsid-ebgp-discard-single-walk", p12SubscribedDriver("shutdown-after-up", []string{eventUpdate}, p12RouteServerReplay))
	Register("plugin/quiesce-barrier", p12ObserveDriver("quiesce-barrier", p12QuiesceBarrier))
	Register("plugin/rbac-web-config-deny", p12WebRBAC)
	Register("plugin/reactor-bus-subscribe", p12SubscribedDriver("bus-test", []string{eventState}, p12BusSubscribe))

	Register("plugin/redistribute-as112-announce", p12ObserveDriver("as112-announce-test", p12AS112Announce))
	Register("plugin/redistribute-as112-community", p12ObserveDriver("as112-community-test", p12AS112Single("request fakeas112 emit add asn 112 community nopeer")))
	Register("plugin/redistribute-as112-not-imported", p12ObserveDriver("as112-nocfg-test", p12RedistributeNotConfigured("fakeas112")))
	Register("plugin/redistribute-as112-origin-ebgp", p12ObserveDriver("as112-ebgp-test", p12AS112Single("request fakeas112 emit add asn 112")))
	Register("plugin/redistribute-as112-withdraw", p12ObserveDriver("as112-withdraw-test", p12AS112Withdraw))

	p12RegisterFilterFixtures()

	Register("plugin/redistribute-l2tp-announce", p12ObserveDriver("l2tp-announce-test", p12L2TPAnnounce))
	...
	Register("plugin/relay-withdraw-reflector", p12ObserveDriver("relay-rr", p12RelayWithdraw(true)))
	Register("plugin/reload-hello-interval", p12ObserveDriver("reload-hello-interval-test", p12ReloadL2TP("hello-interval", 45, 90)))
	Register("plugin/reload-hello-retries", p12ObserveDriver("reload-hello-retries-test", p12ReloadL2TP("hello-retries", 1, 4)))
...
func p12ObserveDriver(name string, scenario p12Scenario) Driver {
...
func p12RequireDone(ctx context.Context, plugin *sdk.Plugin, command string) error {
...
// p12PeerCounterAttempts bounds the wait for a peer counter to reach its minimum.
const p12PeerCounterAttempts = 40

func p12WaitPeerCounter(ctx context.Context, plugin *sdk.Plugin, selector, field string, minimum int) bool {
	return Poll(ctx, p12PeerCounterAttempts, 250*time.Millisecond, func() bool {
...
func p12Quiesce(ctx context.Context, plugin *sdk.Plugin) error {
	err := p12RequireDone(ctx, plugin, "request quiesce")
	return err
}

// p12SettleDelay is the pause between two settle cycles.
const p12SettleDelay = 200 * time.Millisecond

func p12Settle(ctx context.Context, cycles int) {
	Poll(ctx, cycles+1, p12SettleDelay, func() bool { return false })
}
