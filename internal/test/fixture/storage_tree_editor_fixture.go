...
`, port)
	...
	clientEnv := extra1Environment(map[string]string{"ZE_CONFIG_DIR": clientDir, "ZE_SSH_HOST": "127.0.0.1", "ZE_SSH_PORT": sshPort, "ZE_SSH_USERNAME": "admin", "ZE_SSH_PASSWORD": "testpass", "TERM": "xterm", "NO_COLOR": "1"})
	...
	if !bytes.Contains(committed, []byte("2.2.2.2")) {
		return fmt.Errorf("successful editor commit did not update explicit source: %s", committed)
	}
	if err := storageAssertActive(path, "2.2.2.2"); err != nil {
		return err
	}
	...
	external := bytes.ReplaceAll(committed, []byte("2.2.2.2"), []byte("9.9.9.9"))
	...
	fmt.Fprintln(os.Stdout, "OK: tree login, editor commit, restart and external conflict")
	return nil
}

func storageAssertActive(path, routerID string) error {
	...
	if !bytes.Contains(active, []byte(routerID)) {
		return fmt.Errorf("active config does not hold router-id %s: %s", routerID, active)
	}
	return nil
}

func storageEditorConflict(ctx context.Context, environment []string, path string, external []byte) error {
	command := exec.CommandContext(ctx, "ze", "config", "edit", filepath.Base(path))
