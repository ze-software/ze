...
func leNamespaceDispatch(ctx context.Context) error {
	...
	// Two namespaces are probed: `spec`, whose members come from one flattened
	// lifecycle beside separate tools, and `go`, whose members are all tools.
	for _, probe := range []struct {
		...
		if bare.code != 0 {
			return fmt.Errorf("le-namespace-dispatch: bare `le %s` exited %d, want 0: %q", probe.token, bare.code, bare.stderr)
		}
		listing := bare.stdout + bare.stderr
		for _, held := range probe.held {
			if !strings.Contains(listing, held) {
				return fmt.Errorf("le-namespace-dispatch: bare `le %s` did not name %q: %q", probe.token, held, listing)
			}
		}
	}
