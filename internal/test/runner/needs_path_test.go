...
// `make ze-gokrazy-deps` and a fresh checkout has no way to satisfy the test.
func TestParseCIOptionNeedsPath(t *testing.T) {
	t.Run("absent-skips-with-reason-and-hint", func(t *testing.T) {
		rec, err := newNeedsPathFixture(t, "option=needs-path:value=gokrazy/modcache/github.com/rtr7:hint=make ze-gokrazy-deps")
		...
		assert.Contains(t, rec.SkipReason, "make ze-gokrazy-deps",
