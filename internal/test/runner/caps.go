...
const skipReasonNetnsLink = "option=netns-link (needs the per-test netns launch mode; run via ./le test qemu netns-test, or ./le test qemu netns-test on Linux)"
...
// shapes of that in the 2026-07-25 `./le test qemu all-tests` run, which
...
// actually run these tests (`./le test qemu all-tests` never will).
