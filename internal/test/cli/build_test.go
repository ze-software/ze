...
// TestBuildZeNoBuild verifies that buildZe honors LE_TEST_NO_BUILD the same way
...
func TestBuildZeNoBuild(t *testing.T) {
	...
	require.Error(t, err, "buildZe must fail when LE_TEST_NO_BUILD is set but bin/ze is missing")
	...
	require.NoError(t, err, "buildZe must reuse a pre-built bin/ze under LE_TEST_NO_BUILD")
