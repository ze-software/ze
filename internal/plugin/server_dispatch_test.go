...
func TestDispatchCommandToPlugin(t *testing.T) {
	...
	proc := NewProcess(PluginConfig{Name: "test-dispatch"})
	proc.mu.Lock()
	proc.engineConnA = NewPluginConn(engineSide, engineSide)
	proc.mu.Unlock()

	d := NewDispatcher()
	d.Register("test command", func(_ *CommandContext, args []string) (*Response, error) {
		return &Response{
			Status: "done",
...
func TestDispatchCommandNotFound(t *testing.T) {
	...
	proc := NewProcess(PluginConfig{Name: "test-dispatch-notfound"})
	proc.mu.Lock()
	proc.engineConnA = NewPluginConn(engineSide, engineSide)
	proc.mu.Unlock()
...
func TestDispatchCommandPluginError(t *testing.T) {
	...
	proc := NewProcess(PluginConfig{Name: "test-dispatch-error"})
	proc.mu.Lock()
	proc.engineConnA = NewPluginConn(engineSide, engineSide)
	proc.mu.Unlock()

	d := NewDispatcher()
	d.Register("failing command", func(_ *CommandContext, _ []string) (*Response, error) {
		return &Response{
			Status: "error",
...
func TestDispatchCommandEmptyCommand(t *testing.T) {
	...
	proc := NewProcess(PluginConfig{Name: "test-dispatch-empty"})
	proc.mu.Lock()
	proc.engineConnA = NewPluginConn(engineSide, engineSide)
	proc.mu.Unlock()
...
func TestDispatchCommandDirectBridge(t *testing.T) {
	...
	d := NewDispatcher()
	d.Register("bridge test", func(_ *CommandContext, _ []string) (*Response, error) {
		return &Response{
			Status: "done",
	...
	proc := NewProcess(PluginConfig{Name: "test-direct-bridge"})
