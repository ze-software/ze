...
// Related: server_startup.go — JSON-mode handleProcessStartupRPC
...
func (s *Server) handleTextProcessStartup(proc *Process, tcA, tcB *rpc.TextConn) {
	name := proc.Name()
	...
	s.progressThroughStages(proc, name, stageProgression{
		...
		deliver: func(p *Process) { s.deliverConfigText(p, tcB) },
	})
	...
	s.progressThroughStages(proc, name, stageProgression{
		...
		deliver: func(p *Process) { s.deliverRegistryText(p, tcB) },
	})
	...
	// --- Final: Ready → Running ---
	if !s.stageTransition(proc, name, StageReady, StageRunning) {
		return
	}
...
func (s *Server) deliverConfigText(proc *Process, tcB *rpc.TextConn) {
...
func (s *Server) deliverRegistryText(proc *Process, tcB *rpc.TextConn) {
