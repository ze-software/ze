...
- [ ] [MUST] PE advertising ISD route must attach export BGP Route Target Extended Community of the associated routing instance (Section 3.3.1)
- [ ] [MUST] PE advertising ISD route must use IPv6 address of PE as nexthop in MP_REACH_NLRI (Section 3.3.1)
- [ ] [MUST] ISD route update must have a prefix SID attribute (Section 3.3.1)
- [ ] [MUST] PE withdrawing ISD route must attach export BGP Route Target Extended Community (Section 3.3.2)
- [ ] [MUST] DSD address in NLRI must be a unique PE identifier (Section 3.3.4)
- [ ] [MUST] PE announcing DSD route must attach a BGP MUP Extended Community (Section 3.3.4)
- [ ] [MUST] PE advertising DSD route must use IPv6 address of PE as nexthop in MP_REACH_NLRI (Section 3.3.4)
- [ ] [MUST] DSD route update must have a prefix SID attribute (Section 3.3.4)
- [ ] [MUST] BGP speaker announcing T1ST must attach a BGP MUP Extended Community (Section 3.3.5)
- [ ] [MUST] MUP Controller must set nexthop of T1ST route to the controller address (Section 3.3.7)
- [ ] [MUST] Controller must announce T1ST route using AFI of the route and SAFI BGP-MUP to all BGP speakers in SRv6 domain (Section 3.3.7)
- [ ] [MUST] Controller must attach Route Target Extended Community of routing instances in the PE for T2ST (Section 3.3.10)
- [ ] [MUST] Controller must set nexthop of T2ST route to MUP Controller address (Section 3.3.10)
- [ ] [MUST] Unknown Route Types for supported Architecture Types must be silently ignored (Section 3.1)
- [ ] [MUST] Receiver must ensure ISD Address field value is address of originator of locator in prefix SID attribute (Section 3.3.3)
- [ ] [MUST] On MP_UNREACH_NLRI, receiver must delete withdrawn ISD route from routing instance table (Section 3.3.3)
- [ ] [MUST] Receiver must ensure DSD nexthop in MP_REACH_NLRI is identical to originator of locator in prefix SID attribute (Section 3.3.6)
- [ ] [MUST] On MP_UNREACH_NLRI, receiver must delete withdrawn DSD route from routing instance table (Section 3.3.6)
- [ ] [MUST] PE receiving T1ST routes in MP_UNREACH_NLRI must delete all routes from associated routing instance (Section 3.3.9)
- [ ] [MUST] PE must handle T2ST without MUP Extended Community as treat-as-withdraw (Section 3.3.12)
- [ ] [MUST] ISD with prefix length exceeding max for AFI: treat-as-withdraw per RFC 7606 (Section 3.1.1)
- [ ] [MUST] Speaker must skip malformed NLRIs and continue processing rest of Update message (Section 3.1.1)
- [ ] [MUST] DSD with wrong address size for AFI: treat-as-withdraw per RFC 7606 (Section 3.1.2)
- [ ] [MUST] T1ST with prefix length exceeding max for AFI: treat-as-withdraw per RFC 7606 (Section 3.1.3)
- [ ] [MUST] T1ST with TEID=0: treat-as-withdraw per RFC 7606 (Section 3.1.3.1)
- [ ] [MUST] T1ST with invalid Endpoint Address Length (not 32 or 128): treat-as-withdraw (Section 3.1.3.1)
- [ ] [MUST] T1ST with invalid Source Address Length (not 0, 32, or 128): treat-as-withdraw (Section 3.1.3.1)
- [ ] [MUST] T2ST with Endpoint Length exceeding max for AFI: treat-as-withdraw (Section 3.1.4)
- [ ] [MUST] T2ST with TEID=0: treat-as-withdraw per RFC 7606 (Section 3.1.4.1)
- [ ] [MUST] T1ST NLRI architecture field MUST be encoded as specified for 3gpp-5g; otherwise treat-as-withdraw (Section 3.1.3.1)
- [ ] [MUST] T2ST NLRI architecture field MUST be encoded as specified for 3gpp-5g; otherwise treat-as-withdraw (Section 3.1.4.1)
- [ ] [MUST] ISD/DSD without prefix SID attribute: treat-as-withdraw (Section 3.3.3, 3.3.6)
- [ ] [MUST] Nexthop/locator mismatch on ISD/DSD: treat-as-withdraw (Section 3.3.3, 3.3.6)
- [ ] [MUST] PE and MUP Controller MUST establish a BGP session to exchange BGP-MUP NLRIs for both IPv4 and IPv6 AFIs (Section 3.3)
- [ ] [MUST] ISD prefix SID function MUST be GTP4.E if BGP AFI is IPv4, or MUST be GTP6.E if BGP AFI is IPv6 (Section 3.3.1)
- [ ] [MUST] When withdrawing DSD route, BGP speaker MUST attach a BGP MUP Extended community of the associated routing instance (Section 3.3.5)
- [ ] [MUST] Controller MUST advertise the withdrawal of the Type 1 ST route (Section 3.3.8)
- [ ] [MUST] Controller MUST advertise the Type 1 ST route with Destination prefix, TEID, QFI, Endpoint Address, and optionally Source Address (Section 3.3.7)
- [ ] [MUST] PE receiving T1ST in MP_UNREACH_NLRI without Source address MUST delete all matching T1ST routes with different Source addresses (Section 3.3.9)
- [ ] [MUST] Controller MUST advertise the withdrawal of the Type 2 ST route (Section 3.3.11)
- [ ] [SHOULD] Receiver of ISD routes should ignore nexthop in MP_REACH_NLRI and use prefix SID locator instead (Section 3.3.3)
- [ ] [SHOULD] Controller should attach Route Target Extended Community which PEs are importing for T1ST (Section 3.3.7)
- [ ] [SHOULD] PE should use received Tunnel Endpoint Address in T1ST NLRI as key to lookup associated ISD route (Section 3.3.9)
- [ ] [SHOULD] Receiver of DSD routes should ignore the nexthop in MP_REACH_NLRI attribute (Section 3.3.6)
- [ ] [SHOULD] PE receiving T1ST routes should ignore the received nexthop in MP_REACH_NLRI (Section 3.3.9)
- [ ] [SHOULD] PE should generate forwarding SID for GTP4/6.E based on SRv6 MUP procedures (Section 3.3.9)
- [ ] [SHOULD] If PE cannot generate prefix SID, it should mark the received T1ST route as invalid (Section 3.3.9)
- [ ] [SHOULD] Receiver of T2ST routes should ignore received nexthop in MP_REACH_NLRI (Section 3.3.12)
- [ ] [SHOULD] PE receiving T2ST without BGP MUP Extended community should consider the route malformed (Section 3.3.12)
- [ ] [SHOULD] When withdrawing T1ST, controller should attach Route Target Extended community for the corresponding Direct segment (Section 3.3.8)
- [ ] [SHOULD] When advertising T2ST, controller should attach BGP MUP Extended community for the Direct segment (Section 3.3.10)
- [ ] [SHOULD] When withdrawing T2ST, controller should attach BGP MUP Extended community and Route Target Extended community (Section 3.3.11)
- [ ] [SHOULD] RFC 5925 authentication should be used where authentication of BGP control packets is needed (Section 4)
- [ ] [SHOULD NOT] PEs and MUP Controller should not establish BGP sessions with untrusted domains without explicit configuration (Section 4)
- [ ] [SHOULD] RFC 5925 procedures should be enforced at untrusted domain boundaries (Section 4)
- [ ] [SHOULD] Establishing BGP sessions over encrypted paths should be considered to protect from eavesdropping (Section 4)
- [ ] [SHOULD] PEs should impose an upper bound on number of routes stored to protect control plane load (Section 4)
- [ ] [MAY] Implementation may log an error when unknown Route Types are ignored (Section 3.1)
- [ ] [MAY] BGP speaker may have local configuration for using a Source address when Source Address Length is 0 in T1ST (Section 3.1.3.1)
- [ ] [MAY] ISD IP prefix may include a gNodeB address connecting to the PE (Section 3.3.1)
- [ ] [MAY] DSD prefix SID function may be End.DT4/6 or End.DX4/6 (Section 3.3.4)
