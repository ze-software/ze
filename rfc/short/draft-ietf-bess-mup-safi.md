...
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.1-1] [MUST] ISD with prefix length exceeding max for AFI: treat-as-withdraw per RFC 7606 (Section 3.1.1) {gap: ParseMUP reads the ISD prefix length and returns an error above 32 for AFI 1 or 128 for AFI 2 (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyISD), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.1-2] [MUST] Speaker must skip malformed NLRIs and continue processing rest of Update message (Section 3.1.1) {gap: ParseMUP validates the whole route type body and returns an error for a malformed one (internal/component/bgp/plugins/nlri/mup/types.go, parseBody), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn, and no code skips it and continues}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.2-1] [MUST] DSD with wrong address size for AFI: treat-as-withdraw per RFC 7606 (Section 3.1.2) {gap: ParseMUP measures the DSD address against the AFI and returns an error for any other size (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyDSD), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3-1] [MUST] T1ST with prefix length exceeding max for AFI: treat-as-withdraw per RFC 7606 (Section 3.1.3) {gap: ParseMUP reads the T1ST prefix length and returns an error above 32 for AFI 1 or 128 for AFI 2 (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyT1ST), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-1] [MUST] T1ST with TEID=0: treat-as-withdraw per RFC 7606 (Section 3.1.3.1) {gap: ParseMUP reads the T1ST TEID and returns an error when it is zero (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyT1ST), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn. On the encode side parseTEIDWithBits still maps an absent TEID to zero bits, which writeT1STData omits from the NLRI (internal/component/bgp/plugins/nlri/mup/encode.go)}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-2] [MUST] T1ST with invalid Endpoint Address Length (not 32 or 128): treat-as-withdraw (Section 3.1.3.1) {gap: ParseMUP reads the T1ST Endpoint Address Length and returns an error for a value other than 32 or 128 (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyT1ST), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-3] [MUST] T1ST with invalid Source Address Length (not 0, 32, or 128): treat-as-withdraw (Section 3.1.3.1) {gap: ParseMUP reads the T1ST Source Address Length and returns an error for a value other than 0, 32 or 128 (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyT1ST), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-6] [MUST] T1ST: if the octets remaining after the mandatory fields are < 0 the mandatory fields exceed the declared Length, the NLRI is malformed and MUST be treated as Treat-as-withdraw (Section 3.1.3.1)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-7] [MUST] T1ST: if exactly 1 octet remains the encoding is invalid, because a valid TLV requires at minimum a Type byte and a Length byte, and MUST be treated as Treat-as-withdraw (Section 3.1.3.1)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-8] [MUST] T1ST: unknown TLV types MUST be ignored for local processing (Section 3.1.3.1)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-9] [MUST] T1ST: unknown TLV types MUST be propagated unchanged when re-advertising the route to other BGP peers (Section 3.1.3.1)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.3.1-10] [MUST] T1ST: any TLV parsing error MUST result in Treat-as-withdraw (Section 3.1.3.1)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.4-1] [MUST] T2ST with Endpoint Length exceeding max for AFI: treat-as-withdraw (Section 3.1.4) {gap: ParseMUP reads the T2ST Endpoint Length and returns an error above 64 for AFI 1 or 160 for AFI 2 (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyT2ST), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.4.1-1] [MUST] T2ST with TEID=0: treat-as-withdraw per RFC 7606 (Section 3.1.4.1) {gap: ParseMUP reads the T2ST TEID and returns an error when a present TEID is zero (internal/component/bgp/plugins/nlri/mup/types.go, parseBodyT2ST), but that parse serves the JSON decode path (DecodeNLRIHex) and the CLI decoder, while the ingress recognizer reads the architecture and route type alone (internal/component/bgp/plugins/nlri/mup/rfc7606.go, RecognizeNLRI), so a received route carrying it is still stored rather than treated as withdrawn}
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.4.1-3] [MUST NOT] T2ST: the Endpoint Length MUST NOT extend beyond the TEID field (Section 3.1.4.1)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.1.5-1] [MUST] A TLV received in a route type for which it is not applicable MUST be ignored (Section 3.1.5)
...
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.3.12-4] [MUST] A PE receiving a Type 2 ST route with a Direct Segment type BGP MUP Extended Community MUST resolve the route using the DSD routes matching that community (Section 3.3.12)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.3.12-5] [MUST NOT] DSD routes MUST NOT be used to resolve Type 2 ST routes that do not carry a Direct Segment type BGP MUP Extended Community (Section 3.3.12)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.3.12-6] [MUST] A PE receiving a Type 2 ST route with an Interwork Segment type BGP MUP Extended Community MUST resolve the route using ISD routes that carry a matching Interwork Segment type BGP MUP Extended Community (Section 3.3.12)
- [ ] [DRAFT-IETF-BESS-MUP-SAFI-3.3.12-7] [MUST] A PE receiving a Type 2 ST route without a BGP MUP Extended Community MUST resolve the route using ISD routes for the default Interwork Segment, that is ISD routes that carry no BGP MUP Extended Community (Section 3.3.12)
