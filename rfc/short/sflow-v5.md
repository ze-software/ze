...
- [ ] [SFLOW-V5-x-1] [MUST] Datagram version field MUST be set to 5 (Datagram Format)
- [ ] [SFLOW-V5-x-2] [MUST] Agent address MUST be a stable IP (e.g., loopback) that uniquely identifies the device across reboots (Agent Architecture)
- [ ] [SFLOW-V5-x-3] [MUST] Agent address + sub_agent_id MUST uniquely identify a sampling entity (Agent Architecture)
- [ ] [SFLOW-V5-x-4] [MUST] Each sub-agent MUST maintain its own sequence number space (Agent Architecture)
- [ ] [SFLOW-V5-x-5] [MUST] Datagram size MUST NOT exceed path MTU (Transport)
- [ ] [SFLOW-V5-x-6] [MUST] Samples MUST NOT be held more than 1 second before sending (Transport)
- [ ] [SFLOW-V5-x-7] [MUST] All structures MUST use XDR encoding: 4-byte alignment, big-endian (Datagram Format)
- [ ] [SFLOW-V5-x-8] [MUST] Variable-length arrays and opaque data MUST be prefixed by a 4-byte count and padded to 4-byte boundary (XDR Encoding)
...
- [ ] [SFLOW-V5-x-10] [MUST] flow_sample MUST include the actual sampling_rate used by the agent (Flow Sample)
- [ ] [SFLOW-V5-x-11] [MUST] sample_pool MUST track total packets seen by the data source (Flow Sample)
- [ ] [SFLOW-V5-x-12] [MUST] Expanded sample types MUST be used when ifIndex exceeds 2^24-1 or 2^30-1 (Expanded Flow/Counter Sample)
- [ ] [SFLOW-V5-x-13] [MUST] Unknown record formats MUST be skipped using the opaque length prefix (Record Wrappers)
...
- [ ] [SFLOW-V5-x-15] [MUST] All counters MUST be cumulative since boot (Counter Polling)
