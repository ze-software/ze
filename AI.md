...
The activity page is the calendar heatmap `/project/activity/` draws, from the
same measurement, written as one standalone document that carries its
stylesheets inside it: a deck embeds it as an iframe, where no link to the
site's assets resolves. `today <date>` pins the window, so a published deck
keeps the year it was presented with.
