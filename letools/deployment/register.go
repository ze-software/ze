...
func init() {
	...
	// set: the listing is rows, and a proof's report is one verdict carrying a
	// log tail. rowsInKeyed cannot choose between them, so the shape says there
	// are no rows and the engine refuses `| count` by name, before a container
