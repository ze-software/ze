...
// VALIDATES: a failed suite derivation is reported by BOTH suite checks.
// PREVENTS: an empty derivation reading as "no suites to check", which would
// disable the checks on exactly the failure they must report. The fixture has
// no `le` to ask, so the derivation fails the way a broken one would.
func TestDriftReportsAFailedSuiteDerivation(t *testing.T) {
	root := t.TempDir()
	...
	for _, iss := range report.Issues {
		if strings.Contains(iss.Message, "could not derive") {
			files[iss.File] = true
		}
	}
	for _, want := range []string{makefileName, functionalTestsDoc} {
		if !files[want] {
			t.Errorf("%s did not report the failed derivation: %v", want, messages(report))
		}
	}
}
