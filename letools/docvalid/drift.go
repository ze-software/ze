...
// functionalGateSuiteTimeout bounds the derivation. This runs inside
// ze-doc-verify, and a wedged `le` would otherwise hold that gate open with no
// output and no deadline.
const functionalGateSuiteTimeout = 60 * time.Second
...
// moved into a program. It followed $(MAKE) delegation and read
// `$(ZE_TEST_RUN) <suite>` out of the recipe, and neither survives a recipe
// that delegates to a program: the derivation goes empty, and an empty
// derivation here is reported as drift in the DOCUMENT, which sends a reader to
// edit prose that was never wrong.
//
...
func functionalGateSuites(root string) []string {
	ctx, cancel := context.WithTimeout(context.Background(), functionalGateSuiteTimeout)
	defer cancel()
	//nolint:gosec // the program is `le` inside the checkout this gate judges
...
// suiteDerivationFailed is what both suite checks report when `le` answered no
// gating suite at all. It names the derivation rather than the document,
// because the document is not what failed.
const suiteDerivationFailed = "could not derive ze-functional-test suites from `le functional --list --json`"
