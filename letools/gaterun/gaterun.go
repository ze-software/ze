...
func Stream(argv []string, dir string, environ []string) int {
	...
	// pipe open. letools/functional runs every suite under `timeout` to prevent
	// this failure.
...
//
// ExitCode is exported because letools/lejob starts its own child.
// letools/lejob tees that child's output to a job log. Both child execution
