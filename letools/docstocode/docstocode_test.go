...
func TestOnlyTheUpdateActionWrites(t *testing.T) {
	...
	// index, each with a check and an update. The mirror's two keep their whole
	// gate names as verbs, because leaction removes `ze-docs-to-code-` and a
	...
	for _, row := range list.Actions {
		switch row.Verb {
		case "check":
			if row.Writes || row.Gate != "" {
				t.Errorf("check is %+v, and no Make target names it", row)
			}
		case "update":
			if !row.Writes || row.Gate != "ze-docs-to-code-update" {
				t.Errorf("update is %+v", row)
			}
		case "ze-doc-index-check":
			if row.Writes {
				t.Errorf("the reverse index's check is %+v, and it writes nothing", row)
			}
		case "ze-doc-index-update":
			if !row.Writes {
		...
		if row.Forks != nil {
			t.Errorf("%s publishes a fork: this area does its work in Go", row.Verb)
		}
	}
}
