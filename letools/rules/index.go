...
func (r IndexReport) Text() string {
	...
	if r.Stale {
		tb.Str("WARNING: ").Str(r.File).Str(" is stale -- run: make ze-rules-index-update\n")
	}
	return tb.String()
}
...
// indexHeader is INDEX.md above its first row.
//
// The generated line still names the Python script, because until the swap both
// halves write the same bytes into the same file.
var indexHeader = []string{
	"# Ze Rules Index",
	"",
	"<!-- GENERATED by scripts/dev/rules_index.py -- do not edit -->",
	"<!-- Regenerate: make ze-rules-index-update -->",
	"",
