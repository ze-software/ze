...
//
// TWO THINGS FAIL CLOSED, and both are changes from
// scripts/codegen/iana_asn.go. A run that parsed no ASN record writes nothing,
// where the script wrote the empty table it had built and reported success: five
// HTTP 200 responses carrying no ASN record would leave an EMPTY seedRIRTable
// committed, and every IRR lookup would fall back to nothing. And the table is
// built in memory and written in one call, where the script streamed into a file
// os.Create had already truncated, so a failure midway left the irr package
...
func tableSource(entries []Entry, generated string) []byte {
	...
	b.Str("// Code generated by scripts/codegen/iana_asn.go; DO NOT EDIT.\n")
