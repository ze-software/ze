...
func TestAPathNoPrefixClaimsIsNotGrouped(t *testing.T) {
	if group, mapped := GroupOf("letools/changed/changed.go"); mapped {
		t.Errorf("letools path claimed by group %q, want no group", group.Name)
	}
}
...
func TestAnUnmappedFileBecomesItsOwnPackageDirectory(t *testing.T) {
	selection := groupFiles([]string{"letools/changed/changed.go", "letools/changed/scope.go"})
	if len(selection.Groups) != 0 {
		t.Errorf("%d groups, want 0: %v", len(selection.Groups), selection.Groups)
	}
	if len(selection.unmapped) != 1 || selection.unmapped[0] != "./letools/changed" {
		t.Fatalf("unmapped is %v, want exactly [./letools/changed]", selection.unmapped)
	}
}
...
func TestAPackageResolutionFailureAnswersAnErrorRatherThanDroppingThePackage(t *testing.T) {
	...
	rec := &recorder{fail: map[string]error{"go list -e -f {{if not .Error}}{{.Dir}}{{end}}" +
		" ./letools/changed": boom}}

	if _, err := (Selector{Root: t.TempDir(), Run: rec.run}).Packages([]string{"./letools/changed"}); err == nil {
...
func TestABrokenModuleAnswersAnErrorRatherThanAnEmptySelection(t *testing.T) {
	...
	writeFile(t, root, "letools/x/x.go", "package x\n")

	_, err := (Selector{Root: root}).Packages([]string{"./letools/x"})
	if err == nil {
