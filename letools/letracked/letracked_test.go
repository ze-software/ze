...
const registerGo = `package main

import (
	_ "github.com/ze-software/ze/letools/alpha"
	_ "github.com/ze-software/ze/letools/beta"
)
`
...
// TestANamedImportIsRefused is the other half: the composition root registers,
// it does not use. An import by name is a tool le calls rather than dispatches.
func TestANamedImportIsRefused(t *testing.T) {
	src := "package main\n\nimport alpha \"github.com/ze-software/ze/letools/alpha\"\n"
	if _, err := ToolImports([]byte(src)); err == nil {
		t.Error("a named import in the composition root was accepted")
	}
}
...
func TestACommandTheBinaryDoesNotOfferIsBroken(t *testing.T) {
	...
	if !strings.Contains(broken[0].Detail, "root command") {
...
func TestABrokenVerdictNamesEveryFinding(t *testing.T) {
	text := Verdict{
		Rev:    "HEAD",
		Areas:  33,
		Broken: []Broken{{Package: "beta", Detail: "registers a root command and cmd/le/register.go does not import it"}},
	}.Text()
...
// with a commit that predates cmd/le. It verifies the third exit code. A run
// that failed to judge a commit must not report that the commit is clean or
// broken.
//
...
func TestACommitWithNoCompositionRootCannotBeJudged(t *testing.T) {
	root := repoRoot(t)
	// The parent of the commit that ADDED cmd/le/register.go. Named as a sha
	// rather than derived, because deriving it means asking git the same
	// question this test is the answer to.
...
func TestTheCheckoutsOwnHeadIsClean(t *testing.T) {
	if testing.Short() {
		t.Skip("builds cmd/le out of a git archive")
	}
	root := repoRoot(t)
