...
// in every shipped flavor (letools/trackedbuild). Thus, the exact Python failure
// cannot occur, but two other failures can occur:
//
//   - A tool package is committed, compiles, and registers a root command, but
//     cmd/le/register.go does NOT blank-import it. The tool is absent from le
//     without a compiler error or test failure. The contract test for cmd/le
//     compares imports with registrations. An unimported package is in neither
//     list.
//   - A tool has an init() that panics or registers under an unexpected name. A
//     compile-only check cannot detect this behavior because it occurs when the
//     binary starts.
//
// This gate builds cmd/le from the commit, RUNS it, and compares three lists. It
// compares the packages imported by the committed composition root, the
// packages registered by the committed tree, and the commands offered by the
// built binary. It builds one package instead of six flavors. This keeps the
// cost low enough to run after a commit.
...
// registerPath is the composition root, relative to the checkout.
const registerPath = "cmd/le/register.go"

// toolsDir is where the tool packages live, relative to the checkout.
const toolsDir = "letools"

// registerCall is the source text that makes a package a tool: it joins the
// shared registry as a root command.
...
// ToolImports returns the sorted short names of the le tool packages that a
// composition root blank-imports.
//
// It REFUSES an import that is not a tool package. It also refuses a named
// import. The composition root registers tools. It does not call them. Either
// form means that the file is no longer a list of tools. The check cannot
// measure a commit against such a list.
func ToolImports(src []byte) ([]string, error) {
	...
	for _, spec := range file.Imports {
		...
		if spec.Name == nil || spec.Name.Name != "_" {
			var tb textbuf.Buffer
			return nil, errors.New(tb.Str(registerPath).Str(" imports ").Str(path).
				Str(" by name: the composition root only registers").String())
		}
...
// root that join the shared registry as root commands.
//
...
func Compare(imported, registering, offered []string) []Broken {
	...
	for _, name := range registering {
		if !importedSet[name] {
			broken = append(broken, Broken{
				Package: name,
				Detail:  "registers a root command and cmd/le/register.go does not import it",
			})
		}
	}
	for _, name := range imported {
		if !registeringSet[name] {
			broken = append(broken, Broken{
				Package: name,
				Detail:  "is imported by cmd/le/register.go and registers no root command",
			})
		}
	}
	...
	if len(offered) != wired {
		...
		broken = append(broken, Broken{
			Package: "le",
			Detail: tb.Str("offers ").Int(int64(len(offered))).Str(" root command(s) and ").
...
// buildAndList builds cmd/le from the extracted tree, runs it, and returns the
// root commands that it offers.
//
...
func buildAndList(ctx context.Context, dest string) ([]string, error) {
	...
	build := exec.CommandContext(ctx, "go", "build", "-o", binary, "./cmd/le") //nolint:gosec // binary is a path inside the scratch tree
	build.Dir = dest
	...
	if err := build.Run(); err != nil {
		var tb textbuf.Buffer
		return nil, errors.New(tb.Str("go build ./cmd/le: ").Str(strings.TrimSpace(complaint.String())).String())
	}
