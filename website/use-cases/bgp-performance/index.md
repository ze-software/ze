...
AMS-IX used ExaBGP as a lightweight BGP route source. bgperf formalised the same idea with tester, target, and monitor containers. Ze can run that setup with `le perf`: Ze supplies the sender and receiver, the target is any BGP speaker you want to test, and the output is a repeatable report instead of a stopwatch and screenshots.
...
<!-- source: ../main/internal/perf/cli/cmd_run.go -- le perf sender, receiver, repeat, JSON, and output flags -->
...
| How long did convergence take? | `convergence-ms`, throughput, and latency percentiles from `le perf` |
...
```
le perf sender  --->  DUT under test  --->  le perf receiver
AS 65001             AS 65000             AS 65002
```

| Node | AS | Played by | Job |
| --- | --- | --- | --- |
| Sender | 65001 | `le perf` | Generate the route set and inject it into the DUT |
...
| Receiver | 65002 | `le perf` | Record when each prefix arrives |

`le perf` matches the prefixes the sender injected against the ones the receiver saw, so it can report convergence, throughput, latency percentiles, withdrawal time, and lost routes. In bgperf terms, the sender is the tester, the DUT is the target, and the receiver is the monitor.
...
```text
./le perf run dut frr
./le perf report test/perf/results/*.json
```
...
The sender and receiver addresses must be valid local addresses in the lab where `le perf` runs. The Docker runner handles that wiring. In a manual lab, bind those addresses on the host or run `le perf` in the right network namespace.

Then render the report.

```text
./le perf report results/frr-100k.json
```
...
`le perf` reports the fields that are easy to lose when the test is driven by an external injector.
...
```text
./le perf send --dut-addr 172.31.0.2 --dut-asn 65000 --routes 100000 --output result.json
```
...
The current `le perf` setup is one sender session and one receiver session. That is enough to test route propagation and DUT table pressure, but not hundreds of independent BGP client sessions in one run.
...
| File | What it proves |
| --- | --- |
| `internal/perf/cli/cmd_run.go` | `le perf send` accepts DUT, sender, receiver, repeat, warmup, JSON, output, family, MP, and batch flags |
...
```text
./le perf run dut "frr bird gobgp"
./le perf report test/perf/results/*.json
```
