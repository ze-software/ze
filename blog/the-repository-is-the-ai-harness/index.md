...
Ze is a network operating system spread over 770+ Go packages. A model can open any one of them in under a second and still have no idea which package a change belongs in, which rule it is about to break, or which test would catch it if it gets that wrong. Nothing in the repository tells it.
...
`ai/PACKAGE-MAP.md` gives one short description for each of those 770+ packages, and that saves a great deal of blind exploration.

Production Go files carry a `// Design:` line near their top, so opening the implementation reveals the document explaining why it exists. Closely connected files also point to each other with `// Detail:`, `// Overview:` and `// Related:` comments. There are 4,600+ of the first kind and 4,300+ of the second.
...
The rule continues in the same way for interoperability, editor behaviour, fleet management and cross-component work. Unit tests on their own are accepted for genuinely internal logic, and the rule lists those cases so the exception cannot be invented on the spot. Everything else owes both kinds. Around 1,900+ `.ci` scenarios and 170 editor `.et` scenarios are what that produces, next to the Go unit tests.
