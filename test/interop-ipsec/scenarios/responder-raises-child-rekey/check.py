#!/usr/bin/env python3
"""Scenario responder-raises-child-rekey: the ORIGINAL RESPONDER raises its own exchange, Ze -> strongSwan.
...
Why scenario child-rekey does not cover it. There Ze is the connection
INITIATOR, whose counter was always right, and reverting the responder fix
leaves child-rekey green. Every other responder scenario has Ze ANSWERING an
exchange charon started (responder-psk, responder-ike-rekey,
responder-accepts-reinit, responder-eap-tls13). This is the only direction
where a responder-role Ze speaks first.

Note: requires the Docker strongSwan interop lab; run under
      `make ze-interop-ipsec-test IPSEC_INTEROP_SCENARIO=responder-raises-child-rekey`.
"""
...
def check():
    ...
    # (the reason scenario child-rekey carries the same note).
