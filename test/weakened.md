...
| Test | Reason |
|------|--------|
| assertAnswerShape | The helper lost its `wantKey` parameter and the head-`key=` assertion that went with it, and gained an assertion that the head states a `type=`. The envelope key stopped being a property of the head: under `type=json` the whole answer is one document and the envelope is INSIDE it, so asserting a head `key=` would hold for one type and be meaningless for the other two. What replaced it is stronger, because a head with no `type=` leaves a consumer unable to read the items that follow, and `checkAnswerType` refuses that shape on the parsing side. The envelope key itself is still asserted where it is a fact: `TestStreamedAndBufferedAnswersAreIdentical` compares the reassembled document against the buffered one over 13 cases, each shape on both sides of the buffering threshold, and an envelope that went missing fails it. |
