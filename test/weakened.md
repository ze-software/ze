...
| Test | Reason |
|------|--------|
| TestProtoNameUnknown | It asserted the defect: that an IP protocol outside the five `protoName` knew was rendered as its DECIMAL NUMBER in a `MatchProtocol`, which the nft backend then refused with `unknown protocol "132"`, aborting the reconcile for every owner. The behaviour it pinned is gone, so the test cannot be kept. `TestComponentToMatchRejectsUnnamedProtocol` replaces it and asserts the refusal instead, and `TestComponentToMatchEveryWireValue` walks all 256 one-octet values so no number can reach a backend as digits. Coverage of the named path grew: `TestComponentToMatchSCTPName` and `TestComponentToMatchEveryCanonicalNumber` cover the five names `ianaProtocolNumbers` already knew and `protoName` did not. |
