...
| Test | Reason |
|------|--------|
| TestFillOverallOrdersByRenderedWidth | The whole test left, because the `overall` way it covered is removed (owner decision, 2026-08-19). It asserted that `fill overall` ordered the remaining columns by the width they render at, and that `reverse` flipped that. Measuring a column means rendering every cell of the answer, so the first row could not be written until the last had been read, which is the one thing a streamed answer cannot do. Bare `fill` and `fill alpha` are untouched and still covered by `TestParsePipeFillWayAndReverse`, `TestDisplayThenFillAlpha`, `TestFillAloneOrdersEveryField`, `TestFillDefaultUsesTheDeclaredOrderForTheRemainder` and `TestFillDefaultAloneMatchesTheBuiltInOrder`. The removal itself gains cover it never had: `TestFillOverallWayWasRemoved` asserts the operator still parses, that `overall` is refused BY NAME, and that the completer stops offering the word. |
