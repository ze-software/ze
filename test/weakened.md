...
| Test | Reason |
|------|--------|
| TestRunEngineStepsEventIgnoresPreSubscriptionDeliveries | DELETED because it asserted the defect. It pinned the rule that an expect=event step ignores any delivery that arrived before the step opened its subscription, which is exactly why the first sa-up of a session could never be observed: the daemon emits it during startup, before the executor reaches step one. The contract is now the opposite, and TestRunEngineStepsEventMatchesADeliveryThatPrecededTheStep asserts it in the same file. Keeping both would pin two contradictory rules. |
| TestEngineEventBufferScrollbackAndPositions | RENAMED and rewritten as TestEngineEventBufferScrollbackAndLiveDelivery. It drove EngineEventBuffer.Len and waitFrom, the position-window pair that existed only to serve the exclusive-subscription step above; both are deleted with it. The replacement covers the scrollback behaviour that survived, over the envelope-matching path that replaced the window. |
| TestRunEngineStepsEventAndStream | Two assertions rather than three, and the one that went was an assertion about the deleted mechanism: it checked that the step dispatched `request subscribe` and `request unsubscribe` around its wait. The executor now declares one startup subscription before Run, so no step subscribes and there is no dispatch to assert. What the test is FOR, that an expect=event and an expect=stream step each match their delivery, is unchanged and still asserted. Discrimination for the new path is carried by TestRunEngineStepsEventRejectsAnotherEventAndABarePayload, which is new in this commit. |
