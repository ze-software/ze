...
| Test | Reason |
|------|--------|
| TestPrintDaemonOutputPrintsWhatTheDaemonRendered | The test left because the function it covered left. `printDaemonOutput` took the whole answer as one string and printed it, which is the behaviour this commit removes: the client no longer collects the answer before showing it. Its replacement is `daemonOutput`, a writer the body streams through, and the property is covered by two tests that assert MORE than the old one did. `TestTheOperatorSeesTheDaemonRenderingWhileItArrives` pins that each piece reaches the terminal as it arrives rather than at the end, which the old whole-string test could not observe. `TestTheTranscriptRecordsWhatTheOperatorSaw` pins that the transcript still records exactly what was shown. The one behaviour worth keeping from the old test, that an empty answer prints `OK` only when the chain named no format, is asserted in both. |
