...
| Test | Reason |
|------|--------|
| TestAllExpandsToEveryActionOfTheTable | Its subject was REPLACED, and the replacement asserts more. The test proved `all` expanded to every action of the group table, which is the defect this commit removes: that expansion is what made `all` compile six package prefixes and report green over the rest of the checkout. It drove a two-action fixture area, so it never read a real argv. `TestAllRunsTheCheckoutThenTheTagGuardedGroups` replaces it on the real command path, asserting the exact argv of each command that reaches the process runner, that they arrive in order, and that the first failure's code is answered. `TestAllSweepsTheCheckoutThenTheTagGuardedGroups` pins the population itself, and `TestAllCoversEveryGoDirectoryOfTheCheckout` refuses one that leaves a Go directory of the checkout out. |
