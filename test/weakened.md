...
| Test | Reason |
|------|--------|
| TestNoLowerLayerAnnotationMovesThePublishedShare | Renamed to `TestNoAnnotationExceptSinglePolarityMovesThePublishedShare`, which asserts over every annotation kind but `single-polarity` as a subtest each, where this asserted over `lower-layer` alone. The property is not about one kind: only `single-polarity` says a requirement IS proven, and every other kind says it is not, so each belongs in the denominator and outside the numerator. The population is READ from `AnnotationKinds()` rather than named, so a kind added tomorrow is held to the rule the day it arrives; a hand list would have covered `lower-layer` and missed `feature-declined`, both of which arrived on 2026-09-03. It keeps the refusal on a kind with zero rows, so a case that would prove nothing still fails. Nothing left the suite. |
