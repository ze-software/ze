...
| Test | Reason |
|------|--------|
| TestLookupPrefixesEmpty | It asserted the defect: that an IRR reply carrying no prefixes returned an empty list with a nil error, which is what let a transient upstream answer overwrite a populated cache entry. `parseReply` now reads the RPSL status line, so the key-not-found, query-ok-zero-results and server-error replies are distinguishable and the last two are errors. `TestLookupPrefixesDoesNotCacheEmptyAnswer` replaces it and asserts the answer is not cached, and the store-side tests `TestRefreshKeepsLastGoodOnEmptyAnswer` and `TestRefreshStoresNothingOnFirstEmptyAnswer` cover what the old test could not: that a populated entry survives. |
| TestRefreshFailureKeepsLastGood | One table case is removed: it fed an empty successful answer and asserted the entry was OVERWRITTEN, which is the behaviour this commit deletes. The remaining case, a hard query error, is unchanged. The removed case is replaced by `TestRefreshKeepsLastGoodOnEmptyAnswer`, which drives the same input through the same producer and asserts the opposite, so the coverage moved rather than left. |
