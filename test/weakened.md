...
| Test | Reason |
|------|--------|
| CreateVLAN | One fake-backend table case is removed, not an assertion about the product: the case created a VLAN on a device the fake did not hold, which netlink answers "not found". Six tests were passing on states no kernel produces, so their SETUP was corrected and this case had no corrected form. The live-kernel replacement is stronger: `config_apply_resolve_integration_linux_test.go` creates VLANs on a real device in a private netns, which is what found that a VLAN inherits its parent's MAC and made the parent's own binding read as ambiguous. |
