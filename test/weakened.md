...
| Test | Reason |
|------|--------|
| TestDashboardSelectedRowKeepsOneStyle | Renamed to `TestDashboardSelectedRowKeepsStateColor`, which the detector reads as a deletion. The rename IS the assertion changing sides. It asserted that the state color survives ON TOP of the selection background, and that every run of the row sits inside that background. The selection no longer paints a background at all, so both halves pinned a shape that is gone. Coverage RISES: the replacement asserts that NO background introducer (SGR 40-47, 100-107) appears on the row, that the row carries the selection marker, and that the state color survives on the selected row AND on an unselected one. The old test never caught the defect that caused this change: light-on-light satisfied it. Its helper `unstyledRuns` searched for the literal background code 46 and asked nothing about the foreground over it. |
| unstyledRuns | Deleted with the assertion it served. It walked the row looking for the literal background code 46, and answered which text sat outside a span carrying it. The selection paints no background now, so the helper measures a property no row has. `backgroundCodes` replaces it and asks the opposite question: which background parameter, over the whole SGR range 40-47 and 100-107, the row sets at all. That is the stronger check, because it fails for ANY background rather than for one hard-coded number. |
