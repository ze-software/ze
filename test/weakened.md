...
| Test | Reason |
|------|--------|
| codes | Not a lost assertion: it was DUPLICATED. `codes` was declared in both `internal/component/doctor/checks_redistribute_test.go` (no build tag) and `checks_mpls_linux_test.go` (`//go:build linux`), so on linux the package failed to typecheck and not one of its tests had built or run since `3c71ff73e`. This commit removes the linux copy and keeps the untagged one, which every platform reaches, with the indexed loop the linux copy carried. Nothing the two helpers asserted is gone, because a helper asserts nothing: the tests that call it now compile, which is strictly more coverage than the zero tests that ran before. |
