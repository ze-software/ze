...
| Test | Reason |
|------|--------|
| TestReplaceCertUpdatesSecrets | One `require` becomes an `assert` because the assertion it guarded no longer ends the test: `runReplaceCert` writes nothing itself now, so the case continues to check both halves of the pair rather than stopping at the first. No case and no assertion left the suite. The behaviour the test was written for gains five new tests around it, each proven red against HEAD: `TestReplaceCertRefusesMismatchedPair`, `TestReplaceCertRefusesUnparseablePEM`, `TestReplaceCertRefusesEmptyFile`, `TestReplaceCertExpiredCertificate` and `TestReplaceCertWritesCertificateThroughTempFile`. |
