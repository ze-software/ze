...
| Test | Reason |
|------|--------|
| generateTestCertDER | A test HELPER, not a test, and the detector reports its name leaving the file. It generated a certificate and discarded the key that signed it, which is why `TestCheckWebTLS_ExpiredCert` was able to pass the literal bytes `key-data` as a key: nothing read the key, so nothing objected. This commit gives `checkWebTLS` a branch that does read it, so the helper had to return both halves. `generateTestCertKey` is now the one primitive and returns the pair; `generateTestCertPEM` wraps it for the PEM callers. No case, assertion, or test left the suite. Its two callers moved to `generateTestCertKey(t, notBefore, notAfter)` unchanged in what they assert, and four tests were added beside them: `TestCheckWebTLS_MatchingPair`, `TestCheckWebTLS_MismatchedPair`, `TestCheckWebTLS_UnreadableKey` and `TestRunChecksReportsUnusableWebTLSPair`. |
