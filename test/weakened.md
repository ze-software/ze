...
| Test | Reason |
|------|--------|
| TestParseTrailingOptsUnknownTokenStops | RENAMED to `TestParseTrailingOptsUnknownTokenErrors` and its expectation INVERTED, on Thomas's instruction of 2026-08-31. It asserted that `parseTrailingOpts` returns no error and empty options for the input `bogus tag a b`, which is the defect: `announce flowspec destination-ipv4 1.1.1.1/32 discard rate-limit 500` dropped `rate-limit 500` and announced a plain discard with nothing said to the operator. The assertion was wrong because no caller hands this function a slice that can legitimately begin with a non-option token, so a silent stop can only lose words the operator typed. The same fixture now asserts `errTrailingOptUnclaimed` and that the message names the offending token, and `TestFlowspecActionAfterDiscardIsRefused` drives the reported command through `handleAnnounceFlowspec` and asserts nothing is dispatched. |
