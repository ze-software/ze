...
| Test | Reason |
|------|--------|
| addNftIntegrationTableWithChain | No coverage left the suite: the helper's body moved one call deep, into `addNftIntegrationTableInFamily` in the same file. The legacy-table proofs need to seed a table in the family its producer wrote -- `ddos-local` in ip, `anomaly-shape6` in ip6 -- and the old helper hardcoded `nftables.TableFamilyINet`. `addNftIntegrationTableWithChain` now passes that family through and is one line, so the detector reads its two `t.Fatalf` guards and its chain branch as gone from that func. Both are unchanged in the function it calls, and its seven call sites reach them through it. Re-run: `make ze-qemu-integration-test`, `internal/plugins/firewall/nft` ok. |
