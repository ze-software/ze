...
| Test | Reason |
|------|--------|
| TestLowerTermInetAddrMatchGuardsOnNFProto | An EXTRACTION, and the test is strictly stronger after it. `lowerTerm` now returns `[][]expr.Any` because a term that writes the network header emits one rule per family, so every caller had to change. The three assertions the checker counts as removed are one `if err != nil { t.Fatalf("lowerTerm: %v", err) }` block per test, moved verbatim into the new `lowerTermOneRule` helper in the same file. That helper then ADDS an assertion none of these tests had, `len(rules) != 1`, which is what pins that a single-family term still lowers to exactly one rule. So each test gained a check and lost none. `check_weakened_tests.py` counts assertions textually inside one function and cannot follow a `t.Helper()`, so an extraction reads to it as a deletion. |
| TestLowerTermSingleFamilyTableOmitsNFProtoGuard | Same extraction, same helper, same net gain. This one also carries the guarantee that no `ip` or `ip6` rule changed shape: it asserts the guard is ABSENT outside an inet table, and it stayed green through the whole change. |
| TestLowerMatchInSetInetAddrGuardsOnNFProto | Same extraction, same helper, same net gain. |