...
| Test | Reason |
|------|--------|
| handshakePeerCerts | Not a weakening: the two assertions MOVED, into the function this helper now calls. `handshakePeerCerts` (`internal/component/web/server_tls_test.go`) used to dial, assert `require.NoError` on the dial and `require.True` on the `*tls.Conn` type assertion, and read `PeerCertificates`. `TestWebServerUpdateTLSCertificate` needs a connection HELD across the rotation, which a helper that closes its own connection cannot give it, so the dial and both assertions were extracted into `dialHeld` and `handshakePeerCerts` became a two-line caller of it. Every caller of `handshakePeerCerts` still runs both assertions, one call deeper. The audit counts assertions per function body, so an extraction reads to it as a removal. Read `dialHeld` directly above it: `require.NoError(t, err)` and `require.True(t, ok, "expected a TLS connection")` are both there. |
