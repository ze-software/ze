...
| Test | Reason |
|------|--------|
| task-forbidden | `reject=stderr:pattern=does not support task-augmented calls` left the file. It asserted the absence of a phrase no current build can write, so it removed no coverage: `callTool` (`internal/component/mcp/streamable_tools.go`) reads `lookupTaskSupport` only for `TaskSupportRequired` and carries no TaskSupportForbidden branch, and the phrase appears in one doc comment (`TaskSupportLevel`, `internal/component/mcp/tools.go`) and in no format string anywhere under `internal`, `pkg` or `cmd`. A refusal returning instead is caught by the positive pair that stays: the probe must answer `code=ok` with a result spelling `"resultType":"complete"`. The reviewer re-ran the file at HEAD 50468ee34 after the removal, PASS in 2.7s. |
