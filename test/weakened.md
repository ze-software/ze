...
| Test | Reason |
|------|--------|
| commitFixtureTree | NOT weakened: its two inline `t.Fatalf` sites and its `IndexUpdate` call were EXTRACTED into `gitFixture`, `commitFixture` and `layFixture`, which the helper now calls in the same order. The extraction exists so `commitFixtureTip` can build a base commit, a tip commit and a working tree from one place, which is what the `HEAD^` obligation tests need: the discrimination gate bills a tag against the commit before it, so its fixtures must carry two commits rather than one. Every assertion still runs, on the same failures, with the same messages. |
