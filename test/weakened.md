...
| Test | Reason |
|------|--------|
| proto_json_tags_test | The file is deleted because it MOVED, and this row was written by reading the destination rather than by remembering it. `scripts/codegen/proto_json_tags_test.py` becomes `scripts/le/proto_json_tags_test.py`, carrying all three of its cases with their assertions unchanged: `test_rewrites_each_declared_field_occurrence` (two occurrences of one field plus a second field, and both original spellings absent afterwards), `test_refuses_generated_tag_count_mismatch` and `test_refuses_proto_without_explicit_names`. Two cases are added that the original could not have had, because the move is what created the risk: `main()` located the repository with `Path(__file__).resolve().parents[2]`, true only in `scripts/codegen`, and it now reads `REPO_ROOT`. None of the three ported cases calls `main()`, so none would have caught a wrong root. `test_the_declared_paths_exist` asserts the two files the module reads are really there, and `test_the_real_proto_declares_names_to_apply` is its non-vacuity partner: the rewriter raises on a proto declaring no `json_name`, so the first case would pass on a proto that had silently stopped matching. Nothing is lost. The subject gained a guard. |
