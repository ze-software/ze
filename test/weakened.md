...
| Test | Reason |
|------|--------|
| TestExtractRadiusConfigProfileAttrClass | DELETED with the `class` enum value it asserted. RFC 2865 Section 5.25 says "The client MUST NOT interpret the attribute locally", and the test pinned the opposite: that a YANG `profile-attribute: class` reached attribute 25 so ze admin auth could read an RBAC profile name out of Class. The behaviour is a conformance defect, so the test was pinning the defect. `profileAttrType` now answers Filter-Id for every input and the enum is gone from `ze-radius-conf.yang`, so the config the test built can no longer be written. `TestExtractRadiusConfigProfileAttrFilterID` covers the surviving path. |
| TestRadiusProfileMappingClass | DELETED for the same reason, one layer down: it asserted `profileAttrType("class")` answers `AttrClass`. That mapping is what RFC 2865 Section 5.25 forbids and it no longer exists. The function's remaining behaviour, that every value including an unknown one answers Filter-Id, is asserted by its sibling in the same file. |
