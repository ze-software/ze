...
| Test | Reason |
|------|--------|
| testBridge | A MOVE, and nothing left the suite. The function is byte-identical, `return newBridge(slogutil.DiscardLogger())`, and it now lives in `helpers_test.go`, which this commit adds beside `bridge_test.go` in the same package. Every caller still calls it, the package type-checks under `go vet` with the full feature tags, and its tests pass. `check_weakened_tests.py` reads one file at a time, so a helper relocated to a sibling file inside one commit is indistinguishable to it from a helper deleted: it sees the body replaced by an empty string in the file it is reading and cannot see the file that gained it. The row exists to say which of the two happened. |