...
| Test | Reason |
|------|--------|
| TestBGPDecodeTextPayloadRendersInNoFormat | Two assertions leave this test and BOTH are re-asserted elsewhere, each in a build that runs them. It read `bgpDecodeLinked`, a build-tag mirror of the decoder seam, and now reads the seam itself with `pluginreg.GetPacketDecoder()`, so nothing reads the mirror any more. Its `ze_bgp` half went with the constant; the `!ze_bgp` half, `bgp_decode_nolink_test.go`, is NOT deleted, because deleting a `_test.go` needs the owner, and the `compile-out` lint pass reports the orphaned constant until he rules. (1) `require.Nil(t, decode, "a build without BGP must expose no decoder")` was the `!ze_bgp` arm. `TestBuildTag_BGP_Absent` (`build_tag_bgp_absent_test.go`, `//go:build !ze_bgp`) already fails on a non-nil `GetPacketDecoder()`, in every `!ze_bgp` build rather than only the `ze_web` ones this file is built for. (2) `require.NotNil(t, decode)` was the `ze_bgp` arm, and it was the ONLY assertion in the tree that a `ze_bgp` build fills the seam: a grep for `GetPacketDecoder` over every `_test.go` found no other. Losing it would have let a broken `bgp/cli` init empty this test and its two siblings in silence. It is replaced by `TestBuildTag_BGP_PresentFillsTheDecoderSeam` (`bgp_decode_link_test.go`, `//go:build ze_bgp`), the positive half of the absent test's seam assertion. Proven to discriminate: removing the blank import of `internal/component/bgp/cli` makes it fail with "hex-packet decoder seam is nil"; restored and re-run green. |
