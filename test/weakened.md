...
| Test | Reason |
|------|--------|
| TestAnswerResultTakesTheRecordPathOnlyWhenNegotiated | Deleted. It asserted the negotiation branch this commit removes. One subtest read the single-line projection a peer that declared nothing used to receive. No peer receives it now. The other subtest read the record sequence, and `TestDispatchCommandAlwaysAnswersRecords` replaces it. That test drives the same bytes through `opDispatchCommand` and `opDispatchCommandArgs`, so two dispatch ops are covered where one was. |
| mustMarshal | Deleted. It was a helper of the test above and had no other caller. Nothing it did left the suite: it marshaled a value for a line the wire no longer carries. |
| TestDispatchCommandToPlugin | Assertion count fell from 5 to 4. The unmarshal check moved into `dispatchAnswerOverSocket`, the shared reader this commit adds. The reader returns an error instead of asserting, and `require.NoError` at the call site is what reads it, so a decode failure still fails the test. |
| TestHandleDispatchCommandRPCPreservesPluginIdentity | Assertion count fell from 6 to 5, for the reason above. The unmarshal check moved into `dispatchAnswerOverSocket` and its failure arrives as the error the test requires to be nil. |
| TestDispatchCommandPluginError | Assertion count fell from 6 to 4. The unmarshal check moved into `dispatchAnswerOverSocket`. The command failure moved from `output.Error` to a Go error, because the record frame carries it on the terminator, so `require.Error` plus `assert.Contains` replace the two field reads. Both halves are still asserted. |
| answer-not-negotiated | Deleted, and replaced by `test/plugin/answer-unconditional.ci`. Its premise was the negotiation this commit removes: it asserted that a plugin declaring no protocol name received the single-line frame. The replacement keeps every other assertion, the byte comparison between the two peers included, and inverts only the frame the silent peer is checked against. |
