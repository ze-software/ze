...
| Test | Reason |
|------|--------|
| countedDigitsFixture | The helper spelled the base-36 length character in front of a counted field's digits, and this commit deletes that character from the wire, so the shape it spelled no longer exists. It carried no assertion of its own: countedNumberFixture and countedTextFixture spell the two fields now, every assertion that read the old shape reads the new one, and TestCountedFieldsAreToldApartByTheirColon adds five cases over the two guards this commit introduces. Coverage moved rather than went. |
