...
| Test | Reason |
|------|--------|
| TestEngineExecuteStringOutput | A RENAME, and the case it covered is still run. It asserted that a command answering with plain text reached the caller as a quoted JSON string, which was AC-2's wording, "Execute with non-JSON output returns string". The owner ruled on 2026-08-19 that a response payload is structured data and that `RawJSON` refuses anything else (`ai/rules/cli.md`), so that assertion now describes a shape the type refuses. The same command, `daemon reload`, is asserted by `TestEngineExecuteSentenceOutput` in the same file, which checks the sentence arrives in a field rather than as a bare string. The behaviour the old name pointed at did not go untested either: `TestEngineExecuteRefusesNonJSONPayload` drives a text payload through the engine and asserts the refusal, and `TestRawJSONRefusesNonJSON` covers the type with the empty and valid-JSON cases beside it. Three tests where there was one. |
