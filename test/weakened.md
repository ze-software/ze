...
| Test | Reason |
|------|--------|
| TestMidResponderEstablishDoesNotWrapTheCounter | One assertion is removed because it pinned the violation: its negative control asserted that finishResponderEstablish set sa.NextMsgID from the PEER's IKE_AUTH id, which RFC 7296 Section 2.2 forbids by giving each end its own request counter. An assertion that a defect is present cannot survive the fix. The test keeps its subject, the 32-bit ceiling, and gains the corrected expectation; the removed claim is replaced by RFC7296-2.2-3's own tagged pair, TestResponderFirstRequestMatchesWhatTheInitiatorExpects positive and this test negative. |
