...
| Test | Reason |
|------|--------|
| TestRFC6793ReceivedAS4AggregatorAccepted | Two assertions left, and the behavior they described is the one this commit removes. They pinned the pre-RFC split: an AGGREGATOR interned with AS_TRANS in it, and the AS4_AGGREGATOR kept beside it in OtherAttrs, uninterpreted. RFC 6793 Section 4.2.3 says that when the AGGREGATOR carries AS_TRANS the AGGREGATOR is ignored and the AS4_AGGREGATOR is taken as the aggregating node, so after this commit there is no second attribute to find and no AS_TRANS to assert. The test keeps the requirement it is tagged for, RFC6793-4.2.3-2, that the pair is ACCEPTED rather than rejected, and its surviving assertion is strictly stronger than the two it replaces: the four-octet aggregating AS is now the interned aggregator itself. Which of the pair the route records is RFC6793-4.2.3-3 through -7, proven positively and negatively in the new `rfc6793_reconstruct_test.go`. |
