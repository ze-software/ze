...
| Test | Reason |
|------|--------|
| dev_setup_test | The file it tested is deleted in this commit. `scripts/dev/dev-setup.py` was replaced by the `setup` subprogram of `le`, so a suite naming its functions has no subject left. Coverage RISES rather than falls, and it moved BEFORE the deletion rather than after. The old suite declared 79 tests; `le` now declares 155 across `scripts/le/*_test.py`, discovered by the same `make ze-unit-test` through a new `scripts/le` root in `pythonTestRoots`. Every class was mapped, not assumed. `process_test.py` holds `TestPrivilegeMode` and `TestRunPrivileged`. `install_test.py` holds `TestAptInstalls`, `TestOSDetect` and the `go install` half. `system_test.py` holds `TestLoopbackAddresses`. `setup_test.py` holds `TestCheckModeExit`, `TestUnsupportedOS`, `TestSmokeCheck` and `TestDevSetupRequiresPinnedStaticcheck`. `devtools_test.py` holds `TestGoplsAnswers`, `TestPyrightAnswers` and `TestApplianceChecksMarker`. The reasoning in each docstring came across with the case. That reasoning is what the case is worth. Every one records a way the setup program hung, wrote a password into a config file, or reported green on absence. |
