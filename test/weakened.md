...
| Test | Reason |
|------|--------|
| TestSourceRestrictedExcusesASupportClaimAndDebtDoesNot | Renamed, not removed. The same table now runs as `TestOnlyANonNormativeDispositionExcusesASupportClaim` with the `source-restricted` case flipped from excused to refused, because that disposition no longer excuses a public support claim. Nothing left the suite: four cases before, four after, and the audit reads the rename as a deletion. |
| TestNativeImplementationFixture | The seal now hashes HEAD's committed blobs instead of the working tree, so it no longer records whatever another session happened to have uncommitted. Reading git can fail, so the three added skips are the git-unavailable paths, and the one `t.Fatalf` lost is the working-tree `os.ReadFile` guard the blob lookup replaced with `HEAD lists %s and holds no blob for it`. On any checkout where git answers, every assertion still runs. |
