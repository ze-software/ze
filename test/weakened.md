...
| Test | Reason |
|------|--------|
| TestRIBShowInStaleFlag | Four `require.True` calls per site became one `require.NotEmpty` plus a helper call, at two sites. They asserted that `before["adj-rib-in"]` existed, was a map, held the peer's key, and that the value was an array: the four levels of an envelope `show bgp rib` no longer answers (owner ruling, 2026-08-23, flat rows). The stale-flag assertions this test exists for are untouched, and the navigation is now covered in two places instead of inline: `receivedRowsByPeer` requires the answer to carry a `routes` list, and `requireRowsBelongTo` requires every selected row to name the peer it was selected for and to say which way it went. That second check is NEW and could not have existed before: the old shape put a route UNDER its peer's key, so it could not belong to another peer, while selecting on a field is a filter that can be wrong. The checker counts `require.`/`assert.` calls in the test body and does not follow helper calls, which is why this reads as a reduction. Re-run at HEAD after the change: the whole `internal/component/bgp/plugins/rib` package PASSES |
| requireFirstRoute | Its assertions about the ENVELOPE went with the envelope. It navigated `result[topKey]` then `[peerAddr]` then `[0]`, and checked each level existed and had the right type: four `require` calls about a two-level object `show bgp rib` no longer answers (owner ruling, 2026-08-23, flat rows). It selects a row by its `peer` and `direction` FIELDS now, so those four levels do not exist to check. Nothing it proved is lost: every caller still receives the same route and still asserts the same fields on it, and the not-found case is now a `require.Fail` naming the peer, the direction and the rows that were present, which is a better message than `expected adj-rib-in key` was. The sibling helper `requirePeerRoutes` changed the same way for the same reason. Re-run at HEAD after the change: the whole `internal/component/bgp/plugins/rib` package PASSES |
| TestShowJSONContractUnchanged | RENAMED, not removed, and the replacement asserts more. It becomes `TestShowJSONContractIsFlatRows` in the same file. The rename is the point: it asserted that `show bgp rib` still answered a top-level `adj-rib-in` / `adj-rib-out` pair of maps keyed by peer, and the owner ruled on 2026-08-23 that the command answers flat rows instead, one per route, with `peer` and `direction` as fields, because a row operator has nothing to select on until they are. A test whose name says `Unchanged` would be lying about a contract that changed by decision. Every assertion it made about the route survives against the new shape: prefix, next-hop, origin, as-path, community, med, local-preference on the received row, and prefix on the sent one. It gains two: each row must NAME its peer and direction, and the old envelope keys must be ABSENT, so the two-level shape cannot come back and quietly take the row operators away again. Re-run at HEAD after the change: PASS, and its sibling `TestShowRowsAreDeterministic` is mutation-proven against the sort it depends on |
