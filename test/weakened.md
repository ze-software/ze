...
| Test | Reason |
|------|--------|
| TestTheHomepageProofStripCarriesItsSixStatSpans | It pinned the home page proof strip at exactly six stat spans. The RFC card stopped publishing a bare enrolled count and now publishes the proven share with both populations it is drawn from, so a name fixing the number at six states a shape the strip no longer has. TestTheHomepageProofStripCarriesItsStatSpans asserts ten spans where this asserted six, checks each one's value against the same snapshot, and derives its count assertion from the list rather than hardcoding a number a future card would have to edit. Nothing left the suite. |
