...
| Test | Reason |
|------|--------|
| TestDropdownMultilineDescriptionCollapsed | Its whole subject was the removed column. It asserted that a newline inside a candidate's DESCRIPTION was folded to spaces, so the menu frame survived. This commit deletes the description column. No description reaches the menu, so no newline is left to fold. The test is renamed TestDropdownMultilineDescriptionStaysOutOfTheBox. It keeps every frame assertion unchanged, and it adds two: no fragment of either description appears anywhere in the box. Nothing left the suite. |
