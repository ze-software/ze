...
| verify_wiring_docs_test | Two of the four assertions the function above carried were negative: a non-fuzz internal test file, and a non-internal path, must NOT route the fuzz gate. They are satisfied by the absence rather than lost. Nothing replaces the read-and-reject branch they shared, and nothing can: that branch was `is_fuzz_source` (`scripts/dev/verify_wiring_docs.py`), which read a file and rejected it when it declared no `func Fuzz`. It is deleted with the gate it fed, so there is no surviving code for a test to drive. **Correction, 2026-08-26**: this row previously said `TestVerifyWiringDocsRoutesGoChanges` still drove that branch. No such test exists, in this file or anywhere in the tree, and neither does the branch. The row was written from what its author intended rather than from the file, an independent review found it by grepping the name, and the wider class is recorded in `plan/journal/claim-outlives-the-evidence-it-cites.md`. |
