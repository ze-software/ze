...
| Test | Reason |
|------|--------|
| TestTheRFCComplianceProducerClaimsItsPublishedRoute | NOT weakened: the `t.Fatalf("the producer claims %d routes, want 1")` it dropped had become an assertion about the wrong producer. The `rfc-compliance` producer now answers its index route plus one detail route per summary stem, 191 today, so a `want 1` guard fires on every correct run and its `t.Fatal` would stop the test before the published-route check below it. What replaced it asserts MORE, not less: the index route is still checked against the published artifact, and every other claimed route must be a child of `/quality/rfc-compliance/` ending in a slash, which the retired single-route assertion never checked at all. `TestTheRFCLedgerClaimsEachPublishedRouteOnce` and `TestTheRFCLedgerClaimsOnlyPublishedRoutes` hold the exact route population and the no-double-claim rule that the count once stood in for. |
