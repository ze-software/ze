...
| Test | Reason |
|------|--------|
| TestClosureStemUsesOnlyNewJournalEvidence | RENAMED to `TestClosureStemReadsRemovalsAndNotJournalRows`, and one assertion INVERTED in the same edit because the behavior it pinned is what this commit removes. It asserted that an added journal row naming one spec makes the commit that spec's closure. Thomas ruled on 2026-08-31 that a row is evidence about a spec and never its closure, so the same case now asserts the empty stem. Nothing left the suite: the two surviving cases (an unchanged journal closes nothing, a removed spec closes itself) are carried over unchanged, a third case is added for a removal beside a journal row, and `TestClosureStemStillRefusesAMalformedJournalRow` is new for the shape check that outlives the inference. The renamed test was forced red by restoring the stem contribution and failed with `added journal row closure = "new-spec"`. Recorded as a row in `plan/journal/gate-fires-outside-its-population.md` as well. |
