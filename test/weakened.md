...
| Test | Reason |
|------|--------|
| TestAppendRequestLengthPrefixedID | Renamed to `TestAppendRequestSpellsTheSameIDField`, in place, because the id carries no length prefix any more. No coverage left the suite: the same three id widths are written into a request line, both response lines and a record line, and each is still required to open with the same id field. The detector reports a deleted func because a rename is a delete plus an add in one hunk. |
| TestAnswerIDLengthPrefixRoundTrip | Renamed to `TestAnswerIDRoundTrip` and its three length-character assertions removed with the length character. The coverage that still applies is unchanged and still asserted: every digit count from 1 to 20, the smallest and the largest id of each width, the `#`, the decimal digits, the space that closes the field, and the id reading back through `ParseLine`. |
| TestAnswerIDLengthCharacterRejected | Replaced by `TestAnswerIDFieldRejected`, which asks the same question of the field the id now is. Seven cases went with the length character they tested: a lower-case length, a length below `0`, one above `Z`, a length of zero, a length longer or shorter than its digits, and a separator that is not a colon. None of them is expressible: no length reaches the wire. Every other case survives and four are added, including the last-valid boundary of twenty digits, which the old table did not carry. |
