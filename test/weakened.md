...
| Test | Reason |
|------|--------|
| TestNetlinkXFRMPatchApplied | Its assertion count goes 1 -> 0 because the body MOVED to `assertVendorPatchApplied` (`scripts/dev/vendor_patch_test.go`), which this commit adds and which both vendor-patch guards now call. The detector counts assertions inside the named function, and there is now one line there. Coverage RISES, and the point of the move is that the old body could not fail when it mattered: `go test` caches a passing result until one of the test's INPUTS changes, it learns those inputs from testlog, and testlog records only files the TEST BINARY opens. The check runs `git apply`, so git opened the vendored file and the binary never did -- the cache served green while the patch was reverted underneath it. Measured on this commit: with the patch reverse-applied and no `-count=1`, the old shape exits 0 and the new one exits 1. The helper reads the patch and every file it targets through `os.ReadFile` first, which puts them in testlog. Five failure modes were driven and each is now detected on a cache-eligible run: patch reverse-applied, patch file deleted, patch file emptied, patch naming no target, and a partial revert that keeps the comment while restoring the buggy line. |
