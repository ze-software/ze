...
| Test | Reason |
|------|--------|
| bgp_decode_nolink_test | The whole file is deleted, 14 lines carrying one const `bgpDecodeLinked = false` and its doc comment. Nothing read it: a repo-wide grep for the symbol returns only its own declaration and that comment. It is the `!ze_bgp` half of a build-tag mirror the tests stopped using, and its own surviving sibling says so: `bgp_decode_link_test.go` records that the withBGPDecode tests "read the seam itself rather than a build-tag mirror of it, and build_tag_bgp_absent_test.go asserts the nil in a build that runs". So the behaviour this file was written to express is still tested, by a test that executes, rather than by a constant nobody consults. No coverage leaves the suite. Thomas ruled the deletion on 2026-08-24 and ran it himself, because the test-deletion hook needs an interactive approval no agent can supply. It clears the last `cmd/ze/hub:unused` finding, which was blocking `ze-precommit-verify` and therefore every push. |
