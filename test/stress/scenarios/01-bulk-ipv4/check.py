...
def check():
    ...
    for prefix_count, prefix_base, timeout in ROUNDS:
        ...
        with Timer("ze processing") as t_process:
            ze.wait_rib_count(prefix_count, timeout=timeout)
        ...
        rate = prefix_count / t_process.elapsed if t_process.elapsed > 0 else 0
        log_pass("%d prefixes: injection %.2fs, processing %.2fs (%.0f routes/s)"
