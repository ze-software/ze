...
def build_linux_binary():
    ...
    if os.path.exists(ZE_PERF_LINUX):
        ...
        r = subprocess.run(
            ["file", ZE_PERF_LINUX], capture_output=True, text=True, timeout=5
        )
...
def ensure_colima_memory(needed_duts):
    ...
    try:
        ...
        print(
            f"  Colima VM has {mem_gib:.1f} GiB, "
...
def build_images(needed_duts):
    ...
    if "ze" in needed:
        ...
        docker(
            ...
            "ze-interop",
            "-f",
            os.path.join(INTEROP_DIR, "Dockerfile.ze"),
            PROJECT_ROOT,
            "--quiet",
            timeout=600,
        )
    ...
    if "bird" in needed:
        ...
        docker(
            ...
            "bird-interop",
            "-f",
            os.path.join(INTEROP_DIR, "Dockerfile.bird"),
            INTEROP_DIR,
            "--quiet",
            timeout=120,
        )
    ...
    if "gobgp" in needed:
        ...
        try:
            docker(
                ...
                "gobgp-interop",
                "-f",
                os.path.join(INTEROP_DIR, "Dockerfile.gobgp"),
                INTEROP_DIR,
                "--quiet",
                timeout=300,
            )
    ...
    if "rustbgpd" in needed:
        ...
        try:
            docker(
                ...
                "rustbgpd-interop",
                "-f",
                os.path.join(INTEROP_DIR, "Dockerfile.rustbgpd"),
                INTEROP_DIR,
                "--quiet",
                timeout=900,
            )
    ...
    if "rustybgp" in needed:
        ...
        try:
            docker(
                ...
                "rustybgp-interop",
                "-f",
                os.path.join(INTEROP_DIR, "Dockerfile.rustybgp"),
                INTEROP_DIR,
                "--quiet",
                timeout=900,
            )
        ...
        if not os.path.isdir(FREERTR_DIR):
            ...
            try:
                docker(
                    ...
                    "freertr-interop",
                    "-f",
                    os.path.join(INTEROP_DIR, "Dockerfile.freertr"),
                    FREERTR_DIR,
                    "--quiet",
                    timeout=600,
                )
...
def stop_dut(dut_name):
    docker("rm", "-f", container_name(dut_name), check=False, timeout=30, capture=True)
...
def run_perf(dut):
    ...
    docker(
        ...
        "NET_ADMIN",
        "-v",
        f"{ZE_PERF_LINUX}:/usr/local/bin/ze-perf:ro",
        "-v",
...
def cleanup():
    ...
    for dut in DUTS:  # noqa: B007 -- need dut["name"] not index
        stop_dut(dut["name"])
    docker(
        "rm", "-f", f"ze-perf-runner-{SUFFIX}", check=False, timeout=30, capture=True
    )
...
def parse_args():
    ...
    parser.add_argument(
        "duts", nargs="*", metavar="DUT", help="DUT names (default: all)"
    )
...
def main():
    ...
    if not duts:
        print(
            f"error: no matching DUTs. Available: {', '.join(d['name'] for d in DUTS)}",
            file=sys.stderr,
        )
        return 1
    ...
    if r.returncode != 0:
        ...
        if r.returncode != 0:
            print(
                f"error: cannot create Docker network: {r.stderr.strip()}",
                file=sys.stderr,
            )
            return 1
