...
| Test | Reason |
| ------ | ------ |
| TestBgpIdentityPrefersTheFourOctetASNCapability | Thomas approved `plan/spec-fixit-locrib-peer-fields-contradict-rfc9069.md` on 2026-09-03, whose AC-2 requires the Loc-RIB Peer BGP ID to come from configuration and to be "never 0". The id is `RFC9069-x-6 negative`. The test is DELETED because its producer is: `bgpIdentityFromSentOpen` read the router's own ASN and router-id out of a cached sent OPEN, which is the defect the spec exists to fix, and `ai/rules/no-layering.md` requires the old path to go rather than stand beside the config read. The negative polarity is NOT lost: `TestLocRIBPeerHeaderCarriesTheIdentityItIsGiven` carries `RFC9069-x-6 negative` and still requires two identities to produce two Peer AS values. The wire property this test demonstrated, AS_TRANS in My AS with the true ASN in the 4-octet capability, is asserted directly on the producer that writes it by the new `TestFabricatedOpenUsesASTransForFourByteASN`. |
| TestHandleBestChangeEmitsPeerUpThenRM | Same approval and same spec. The ids are `RFC9069-x-3 positive` and `RFC9069-x-7 positive`. The change is one struct field: the test plugin gains `identity: &localIdentity{asn: 65000, routerID: 0x01020305}`, the same ASN and router-id its cached OPEN fixture already carried, because the identity now comes from configuration rather than from that cache. Every assertion is unchanged, and the Peer Up it reads off the wire carries the same two values it asserted before. |
| TestLocRIBSinglePeerUpPerInstance | Same approval, same spec, same one-line change for the same reason. The id is `RFC9069-x-2 positive`. The test still drives two peers' best changes and still requires exactly one Loc-RIB Peer Up; nothing about the count, the guard, or the assertions moved. |
| TestConcurrentDumpsStayAddressedToTheirOwnCollector | Same approval, same spec, same one-line change for the same reason. The ids are `RFC7854-x-16 positive` and `RFC7854-x-17 positive`. The test still requires a Peer Up for every established BGP peer on each new BMP session and the dump that follows it, and still requires each of two concurrent dumps to reach only its own collector. |
| TestRFC8671UnrelatedBGPChangeBouncesNothing | Same approval, same spec. The id is `RFC8671-7.2-1 negative`. The test hand-builds a `bgp` subtree, and that subtree now carries `router-id` and `session asn local`, which are `mandatory true` in `internal/component/bgp/yang/ze-bgp-conf.yang` and which a real config-verify therefore always delivers. Without them the fixture asked whether a reload that also DELETED the router's identity bounces nothing, which is a different question and one whose answer is no. The test's own claim is unchanged: a reload that adds a neighbor and moves no leaf deciding what the session carries still owes the collector nothing, and it still requires silence. |
| TestBMPReloadTurningLocRIBOffUnsubscribesAndSaysSo | Same approval, same spec. The id is `RFC9069-5.3-1 positive`. One precondition check is ADDED, ahead of the existing body: the startup configuration must have delivered a router identity, because a Loc-RIB Peer Down cannot be built without one now that the plugin declines to emit a zero. Every original assertion stands: the collector still reads a Loc-RIB Peer Down with reason code 6 before the Termination, and the reload still leaves the plugin unsubscribed with `locRIBUp` clear. |
| TestRFC9069ReloadTurningLocRIBOnAnnouncesTheRouterIdentity | Same approval, same spec, and this one is STRENGTHENED. The ids are `RFC9069-x-6 positive` and `RFC9069-5.2-1 positive`. The fixture's cached sent OPEN is replaced by an EMPTY OPEN cache, and the ASN 4200000001 and router-id 10.20.30.1 the body already asserted now arrive through the `bgp` container the reload commits. That is the discriminating condition RFC 9069 Section 1.1 names, a Loc-RIB monitored where there are "no preexisting BGP peers": an implementation reading a cached OPEN emits nothing at all here while passing every case that has a peer up. No assertion was removed, and the Peer AS, the Peer BGP ID, the twice-repeated fabricated OPEN and its capability set are still read off the collector socket. |
