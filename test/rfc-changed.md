...
| Test | Reason |
| ------ | ------ |
| TestRFC8671StatisticsReportClearsTheOFlag | Thomas ruled on 2026-08-31 that `RFC8671-6.2-1` does not comply, because the requirement implies a Statistics Report will be created and ze creates none, so a report added later carrying a non-zero O flag would leave this test green. The approved change removes the `RFC requirement: RFC8671-6.2-1 positive` tag and keeps every assertion and every line of the body. The requirement is NOT still proven, and that is the point of the change: the claim was wider than the body. The body calls `senderSession.writeStatisticsReport` directly and proves the ENCODER clears the flag, while the tag claimed a Statistics Report goes out with it clear. Narrowing the claim was not open: `evaluate` in `internal/le/rfc/check_core.go` refuses ANY tag on a requirement annotated `{gap}`, whatever wording the claim carries. `rfc/short/rfc8671.md` now carries that `{gap}`, and `checkCoverageRatchet` reports the lost polarity, which is the disclosure Thomas ordered rather than a hidden retirement. |
| TestRFC8671RouteMonitoringKeepsTheOFlag | The same ruling and the same change, for the negative half. The approved change removes the `RFC requirement: RFC8671-6.2-1 negative` tag and keeps every assertion. This body sends a Route Monitoring message and requires the O flag to survive, so it never exercised a Statistics Report at all: it guarded against a Section 6.2 fix that cleared the flag sender-wide. That guard still runs as an ordinary unit test beside `TestRFC8671OFlagSetOnAdjRIBOutMessages`, which is the tagged proof of `RFC8671-6.1-1`. `RFC8671-6.2-1` keeps no proof, which is what a `{gap}` states. |
