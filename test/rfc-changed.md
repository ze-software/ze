...
| Test | Reason |
| ------ | ------ |
| radius-acct-wire | Thomas approved this on 2026-09-03. He was told in the same message that changing this file's config or adding a CHAP assertion needs his row here, and he answered "fix the .ci config too". The id is `RFC2866-4.1-1 positive`. The tagged assertion is UNTOUCHED: `OK: Accounting-Start Framed-IP-Address matches the IPCP-negotiated address 10.99.7.10` still reads what the RADIUS mock decoded, and the peer still compares it against the address it settled on over IPCP. What changes sits upstream of it. `auth-method none` becomes `chap-md5` and `allow-no-auth true` goes, so the config states the authentication the session performs; two `expect=stdout` lines are ADDED for the CHAP Response and the CHAP Success. The leaf is inert either way, because the peer sends the RFC 2661 Section 4.4.5 proxy LCP AVPs and `EvaluateProxyLCP` reads the method from the LAC's Last-Sent CONFREQ, so this is the config telling the truth rather than a change of behaviour. Dropping `allow-no-auth` makes a session that reaches the auth phase with no method fail loudly instead of running unauthenticated. Measured in QEMU on the arm64 runtime kernel 7.2 after the change: `radius-acct-wire` green and the whole l2tp suite 23 of 23. No discrimination record is created or lost by this change: `./le rfc discriminate stem rfc2866` lists all 35 rfc2866 tags as unproven with zero records, so there is none to go stale and `./le rfc check` demands none. |
