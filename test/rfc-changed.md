...
| Test | Reason |
|------|--------|
| check | Thomas instructed on 2026-08-24 that the IPsec interop scenarios must not be numbered, so all 20 scenario directories lose their numeric prefix and every `check.py` moves with its directory. The gate sees a path that no longer holds the test and reports a deletion, a rename and a move as one shape; this is the rename. NO ASSERTION CHANGED. Measured rather than asserted: each of the 20 files was diffed against its HEAD copy, and every changed line is a docstring or a comment naming a scenario, plus the `IPSEC_INTEROP_SCENARIO=` example in five headers (child-rekey-narrowing, initiator-rekey-answer-narrows, delete-while-window-held, responder-raises-child-rekey, peer-reload-narrowing). `responder-raises-child-rekey` carries the largest prose diff because its header explains, by name, which sibling scenarios do not cover its direction; those names were numbers. Every requirement each file proves stays proven by the same code, `RFC9190-2.5-1` included, and `make ze-interop-ipsec-test IPSEC_INTEROP_SCENARIO=eap-mschapv2` passes end to end after the rename with SAs established and ESP counters advancing on both sides. |
