...
| Test | Reason |
|------|--------|
| TestRouteRefreshInvalidLengthNotDelivered | Thomas ruled on 2026-08-31, answering a which-way question that quoted RFC 7313 Section 5's two scoping sentences. He chose A1 and B2: a malformed ROUTE-REFRESH from a peer that never sent the Enhanced Route Refresh Capability draws RFC 4271 Section 6.1 Bad Message Length, and so does Message Subtype 0 from a peer that did, because Section 5 scopes its MUST to "Message Subtype 1 and 2". This test's two bodies carry subtype octet 0x00, so under that ruling they no longer draw Error Code 7. Its bodies move to subtype 0x01 and 0x02, which keeps its `RFC7313-5-1 positive` and `RFC7313-5-2 positive` tags truthful over the subtypes the requirement is actually about. It was the test pinning the non-conformant unconditional behaviour. Nothing is dropped: the RFC 4271 answers it used to assert wrongly are now asserted, in both polarities, by TestRouteRefreshBadLengthWithoutCapability70 and TestRouteRefreshBadLengthByMessageSubtype. |
