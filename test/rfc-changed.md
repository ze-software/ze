...
| Test | Reason |
| ------ | ------ |
| TestRFC2385FailedConnectDoesNotDisableSigning | Thomas approved this on 2026-09-02, answering a question that listed every edit by file. The id is `RFC2385-2.0-4 positive` and it proves exactly what it proved before. Two lint fixes, and one STRENGTHENS the test: `net.Listen` becomes `(*net.ListenConfig).Listen(t.Context(), ...)`, the same listener now bound to the test's context, and the unchecked `closed.Addr().(*net.TCPAddr)` assertion takes the two-value form and fails the test on a miss instead of panicking. No assertion about MD5 signing survival moved. |
| TestRFC2865SubscriberAccessRequestCarriesCredential | Same approval. The id is `RFC2865-4.1-3`, both polarities, and both are still proven. The only edit is to the shared helper `setupCapturingAuth`, which dropped an `attrs []radius.Attr` parameter that all six call sites passed as nil; its body passes nil to `startCapturingRADIUS` directly, whose own signature is unchanged. Every credential case and every `assertNoRequest` is byte for byte the same. |
| TestRFC2865SubscriberZeroLengthUserNameOmitted | Same approval, same helper edit, same reason. The id is `RFC2865-5-8`, both polarities. The Access-Request read off the wire and the omitted User-Name it asserts are untouched. |
| TestRFC8907ClientNeverSendsUnobfuscatedBody | Same approval. The id is `RFC8907-10-1 positive` and nothing about the proof moved. One word inside a `t.Fatal` message changes from `travelled` to `traveled`, which is this repository's declared US-English spelling. The flag octet the test reads and the cleartext body it compares against are unchanged. |
| TestGTSMDialerSetsOutgoingTTLTo255 | Same approval. The id is `RFC5082-3-1 positive`. The shared helper `socketOption` dropped a `level` parameter that only ever received `unix.IPPROTO_IP`; the body names that level directly and the getsockopt failure message names it in place of a number. The option read and the value required, IP_TTL and 255, are unchanged. |
| TestGTSMDialerWithoutOutTTLLeavesTheDefault | Same approval, same helper edit, same reason. The id is `RFC5082-3-1 negative`, the other polarity of the row above, and the default TTL it asserts is untouched. |
| TestGTSMNoFloorDeliversAnUnknownPacket | Same approval. The id is `RFC5082-3-4 negative`. Two edits, both mechanical: the same `socketOption` parameter drop, and `net.Listen` becomes `(*net.ListenConfig).Listen(t.Context(), ...)` for the borrowed listener. The IP_MINTTL read and the zero floor it requires are unchanged. |
