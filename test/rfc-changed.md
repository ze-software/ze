...
| Test | Reason |
|------|--------|
| TestRFC2865FailoverRegeneratesRequestAuthenticator | Thomas authorised adopting the requirement-id scheme of the committed extraction sign-off `rfc/extraction/rfc2865.json` (2026-08-31). This test's tags move from `RFC2865-4.1-1` to `RFC2865-4.1-6`, the id the sign-off gives Section 4.1 "The Request Authenticator value MUST be changed each time a new Identifier is used." No assertion, case or value changes. |
| TestRFC2865EmptySharedSecretBuildsNoClient | The same 2026-08-31 authorisation. Tags move from `RFC2865-3-6` to `RFC2865-3-8`, the id the sign-off gives Section 3 "The secret MUST NOT be empty (length 0) since this would allow packets to be trivially forged." `RFC2865-3-6` names the padding-octet sentence in the sign-off scheme. Body unchanged. |
| TestRFC2865UnsupportedServiceTypeIsRejection | The same 2026-08-31 authorisation. This test carried two ids for one obligation, `RFC2865-5.6-1` and `RFC2865-1.1-1`, and both become `RFC2865-1.1-2`: the sign-off maps the Section 1.1 and Section 5.6 sentences to that single id. Both polarities are kept. Body unchanged. |
| TestRFC2865ZeroLengthTextIsOmitted | The same 2026-08-31 authorisation. Tags move from `RFC2865-5-4` to `RFC2865-5-8`, the id the sign-off gives Section 5 "Text of length zero (0) MUST NOT be sent; omit the entire attribute instead." `RFC2865-5-4` names the attribute-order sentence in the sign-off scheme. Body unchanged. |
| TestRFC2865SubscriberAccessRequestCarriesCredential | The same 2026-08-31 authorisation, on the L2TP subscriber path. Tags move from `RFC2865-4.1-2` to `RFC2865-4.1-3`, the id the sign-off gives Section 4.1 "An Access-Request MUST contain either a User-Password or a CHAP-Password or a State." `RFC2865-4.1-2` names the NAS-IP-Address obligation in the sign-off scheme. Body unchanged. |
| TestRFC2865SubscriberZeroLengthUserNameOmitted | The same 2026-08-31 authorisation. Tags move from `RFC2865-5-4` to `RFC2865-5-8`, as for the RADIUS-side test of the same sentence. Body unchanged. |
| TestRFC2865SubscriberServiceTypeAuthorization | The same 2026-08-31 authorisation. `RFC2865-5.6-1` and `RFC2865-1.1-1` both become `RFC2865-1.1-2`, as for the RADIUS-side test of the same obligation. Both polarities are kept. Body unchanged. |
