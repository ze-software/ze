...
| Test | Reason |
|------|--------|
| TestRFC5880UnsupportedAuthTypesRejected | Thomas ruled on 2026-08-31, verbatim: "implement it", answering a which-way question that offered implementing Simple Password against recording an owner ruling that ze does not offer it. Implementing it makes `TestRFC5880UnsupportedAuthTypesRejected` false as written: it asserted auth type 1 is refused, and type 1 is now supported. Its case list moves to types 0, 6 and 200, the reserved and unassigned values, which is what `RFC5880-6.7-1 negative` actually claims. The file-scope tag list is collateral: the file carries eighteen RFC 5880 tags and the detector reports the file when any one test in it changes. |
| TestRFC5880KeyManagementRejectsIncompleteConfig | The same 2026-08-31 "implement it" ruling. `TestRFC5880KeyManagementRejectsIncompleteConfig` refused a Simple Password configuration by name at the parser; it now refuses one on the RFC's own ground, a secret outside the 1 to 16 bytes Section 6.7.2 allows. Tags `RFC5880-6.7.3-7` and `RFC5880-6.7.4-5` still describe what it asserts. |
