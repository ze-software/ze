...
| Test | Reason |
|------|--------|
| TestRFC7427SignatureAlgorithmIsOneThePeerSent | Thomas commissioned this test on 2026-08-30, in the answer that ordered the two RFC 7427 defects fixed and their proof written; it was added in a3e44f267 and this is a lint repair to it, made in the same session, not a change of evidence. `./le verify lint run` reported `gocritic stringXbytes` on the comparison, so `string(algID) != string(tc.want)` became `!bytes.Equal(algID, tc.want)`. The two are the same predicate over the same operands: `bytes.Equal` and a string comparison of the same byte slices agree for every input, so no case changes verdict. The gate reports the function because a tagged test is compared byte for byte after comments are stripped, and this edit is inside the tagged function. RFC7427-4-1 keeps both polarities on this function: every table case, the algorithm identifiers it expects, and the error it expects for a peer that offered nothing the key can sign with are untouched. |
