...
| Test | Reason |
| ------ | ------ |
| TestRFC2759PeerEndsSessionOnBadAuthenticatorResponse | Thomas approved this on 2026-09-01, telling me to finish the RFC 3748 work. The id is `RFC2759-x-7 negative` and it is still proven. RFC 3748 Section 4.2 states "The peer MUST silently discard Success packets", and the peer answered a Success arriving after its own refusal with `errSessionEnded`. `handleEAPResponse` (`internal/component/ike/engine/fsm.go`) reads any non-nil `Err` as a reason to set `StateDead`, so one unauthenticated packet chose the moment the SA died. The refusal is now `peerDiscard()`. The case asserts the discard and the absence of an error in place of the error, and every property it existed for is untouched below it: no MSK, no completion, no success. |
| TestRFC3748OneMethodPerConversation | Same approval. The id is `RFC3748-2.1-1 negative` and it is still proven. RFC 3748 Section 4.1 states "An EAP server receiving a Response not meeting these requirements MUST silently discard it", and `handleMethod` answered a Response whose Type was neither the method's nor a Nak with an EAP-Failure, which ends a conversation on a packet the RFC says not to read. The case now asserts a silent discard and adds an assertion that the discarded Response did not complete the exchange. The property it is named for is unchanged: a second method Type is not accepted. |
| TestEapfmtPeerResponseTypeIsNakOrMatchesRequest | Same approval. The id is `RFC7296-2.16-1 positive` and it is still proven. RFC 3748 Section 2.1 forbids an authenticator to send a Request of a different Type before the method's final round completes, so the peer discards one. The case asserted an error where the discard now carries none, for the same `StateDead` reason as the first row. It asserts `Discarded` and the absence of an error instead, and keeps its existing assertion that the mismatched Request draws no response. |
