...
| Test | Reason |
| ------ | ------ |
| TestRFC6793ReceivedAS4AggregatorAccepted | Thomas approved this on 2026-09-04, in answer to the change described to him: the fix he authorised to the RFC 6793 receive-side reconstruction makes the behaviour this test asserted non-conformant, so the assertions had to move with it. The id is `RFC6793-4.2.3-2 positive`. The test pinned the pre-fix split: an AGGREGATOR interned with AS_TRANS (23456) inside it, and the AS4_AGGREGATOR kept beside it in OtherAttrs, uninterpreted. RFC 6793 Section 4.2.3 says that when the AGGREGATOR carries AS_TRANS the AGGREGATOR is ignored and the AS4_AGGREGATOR is taken as the aggregating node, so after the fix there is no second attribute to find and no AS_TRANS to assert. **The requirement the tag names is unchanged and still driven**: the pair is ACCEPTED rather than rejected, which is what RFC6793-4.2.3-2 obliges. Its surviving assertion is stronger than the two it replaces, because the four-octet aggregating AS is now the interned aggregator itself rather than bytes sitting unread beside it. Which of the pair the route records is RFC6793-4.2.3-3 through -7, proven in both polarities by the new `rfc6793_reconstruct_test.go`, so no coverage left the suite. |
