...
| Test | Reason |
|------|--------|
| TestNattRekeyedChildKeepsUDPEncap | Thomas approved this on 2026-08-22, after the coordinator read the diff at the source and put it to him. The change is one argument: `newRekeyedChild` (`internal/component/ike/engine/rekey.go`) gained a `negotiated []tsPair` parameter so a rekey installs the scope THIS exchange agreed, and this call site now passes `nil`, which is the value that means "inherit the retired pair's selectors" -- the behaviour the test already exercised. A comment was added beside it. RFC7296-2.23-8 is still proven because every assertion, condition and failure message in the test is byte-identical: it still builds a Child SA with `NATDetected` and `UDPEncap` set, still calls the builder, and still asserts `got.UDPEncap != encap` in both polarities of the `encap` loop. The gate fires because it compares text and this line's text moved. |
| TestNegInitiatorRejectsUnproposedOffer | Thomas approved this on 2026-08-22, on the same reading. The change is a STRENGTHENING of the fixture, not a relaxation. `negESPRekeyInner` and `negESPRekeyAccepted` (`internal/component/ike/engine/rfc7296_negotiation_test.go`) built a CREATE_CHILD_SA rekey RESPONSE carrying SA and Nr and no traffic selectors. RFC 7296 Section 1.3.3 puts TSi and TSr in that response, and `applyChildRekeyResponse` now refuses one that omits either, so the old fixture was a wire shape no conforming peer sends. Both helpers take the old Child SA and append its own scope through `childRekeyAnswerTS` (`rekey_wire_test.go`), and the two call sites pass it. RFC7296-3.3.6-3 is still proven because no assertion moved: the test still accepts a consistent ESP offer and still fails the exchange on an offer carrying `negUnproposedKeyLength`, which is the requirement's own subject. |
