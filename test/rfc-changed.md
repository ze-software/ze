...
| Test | Reason |
| ------ | ------ |
| TestLocRIBPeerHeader | Thomas approved this on 2026-09-01, after reading exactly what changes. The test passes one added argument, `time.Time{}`, to `locRIBPeerHeader`. No assertion, expected value, quoted RFC sentence or test name changes. The signature widened because RFC 9069 Section 5.1 states the Timestamp is "the time when the encapsulated routes were installed in the Loc-RIB" and "If zero, the time is unavailable", and the producer stamped `time.Now()` on every message, dating a full-table replay to the collector's connect instead. The zero this test passes IS the RFC's answer for a header with no install time, so the case it drives is unchanged. Ids `RFC9069-x-1`, `-x-6` and `-x-7`. |
| TestLocRIBPeerHeaderCarriesTheIdentityItIsGiven | Same approval, same single added `time.Time{}` argument, same reason. Ids `RFC9069-x-1`, `-x-6` and `-x-7`. Nothing else in the body moves. |
| TestRFC8671ReservedPeerFlagsTransmittedAsZero | Same approval. One added `time.Time{}` argument, and one COMMENT corrected in the same file: it said a Loc-RIB Peer Down carries no Information TLV, which this commit makes false, because RFC 9069 Section 5.2.1 requires the VRF/Table Name TLV to be repeated on the Peer Down when the Peer Up carried it. The corrected comment names `RFC9069-5.2.1-1` as the row that asserts the TLV, so the claim moves to the test that proves it rather than being dropped. The assertion on the reserved flags is untouched. |
