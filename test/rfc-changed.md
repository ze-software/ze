...
| Test | Reason |
| ------ | ------ |
| zzprobe_prefixsid_announce_test | Thomas approved the rename on 2026-09-05 and ran the `rm` himself, the gate having no environment escape for deleting a `_test.go` path. This commit removes the emptied file only. The test it carried moved to `forward_prefix_sid_announce_rail_test.go` in `4054ed854`, where it is GREEN against the fix and gained two positives it never had: the configured external frame equals the stripped frame plus attribute 40 byte for byte, and an internal peer's frame does not depend on the leaf. `RFC8669-8-1` is proven in both polarities by `rfc/discrimination/rfc8669.json`, recorded under a disabled `prefixSIDAllowedTo`, so nothing this file asserted is unproven after the removal. |
