...
| Test | Reason |
|------|--------|
| TestAuthPSKCompute | Thomas ordered the tag REMOVED on 2026-08-30, after an independent verification of the row `a3e44f267` wrote for this same test. That row claimed re-homing the tag from `RFC7427-4-1` to `RFC7427-3-4` was "the mechanical consequence" of the correction he approved, and it was not: `validateID` (`internal/le/rfc/summary.go:236`) forces the SUMMARY line to take a §3 id, and no check in `internal/le/rfc/` binds a test to any id, so the destination was a free choice made under his name. The choice was wrong. `computePSKAuth` (`internal/component/ike/engine/auth.go:300`) never reads `sa.RemoteHashAlgos` and returns `wire.AuthMethodPSK` as a constant selected by `sa.PeerCfg.Auth.Mode`, while RFC7427-3-4's entire content is the condition that the SIGNATURE_HASH_ALGORITHMS notify was sent and received. Delete the Section 3 gate in `computeX509Auth` and every assertion in this test stays green, so it was a carrier that could not fail. Nothing is lost: RFC7427-3-4 keeps both polarities on `TestRFC7427DigitalSignatureNeedsTheNotify`, which asserts the refusal directly, and `rfc/requirements/rfc7427.md` is regenerated from the tags so the requirement's evidence list drops only the non-discriminating entry. The test itself is untouched below the comment: same fixture, same two assertions on AUTH method 2. |
