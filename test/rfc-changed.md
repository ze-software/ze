...
| Test | Reason |
| ------ | ------ |
| TestAnnounceRailKeepsPrefixSIDInsideTheSRDomain | Call-site adaptation to a changed parameter type: `buildBatchAnnounceUpdate` takes `localASPrepend` where it took `uint32`, so the `65000` this test passed becomes `localASOnly(65000)`. No assertion, expectation or claim changed. RFC8669-8-1 stays proven in both polarities because the test still drives the same builder over the same two cases and still reads the Prefix-SID out of the emitted attributes: `localASOnly(65000)` is the value `65000` carried in the new type, and its `secondary` field is zero, so the AS_PATH those two frames carry is byte-identical to the one they carried before. Approved by Thomas, 2026-09-06. |
| reactor_api_batch_attr_order_test | Call-site adaptation to a changed parameter type: `buildBatchAnnounceUpdate` and `buildBatchASPath` take `localASPrepend` where they took `uint32`, so `c.localAS` and `65000` become `localASOnly(...)`. The unit is the file rather than a function because the two changed lines sit in the table helpers `buildBatchRail` and its sibling, which no tagged func encloses. No assertion, expectation or claim changed. RFC4271-5-7 stays proven because the ordering assertions read the emitted attribute type codes and this change moves no attribute: every case still declares one local AS, `localASOnly` leaves `secondary` zero, and the AS_PATH keeps the same single AS number at the same type-code position. Approved by Thomas, 2026-09-06. |
| TestAnnounceStripsLocalPrefTowardExternalPeer | Call-site adaptation to a changed parameter type: `buildBatchAnnounceUpdate` and `buildBatchASPathAttr` take `localASPrepend` where they took `uint32`, so `65000` becomes `localASOnly(65000)`. No assertion, expectation or claim changed. RFC4271-5.1.5-2 and RFC4271-5.1.5-1 stay proven in both polarities because the test still asserts the presence and absence of attribute type 5 toward internal and external peers, and this change touches only which AS numbers reach attribute type 2. `localASOnly` leaves `secondary` zero, so that attribute is byte-identical too. Approved by Thomas, 2026-09-06. |
