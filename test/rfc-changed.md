...
| Test | Reason |
| ------ | ------ |
| TestLabeledRsrvIgnoredOnReceive | Thomas approved this on 2026-09-02, after reading both diffs in full. The ids are `RFC8277-2.2-3 positive` and `RFC8277-2.2-3 negative`, and both are proven by the same assertions as before. The registered Splitter became a walk, so the two `SplitLabeled(data, false)` calls take the in-package helper `splitAll`, which collects the walk into the slice the old call returned. Every label, CIDR and framing assertion, and the Rsrv fixtures either side of them, are byte for byte the same. |
| TestRFC9830NLRIAddressFamilies | Same approval. The id is `RFC9830-2.1-2 negative`. One call gains the walk's third argument as `nil`, which is the count pass and reads no NLRI. The refusal it requires, a length of 100 bits that is not byte-aligned, and the `ErrSRPolicyTruncated` case above it, are unchanged. |
