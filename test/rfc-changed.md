...
| Test | Reason |
|------|--------|
| TestLocRIBPeerHeader | Thomas ruled on 2026-08-31, verbatim: "populate Peer AS and fabricate the OPEN with capabilities, then correct both summary rows and the ledger". RFC 9069 Section 5.1 requires Peer AS to be "Set to the primary router BGP autonomous system number", and ze sent 0. This test ASSERTED the zero -- it proved ze did the wrong thing, and rfc/short/rfc9069.md declared that wrong behaviour as RFC9069-x-6 at MUST level with this test bound to it. The test now asserts the router's own 4-octet ASN. A test that proved a defect is not evidence to preserve; RFC9069-x-6 keeps its id, its text is corrected to what the RFC requires, and its proof now points the same way. |
| TestHandleBestChangeEmitsPeerUpThenRM | Same ruling. RFC 9069 Section 5.2 requires the Loc-RIB Peer Up to carry a fabricated OPEN whose "Capabilities MUST include the 4-octet ASN and all necessary capabilities", and ze sent nil for both OPEN fields. This test asserted the zero-length OPENs, which rfc/short/rfc9069.md declared as RFC9069-x-3 at MUST level. It now asserts a fabricated OPEN in both the sent and received fields, built from the same family list the dump closes with End-of-RIB markers so the advertisement and the delivery are one declaration. RFC9069-x-3 keeps its id with corrected text and correspondingly corrected proof. |
