...
| Test | Reason |
|------|--------|
| TestNegotiatePeerAuthProtoRejected | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestNegotiatePeerAuthProtoAccepted | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661ConfigureRejectDoesNotReorderOptions | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661BooleanOptionsUseRejectNotNak | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661ValuedOptionUsesNak | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661NakValueIsAcceptable | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661RejectedValueStaysUnacceptable | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661NakPreservesRequestOrder | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661NakOrderFollowsRequestNotAFixedOrder | Thomas ruled on 2026-08-31, verbatim "follow the RFC", commissioning the RFC 1661 Section 6 option-length fix. Making the PPPoE client answer as the LNS does required both to share one negotiation policy type, and an exported `NegotiatePeerOptions` taking an unexported parameter type was unusable outside the package, so `lCPNegPolicy` became `LCPNegPolicy`. That single capital letter is the ENTIRE diff for this test against HEAD, verified by an independent reader in round 3. No assertion, case or value changed. |
| TestRFC1661PeerMagicNumberAcked | The same 2026-08-31 ruling, and the change is a COMMENT rather than an assertion. Its comment said a wrong-Length peer Magic-Number still draws a Configure-Reject and that RFC1661-6.4-1 is therefore annotated `{gap}`. Both halves became false in this commit: the wrong-Length case draws a Nak and the gap is deleted. The comment now names the two tests covering the other Magic-Number shapes. The body carries the `LCPNegPolicy` rename and nothing else. |
| TestRFC1661ZeroMagicNumberRefused | Thomas ruled on 2026-08-31, answering a which-way question that quoted both RFC 1661 Section 6.4 sentences: "A Magic-Number of zero is illegal and MUST always be Nak'd, if it is not Rejected outright" and "if an implementation does transmit a Configure-Request with a Magic-Number Configuration Option, then it MUST NOT respond with a Configure-Reject when it receives a Configure-Request with a Magic-Number Configuration Option". He chose the Nak. This test asserted the Configure-Reject the second sentence forbids ze, since `generateMagic` never returns zero and `run` fails the session if it errors, so ze always transmits a Magic-Number. The reject list is now asserted EMPTY, the Nak is checked for type, 4-octet width and a value that is neither zero nor ze's own, and the test gains an `RFC1661-6.4-1 positive` tag. The only assertion dropped is the one that pinned the non-conformant answer. |
| TestLCPConfigRequestMRU | The same 2026-08-31 "follow the RFC" ruling. `sendLCPConfigRequest` gained a `logger *slog.Logger` parameter so a refusal to send speaks rather than returning silently (`ai/rules/principles.md`), and this call site passes it. Verified against HEAD by an independent reader in round 5: one changed line, the added argument. The `RFC2516-x-9 positive` tag and every assertion are byte-identical. |
