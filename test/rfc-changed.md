...
| Test | Reason |
| ------ | ------ |
| radius-acct-wire | Thomas approved this on 2026-09-04, in answer to the change put to him: he chose "approve, land all three" for `test/l2tp/radius-acct-wire.ci` and the two fixture files beside it. The change ADDS assertions and removes none, which is the fourth row of this file's own "What this gate cannot see" table: the tagged claim, `RFC requirement: RFC2866-4.1-1 positive` over Framed-IP-Address on the Accounting-Start, is untouched and every `expect=` line that proved it is still there. What the commit adds is a second and a third record to wait for, and four assertions per record for the session attributes `spec-radius-acct-session-attributes` emits: Event-Timestamp inside the run's own clock window, Calling-Station-Id equal to the ICRQ Calling Number, Acct-Delay-Time present and numeric, and Acct-Terminate-Cause ABSENT on the Start and on the Interim-Update and equal to 2 on the Stop. RFC2866-4.1-1 is therefore proven by strictly more of the same test than before. |
