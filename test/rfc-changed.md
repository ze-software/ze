...
| Test | Reason |
| ------ | ------ |
| TestEapfmtResponseIdentifierMatchesRequest | Thomas approved this on 2026-09-04, asked which way rather than whether, and chose "approve the split as done". The ids are `RFC7296-3.16-1 positive` and `RFC7296-3.16-1 negative`. The change is forced by the package move from `internal/component/ike/eap` to `internal/core/eap`, which exists so the RADIUS admin backend can drive the EAP peer without depending on IKE. The old body proved two things at once: that the peer copies the request Identifier into its response, and that the IKE payload carrying those octets keeps it in octet 1. The second half went through `eapfmtWireOctets`, which builds a `wire.PayloadEAP`, and a package under `internal/core/` importing `internal/component/ike/wire` inverts the tier direction the move exists to fix, so no shape keeps the body byte-identical. Both polarities are still proven on the peer's own units: three request identifiers of 200, 7 and 91 still defeat a round counter and a constant. The claim text was NARROWED to stop where the body stops, per `ai/rules/rfc-compliance.md`, and it names where the carrier half went. No assertion was dropped: `TestEapfmtCarrierKeepsTheResponseIdentifier` (`internal/component/ike/wire/payload_eap_carrier_test.go`) asserts wire octet 1 over the same peer session and the same identifier 200. |
| TestEapfmtSuccessAndFailureCarryNoTypeField | Same approval, same move, same cause, and the same two ids. The EAP-layer half stays here and still proves the four-octet terminal packet carries no Type field. The carrier half is `TestEapfmtCarrierSuccessAndFailureStayFourOctets` (`internal/component/ike/wire/payload_eap_carrier_test.go`), which encodes a Success and a Failure carrying `TypeData` of `{9, 9, 9}` and asserts the payload still emits four octets, so a carrier that leaked the type data would fail it. The new file is untagged on purpose: Thomas was offered moving the ids to it, or tagging both halves, and chose neither. Three rows were written in `test/weakened.md` for the split itself. |
