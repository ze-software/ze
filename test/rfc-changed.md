...
| Test | Reason |
|------|--------|
| checkAddPathReadvertiseCollision | Thomas approved `plan/spec-restore-bespoke-interop-assertions.md` on 2026-08-31, scope settled at all fifteen bespoke checkers with no trimming, and its AC-1 names this function. The change DELETES a guard and adds assertions; it removes none. The body opened with `checkScenario(ctx, check, "bgp-addpath-readvertise-collision-frr")`, and `checkScenario` (`check_engine.go:66`) answers `scenario %s has no typed assertions` for every name absent from `scenarioOperations`. This name is absent by design, because `TestCheckerPopulationMatchesProducer` refuses a bespoke checker that also holds a generic fallback, so the guard always fired and RFC7911-2-2 had never executed one line of its evidence. After the change the function waits for both sessions, requires ADD-PATH receive negotiated, injects GoBGP's path, compares the two Path Identifiers FRR received, forces a re-establishment and compares them again. RFC7911-2-2 is proven for the first time rather than still proven. |
| checkRFC2545NextHops | Thomas approved `plan/spec-restore-bespoke-interop-assertions.md` on 2026-08-31, and its AC-1 and AC-10 name this function. `rfc/requirements/rfc2545.md` binds RFC2545-3-2 and RFC2545-3-3 to it by function name as interop evidence, and the same always-firing `checkScenario` guard meant neither requirement had ever executed its assertion. The change deletes the guard and ADDS the assertion the RFC2545-3-3 positive tag already claimed but the body did not make: FRR installs 2001:db8:5601::/48 through the link-local next hop, read from `show ipv6 route bgp` and required on the ONE line that names the prefix. The next-hop shape assertions, the session wait and the two route waits are added beside it. Both requirements gain evidence; neither loses any. |
