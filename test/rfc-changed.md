...
| Test | Reason |
| ------ | ------ |
| TestMEDRemovalMechanismIsConfigurable | Thomas approved this on 2026-09-05, in answer to the change put to him with the discrimination evidence beside it. The edit is one call site and no assertion: `textDeltaToModOps` gained the pooled scratch it now carves value bytes from (spec-perf-next-2-filter-delta-alloc, Phase B), so the call reads `textDeltaToModOps(newTestScratch(t), origAttrs, modAttrs, &mods)`. The `require.Zero` beside it and its message are untouched. `RFC4271-5.1.4-2 negative` is still proven: the export converter emitting a MED operation was mutated in and the test went RED on that exact message, then GREEN on restore. One thing the walk surfaced and this row records rather than hides: removing `faMEDRemove` from the converter's skip list does NOT redden the test, because the directive also has no encoder in the generic path, so no operation is emitted either way. The arm therefore pins the OUTCOME the MUST cares about, no removal on export, rather than the skip that is one of two reasons for it. The claim is honest at the outcome level and no wider than the assertion. |
