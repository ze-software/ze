...
| Test | Reason |
| ------ | ------ |
| TestSuccessIdentifierMatchesResponse | Thomas approved this on 2026-09-01, after reading what changes and why. The id is `RFC3748-4.2-4 positive` and it is still proven by the same assertion over the same oracle. What changes is the FIXTURE, not the test: it built a Session whose outstanding Identifier was 3 and fed it a Response carrying 0x5A. `(*Session).Process` now discards that Response, because RFC 3748 Section 4.1 states "An authenticator receiving a Response whose Identifier value does not match that of the currently outstanding Request MUST silently discard the Response", so the old fixture drives the discard instead of the Success arm it exists to check. The outstanding Identifier is set to 0x5A, which keeps every property the case discriminated: the value is still distinctive, so a terminal packet built from the session counter would carry the counter's own value rather than this one, and the assertion that Success echoes 0x5A still fails against that break. |
| TestFailureIdentifierMatchesResponseOnNAK | Same approval, same reason, id `RFC3748-4.2-4 positive`. The outstanding Identifier is pinned to 1 before the identity round and to 0x7E before the NAK, so both Responses answer the Request actually outstanding at that moment. The assertion that the Failure carries 0x7E is untouched. |
| TestIdentityFailureIdentifierMatchesResponse | Same approval, same reason, id `RFC3748-4.2-4 positive`. The outstanding Identifier is set to 0x11 before the Response. The assertion that the Failure carries 0x11 is untouched. |
