...
| Test | Reason |
| ------ | ------ |
| TestRadiusAdminEapAcceptWithEapFailureStillAuthorizes | Thomas approved this on 2026-09-04, in answer to the change put to him: he chose "approve both" over the new test alone and over the log-line alternative. The other half of that decision, the new `TestRadiusAdminEapProcessesTheEAPMessageBeforeTheCode`, needs no row here: it is added against HEAD rather than changed, and the commit gate refuses a row for a test it does not compute as changed. This test's ASSERTION is unchanged and was always true: an Access-Accept carrying an EAP packet the peer rejects still logs the operator in. What was wrong is that it could not have been RED. Its prose claimed it prevents the defect the same commit fixed, `authenticateEAP` returning the peer's error before it read `resp.Code`, and its fixture concluded AFTER the method, where the peer sits in peerStateMethodDone and RFC 3748 Section 4.2 makes `(*PeerSession).Process` discard the EAP-Failure silently: `processEAPMessage` returned nil, so an early `return eapErr` would have returned nil too (`plan/journal/green-that-could-not-have-been-red.md`, 2026-09-04). The fixture now concludes at the identity round, where the peer is in peerStateIdentity and the same Failure produces ErrEAPFailure, so the early-return defect is reachable. `RFC3579-2.6.3-1 negative` is still proven: a decision following anything other than the RADIUS Packet Type would refuse this login, and it succeeds. `RFC3579-2.6.3-2 positive` is still proven: the reply carries an EAP packet the peer does not accept and the login is granted. Both are proven by strictly more than before, because the peer's objection now exists to be overridden. The record this test carried was invalidated by the body change and both tags were re-recorded with `./le rfc discriminate-record`. |
