...
| Test | Reason |
|------|--------|
| TestFlowSpecComponentsAscendingOrder | Thomas approved this adaptation on 2026-08-30, in answer to a direct question naming the change and this file's owner-only rule. `FlowSpec.AddComponent` gained an `error` return, because RFC 8955 Section 4.2 allows a component type "exactly once" and the model now merges a repeat rather than letting the encoder emit two components of one type. Every call site in the tree moved with it. In this test the whole change is `fs.AddComponent(x)` becoming `mustAddComponent(fs, x)`, a `t.Helper()` that fails the test when the call errors. No assertion was altered, reordered, or removed, and no tag was dropped: RFC5575-4-3, RFC5575-4-4, RFC8955-4.2-1 and RFC8955-4.2-2 keep the same positive tags on this function, and the ascending-order assertion they name is untouched. The adaptation is a strengthening rather than a weakening, because the return value it now checks was previously not checked at all. |
| TestFlowSpecIPv4Basic | Thomas approved this adaptation on 2026-08-30, under the same answer and for the same mechanical reason as `TestFlowSpecComponentsAscendingOrder`: two `fs.AddComponent(...)` calls became `mustAddComponent(fs, ...)` after `AddComponent` gained an `error` return. No assertion changed. RFC5575-4-2 keeps its positive tag, and the AFI 1 / SAFI 133 pair it names is asserted exactly as before. |
| TestFlowSpecIPv6Basic | Thomas approved this adaptation on 2026-08-30, under the same answer and for the same mechanical reason: the `fs.AddComponent(...)` calls became `mustAddComponent(fs, ...)` after `AddComponent` gained an `error` return. No assertion changed. RFC8956-2-2 keeps its positive tag, and the AFI 2 / SAFI 133 pair it names is asserted exactly as before. |
