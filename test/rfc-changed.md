...
| Test | Reason |
|------|--------|
| TestRFC8671ReservedPeerFlagsIgnoredOnReceipt | The divergence replay of origin/main commit f1245bbf4 (Thomas ordered the replay, 2026-08-31). The test becomes table-driven over two base flag sets rather than one. It ran only the set with V, L, A and O all set, which cannot catch a decoder that ORs a reserved bit into L or A because those bits are already set; the second base sets V and O alone, and can. Both bases now run, so the `RFC8671-4-1` claim is strictly better proven and no case is removed. |
| TestRFC8671AdminLabelOrderPreserved | The same replay. The label fixture widens from two labels to three, which is what the other side of the divergence had. Ordering is the property under test, and three elements distinguish a stable order from a swap where two cannot. No assertion is weakened and `RFC8671-6.3.1-1` is unchanged. |
| TestRFC8671AdminLabelReversedOrderPreserved | The same replay, and the same widening from two labels to three on the reversed-order polarity of `RFC8671-6.3.1-1`. The pair is kept symmetric on purpose: a reversed-order test over two elements proves less than the forward one it is paired with. |
