...
| Test | Reason |
| ------ | ------ |
| TestInitialSyncClosesTheQueueGateBeforeItWaitsForRoutePushingPlugins | APPROVED by Thomas, 2026-09-01. He was shown the two lines that move and the reading below, and answered "commit". What changes is the API-sync barrier (handover item W-1): `resetAPISync` takes the NAMES of the route-pushing bindings instead of a count, and `SignalAPIReady` takes the `plugin.Sender` that reported, so a report can be credited to the process that sent it. Two lines of this test move with those signatures. `peer.resetAPISync(1)` becomes a `ProcessBindings` literal naming one route pusher plus `peer.resetAPISync([]string{"pusher"})`, and `peer.SignalAPIReady()` becomes `peer.SignalAPIReady(plugin.ProcessSender("pusher"))`. No assertion, no expected value, no quoted RFC sentence, no test name and no tag changes. The id is `RFC4724-4-1 positive`, and it is still proven, by the same four assertions over the same oracle: the test still enters the wait through `triggerClock.waiting`, still asserts the marker is off the wire and `pendingSync` true inside it, still asserts `shouldQueue` and `forwardOrderHold` false there, and still asserts the marker reaches the wire once the signal arrives. The barrier it observes is strictly narrower after the change, because only the named process releases it. |
