...
| Test | Reason |
|------|--------|
| TestRFC7607AS4PathReachesTheAttributeWalk | Thomas ordered the repair on 2026-08-31 during the divergence replay. The test was red. Its fixture was an AS4_PATH attribute alone with NLRI declared present, so the walk recorded the three RFC 7606 Section 3.d missing-mandatory-attribute verdicts for ORIGIN, AS_PATH and NEXT_HOP, and treat-as-withdraw outranked the attribute discard the test asserts. That fixture could not prove `RFC7607-2-3` either way: the verdict it observed came from the absent attributes and not from the AS 0. The fixture now declares no NLRI, so Section 3.d asks for nothing and Section 5.2 escalates only an action stronger than attribute discard, which leaves the AS4_PATH as the only attribute in the UPDATE and the only source of a verdict. The test also asserts the result names attribute code 17, so the discard is pinned to AS4_PATH rather than to any other attribute. `RFC7607-2-3` positive is proven where it was not proven before, and no case is removed. |
