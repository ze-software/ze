...
| Test | Reason |
| ------ | ------ |
| TestRADIUSAuthNASIPAddress | Thomas ordered this work on 2026-08-31: prove the three RFC 2865 MUSTs that are implemented and untested, and prefer tagging and extending the existing NAS-IP-Address test over writing a duplicate. This is the gate reporting an ADDED tag, the first row of the "What this gate cannot see" table above: at HEAD the test carries no `RFC requirement:` tag at all, so no compliance claim loses its evidence and nothing is retired. The change adds the tag `RFC2865-4.1-2 positive` over the NAS-IP-Address assertion the test already made, adds the tag `RFC2865-4.1-1 positive`, and adds the assertion that tag needs: the decoded request Code field equals 1 (Access-Request). Both requirements are proven where neither was proven before, and no assertion is removed or weakened. Recorded in `rfc/discrimination/rfc2865.json`. |
