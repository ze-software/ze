...
| Test | Reason |
| ------ | ------ |
| TestVirtualIPPoolExhausted | Thomas ordered the gap on RFC3948-5.1-1 on 2026-08-31, and the same ruling he gave for RFC8671-6.2-1 the same day applies unchanged: a requirement whose behavior nothing produces is not proven by a test that calls the producer directly. The approved change REMOVES the tag `RFC requirement: RFC3948-5.1-1 negative` and keeps every assertion and every line of the body. The claim was false, not merely wide: it said the security gateway refuses rather than hand a second client an address already in use, while `Pool.Allocate` has no non-test caller and `registerIKE` discards the pool, so the gateway hands out nothing and refuses nothing. Narrowing was not open, because `checkStaleAnnotations` in `internal/le/rfc/check_core.go` refuses any tag on a requirement annotated `{gap}`. |
| TestVirtualIPPoolNeverHandsOneAddressTwice | The same 2026-08-31 ruling, for the positive half. The approved change removes the tag `RFC requirement: RFC3948-5.1-1 positive` and keeps every assertion. Its claim was the plainer falsehood of the two: that the gateway assigns each client a locally unique address instead of carrying the address the client brought. No address is ever assigned, because no engine code builds a Configuration payload and the pool is discarded at construction. The test still proves what its body checks, that the pool never hands one address to two callers, and it keeps running as an ordinary unit test. `plan/spec-ike-virtual-ip-assignment.md` carries the work that would earn the tag back. |