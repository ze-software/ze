...
| Test | Reason |
| ------ | ------ |
| new-v6 | Thomas approved this on 2026-09-05, in answer to the change put to him with the measurement beside it: "ok fix the red". The edit is three `cmd=api:...text=` lines and NO assertion. Each drove the file with `peer * update text ...`, a path spec-fixit-send-names-its-destination deleted, so each now reads `send bgp * update text ...`. `RFC2545-3-1 negative`, `RFC2545-3-2 negative` and `RFC2545-3-4 positive` are still proven, and the reason is that none of them is asserted by the line that changed: all three are pinned by the `expect=bgp:...hex=` lines, which are byte-identical in the diff. `800E26 0002 01 10` followed by ONE 16-octet address is what carries -3-1 and -3-4, and the `10` octet in it is -3-2. What the respelling changes is whether the assertions are REACHED. Measured both ways on this tree: at HEAD the file is one of the 33 encode failures, because the command matches nothing, no UPDATE is produced and the peer times out with `received messages: 0` -- so the evidence for all three requirements is currently absent, not weaker. With the respelling `./le functional encode` answers 58 of 59, the one remaining failure being `nexthop-self-ipv6-encode` asking for `fd00::2` on this host's loopback. The change RESTORES the proof rather than trading it. |
| med-locally-set-reaches-peer | Thomas approved this on 2026-09-05, in the same answer. The edit is one `cmd=api:...text=` line and NO assertion: `peer * update text origin igp med 100 ...` becomes `send bgp * update text origin igp med 100 ...`. `RFC4271-5.1.4-1 negative` is still proven for the same reason: the MUST NOT is about a MULTI_EXIT_DISC RECEIVED from a neighboring AS, the file discriminates that from a metric ze ORIGINATES toward AS 65001, and what pins it is the `80 04 04` header in the untouched hex beside the annotated attribute breakdown. The announce rail (`writeAnnounceUpdate`, `reactor/reactor_wire.go`) it exercises is unchanged, and so is the sibling it must disagree with, `med-not-propagated-across-as.ci`. One thing this row records rather than hides: unlike `new-v6`, the RED half was not separately measured for this file. Its green half was (it is absent from the failures of two `./le functional plugin` runs over the respelled tree), and the mechanism of the red is the same dead command, but nobody has watched this one fail. |
