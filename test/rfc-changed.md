...
| Test | Reason |
|------|--------|
| prefix-maximum-enforce | Thomas approved correcting the Cease/MaxPrefixes Data field on 2026-08-24, asked as "we want RFC compliance, so why is it even a question". That approval landed the code and the Go tests and MISSED this fixture and two siblings, so HEAD went red on three functional tests and was pushed that way. The hex read `...00000003`, the count that crossed the bound; it now reads `...00000002`, the configured maximum, which is what RFC 4486 Section 4 Figure 1 labels "Prefix upper bound". The fixture is not being changed to match broken code: it is being completed to match the approved change, and its own header now states that the count is 3 and never reaches the wire. `RFC4486-4-1` stays proven, and it is the only requirement this file is tagged for: the assertion still pins `03` for Cease and `06 01` for the code and the MaxPrefixes subcode, which is the whole of what that MUST requires. Only the four octets `RFC4486-4-10` governs moved. Measured: with the daemon reverted to sending the count, this fixture fails on the same byte. |
