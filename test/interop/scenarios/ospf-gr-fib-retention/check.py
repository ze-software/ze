#!/usr/bin/env python3
"""Scenario ospf-gr-fib-retention: non-stop forwarding across a Ze OSPFv2 restart.
...
def check():
    ...
    #   2. an ICMP probe to the FRR-advertised prefix keeps succeeding (no gap);
    #   3. after GR exit the route is refreshed (RTPROT_ZE, not swept).
    log_pass("ospf-gr-fib-retention: RTPROT_ZE routes retained across the restart")
