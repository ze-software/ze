...
OUTPUT_ROOT = SOURCE_ROOT.parent / "_site"
MAIN_REPO = SOURCE_ROOT.parent / "main"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Build the website, serve ../_site, and open it in a browser."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-build",
        action="store_true",
        help="serve the existing ../_site artifact without rebuilding it",
    )
    parser.add_argument(
        "--no-open",
        action="store_true",
...
def main():
    ...
    if not (OUTPUT_ROOT / "index.html").is_file():
        raise SystemExit(
            "website artifact not found; run without --no-build to generate ../_site"
        )
