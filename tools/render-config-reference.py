...
Each group's whole configuration is shown as one merged, collapsible tree
-- generated by running the live `ze yang tree --json --config` (cached by
tools/extract-yang-config-tree.py to data/yang-config-tree.json), the same
unified YANG walker Ze's own CLI uses (it already merges every config YANG
module in the tree, not just one plugin's own file), sliced to the subtree
whose name matches the group's key. Every container and YANG list becomes
a nested <details> so a 600-node subtree (e.g. bgp) opens compact -- only
the top level expanded by default -- and a reader drills into just the
part they care about, rather than reading raw YANG module source or a
fully-unrolled wall of text. Plugin YANG modules (resolved by
extract-plugin-registry.py via its real Go import statement, not a
directory-name guess) are linked to their real source on Codeberg for
whoever wants the actual schema, not embedded inline -- the merged tree
already carries the same information in a form a non-developer can read.
"""
...
def render_plugin(plugin):
    ...
    if plugin["yang_files"]:
        links = " &middot; ".join(
            render_yang_link(rel_path) for rel_path in plugin["yang_files"]
        )
        parts.append('<p class="config-plugin-meta">YANG source: %s</p>' % links)
    else:
        parts.append(
...
def node_label(node):
    """The always-visible line for a node -- shown in a <summary> for a
    branch or directly in a leaf's <li>, so scanning collapsed summaries
    alone already tells a reader what's there without expanding anything."""
...
def render_config_tree_node(node, depth):
    children = node.get("children") or []
    label = node_label(node)
    if not children:
        return "<li>%s</li>" % label
    inner = "".join(render_config_tree_node(child, depth + 1) for child in children)
    open_attr = " open" if depth == 0 else ""
    return (
        '<li><details class="yang-branch"%s><summary>%s</summary>'
        "<ul>%s</ul></details></li>"
    ) % (open_attr, label, inner)
...
def render_config_tree(root_node):
    ...
    parts = [
        ...
        '<p class="config-tree-hint">Whole config for this group, merged '
        "from every YANG module that contributes to it and shown as one "
        "collapsible tree -- click a section to expand it.</p>",
        '<ul class="yang-tree">',
    ]
    for child in children:
        parts.append(render_config_tree_node(child, 0))
    parts.append("</ul></div>")
    return "\n".join(parts)
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                input.addEventListener("input", function () {
                    ...
                    groups.forEach(function (group) {
                        ...
                        var tree = group.querySelector(".config-tree");
                        var treeMatch =
                            q !== "" &&
                            tree &&
                            tree.textContent.toLowerCase().indexOf(q) !== -1;
                        var anyPlugin = false;
                        plugins.forEach(function (plugin) {
                            var match = q === "" || plugin.textContent.toLowerCase().indexOf(q) !== -1;
                            plugin.style.display = match ? "" : "none";
                            if (match) anyPlugin = true;
                        });
                        // The merged tree is a group's whole config, so a query
                        // that only matches a leaf deep in the tree still shows
                        // the group (native find-in-page then expands the
                        // matching <details> on its own).
                        var show = q === "" || anyPlugin || treeMatch;
...
def render_plugin_markdown(plugin):
    ...
    if plugin["yang_files"]:
        ...
        for rel_path in plugin["yang_files"]:
            filename = rel_path.rsplit("/", 1)[-1]
            if read_yang(rel_path) is None:
                links.append("%s *(not found on disk at render time)*" % filename)
            else:
                links.append("[%s](%s)" % (filename, yang_source_href(rel_path)))
        parts.append("YANG source: %s" % ", ".join(links))
        parts.append("")
    else:
        parts.append(
...
def render_config_tree_markdown(root_node):
    ...
    if not children:
        return ""
    lines = [
        "Whole config for this group, merged from every YANG module that "
        "contributes to it.",
        "",
    ]
    for child in children:
