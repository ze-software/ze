...
| Fuzz target | Does the same rule hold for generated inputs and saved corpus entries? | `make ze-fuzz-one FUZZ=FuzzParseNLRI PKG=./internal/component/bgp/wire/ TIME=30s` |
| gomu mutation run | Would the tests fail if the implementation made a small wrong decision? | `make ze-mutation-changed` |
| Race run | Does the behavior still hold when goroutines are scheduled differently? | `make ze-unit-test-race-changed` |
| Coverage report | Which branches ran without a strong assertion attached? | `make ze-unit-test-cover` |
...
| BGP group | `make ze-test-bgp` | Wire, FSM, peer, and BGP component changes. |
| Core group | `make ze-test-core` | Core libraries and shared infrastructure. |
| Plugin group | `make ze-test-plugins` | Runtime plugin logic and plugin boundaries. |
| Config group | `make ze-test-config` | YANG, config parsing, validation, and rendering. |
| CLI group | `make ze-test-cli` | Command parsing and user-visible formatting. |
