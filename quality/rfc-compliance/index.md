...
```
rfc-requirements OK: 2720 gated MUST-level requirement(s) across 166 enrolled RFC(s); 2575 test tag(s) resolved.
```

| Metric | Value |
|---|---:|
| Gate issues | 0 |
| Gated MUST-level requirements | 2,720 |
| Enrolled RFCs | 166 |
| Resolved test tags | 2,575 |
| Declared gaps | 535 |
| RFCs with declared gaps | 84 |
...
|---|---:|---:|---|
| Positive and negative tests | 974 | 35.8% | `positive tag + negative tag` |
| One polarity plus reason | 370 | 13.6% | `{single-polarity} annotation + required tag` |
| Not applicable | 841 | 30.9% | `{not-applicable} annotation` |
| Declared gap | 535 | 19.7% | `{gap} annotation + public ledger disclosure` |
...
- **RFC 1350:** RFC1350-2-3 unmet (Sorcerer's Apprentice fix): sendAndWaitACK retransmits DATA on any non-matching ACK (handler.go:407) instead of silently ignoring a duplicate or stale ACK.
- **RFC 4301:** One MUST gap (RFC4301-4.4.1.1-1): SPD selectors are limited to IP-prefix + next-layer-protocol; TCP/UDP/SCTP port and ICMP type/code selectors are not modeled or projected (internal/component/ike/dataplane/dataplane.go:110-130; engine/initiator.go:325 tsToIPNet drops ports).
...
- **RFC 7313:** Four MUST-level receive-side gaps annotated in `rfc/short/rfc7313.md`: RFC7313-4-4/4-5 -- a received BoRR/EoRR is log-only (internal/component/bgp/plugins/rib/rib.go:751-756), so ze marks no Adj-RIB-In routes stale and purges none; and RFC7313-4-6/4-7 -- neither the send nor receive path applies a Graceful-Restart End-of-RIB gate to BoRR emission or acceptance.
...
|---|---|---|
| Requirement source | `rfc/short/*.md` | 2,720 gated MUST-level requirements |
| Enrollment | `rfc/enrolled.txt` | 166 enrolled RFCs |
| Test tags | `internal/, pkg/, test/` | 2,575 resolved tags |
| Public ledger | `docs/features/rfc-status.md` | 84 RFCs with gaps |
| Semantic audits | `rfc/audit/*.json` | 52 fresh, 0 stale, 2,668 missing |
