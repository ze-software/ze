...
|---|---:|---|---|
| Tested both ways | 61.5% | 8 of 13 gated MUSTs | a positive test proves Ze does what the requirement demands and a negative one proves it refuses what the requirement forbids |
...
**Positive and negative tests (8):** [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-3-2`](#draft-ietf-idr-linklocal-capability-3-2), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-1`](#draft-ietf-idr-linklocal-capability-4-1), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-2`](#draft-ietf-idr-linklocal-capability-4-2), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-3`](#draft-ietf-idr-linklocal-capability-4-3), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-4`](#draft-ietf-idr-linklocal-capability-4-4), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-7`](#draft-ietf-idr-linklocal-capability-4-7), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-8`](#draft-ietf-idr-linklocal-capability-4-8), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-9`](#draft-ietf-idr-linklocal-capability-4-9)

**Annotated instead of tested (5):** [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-3-1`](#draft-ietf-idr-linklocal-capability-3-1), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-5`](#draft-ietf-idr-linklocal-capability-4-5), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-6`](#draft-ietf-idr-linklocal-capability-4-6), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-5-1`](#draft-ietf-idr-linklocal-capability-5-1), [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-6-1`](#draft-ietf-idr-linklocal-capability-6-1)
...
| `DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-3-1` | "If an implementation intends to send a single IPv6 Link-Local forwarding address in the Next Hop field of the MP_REACH_NLRI, it MUST set the length of the Next Hop field to 16 and include only the IPv6 Link-Local address in the Next Hop field" (§3) | MUST | 3 | **positive:** no positive test. **negative:** no negative test. **{gap}:** no ze producer ever intends to send a single IPv6 Link-Local forwarding address. precomputeNextHop (internal/component/bgp/reactor/peer_forward_facts.go) derives the next hop from the session's global address alone, applyLinkLocalNextHop (internal/component/bgp/reactor/link_scope.go) writes a link-local address only as the SECOND of two, and ValidateGlobalNextHop (internal/core/bgp/attribute/nexthop_form.go) refuses a link-local address in the first slot, so no path selects the 16-octet link-local-only form |
...
| `DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-5` | "A Route Reflector (RR) reflecting a route with a link-local-only next hop MUST NOT advertise that route to a client unless the client shares the same link-layer segment as the original advertiser" (§4) | MUST NOT | 4 | **positive:** no positive test. **negative:** no negative test. **{gap}:** ze acts as a route reflector ((*routeReflector).forwardUpdate, internal/component/bgp/plugins/rr/rr.go, fans a cached UPDATE to every peer) and nothing on that path classifies a next hop as link-local-only: parseNextHops (internal/core/bgp/attribute/mpnlri.go) runs no fe80::/10 test on a 16-octet next hop, so the route is reflected to a client whatever link-layer segment that client is on |
| `DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-6` | "For all other clients, the RR MUST either rewrite the next hop to its own address (next-hop-self) or consider the route ineligible for advertisement to that specific peer" (§4) | MUST | 4 | **positive:** no positive test. **negative:** no negative test. **{gap}:** the reflection path names no next hop at all ((*routeReflector).forwardUpdate, internal/component/bgp/plugins/rr/rr.go), and applyFactsNextHop (internal/component/bgp/reactor/peer_forward_facts.go) rewrites one only where the operator configured a next-hop mode for that peer, so ze neither rewrites the next hop to its own address for a client off the advertiser's segment nor makes the route ineligible for it |
...
| `DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-5-1` | "When this combination has not been negotiated, a sender MUST follow the rules in Section 3 of [RFC8950] and encode the Next Hop as 32 octets" (§5) | MUST | 5 | **positive:** no positive test. **negative:** no negative test. **{gap}:** no producer conditions the IPv4-NLRI next-hop encoding on this capability combination. parseCapability (internal/core/bgp/capability/capability.go) has no case for code 77, so ze records no negotiated state to read, and (*RIBManager).injectRoute (internal/component/bgp/plugins/rib/rib_commands.go) builds the MP_REACH for an IPv4 NLRI with the one IPv6 address it was given, a 16-octet Next Hop field rather than the 32 octets this requirement asks of a sender outside the negotiated combination |
...
|---|---|---|
| [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-3-1`](#draft-ietf-idr-linklocal-capability-3-1) "If an implementation intends to send a single IPv6 Link-Local forwarding address in the Next Hop field of the MP_REACH_NLRI, it MUST set the length of the Next Hop field to 16 and include only the IPv6 Link-Local address in the Next Hop field" (§3) | {gap}, no test | no ze producer ever intends to send a single IPv6 Link-Local forwarding address. precomputeNextHop (internal/component/bgp/reactor/peer_forward_facts.go) derives the next hop from the session's global address alone, applyLinkLocalNextHop (internal/component/bgp/reactor/link_scope.go) writes a link-local address only as the SECOND of two, and ValidateGlobalNextHop (internal/core/bgp/attribute/nexthop_form.go) refuses a link-local address in the first slot, so no path selects the 16-octet link-local-only form |
| [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-5`](#draft-ietf-idr-linklocal-capability-4-5) "A Route Reflector (RR) reflecting a route with a link-local-only next hop MUST NOT advertise that route to a client unless the client shares the same link-layer segment as the original advertiser" (§4) | {gap}, no test | ze acts as a route reflector ((*routeReflector).forwardUpdate, internal/component/bgp/plugins/rr/rr.go, fans a cached UPDATE to every peer) and nothing on that path classifies a next hop as link-local-only: parseNextHops (internal/core/bgp/attribute/mpnlri.go) runs no fe80::/10 test on a 16-octet next hop, so the route is reflected to a client whatever link-layer segment that client is on |
| [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-4-6`](#draft-ietf-idr-linklocal-capability-4-6) "For all other clients, the RR MUST either rewrite the next hop to its own address (next-hop-self) or consider the route ineligible for advertisement to that specific peer" (§4) | {gap}, no test | the reflection path names no next hop at all ((*routeReflector).forwardUpdate, internal/component/bgp/plugins/rr/rr.go), and applyFactsNextHop (internal/component/bgp/reactor/peer_forward_facts.go) rewrites one only where the operator configured a next-hop mode for that peer, so ze neither rewrites the next hop to its own address for a client off the advertiser's segment nor makes the route ineligible for it |
| [`DRAFT-IETF-IDR-LINKLOCAL-CAPABILITY-5-1`](#draft-ietf-idr-linklocal-capability-5-1) "When this combination has not been negotiated, a sender MUST follow the rules in Section 3 of [RFC8950] and encode the Next Hop as 32 octets" (§5) | {gap}, no test | no producer conditions the IPv4-NLRI next-hop encoding on this capability combination. parseCapability (internal/core/bgp/capability/capability.go) has no case for code 77, so ze records no negotiated state to read, and (*RIBManager).injectRoute (internal/component/bgp/plugins/rib/rib_commands.go) builds the MP_REACH for an IPv4 NLRI with the one IPv6 address it was given, a 16-octet Next Hop field rather than the 32 octets this requirement asks of a sender outside the negotiated combination |
