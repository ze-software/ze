...
Unsupported. Every requirement this repository extracted from RFC 8538, the tests bound to it, and what a reader has verified about them. This summary is not enrolled.
...
Cease Hard Reset subcode (9) name is known.

**What the ledger says remains:**

The GR N-bit is decoded but not acted upon; routes are dropped on any NOTIFICATION, so RFC 8538 route retention is not implemented.

## Coverage

RFC 8538 declares no MUST-level requirement, so the gate counts nothing here.

## Requirements

RFC 8538 declares no requirement, so this summary generates no shard.

## Gaps and untested MUSTs

RFC 8538 declares no gap, and every gated MUST it carries has a test bound to it.

## Proof state

RFC 8538 carries no gated, tagged or audited requirement, so there is no proof state to state.

## Extraction sign-off

No extraction sign-off exists for RFC 8538, so no reviewer has walked its text sentence by sentence.

## Superseded

No document obsoletes RFC 8538, so its obligations are stated where they were written.
