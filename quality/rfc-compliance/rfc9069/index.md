...
|---|---:|---|---|
| Tested both ways | 46.7% | 7 of 15 gated MUSTs | a positive test proves Ze does what the requirement demands and a negative one proves it refuses what the requirement forbids |
| One polarity plus reason | 53.3% | 8 of 15 gated MUSTs | the requirement admits no counter-case, so one polarity plus a recorded reason is the whole proof available for it |
...
| Proven by a recorded break | 43.5% | 10 of 23 tagged units | a red was observed once under a recorded procedure, and the unit, the claim and the producer it rested on still hash to what was recorded. The break is not re-run. A test pair is not a proof until one has been observed |
...
Enrolled: Support for Local RIB in the BGP Monitoring Protocol (BMP): fifteen MUST-level requirements, all met (ze implements the Loc-RIB Instance Peer type, on the sender side and the receiver side). The per-peer header carries Flags 0 (x-1), a zero Peer Address (x-5), the router's own 4-octet ASN as Peer AS (x-6), the local router-id as Peer BGP ID (x-7) and the install time of the routes it encapsulates or zero where that is unknown (5.1-1); both values come from `bgp router-id` and `bgp session asn local`, so they are known before any BGP peer comes up, which is the case Section 1.1 names. The Peer Up carries a fabricated OPEN repeated into the received field (x-3) advertising the 4-octet ASN capability and one address family per family the dump delivers (5.2-1), plus the VRF/Table Name TLV naming that Loc-RIB `global` and repeated on the Peer Down (5.2.1-1); the Peer Down uses reason code 6 (5.3-1). Route Monitoring conveys locally sourced routes under the Loc-RIB peer type (4.2-1) with 4-byte ASN encoding (5.4.1-1), one Peer Up per RIB instance per session (x-2), a full-table dump when monitoring starts (x-4), and a Peer Down / Peer Up bounce when a change alters what the session carries (6.1.3-1). The receiver reads the address families off a Peer Up OPEN, for every peer type including 3, and reports them per peer (6.1.1-1). Tests bound per requirement in rfc/requirements/rfc9069.md.
...
| `RFC9069-4.2-1` | Locally sourced routes communicated by BMP are conveyed using the Loc-RIB Instance Peer Type (§4.2). Proven positive by TestLocRIBFeedConveysRoutesWithTheLocRIBPeerType, which reads the Peer Up and the Route Monitoring one best change puts on the wire and requires Peer Type 3 on both, and negative by TestPeerHeaderFromEvent, where the header of a monitored BGP peer carries Peer Type 0: the type discriminates the Loc-RIB feed rather than being stamped on every message | MUST | 4.2 - Peer Flags | **positive:** `unit/verify` [`TestLocRIBFeedConveysRoutesWithTheLocRIBPeerType`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L136). **negative:** `unit/verify` [`TestMonitoredPeerRouteMonitoringIsNotTheLocRIBPeerType`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L157) |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| positive | [`TestLocRIBSinglePeerUpPerInstance`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/bmp_locrib_test.go#L482) | unit/verify | unproven |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| positive | [`TestHandleBestChangeEmitsPeerUpThenRM`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/bmp_locrib_test.go#L415) | unit/verify | unproven |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| positive | [`TestStartLocRIBTriggersInitialDump`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/bmp_locrib_test.go#L582) | unit/verify | unproven |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| positive | [`TestFabricatedLocRIBOpenCarriesTheRequiredCapabilities`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/bmp_locrib_test.go#L319) | unit/verify | unproven |
...
| positive | [`TestRFC9069ReloadTurningLocRIBOnAnnouncesTheRouterIdentity`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc8671_test.go#L1239) | unit/verify | unproven |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| positive | [`TestBMPReloadTurningLocRIBOffUnsubscribesAndSaysSo`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc8671_test.go#L1161) | unit/verify | unproven |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestMonitoredPeerRouteMonitoringIsNotTheLocRIBPeerType`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L157) | unit/verify | revert, verified |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestMonitoredPeerUpCarriesNoTableNameTLV`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L253) | unit/verify | revert, verified |
| positive | [`TestLocRIBPeerUpAndPeerDownCarryTheGlobalTableName`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L187) | unit/verify | revert, verified |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestReceiverRecordsNoFamiliesWhenThePeerUpAdvertisesNone`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L318) | unit/verify | revert, verified |
| positive | [`TestReceiverRecordsThePeerUpAddressFamilies`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L285) | unit/verify | revert, verified |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestAConfigurationThatAltersNoBehaviorBouncesNothing`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L493) | unit/verify | revert, verified |
| positive | [`TestBehaviorChangeBouncesThePeersOfALocRIBSession`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/plugins/bmp/rfc9069_test.go#L457) | unit/verify | revert, verified |
