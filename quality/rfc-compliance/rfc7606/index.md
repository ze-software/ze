...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestEnforceRFC7606_InvalidWithdrawnNLRI`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/reactor/session_validate_test.go#L177) | unit/verify | unproven |
...
| negative | [`TestEnforceRFC7606_InvalidTrailingNLRI`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/reactor/session_validate_test.go#L203) | unit/verify | unproven |
...
Audit verdict: enforced (the tests do what the requirement demands), fresh. Re-audited 2026-08-27. {single-polarity} positive remains correct because rejection cannot conform. Four message tests require ActionNone for late MP_UNREACH, both distinct MP attributes, MP_REACH plus legacy NLRI, and MP_UNREACH plus legacy NLRI; the earlier late-MP_REACH unit covers the other position. test/plugin/rfc7606-receive-combinations.ci drives those four shapes through the daemon and requires distinct relayed components, while rfc7606-relay-one-field requires the legacy Withdrawn Routes plus NLRI combination and rejects an UPDATE-error NOTIFICATION. routeServerReplay13 only waits for EoR and a final forwarded prefix, so the ze-peer expectations remain the discriminating evidence. The native interop check runs scenarioOperations plus withdrawal extras: FRR must keep the session, install the announcement, observe the withdrawal, and not retain the withdrawn route. A receive-side position or combination restriction prevents one of these exact outcomes. Re-audited 2026-09-13 against RFC 7606 Section 5.1 in rfc/full/rfc7606.txt, which states: "Since older BGP speakers may not implement these restrictions, an implementation MUST still be prepared to receive these fields in any position or combination." The verdict was STALE-UNIT rather than shifted: internal/le/interoplab/bgp/check_rfc.go::checkRFC7606MixedUpdate moved at commit 125ab22025, which replaced a one-line body of checkScenario(ctx, check, "bgp-rfc7606-relay-shape-frr") with eight ordinal assertions. That scenario name is absent from scenarioOperations by design, so the earlier body asserted nothing, and the interop sentence of the note above was an over-claim on the day it was written. The body now requires the FRR session, refuses a renumbered lab whose raw-hex NEXT_HOP octets name no peer, waits for the replayed 10.0.0.0/24 and requires FRR to attribute AS_PATH 65004 alone to it, requires the announced half 203.0.113.0/24 of the ONE UPDATE that mixes Withdrawn Routes with NLRI to reach FRR and to be INSTALLED through the injector address 172.30.0.9 on one line, requires the withdrawn half 198.51.100.0/24 to be absent behind that arrival, and requires the session a second time, because a receive-side rejection passes a NOTIFICATION. A ze that refused the mixed shape fails assertions 5, 6 and 8, and a rail that relayed nothing fails assertions 3 and 4, so the absence assertion is never read on its own. The other seven units of this verdict fingerprint byte-identical to what the 2026-08-29 pass judged. Verdict stays enforced, and the interop carrier enforces it for the first time. One reading is carried forward rather than acted on here: the checklist line names MP_REACH and MP_UNREACH, while the RFC sentence says "these fields", which also covers the non-empty Withdrawn Routes and NLRI fields of the second bullet. The evidence already proves the wider sentence, so the row under-states what is proven rather than over-claiming it.
...
Audit verdict: enforced (the tests do what the requirement demands), fresh. Re-audited 2026-08-27 across all 15 tagged units and the ingress producer. Positive unit evidence proves EVPN, MCAST-VPN and BGP-MUP recognized routes survive while an unrecognized route between them is removed; ADD-PATH offsets, MP_UNREACH, treat-as-withdraw synthesis, abandoned attribute walks, empty results, and unparseable typed framing have distinct exact outcomes. The BGP-LS negative requires an overriding family to preserve the same unknown type. The three functional fixtures retain complementary contains/reject wire assertions; compiled routeServerReplay13 only waits for EoRs and a later IPv4 fence. The native interop speaker oracle parses every relayed EVPN NLRI, fails on any type outside 1..5, and the checker requires result=PASS, established=yes, route-bearing-updates>=1 and evpn-nlri>=1, so absence is non-vacuous. Removing discard fails reject/PASS, while blanket or over-discard fails the surviving-route contains checks and BGP-LS negative. Targeted re-audit 2026-08-28: rfc7606-54-bgpls-override-propagates.ci only replaced its Python readiness observer with compiled routeServerReplay13 and enabled ze-peer linger. Its source UPDATE and contiguous known-plus-unknown BGP-LS contains assertion did not change. The observer waits for EoRs and the later IPv4 fence only; it cannot make the BGP-LS assertion pass. If Ze drops, rewrites, or reorders unknown type 99, the receiver no longer contains the asserted bytes. Targeted re-audit 2026-08-29: TestRFC7606Section54PropagatesUnknownBGPLSType changed its KNOWN fixture only, from a two-octet Node stub to a well-formed Node NLRI carrying its Protocol-ID, Identifier and a Local Node Descriptors TLV. Its assertion is unchanged: the MP_REACH survives and its NLRI section is byte-identical to what arrived, unknown type 99 included. The edit STRENGTHENS the case rather than weakening it. The old stub could not hold a Protocol-ID and Identifier, so RFC 9552 Section 8.2.2's syntactic walk would drop it as malformed, and the surviving-bytes assertion could have been satisfied by a different rule than the one under test. Discrimination is unchanged and rests on the nlritype registry: BGP-LS registers no recognizer, so the Section 5.4 filter does not fire for it. Register one that rejects type 99 and the equality assertion fails on the first differing byte. Re-ran the fourteen TestRFC7606Section54 units: all pass. Re-audited 2026-09-13 against RFC 7606 Section 5.4 in rfc/full/rfc7606.txt, which states: "A BGP speaker advertising support for such a typed address family MUST handle routes with unrecognized NLRI types within that address family by discarding them, unless the relevant specification for that address family specifies otherwise." The verdict was STALE-UNIT rather than shifted: internal/le/interoplab/bgp/check_rfc.go::checkRFC7606TypedNLRIDiscard moved at commit cb882fd581, which replaced a one-line body of checkScenario(ctx, check, "bgp-rfc7606-typed-nlri-discard") with two ordinal assertions. That scenario name is absent from scenarioOperations by design, so the earlier body asserted nothing, and the interop sentence of the note above was an over-claim on the day it was written. The body now polls the strict speaker's container log for its end-of-run report, then puts it through requireSpeakerEVPNDiscard (check_rfc_predicate.go:824), which requires the verdict line, the oracle name no-unrecognized-evpn-type, established yes, one route-bearing UPDATE and one decoded EVPN NLRI before it reads PASS. Read at the producer rather than taken from the note: applyEVPNOracle (speaker.go:502) walks every EVPN NLRI of each relayed MP_REACH and MP_UNREACH, counts it, and calls verdict.fail for a route type below 1 or above 5, so a relayed unassigned type cannot read as PASS. The four floors before PASS are what stop a relay that was broken outright from reading the same as a discard. The other fourteen units of this verdict fingerprint byte-identical to what the 2026-08-29 pass judged, the BGP-LS negative among them, which is the unless-otherwise-specified half. Verdict stays enforced, and the interop carrier enforces it for the first time. Targeted re-audit 2026-09-14: test/plugin/rfc7606-54-bgpls-override-propagates.ci changed its WIRE INPUT and the expect=bgp contains assertion that reads it back, so the verdict was STALE-UNIT rather than shifted. Commit 53d286faf replaced a Node NLRI of Total NLRI Length 11, a Protocol-ID, an 8-octet Identifier and two trailing octets that cannot frame a 4-octet TLV header, with the 13-octet well-formed form of Total NLRI Length 9, which is the same lsNode that internal/component/bgp/plugins/nlri/ls/unknown_type_skip_test.go carries. The assertion moved with the bytes and still pins the WHOLE NLRI section, 0001000902000000000000000000630004DEADBEEF, so unknown type 99 must arrive in place and unchanged beside the type ze parses. RFC 7606 Section 5.4 was read again in rfc/full/rfc7606.txt: "A BGP speaker advertising support for such a typed address family MUST handle routes with unrecognized NLRI types within that address family by discarding them, unless the relevant specification for that address family specifies otherwise." This carrier is the unless-otherwise half, and RFC 9552 Section 5.2 is the specification that says otherwise. Read at the producer rather than taken from the fixture comment: RetainWellFormedNLRI (internal/component/bgp/message/rfc7606_bgpls_nlri.go) frames NLRI 1 at [0,13) and NLRI 2 at [13,21) through bgplsNLRIBounds; bgplsNLRIWellFormed keeps NLRI 1 because its 9-octet body is exactly bgplsNLRIFixedLen, the Protocol-ID and the Identifier, leaving no octet the TLV walk must frame, and keeps type 99 at the first branch because the type is above bgplsNLRITop and nothing about an unrecognized type can be called malformed. dropped is 0, so the section is relayed byte-identical and the assertion can be met. The OLD bytes took the other branch: an 11-octet body leaves 2 octets after the fixed fields, off+bgplsTLVHeaderLen runs past the body, and NLRI 1 was discarded, so the previous assertion could not be satisfied by conforming code. The edit STRENGTHENS the carrier rather than weakening it. Discrimination is unchanged and still rests on the nlritype registry: BGP-LS registers no Section 5.4 recognizer, so a blanket discard of unrecognized types removes 00630004DEADBEEF and the contains assertion fails on the first differing byte. The other fourteen units fingerprint byte-identical to what the 2026-09-13 pass judged and were not re-judged. The fixture was not re-run in this pass, so this judgement rests on the producer walk over the exact wire bytes rather than on an observed green. Verdict stays enforced.
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestEnforceRFC7606_ShortBody`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/reactor/session_validate_test.go#L153) | unit/verify | unproven |
...
| negative | [`TestEnforceRFC7606_InvalidTrailingNLRI`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/reactor/session_validate_test.go#L201) | unit/verify | unproven |
...
| Polarity | Test | Kind and tier | Proof state |
|---|---|---|---|
| negative | [`TestEnforceRFC7606_InvalidWithdrawnNLRI`](https://github.com/ze-software/ze/blob/main/internal/component/bgp/reactor/session_validate_test.go#L175) | unit/verify | unproven |
