...
| `show X` offline shortcut answering DATA | `ze show plugin list` | `MustRegisterLocalData("show plugin list", handler, Meta{Mode: "offline"}, command.RenderLocalAnswer)`, plus `command.RegisterShape` and `command.RegisterColumns`, in the owner package. Template: `internal/component/plugin/register.go` |
