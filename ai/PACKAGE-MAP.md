...
| `internal/le/evidence` | runs the verify gate the way a release candidate must be judged: over a CLONE of the committed tree, inside a container, on a machine that carries none of the developer's own state |  |
...
| `internal/le/fuzz` | is scripts/le/application/fuzz.py, ported |  |
...
| `internal/le/gotoolchain` | is the Go port of scripts/le/devtools/toolchain.py |  |
...
| `internal/le/leaction` | ports the Python `le` AREA once |  |
...
| `internal/le/letracked` | contains ports of scripts/le/application/tracked.py and scripts/le/devtools/tracked.py |  |
| `internal/le/lintgate` | owns the native full-tree lint stage |  |
| `internal/le/parity` | measures how much of the Python le has become Go, and it is the reason the migration may run two implementations at once |  |
| `internal/le/perfbench` | ports scripts/dev/perf-suggest.py |  |
...
| `internal/le/pyfmt` | renders a value the way Python's repr and str render it |  |
| `internal/le/pylint` | is scripts/le/application/lint.py, ported |  |
...
| `internal/le/rfc` | is scripts/dev/rfc_requirements.py, ported |  |
| `internal/le/rules` | owns the checks for `ai/rules/` |  |
...
| `internal/le/specstatus` | reads the metadata table at the top of every plan/spec-*.md and answers the inventory `make ze-spec-status` prints: one record per spec, carrying its status, its bucket, and whether a skeleton is... |  |
| `internal/le/staticcheckmatrix` | type-checks the working tree once per feature-tag combination Ze can be built in |  |
| `internal/le/ste` | ports scripts/dev/ste_check.py |  |
...
| `internal/le/testunit` | runs the five race-instrumented Go test groups that scripts/le/application/unit.py serves during the duplicate-then-swap migration |  |
...
| `internal/le/verify` | orchestrates the native actions that make up the full pre-commit verification gate |  |
...
| `internal/le/weakened` | detect and record test weakenings against a commit baseline |  |
