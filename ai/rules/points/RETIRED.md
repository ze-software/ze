...
| `cli/cli-grammar-keywords-before-values/deprecate-a-released-grammar-do-not-cut-it` | it told an author to keep an old grammar behind a deprecation warning, which `ai/rules/go-standards.md` "No Backwards Compatibility" bans outright while Ze has no users; what an author does instead is stated by `cli/cli-grammar-keywords-before-values/replace-an-unreleased-grammar-outright` |
