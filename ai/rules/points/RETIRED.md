...
| `commands/write-ad-hoc-scratch-under-your-per-session-dir/what-is-swept-at-session-end-and-what-stays` | nothing under `tmp/session/` is swept at session end or on a timer any more, so the point named a mechanism that no longer exists; what stays put, and the operator's `make ze-sessions-clean`, are stated by `commands/write-ad-hoc-scratch-under-your-per-session-dir/nothing-is-deleted-automatically-and-what-stays-put` |
