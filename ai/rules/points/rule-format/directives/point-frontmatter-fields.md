...
|-------|--------|---------|
| `kind` | `directive`, `table`, `note`, `heading`, `fence` | What the block IS. `heading` and `fence` are structural, so `make ze-rules-gate-map-report` excludes them from the gated and ungated counts |
...
| `rationale` | a repo-relative path, or the line absent | The record of WHY this instruction exists: a `plan/journal/<class>.md` class file, or an `ai/rationale/*.md` file. `make ze-rules-gate-map-report` fails when it names no file on disk |
