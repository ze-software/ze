...
- The lowercase spellings `must`, `shall`, `should` and `may` MUST NOT appear in a directive body, and a block that states no obligation is `kind: note` or `kind: table`. `writePointLanguage` (`internal/le/hookruntime/writeedit.go`) refuses the write and `./le ai rules lint` refuses the finished tree. The accepted keywords and how each maps to a level are in `docs/contributing/rule-authoring.md`.
