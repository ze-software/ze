...
stage:
---
- **`go test`, lint analysis and a `le-test` runner MUST NOT start raw from Bash.** The generic admission grammar is `./le job run label <label> [quiet] command <argv...>`: one job runs while peers queue, and the child exit status is preserved.
