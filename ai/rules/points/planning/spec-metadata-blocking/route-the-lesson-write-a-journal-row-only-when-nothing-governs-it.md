...
**A row is written only when the work produced a lesson, MUST NOT be written as an artifact of closing a spec, and holds exactly five cells, `| Date | Spec | Surface | Symptom | Fix |`, in `plan/journal/<PROBLEM-class>.md`, never a file named for the subsystem.** No gate asks for a lesson and none MUST be added. `parseJournalRow` and `evidenceRows` (`internal/le/journal/journal.go`) reject malformed rows and unreadable Spec keys. The Spec cell attributes the occurrence; `closureStem` (`internal/le/commit/review.go`) checks journal validity but discards its stems, and `closedSpecStem` recognises closure only from a removed spec that is not a relocation between release buckets. Commit B therefore owes the review artifact; a journal row never makes commit A a closure. None of this is permission to prune a defect record (`ai/rules/completion.md`).
