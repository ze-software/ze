...
## No Poll Loops
`ai/rules/no-poll-loops.md`
**When:** waiting for a command, a build, a QEMU boot, a lab, or another session's run to finish — **Severity:** blocking — **Related:** bash-output, no-fork-loops

## Directives
Never poll for work you launched.
...
The summary is 25 to 35 lines and uses this fixed 5-section format.
| Section | Content |
|---------|---------|
...
## Reference
Learned: 363, retired on 2026-08-01 and not carried into `plan/learned/DESIGN-HISTORY.md`, which records subsystem design and not agent workflow.
...
## Directives
Every `ai/rules/*.md` rule MUST open with a title and a machine-readable metadata block, so tooling can parse triggers and severity without guessing.
| Element | Requirement |
|---------|-------------|
