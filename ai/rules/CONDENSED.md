...
| `block-premature-stop.sh` | Stop (**first**, ahead of `session-end-summary.sh`) | **Live and BLOCKING** since 2026-07-31, on owner instruction, with `timeout: 10` and `blocking: true`. Four gates run. The stop-phrase scan blocks with exit 2 (`:75-80`, `:142-151`). It scans a filtered copy (`:30-33`): a phrase inside a fenced block or inline backticks is NAMED, not used, and does not block. The spec-closure gate blocks with exit 2 (`:95-103`) and shells to `scripts/dev/spec-closure-check.py --spec`, measured at 0.11s against the 10s timeout. The spec still-in-progress warning exits 1 (`:107-109`). The delegation nudge exits 1 and never blocks (`:118-120`). The last three fire ONLY when this session CLAIMED a spec. Each one sits behind the `tmp/session/.session-<SID>` marker read at `:87-90`. With no spec claimed, only the stop-phrase scan runs (fixture `delegation-no-spec-no-nudge`). ORDER IS LOAD-BEARING. `session-end-summary.sh:87` calls `_release_session`, which deletes that marker (`.claude/hooks/lib/state-file.sh:71-76`). A `Stop` array that ran the summary first would silently disable three of the four gates. HISTORY: the hook was registered on no event from 2026-06-29 (`41e5fa44f`) to 2026-07-31. It stayed on disk, its fixtures stayed green, and three rules called it live. Only the `Stop` array said otherwise, and no reader checked it. Two fixtures now pin the registration and the order: `delegation-stop-hook-registered` and `delegation-stop-hook-runs-before-marker-release` (`scripts/dev/hook-fixture-check.py`). |
...
| Stop-hook block | `.claude/hooks/block-premature-stop.sh` | This session CLAIMED a spec, the detector exits 3 for it, and no ack exists. The hook then refuses the session an end (exit 2, `:95-103`). Live and first in the `Stop` array since 2026-07-31. It sat on no event from 2026-06-29 (`41e5fa44f`) until then. Escape hatch: record why the spec is genuinely open in `tmp/session/.closure-ack-<stem>`, which `spec-closure-check.py:265-268` reads to return 0 instead of 3. The gate is scoped to a claimed spec, because it sits behind the `tmp/session/.session-<SID>` marker (`:86-90`). A session that claimed no spec is never asked to close one. |
...
- **`.claude/hooks/block-premature-stop.sh` IS registered on `Stop`, and it fires.** Thomas re-registered it on 2026-07-31, after it sat on no event from 2026-06-29 (`41e5fa44f`). It warns with exit 1 when this session CLAIMED a spec and never spawned an agent (`.claude/hooks/block-premature-stop.sh:118-120`). It reads the marker `mark-agent-spawned.sh` writes. The nudge never blocks. It is scoped: it sits behind the claim marker `tmp/session/.session-<SID>` (`:86-90`). A session that claimed no spec gets no nudge, whatever it ran inline (fixture `delegation-no-spec-no-nudge`). Two fixtures pin the registration and its position ahead of `session-end-summary.sh`, which deletes that marker: `delegation-stop-hook-registered` and `delegation-stop-hook-runs-before-marker-release`.
