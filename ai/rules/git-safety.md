...
3. The helper writes executable scripts, uses `git commit -F <message-file>`, and rejects ignored/generated paths. It never writes over an existing script unless `--script` names it, with `--replace` or `--append`. It also **gates on verify-status**: `create` runs `verify-status.sh check <the commit's files>` and refuses unless THOSE paths are FRESH, or unless you pass `--unverified "<reason>"` (owner override, or a failure you tried and could not reproduce, logged in `plan/known-failures/`). This makes "verify before commit" enforced rather than honor-system. The freshness question is scoped to the `--file` list, so another session's edit to a path this commit does not carry leaves the verdict FRESH. Scope is no route around a red: a run that FAILED is STALE for every file list, and a structural gate red charged to this commit still refuses it (`ai/rules/precommit-verify.md`). When the commit carries `.go`, `go.mod`, `go.sum` or `vendor/`, it also **gates on full-verify coverage**: `create` refuses unless a full `make ze-precommit-verify` STARTED after the newest Go file in the commit was last written, whatever that run's exit code was. Override with `--missing-full-verify-ok "<reason>"`. `--unverified` MUST NOT clear it: that flag says a red was explained, and this gate asks whether the run happened at all.
