...
**Current verification claims MUST name canonical source and the command that produced the directly observed result, including scope, failures, and skips.** A volatile session path MUST NOT serve as retained proof. An unchanged check already observed in the current work MUST NOT be rerun merely to renew its timestamp.
