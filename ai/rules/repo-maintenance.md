...
- **Exception:** `scripts/dev/rules_condensed.py` generates TWO artifacts from one parse of the other rule files. Never edit either by hand; run `make ze-rules-condensed`. To change what they contain, edit the rule itself in the canonical format (`ai/rules/rule-format.md`), then regenerate.
