...
| `c_test_weakening` | `testing.md` | `_test.go`, and `.ci`/`.et` under `test/` | Blocks deleting OR weakening tests: removed funcs/cases/assertions, added `t.Skip`, `require`->`assert` downgrade, commented-out asserts, `ignore` build tag, and on a `.ci`/`.et` a dropped `expect=`/`reject=`/`cmd=`/`assert`/`fail(`, an inverted `reject=`, an emptied needle, or an assertion that cannot fail. Escape: `// test-relax: <reason>`, or `# test-relax: <reason>` on a carrier that comments with `#`. The escape opens only on a justification the edit WRITES, keyed on the normalized sentence (`_writes_new_relax_reason`): one already in the file buys nothing, and neither does reindenting or rewording it. The stock is capped by `make ze-relax-census`. BLOCKING. |
