...
| `scripts/dev/hook-fixture-check.py` | Behaviour the golden table cannot isolate: `c_format_alloc` (called directly), `validate-spec.sh` (ASCII/Unicode/malformed specs), the `commit_helper.py` commit-time gates (git-initialized fixtures), `delegation` (`mark-agent-spawned.sh`, the Stop-hook nudge, and the `subagent-context.sh` spec injection, over fixture projects), and `delegation-reminder` (the UserPromptSubmit stdout contract). Sections selectable with `--only`. |
...
| `block-premature-stop.sh` | **none -- NOT REGISTERED** | **This hook never fires.** Thomas removed it from the `Stop` event on 2026-06-29, in commit `41e5fa44f`. The script stays on disk, and `hook-fixture-check.py --only delegation` still passes against it. Every signal a reader checks says "live". The one signal that decides says otherwise: the `Stop` array in `.claude/settings.json`. Three gates are inert as a result. The stop-phrase scan (exit 2), the spec-closure gate (`scripts/dev/spec-closure-check.py`, exit 2, described as live by `ai/rules/planning.md` "Closure Enforcement"), and the delegation warning (exit 1, `ai/rules/spec-delegation.md`). Owner decision 2026-07-30: leave it unregistered and correct the documents. |
