...
- **A finding outside the round's scope is fixed in this round when the goal this work exists to achieve depends on it. It is homed otherwise.** That is `ai/rules/completion.md`'s question unchanged. The test is DEPENDENCY, never causation. A defect this change did not introduce is in scope the moment the work depends on that path, which is what "pre-existing" never excuses.
...
- **A summary is useful only if it helps future software development or explains past design (owner directive, 2026-08-03). If it does neither, it is not useful and does not get written or kept.** The two tests are the sections themselves: `Decisions` explains past design when it names what was chosen AND what was rejected; `Gotchas` and `Consequences` help future work when they state something the code alone cannot tell a reader. A summary that satisfies neither is deleted, not filed.
- **Usefulness is a property of the content, not of whether anything cites it.** Measured 2026-08-03 over 903 summaries: 63 fail the test outright, and 445 pass it while nothing in the tree points at them. An uncited summary that carries a real constraint is a ROUTING failure, so route it; only the 40 that fail both are waste.
