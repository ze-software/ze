...
4. **Completion (YANG-driven).** `updateCompletions` (`model.go:641`) picks the source by mode:
   config completion `Completer.Complete` (`completer.go:118`) walks the editor's `*config.Tree`;
   leaf values come from `valueCompletions` (`completer.go:769`) — `ze:validate` `CompleteFn`
   first (`validateCompletions`, `completer.go:835`), then YANG `enum`/`bool`/`union`, else a
...
6. **Daemon dispatch (online).** `Dispatcher.Dispatch` (`internal/component/plugin/server/command.go:538`)
   ...
   (`command.go:53,72`): `name := wireToPath[reg.WireMethod]`; read-only class from
   `IsReadOnlyPath` (`command.go:106`).
...
- **Dispatch key = YANG path, not wire method.** `LoadBuiltins` registers under
  `wireToPath[reg.WireMethod]` (`command.go:55`); a handler with no YANG entry is skipped. Moving
...
- **Completion source of truth is YANG.** Config editor completes from the live `*config.Tree`;
  ...
  `CompleteFn` yields actionable values and takes priority over static `enum` (`completer.go:778`);
...
- **`tokenizeCommand` ≠ shell.** Double-quotes group tokens; there are no backslash escapes
  (`model_commands.go:271`). The daemon dispatcher has its own `tokenize` (`command.go:539`).
