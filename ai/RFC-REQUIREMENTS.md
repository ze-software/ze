...
5093 requirements across 189 summaries. 3333 are MUST-level; 3238 of those are enrolled and gated by `./le rfc check`.
...
201 MUST-level requirement(s) still owe work across 182 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
...
| `rfc3748` | 35 | 21 | 0 | 14 | 0 | 0 | 0 | **enrolled** |
...
50 of 1770 auditable requirement(s) carry a `ze-rfc-audit` verdict (2.82%), across 1 of 181 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.
...
The remaining 1720 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `./le rfc check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.

Two partitions over two populations, because one denominator cannot carry both questions. **Requirements:** `Auditable` (1770) = `Audited` (50) + `Unaudited` (1720). **Records:** all 52 recorded verdict(s) = `Proven` (48) + `Not proven` (4), and the worklist below names every one of those 4. A verdict can sit on a requirement that is not auditable -- an annotated `{gap}` or `{not-applicable}` line carries no tagged test -- so the record totals are the wider of the two and are never a subset of `Audited`.
...
Signed off by register: rfc2119 25, prose 26, manual-walk 3. Unsigned (grandfathered) backlog: 127 of 181 enrolled. Every register counts toward the drain quota; each is published apart so a count can never be read as stronger evidence than it is.
...
| `rfc3748` | -- | -- | -- | -- | -- | 35 | UNSIGNED (grandfathered) |
