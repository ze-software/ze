...
| `rfc7999` | backlog | **DEBT** | BLACKHOLE Community, IETF category Informational. The blocker is gone: rfc/full/rfc7999.txt is present, and the summary was re-authored against it on 2026-08-08. It now captures 16 requirements, of which 4 are MUST-level and gated: one Tx obligation in section 3.1, two Rx conditions from the single MUST sentence of section 3.3, and one operator obligation on origin validation in section 3.3. The other 12 are 6 SHOULD, 1 SHOULD NOT, 1 RECOMMENDED and 4 MAY. Two of the three things owed before enrolment now exist: docs/features/rfc-status.md carries a row, and three of the four MUSTs carry a tagged positive-and-negative pair (RFC7999-3.3-1, 3.3-2 and 3.3-4). Two remain. There is no extraction sign-off at rfc/extraction/rfc7999.json. RFC7999-3.1-2 is a sender-side obligation with no producer, because Ze originates no BLACKHOLE-tagged announcement, so it needs an owner-authorised annotation rather than a test. |
