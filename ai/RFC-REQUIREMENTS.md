...
5450 requirements across 194 summaries. 3586 are MUST-level; 3309 of those are enrolled and gated by `./le rfc check`.

One RFC's requirement rows are one file, named for its summary's stem: `rfc/requirements/draft-abraitis-bgp-version-capability.md` holds DRAFT-ABRAITIS-BGP-VERSION-CAPABILITY. This page is the index over those 194 files.
...
298 MUST-level requirement(s) still owe work across 187 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
...
|---|---|---|---|---|---|---|---|---|
| `draft-abraitis-idr-addpath-paths-limit` | 4 | 4 | 0 | 0 | 0 | 0 | 0 | **enrolled** |
| `draft-ietf-bess-mup-safi` | 40 | 1 | 0 | 39 | 0 | 0 | 0 | **enrolled** |
| `draft-ietf-idr-bgp-bfd-strict-mode` | 4 | 4 | 0 | 0 | 0 | 0 | 0 | **enrolled** |
...
| `rfc3101` | 17 | 17 | 0 | 0 | 0 | 0 | 0 | **enrolled** |
| `rfc3209` | 13 | 5 | 0 | 8 | 0 | 0 | 0 | **enrolled** |
| `rfc3579` | 31 | 17 | 0 | 14 | 0 | 0 | 0 | **enrolled** |
...
| `rfc4271` | 101 | 76 | 0 | 25 | 0 | 0 | 0 | **enrolled** |
| `rfc4301` | 20 | 6 | 0 | 14 | 0 | 0 | 0 | **enrolled** |
...
| `rfc5072` | 16 | 6 | 0 | 10 | 0 | 0 | 0 | **enrolled** |
| `rfc5176` | 22 | 21 | 0 | 1 | 0 | 0 | 0 | **enrolled** |
| `rfc5187` | 4 | 4 | 0 | 0 | 0 | 0 | 0 | **enrolled** |
| `rfc5216` | 21 | 14 | 0 | 7 | 0 | 0 | 0 | **enrolled** |
...
| `rfc5305` | 8 | 5 | 0 | 3 | 0 | 0 | 0 | **enrolled** |
...
| `rfc6793` | 30 | 28 | 0 | 2 | 0 | 0 | 0 | **enrolled** |
...
| `rfc7311` | 5 | 4 | 0 | 1 | 0 | 0 | 0 | **enrolled** |
...
| `rfc5282` | 19 | 15 | 0 | 0 | 4 | 4 | 0 | **enrolled** |
...
| `rfc5798` | 55 | 33 | 7 | 0 | 15 | 22 | 0 | **enrolled**, superseded by RFC9568 |
...
| `rfc9190` | 52 | 19 | 7 | 0 | 26 | 33 | 0 | backlog |
| `rfc8362` | 38 | 0 | 0 | 0 | 38 | 38 | 0 | backlog |
| `rfc6514` | 133 | 0 | 0 | 0 | 133 | 133 | 0 | backlog |

## Audit coverage

54 of 1887 auditable requirement(s) carry a `ze-rfc-audit` verdict (2.86%), across 2 of 183 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.

**Proven** (54) is the count that means what the badge implies: a verdict of `enforced` -- the tests would fail if the code stopped complying -- that is still fresh. It is NOT the **Both** column of the rollup above: that one answers which polarities exist, and a requirement can have both and still be judged `weak`. Every one of the 5 verdict(s) that is audited but not proven is named below with its verdict, so no requirement can read as proven and weak at once.

The remaining 1833 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `./le rfc check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.

Two partitions over two populations, because one denominator cannot carry both questions. **Requirements:** `Auditable` (1887) = `Audited` (54) + `Unaudited` (1833). **Records:** all 59 recorded verdict(s) = `Proven` (54) + `Not proven` (5), and the worklist below names every one of those 5. A verdict can sit on a requirement that is not auditable -- an annotated `{gap}` or `{not-applicable}` line carries no tagged test -- so the record totals are the wider of the two and are never a subset of `Audited`.

| RFC | Auditable | Audited | Proven | Not proven | Unaudited |
|---|---|---|---|---|---|
| `rfc7606` | 50 | 50 | 48 | 4 | 0 |
| `draft-abraitis-idr-addpath-paths-limit` | 4 | 4 | 6 | 1 | 0 |
| `draft-ietf-bess-mup-safi` | 3 | 0 | 0 | 0 | 3 |
| `draft-ietf-idr-bgp-bfd-strict-mode` | 4 | 0 | 0 | 0 | 4 |
...
|---|---|---|
| `DRAFT-ABRAITIS-IDR-ADDPATH-PATHS-LIMIT-3-7` | `weak` | tagged and green, but cannot fail on non-compliance |
...
An escape (`no-break`) is a recorded claim that no break exists, and the gate checks a precondition for each reason it accepts: `declaration-only` 2, `foreign-producer` 0, `generated-producer` 0. It is counted apart from a proof because a claim that nothing can break is debt, not evidence.

17 record(s) can be deleted: the tag each one proved is gone. A record dies with its tag, so an orphan is removed rather than re-recorded.

23 `RFC requirement:` comment(s) sit in production Go that no carrier claims. Nothing resolves them and nothing runs them, and 21 of them carry no polarity, so no scanner would accept them even if a carrier did claim the file. `./le rfc check` names each one.
...
Signed off by register: rfc2119 29, prose 26, manual-walk 3. Unsigned (grandfathered) backlog: 125 of 183 enrolled. Every register counts toward the drain quota; each is published apart so a count can never be read as stronger evidence than it is.
...
| `draft-ietf-idr-bgp-bfd-strict-mode` | rfc2119 | 4 | 4 | 0 | 0.00 | 4 | 2026-09-08 (ze-implement (spec-bgp-bfd-strict)) |
...
| `rfc2865` | rfc2119 | 74 | 23 | 51 | 0.69 | 30 | 2026-08-30 (ze-implement walk agent, spec-rfcgate-6-supported-extraction-signoff phase 2 (Tier 1, rfc2865)) |
...
| `rfc3579` | rfc2119 | 56 | 30 | 26 | 0.46 | 31 | 2026-09-03 (ze-work agent, plan/spec-radius-admin-eap.md phase 2, rfc3579) |
...
| `rfc8671` | rfc2119 | 10 | 8 | 2 | 0.20 | 10 | 2026-09-05 (ze-fix agent, false public conformance claim on rfc8671, section 5.2 re-walked 2026-09-05) |
...
| `rfc2516` | -- | -- | -- | -- | -- | 22 | UNSIGNED (grandfathered) |
| `rfc2661` | -- | -- | -- | -- | -- | 20 | UNSIGNED (grandfathered) |
...
| `rfc3101` | -- | -- | -- | -- | -- | 17 | UNSIGNED (grandfathered) |
...
| `rfc4271` | -- | -- | -- | -- | -- | 101 | UNSIGNED (grandfathered) |
| `rfc4301` | -- | -- | -- | -- | -- | 20 | UNSIGNED (grandfathered) |
...
| `rfc5072` | -- | -- | -- | -- | -- | 16 | UNSIGNED (grandfathered) |
...
| `rfc7311` | -- | -- | -- | -- | -- | 5 | UNSIGNED (grandfathered) |
...
| `rfc5065` | 20 | 3 | **RE-AUTHOR**: source is normative, summary captured nothing |
