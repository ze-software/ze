...
4818 requirements across 179 summaries. 3132 are MUST-level; 3037 of those are enrolled and gated by `./le rfc check`.

One RFC's requirement rows are one file, named for its summary's stem: `rfc/requirements/draft-abraitis-idr-addpath-paths-limit.md` holds DRAFT-ABRAITIS-IDR-ADDPATH-PATHS-LIMIT. This page is the index over those 179 files.
...
68 MUST-level requirement(s) still owe work across 174 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
...
| `rfc2865` | 17 | 13 | 0 | 4 | 0 | 0 | 0 | **enrolled** |
...
| `rfc3948` | 11 | 5 | 0 | 6 | 0 | 0 | 0 | **enrolled** |
...
| `rfc5176` | 8 | 7 | 0 | 1 | 0 | 0 | 0 | **enrolled** |
...
| `rfc8671` | 8 | 7 | 0 | 1 | 0 | 0 | 0 | **enrolled** |
...
50 of 1699 auditable requirement(s) carry a `ze-rfc-audit` verdict (2.94%), across 1 of 172 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.
...
The remaining 1649 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `./le rfc check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.

Two partitions over two populations, because one denominator cannot carry both questions. **Requirements:** `Auditable` (1699) = `Audited` (50) + `Unaudited` (1649). **Records:** all 52 recorded verdict(s) = `Proven` (48) + `Not proven` (4), and the worklist below names every one of those 4. A verdict can sit on a requirement that is not auditable -- an annotated `{gap}` or `{not-applicable}` line carries no tagged test -- so the record totals are the wider of the two and are never a subset of `Audited`.
...
Signed off by register: rfc2119 20, prose 21, manual-walk 2. Unsigned (grandfathered) backlog: 129 of 172 enrolled. Every register counts toward the drain quota; each is published apart so a count can never be read as stronger evidence than it is.
...
| `rfc2865` | rfc2119 | 74 | 22 | 52 | 0.70 | 17 | 2026-08-30 (ze-implement walk agent, spec-rfcgate-6-supported-extraction-signoff phase 2 (Tier 1, rfc2865)) |
...
| `rfc3948` | rfc2119 | 15 | 7 | 8 | 0.53 | 11 | 2026-08-31 (ze-work agent, spec-rfcgate-6 phase 2, rfc3948) |
...
| `rfc8671` | rfc2119 | 10 | 6 | 4 | 0.40 | 8 | 2026-08-31 (ze-work agent, spec-rfcgate-6 phase 4, rfc8671) |
...
| `rfc5176` | -- | -- | -- | -- | -- | 8 | UNSIGNED (grandfathered) |
