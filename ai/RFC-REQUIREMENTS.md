...
5111 requirements across 189 summaries. 3351 are MUST-level; 3256 of those are enrolled and gated by `./le rfc check`.

One RFC's requirement rows are one file, named for its summary's stem: `rfc/requirements/draft-abraitis-bgp-version-capability.md` holds DRAFT-ABRAITIS-BGP-VERSION-CAPABILITY. This page is the index over those 189 files.
...
216 MUST-level requirement(s) still owe work across 182 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
...
| `rfc3748` | 53 | 24 | 0 | 14 | 15 | 15 | 0 | **enrolled** |
...
50 of 1773 auditable requirement(s) carry a `ze-rfc-audit` verdict (2.82%), across 1 of 181 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.
...
The remaining 1723 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `./le rfc check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.

Two partitions over two populations, because one denominator cannot carry both questions. **Requirements:** `Auditable` (1773) = `Audited` (50) + `Unaudited` (1723). **Records:** all 52 recorded verdict(s) = `Proven` (48) + `Not proven` (4), and the worklist below names every one of those 4. A verdict can sit on a requirement that is not auditable -- an annotated `{gap}` or `{not-applicable}` line carries no tagged test -- so the record totals are the wider of the two and are never a subset of `Audited`.
...
| `rfc3748` | rfc2119 | 103 | 44 | 59 | 0.57 | 53 | 2026-08-31 (rfc3748walk (extraction sign-off agent, spec rfcgate-6-supported-extraction-signoff)) |
...
Of those exclusions, 25 carry `relocated-to-spec` (`rfc3748` 12, `rfc7296` 12, `rfc7947` 1): the obligation is owed, by the spec the site names, under the requirement id reserved for it there. `./le rfc check` refuses the sign-off unless that spec exists and still names that id, so a relocation cannot outlive the document it points at. It is counted in `Excluded` and in the ratio above because this walk did decline to map the sentence HERE, and it is counted apart because a homed obligation and a dismissed sentence are not the same fact.
