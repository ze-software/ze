...
4852 requirements across 180 summaries. 3165 are MUST-level; 3065 of those are enrolled and gated by `./le rfc check`.

One RFC's requirement rows are one file, named for its summary's stem: `rfc/requirements/draft-abraitis-idr-addpath-paths-limit.md` holds DRAFT-ABRAITIS-IDR-ADDPATH-PATHS-LIMIT. This page is the index over those 180 files.
...
74 MUST-level requirement(s) still owe work across 175 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.

**Enrollable now** (1): every MUST-level requirement is already covered or annotated, so adding these to `rfc/enrolled.txt` would gate them without any new work: `rfc7607`
...
| `rfc5176` | 9 | 8 | 0 | 1 | 0 | 0 | 0 | **enrolled** |
...
| `rfc2865` | 29 | 18 | 0 | 6 | 5 | 5 | 0 | **enrolled** |
| `rfc7607` | 5 | 5 | 0 | 0 | 0 | 0 | 0 | enrollable |
...
50 of 1721 auditable requirement(s) carry a `ze-rfc-audit` verdict (2.91%), across 1 of 172 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.
...
The remaining 1671 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `./le rfc check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.

Two partitions over two populations, because one denominator cannot carry both questions. **Requirements:** `Auditable` (1721) = `Audited` (50) + `Unaudited` (1671). **Records:** all 52 recorded verdict(s) = `Proven` (48) + `Not proven` (4), and the worklist below names every one of those 4. A verdict can sit on a requirement that is not auditable -- an annotated `{gap}` or `{not-applicable}` line carries no tagged test -- so the record totals are the wider of the two and are never a subset of `Audited`.
...
Signed off by register: rfc2119 19, prose 21, manual-walk 2. Unsigned (grandfathered) backlog: 130 of 172 enrolled. Every register counts toward the drain quota; each is published apart so a count can never be read as stronger evidence than it is.
...
| `rfc5176` | -- | -- | -- | -- | -- | 9 | UNSIGNED (grandfathered) |
