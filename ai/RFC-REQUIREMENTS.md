...
52 MUST-level requirement(s) still owe work across 172 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
...
| `rfc7999` | 4 | 3 | 0 | 0 | 1 | 1 | 0 | backlog |
| `rfc1035` | 27 | 25 | 0 | 0 | 2 | 2 | 0 | backlog |
| `rfc9190` | 51 | 2 | 0 | 0 | 49 | 49 | 0 | backlog |

## Audit coverage

50 of 1598 auditable requirement(s) carry a `/ze-rfc-audit` verdict (3.13%), across 1 of 170 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.
...
The remaining 1548 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `make ze-rfc-check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.

Two partitions over two populations, because one denominator cannot carry both questions. **Requirements:** `Auditable` (1598) = `Audited` (50) + `Unaudited` (1548). **Records:** all 52 recorded verdict(s) = `Proven` (50) + `Not proven` (2), and the worklist below names every one of those 2. A verdict can sit on a requirement that is not auditable -- an annotated `{gap}` or `{not-applicable}` line carries no tagged test -- so the record totals are the wider of the two and are never a subset of `Audited`.
...
| `rfc7999` | backlog | **DEBT** | BLACKHOLE Community, IETF category Informational. The blocker is gone: rfc/full/rfc7999.txt is present, and the summary was re-authored against it on 2026-08-08. It now captures 16 requirements, of which 4 are MUST-level and gated: one Tx obligation in section 3.1, two Rx conditions from the single MUST sentence of section 3.3, and one operator obligation on origin validation in section 3.3. The other 12 are 6 SHOULD, 1 SHOULD NOT, 1 RECOMMENDED and 4 MAY. Three of the four things owed before enrolment now exist: docs/features/rfc-status.md carries a row, three of the four MUSTs carry a tagged positive-and-negative pair (RFC7999-3.3-1, 3.3-2 and 3.3-4), and rfc/extraction/rfc7999.json carries a valid sign-off, signed 2026-08-13 at register prose, 1 of 4 sites excluded. RFC7999-3.1-2 is what remains, and it is the whole of what blocks enrolment: measured against scripts/dev/rfc_requirements.py evaluate() with rfc7999 added to the enrolled set, it is the single violation. Its earlier reason is withdrawn as false. Ze DOES originate a BLACKHOLE-tagged announcement: handleAnnounceBlackhole in internal/component/bgp/plugins/cmd/announce/announce.go attaches attribute.CommunityBlackhole and sends it to the peers the command selector names, consulting no record of any agreement. The obligation binds the two networks rather than the speaker, so whether it is met by a per-peer Tx agreement leaf and a tagged pair, or by an owner-authorised annotation, is the owner's ruling and nobody else's. |
