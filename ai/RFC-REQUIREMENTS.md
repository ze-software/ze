...
5444 requirements across 193 summaries. 3580 are MUST-level; 3303 of those are enrolled and gated by `./le rfc check`.
...
298 MUST-level requirement(s) still owe work across 186 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
