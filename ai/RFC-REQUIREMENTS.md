...
4959 requirements across 187 summaries. 3235 are MUST-level; 3131 of those are enrolled and gated by `./le rfc check`.

One RFC's requirement rows are one file, named for its summary's stem: `rfc/requirements/draft-abraitis-bgp-version-capability.md` holds DRAFT-ABRAITIS-BGP-VERSION-CAPABILITY. This page is the index over those 187 files.
...
123 MUST-level requirement(s) still owe work across 180 summaries. **Outstanding** = has only one polarity, or has no test and no annotation; those are the tests that do not exist yet.
...
**Superseded** (7 summaries): the document has been obsoleted, so every requirement it states carries a `{superseded}` marker naming where that obligation now lives. The marker is a fact about the DOCUMENT: these requirements stay gated, counted and ratcheted. `rfc1334` -> RFC1994, `rfc3768` -> RFC9568, `rfc5549` -> RFC8950, `rfc5575` -> RFC8955, `rfc6810` -> RFC8210, `rfc7627` -> RFC9846, `rfc7752` -> RFC9552. 27 requirement(s) point at a section of a successor whose summary declares no row (`unextracted`), and 27 point at a document this repository does not hold (`unresolved`). Both are debt, not settled pointers.
...
| `rfc2865` | 29 | 23 | 0 | 6 | 0 | 0 | 0 | **enrolled** |
...
| `rfc3748` | 31 | 17 | 0 | 14 | 0 | 0 | 0 | **enrolled** |
...
| `rfc7947` | 2 | 1 | 0 | 1 | 0 | 0 | 0 | **enrolled** |
...
| `rfc9069` | 10 | 2 | 0 | 8 | 0 | 0 | 0 | **enrolled** |
...
| `rfc5082` | 4 | 0 | 0 | 0 | 4 | 4 | 0 | **enrolled** |
| `draft-abraitis-bgp-version-capability` | 11 | 0 | 0 | 0 | 11 | 11 | 0 | **enrolled** |
| `draft-ietf-idr-linklocal-capability` | 14 | 0 | 0 | 0 | 14 | 14 | 0 | **enrolled** |
| `rfc5282` | 19 | 0 | 0 | 0 | 19 | 19 | 0 | **enrolled** |
| `rfc1035` | 27 | 25 | 0 | 0 | 2 | 2 | 0 | backlog |
| `rfc2385` | 9 | 0 | 0 | 3 | 6 | 6 | 0 | backlog |
| `rfc7627` | 17 | 0 | 0 | 0 | 17 | 17 | 0 | backlog, superseded by RFC9846 |
| `rfc9190` | 51 | 2 | 1 | 0 | 48 | 49 | 0 | backlog |

## Audit coverage

50 of 1747 auditable requirement(s) carry a `ze-rfc-audit` verdict (2.86%), across 1 of 178 enrolled RFC(s). **Auditable** = gated, enrolled, and polarity coverage complete: a pair of tests, or one test over a `{single-polarity}` line saying why the other cannot exist. Until then there is nothing for an auditor to judge.
...
Signed off by register: rfc2119 21, prose 22, manual-walk 3. Unsigned (grandfathered) backlog: 132 of 178 enrolled. Every register counts toward the drain quota; each is published apart so a count can never be read as stronger evidence than it is.
...
| `draft-abraitis-bgp-version-capability` | -- | -- | -- | -- | -- | 11 | UNSIGNED (grandfathered) |
| `draft-ietf-bess-mup-safi` | -- | -- | -- | -- | -- | 40 | UNSIGNED (grandfathered) |
| `draft-ietf-idr-linklocal-capability` | -- | -- | -- | -- | -- | 14 | UNSIGNED (grandfathered) |
...
| `rfc3748` | -- | -- | -- | -- | -- | 31 | UNSIGNED (grandfathered) |
...
| `rfc5176` | -- | -- | -- | -- | -- | 22 | UNSIGNED (grandfathered) |
...
| `rfc7947` | -- | -- | -- | -- | -- | 2 | UNSIGNED (grandfathered) |
...
| `rfc9069` | -- | -- | -- | -- | -- | 10 | UNSIGNED (grandfathered) |
...
Of those exclusions, 12 carry `relocated-to-spec` (`rfc7296` 12): the obligation is owed, by the spec the site names, under the requirement id reserved for it there. `./le rfc check` refuses the sign-off unless that spec exists and still names that id, so a relocation cannot outlive the document it points at. It is counted in `Excluded` and in the ratio above because this walk did decline to map the sentence HERE, and it is counted apart because a homed obligation and a dismissed sentence are not the same fact.
