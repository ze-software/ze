...
GENERATED by `make ze-rfc-index-update` -- do not edit. Requirement text is authored in `rfc/short/*.md`; the test links are derived from `RFC requirement:` tags in the tests themselves (`ai/rules/evidence.md`).

4744 requirements across 178 summaries. 3067 are MUST-level; 2972 of those are enrolled and gated by `make ze-rfc-check`.

One RFC's requirement rows are one file, named for its summary's stem: `rfc/requirements/draft-abraitis-idr-addpath-paths-limit.md` holds DRAFT-ABRAITIS-IDR-ADDPATH-PATHS-LIMIT. This page is the index over those 178 files.
...
|---|---|---|---|
| `unit/verify` | `*_test.go` | `make ze-unit-test` | ze-precommit-verify (unit stage) |
| `functional/verify` | `*.ci` | `make ze-functional-test` | ze-precommit-verify (functional stage) -- suites: `encode`, `plugin`, `parse`, `decode`, `reload`, `ui`, `editor`, `managed`, `l2tp`, `firewall`, `policy`, `ipsec`, `ldp`, `rsvpte`, `isis`, `ospf`, `ospfv3`, `web`, `install`, `appliance`, `l2tp-wire`, `isis-wire`, `ospf-wire`, `runner` |
| `editor/verify` | `*.et` | `make ze-functional-editor-test` | ze-precommit-verify (functional stage) -- suites: `editor` |
| `functional/verify` | `*.ci` | `make ze-functional-exabgp-test` | ze-precommit-verify (exabgp stage) |
| `interop/nightly` | `*/check.py` | `make ze-interop-test` | .github/workflows/evidence-nightly.yml (advisory) |
| `interop/nightly` | `*/check.py` | `make ze-interop-ipsec-test` | .github/workflows/evidence-nightly.yml (advisory) |

A tag in a carrier nothing executes is REFUSED by `make ze-rfc-check`, not listed here with a caveat. These have no automated caller: `any *.ci the table above does not cover`, `any *.et the table above does not cover`, `any */check.py the table above does not cover`, `test/interop-l2tp/`, `test/interop-pppoe/`. A tag in one of them would be an absence of evidence wearing evidence's clothes, so the scanner denies it and names the fix (`ai/rules/evidence.md`).
...
| `rfc3768` | 39 | 30 | 0 | 9 | 0 | 0 | 0 | **enrolled**, superseded by RFC9568 |
...
| `rfc5575` | 12 | 0 | 0 | 12 | 0 | 0 | 0 | **enrolled**, superseded by RFC8955 |
...
| `rfc8955` | 22 | 7 | 0 | 15 | 0 | 0 | 0 | **enrolled** |
...
The remaining 1561 carry no verdict at all. That is not a violation: the audit is sampled and the gate is total, so a missing verdict never fails `make ze-rfc-check`. It is published because an unmeasured semantic half is indistinguishable from a clean one.
...
Signed off by register: rfc2119 2, prose 3, manual-walk 1. Unsigned (grandfathered) backlog: 165 of 171 enrolled. Every register counts toward the drain quota; each is published apart so a count can never be read as stronger evidence than it is.
