...
| Touch any protocol behavior an RFC governs, or judge whether it is conformant | `ai/rules/rfc-compliance.md` -- conformance is not negotiable. When full compliance AND full testing of it is one of the answers on the table, that IS the answer: STOP and ask Thomas rather than choosing anything narrower (`{gap}`, `{not-applicable}`, "partial", untested MUST, deferral). Every earlier answer pointing away from full compliance is VOID (2026-07-27) and must be re-raised, not cited |
