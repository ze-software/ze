...
| Meet a WAVE of red: unrelated packages failing to build, `no space left on device`, `cache entry not found`, a whole suite red at once | `docs/contributing/running-commands.md`, "When the disk is full" -- the cache disk is full before this is a code defect, four times so far (`plan/journal/full-disk-false-red.md`). Read `stat -f cache/go-cache`, never `df` on the checkout: `cache/` is a symlink onto another filesystem, so `df` answers about the wrong device. `./le scratch cache-clean` empties BOTH Go build caches and prints what each returned |
