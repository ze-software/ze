...
13. **ze-go-style.md**: Read `docs/contributing/ze-go-style.md`, then check the six judgement calls no linter makes. (a) Can a peer reach any `panic()` in the diff? Trace the input back to the socket. A `panic()` reachable from a socket is critical severity, because a malformed message is an operating error and returns an error. (b) What bounds each new loop, queue, retry, and cache? An endless loop by design says so in a comment. (c) Does each new name say what the value IS, never its Go type, with the qualifier last by descending significance? (d) Does each new lifecycle or paired call state its obligation with MUST on BOTH sides? (e) Does the diff duplicate or alias a value that already exists? (f) Is any return type wider than it needs to be: nothing over `bool`, `bool` over a value, a value over `(value, ok)`, `(value, ok)` over `(value, error)`?
14. **Altitude check**: Is each change at the right depth? A special case layered on shared infrastructure is a sign the underlying mechanism should be generalized instead. Prefer deepening the shared abstraction over adding per-caller workarounds. Flag bandaid fixes with the deeper alternative named.
...
From the diff, determine what feature is being added (new NLRI family, new plugin, new protocol support, new command, etc.). Read the spec if one exists (run `./le spec current`).
