...
0. **Automated pre-checks (run both, fix or report findings before proceeding):**
    ...
    - `make ze-relax-census` — the diff-scoped audit above sees only the tokens THIS change adds. The census counts the whole stock against `test/relax-ceiling.txt`. If the change raises the ceiling, that is a review finding in its own right: the raise must be justified by the token it admits, in the same commit (`TEST-RELAX-AUDIT.md`).
