...
1. **Selected spec:** Run `./le spec current`. If a spec is selected, read it and extract:
...
2. **Session state:** Run `./le spec state latest spec <spec-stem>` and read the reported per-spec state file. Extract the most recent session entry.
