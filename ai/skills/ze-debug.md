...
- **Diagnosis before fix (BLOCKING):** No edit until the five-part Diagnosis is written (`ai/rules/diagnosis-before-fix.md`). The success criterion is "root cause fixed at the owning layer", not "test green". Trigger words ("let me just rename / skip / special-case / adjust the test") mean stop and diagnose.
