...
| `make ze-rfc-check` | `scripts/dev/rfc_requirements.py` | Gate: every MUST-level requirement of an enrolled RFC (`rfc/enrolled.txt`) is bound to a positive AND a negative test, or carries a reasoned annotation. Also ratchets against HEAD: enrolment cannot shrink, a requirement cannot lose a polarity it had, a NEW summary with gated MUSTs must be enrolled, and an extraction sign-off cannot disappear (`ai/rules/rfc-compliance.md`, "the five ratchets"). Enrolling a stem not enrolled at HEAD requires `rfc/extraction/<stem>.json`. |
| `make ze-rfc-index` | `scripts/dev/rfc_requirements.py` | Regenerates `ai/RFC-REQUIREMENTS.md`: requirement -> enforcing tests, plus the Coverage-by-RFC backlog naming what is still owed and the Extraction sign-off table naming what is still unbounded. |
