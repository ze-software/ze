...
before grepping. All three are generated from the tree (`make ze-discovery-index-update`)
...
| Which problems recur? | `plan/journal/` (one file per class; `make ze-journal-report` prints classes with 2+ rows) |
...
| Prove an RFC MUST is enforced (tag a test, coverage gate) | `ai/skills/ze-rfc.md` | Tag the test `RFC requirement: <id> <polarity>` (both polarities); `make ze-rfc-check` gates coverage; `make ze-rfc-index-update` writes `rfc/requirements/<stem>.md` for that RFC's rows and `ai/RFC-REQUIREMENTS.md` for the index over all of them; audit with `/ze-rfc-audit` |
...
| Rename the module path (host or owner change) | `scripts/dev/rename_module_path.py --help` | Dry-run plan, then `--apply`; regenerate protobuf with `make ze-proto-generate` |
...
| Verification/self-check gate | `ai/rules/repo-maintenance.md` | `ai/rules/repo-maintenance.md`, `docs/contributing/documentation-testing.md` | `mk/check-docs.mk` for doc/inventory targets |
...
|---|---|---|
| Reactor / session | `docs/architecture/core-design.md` sections 1-5 | `ai/rules/goroutine-lifecycle.md`, `make ze-unit-reactor-test-race` required |
...
7. Run make ze-doc-verify after editing docs/
```
...
| `session-scratch.sh` | `scripts/dev/` | Print (and create) this session's private scratch dir, the `scratch/` subdirectory of `tmp/session/<YYYY-MM-DD>-<session-id>/`. Use for ad-hoc command output instead of fixed names at the `tmp/` root, which collide between concurrent sessions. Nothing under `tmp/session/` is ever removed automatically; `make ze-session-clean BEFORE=<YYYY-MM-DD>` and `make ze-session-reap` are the operator's cleanup. See `ai/rules/commands.md`. |
| `session-reap.py` | `scripts/dev/` | Remove the session directories under `tmp/session/` whose Claude session has EXITED, and the flat markers keyed to them. It reads no date, because a directory dated last week can belong to a session still open in a terminal; it asks whether the owner is still running. Four independent signals KEEP a directory (this session's own id; a `.sid-by-pid-<pid>-<start>` pin whose pid and start time both still match; a sid in a running process's argv; a transcript under `$CLAUDE_CONFIG_DIR/projects/` written at or after the oldest running CLI started), and it removes only when all four are silent, so it errs toward keeping and fails closed when it cannot tell. Gate: none -- `make clean` runs it, `make ze-session-reap DRY=1` names what would go. Tested by `scripts/dev/session_reap_test.py`. |
| `make ze-session-binary-path` | `mk/helper-session.mk` | Print THIS session's `ze` binary path. Under an AI session every canonical binary is built under its BARE name into this session's own directory, `tmp/session/<YYYY-MM-DD>-<session-id>/bin/` (`ZE_BIN_DIR`), so a sibling session's `make ze-build` cannot overwrite the binary you are testing against; off-session the path is the plain `bin/ze` it always was. **Never hardcode `bin/ze`** in a command, script, or doc -- use `$(make ze-session-binary-path)`. Test binaries go to a private `bin/` subdir of a throwaway directory under `$(ZE_SCRATCH_DIR)`, because `.ci` tests exec them by bare name. See `ai/rules/commands.md` "Your Binaries Live In This Session's Directory". |
| `journal.py` | `scripts/dev/` | Read `plan/journal/*.md` at git HEAD and report problem classes with 2+ rows. Each class file is one problem pattern, each row one occurrence. `make ze-journal-report` prints the class, its row count, and the date span. Folded into `make ze-doc-verify`. |
| `digest_check.py` | `scripts/dev/` | Validate the `file:line` anchors in `ai/digests/*.md`: each resolves to a real file (subsystem-relative via the digest's `<!-- digest-base: -->` header) and an in-range line. Keeps the hand-maintained flow digests honest as code moves. Gate: `make ze-digest-check`, folded into `make ze-doc-verify`. |
| `check_doc_links.py` | `scripts/dev/` | Five checks over the path references in the tree. (1) Every backticked path and markdown link in `ai/`, `.claude/rules/` and the `plan/` meta documents resolves. `plan/learned/DESIGN-HISTORY.md` is scanned, not exempt. (2) Every `// Design:` target resolves. (3) Every backticked `*.sh` filename and `c_*`/`check_*` function name in the hook-describing documents names something in the tree. Those names resolve against top-level `def` names, not against the `CHECKS` registry. (4) Every `doc-links: ignore` marker states a reason: `<!-- doc-links: ignore (why) -->`. Checks 1, 3 and 5 skip a line only for a marker that states one, and check 4 sweeps every TRACKED file, so a marker outside the walked corpus is audited rather than left as decoration. (5) Every path reference in every OTHER tracked file resolves, under check 1's grammar. `vendor/` and `third_party/` are excluded: their references point into another repository. `plan/handover/` is excluded too: it records the tree as it was. `scripts/dev/doc_citation_baseline.txt` grandfathers the pairs that predate check 5. `check_baseline_growth` refuses every pair the baseline holds and HEAD does not, so that file only shrinks. It compares the pairs, never their number: a repair and a new dead citation in one commit leave the total unmoved and are still refused. Checks 4 and 5 share one read of each tracked file. Gate: `make ze-doc-links-check`, its own `ze-precommit-verify` stage. `make ze-doc-verify` runs no part of it; `ze-generated-files-reconcile`'s recipe ends with the `--md-only` subset, which drops check 2 and keeps 1, 3, 4 and 5. |
...
| `stress-repro.py` | `scripts/dev/` | Reproduce load-dependent / flaky-in-full-verify test failures WITHOUT the full suite: CPU+GC burners oversubscribe every core while many concurrent `ze-test <suite>` runs loop, capturing the first failure's untruncated output (`GOTRACEBACK=all`; optional `--race`). `<suite>` and `--test` are whitespace-split, so `"bgp plugin" --test 97` works. Add `--any-failure` for assertion flakes (a missed `expect=` never carries a crash signature, so the default crash-only mode reports "not reproduced" and discards the evidence). Honours `ZE_TEST_NO_BUILD`, so rebuild this session's `ze` (`make ze-build`, path from `make ze-session-binary-path`) after changing daemon source or the verdict is against a stale binary. Writes `tmp/stress-repro/<slug>-<ts>.log`. See `ai/rules/testing.md`. |
| `rename_module_path.py` | `scripts/dev/` | Rename the Go module path repo-wide: every import, `go.mod`/`replace` target, goimports `local-prefixes`, build config, AND the directories whose names mirror the module path (`gokrazy/ze/builddir/<module>/`). Re-sorts import groups with `goimports -format-only -local <new>` because the module-local group is keyed by the module path. REFUSES `*.pb.go` (the rawDesc encodes `go_package` with a varint length prefix, so a textual rewrite corrupts it silently) and points at `make ze-proto-generate`; reports, never silently drops, occurrences that are not module paths (an absolute checkout path) and leftover references to the old HOST (hosting URLs, history). Re-stamps the `rfc/audit/*.json` verdict fingerprints the rename staled (they hash the whole enclosing test file, so one rewritten import line invalidates a verdict about untouched assertions), re-sealing ONLY where it can prove per file that HEAD's content under the rename equals the current content, and refusing anything else. Dry-run by default, `--apply` to execute; does no git operations. |
| `make ze-proto-generate` | `Makefile` | Regenerate `api/proto/*.pb.go` from `api/proto/ze.proto` using the vendored `protoc-gen-go` / `protoc-gen-go-grpc` (versions pinned by `go.mod`; needs `protoc` on PATH). Required after a module-path rename. |
| `bundle-html.py` | `website/presentations/tools/` | Inline local images, slides.md, and embeds into HTML as a self-contained file. Output: `<name>-inlined.html`. Accepts multiple files. |
| `make ze-doc-wiring-check` | `mk/check-docs.mk` | Changed-file-aware wiring, documentation, command, and inventory gate used by `make ze-precommit-verify`. |
| `make ze-dependency-vulnerability-check` | `Makefile` | Mandatory SCA stage in both `ze-precommit-verify` modes. Keeps the outer Go process host-native and launches govulncheck with a Linux/amd64 environment, so the scanner loads that package graph. Requires the live Go vulnerability database. |
| `go run ./scripts/status/verify_run.go ze-precommit-verify` | `scripts/status/verify_run.go` | Verify protocol runner used by `make ze-precommit-verify`. Writes `tmp/ze-verify.log`, per-stage logs, compact failure indexes, and `tmp/ze-verify.status`. |
| `make ze-verify-scope-selector` | `scripts/checks/verify_scope_selector.go` | Prints what a `-changed` target will judge: the Go packages to retest and the feature tags the change can reach, from one tag-aware reverse import graph bounded at depth 2. `ARGS="--print=packages\|tags\|both"`, `--paths-from=FILE` to ask about a path list you supply, `--depth=N`, `--drop-log=FILE` for what the bound dropped. A non-Go path seeds the packages whose tests READ it. It widens to `./...` and names the reason on stderr whenever it cannot narrow, one of which is `tmp/ze-verify.status` holding no green commit. `verify_run.go` runs it once per verify and publishes both halves: the packages as `ZE_VERIFY_SCOPE_PACKAGES`, and the feature tags as `ZE_VERIFY_SCOPE_TAGS` for `ze-staticcheck-feature-matrix-check`. The tag half is published only when the selection succeeded, so a failed selection widens the matrix instead of narrowing it. `scripts/dev/changed-pkgs.sh` is what the recipes call. Contract: `docs/architecture/testing/verify-freshness-scope.md`. |
| `verify-status.sh` | `scripts/dev/` | Checks whether the current tree is byte-identical to the last passing `ze-precommit-verify` run. Commit preparation must treat FRESH as authoritative and skip rerunning verify. `check PATH...` asks the same question about those paths only, which is what lets a session hold evidence about its own files while other sessions edit the rest of the checkout. |
| `make ze-doc-verify` | `mk/check-docs.mk` | Documentation drift, YANG command handler contracts, stale source anchors, the symbols those anchors name (`check_anchor_symbols` in `scripts/dev/code_to_docs.py`, which resolves each token after an anchor's `--` against the anchored file's own top-level declarations), the six rule-corpus gates, the discovery indexes, the problem journal, and the `ai/digests/` anchors. It does NOT run `check_doc_links.py`: that is `make ze-doc-links-check`, its own `ze-precommit-verify` stage. |
...
| `make ze-ai-skills-sync` | `scripts/dev/skill_sync.sh` | Sync canonical `ai/skills/*.md` to `.claude/skills/`, `.codex/skills/`, and `.agents/skills/`; also regenerates `CLAUDE.md` and `AGENTS.md` from `ai/INSTRUCTIONS.md`. |
| `make ze-spec-status` / `make ze-spec-status-json` | `mk/report-inventory.mk` | Spec progress overview (committed backlog vs skeleton idea capture, stale-skeleton flags) plus a non-blocking completed-but-not-closed advisory. |
| `make ze-spec-citation-check` | `mk/check-docs.mk` | Spec citation freshness: fails on a `plan/spec-*.md` citing a sibling spec absent on disk (grandfathered via `plan/.citation-baseline`); WARNs on `path:line` line-token drift. Runs on the verify path when a `plan/` file changes. A closure feeds it: the author who removes a spec clears every sibling that cites it, inside commit A (`ai/rules/planning.md`, "Spec Closure"). |
...
| `make ze-mutation-test-changed` | `mk/test-mutation.mk` | Incremental mutation testing on changed files only. |
| `make ze-mutation-report` | `mk/test-mutation.mk` | Mutation testing with HTML report output to `tmp/mutation-report.html`. |
| `make ze-test-health-update` | `scripts/dev/testing_health.py` | Regenerates `docs/features/test-health.md` and `test/health/latest.json`: whether a regression would be caught, not how many tests exist. Read it before claiming the suite is healthy. |
...
| `make ze-test-sensitivity-check` | `scripts/checks/inert_tests.go` | Ratchets tests that cannot fail and test files no `go test` target builds. Stage 10 of `ze-precommit-verify`, both modes. |
| `make ze-test-weakened-check` | `scripts/dev/check_weakened_tests.py` | Selftests the weakened-test checker, then checks that `test/weakened.md` parses. A stage of `ze-precommit-verify` in both modes. The file records every test weakening the commit in hand needs accepted, one row of test name and reason, and it is replaced per commit so it never accumulates. The same checker is called by `c_test_weakening` (`.claude/hooks/pretool-writeedit.py`) at edit time and by `weakened_problems` (`scripts/dev/commit_helper.py`) at commit time, so the two gates cannot disagree about what a diff weakens. This target reads no working tree: whether a commit's weakenings are covered is a question about that commit's `--file` list, which a verify stage does not have. |
...
| `make ze-repository-tracked-build-check` | `scripts/checks/tracked_build.go` | Compiles the tree GIT HOLDS, not the working tree every other gate reads: catches a consumer committed without its producer. `REV=<sha>` judges any commit, `ARGS=--keep` keeps the extracted tree. Runs in `ze-precommit-verify`, both modes. |
| `make ze-staticcheck-feature-matrix-check` | `scripts/checks/staticcheck_feature_matrix.go` | Structural type-check for working-tree production and `_test.go` sources. It derives N+2 rows from the N unique manifest features: distro all-on, bare core, and one row for each omitted feature. Its guarantee is limited to direct single-feature omissions. Under a verify run it reads `ZE_VERIFY_SCOPE_TAGS` and judges only the rows the change can move: the two rows that omit no tag, plus one row per tag the change reached. A tag a changed file NEGATES counts as reached, because a file constrained `!ze_ssh` compiles in `without_ze_ssh` and in no other row (3 of 38 for a `ze_ssh`-local change, 17.7s against 215s hot -- an order of magnitude, not a benchmark: the box was shared and all three timed runs exited 1 on a compile error already in the tree). An unset, unreadable, or every-tag answer, and an answer naming a tag `feature-gates.txt` does not declare, each judge all 38; an EMPTY answer judges 2. `all_features` and `core_only` are never subtracted. Rerun this target directly. `ze-repository-tracked-build-check` remains the committed-tree final-link proof. |
| `make ze-test-health-check` | `scripts/dev/testing_health.py` | Fails `ze-precommit-verify` when a STRUCTURAL fact drifts (an orphaned test file, an unproven RFC, a metric status). Volume counters are published, not gated. The target a developer meets when verify goes red on this feature. Runs inside `ze-generated-files-check`. |
| `make ze-vendor-web-sync` | `scripts/vendor/sync_web.go` | Copies each asset from `third_party/web/<package>/` into every consumer `assets/` directory. `//go:embed` cannot reach outside its own package, so one library is vendored once per consumer and each copy is a GENERATED file that stays tracked in git. Part of `make generate` since 2026-08-15, which is what makes a hand-edited copy a transient state rather than a permanent one. `--root DIR` names another tree. |
| `make ze-vendor-web-check` | `scripts/vendor/check_web.go` | The read-only twin of the sync, and a stage of `ze-precommit-verify` in both modes. It exits non-zero and names each consumer copy that differs from its source, is missing, or has an unreadable source. Until 2026-08-15 it printed `DRIFT:` and exited 0, so every gate reading it stayed green over a drifted tree. It reads NO network: a consumer subscribes to a vendor directory by holding one file of it, and both the directory list and the consumer list are derived from the two trees, so a new asset needs no edit here. `--updates` adds the npm registry query, and `make ze-vendor-web-update-report` is that call. |
| `make ze-htmx-upgrade-check` | `scripts/dev/htmx_upgrade_check.py` | Runs htmx's own upgrade scanner (vendored at `third_party/web/htmx-upgrade-check.py`) over every package that embeds an htmx core file, and is a stage of `ze-precommit-verify` in both modes. The scan scope is DERIVED: an `internal/**/assets/` directory holding `htmx*.js` enrols the package that owns it, so a fourth interface is scanned the day it embeds the library. It reads `.templ` and `.go` beside the scanner's own extensions, because Ze writes markup in templ sources, in Go string literals and in captured fixtures. Every issue it reports must be FIXED or hold a row in `scripts/dev/htmx-upgrade-explained.txt` as `<path> \| <category> \| <reason>`; the row fails the gate both ways, so a stale exemption is red. `make ze-htmx-upgrade-report` prints every issue, explained or not, and exits 0. |
| `make ze-web-assets-check` | `scripts/codegen/web_assets.go` | The read-only twin of the per-page import generator, and a prerequisite of `ze-generated-files-check`. The generator walks each page's templ component graph, collects the `hx-*` and `sse-*` attributes that page reaches, and writes the set into `page_assets.go` in `internal/component/web`, `internal/component/lg` and `internal/chaos/web`. A head block renders that set instead of hand-writing its script tags, so the L2TP, sign-in and snapshot pages load no htmx at all and only the looking glass's peers page loads the SSE extension. A page is a component rendering `<head>`, which names itself as `pageAssets(pgL2tpList)`, or a body carrying `//ze:page`, which the shell names at render time as `pageAssets(v.Page)`. An unknown page gets every asset the package renders: one file too many costs bytes, one missing gives a page that renders correctly and does nothing. `--json` prints the sets and writes nothing. The derivation OVER-approximates from source; `TestPageImportsCoverRenderedAttributes` reads the captured fixtures and under-approximates from output. A disagreement is a defect in one of the two and neither is relaxed to make the other pass. |
| `make ze-chaos-golden-update` | `Makefile` | Recaptures the 52 chaos dashboard fixtures under `internal/chaos/web/testdata/golden/` after a DELIBERATE markup change. It is `ze-web-golden-update`'s twin. The CHECK needs no target: `TestChaosGoldenOutput` is a plain Go test that `ze-unit-test` runs, and it names each page whose bytes moved. Read the diff before committing it: every byte it rewrites is a byte an operator receives. |
| `make ze-dev-setup` | `scripts/dev/dev-setup.py` | Unified dev setup: installs all build deps, linters, and appliance/evidence tools (qemu, e2fsprogs, xorriso, grub, uv; optional Linux L2TP-evidence deps xl2tpd, ppp). Also two Linux system-state prerequisites that are not packages: `userns-unrestricted` (Chrome sandbox) and `kvm-access` (the `kvm` group, without which QEMU evidence cannot start). OS autodetect (brew/apt). `CHECK=1` for probe-only mode. Drift-guarded against `applianceDoctorChecks()`. |
...
Structural decisions, patterns, and gotchas. Recurrence data: `plan/journal/` (`make ze-journal-report`).
...
| templ, `.templ`, generated markup, view-model type safety | `make ze-templ-output-check` (Dev Tools above), `tools.go`, `internal/component/web/templ_typesafety_test.go` |
| per-page asset imports, `page_assets.go`, `pageAssets`, `//ze:page`, htmx extension per page | `make ze-web-assets-check` (Dev Tools above), `scripts/codegen/web_assets.go`, `internal/test/markupcheck/head.go` |
...
| RFC requirement coverage, RFC MUST tests, rfc-requirements, RFC requirement tag, ze-rfc-check, ze-rfc-index-update | `make ze-rfc-check`, `rfc/requirements/<stem>.md` (one RFC's rows), `ai/RFC-REQUIREMENTS.md` (the index), `ai/skills/ze-rfc.md`, `docs/contributing/rfc-implementation-guide.md`, `docs/functional-tests.md` (RFC Requirement Tags) |
| RFC extraction sign-off, extraction completeness, what the summary MISSED, unextracted obligation, normative site, extraction register, rfc2119/prose/manual-walk, drain budget, ze-rfc-extraction-create, ze-rfc-extraction-status | `rfc/extraction/README.md`, `make ze-rfc-extraction-create`, `make ze-rfc-extraction-status`, `rfc/drain-budget.txt`, `ai/rules/rfc-compliance.md` (Extraction Completeness, the five ratchets), `ai/RFC-REQUIREMENTS.md` (Extraction sign-off) |
...
| changed set, scoped verify, which packages changed, change-set selector, reverse dependency depth, feature scope, staticcheck matrix rows, `ZE_VERIFY_SCOPE_TAGS` | `make ze-verify-scope-selector`, `scripts/checks/verify_scope_selector.go`, `scripts/checks/staticcheck_feature_matrix.go`, `scripts/dev/changed-pkgs.sh`, `docs/architecture/testing/verify-freshness-scope.md`, `ai/rules/commands.md` |
| declared failure group, VERIFY FAILURE GROUP, whose red is this, attributing a structural red, failure index | `scripts/status/verify_run.go` (`parseDeclaredGroups`, `classifyStage`), `scripts/dev/verify_wiring_docs.py` (`declare_failure_group`), `scripts/dev/commit_helper.py` (`structural_gate_reds`), `docs/architecture/testing/verify-freshness-scope.md`, `ai/rules/precommit-verify.md` |
| commit, commit script, commit message, verified commit, verify freshness, owner override, commit no test, verification debt, gate owed, push refused | `scripts/dev/commit_helper.py`, `scripts/dev/verify-status.sh`, `ai/rules/git-safety.md`, `ai/rules/precommit-verify.md`, `ai/skills/ze-commit.md`, `ai/skills/ze-commit-check.md` |
...
| inventory, command-list, doc drift, source anchor, doc index | `ai/rules/repo-maintenance.md`, `ai/rules/writing.md`, `docs/contributing/documentation-testing.md`, `mk/check-docs.mk` |
...
| Dependabot alert on vendored go.mod, gokrazy/modcache manifest, bump gokrazy init, appliance dependency bump, CVE on vendored appliance dep | `ai/rules/platform-linux.md`, `mk/build-gokrazy.mk` (`ze-gokrazy-deps-download`), `.github/dependabot.yml` |
| installer initrd QEMU evidence, R-6 fault injection, ze.mac pin, rescue console, Ventoy ISO-on-FAT, ze_installer_fault, ZE_INITRD_FAULT | `scripts/evidence/effective-install-scenarios-qemu.py`, `scripts/evidence/effective-install-ventoy-qemu.py`, `internal/install/disk/fault_linux.go`, `mk/test-integration.mk` (`ze-qemu-install-scenarios-test`, `ze-qemu-install-ventoy-test`), `docs/functional-tests.md` |
| VPP hugepage boot reservation, poll-sleep-microseconds, image.hugepages, doctor-vpp-hugepages, hugepage QEMU evidence | `internal/appliance/kernelargs.go`, `internal/component/vpp/doctor_linux.go`, `internal/component/vpp/startupconf.go`, `scripts/evidence/effective-vpp-hugepages-qemu.py`, `mk/test-integration.mk` (`ze-qemu-vpp-hugepages-test`), `docs/guide/vpp.md`, `docs/guide/appliance.md` |
...
| .ci test prerequisite, option=needs-path, caps=net-raw, caps=net-admin, caps=bpf, test skips instead of failing, missing modcache, ze-gokrazy-deps-download prerequisite | `docs/architecture/testing/ci-format.md` (Options table), `ai/rules/platform-linux.md`, `internal/test/runner/caps.go`, `internal/test/runner/needs_path.go` |
...
| code-to-docs, reverse index, which docs | `ai/CODE-TO-DOCS.md` (generated, `make ze-doc-index-update`) |
