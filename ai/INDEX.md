...
| `make ze-doc-wiring-check` | `mk/inventory.mk` | Changed-file-aware wiring, documentation, command, and inventory gate used by `make ze-precommit-verify`. |
...
| `make ze-cli-grammar-check` / `-json` | `mk/inventory.mk` | CLI grammar gate: every built-in command obeys the verb-first rules R1-R9 (`ai/rules/cli.md`; R9 = compound-vs-namespace split) and no `.yang` carries a `--flag`. In `make ze-precommit-verify`. |
| `make ze-doc-index-update` | `mk/inventory.mk` | Regenerate `ai/CODE-TO-DOCS.md`, the source-to-document reverse index. |
