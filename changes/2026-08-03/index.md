...
MUST level first, and nothing goes ahead of it. Three things, in that order. Fixing the defects this has already turned up, which is where the current backlog sits. Then the 85 MUSTs that still owe a test. Then reading the remaining 165 documents end to end, so every list of MUSTs is known to be complete.
