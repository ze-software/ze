...
`le test functional select` prints the run list a gating run would start for this checkout, and runs nothing. It is what an operator reads when a run started fewer suites than they expected, and it is the run's own decision rather than a second derivation of one: `planRun` (`internal/le/test/functional/suitemap.go`) produces the plan, `le test functional select` prints it, and `runGating` executes it.
...
A run can ATTACH. A second run of the same work over the same tree takes the holder's exit code rather than judging twice, and the certificate on disk is the one the holder wrote. Such a report SAYS so: `Report.Attached` names the holder's registry entry and its progress log, because the run judged nothing and therefore carries no stages and no log directory of its own. Without that field a zero code reads as a green nobody earned, which is the reading `admissionFailure` refuses to publish for the other path that reaches no verdict.

<!-- source: internal/le/verify/current.go -- attachedReport -->
...
|------|----------------------|--------|
| `./le go lint run`, `./le repo changed scope` | the `.go` file each finding sits in | a drop when none of them is in the file list |
...
`./le commit debt-clear part <n> of <m>` runs ONE piece of that verification. The whole population is every stage the mode declares (`verifyengine.StagesForMode`, counted for you on the `stages:` line of `./le verify reds file <path>`; the number moves whenever a stage is added or retired, so it is not written down here). It uses the whole machine, so on a box several sessions share it is killed before a single row is reachable: on 2026-09-07 the OOM killer ended a pass with 3302 rows open across 216 shards, and nothing cleared. A piece that exits 0 records itself in `tmp/ze-verify-debt-parts.json`, so a piece killed mid-flight costs that piece rather than the pass.
...
"Wrote this row" has two arms and either one answers it. The commit ADDED an OPEN ledger row holding this row's GATE under this row's REASON, read from `git show <sha> -- <shard>` and parsed back with `parseDebtRow`, or the commit appears in the history of the row's OWN line, read with `git log -L<line>,<line>:<shard>`. The status cell is read because `clearDebtRows` rewrites a row in place, so a debt-clear's diff ADDS the row it cleared, spelled `cleared`. `recordDebt` is the only producer that OPENS an obligation and it writes `open`, so a commit that only flipped a sibling row of this gate and this reason opened nothing and binds nothing. The line history was the whole condition until 2026-09-08 and it cannot reach three of this ledger's rows: before the 2026-09-07 dedup a session's second commit under one gate and one reason wrote a row of its OWN, that pass merged each such pair and DELETED the second row's line, and no line history reaches a line that was deleted. Gate and reason are the dedup's own merge key, so both cells are byte-identical across the merged pair and they recover the second commit.

The row's Line is a WORKING-TREE coordinate and `git log -L` resolves it against HEAD, so the second arm first requires HEAD to hold this row's own text at that line. Where it does not, the ledger carries an uncommitted edit, the number answers about another row, and the discharge is refused rather than derived: a row the same session wrote on the line below binds the commit that wrote IT to a row it never wrote. `recordDebt` appends a row and `extendDebtRow` rewrites one in place, so no producer moves a line and the refusal asks the operator to commit the ledger.
...
What that still admits is stated on `dischargeCommits` itself. The first arm admits a commit that added a DIFFERENT row of this shard under this row's gate AND this row's reason, and two such rows are the same obligation by the merge key `openDebtRowAt` reads. A row of another gate carrying this reason, and a row whose longer reason contains this one, are different obligations and neither binds. Neither does a `cleared` line of this exact pair, which is a debt-clear rewriting a row rather than a commit opening one. Both arms admit a commit that REWROTE the shard wholesale, the 2026-09-07 dedup among them, because it adds every line and is in the history of every line, so it can stand in for one of a multi-commit row's commits. Telling a rewrite from an extension needs a diff analysis the code does not do. A row whose `(+N more)` counts more RECORDINGS than commits, which a re-run of `create` with a reworded subject leaves behind, cannot be answered this way and stays open: over-refusing is the safe direction, because the alternative discharges N commits' obligation on evidence about one.
