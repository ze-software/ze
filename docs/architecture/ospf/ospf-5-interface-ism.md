...
- **The ISM keeps its own neighbor records, separate from the NSM.** RFC 2328
...
- A config reload recreates an interface runtime when either half of
  ...
  own block and compares the restart boundaries, including area, network type,
  timers, and authentication. Cost is excluded. Recreating a runtime empties its
  neighbor map, clears its DR and its BDR, and reports the interface down.
  <!-- source: internal/plugins/ospf/instance.go -- reconcile, interfaceParamsEqual, interfaceGlobalParamsChanged -->
- `reference-bandwidth` changes and explicit interface `cost` changes re-price
  the interface in place. Adding a cost overrides auto-cost, and removing it
  restores auto-cost from the current reference bandwidth and link speed.
  `repriceInterfaceLocked` updates the enrolled config that origination reads,
  then calls `SetCost` for the runtime snapshots. The interface keeps its
  neighbors and DR state in both address families.
...
- Neighbor inactivity scheduling uses the exact next `LastSeen` plus
