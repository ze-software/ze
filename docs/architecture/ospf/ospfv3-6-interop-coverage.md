...
- **The native repository check carries an interface-seam exemption.** A method
  on an UNEXPORTED receiver type that satisfies a same-package exported
  interface is exempt. An interface method cannot be unexported without
  breaking an external implementor, and a cross-package test backend implements
  this one.
