...
| Demo call sites | Every `ze <token>` invocation under `demos/terminal/`: the position-1 token must be a YANG verb, a registered root, or the `-` stdin sentinel | `./le cli grammar` reads the demo sources; `./le site terminal-demo check-all` validates the published artifacts |
