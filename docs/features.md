...
| Commit-Time Backend Capability Check | YANG nodes that correspond to backend-specific features carry a `ze:backend "<names>"` annotation. On commit (daemon reload, first-apply, and `ze config validate`), the walker rejects the config with the YANG path and the list of supporting backends whenever the active backend does not implement the feature -- instead of letting an Apply-time "not supported" error fire inside the backend. The gate covers `interface` (netlink-only bridge, tunnel, wireguard, veth, mirror under the vpp backend) and `traffic-control` (the backend-leaf CALL wired into `OnConfigure`/`OnConfigVerify`; tc-only feature annotations ship with `spec-fw-7-traffic-vpp`). Firewall adopts the same pattern once fw-8 lands. |
