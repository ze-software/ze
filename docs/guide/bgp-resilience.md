...
For a task-oriented example, see the [FlowSpec route-reflector guide](flowspec-route-reflector.md). The same reflection topology applies to other negotiated families, subject to their plugin support.
...
Verify send and receive modes, path identifiers, best-path state, and installed ECMP next hops separately. See [ADD-PATH](add-path.md) and the [BGP protocol page](../features/bgp-protocol.md) for current support.
...
- [BGP policy](bgp-policy.md)
- [Graceful Restart](graceful-restart.md)
- [ADD-PATH](add-path.md)
- [Lifecycle and rollback](lifecycle.md)
