...
Changing `reference-bandwidth` costs no adjacency. Ze re-prices each interface in place and originates its Router-LSA at the commit, so no neighbor goes down and no database exchange runs. Raise the leaf across a router with forty auto-costed links and all forty adjacencies stay up. That promise covers `reference-bandwidth` alone. Two reloads outside an interface's own block do restart it, because both change what it stamps into the Hellos it sends: a new router id, and an area type that changes the E-bit or the N-bit. A change inside the block restarts that one interface, and its own `cost` leaf is one of those fields: setting `cost 5` on eth1 recreates eth1's runtime and drops its neighbor while the adjacency re-forms.
