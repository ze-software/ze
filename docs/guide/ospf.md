...
Changing `reference-bandwidth` preserves established adjacencies. Ze re-prices each interface in place and requests Router-LSA origination at the commit. If MinLSInterval prevents publication, the maintenance timer retries after the interval, including on passive-only and loopback-only routers. That promise covers `reference-bandwidth` alone. A new router ID or an area-type change that changes the E-bit or N-bit restarts the affected interfaces. Changes inside an interface block also restart that interface, including its own `cost` leaf.
