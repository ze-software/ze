...
If Ze terminates TLS itself, set `tls true`. This requires a zefs blob store to hold the certificate, so create one first with `ze init` (see [Build and install Ze on Ubuntu](ubuntu-build-install.md)); otherwise startup fails with `looking glass TLS requires blob storage (run ze init first)`.
