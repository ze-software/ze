...
| Test files that expect a specific error | Q2 | **1101 / 3235** (attention) | Take the lowest-ranked subsystem and add malformed-input or fault-injection cases. |
...
**23718 test functions** (ok)

3235 Go test files, 78 fuzz targets, 132 benchmarks, 1728 .ci scenarios, 166 .et editor tests. Counts cover internal, cmd, pkg, scripts, test only: vendor/ and gokrazy/modcache/ are third-party module trees and are excluded.
...
Their build tags are supplied by no native test action, so these tests exist
but never run. Add the tag to the appropriate Go action, or delete the file.
<!-- source: internal/le/testsensitivity/tags.go -- build-tag satisfiability -->
