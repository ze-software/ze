...
| Tests with no reachable failure call | Q1 | **134 / 28068 (floor 132)** (attention) | Add a real assertion, or annotate with `// test-asserts-nothing: <why>` when the oracle is genuinely implicit (a must-not-panic smoke test). |
...
**28099 test functions** (ok)

3787 Go test files, 80 fuzz targets, 132 benchmarks, 1867 .ci scenarios, 170 .et editor tests. Counts cover internal, cmd, pkg, test only: vendor/ and gokrazy/modcache/ are third-party module trees and are excluded.
