...
| RFC 6811 | RPKI origin validation | Partial | Origin validation states (Valid/Invalid/NotFound) from ROA/VRP lookup with four-octet ASN support, RTR transport, automatic reject of Invalid routes, and a fail-open guard. | Three RFC 6811 MUST gaps, gated per requirement in `rfc/short/rfc6811.md`: (2-2) Invalid routes are auto-rejected with no configuration to retain them for policy handling; (3-1) the validation state is not exposed to the route-policy language for match/set; (4-1) a VRP mapping change does not automatically re-validate installed routes (only ASPA changes do). |
