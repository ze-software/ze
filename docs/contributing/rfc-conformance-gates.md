...
| `revert` on a `.ci` | the producing function's body replaced by a halt | `ze-test <suite> <name>`, ONE `.ci`, against the isolated set `testfunctional.Prepare` builds under the same overlay |
