...
- `AttrIterator` reused from `internal/plugin/bgp/attribute/` — do not duplicate parsing logic.
...
- `AttrIterator` is at `internal/plugin/bgp/attribute/` not `internal/bgp/attribute/` — wrong path causes compile failure.
...
- `internal/plugin/rib/storage/routeentry.go` — RouteEntry with per-attribute handles, Has*, Release, AddRef, Clone
- `internal/plugin/rib/storage/attrparse.go` — ParseAttributes() using AttrIterator
- `internal/plugin/rib/storage/familyrib_perattr.go` — FamilyRIBPerAttr with per-attr storage
