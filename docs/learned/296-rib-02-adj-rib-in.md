...
- Shared package at `internal/component/bgp/shared/` (infrastructure), not `internal/component/bgp/plugins/` (implementations) — correct package boundary
...
- `internal/component/plugin/` = infrastructure to support plugins; `internal/component/bgp/plugins/` = plugin implementations
...
- `internal/component/bgp/shared/` — event.go, route.go, format.go, nlri.go (extracted from bgp-rib)
- `internal/component/bgp/plugins/adj_rib_in/` — rib.go (AdjRIBInManager, RawRoute), rib_commands.go, register.go, schema/
- `internal/component/bgp/plugins/rib/event.go` — type aliases to shared package
