...
Review all 56 documentation files under `docs/` (excluding `docs/plan/`) and fix inaccuracies against actual source code — wrong struct field names, stale spec references, removed types, and TODO items that were already implemented.
...
- Completed specs consistently referenced as `docs/plan/spec-*.md` in docs — must be updated to `docs/learned/NNN-*.md`.
