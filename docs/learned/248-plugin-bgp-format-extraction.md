...
Extract `text.go`, `json.go`, `server_bgp.go`, `validate_open.go` from `internal/plugin/` into `internal/plugins/bgp/format/` and `internal/plugins/bgp/server/` using a BGPHooks callback struct pattern to break the circular import.
...
- `internal/plugin/types.go` — `BGPHooks` struct + 6 callback function types
- `internal/plugin/server.go` — `bgpHooks` field + dispatch through hooks
- `internal/plugins/bgp/format/text.go`, `json.go`, `codec.go` — all Format* functions and JSONEncoder
- `internal/plugins/bgp/server/events.go`, `codec.go`, `validate.go`, `hooks.go` — event dispatch, codec RPCs, OPEN validation, `NewBGPHooks()` factory
