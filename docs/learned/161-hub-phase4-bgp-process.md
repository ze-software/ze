...
Move BGP code (`internal/bgp/*`, `internal/rib/`, `internal/reactor/`) under `internal/plugin/bgp/` and make `ze bgp` work as a forked child process of the hub completing the 5-stage protocol.

## Decisions

- `internal/rib/` (BGP engine's peer-to-peer zero-copy routing) moves WITH BGP to `internal/plugin/bgp/rib/`. It is NOT the same as `internal/plugin/rib/` (adj-RIB tracking plugin). Both names are similar but are completely different concerns.
...
- `internal/rib/` vs `internal/plugin/rib/` naming collision risk: these are easy to confuse. The former is BGP-engine-internal (zero-copy), the latter is a separate adj-RIB process.

## Files

- `internal/plugin/bgp/` — all BGP protocol code (moved from `internal/bgp/`, `internal/rib/`, `internal/reactor/`)
