...
- `ze-hub-conf.yang` stays in `internal/hub/schema/` (moved from yang/modules/) — it's a domain definition owned by hub, not a YANG library file.
...
- Init-based registration (same pattern as `internal/plugin/registry/`) scales to any number of YANG modules without touching the loader.
...
- `internal/plugin/schema/` — ze-plugin-conf.yang, embed.go, register.go (moved from yang/modules/)
- `internal/hub/schema/` — ze-hub-conf.yang, embed.go, register.go
