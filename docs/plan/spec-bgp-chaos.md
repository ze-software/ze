...
Build `ze-chaos`, a long-lived chaos monkey tool for testing Ze's BGP route server (route reflector) route propagation behavior.
...
### Entry Point
- `ze-chaos` binary starts, generates scenario from seed
...
|-------|-------------------|-------------------|
| AC-1 | `ze-chaos --seed 42 --peers 4 --duration 30s` | Runs 4 peers for 30s, prints seed, exits with summary |
...
`cmd/ze-chaos/main.go` — standalone binary, built alongside `ze`, `ze-peer`, `ze-test`

### CLI Interface

```
ze-chaos - Chaos monkey for Ze BGP route server testing
...
```
# Generated by ze-chaos (seed: 1234567890)
...
```
ze-chaos | seed: 1234567890 | uptime: 2m35s | chaos: 3 events
...
- `cmd/ze-chaos/main.go` - CLI entry point and flag parsing
- `cmd/ze-chaos/orchestrator.go` - Lifecycle coordination
- `cmd/ze-chaos/scenario/generator.go` - Seed-based scenario generation
- `cmd/ze-chaos/scenario/profile.go` - PeerProfile type
- `cmd/ze-chaos/scenario/routes.go` - Per-family route generation
- `cmd/ze-chaos/scenario/config.go` - Ze config generation
- `cmd/ze-chaos/peer/simulator.go` - Per-peer goroutine
- `cmd/ze-chaos/peer/session.go` - BGP session (OPEN/KEEPALIVE)
- `cmd/ze-chaos/peer/sender.go` - Route scheduling and sending
- `cmd/ze-chaos/peer/receiver.go` - Incoming message handling
- `cmd/ze-chaos/chaos/scheduler.go` - Chaos event scheduling
- `cmd/ze-chaos/chaos/events.go` - Chaos event types
- `cmd/ze-chaos/chaos/executor.go` - Event execution
- `cmd/ze-chaos/validation/model.go` - Expected route state model
- `cmd/ze-chaos/validation/tracker.go` - Actual received route state
- `cmd/ze-chaos/validation/checker.go` - Expected vs actual comparison
- `cmd/ze-chaos/validation/convergence.go` - Latency tracking
- `cmd/ze-chaos/report/dashboard.go` - Live terminal output (ANSI TTY + line fallback)
- `cmd/ze-chaos/report/jsonlog.go` - NDJSON event logging
- `cmd/ze-chaos/report/summary.go` - Exit summary (iBGP/eBGP counts)
- `cmd/ze-chaos/report/metrics.go` - Prometheus endpoint (per-instance registry)
- `cmd/ze-chaos/report/reporter.go` - Reporter struct (synchronous event multiplexer)
- `cmd/ze-chaos/peer/event_string.go` - EventType.String() method
- Test files for all packages above

## Files to Modify

- `Makefile` - Add `ze-chaos` build target
...
|-------------|--------|----------|-------|
| Single Go binary, multiple peers | ✅ Done | `cmd/ze-chaos/` | Phases 1-4 (specs 255-258) |
