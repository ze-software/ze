...
3. `docs/plan/spec-arch-0-system-boundaries.md` — umbrella arch spec (Bus, Subsystem, Plugin)
...
### Architecture Docs
- [ ] `docs/plan/spec-arch-0-system-boundaries.md` — Bus, Subsystem, Plugin boundaries
...
- [ ] `internal/plugin/registry/` — plugin registration via `init()`, `Register()` function
...
### Integration Points
- `internal/plugin/registry/` — interface plugin registers here
...
- `internal/plugin/registry/registry.go` — ensure interface plugin can register (may already be sufficient)
- `internal/plugin/all/all.go` — blank import for new plugin packages (auto-generated by `make generate`)
