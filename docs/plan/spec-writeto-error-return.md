...
var (
    ...
    ErrInvalidState   = errors.New("wire: invalid state for serialization")
)
```
...
| `ErrInvalidState` | Object malformed | Empty AS_PATH, nil NLRI, inconsistent lengths |
...
```go
// CheckedWriteTo validates capacity and state, then delegates to WriteTo
func (x *Foo) CheckedWriteTo(buf []byte, off int) (int, error) {
    // 1. Validate state (where applicable)
    if x.field == nil {
        return 0, wire.ErrInvalidState
    }
    ...
    // 3. Delegate to unchecked WriteTo
...
| Test | File | Validates |
|------|------|-----------|
| `TestCheckedWriteToBufferTooSmall` | `pkg/bgp/wire/writer_test.go` | Returns ErrBufferTooSmall when buffer insufficient |
| `TestCheckedWriteToInvalidState` | `pkg/bgp/attribute/*_test.go` | Returns ErrInvalidState for malformed objects |
| `TestCheckedWriteToSuccess` | Various `*_test.go` | Returns (n, nil) on success, matches WriteTo |
...
### Pre-Implementation
- [ ] `go run scripts/analyze-writeto.go` verifies all WriteTo have Len()
- [ ] Required docs read

### 🧪 TDD Cycle
- [ ] Error types created (`pkg/bgp/wire/errors.go`)
- [ ] Tests written
- [ ] Tests FAIL
- [ ] `CheckedWriteTo` added to interfaces
- [ ] All `CheckedWriteTo` methods implemented
- [ ] Tests PASS
...
### Documentation
- [ ] `docs/architecture/buffer-architecture.md` updated with CheckedWriteTo

### Completion
- [ ] Spec moved to `docs/plan/done/NNN-writeto-error-return.md`
