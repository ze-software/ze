...
- Replaced trigger action dropdown with 8 individual icon buttons in `writeControlSidebar`
- Added `chaosActionIcon()` mapping each action to a Unicode icon (⚡⛔⏳💥🌀💢⚠🔄)
- Added `chaosActionLabel()` mapping each action to a short label (Disconnect, Cease, etc.)
- Added `writeTriggerButtons()` rendering a flex-wrap grid of span.badge elements
- Each button has hx-get, tooltip (from chaosActionImpact), and active-state onclick handler
- CSS: `.trigger-grid`, `.trigger-btn`, `.trigger-active`, `.trigger-icon` styles

### Bugs Found/Fixed
- None

### Documentation Updates
- None required (internal UI change, no config/API impact)

### Deviations from Plan
- Functional test `test/chaos/trigger-buttons.ci` skipped — pure rendering change with no protocol path; fully covered by unit tests
- Active state uses inline onclick (querySelectorAll-based) rather than HTMX hx-on — simpler, no new JS dependencies
...
|-------------|--------|----------|-------|
| Replace dropdown with icon buttons | ✅ Done | control.go:367-393 | writeControlSidebar + writeTriggerButtons |
| Icon per action type | ✅ Done | control.go:497-515 | chaosActionIcon helper |
| Short label per action type | ✅ Done | control.go:518-536 | chaosActionLabel helper |
| Flex-wrap grid layout | ✅ Done | style.css:933-937 | .trigger-grid with flex-wrap |
| Active/highlighted state | ✅ Done | style.css:936 | .trigger-active class |

### Acceptance Criteria
| AC ID | Status | Demonstrated By | Notes |
|-------|--------|-----------------|-------|
| AC-1 | ✅ Done | TestControlPanelRendersTriggerButtons, TestWriteTriggerButtons | 8 buttons, no dropdown |
| AC-2 | ✅ Done | TestWriteTriggerButtonIcons, TestChaosActionIcon | Unicode icon + label |
| AC-3 | ✅ Done | TestWriteTriggerButtonTooltips | title attr from chaosActionImpact |
| AC-4 | ✅ Done | TestWriteTriggerButtonHTMX | hx-get with action param, hx-target |
| AC-5 | ✅ Done | TestControlPanelNoTriggerWhenStopped | No buttons when stopped |
| AC-6 | ✅ Done | TestWriteControlSidebarStopped | Empty sidebar when stopped |
| AC-7 | ✅ Done | TestWriteTriggerButtons | flex-wrap grid container |
| AC-8 | ✅ Done | TestControlPanelRendersTriggerButtons | trigger-active class via onclick |
| AC-9 | ✅ Done | TestWriteTriggerButtonHTMX | POST /control/trigger unchanged |

### Tests from TDD Plan
| Test | Status | Location | Notes |
|------|--------|----------|-------|
| TestWriteTriggerButtons | ✅ Done | dashboard_test.go | 8 buttons, no dropdown |
| TestWriteTriggerButtonIcons | ✅ Done | dashboard_test.go | Icons for all 8 actions |
| TestWriteTriggerButtonLabels | ✅ Done | dashboard_test.go (as TestChaosActionLabel) | Short labels |
| TestWriteTriggerButtonTooltips | ✅ Done | dashboard_test.go | Impact text in title |
| TestWriteTriggerButtonHTMX | ✅ Done | dashboard_test.go | hx-get + hx-target |
| TestControlPanelRendersTriggerButtons | ✅ Done | dashboard_test.go | Sidebar uses buttons |
| TestControlPanelNoTriggerWhenStopped | ✅ Done | dashboard_test.go | Hidden when stopped |
| TestChaosActionIcon | ✅ Done | dashboard_test.go | Non-empty for all 8 |
| TestChaosActionLabel | ✅ Done | dashboard_test.go | Short label for all 8 |
| test/chaos/trigger-buttons.ci | ❌ Skipped | N/A | Pure rendering; unit tests cover |

### Files from Plan
| File | Status | Notes |
|------|--------|-------|
| cmd/ze-chaos/web/control.go | ✅ Done | Replaced dropdown, added icon/label/buttons |
| cmd/ze-chaos/web/assets/style.css | ✅ Done | Added trigger grid styles |

### Audit Summary
- **Total items:** 22
- **Done:** 21
- **Partial:** 0
- **Skipped:** 1 (functional test — pure rendering, unit tests cover)
- **Changed:** 0
