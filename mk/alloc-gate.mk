...
# is machine-independent). Registered as a ze-verify stage in
# scripts/status/verify_run.go, so every full `make ze-verify` and CI push runs
# it. Kept OUT of ze-verify-changed to avoid slowing the inline dev loop.
#
...
# Docker throughput/p99 matrix stays in `make ze-perf-gate`.
