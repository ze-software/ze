...
def render_go_cards(stats: GoBucketStats) -> str:
    return "\n".join(
        [
            ...
            f'<div class="stat"><span>Code lines</span><strong>{display_number(stats.code_lines)}</strong></div>',
            f'<div class="stat"><span>Blank lines</span><strong>{display_number(stats.blank_lines)}</strong></div>',
            f'<div class="stat"><span>Comment lines</span><strong>{display_number(stats.comment_lines)}</strong></div>',
        ]
    )
...
def render_vendor_bucket(stats: GoStats) -> str:
    return f"""<div class="go-bucket">
...
{render_go_cards(stats.vendor)}
            ...
            <p class="note">Tracked vendored .go files under vendor/. Module count comes from vendor/modules.txt.</p>
        </div>"""
...
{render_go_bucket("Total First-Party Go", go_stats.total, "Tracked .go files outside vendor/, including tests.")}
{render_go_bucket("Production Go", go_stats.code, "Tracked .go files outside vendor/ and excluding _test.go files.")}
{render_go_bucket("Test Go", go_stats.tests, "Tracked _test.go files outside vendor/.")}
{render_vendor_bucket(go_stats)}
        </div>
        <p class="note">Comment lines are full-line <code>//</code> comments or block-comment lines; inline comments remain code lines.</p>
    </section>
    </div>
...
function setMetric(metric) {{
    ...
    document.getElementById("total-label").textContent = summary.totalLabel;
    document.getElementById("total-value").textContent = summary.totalValue;
    document.getElementById("active-label").textContent = summary.activeLabel;
    document.getElementById("active-value").textContent = summary.activeValue;
    document.getElementById("peak-label").textContent = summary.peakLabel;
    document.getElementById("peak-value").textContent = summary.peakValue;
    document.getElementById("top-heading").textContent = summary.topHeading;
    document.getElementById("threshold-label").textContent = summary.thresholdLabel;
    document.getElementById("threshold-value").textContent = summary.thresholdValue;
