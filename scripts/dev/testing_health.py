...
Called by: make ze-test-health-update, ze-test-health-check, ze-test-health-record
"""
...
# evidence runs in the scheduled advisory workflow rather than in ze-precommit-verify. It is a
...
# the whole ze-precommit-verify run with no diagnostic.
...
def tracked_files(root: Path) -> set[str]:
    ...
    working tree so `make ze-precommit-verify` catches an inert test before it is
...
def collect_mutation(root: Path) -> Metric:
    ...
    if not path.exists():
        return Metric(
            ...
            action="Run `make ze-mutation-test-changed`, then `make ze-test-health-record`.",
        )
    rows = []
...
def collect_adoption(root: Path) -> Metric:
    ...
    if shallow == "true":
        ...
        # differ from the committed one and red ze-generated-files-check with no
...
def render_markdown(metrics, record) -> str:
    ...
    add("")
    add(
        "GENERATED by `make ze-test-health-update` -- do not edit. Source: "
    ...
    add(
        ...
        "by `make ze-precommit-verify` and cannot lag the tree. The volume counters are as of "
        "the last `make ze-generated-files-update` and may lag by a few tests; they are deliberately "
    ...
    add(
        ...
        "`make ze-precommit-verify` enforces them."
    )
    add(
...
def tighten_baseline(root: Path, metrics, bootstrap: bool = False) -> bool:
    ...
    && make ze-test-health-update` would mint today's counts as the new floors and
    `make ze-precommit-verify` would then pass. Creating the file is a deliberate act,
...
def do_write(root: Path, bootstrap: bool = False) -> int:
    ...
    # ("run make ze-test-health-update to tighten it") therefore broke the staleness
...
def _gated_list(by_key, metric_key, list_field, count_of, count_path, means) -> list:
    ...
    if listed is None:
        raise CollectError(
            ...
            f"same as zero. Run `make ze-test-health-update` and commit {LATEST}."
        )
    ...
    if len(listed) != counted:
        raise CollectError(
            ...
            f"Run `make ze-test-health-update` and commit {LATEST}."
        )
    return listed
...
def structural_facts(record) -> dict:
    ...
    `make ze-generated-files-update` without reading it.
...
def do_check(root: Path) -> int:
    ...
    for rel in (PAGE, LATEST):
        if not (root / rel).exists():
            print(
                f"test-health: {rel} does not exist. Run `make ze-test-health-update`.",
                file=sys.stderr,
            )
            return 1
    ...
    if got != want:
        ...
        print(
            "  Run `make ze-test-health-update` and commit the result.",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        ...
        f"(volume counters are not gated; `make ze-generated-files-update` refreshes them)"
    )
    return 0
...
def do_record(root: Path, bootstrap: bool = False) -> int:
    ...
    # ze-precommit-verify fails on a staleness the recorder itself caused.
    return do_write(root, bootstrap=bootstrap)
