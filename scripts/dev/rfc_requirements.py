...
def tag_site(t: Tag, root: str, cache: Dict[str, str]) -> str:
    """Where a tag sits, NAMED so the citation survives an edit above it.

    `file` plus the enclosing top-level function, never `file:line`. The line is
    derived at scan time and recoverable on demand with `--locate`, so storing it in
    a generated page bought a reader nothing and cost that page a rewrite on every
    edit above any tag in it. 3591 such citations stood across 177 shards.

    The symbol is the SAME anchor `unit_shas` already keys audit fingerprints on,
    resolved through the same `rfc_tagged_scope.func_name_at`, so the two layers
    cannot disagree about which unit an obligation sits in. Its docstring states the
    property this relies on: a key found by name survives every edit above the
    function it names, and dies exactly when that function is renamed or removed.

    None comes back for a non-Go carrier, and for a line no single span encloses. The
    bare path is the honest answer there, exactly as it is when a fingerprint key
    declares the whole file to be the unit.

    Nothing about WHERE a tag may sit changes. `scan_go_tags` places tags inline at
    the case on purpose, because a function-level-only tag stays green after the one
    enforcing case is deleted. Naming the enclosing function in a rendered citation
    does not move the tag, so deleting that case still deletes the tag and still
    drops the coverage.
    """
    content = _read_source(t.file, root, cache)
...
def _tag_sites(found: Sequence[Tag], polarity: str, root: str, cache: Dict[str, str]) -> str:
    ...
    reader is told which unit enforces the requirement, and the count of inline cases
    inside it was never a fact the page could keep current anyway.
    """
    seen: Dict[str, None] = {}
    for t in found:
...
def render_shards(
    ...
    for rfc in shard_stems(requirements):
        ...
        out.append("")
        # No line numbers here, deliberately. A citation names the file and the enclosing
        # function, which is what `ai/rules/writing.md` asks for and what survives an edit
        # above the tag. `--locate <id>` computes the lines from a fresh scan when somebody
        # actually needs to jump to one.
        out.append(
            ...
            f"`RFC requirement:` tags in the tests themselves. Run "
            f"`python3 scripts/dev/rfc_requirements.py --locate <requirement-id>` for the "
            f"exact lines. The index over every RFC is `ai/RFC-REQUIREMENTS.md`."
        )
        out.append("")
        ...
        for r in sorted(reqs, key=lambda r: r.rid):
            ...
            pos = _tag_sites(found, "positive", PROJECT_DIR, source_cache)
            neg = _tag_sites(found, "negative", PROJECT_DIR, source_cache)
            marks: List[str] = []
...
def run_locate(rid: str) -> int:
    """Print every tagged site for ONE requirement id, with its line, computed now.

    This is the half that makes the shards' silence about lines affordable. The line was
    never a fact worth STORING: it is derived from a scan, it is correct only for the tree
    it was scanned in, and a page that keeps it must be rewritten whenever anything above a
    tag moves. Computing it on demand costs one scan and is right by construction.

    Exit 1 for an id no tag carries, which is a real answer rather than an error: an
    enrolled requirement with no enforcing test is exactly what `--check` reports as
    uncovered, and printing nothing with exit 0 here would look like success.
    """
    tags = [t for t in scan_tree() if t.rid == rid]
    if not tags:
        print(f"{rid}: no `RFC requirement:` tag carries this id")
        return 1
    cache: Dict[str, str] = {}
...
def main(argv: Sequence[str]) -> int:
    ...
    if "--locate" in args:
        # Takes its value by index, the way --show and --extract-skeleton do.
        i = args.index("--locate")
        rid = args[i + 1] if i + 1 < len(args) else ""
        if not rid or rid.startswith("-"):
            print(
                f"{RED}{BOLD}rfc-requirements: cannot run{RESET}: --locate needs a "
                f"requirement id, e.g. --locate RFC7606-7.1-1"
            )
            return 2
        return run_locate(rid)
    if "--show" in args:
