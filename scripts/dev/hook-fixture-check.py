...
def run_test_first(results: Results) -> None:
    ...
    try:
        ...
        results.check('test-first-untested-package-blocks', r is not None and r[0] == 2, repr(r))
...
def run_rendered_rule(results: Results) -> None:
    ...
    code, err = _writeedit(
        os.path.join(rules, 'points', 'performance', 'hot-path-rule', 'some-directive.md')
    )
    ...
    code, err = _writeedit(os.path.join(ROOT, 'docs', 'contributing', 'writing-style.md'))
    ...
    results.check('point-overwrite-section-point-edit-allowed', code == 0, repr((code, err)))
    ...
    code, err = _writeedit(
        os.path.join(rules, 'points', 'performance', 'hot-path-rule', 'no-such-slug-exists.md'),
        tool='Write',
    )
    ...
    results.check('point-overwrite-other-checkout-allowed', code == 0, repr((code, err)))
    ...
    code, err = _writeedit(
        os.path.join(rules, 'points', 'performance', 'hot-path-rule', 'nested', 'manifest.md'),
        tool='Write',
    )
    ...
    point_rel = (
        'ai/rules/points/performance/hot-path-rule/apply-the-hot-path-ban-to-these-packages.md'
    )
    ...
    code, err = _multiedit(point, [{'old_string': 'the hot path', 'new_string': 'the hot path'}])
    results.check('point-overwrite-multiedit-targeted-allowed', code == 0, repr((code, err)))
...
def _run_validate_spec(script: str, spec_text: str, *, argv=None, payload=_UNSET, sources=None):
    ...
    try:
        ...
        with open(os.path.join(work, 'ai', 'CODE-TO-DOCS.md'), 'w', encoding='utf-8') as fh:
            fh.write(
...
def run_validate_spec(results: Results) -> None:
    ...
    malformed = _VALID_SPEC.replace('@ARROW@', '->').replace('## Data Flow', '## Not Data Flow')
    ...
    rc, err = _run_validate_spec(script, base.replace(_CB, '## Current Behavior\n\n- [ ] `.go`'))
    results.check(
...
def run_commit_gate(results: Results) -> None:
    ...
    try:
        ...
        results.check('commit-gate-deferral-in-diff-logged-ok', not problems, repr(problems))
    finally:
        shutil.rmtree(repo, ignore_errors=True)
    ...
    try:
        ...
        problems = ch.deferral_in_diff_problems(Path(repo), ('scripts/dev/patterns.py',), ())
        results.check(
    ...
    try:
        ...
        problems = ch.spec_audit_problems(Path(repo), ('plan/learned/099-fixture.md',), '')
        results.check('commit-gate-spec-audit-no-claim-skips', not problems, repr(problems))
        ...
        _write(repo, 'plan/spec-fixture.md', empty_pcv)
        problems = ch.spec_audit_problems(Path(repo), ('internal/x/y.go',), 'spec-fixture.md')
        results.check('commit-gate-spec-audit-non-closure-skips', not problems, repr(problems))
    finally:
        shutil.rmtree(repo, ignore_errors=True)
    ...
    try:
        ...
        results.check('commit-gate-spec-audit-journal-row-blocks', bool(problems), repr(problems))

        _write(repo, 'plan/spec-fixture.md', filled_pcv)
        problems = ch.spec_audit_problems(
            Path(repo), ('plan/journal/a-class.md',), 'spec-fixture.md'
        )
        results.check('commit-gate-spec-audit-journal-filled-ok', not problems, repr(problems))
    ...
    try:
        ...
        script_exists = bool(glob.glob(os.path.join(repo, 'tmp', 'commit-cafe1234-*.sh')))
        results.check(
        ...
        script_exists = bool(glob.glob(os.path.join(repo, 'tmp', 'commit-cafe9abc-*.sh')))
        results.check(
    ...
    try:
        ...
        script_exists = bool(glob.glob(os.path.join(repo, 'tmp', 'commit-abcd1234-*.sh')))
        results.check(
    ...
    try:
        ...
        results.check('commit-gate-index-unrelated-commit-passes', not problems, repr(problems))
        ...
        results.check('commit-gate-index-foreign-staleness-passes', not problems, repr(problems))
    ...
    try:
        ...
        results.check(
            ...
            not ch.test_coverage_problems(Path(repo), ('internal/alpha/a.go', 'test/bgp/thing.ci')),
    ...
    try:
        ...
        results.check(
            ...
            'c_os_exit' in ''.join(ch.go_style_problems(Path(repo), ('internal/alpha/x.go',))),
        ...
        results.check(
            ...
            'c_os_exit'
            in ''.join(ch.go_style_problems(Path(repo), ('internal/alpha/vendoring.go',))),
        ...
        results.check(
            ...
            'c_os_exit'
            in ''.join(ch.commit_gate_problems(Path(repo), ('internal/alpha/x.go',), ())),
    ...
    try:
        ...
        _regen_package_map(repo)
        with contextlib.redirect_stderr(io.StringIO()):
            problems = ch.discovery_index_problems(Path(repo), ('internal/newpkg/thing.go',))
        results.check(
    ...
    try:
        ...
        with contextlib.redirect_stderr(io.StringIO()):
            problems = ch.discovery_index_problems(Path(repo), (), ('internal/beta/b.go',))
        results.check('commit-gate-index-removal-stale-blocks', bool(problems), repr(problems))
        ...
        results.check('commit-gate-index-removal-regenerated-passes', not problems, repr(problems))
    finally:
        shutil.rmtree(repo, ignore_errors=True)
    ...
    try:
        ...
        script_exists = bool(glob.glob(os.path.join(repo, 'tmp', 'commit-beef1234-*.sh')))
        results.check(
...
def _run_session_id_mint(results: Results) -> None:
    ...
    try:
        ...
        results.check(
            ...
            c1 == c2 and _UUID_RE.match(c1) is not None and c1 != 'claude-session-fallback',
            f'c1={c1!r} c2={c2!r}',
        )
    finally:
...
def _run_hook_payload_status(results: Results) -> None:
    ...
    for name, payload, expected_status, expected_stdout in (
        ...
        results.check(
            name,
            proc.returncode == expected_status and proc.stdout.strip() == expected_stdout,
...
def _run_subagent_parent_payload_context(results: Results) -> None:
    ...
    try:
        ...
        parent_scratch = os.path.relpath(os.path.join(os.path.dirname(state_dir), 'scratch'), work)
        env = _fork_payload_env(work)
        proc = subprocess.run(
...
def _run_subagent_absent_parent_payload(results: Results) -> None:
    ...
    try:
        ...
        parent_ids = [
            line.removeprefix(parent_prefix) for line in lines if line.startswith(parent_prefix)
        ]
        ...
        results.check(
            ...
            f'rc={proc.returncode} additionalContext={additional!r} err={proc.stderr!r}',
        )
    finally:
        shutil.rmtree(work, ignore_errors=True)
    ...
    try:
        ...
        for label, bad in (
            ...
            results.check(
                ...
                f'rc={malformed.returncode} updated={malformed_update!r} err={malformed.stderr!r}',
            )
...
def _run_fork_parent_session_id(results: Results) -> None:
    ...
    try:
        ...
        results.check(
            ...
            env_probe.returncode == 0 and env_probe.stdout.splitlines() == [parent_sid, 'unset'],
        ...
        results.check(
            ...
            f'rc={review_path.returncode} out={review_path.stdout!r} err={review_path.stderr!r}',
        )
        ...
        results.check(
            ...
            and scratch_one.stdout.strip() == scratch_two.stdout.strip() == expected_scratch,
...
def _run_fork_tool_session_id(results: Results) -> None:
    ...
    try:
        ...
        results.check(
            ...
            f'rc={canonical.returncode} out={canonical.stdout!r} err={canonical.stderr!r}',
        )
        ...
        results.check(
            ...
            f'rc={human_make.returncode} out={human_make.stdout!r} err={human_make.stderr!r}',
        )
        ...
        results.check(
            ...
            f'rc={human_review.returncode} out={human_review.stdout!r} err={human_review.stderr!r}',
        )
    finally:
        shutil.rmtree(work, ignore_errors=True)
...
def run_session_id(results: Results) -> None:
    ...
    results.check('session-id-commit-helper-walk-removed', not psfield, '; '.join(psfield))
    argv_files = {
        ln.split(':', 1)[0] for ln in _grep_lines('_session_id_from_argv', '.claude/hooks')
    }
    results.check(
...
def _run_rfc_test_guard(results: Results, mod) -> None:
    ...
    results.check('rfc-guard-marker-no-longer-approves', r is not None and r[0] == 2, repr(r))
    ...
    r = scoped(tagged, '', go)
    results.check('rfc-guard-import-exemption-spares-no-test-delete', r is not None, repr(r))
...
def _rfc_guard_scope_cases(results: Results, cw, tmp: str) -> None:
    ...
    results.check('rfc-guard-blocks-body-edit-tag-outside-hunk', blocked_on_rfc(r), repr(r))
    ...
    results.check('rfc-guard-replace-all-reaches-tagged-copy', blocked_on_rfc(r), repr(r))
    ...
    results.check('relax-ci-tautology-swap-blocked', r is not None and r[0] == 2, repr(r))
    ...
    results.check('relax-go-tautology-swap-blocked', r is not None and r[0] == 2, repr(r))
    ...
    results.check('relax-ci-emptied-needle-blocked', r is not None and r[0] == 2, repr(r))
    ...
    results.check('rfc-guard-one-line-func-neighbour-still-blocks', blocked_on_rfc(r), repr(r))
...
def _weakened_hatch_cases(results: Results, mod, work: str) -> None:
    ...
    def contract(*rows: str) -> None:
        with open(os.path.join(work, 'test', 'weakened.md'), 'w', encoding='utf-8') as fh:
            fh.write(
...
def _rfc_changed_ledger_cases(results: Results, mod, work: str) -> None:
    ...
    r = edit(body, swap)
    results.check('rfc-changed-wrong-package-qualifier-buys-nothing', blocked_on_rfc(r), repr(r))
...
def run_draft_incubator(results: Results) -> None:
    ...
    results.check('draft-edit-rfc-tagged-passes', edit(draft) is None, repr(edit(draft)))
    ...
    results.check('live-go-test-rm-still-needs-approval', r is not None and r[0] == 2, repr(r))

    r = deletion('rm test/draft/a.ci internal/x/y_test.go', None)
    results.check('mixed-go-test-rm-still-needs-approval', r is not None and r[0] == 2, repr(r))
    ...
    results.check('mixed-recursive-rm-still-needs-approval', r is not None and r[0] == 2, repr(r))

    r = deletion('git rm test/draft/a.ci test/plugin/live', None)
    results.check('mixed-git-rm-still-needs-approval', r is not None and r[0] == 2, repr(r))

    # A path is matched by where it lands, not by the prefix it is spelled with.
    r = deletion('rm test/draft/../plugin/live.ci', None)
    results.check('draft-traversal-rm-still-needs-approval', r is not None and r[0] == 2, repr(r))
...
def run_design_gate(results: Results) -> None:
    ...
    results.check('design-gate-hooks-spec-reads-its-hook', not _design_blocked(r), repr(r))
    ...
    results.check('design-gate-yang-spec-reads-its-model', not _design_blocked(r), repr(r))

    r = _design_case(
        '- `scripts/dev/commit_helper.py` - the helper',
        ('/repo/scripts/dev/commit_helper.py',),
    )
    results.check('design-gate-tooling-spec-reads-its-tool', not _design_blocked(r), repr(r))
    ...
    results.check('design-gate-daemon-spec-uninvestigated-blocked', _design_blocked(r), repr(r))
    ...
    results.check('design-gate-daemon-spec-grounded-by-hook-blocked', _design_blocked(r), repr(r))
    ...
    results.check('design-gate-daemon-spec-reads-its-go', not _design_blocked(r), repr(r))
    ...
    results.check('design-gate-subject-read-showed-nothing-blocked', _design_blocked(r), repr(r))
    ...
    results.check('design-gate-subjectless-spec-any-source', not _design_blocked(r), repr(r))

    r = _design_case(
        '- `docs/guide/x.md` - the page',  # <!-- doc-links: ignore (fixture literal in a hook test corpus, deliberately absent from the tree) -->
        (),
    )
    results.check('design-gate-subjectless-spec-nothing-blocked', _design_blocked(r), repr(r))
    ...
    try:
        ...
        results.check('design-gate-lsp-grounds-go-spec', not _design_blocked(r), repr(r))
    finally:
        shutil.rmtree(work, ignore_errors=True)
    ...
    results.check('design-gate-multi-kind-needs-every-kind', _design_blocked(r), repr(r))
    ...
    results.check('design-gate-multi-kind-both-read-allowed', not _design_blocked(r), repr(r))
    ...
    results.check('design-gate-checklist-row-is-not-a-subject', _design_blocked(r), repr(r))
    ...
    results.check('design-gate-files-to-create-is-a-subject', _design_blocked(r), repr(r))
    ...
    results.check('design-gate-checklist-adds-no-requirement', not _design_blocked(r), repr(r))
    ...
    results.check('design-gate-new-spec-subject-from-payload', _design_blocked(r), repr(r))
    ...
    for path in (
        ...
        supplied = {
            name.split('-')[2]
            for name in written
            if name.startswith('.source-read-') and name != f'.source-read-{_DESIGN_SID}'
        }
        demanded = mod._spec_subject_kinds('## Files to Modify\n\n- `%s` - the file\n' % path)
        results.check(
    ...
    results.check('design-gate-first-cell-is-still-a-subject', _design_blocked(r), repr(r))
    ...
    work = _mark_project()
    try:
        _read_source(work, '/repo/internal/x/y.go', {'file': {'numLines': 1, 'totalLines': 900}})
        r = _write_spec(
            work,
            '- `internal/x/y.go` - the daemon',  # <!-- doc-links: ignore (fixture literal in a hook test corpus, deliberately absent from the tree) -->
        )  # <!-- doc-links: ignore (fixture path in a hook case, deliberately absent) -->
        results.check('design-gate-keyhole-read-does-not-ground', _design_blocked(r), repr(r))
    finally:
        shutil.rmtree(work, ignore_errors=True)
...
def _deleg_project(spec: str | None, status: str = 'ready', spawned: bool = False):
    ...
    if spec:
        ...
        with open(os.path.join(work, 'tmp', 'session', f'.session-{_DELEG_SID}'), 'w') as fh:
            fh.write(spec + '\n')
    if spawned:
        with open(os.path.join(work, 'tmp', 'session', f'.agent-spawned-{_DELEG_SID}'), 'w') as fh:
...
def _deleg_state_dir(work: str) -> str:
    ...
    found = sorted(
        d for d in glob.glob(os.path.join(root, f'????-??-??-{_DELEG_SID}')) if os.path.isdir(d)
    )
    session = (
        found[0]
        if found
...
def _run_stop_hook(work: str, message: str | None = None) -> tuple[int, str]:
    ...
    payload = json.dumps(
        {'last_assistant_message': message or 'Implemented the change and ran the tests.'}
    )
    r = subprocess.run(
...
def run_delegation(results: Results) -> None:
    ...
    try:
        ...
        results.check(
            ...
            os.path.isfile(os.path.join(work, 'tmp', 'session', f'.agent-spawned-{_DELEG_SID}')),
    ...
    try:
        rc, err = _run_stop_hook(work)
        results.check('delegation-nudge-silent-when-spawned', 'Delegation:' not in err, err)
    ...
    try:
        ...
        stray = (
            'I fixed the ` escaping bug. Would you like me to also run `make ze-precommit-verify`?'
        )
    ...
    try:
        ...
        results.check(
            ...
            and os.path.isfile(
                os.path.join(_deleg_state_dir(work), f'session-state-fixture-{_DELEG_SID}.md')
            ),
...
def _state_project(body: str) -> tuple[str, str]:
    ...
    with open(os.path.join(work, 'dirty.txt'), 'w') as fh:
        fh.write('uncommitted\n')
    state = os.path.join(_deleg_state_dir(work), f'session-state-fixture-{_DELEG_SID}.md')
...
def run_session_state_location(results: Results) -> None:
    ...
    try:
        ...
        nested = glob.glob(os.path.join(work, 'tmp', 'session', '*', 'state', 'session-state-*.md'))
        results.check(
    ...
    work = _deleg_project(spec='spec-fixture.md')
    try:
        os.makedirs(os.path.join(work, 'tmp', 'session', f'2026-01-02-{_DELEG_SID}', 'state'))
        r = _run_state_lib(work, '_state_file')
        results.check(
    ...
    try:
        ...
        nested = _plant(
            os.path.join(sess, '2026-08-01-oldsession', 'state', 'session-state-fixture-old2.md'),
            '# nested digest\n',
            age_hours=1,
        )
        ...
        results.check(
            ...
            r.returncode == 0 and r.stdout.strip().endswith('session-state-fixture-old2.md'),
    ...
    try:
        ...
        results.check(
            ...
            r.returncode == 0 and r.stdout.strip().endswith('session-state-fixture-oldsession.md'),
    ...
    try:
        ...
        strays = [os.path.basename(p) for p in glob.glob(os.path.join(sess, '*', 'state', '.*'))]
        results.check(
...
def run_delegation_reminder(results: Results) -> None:
    ...
    results.check('delegation-reminder-exits-zero', r.returncode == 0, f'rc={r.returncode}')
...
def run_phase_gates(results: Results) -> None:
    ...
    for name, prompt in (
        ...
        r = spawn(prompt)
        results.check(f'agent-style-guide-quiet-{name}', r.returncode == 0, repr(r.stderr))
    ...
    with tempfile.TemporaryDirectory() as home:
        ...
        results.check('review-model-blocks-spawn-off-opus-5', r.returncode == 2, repr(r.stderr))
        results.check(
        ...
        results.check('review-model-allows-spawn-on-opus-5', r.returncode == 0, repr(r.stderr))
        ...
        probe_src = (
            ...
            "print(rm.running_model() or 'unknown')" % os.path.join(ROOT, 'scripts', 'dev')
        )
        r = subprocess.run(
        ...
        results.check('review-model-unknown-stands-down', r.returncode == 0, repr(r.stderr))
...
def run_rfc_language(results: Results) -> None:
    ...
    results.check('rfc-language-write-with-keyword-allowed', code == 0, repr((code, err)))
    ...
    for kind in ('note', 'table'):
        ...
        results.check(f'rfc-language-write-{kind}-allowed', code == 0, repr((code, err)))
    ...
    code, err = _writeedit(directive, tool='Edit', content='- The error reads `it should exist`.')
    results.check('rfc-language-edit-modal-in-code-span-allowed', code == 0, repr((code, err)))
    ...
    results.check('rfc-language-edit-without-keyword-allowed', code == 0, repr((code, err)))
    ...
    code, err = _writeedit(directive, tool='Edit', content='- A must-fix defect MUST be fixed.')
    results.check('rfc-language-edit-hyphenated-compound-allowed', code == 0, repr((code, err)))
...
def run_raw_job_admission(results: Results) -> None:
    ...
    r = refused(f'ZE_ADMIT_RAW="bisecting one 2s case" {go_test} -run TestX ./internal/core/env')
    ...
    results.check('an-empty-reason-does-not-admit', r is not None and r[0] == 2, repr(r))
...
def run_governed_doc_edit(results: Results) -> None:
    ...
    results.check('governed-bare-newline-passes', not blocked(split), split.replace('\n', '\\n'))
...
def run_journal_row_shape(results: Results) -> None:
    ...
    try:
        ...
        r = call(
            write(
                'spec-cell.md',
                header + '| 2026-08-22 | walked into during closure | hooks | s | f |\n',
            )
        )
        results.check(
...
def run_ci_sleep_marker(results: Results) -> None:
    """`c_ci_sleep_justification` (pretool-writeedit.py) after it began BLOCKING.

    It used to accept ANY `#` near the sleep, which `# settle` satisfied while
    teaching a reader nothing, so it could only warn: an Edit fragment that did
    not carry the comment already above a sleep would have been refused falsely.
    A `# sleep(<kind>): <reason>` marker is written WITH the sleep, in the same
    fragment, so its absence is the author's omission and the block is safe.

    What is pinned: a bare sleep and a vague comment are both refused, the kind
    is a CLOSED set so an invented one is refused, a marker with no reason after
    the colon is refused, both placements are accepted, and the two shapes that
    are not a sleep at all stay untouched.
    """
    print('ci-sleep-marker:')
    mod = _load_pretool_writeedit()
    ci = os.path.join(os.getcwd(), 'test', 'plugin', 'fixture.ci')

    def verdict(body: str) -> str:
        out = mod.c_ci_sleep_justification({'fp': ci, 'tool': 'Write', 'ti': {'content': body}})
        return 'block' if out and out[0] == 2 else 'pass'

    for label, body, want in (
        ('bare-sleep', 'time.sleep(2)', 'block'),
        ('vague-comment', '# settle\ntime.sleep(2)', 'block'),
        ('invented-kind', '# sleep(whenever): because\ntime.sleep(2)', 'block'),
        ('empty-reason', '# sleep(timer):\ntime.sleep(2)', 'block'),
        ('marker-above', '# sleep(timer): tracker ticks once a second\ntime.sleep(2)', 'pass'),
        ('marker-trailing', 'time.sleep(2)  # sleep(no-signal): no readback exists', 'pass'),
        (
            'poll-interval',
            '# sleep(poll-interval): loop breaks on the nft table\ntime.sleep(0.2)',
            'pass',
        ),
        ('commented-out-sleep', '# time.sleep(2)', 'pass'),
        ('no-sleep', "print('hi')", 'pass'),
    ):
        got = verdict(body)
        results.check(f'ci-sleep-{label}-{want}', got == want, f'got {got}')

    # The guard is scoped to .ci under test/. A sleep in a Python tool is a
    # different population with a different answer, and this hook must not
    # reach it (scripts/dev/audit-test-relaxation.py owns that decision).
    out = mod.c_ci_sleep_justification(
        {
            'fp': os.path.join(os.getcwd(), 'scripts', 'dev', 'tool.py'),
            'tool': 'Write',
            'ti': {'content': 'time.sleep(2)'},
        }
    )
    results.check('ci-sleep-non-ci-path-untouched', out is None, repr(out))
...
def run_python_weakening_arms(results: Results) -> None:
    ...
    def skipped(decorator: str) -> str:
        return base.replace('    def test_x(self):', f'    {decorator}\n    def test_x(self):')
    ...
    errs, soft = errs_for(scenario, 'def check(api):\n    pass\n', check_py)
    results.check('py-arm-tagged-check-py-no-assert-notice', (errs, soft) == ([], []), repr(soft))
...
SECTIONS = {
    ...
    'ci-sleep-marker': run_ci_sleep_marker,
}
