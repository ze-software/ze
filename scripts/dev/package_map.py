...
# `letools` is le's plugin tree. It sits at top level rather than under
# internal/ because le is a different program over the same engine, and no
# build links its packages with ze's. A tree missing from here is a tree no
# agent finds by reading the map.
ROOTS = ("internal", "pkg", "cmd", "letools")
...
def render(packages: dict[str, tuple[str, str]]) -> str:
    ...
    lines = [
        ...
        "",
        "One line per Go package under `internal/`, `pkg/`, `cmd/`, `letools/`.",
