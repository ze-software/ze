...
`make ze-session-clean BEFORE=<date>` (driven in session_bin_dir_test.py).
...
class TestCleanTmpPreservesSessionRoot(unittest.TestCase):
    """AC-10: `make ze-scratch-clean` never reaches a live session's directory.
    ...
    def test_a_live_session_survives_the_sweep(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                ...
                "ze-scratch-clean reached a session's binaries",
            )
            self.assertTrue(
                (flat / ".session-sid-LIVE").is_file(),
                "ze-scratch-clean reached a session's claim marker",
            )
    ...
    def test_a_stale_tmp_root_is_not_swept_as_its_own_child(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(tmp_root.is_dir(), "ze-scratch-clean removed the tmp/ root")
            self.assertTrue(
                (session / "bin" / "ze").is_file(),
                "ze-scratch-clean took a session's binaries with the tmp/ root",
            )
            self.assertTrue(
                (session / "state" / "session-state-spec-x-sid-LIVE.md").is_file(),
                "ze-scratch-clean took a session's spec digest with the tmp/ root",
            )
