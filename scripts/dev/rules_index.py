...
def build(rules_dir):
    ...
    for name, title, summary, severity in rows:
        lines.append(
            f"| {title} | {summary} | {severity} | `ai/rules/{name}` |"  # <!-- doc-links: ignore (f-string template; {name} is substituted per row) -->
        )
    lines.append("")
    return "\n".join(lines), missing
...
def main():
    ...
    if check_mode:
        ...
        if current != content:
            problems.append(
                f"{output_file.relative_to(root)} is stale -- run: make ze-rules-index-update"
            )
        if problems:
            for p in problems:
                print(f"WARNING: {p}")
