...
Changing `reference-bandwidth` costs no adjacency. Ze re-prices each interface in place and originates its Router-LSA at the commit, so no neighbor goes down and no database exchange runs. Raise the leaf across a router with forty auto-costed links and all forty adjacencies stay up. Two reloads do restart an interface, because both change what it stamps into the Hellos it sends: a new router id, and an area type that changes the E-bit or the N-bit.
...
<!-- source: internal/plugins/ospf/instance.go -- interfaceRuntimeConfigLocked, lsdbTopology, interfaceGlobalParamsChanged, repriceInterfaceLocked -->
