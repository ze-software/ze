...
// cmdConfigFmtCLI is the CLI entry point for "ze bgp config fmt".
func cmdConfigFmtCLI(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `Usage: ze bgp config fmt [options] <config-file>

Format and normalize configuration file.

Formats current config files only. For old ExaBGP configs, run "ze bgp config migrate" first.
...
Examples:
  ze bgp config fmt config.conf          # Print formatted config to stdout
  ze bgp config fmt -w config.conf       # Write back to file
  ze bgp config fmt --check config.conf  # Check if formatting needed (for CI)
  ze bgp config fmt --diff config.conf   # Show what would change
  ze bgp config fmt -                    # Read from stdin, write to stdout
`)
	}
