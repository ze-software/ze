...
// cmdConfigCheckCLI is the CLI entry point for "ze bgp config check".
func cmdConfigCheckCLI(args []string) int {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, `Usage: ze bgp config check [options] [config-file]
...
func outputCheckText(result checkResult) int {
	if !result.needsMigration {
		...
		fmt.Println("  ze bgp config migrate <file> -o <output>")
		fmt.Println("  ze bgp config migrate <file> --in-place")
	}
