...
var _ = register("exabgp", "Run ExaBGP compatibility tests (test/exabgp-compat/encoding/*.ci)", exabgpCmd)

const exabgpSuiteEncoding = "encoding"

var (
	errExaBGPPortTimeout = errors.New("timed out waiting for ExaBGP mock BGP port")
	errExaBGPMissingPort = errors.New("ExaBGP mock BGP server did not report a port")
)
...
type exabgpSuite struct {
	...
	rootDir string
	tests   *runner.Tests
	byNick  map[string]*exabgpCompatTest
}

func exabgpCmd() int {
	if err := exabgpMain(); err != nil {
...
func exabgpMain() error {
	...
	if suiteName != exabgpSuiteEncoding {
		return errors.New("only ExaBGP encoding compatibility tests are available")
	}
	...
	if suite.tests.Count() == 0 {
		return errors.New("no ExaBGP compatibility tests found")
	}
	suite.tests.Sort()
...
Run ExaBGP compatibility encoding tests using Ze's standard test selection and progress output.
...
func discoverExaBGPSuite(baseDir string) (*exabgpSuite, error) {
	root := filepath.Join(baseDir, "test", "exabgp-compat")
	...
	suite := &exabgpSuite{
		...
		rootDir: root,
		tests:   tests,
		byNick:  make(map[string]*exabgpCompatTest, len(files)),
	}
...
func parseExaBGPCI(root string, rec *runner.Record, ciFile string) (*exabgpCompatTest, error) {
	file, err := os.Open(ciFile) //nolint:gosec // ciFile comes from the discovered test/exabgp-compat fixture set.
	if err != nil {
		return nil, err
	}
	defer func() { _ = file.Close() }()

	test := &exabgpCompatTest{
		record:         rec,
		ciFile:         ciFile,
		tcpConnections: 1,
	}
	...
	if len(test.configs) == 0 {
		return nil, errors.New("ExaBGP compatibility test has no option=file: " + ciFile)
	}
	return test, nil
}
...
func runOneExaBGPTest(ctx context.Context, test *exabgpCompatTest, cli exabgpCLI) (bool, exabgpRunDetail) {
	rec := test.record
...
func printExaBGPFailure(test *exabgpCompatTest, detail exabgpRunDetail) {
...
func printExaBGPOutput(_ *exabgpCompatTest, detail exabgpRunDetail) {
	if detail.port > 0 {
...
func runExaBGPServerForeground(test *exabgpCompatTest, port int, saveDir string) error {
	if port < 0 {
		return errors.New("--port must be >= 0")
	}
	python := pythonInterpreter()
	args := exaBGPServerArgs(test, port, saveDir)
	cmd := exec.CommandContext(context.Background(), python, args...) //nolint:gosec // python and args target repository-owned ExaBGP compatibility fixtures.
...
func runExaBGPClientForeground(test *exabgpCompatTest, port int) error {
...
func startExaBGPServer(ctx context.Context, test *exabgpCompatTest, port int, saveDir string) (*exaProcess, <-chan int, error) {
	python := pythonInterpreter()
	args := exaBGPServerArgs(test, port, saveDir)
...
func startExaBGPClient(ctx context.Context, test *exabgpCompatTest, port int) (*exaProcess, error) {
	return startExaProcess(ctx, "client", exaBGPWrapperPath(test), exaBGPClientArgs(test), exaBGPClientEnv(test, port), nil)
}

func exaBGPServerArgs(test *exabgpCompatTest, port int, saveDir string) []string {
...
func exaBGPClientArgs(test *exabgpCompatTest) []string {
...
func exaBGPServerEnv(test *exabgpCompatTest, port int) []string {
	env := os.Environ()
	env = append(env,
...
func exaBGPClientEnv(test *exabgpCompatTest, port int) []string {
	env := os.Environ()
...
func exaBGPWrapperPath(test *exabgpCompatTest) string {
	return filepath.Join(filepath.Dir(filepath.Dir(test.ciFile)), "bin", "exabgp")
}

func bgpMockPath(test *exabgpCompatTest) string {
