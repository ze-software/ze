...
var defaultWeights = []actionWeight{
	...
	{ActionPartialWithdraw, 15},
	{ActionFullWithdraw, 5},
	{ActionDisconnectDuringBurst, 5},
	{ActionReconnectStorm, 5},
	{ActionConnectionCollision, 5},
	{ActionMalformedUpdate, 5},
...
// Tick checks whether a chaos event should fire at the given time.
// It returns zero or one scheduled actions. The established slice
// indicates which peers are currently in Established state.
func (s *Scheduler) Tick(now time.Time, established []bool) []ScheduledAction {
	...
	// Select action type by weighted random.
	action := s.selectAction()

	// Select target peer.
	peerIdx := candidates[s.rng.Intn(len(candidates))]
...
func (s *Scheduler) selectAction() ChaosAction {
	...
	for _, w := range defaultWeights {
		cumulative += w.weight
		if roll < cumulative {
			action := ChaosAction{Type: w.action}
			if w.action == ActionPartialWithdraw {
